...
Функция СоздатьДокументыРТУНаДату(ДатаСоздания, ТекстОшибки = "")
	...
	|	СУММА(ПартииТоваровОбороты.КоличествоРасход) КАК Количество,
	|	СУММА(-ПартииТоваровОбороты.СуммаРублиОборот) КАК Сумма
	|ПОМЕСТИТЬ ТаблицаПродажПоРегистратору
	|ИЗ
	|	РегистрНакопления.ПартииТоваров.Обороты(&НачПериода, &КонПериода, Регистратор, Не Склад.Бонусный) КАК ПартииТоваровОбороты
	|ГДЕ
