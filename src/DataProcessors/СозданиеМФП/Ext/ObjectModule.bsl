...
Процедура  УстановитьГраницуЗапретаПоТекущемуПользователю()
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_УстановитьГраницуЗапретаПоТекущемуПользователю";
	...
	Запрос.Текст = 
		...
		|	И ГраницыЗапретаИзмененияДанных.Пользователь = &Пользователь
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Если Выборка.Следующий() Тогда
		 ГраницаЗапрета = Выборка.ГраницаЗапретаИзменений; 
	КонецЕсли;
	
КонецПроцедуры

//СОЗДАНИЕ
Процедура ВыполнитьСозданиеМФПАвтомат(СоздаватьРТУ = Истина, ЗаполнитьРегистрВозвратыМФП = Истина, СоздаватьДокументыВозврата = Истина) Экспорт
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ВыполнитьСозданиеМФПАвтомат";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	УстановитьГраницуЗапретаПоТекущемуПользователю();	
	
	Если Не ЗначениеЗаполнено(ГраницаЗапрета) Тогда
		ГраницаЗапрета = Дата(2018,12,31);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ГраницаМФП.ДатаСоздания КАК ДатаСоздания
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ДатаСоздания <=  ГраницаЗапрета Тогда
			РегистрыСведений.ГраницаМФП.УдалитьЗапись(Выборка.ДатаСоздания);
			Сообщить("Дата "+Выборка.ДатаСоздания +" пропущена. Граница запрета: "+ГраницаЗапрета);
			Продолжить;
		КонецЕсли;
		
		ПометитьНаУдалениеДокументы(Выборка.ДатаСоздания, СоздаватьРТУ, СоздаватьДокументыВозврата);
		
		ТекстОшибки = СоздатьДокументыНаДату(Выборка.ДатаСоздания,  СоздаватьРТУ, ЗаполнитьРегистрВозвратыМФП, СоздаватьДокументыВозврата);
	...
	//	Если мАвтомат Тогда
	//		
	//		Если ДатаНач < Дата(2019,1,1) Тогда //Временный костыль.
	//			РегистрыСведений.ГраницаМФП.СдвинутьГраницу(ДатаНач);
	//			Продолжить;
	//		КонецЕсли;
	//		
	//		ПометитьНаУдалениеДокументы(ДатаНач, СоздаватьРТУ, СоздаватьДокументыВозврата);
	//	КонецЕсли;
	//	
...
Процедура ВыполнитьСозданиеМФП(СоздаватьРТУ = Истина, ЗаполнитьРегистрВозвратыМФП = Истина, СоздаватьДокументыВозврата = Истина) Экспорт
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ВыполнитьСозданиеМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция СоздатьДокументыНаДату(ДатаСоздания, СоздаватьРТУ = Истина, ЗаполнитьРегистрВозвратыМФП = Истина, СоздаватьДокументыВозврата = Истина)
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_СоздатьДокументыНаДату";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	Если СоздаватьРТУ И ЗаполнитьРегистрВозвратыМФП И СоздаватьДокументыВозврата  Тогда //И ТекстОшибки = "" Тогда  //В случае ошибки все равно сдвинем
		РегистрыСведений.ГраницаМФП.УдалитьЗапись(ДатаСоздания);
	КонецЕсли;
...
Процедура ПометитьНаУдалениеДокументыЗаПериод(УдалятьРТУ = Истина, УдалятьВозвраты = Истина) Экспорт
	...
	ДатаНач = НачалоПериода;
	Пока ДатаНач <= ОкончаниеПериода Цикл
		ПометитьНаУдалениеДокументы(ДатаНач, УдалятьРТУ, УдалятьВозвраты);
...
Процедура ПометитьНаУдалениеДокументы(ДатаДокумента, УдалятьРТУ = Истина, УдалятьВозвраты = Истина) Экспорт
	...
	Запрос.Текст = 
		...
		|	ИЛИ РеализацияТоваровУслуг.ВидОперации = &ВидОперацииРТУКомиссия)
		...
		|	И (ПоступлениеТоваровУслуг.ВидОперацииПоступления = &ВидОперацииПТУ
		|	ИЛИ ПоступлениеТоваровУслуг.ВидОперацииПоступления = &ВидОперацииПТУКомиссия)
		...
		|	И ВозвратТоваровПоставщику.ВидОперации = &ВидОперацииВозвратПоставщику
		|	И НЕ ВозвратТоваровПоставщику.ПометкаУдаления
		|	И &УдалятьВозвраты";
	
	Запрос.УстановитьПараметр("ВидОперацииПТУ", 				Перечисления.ВидыПоступленияТоваров.МФП);
	Запрос.УстановитьПараметр("ВидОперацииРТУ", 				Перечисления.ВидыОперацийРеализацияТоваров.МФП);
	Запрос.УстановитьПараметр("ВидОперацииПТУКомиссия", 		Перечисления.ВидыПоступленияТоваров.МФПКомиссия);
	Запрос.УстановитьПараметр("ВидОперацииРТУКомиссия", 		Перечисления.ВидыОперацийРеализацияТоваров.МФПКомиссия);
	Запрос.УстановитьПараметр("ВидОперацииВозврат", 			Перечисления.ВидыОперацийВозвратаОтПокупателя.МФП);
	Запрос.УстановитьПараметр("ВидОперацииВозвратПоставщику", 	Перечисления.ВидыОперацийВозвратПоставщику.МФП);
...
Функция СоздатьДокументыРТУНаДату(ДатаСоздания, ТекстОшибки = "")
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_СоздатьДокументыРТУНаДату";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	ИспользуетсяКомиссия = ИспользуетсяКомиссияНаДату(ДатаСоздания);
	...
	Запрос.УстановитьПараметр("ОрганизацияПокупатель", ОрганизацияПокупательСписаниеПартий);
	...
	Запрос.УстановитьПараметр("ЗаменятьНДС", ДатаСоздания < Дата(2019,1,1));
	Запрос.УстановитьПараметр("ИспользуетсяКомиссия", ИспользуетсяКомиссия);
	...
	|	ПартииТоваровОбороты.Количество КАК Количество,
	|	ПартииТоваровОбороты.СуммаРубли КАК Сумма,
	|	ЕСТЬNULL(РеализацияТоваровУслугТовары.СтрокаЗаявки, ЕСТЬNULL(ВозвратТоваровПоставщикуТовары.СтрокаЗаявки, ЗНАЧЕНИЕ(Справочник.ИдентификаторыСтрокЗаявок.ПустаяСсылка))) КАК СтрокаЗаявки,
	|	ЕСТЬNULL(РеализацияТоваровУслугТовары.ЦенаСоСкидкой, ЕСТЬNULL(ВозвратТоваровПоставщикуТовары.Цена, 0)) КАК Цена
	|	Поместить ТаблицаПродажПоРегистратору
	|ИЗ
	|	РегистрНакопления.ПартииТоваров КАК ПартииТоваровОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
	|		ПО ПартииТоваровОбороты.Регистратор = РеализацияТоваровУслугТовары.Ссылка
	|			И ПартииТоваровОбороты.НомерСтрокиВДокументе = РеализацияТоваровУслугТовары.НомерСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщикуТовары
	|		ПО ПартииТоваровОбороты.Регистратор = ВозвратТоваровПоставщикуТовары.Ссылка
	|			И ПартииТоваровОбороты.НомерСтрокиВДокументе = ВозвратТоваровПоставщикуТовары.НомерСтроки
	|ГДЕ
	|	ПартииТоваровОбороты.Активность
	|	И ПартииТоваровОбороты.Период МЕЖДУ &НачПериода И &КонПериода
	|	И НЕ ПартииТоваровОбороты.ВнутреннееПеремещение
	|	И (ПартииТоваровОбороты.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	...
	|					КОНЕЦ)
	|	И ПартииТоваровОбороты.Количество > 0
	|	И ПартииТоваровОбороты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И НЕ ЕСТЬNULL(РеализацияТоваровУслугТовары.ЦенаСоСкидкой, ЕСТЬNULL(ВозвратТоваровПоставщикуТовары.Цена, 0)) = 0
	|	И Выбор когда ПартииТоваровОбороты.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг Тогда
	|		ВЫРАЗИТЬ(ПартииТоваровОбороты.Регистратор КАК Документ.РеализацияТоваровУслуг).СтатусДокумента = &СтатусОтгружен
	|	Иначе Истина Конец
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаПродажПоРегистратору.Номенклатура,
	|	ТаблицаПродажПоРегистратору.Качество,
	|	ТаблицаПродажПоРегистратору.СтрокаПрихода,
	|	ТаблицаПродажПоРегистратору.УчитыватьНДС,
	|	ТаблицаПродажПоРегистратору.ОрганизацияПродавец,
	|	ТаблицаПродажПоРегистратору.ОрганизацияПокупатель,
	|	СУММА(ТаблицаПродажПоРегистратору.Количество) КАК Количество,
	|	СУММА(ТаблицаПродажПоРегистратору.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ТаблицаПродаж
	|ИЗ
	|	ТаблицаПродажПоРегистратору КАК ТаблицаПродажПоРегистратору
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПродажПоРегистратору.Номенклатура,
	|	ТаблицаПродажПоРегистратору.Качество,
	|	ТаблицаПродажПоРегистратору.СтрокаПрихода,
	...
	|		ПО (НоменклатураБезЦен.Номенклатура = ПартииТоваров.Номенклатура)
	|ГДЕ
	|	ПартииТоваров.Период < &КонПериода
	...
	|   Где ПартииТоваров.ВидДвижения = Значение(ВидДвиженияНакопления.Приход)
	|        
	...
	|	&ИспользуетсяКомиссия И ТаблицаПолитик.ОрганизацияПокупатель <> ТаблицаПолитик.ОрганизацияПокупательСписаниеПартий КАК Комиссия,
	|	Выбор когда &ИспользуетсяКомиссия И ТаблицаПолитик.ОрганизацияПокупатель <> ТаблицаПолитик.ОрганизацияПокупательСписаниеПартий 
	|			Тогда  
	...
	|ПОМЕСТИТЬ ТаблицаПродажПоРегистраторуСПолитиками
	|ИЗ
	|	ТаблицаПродажПоРегистратору КАК ТаблицаПродаж
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаПродажПоРегистраторуСПолитиками.Номенклатура,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Качество,
	|	ТаблицаПродажПоРегистраторуСПолитиками.УчитыватьНДС,
	|	ТаблицаПродажПоРегистраторуСПолитиками.СтрокаПрихода,
	|	ТаблицаПродажПоРегистраторуСПолитиками.СтрокаЗаявки,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавец,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупатель,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавецПолитика,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупательПолитика,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавецСписаниеПартий,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупательСписаниеПартий,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Комиссия,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Комитент,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ДоговорКонтрагента,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Политика,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ПроцентНаценки,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Порядок,
	|	Сумма(ТаблицаПродажПоРегистраторуСПолитиками.Количество) КАК Количество,
	|	Сумма(ТаблицаПродажПоРегистраторуСПолитиками.Сумма) КАК Сумма	
	|ПОМЕСТИТЬ ТаблицаПродажСПолитиками
	|ИЗ
	|	ТаблицаПродажПоРегистраторуСПолитиками
	|Сгруппировать по 
	|	ТаблицаПродажПоРегистраторуСПолитиками.Номенклатура,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Качество,
	|	ТаблицаПродажПоРегистраторуСПолитиками.УчитыватьНДС,
	...
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавец,
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупатель,
	|	ТаблицаПродажПоРегистраторуСПолитиками.Регистратор,
	|	ТаблицаПродажПоРегистраторуСПолитиками.СтрокаПрихода
	|ИЗ
	|	ТаблицаПродажПоРегистраторуСПолитиками КАК ТаблицаПродажПоРегистраторуСПолитиками
	|   
	|ГДЕ
	|   НЕ ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавец = ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупатель
	|	И ТаблицаПродажПоРегистраторуСПолитиками.Политика ЕСТЬ NULL
	| Итоги по  
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПродавец, 
	|	ТаблицаПродажПоРегистраторуСПолитиками.ОрганизацияПокупатель,
	|   ТаблицаПродажПоРегистраторуСПолитиками.Регистратор
	...
	|	Выбор когда &ЗаменятьНДС = Истина И ТаблицаПродажСПолитиками.Номенклатура.СтавкаНДС = Значение(Перечисление.СтавкиНДС.НДС20)
	|		Тогда 
	|				Значение(Перечисление.СтавкиНДС.НДС18) 
	|		Иначе 
	|				ТаблицаПродажСПолитиками.Номенклатура.СтавкаНДС 
	|		Конец  КАК СтавкаНДС,
	|	ТаблицаПродажСПолитиками.Качество,
	|	ТаблицаПродажСПолитиками.СтрокаПрихода,
	|	ТаблицаПродажСПолитиками.СтрокаЗаявки,	
	|	ТаблицаПродажСПолитиками.СтрокаЗаявки.IDSite КАК IDSite,
	...
	|	ТаблицаПродажСПолитиками.Комиссия,
	|	ТаблицаПродажСПолитиками.Комитент,
	|	ВЫБОР
	...
	|   И (ТаблицаПродажСПолитиками.ОрганизацияПокупательПолитика = &ОрганизацияПокупатель
	|     ИЛИ &ВсеОрганизации)
	|ИТОГИ ПО
	|	ОрганизацияПродавец,
	...
	ВыборкаОрганизацияПродавец = Результаты[Результаты.Количество() - 2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Если ВыборкаОрганизацияПродавец.Количество() > 0 Тогда
	...
	Пока ВыборкаОрганизацияПродавец.Следующий() Цикл
		...
		Пока ВыборкаОрганизацияПокупатель.Следующий() Цикл
			...
			ДополнитьТекстОшибки(лТекстОшибки, "Продавец: "+ВыборкаОрганизацияПродавец.ОрганизацияПродавец+". Покупатель: "+ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель);
			
			ВыборкаРегистратор = ВыборкаОрганизацияПокупатель.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаРегистратор.Следующий() Цикл
				ДополнитьТекстОшибки(лТекстОшибки, "Документ: "+ВыборкаРегистратор.Регистратор);
				
				ВыборкаСтрокаПрихода = ВыборкаРегистратор.Выбрать();
				Пока ВыборкаСтрокаПрихода.Следующий() Цикл
					ДополнитьТекстОшибки(лТекстОшибки, "Строка прихода: "+ВыборкаСтрокаПрихода.СтрокаПрихода);
				КонецЦикла;
				
			КонецЦикла;
			
			ДополнитьТекстОшибки(лТекстОшибки,"========================================================="+Символы.ПС);
	...
	Пока ВыборкаОрганизацияПродавец.Следующий() Цикл
		...
		Пока ВыборкаОрганизацияПокупатель.Следующий() Цикл
			...
			Пока ВыборкаУчитыватьНДС.Следующий() Цикл
				...
				ВыборкаКомиссия = ВыборкаУчитыватьНДС.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				
				Пока ВыборкаКомиссия.Следующий() Цикл
					
					ВыборкаКомитент = ВыборкаКомиссия.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					...
					Пока ВыборкаКомитент.Следующий() Цикл
						...
						ВыборкаТовары = ВыборкаКомитент.Выбрать();
						
						ДокументОбъект = НайтиСоздатьДокументМФП(ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ДатаСоздания, ВыборкаУчитыватьНДС.УчитыватьНДС, ВыборкаКомиссия.Комиссия, ВыборкаКомитент.Комитент);
						ЗаполнитьШапкуДокументаМФП(ДокументОбъект, ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаСоздания, ВыборкаУчитыватьНДС.УчитыватьНДС, ВыборкаКомиссия.Комиссия, ВыборкаКомитент.Комитент);
						
						ДокументОбъект_ПТУ = НайтиСоздатьДокументПТУМФП(ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ДатаСоздания, ВыборкаУчитыватьНДС.УчитыватьНДС, ВыборкаКомиссия.Комиссия, ВыборкаКомитент.Комитент);
						ЗаполнитьШапкуДокументаПТУМФП(ДокументОбъект_ПТУ, ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаСоздания, ВыборкаУчитыватьНДС.УчитыватьНДС, ВыборкаКомиссия.Комиссия, ВыборкаКомитент.Комитент);
						
						ДокументОбъект.Товары.Очистить();
						ДокументОбъект_ПТУ.Товары.Очистить();
						Пока ВыборкаТовары.Следующий() Цикл
							...
							//ПТУ
							НоваяСтрока2 = ДокументОбъект_ПТУ.Товары.Добавить();
						...
						Если ДокументОбъект.Товары.Количество() > 0 Тогда
							...
							Попытка
								...
								//ПТУ
								ДокументОбъект_ПТУ.НомерВходящегоДокумента = ДокументОбъект.Номер;
								ДокументОбъект_ПТУ.ДатаВходящегоДокумента  = ДокументОбъект.Дата;
								
								СФ = УчетНдс.НайтиПодчиненныйСчетФактуру(ДокументОбъект.Ссылка);
								Если ЗначениеЗаполнено(СФ) Тогда
									ДокументОбъект_ПТУ.НомерВходящегоДокументаСФ = СФ.Номер;
									ДокументОбъект_ПТУ.ДатаВходящегоДокументаСФ  = СФ.Дата;
								КонецЕсли;
								
								ДокументОбъект_ПТУ.Записать(РежимЗаписиДокумента.Проведение);
								Сообщить("Проведен "+ДокументОбъект_ПТУ);
								
								РегистрыСведений.СоответствиеДокументовМФП.УстановитьСоответствие(ДокументОбъект.Ссылка, ДокументОбъект_ПТУ.Ссылка);
							...
							Исключение
								...
								Сообщить("Не удалось провести документы по причине: "+ОписаниеОшибки);
								...
								ДокументОбъект_ПТУ.Комментарий = ДокументОбъект;
								ДокументОбъект_ПТУ.Записать(?(ДокументОбъект_ПТУ.Проведен, 
								РежимЗаписиДокумента.ОтменаПроведения, 
								РежимЗаписиДокумента.Запись));
								Сообщить("Записан "+ДокументОбъект_ПТУ);
								
								РегистрыСведений.СоответствиеДокументовМФП.УстановитьСоответствие(ДокументОбъект.Ссылка, ДокументОбъект_ПТУ.Ссылка);
								
								ДополнитьТекстОшибки(лТекстОшибки, "Ошибка проведения "+ДокументОбъект+" / "+ ДокументОбъект_ПТУ + ": "+ОписаниеОшибки);
	...
	ОрганизацииБезПолитик = Неопределено;
	ВыборкаОрганизацияПродавец = Неопределено;
	ВыборкаОрганизацияПокупатель = Неопределено;
	ВыборкаУчитыватьНДС = Неопределено;
...
Функция НайтиСоздатьДокументМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ДатаДокумента, УчитыватьНДС, Комиссия = Ложь, Знач Комитент = Неопределено)
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_НайтиСоздатьДокументМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если Комитент = Неопределено Тогда
		Комитент = Справочники.Организации.ПустаяСсылка();
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	И РеализацияТоваровУслуг.Комитент = &Комитент
		|	И РеализацияТоваровУслуг.УчитыватьНДС = &УчитыватьНДС
		|	И НЕ ПОДСТРОКА(РеализацияТоваровУслуг.Номер, 1, 1) = &Номер";
	...
	Запрос.УстановитьПараметр("Номер", "D");
	Запрос.УстановитьПараметр("ВидОперации", ?(Комиссия, Перечисления.ВидыОперацийРеализацияТоваров.МФПКомиссия,Перечисления.ВидыОперацийРеализацияТоваров.МФП));
...
Процедура ЗаполнитьШапкуДокументаМФП(ДокументОбъект, ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаДокумента, УчитыватьНДС, Комиссия = Ложь, Знач Комитент = Неопределено)
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ЗаполнитьШапкуДокументаМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ДокументОбъект.ВидОперации 			= ?(Комиссия, Перечисления.ВидыОперацийРеализацияТоваров.МФПКомиссия, Перечисления.ВидыОперацийРеализацияТоваров.МФП);
	...
	ДокументОбъект.Комитент 			= Комитент;
	
	ДокументОбъект.Комментарий = ?(НЕ ДокументОбъект.УчитыватьНДС, "Без ндс", "");
	
	ДокументОбъект.ДоговорКонтрагента 	= ДоговорДляМФППоПравиламМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, "продажи");
...
Функция НайтиСоздатьДокументПТУМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ДатаДокумента, УчитыватьНДС, Комиссия = Ложь, Знач Комитент = Неопределено)
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_НайтиСоздатьДокументПТУМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	КонтрагентПродавец = Справочники.ПолитикиМФП.КонтрагентОрганизации(ОрганизацияПродавец, ДатаДокумента);
 
	Если Не ЗначениеЗаполнено(КонтрагентПродавец) Тогда
		ВызватьИсключение "Не удалось определить собственого контрагента для организации "+ОрганизацияПродавец+"! Проверьте политику МФП на "+ДатаДокумента;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НАЧАЛОПЕРИОДА(ПоступлениеТоваровУслуг.Дата, ДЕНЬ) = &Дата
		|	И ПоступлениеТоваровУслуг.ВидОперацииПоступления = &ВидОперации
		|	И ПоступлениеТоваровУслуг.Организация = &Организация
		|	И ПоступлениеТоваровУслуг.Контрагент = &Контрагент
		|	И ПоступлениеТоваровУслуг.УчитыватьНДС = &УчитыватьНДС
		|	И НЕ ПОДСТРОКА(ПоступлениеТоваровУслуг.Номер, 1, 1) = &Номер
		|	И ПоступлениеТоваровУслуг.Комитент = &Комитент";
	
	Запрос.УстановитьПараметр("Дата", ДатаДокумента);
	Запрос.УстановитьПараметр("Контрагент", КонтрагентПродавец);
	Запрос.УстановитьПараметр("Организация", ОрганизацияПокупатель);
	Запрос.УстановитьПараметр("УчитыватьНДС", УчитыватьНДС);
	Запрос.УстановитьПараметр("Номер", "D");
	Запрос.УстановитьПараметр("ВидОперации", ?(Комиссия, Перечисления.ВидыПоступленияТоваров.МФПКомиссия,Перечисления.ВидыПоступленияТоваров.МФП));
	Запрос.УстановитьПараметр("Комитент", Комитент);
...
Процедура ЗаполнитьШапкуДокументаПТУМФП(ДокументОбъект, ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаДокумента, УчитыватьНДС, Комиссия = Ложь, Комитент = Неопределено)
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ЗаполнитьШапкуДокументаПТУМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ДокументОбъект.Дата 					= ДатаДокумента;
	ДокументОбъект.ВидОперацииПоступления 	= ?(Комиссия, Перечисления.ВидыПоступленияТоваров.МФПКомиссия, Перечисления.ВидыПоступленияТоваров.МФП);
	ДокументОбъект.Комитент					= Комитент;
	ДокументОбъект.Организация 				= ОрганизацияПокупатель;
	ДокументОбъект.Контрагент 				= КонтрагентПродавец;
	ДокументОбъект.Ответственный 			= ПараметрыСеанса.ТекущийПользователь;
	ДокументОбъект.УчитыватьНДС 			= УчитыватьНДС;
	ДокументОбъект.СуммаВключаетНДС 		= УчитыватьНДС;
	ДокументОбъект.СтатусДокумента			= Справочники.СтатусыДокументов.ПоступлениеТоваровПринят;
	ДокументОбъект.ВалютаДокумента 			= ПараметрыСеанса.ВалютаРубль;
	ДокументОбъект.КурсВзаиморасчетов 		= 1;
	ДокументОбъект.КратностьВзаиморасчетов 	= 1;
	ДокументОбъект.Склад 					= мСкладОсновной;
	...
	ДокументОбъект.ДоговорКонтрагента 	= ДоговорДляМФППоПравиламМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, "возврата");
...
Функция ПроцентыНаценкиПоСтроке(ПравилаСозданияМФП, ДанныеСтроки)
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ПроцентыНаценкиПоСтроке";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Процедура ДобавитьПроцентыНаценки(ПравилаСозданияМФП)
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ДобавитьПроцентыНаценки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ПроцентыНаценкиДляЭтапаЦепочки(ПравилаСозданияМФП, СтрокаЭтапа)
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ПроцентыНаценкиДляЭтапаЦепочки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ЗаполнитьРегистрВозвратыМФП(ДатаСоздания, ТекстОшибки = "") Экспорт
	...
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ЗаполнитьРегистрВозвратыМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если ПроведениеДокументовКлиентСервер.ИспользуетсяНоваяСхемаМФПДляВозвратов(ДатаСоздания) Тогда
		Сообщить("На "+Формат(ДатаСоздания, "ДФ=dd.MM.yyyy")+" используется новая схема возвратов МФП. Регистр ""Возвраты МФП"" заполняется при проведении возвратов от покупателя.");
		Возврат Истина;
	КонецЕсли;
	//Если НЕ ПроведениеДокументовКлиентСервер.ИспользуютсяМФП(ДатаСоздания) Тогда
	//	Возврат Истина;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ПартииТоваровОбороты.Регистратор КАК ВозвратПокупателя,
	...
	|	ТаблицаДвижений.ВозвратПокупателя,
...
Функция СоздатьДокументыВозвратыНаДатуПоДаннымРегистра(ДатаСоздания, ТекстОшибки = "") Экспорт
	...
	Если ПроведениеДокументовКлиентСервер.ИспользуетсяНоваяСхемаМФПДляВозвратов(ДатаСоздания) Тогда
		СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_НоваяСхема(ДатаСоздания, ТекстОшибки);
	Иначе
		СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_СтараяСхема(ДатаСоздания, ТекстОшибки);
	КонецЕсли;
	
КонецФункции

Функция СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_НоваяСхема(ДатаСоздания, ТекстОшибки = "") Экспорт
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_НоваяСхема";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	|	ВозвратыМФП.Качество,
	|	ВозвратыМФП.СтрокаПрихода,
	|	ВозвратыМФП.СтрокаЗаявки,
	|	ВозвратыМФП.ОрганизацияПродавецСписаниеПартий,
	|	ВозвратыМФП.ОрганизацияПокупательСписаниеПартий,
	|	ВозвратыМФП.Количество,
	|	ВозвратыМФП.ЦенаВходящая КАК Цена
	|ПОМЕСТИТЬ ВозвратыМФП
	|ИЗ
	|	РегистрСведений.ВозвратыМФП КАК ВозвратыМФП
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(ВозвратыМФП.Период, ДЕНЬ) = &НачПериода
	|	И НЕ ВозвратыМФП.ОрганизацияПродавецСписаниеПартий = ВозвратыМФП.ОрганизацияПокупательСписаниеПартий
	|	И ВозвратыМФП.ЦенаВходящая > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаПродажСПолитиками.ВозвратПокупателя КАК ВозвратПокупателя,
	...
	|	ТаблицаПродажСПолитиками.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент КАК Коэффициент,
	|	Выбор когда &ЗаменятьНДС = Истина И ТаблицаПродажСПолитиками.Номенклатура.СтавкаНДС = Значение(Перечисление.СтавкиНДС.НДС20)
	|		Тогда Значение(Перечисление.СтавкиНДС.НДС18) Иначе ТаблицаПродажСПолитиками.Номенклатура.СтавкаНДС Конец  КАК СтавкаНДС,
	|	ТаблицаПродажСПолитиками.Качество,
	|	ТаблицаПродажСПолитиками.СтрокаПрихода,
	|	ТаблицаПродажСПолитиками.СтрокаЗаявки,
	|	ТаблицаПродажСПолитиками.СтрокаЗаявки.IDSite КАК IDSite,
	...
	Если ОрганизацииБезПолитик.Количество() > 0 Тогда
		...
		Для каждого Стр ИЗ ОрганизацииБезПолитик Цикл
			лТекстОшибки = лТекстОшибки + Символы.ПС + "Продавец: "+Стр.ОрганизацияПродавец+". Покупатель: "+Стр.ОрганизацияПокупатель+". Документ: "+Стр.ВозвратПокупателя;
		КонецЦикла;
		Сообщить(лТекстОшибки);
	КонецЕсли;
	...
	Пока ВыборкаОрганизацияПродавец.Следующий() Цикл
		...
		Пока ВыборкаОрганизацияПокупатель.Следующий() Цикл
			...
			Пока ВыборкаСклад.Следующий() Цикл
				...
				ДокументОбъект = НайтиСоздатьДокументВозвратОтПокупателяМФП(ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ВыборкаСклад.Склад, ДатаСоздания);
				ЗаполнитьШапкуДокументаВозвратОтПокупателяМФП(ДокументОбъект, ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ПравилаСозданияВозвратовМФП, ДатаСоздания, ВыборкаСклад.Склад);
				
				ДокументОбъект_ВозвратПоставщику = НайтиСоздатьДокументВозвратПоставщикуМФП(ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ВыборкаСклад.Склад, ДатаСоздания);
				ЗаполнитьШапкуДокументаВозвратПоставщикуМФП(ДокументОбъект_ВозвратПоставщику, ВыборкаОрганизацияПокупатель.ОрганизацияПродавец, ВыборкаОрганизацияПокупатель.ОрганизацияПокупатель, ПравилаСозданияВозвратовМФП, ДатаСоздания, ВыборкаСклад.Склад);
				
				ДокументОбъект.Товары.Очистить();
				ДокументОбъект_ВозвратПоставщику.Товары.Очистить();
				Пока ВыборкаТовары.Следующий() Цикл
					...
					Для каждого ПроцентНаценки Из ПроцентыНаценки Цикл
						Если сч < КоличествоПроцентов Тогда
							НоваяСтрока.Цена 		 = НоваяСтрока.Цена/((100+ПроцентНаценки)/100);
						КонецЕсли;
						НоваяСтрока.СебестоимостьЦена = НоваяСтрока.СебестоимостьЦена/((100+ПроцентНаценки)/100);
						сч =сч+1;
					КонецЦикла;
					...
					//Возврат поставщику
					НоваяСтрока_Поставщику = ДокументОбъект_ВозвратПоставщику.Товары.Добавить();
					
					ЗаполнитьЗначенияСвойств(НоваяСтрока_Поставщику, ВыборкаТовары);
					НоваяСтрока_Поставщику.Цена 	= НоваяСтрока.Цена;
					НоваяСтрока_Поставщику.Сумма 	= НоваяСтрока_Поставщику.Цена*НоваяСтрока_Поставщику.Количество;
					ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(НоваяСтрока_Поставщику, ДокументОбъект_ВозвратПоставщику);
				...
				Если ДокументОбъект.Товары.Количество() > 0 Тогда
					...
					Попытка
						...
						ДокументОбъект_ВозвратПоставщику.Комментарий = ДокументОбъект;
						ДокументОбъект_ВозвратПоставщику.Записать(РежимЗаписиДокумента.Проведение);
						Сообщить("Проведен "+ДокументОбъект_ВозвратПоставщику);
						
						РегистрыСведений.СоответствиеДокументовМФП.УстановитьСоответствие(ДокументОбъект.Ссылка, ДокументОбъект_ВозвратПоставщику.Ссылка);
					...
					Исключение
						...
						ОписаниеОшибки = ОписаниеОшибки();
						Сообщить("Не удалось провести документы возврата по причине: "+ОписаниеОшибки);
						...
						ДокументОбъект_ВозвратПоставщику.Комментарий = ДокументОбъект;
						ДокументОбъект_ВозвратПоставщику.Записать(?(ДокументОбъект.Проведен, 
														РежимЗаписиДокумента.ОтменаПроведения, 
														РежимЗаписиДокумента.Запись));
						Сообщить("Записан "+ДокументОбъект_ВозвратПоставщику);
						
						РегистрыСведений.СоответствиеДокументовМФП.УстановитьСоответствие(ДокументОбъект.Ссылка, ДокументОбъект_ВозвратПоставщику.Ссылка);
						
						ДополнитьТекстОшибки(лТекстОшибки, "Ошибка проведения: " + ДокументОбъект+" / "+ ДокументОбъект_ВозвратПоставщику + ": "+ОписаниеОшибки);
...
Функция СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_СтараяСхема(ДатаСоздания, ТекстОшибки = "") Экспорт
...
Процедура СоздатьДокументыВозвратыНаДату(ДатаСоздания)
	...
	СоздатьДокументыВозвратыНаДатуПоДаннымРегистра_СтараяСхема(ДатаСоздания);
...
//Возврат от покупателя
Функция НайтиСоздатьДокументВозвратОтПокупателяМФП(ОрганизацияПродавец, ОрганизацияПокупатель, Склад, ДатаДокумента)
...
Процедура ЗаполнитьШапкуДокументаВозвратОтПокупателяМФП(ДокументОбъект, ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаДокумента, Склад = Неопределено)
...
//Возврат поставщику
Функция НайтиСоздатьДокументВозвратПоставщикуМФП(ОрганизацияПродавец, ОрганизацияПокупатель, Склад, ДатаДокумента)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НАЧАЛОПЕРИОДА(ВозвратТоваровПоставщику.Дата, ДЕНЬ) = &Дата
		|	И ВозвратТоваровПоставщику.ВидОперации = &ВидОперации
	...
	Запрос.УстановитьПараметр("ВидОперации", Перечисления.ВидыОперацийВозвратПоставщику.МФП);
	Запрос.УстановитьПараметр("Дата", ДатаДокумента);
	Запрос.УстановитьПараметр("Контрагент", КонтрагентПродавец);
	Запрос.УстановитьПараметр("Организация", ОрганизацияПокупатель);
	Запрос.УстановитьПараметр("Склад", Склад);
...
Процедура ЗаполнитьШапкуДокументаВозвратПоставщикуМФП(ДокументОбъект, ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, ДатаДокумента, Склад = Неопределено)
	
	лКлючАлгоритма = "Обработка_СозданиеМФП_МодульОбъекта_ЗаполнитьШапкуДокументаВозвратПоставщикуМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ДокументОбъект.Дата 				= КонецДня(ДатаДокумента)-1;
	ДокументОбъект.ВидОперации 			= Перечисления.ВидыОперацийВозвратПоставщику.МФП;
	ДокументОбъект.Организация 			= ОрганизацияПокупатель;
	ДокументОбъект.Контрагент 			= КонтрагентПродавец;
	ДокументОбъект.Ответственный 		= ПараметрыСеанса.ТекущийПользователь;
	ДокументОбъект.УчитыватьНДС 		= Истина;
	...
	ДокументОбъект.Склад 				= ?(ЗначениеЗаполнено(Склад), Склад, мСкладОсновной);
	ДокументОбъект.ПометкаУдаления 		= Ложь;
	ДокументОбъект.флСпособПроведения	= Перечисления.СпособыПроведенияДокументов.ТолькоВзаиморасчеты;
	ДокументОбъект.ДоговорКонтрагента 	= ДоговорДляМФППоПравиламМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, "возврата");
	ДокументОбъект.Филиал				= ДокументОбъект.Склад.Филиал;	
	
	Если ПроведениеДокументовКлиентСервер.ВозвратПоставщику_НовыеСтатусыВДокументе(ДокументОбъект.Дата) Тогда 
		ДокументОбъект.СтатусДокумента  = Справочники.СтатусыДокументов.ВозвратТоваровПоставщикуПодтвержденПоставщиком;
	КонецЕсли;
...
Функция ДоговорДляМФППоПравиламМФП(ОрганизацияПродавец, ОрганизацияПокупатель, ПравилаСозданияМФП, ТипДоговора = "продажи")
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		Если нрег(ТипДоговора) = "продажи" Тогда
			ДоговорМФП = НайденныеСтроки[0].ДоговорКонтрагента;
		ИначеЕсли нрег(ТипДоговора) = "возврата" Тогда
			ДоговорМФП = НайденныеСтроки[0].ДоговорКонтрагентаВозврат;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДоговорМФП) Тогда
		Сообщить("Не найден договор "+ТипДоговора+" для продавца "+ОрганизацияПродавец+", покупателя "+ОрганизацияПокупатель+"! Проверьте заполнение политики МФП покупателя.", СтатусСообщения.Важное);
	КонецЕсли;
...
Функция ИспользуетсяКомиссияНаДату(ДатаСоздания)
	
	ДатаНачалаИспользованияКомиссииМФП = Константы.ДатаНачалаИспользованияКомиссииМФП.Получить();
	
	Возврат ЗначениеЗаполнено(ДатаНачалаИспользованияКомиссииМФП) И ДатаСоздания >= ДатаНачалаИспользованияКомиссииМФП;
