...
Перем СнятьОграничениеПоДатеЗапрета Экспорт; //Добавлено Валиахметов А.А. 17.04.2018
...
Функция DOM_НовыйXMLДокумент() Экспорт
	...
	лПрограммныеИдентификаторы = Новый Массив;
	лПрограммныеИдентификаторы.Добавить("msxml2.domdocument.6.0");
	лПрограммныеИдентификаторы.Добавить("msxml2.domdocument.4.0");
	лПрограммныеИдентификаторы.Добавить("msxml2.domdocument.3.0");
	лПрограммныеИдентификаторы.Добавить("msxml2.domdocument");
	
	Пока (Результат = Неопределено)
		И (лПрограммныеИдентификаторы.Количество() > 0) цикл
		лТекущийПрограммныйИдентификатор = лПрограммныеИдентификаторы[0];
		лПрограммныеИдентификаторы.Удалить(0);
		Попытка
			Результат = Новый COMОбъект(лТекущийПрограммныйИдентификатор);
		Исключение
			Результат = Неопределено; 
		КонецПопытки;
	КонецЦикла;
...
Процедура УстановитьОбменДаннымиЗагрузка(Объект, Значение = Истина) Экспорт
	...
	Если СнятьОграничениеПоДатеЗапрета И ОбщегоНазначения.ЭтоДокумент(Объект.Метаданные()) Тогда 
		Объект.ДополнительныеСвойства.Вставить("СнятьОграничениеПоДатеЗапрета");
...
СнятьОграничениеПоДатеЗапрета = Ложь;
