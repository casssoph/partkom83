...
Процедура Выгрузить(Параметры) экспорт
	...
	//Семенов И.П. 17.05.2019 XX-2486(
	ДБФ.Поля.Добавить("NAZNPLAT", "S", 254); // Назначение платежа
	//)Семенов И.П.
...
Функция СобратьБанкРасходзаПериод(Параметры, Расход = Истина)
	...
	//Семенов И.П. 17.05.2019 XX-2486(
	|	Выразить(ПлатежноеПоручениеИсходящее.НазначениеПлатежа КАК Строка(254)) КАК НазначениеПлатежа,
	//)Семенов И.П.
...
|	   ИЛИ ПлатежноеПоручениеИсходящее.ДоговорКонтрагента.Организация.ОрганизацияЗакрыта >= &ВыбНачПериода )
	...
	//Семенов И.П. 17.05.2019 XX-2486(
	+?(Расход,""," И ПлатежноеПоручениеИсходящее.ВидОперации <> Значение(Перечисление.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВнесениеНаДепозит)")+
	//)Семенов И.П.
	";
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ПТУ.Ссылка КАК ТекДок,
	//Семенов И.П. 17.05.2019 XX-2486(
	|	ПТУ.НазначениеПлатежа КАК НазначениеПлатежа,
	//)Семенов И.П.
	|	ПТУ.Ссылка.Проведен КАК ТекДок_Проведен,
	|	ПТУ.Ссылка.ПометкаУдаления КАК ТекДок_ПометкаУдаления,
	|	ПТУ.Ссылка.Номер КАК ТекДок_НомерДок,
...
Процедура ВыгрузитьДокСтрокаБанковскойВыпискиРасход(ДБФ,Выписка,Параметры)
	...
	//Семенов И.П. 17.05.2019 XX-2486(
	ДБФ.NAZNPLAT = Выписка.НазначениеПлатежа;
	//)Семенов И.П.
