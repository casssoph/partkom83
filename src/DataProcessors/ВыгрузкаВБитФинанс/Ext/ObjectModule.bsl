...
Функция СобратьОстаткиПоВзаиморасчетам(Параметры)
	...
	|ПОМЕСТИТЬ Договоры
	|ИЗ
	|	РегистрНакопления.Взаиморасчеты.ОстаткиИОбороты(&ДтН, &ДтК, День, , ) КАК ВзаиморасчетыОстаткиИОбороты
	|ГДЕ
	|	(ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком) ИЛИ ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))
	
	|	И (ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.Организация.ОрганизацияЗакрыта = ДатаВремя(1,1,1)
	...
	Если Параметры.флVMI Тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"(ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком) ИЛИ ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))","(ВзаиморасчетыОстаткиИОбороты.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))");
	КонецЕсли;
...
|	   ИЛИ ПоступлениеТоваровУслуг.ДоговорКонтрагента.Организация.ОрганизацияЗакрыта >= &ВыбНачПериода )
	...
	Если Параметры.флVMI Тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"(ПоступлениеТоваровУслуг.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком) ИЛИ ПоступлениеТоваровУслуг.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))","(ПоступлениеТоваровУслуг.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))");
	КонецЕсли;
	Если НЕ Параметры.флОферта Тогда
...
|	   ИЛИ ПлатежноеПоручениеИсходящее.ДоговорКонтрагента.Организация.ОрганизацияЗакрыта >= &ВыбНачПериода )
	...
	Если Параметры.флVMI Тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"И (ПлатежноеПоручениеИсходящее.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком) ИЛИ ПлатежноеПоручениеИсходящее.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))","(ПлатежноеПоручениеИсходящее.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))");
	КонецЕсли;
	Если НЕ Параметры.флОферта Тогда
...
|	   ИЛИ КорректировкаДолгаСуммыДолга.ДоговорКонтрагента.Организация.ОрганизацияЗакрыта >= &ВыбНачПериода )
	...
	Если Параметры.флVMI Тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"И (КорректировкаДолгаСуммыДолга.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком) ИЛИ КорректировкаДолгаСуммыДолга.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение))","(КорректировкаДолгаСуммыДолга.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение)))");
	КонецЕсли;
	Если НЕ Параметры.флОферта Тогда
