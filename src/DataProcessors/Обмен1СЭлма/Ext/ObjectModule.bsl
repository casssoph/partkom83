...
Процедура УстановитьДанныеКонтрагента(Структура, ОбъектXDTO)
	...
	Структура.Вставить("ДопустимоеЧислоДнейЗадолженности", CRM.Company.DelayOfPayment);
	
	//Реквизиты торговой точки//
	УстановитьРегионКонтрагента(Структура, CRM.Company.Region.Name);
	Структура.Вставить("Логин", CRM.Company.AssignedTo.Login);
	Структура.Вставить("Пароль", CRM.Company.Site);
...
Процедура ЗаписатьКонтрагента(ДанныеКонтрагента)
	...
	Контрагент.НаименованиеПолное = ДанныеКонтрагента.Наименование;
	...
	ЗаполнитьЗначенияСвойств(Контрагент, ДанныеКонтрагента, "Наименование,Покупатель,Поставщик,СайтГруппаКонтрагента,ИНН,КПП,ЮрФизЛицо,СегментКонтрагента");
...
Процедура ЗаписьЛогаЗагрузки(Файл, ДанныеКонтрагента)
	...
	Если ИСТИНА ИЛИ ДанныеКонтрагента.Ошибка Тогда
		Текст = Новый ТекстовыйДокумент;
		Текст.УстановитьТекст(ДанныеКонтрагента.ТекстОшибки);
		Текст.Записать(НовыйКаталог + "/" + ИмяФайла + ".log");
	КонецЕсли;
...
Процедура УстановитьСегментКонтрагента(Структура, Код)
	...
	Значение = Справочники.СегментыКонтрагентов.НайтиПоКоду(Код);
	Если Значение.Пустая() Тогда
		Структура.Ошибка = Истина;
		ДополнитьОшибку(Структура.ТекстОшибки, "Не определен сегмент контрагента по коду <" + Код + ">");
	КонецЕсли;
	
	Структура.Вставить("СегментКонтрагента", Значение);
