...
Функция ПолучитьДанныеКонтрагента(Контрагент)
	//Семенов И.П. 13.03.2019 XX-1249 добавил в запрос выбор поля УдаленноеРасположение
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	Контрагент.Ссылка.УдаленноеРасположение КАК УдаленноеРасположение
	                      |ИЗ
...
Процедура ВыгрузитьДанныеКонтрагентаВФайл(ИмяФайла, ДанныеКонтрагента, ЛогинELMAТекущегоПользователя, ЛогинELMAЗакрепленногоМенеджера, ЭтоНовый)
	...
	ОбъектCRM = ФабрикаXDTO.Создать(СвойствоCRM.Тип);
		...
		//Семенов И.П. 13.03.2019 XX-1249(
		СтруктураCompany.Вставить("UdalennoeRaspolozhenie",ДанныеКонтрагента.УдаленноеРасположение);
		//)Семенов И.П.
