...
Функция ЗагрузитьИз77_НН() Экспорт
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_ЗагрузитьИз77_НН";
...
Процедура ЗаполнитьПартииПоДанным77_НН(вхДокументСсылка)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_ЗаполнитьПартииПоДанным77_НН";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ЗагрузитьШаблонДокументаИз77(ТекстОшибки = "")
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_ЗагрузитьШаблонДокументаИз77";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Пока RS.EOF() = 0 Цикл
		...
		СоздатьСчетФактуру(ДокОбъект, ДокСсылка, RS);
...
Процедура СоздатьСчетФактуру(ДокОбъект, ДокСсылка, RS);
	
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_СоздатьСчетФактуру";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	//Счет-фактура
	СФ = УчетНДС.НайтиПодчиненныйДокумент(ДокСсылка, "СчетФактураВыданный");
	Если ЗначениеЗаполнено(СФ) Тогда
		СФОбъект = СФ.ПолучитьОбъект();
	Иначе
		СФОбъект = Документы.СчетФактураВыданный.СоздатьДокумент();
		СФОбъект.ДокументОснование = ДокСсылка;
		СФОбъект.ДокументыОснования.Добавить().ДокументОснование = ДокСсылка;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СФОбъект.Номер) Тогда
		СФНомер77 = RS.Fields("DOCNOSF").Value;
		СФДата77 = RS.Fields("dateSF").Value;
		Если ЗначениеЗаполнено(СФНомер77) Тогда
			СФОбъект.Номер = СФНомер77;
		КонецЕсли;
		Если ЗначениеЗаполнено(СФДата77) Тогда
			СФОбъект.Дата = Дата(СФДата77 + "000000");
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СФОбъект.Номер) Тогда
		СФОбъект.ОбменДанными.Загрузка = Истина;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(СФОбъект);
		ДобавитьСвойствоЗагруженИз77(СФОбъект.Ссылка);
		
		Если НЕ ДокОбъект.УчитыватьНДС Тогда
			НовыйДокОбъект = ДокСсылка.ПолучитьОбъект();
			НовыйДокОбъект.УчитыватьНДС = ИСтина;
			НовыйДокОбъект.СуммаВключаетНДС = Истина;
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(НовыйДокОбъект);
		КонецЕсли;
...
Процедура ЗаполнитьПартииПоДанным77(ДокСсылка, RS, Connection = Неопределено)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_ЗаполнитьПартииПоДанным77";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ЗаполнитьТоварыПоДанным77(ДокОбъект, RS, Connection = Неопределено)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_ЗаполнитьТоварыПоДанным77";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция СоздатьДокументПоДанным77(СтрокаТЧ, RS)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_СоздатьДокументПоДанным77";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если Об.ЭтоНовый() ИЛИ ПерезаполнятьШапку Тогда
		...
		//Если НЕ Значениезаполнено(Об.СтатусДокумента) Тогда
			Об.СтатусДокумента = Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен;
		//КонецЕсли;
...
Функция СформироватьТекстЗапросаТЧ(Структура)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_СформироватьТекстЗапросаТЧ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция СформироватьТекстЗапросаШапка(Структура)
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_СформироватьТекстЗапросаШапка";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	|f.Code As firm,
	|left(j_SF.DATE_TIME_IDDOC,8) AS dateSF,
	|j_SF.DOCNO AS DOCNOSF,
	|j_SF.IDDOC AS IDDOCSF
	|from DH1611 AS dh 
	|LEFT JOIN DH2051 as SF on dh.IDDOC = right(SF.SP2038,9)
	|LEFT JOIN _1SJOURN as j_SF on SF.IDDOC = j_SF.IDDOC
...
Функция СформироватьТекстЗапросаПартии(Структура,база = "спб")
	...
	лКлючАлгоритма = "Обработка_ЗагрузитьРТУИз77_СформироватьТекстЗапросаПартии";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
