...
Процедура ЗагрузитьИз77НаСервере() Экспорт
	
	 ОбменДаннымиВызовСервера.ПолучитьДокументИз77(
	 Новый Структура("Номер77,Дата77,ПерезаполнятьШапку,ПерезаполнятьТЧТовары",
	 Номер77,Дата77,ПерезаполнятьШапку,ПерезаполнятьТЧТовары));
...
Процедура ЗаполнитьПартииПоДанным77(ДокСсылка, RS, Connection = Неопределено)
	...
	Пока RSПартии.EOF() = 0 Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаПрихода) Тогда
			...
			//Страна
			СтранаНаименование = RSПартии.Fields("StranaDESCR").Value;
