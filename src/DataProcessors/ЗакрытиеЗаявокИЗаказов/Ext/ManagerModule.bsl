...
Процедура УстановитьПолныйОтказВыборки(ВыборкаНезакрытых, ПричинаОтказа)
	...
	Пока ВыборкаНезакрытых.Следующий() цикл
		...
		Попытка 
			...
			//Закроем заявку
			Если не ОбщегоНазначения.ПолучитьПоследнююКорректировкуЗаявки(ВыборкаНезакрытых.ДокументСсылка) = ВыборкаНезакрытых.ДокументСсылка Тогда //В регистре корректировок заявок не всегда есть запись, добавим еще проверку по документам
