...
Процедура ПриОткрытии(Отказ)
	...
	Объект.ДатаНачала = НачалоДня(ТекущаяДатаСеанса()) - 2*60*60*24;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ВладелецФормы.ИмяФормы) = ВРег("Обработка.ОценкаПроизводительности.Форма.ФормаОтбора") Тогда
		
		Если ВыбранноеЗначение <> Неопределено Тогда
			ОбновитьПоказатели(ВыбранноеЗначение);
		КонецЕсли;
	
	ИначеЕсли ВРег(ИсточникВыбора.ВладелецФормы.ИмяФормы) = ВРег("Справочник.APDEX_КлючевыеОперации.Форма.ФормаВыбора") 
		ИЛИ ВРег(ИсточникВыбора.ВладелецФормы.ИмяФормы) = ВРег("Обработка.ОценкаПроизводительности.Форма.Форма") Тогда
		...
		ПредопределенныйЭлемент = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы");
		Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.APDEX_КлючевыеОперации") тогда
			ТекущаяСтрока.КлючеваяОперация = ВыбранноеЗначение;
		Иначе			
...
Процедура ПроизводительностьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Лев(Поле.Имя, 18) <> "Производительность" Или
		СтрокаТЧ.КлючеваяОперация = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
...
Процедура Обновить(Команда)
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ОбновитьПоказатели_ТолстыйКлиент();
	#Иначе		
		ОбновитьПоказатели();
	#КонецЕсли
...
Процедура ПереместитьВверх(Команда)
	...
	Если Врем.Количество() <= 1 ИЛИ 
		ТекущийИндекс = 0 ИЛИ 
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ПереместитьВниз(Команда)
	...
	Если Врем.Количество() <= 1 Или
		...
		Врем[ТекущийИндекс + 1].КлючеваяОперация = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы") Или
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ОбновитьПоказатели_ТолстыйКлиент(ЗначенияОтбора = Неопределено)
	...
	//ТаблицаКлючевыхОпераций = ПоказателиПроизводительности();
	...
	ИначеЕсли Элементы.Форма.ТекущаяСтраница.Имя = "СтраницаТаблица" Тогда
		...
		//Производительность.Загрузить(ТаблицаКлючевыхОпераций);
...
Процедура УстановитьУсловноеОформлениеТЧ(ПоляОтбора, ПоляОформления, УсловноеОформление);
	...
	ЭлементОтбора.ПравоеЗначение = Справочники.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы;
	// Оформляемое поле
...
Функция ПодготовитьПараметрыЭкспорта()
	...
	ОбщаяПроизводительность = ПредопределенноеЗначение("Справочник.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы");
...
Процедура ОткрытьФормуВыбора()
	...
	ОткрытьФорму("Справочник.APDEX_КлючевыеОперации.ФормаВыбора", ПараметрыФормы, ЭтаФорма);
...
Функция ТаблицаКлючевыхОперацийДляВычисленияAPDEX()
	...
	ТаблицаКлючевыхОпераций.Колонки.Добавить(
		"КлючеваяОперация", 
		Новый ОписаниеТипов("СправочникСсылка.APDEX_КлючевыеОперации"));
	ТаблицаКлючевыхОпераций.Колонки.Добавить(
		"Приоритет", 
...
Функция НастройкаВыполнена()
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			И СтрокаТЧ.КлючеваяОперация <> Справочники.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы 
		Тогда
			...
			//	"Объект");
			Сообщить("Целевое время обязательно должно быть заполнено");
...
Функция ЗагрузитьКлючевыеОперации()
	...
	Запрос.УстановитьПараметр("ОбщаяПроизводительностьСистемы", Справочники.APDEX_КлючевыеОперации.ОбщаяПроизводительностьСистемы);
	...
	|ИЗ
	|	Справочник.APDEX_КлючевыеОперации КАК КлючевыеОперации
	|ГДЕ
	|	НЕ КлючевыеОперации.Ссылка = &ОбщаяПроизводительностьСистемы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет
	|АВТОУПОРЯДОЧИВАНИЕ";
