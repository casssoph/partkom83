...
Процедура СтруктураЗапросаОстаткиНоменклатурыСХарактеристиками(ДатаЗапроса, СписокПараметровЗапроса, ПоказыватьОстатки, РазворачиватьДоКачества) Экспорт
	...
	текст = текст + ПолучитьСтрокуЗапросаСвободныеОстатки(Новый Структура("ТоварыНаСкладах"), ТекстУсловияКачества, Ложь);
	текст = текст + "
	|		) КАК ОбщиеОстатки
	|		
	|	СГРУППИРОВАТЬ ПО
...
Функция ЗапросОстаткиНоменклатурыВРознице(Запрос)
	...
	|	Подбор.Изготовитель                             КАК Изготовитель,
	...
	|	СправочникНоменклатура.Изготовитель                      КАК Изготовитель,
...
Функция ЗапросОстаткиНоменклатуры(Запрос, Организация)
	...
	|	Подбор.Номенклатура.Изготовитель                КАК Изготовитель,
...
Функция ЗапросОстаткиУКомиссионеров(Запрос)
	...
	|			И ТоварыПереданныеОстатки.Сделка = ЗаказыПокупателейОстатки.ЗаявкаПокупателя
	|	СГРУППИРОВАТЬ ПО
...
Функция ЗапросДействующиеИзготовители()
	 ...
	 ТекстЗапроса = "ВЫБРАТЬ
	                ...
	                |	Номенклатура.ЕдиницаХраненияОстатков.Представление КАК ПредставлениеЕдиницаИзмерения,
	                ...
	                |ГДЕ
	                |	НЕ Номенклатура.Изготовитель.ВыведенИзОбращения
...
Функция ПолучитьЗапросДляПодбора(Знач ИмяПодбора, Родитель, ДатаЗапроса, ПересчитатьОстаткиВЕдиницуЦены = Истина, РазворачиватьДоКачества, ВладелецФормы) Экспорт
	...
	ИначеЕсли ИмяПодбора = "ПриходДействующиеИзготовители" Тогда
		
		Запрос.УстановитьПараметр("Родитель"             , Родитель);
		Запрос.Текст = ЗапросДействующиеИзготовители();
