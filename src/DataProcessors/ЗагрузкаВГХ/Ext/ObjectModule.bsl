...
Процедура ПолучитьДанныеВГХ() экспорт
	...
	Если МаксПроцОтклДефолт = 0 Тогда
		МаксПроцОтклДефолт = 100;
	КонецЕсли;
...
Процедура ЗапроситьПоСписку(знач с_uid, знач т_связки)
	...
	тчРез = ВыполнитьЗапросВГХ(с_uid);        //тчРез.выбратьстроку();
	...
	Для каждого стрез из тчРез Цикл
		...
		// + Пушкин 20190628 XX-2770
		л_ВесИзм = стрез.p6;
		c_ВесИзм= Формат(л_ВесИзм, "ЧЦ=15; ЧДЦ=0; ЧГ=0");
		// - Пушкин 20190628 XX-2770
		
		л_отказ = ЗаписатьВГХ(л_тов,л_Длина,л_Ширина,л_Высота,л_Вес,л_Объем,л_ВесИзм);
		...
		СообщитьЛог(Символы.Таб + "output. " + СокрЛП(пп) + ". " + СокрЛП(стрез.p0)+ ". Длина =" + СокрЛП(c_Длина)+ ". Ширина =" + СокрЛП(c_Ширина)+ ". Высота =" + СокрЛП(c_Высота)+ ". Вес =" + СокрЛП(c_Вес)+ ". ВесИзм =" + СокрЛП(c_ВесИзм)+ ". Объем =" + СокрЛП(c_Объем)+ ". Записано =" + СокрЛП(л_рез) + ?(л_рез,"",". err: " + л_отказ)  );
...
Функция ЗаписатьВГХ(л_тов,л_Длина,л_Ширина,л_Высота,л_Вес,л_Объем,л_ВесИзм)
		...
		Если з_Вес + з_Объем + з_Длина + з_Ширина + з_Высота > 0 Тогда
			...
			Попытка
				...
				Если з_Вес > 0 Тогда
					...
					// + Пушкин 20190628 XX-2770
					Попытка
						еи_Метаданные = еи_объект.Метаданные();
						Если еи_Метаданные.Реквизиты.Найти("ВесИзм") = Неопределено Тогда
						Иначе
							еи_объект.ВесИзм = л_ВесИзм;
						КонецЕсли;
					Исключение
					КонецПопытки;
					// - Пушкин 20190628 XX-2770
...
|,case 
|	when ttt.Вес > 0 and ttt.Reference64_Weight > 0 and ttt.Вес > ttt.Reference64_Weight then
|		case
|			when (ttt.Вес / ttt.Reference64_Weight - 1)*100 > ttt.Reference64_K then ttt.Reference64_Weight
|			else ttt.Вес
|		end 
|	else ttt.Вес
| end as Вес2 
...
// - Пушкин 20190628 XX-2770
	...
	|	,ISNULL(_Reference64._Fld96,0) as Reference64_Weight
    |
	|	,case
	|		when ISNULL(_Reference64._Fld199,0) = 0 then " + Формат(МаксПроцОтклДефолт,"ЧЦ=3; ЧДЦ=; ЧВН=; ЧГ=0") + "
	|		else ISNULL(_Reference64._Fld199,0)
	|	 end as Reference64_K
...
Функция ПолучитьИмяФайлаДляЛогирования(пар_Имя) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(л_Имя) Тогда                                           
		...
		л_Имя = "\\nng9-v-1c-07\ДляЗагрузкиОстатков\НСИ\"+ "1с83_ЗагрузкаВГХ_" + ПравилаИгры_метка + ".txt";	
...
Процедура ЗаполнитьСписокНоменклатуры()
	...
	ИначеЕсли НомерСценария = 4 Тогда
		ОпроситьФайлСКодамиНоменклатуры();
	КонецЕсли;
...
Функция ПолучитьВладельцевЕИ(тчСвязки)
	...
	Если тчСвязки.Количество() = 0 Тогда
		Возврат тчРез;
	КонецЕсли;
...
Процедура ОпроситьФайлСКодамиНоменклатуры() Экспорт
	
	Текст = "";
	лМас = Новый Массив;
	ИмяЛогФайлаПолное = "\\nng9-v-1c-07\ДляЗагрузкиОстатков\НСИ\t_e_test.txt";
	пар_ПолноеИмяФайла = ИмяЛогФайлаПолное;
	Сообщить(пар_ПолноеИмяФайла);
	
	Если МаксимальныйРазмерВыборки = 0 Тогда
		МаксимальныйРазмерВыборки = 100;
	КонецЕсли;
	
	Файл = Новый Файл(пар_ПолноеИмяФайла); 
	Сообщить("Файл.Существует() = " + СокрЛП(Файл.Существует()));
	
	Попытка
		//ТекстовыйФайл = Новый ЧтениеТекста(пар_ПолноеИмяФайла, КодировкаТекста.Системная);
		//Текст = ТекстовыйФайл.Прочитать();
		//ТекстовыйФайл.Закрыть();
		
		ТекстовыйФайл = Новый ТекстовыйДокумент;
		ТекстовыйФайл.Прочитать(пар_ПолноеИмяФайла);		
	...
	Для ен = 1 по ВсегоСтрок цикл
		...
		Если лМас.Количество() + 1 > МаксимальныйРазмерВыборки Тогда
			Прервать;
		КонецЕсли;
		...
		мас_стр = СтрРазделить(л_стр,";");
		
		Если мас_стр.Количество() = 1 Тогда
			Сообщить(л_стр);
			
			лМас.Добавить(л_стр);
			
			Текст = СтрЗаменить(Текст,л_стр,л_стр + ";готово " + СокрЛП(ен));
		КонецЕсли;
	...
	Запрос.УстановитьПараметр("КодЫ",лМас);
	тчТов = Запрос.Выполнить().Выгрузить();
	
	Номенклатура.ЗагрузитьЗначения(тчТов.ВыгрузитьКолонку("Ссылка"));
	
	Попытка
		ТекстовыйФайл = Новый ТекстовыйДокумент;
		ТекстовыйФайл.УстановитьТекст(Текст);
		ТекстовыйФайл.Записать(пар_ПолноеИмяФайла,"windows-1251");
	Исключение
	КонецПопытки;
