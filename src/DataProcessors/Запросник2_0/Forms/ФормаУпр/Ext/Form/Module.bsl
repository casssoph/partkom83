...
Функция ВыполнитьЗапросНаСервере(СтруктураУИД)
	...
	Для Каждого ВТИсточник Из МассивВТ Цикл
		...
		КонецЦикла;
	КонецЦикла;
	//Удалим все таблица виды Запрос_
	МассивУдаления = Новый Массив;
	Для Каждого ВременнаяТ Из ВременныеТаблицы Цикл
		Если Лев(ВременнаяТ.Таблица,7) = "Запрос_" Тогда
			МассивУдаления.Добавить(ВременнаяТ);
		КонецЕсли;
	КонецЦикла;
	Для каждого ВременнаяТ Из МассивУдаления Цикл
		ОчиститьХранилищеВТНаФорме(ВременнаяТ.Таблица);
		ВременныеТаблицы.Удалить(ВременныеТаблицы.Индекс(ВременнаяТ));
	КонецЦикла;
	...
	Для Каждого ТекЗапрос Из МассивТекстовЗапросов Цикл
		...
		Иначе
			...
			Если Найти(ТекЗапрос, "УНИЧТОЖИТЬ ")>0 Тогда
				ИмяТаблицы = "Запрос_"+Строка(НомерЗапроса)+"("+СокрЛП(ТекЗапрос)+")";	
			Иначе
				ИмяТаблицы = "Запрос_"+Строка(НомерЗапроса);	
			КонецЕсли;
		...
		Если Найти(обТекущийЗапрос.Текст,"//&УдалениеВТ") Тогда
			Продолжить;
		КонецЕсли;
		Выборка = мРезЗапроса.Выбрать();
		Если ЭтоВременная и Выборка.Следующий() Тогда
			Количество = Строка(Выборка[0]);
		Иначе 
			ВыборкаТест = мРезЗапроса.Выбрать();
			Количество = ВыборкаТест.Количество();
		КонецЕсли;
		СтрокиЗапросов = ВременныеТаблицы.НайтиСтроки(Новый Структура("Таблица",ИмяТаблицы));
		...
		СоздатьХранилищеВТНаФорме(НоваяСтрока.Таблица,ОбТекущийЗапрос,НоваяСтрока,?(ЭтоВременная,Неопределено,мРезЗапроса));
...
&НаСервере
Процедура СоздатьХранилищеВТНаФорме(ИмяТаблицы,Запрос,СтрокаВТ,мРезЗапроса)
	Если мРезЗапроса = Неопределено тогда
		Запрос.Текст = "Выбрать * Из "+ИмяТаблицы;
		ТЗ = Запрос.Выполнить().Выгрузить();
	Иначе
		ТЗ = мРезЗапроса.Выгрузить();
	КонецЕсли;
...
Процедура РезультатТаблицаПриАктивизацииЯчейки(Элемент)
	...
	Если Элементы.НадписьТипЗначения.Заголовок <> ИмяЭлемента Тогда
...
&НаКлиенте
Процедура КонтекстнаяПодсказка(Команда)
	// Вставить содержимое обработчика.
	Спис = Новый СписокЗначений;
	Спис.Добавить("1");
	Спис.Добавить("2");
	ВыбратьИзСписка(Спис,ТекущийЭлемент);
КонецПроцедуры
