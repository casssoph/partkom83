...
Функция ПолучитьСписокДокументов()
	ТабПисем = Неопределено;
	ОбщегоНазначения.СоздатьСтруктуруРегистраСведений("ИсторияОтправкиПисемПоставщикам", табПисем);
	...
	|	И ИсторияОтправкиПисемПоставщикам.Ошибка"
	);
	
	ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Запрос.Выполнить().Выгрузить(), ТабПисем);
