Процедура ВыполнитьПроверку() экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДатаН) тогда
		ДатаН = НачалоДня(ТекущаяДата());//ДобавитьМесяц(ТекущаяДата(),-1);
	КонецЕсли;
...
Процедура ВыполнитьПроверку_2() экспорт
	...
	|ГДЕ
	|	ЭкспрессДоставкаОбмен.Период > &Период
	|	И ЭкспрессДоставкаОбмен.Error = &Error
	|	И ЭкспрессДоставкаОбмен.Description ПОДОБНО &Description
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	РегистрСведений.ЭкспрессДоставкаОбмен КАК ЭкспрессДоставкаОбмен
	|ГДЕ
	|	ЭкспрессДоставкаОбмен.express_delivery_request_id В
	|			(ВЫБРАТЬ
	|				ерр.express_delivery_request_id
	|			ИЗ
	|				ерр)
	|	И (ЭкспрессДоставкаОбмен.operation_type = &ExpressDelivery_1cConfirm
	|			ИЛИ ЭкспрессДоставкаОбмен.operation_type = &ExpressDelivery_1cCancel)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ерр КАК ерр
	|ГДЕ
	|	НЕ ерр.express_delivery_request_id В
	|			(ВЫБРАТЬ
	|				закр.express_delivery_request_id
	|			ИЗ
	|				закр)";

	Запрос.УстановитьПараметр("Error", Истина);
	Запрос.УстановитьПараметр("Период", НачалоДня(ТекущаяДата()));
	Запрос.УстановитьПараметр("Description", "все РТУ проведены;ошибка расчета стоимости услуги");
	Запрос.УстановитьПараметр("ExpressDelivery_1cCancel", "ExpressDelivery.1cCancel");
	Запрос.УстановитьПараметр("ExpressDelivery_1cConfirm", "ExpressDelivery.1cConfirm");
	...
	Для Каждого СтрокаРезультата Из РезультатТаблица Цикл
	   ...
	   сч = сч + 1;
		...
		СтруктураОтчета_express_delivery_request_id = СтрокаРезультата.express_delivery_request_id;
		Сообщить(СокрлП(сч) + ". " + СтруктураОтчета_express_delivery_request_id);
		Ответ = SMSоповещение.РассчитатьУслугуЭД(СтруктураОтчета_express_delivery_request_id);
		Сообщить(Ответ);
		
		Если Ответ = Истина Тогда
			 
			 НаборЗаписей = РегистрыСведений.ЭкспрессДоставкаОбмен.СоздатьНаборЗаписей(); 
			 НаборЗаписей.Отбор.Период.Установить(СтрокаРезультата.Период);
			 НаборЗаписей.Отбор.express_delivery_request_id.Установить(СтрокаРезультата.express_delivery_request_id);
			 НаборЗаписей.Отбор.operation_type.Установить(СтрокаРезультата.operation_type);
			 НаборЗаписей.Прочитать();
			 
			 НовыйНомер = НаборЗаписей[0];
			 НовыйНомер.Error = Ложь;
			 НовыйНомер.Description = "все РТУ проведены";
