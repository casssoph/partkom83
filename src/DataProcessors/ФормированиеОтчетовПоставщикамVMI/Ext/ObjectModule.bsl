		...
		Если ЗначениеЗаполнено(Прайс.Склад) Тогда
			Если ЗначениеЗаполнено(Прайс.Договор) Тогда
				Если ЗначениеЗаполнено(ДатаКон) Тогда
					ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Обработки.ФормированиеОтчетовПоставщикамVMI.ЗаполнитьОстаткамиПоРегиструПартий(Прайс.ПрайсПоставщика, Прайс.Договор, Прайс.Склад, ДатаНач, ДатаКон, ЛогОшибок, Прайс.СкидкаСумма, Прайс.СкидкаПроцент), Товары);
					
				Иначе
					ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Обработки.ФормированиеОтчетовПоставщикамVMI.ЗаполнитьОстаткамиПоРегиструПартий(Прайс.ПрайсПоставщика, Прайс.Договор, Прайс.Склад, , Прайс.ДатаКон, ЛогОшибок, Прайс.СкидкаСумма, Прайс.СкидкаПроцент), Товары);
...
Процедура СформироватьДокументПоНайденнымСтрокам(СтрокаПрайса, СтрокиТоваров, ДатаКонецМесяца = Неопределено, вхПериод)
	...
	ДатаОкончанияПериодаОтчета = СтрокаПрайса.ДатаКон;
		...
		Если СтрокиТоваров.Количество() > 0 тогда
			...
			Если Прайс.КонецМесяцаОтдельно И ДатаКонцаМесяца < Прайс.ДатаКон Тогда
				...
				Если КопияСтрокДо.Количество() > 0 Тогда
					...
					СформироватьДокументПоНайденнымСтрокам(Прайс, КопияСтрокДо, ДатаКонцаМесяца, ПериодОтчета);
			...
			Иначеесли  Прайс.ДелитьОтчетVMI тогда 
				...
				Для Каждого Товар Из СтрокиТоваров Цикл
					Если Товар.ДатаПоступления < ДатаДок Тогда
						СтрокаКопии = КопияСтрокДо.Добавить();
					Иначе
						СтрокаКопии = КопияСтрокПосле.Добавить();
					КонецЕсли;
				...
				КонецЕсли;
				 ...
				 МассивДат = КопияСтрокПосле.ВыгрузитьКолонку("ДатаПоступления");
				 Для каждого Дата из МассивДат цикл 
					СтрокиНаДату = КопияСтрокПосле.НайтиСтроки(новый Структура("ДатаПоступления",Дата)); 
					СформироватьДокументПоНайденнымСтрокам(Прайс, СтрокиНаДату, , ПериодОтчета); 
				 КонецЦикла;	 
			...
			Иначе
				ПериодОтчета = Формат(Прайс.ДатаНач, "ДФ=dd.MM.yy") + "-" + Формат(Прайс.ДатаКон, "ДФ=dd.MM.yy");
				СформироватьДокументПоНайденнымСтрокам(Прайс, СтрокиТоваров, , ПериодОтчета);
