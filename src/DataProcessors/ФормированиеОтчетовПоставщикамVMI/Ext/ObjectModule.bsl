...
Процедура СформироватьДокументы() Экспорт
	Для каждого Прайс Из ПрайсыПоставщиков Цикл
		...
		Если ЗначениеЗаполнено(Прайс.ДатаПоследнегоОтчета) Тогда
			Если КонецМесяца(Прайс.ДатаНач) >= Прайс.ДатаКон Тогда
				Прайс.ДатаНач = КонецДня(Прайс.ДатаПоследнегоОтчета) + 1;
			КонецЕсли;
		КонецЕсли;
