...
Процедура ВыполнитьРегламентноеЗадание() Экспорт 
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ВыполнитьРегламентноеЗадание";
	...
	Если Не МногоПоточно Тогда 	
		Для Каждого СтрокаП Из ПараметрыФормированияСЗ Цикл 
			...
			// 05.08.19 Строганов Роман > 
			Если Параметры.ТипДоставки <> ПредопределенноеЗначение("Справочник.ТипыДоставки.СамовывозСТочкиВыдачи") Тогда
				Продолжить;
			КонецЕсли;
			// 05.08.19 Строганов Роман <
...
Процедура СоздатьДокументыСЗ(Параметры) Экспорт 
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_СоздатьДокументыСЗ_2";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	// Номер самовывоза - сгенерировать и запомнить.
	СоответствиеНомеровСамовывозов = Новый Соответствие;
	...
	Для Каждого СтрокаП Из Покупатели Цикл 
		...
		Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
			...
			// 31.07.19 Строганов Роман > 
			ИсходныеСтроки = ТоварыКонтрагента.НайтиСтроки(ОтборБезДатыОплаты);
			Если ИсходныеСтроки.Количество() Тогда
				СамовывозСклад = ИсходныеСтроки[0].СамовывозСклад;
				СамовывозИмя = ИсходныеСтроки[0].СамовывозИмя;
				СамовывозТелефон = ИсходныеСтроки[0].СамовывозТелефон;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СамовывозСклад) Тогда
				Если ДокументРеализация.Склад.ФизическийСклад =  СамовывозСклад Тогда
					ДокументРеализация.ТипДоставки = Справочники.ТипыДоставки.СамовывозСТочкиВыдачи;
				Иначе
					ДокументРеализация.ТипДоставки = Справочники.ТипыДоставки.МежскладскаяДоставка;
				КонецЕсли;
				ДокументРеализация.Заказчик = СамовывозИмя;
				ДокументРеализация.Телефон = СамовывозТелефон;
				
				//Если Не ЗначениеЗаполнено(ДокументРеализация.НомерСамовывоза) Тогда
					// Номер самовывоза - назначить.
					Ключ = XMLСтрока("" + ДокументРеализация.Контрагент + ДокументРеализация.Склад.ФизическийСклад);
					НомерСамовывоза = СоответствиеНомеровСамовывозов[Ключ];
					Если НомерСамовывоза = Неопределено Тогда 
						НомерСамовывоза =  СгенерироватьНомерСамовывоза();
						СоответствиеНомеровСамовывозов.Вставить(Ключ, НомерСамовывоза); 
					КонецЕсли;
					
					Если ДокументРеализация.ЭтоНовый() Тогда
						СсылкаДокумента = ДокументРеализация.ПолучитьСсылкуНового();
					Иначе
						СсылкаДокумента = ДокументРеализация.Ссылка;
					КонецЕсли;
					
					РегистрыСведений.Документ_РеализацияТоваровУслуг_ДополнительныеРеквизиты.ЗафиксироватьДопСвойство(СсылкаДокумента, "СамовывозНомер", НомерСамовывоза);
					//КонецЕсли;
			Иначе
				ДокументРеализация.ТипДоставки = Параметры.ТипДоставки;
			КонецЕсли;
			
			//ДокументРеализация.МаршрутДоставки = СтрокаТЧ.МаршрутДоставки;
			// 02.08.19 Строганов Роман <
...
Функция СгенерироватьНомерСамовывоза()
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = ГСЧ.СлучайноеЧисло(0, 1000);
	ТекДата = Формат(ТекущаяДата(), "ДФ=ddMMyy");
	
	Возврат ТекДата + СлучайноеЧисло;
...
Процедура ЗаполнитьРеквизитыЭД(Знач ДокументРеализация, Знач Параметры)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ЗаполнитьРеквизитыЭД";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ДокументМаршрутноеЗадание(Реализация,ДокСЗ)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_ДокументМаршрутноеЗадание";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
...
Процедура СоздатьДокументыМЗ(ДокСЗ)
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_СоздатьДокументыМЗ"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ДобавитьПричинуИсключения(ДокСЗ, ДокументРеализация, ДопроводимыеРТУ, ОписаниеОшибки, СтрокиТоваров)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ДобавитьПричинуИсключения";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ПоместитьВДопроведение(ДокументРеализация, ДопроводимыеРТУ, СтрокиТоваров)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ПоместитьВДопроведение";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ДопровестиРТУ(ДопроводимыеРТУ, ДокСЗ)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ДопровестиРТУ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЗаписатьРТУ(ДокСЗ, ДокументРеализация, РегистрироватьВОбмене = Истина)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ЗаписатьРТУ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ПараметрыФормированияСЗ() 
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	И ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели2
	                |	И ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки2 >= ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска2
	                |	И ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ТипДоставки = &ТипДоставки
	                |
	                |ОБЪЕДИНИТЬ
	                |
	                |ВЫБРАТЬ
	...
	//Запрос.УстановитьПараметр("ДтТек", Дата);
	Запрос.УстановитьПараметр("ДтТек", ТекущаяДата());
	...
	// 01.08.19 Строганов Роман > 
	Запрос.УстановитьПараметр("ТипДоставки", Справочники.ТипыДоставки.СамовывозСТочкиВыдачи);
	// 01.08.19 Строганов Роман <
...
Функция ЗаполнитьПараметрыОбработкиСогласноОснованию(ПараметрФормированияСЗ, СтрокаП) экспорт
	...
	Параметры.Вставить("НеУчитыватьПокупателейНеработающихВВыходные", ЭтоВыходнойДень);
...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ЗаполнитьТаблицы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	|				КОНЕЦ
	|				И ВЫБОР
	|					КОГДА &ТолькоСамовывозСТочкиВыдачи
	|						ТОГДА (ЕСТЬNULL(СтрокаЗаявки.ПоследняяКорректировка.ТипДоставки, СтрокаЗаявки.Заявка.ТипДоставки)) = ЗНАЧЕНИЕ(Справочник.ТипыДоставки.СамовывозСТочкиВыдачи)
	//|						ТОГДА СтрокаЗаявки.Заявка.ТипДоставки = ЗНАЧЕНИЕ(Справочник.ТипыДоставки.СамовывозСТочкиВыдачи)
	//
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ
	|				И СтрокаЗаявки.СостояниеЗаявки <> &СтатусЗакрыт) КАК РезервыТоваровОстатки
	|ГДЕ
	...
	// 31.07.19 Строганов Роман > 
	Запрос.УстановитьПараметр("ТолькоСамовывозСТочкиВыдачи", Истина);
	// 31.07.19 Строганов Роман <
	...
	// 02.08.19 Строганов Роман > 
	Товары.Колонки.Добавить("СамовывозИмя"		, Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("СамовывозТелефон"	, Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("СамовывозСклад"	, Новый ОписаниеТипов("СправочникСсылка.Склады"));

	КэшЗаявок = Новый Соответствие;
	// 02.08.19 Строганов Роман <
	...
	Для Каждого СтрокаТовары Из Товары Цикл 
		...
		// 02.08.19 Строганов Роман > 
		ЗаявкаПокупателя = СтрокаТовары.ЗаявкаПокупателя;
		РеквизитыЗаявки = КэшЗаявок[ЗаявкаПокупателя];
		Если РеквизитыЗаявки = Неопределено Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "Выбрать Свойство, Значение Из РегистрСведений.Документ_ЗаявкаПокупателя_ДополнительныеРеквизиты Где Объект = &Объект И Свойство В (&СписокСвойств)";
			СписокСвойств = Новый Массив;
			СписокСвойств.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.СамовывозИмя);
			СписокСвойств.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.СамовывозСклад);
			СписокСвойств.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.СамовывозТелефон);
			Запрос.УстановитьПараметр("СписокСвойств", СписокСвойств);
			Запрос.УстановитьПараметр("Объект", ЗаявкаПокупателя);
			Выборка = Запрос.Выполнить().Выбрать();
			РеквизитыЗаявки = Новый Структура("СамовывозИмя, СамовывозТелефон, СамовывозСклад");
			КэшЗаявок.Вставить(ЗаявкаПокупателя, РеквизитыЗаявки);
			Пока Выборка.Следующий() Цикл
				СтрокаТовары[Выборка.Свойство.ИмяПредопределенныхДанных] = Выборка.Значение;
			КонецЦикла;
		КонецЕсли; 
	...
	Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
		...
		Если Не VIP И Не Параметры.ТипДоставки = Справочники.ТипыДоставки.Самовывоз И Не НеПроверятьМинОтгрузку И Не Параметры.НеУдалятьМинСум И (СуммаОтгрузки < СтрокаТЧ.МинимальнаяСуммаОтгрузки И Параметры.ТипДоставки <> Справочники.ТипыДоставки.ЭкспрессДоставка) И СтрокиРеализаций.Количество() = 0 Тогда  
			Исключен = Истина; 
...
Процедура ОбрезатьТоварыПоСтрокамЗаявки() 
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ОбрезатьТоварыПоСтрокамЗаявки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ОбрезатьПоТоварамНаСкладах()
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ОбрезатьПоТоварамНаСкладах";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ПодобратьПартииДляСПБ(МенеджерВТ)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ПодобратьПартииДляСПБ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ДобавитьВОчередьФормированияСлужебныхЗаданий(ПараметрыФормированияСЗ)
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Самовывоз_МодульОбъекта_ДобавитьВОчередьФормированияСлужебныхЗаданий";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
