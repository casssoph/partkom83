Процедура ВыполнитьРегламентноеЗадание() Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РАЗНОСТЬДАТ(ОчередьФормированияСлужебныхЗаданий.Параметры_ДатаЗапуска, ОчередьФормированияСлужебныхЗаданий.Параметры_ДатаОкончания, СЕКУНДА) КАК Приоритет
	               |ИЗ
	...
	Если Не Результат.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл 
			...
			// 26.07.19 Строганов Роман > XX-2980 Передавать время довыписки +20 минут 
			Если ПотокНеУспел Тогда
				ДатаОкончания = Выборка.ДатаОкончания + 20 * 60;
				ПотокНеУспел = Ложь;
			Иначе
			...
			Попытка
				Если Не ПотокНеУспел Тогда 
					...
					// 26.07.19 Строганов Роман > XX-2980 Передавать время довыписки +20 минут 
					СтруктураП.ДатаОкончания = ДатаОкончания;
					// 26.07.19 Строганов Роман < XX-2980 Передавать время довыписки +20 минут
