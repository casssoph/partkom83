...
Процедура СоздатьДокументНаСервере(ТабличнаяЧасть)
	...
	Для Каждого Стр Из НовПеремещение.Товары Цикл
		ДробнаяЧасть = Стр.Количество - Цел(Стр.Количество);
		Если ДробнаяЧасть>0 Тогда 
			МожноПроводить=Ложь;
			Прервать;
		КонецЕсли;	
		ДробнаяЧасть = Стр.КоличествоНеПринято - Цел(Стр.КоличествоНеПринято);
		Если ДробнаяЧасть>0 Тогда 
			МожноПроводить=Ложь;
			Прервать;
		КонецЕсли;	
		ДробнаяЧасть = Стр.КоличествоПлан - Цел(Стр.КоличествоПлан);
		Если ДробнаяЧасть>0 Тогда 
			МожноПроводить=Ложь;
			Прервать;
		КонецЕсли;	
	...
	Иначе
		Сообщить("Нельзя создавать перемещение с дробным количеством!");
	КонецЕсли;	
