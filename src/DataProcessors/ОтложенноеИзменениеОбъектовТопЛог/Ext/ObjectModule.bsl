	...
	Пока Выборка.Следующий() Цикл 
		Попытка
			НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
				...
				ИначеЕсли ТипЗнч(Выборка.Объект) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
					ОбработатьРТУ(Выборка.Объект);
				КонецЕсли;
...
Процедура ОбработатьРТУ(Объект)
	лКлючАлгоритма = "Обработка_ОтложенноеИзменениеОбъектовТопЛог_МодульОбъекта_ОбработатьРТУ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ГДЕ
	                |	МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.ДокументСсылка = &ДокументСсылка
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ РАЗЛИЧНЫЕ
	                |	МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка
	                |ПОМЕСТИТЬ МЗсНеотмененнымиРТУ
	                |ИЗ
	                |	МЗ КАК МЗ
	                |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.МегаЛогист_МаршрутноеЗадание.ДокументыРеализации КАК МегаЛогист_МаршрутноеЗаданиеДокументыРеализации
	                |		ПО МЗ.Ссылка = МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка
	                |ГДЕ
	                |	НЕ МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.ДокументСсылка.ПометкаУдаления
	                |	И НЕ МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.ДокументСсылка.ПолныйОтказ
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	МЗ.Ссылка,
	                |	МЗ.Ссылка.Проведен КАК Проведен
	                |ИЗ
	                |	МЗ КАК МЗ
	                |		ЛЕВОЕ СОЕДИНЕНИЕ МЗсНеотмененнымиРТУ КАК МЗсНеотмененнымиРТУ
	                |		ПО МЗ.Ссылка = МЗсНеотмененнымиРТУ.Ссылка
	                |ГДЕ
	                |	МЗсНеотмененнымиРТУ.Ссылка ЕСТЬ NULL";
	Запрос.УстановитьПараметр("ДокументСсылка", Объект);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл 
		МЗОбъект = Выборка.Ссылка.ПолучитьОбъект();
		МЗОбъект.Статус = Перечисления.МегаЛогист_СтатусыМаршрутныхЗаданий.Отменен;
		Если Выборка.Проведен Тогда 
			МЗОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			МЗОбъект.Записать(РежимЗаписиДокумента.Запись);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
