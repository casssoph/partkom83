...
Процедура ПеревестиВСтатусПопыткаОтмены()
	...
	|				И НЕ ИдентификаторСобытияПрошлойСменыСтатуса = &ПустаяСтрока) КАК СобытияАктовРассмотренияВозвратаСрезПоследних
	|ГДЕ
	|	СобытияАктовРассмотренияВозвратаСрезПоследних.АктРассмотренияВозврата.СрокВозвратаКлиента <= &КонечныйСрокВозврата";
	
	Запрос.УстановитьПараметр("СтатусДокумента", 	Справочники.СтатусыДокументов.АРВ_ГВИстечениеСроков);
	
	// От текущей даты отнять 2 рабочих дня
	...
	КонечныйСрокВозврата = НачалоДня(ТекущаяДата()) - КоличествоДней * 86400;
	
	Запрос.УстановитьПараметр("КонечныйСрокВозврата", КонечныйСрокВозврата);
	Запрос.УстановитьПараметр("ПустаяСтрока", 		"");
