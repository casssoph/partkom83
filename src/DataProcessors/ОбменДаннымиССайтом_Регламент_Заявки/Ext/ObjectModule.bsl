//Входящие функции
Процедура ЗарегистрироватьСообщениеВИсторииОбменаССайтом_ЗаявкиПокупателей(ИдентификаторОтправителя, ИдентификаторПолучателя, СообщениеОбмена, НомерПринятого) Экспорт
...
Процедура ВыполнитьРегламентноеЗадание() Экспорт
	...
	Если ОбъектыXDTO.Количество() > 0 Тогда
		ЗарегистрироватьСообщениеВИсторииОбменаССайтом_ЗаявкиПокупателей(КодОтправителя, КодПолучателя, ОтветСервера, НомерПринятого);
		ОбработатьПакетЗаявокССайта(ОбъектыXDTO, URI, НомерСообщения);
	КонецЕсли;
...
КонецПроцедуры
Функция ОбработатьПакетЗаявокССайта(СписокОбъектовXDTO, URI, НомерСообщения) Экспорт
	...
	Для Каждого ОбъектXDTO Из СписокОбъектовXDTO цикл
		Если ОбъектXDTO.Тип() = ФабрикаXDTO.Тип(URI, "Документы.ЗаявкаПокупателя") Тогда
			СтруктураДокумента = СтруктураДокументаЗаявкаПокупателя(ОбъектXDTO);
			КонтрольЗаполненияДокумента(СтруктураДокумента);
			ЗаписатьДокументЗаявки(СтруктураДокумента);
			СформироватьСамовывозПоЗаявке(СтруктураДокумента);
			ЗаписатьЛогЗагрузкиДокумента(СтруктураДокумента, НомерСообщения);
		ИначеЕсли ОбъектXDTO.Тип() = ФабрикаXDTO.Тип(URI, "Документы.ЗаявкаПокупателяОтмена") Тогда
			...
			ЗаписатьЛогОтменыЗаявки(СтруктураОбъекта, НомерСообщения);
		КонецЕсли;
	КонецЦикла;
...
//Загрузка заявок
Функция СтруктураДокументаЗаявкаПокупателя(ОбъектXDTO)
	...
	Для Каждого СтрокаXDTO Из ОбъектXDTO.items.ПолучитьСписок("item") Цикл
		...
		УстановитьРеквизитыНоменклатуры(СтруктураОбъекта, СтрокаТовары, СтрокаXDTO.Номенклатура);
		...
		СтруктураОбъекта.РеквизитыПоставщика.Вставить(СтрокаТовары.IDSite, Новый Структура("ИзготовительПоставщика,АртикулПоставщика", СтрокаXDTO.Номенклатура_ИзготовительПоставщика, СтрокаXDTO.Номенклатура_АртикулПоставщика));
...
КонецПроцедуры
Процедура ЗаписатьДокументЗаявки(СтруктураДокумента)
...
КонецПроцедуры
Процедура ЗаписатьЛогЗагрузкиДокумента(СтруктураДокумента, НомерСообщения)
	
	Запись = РегистрыСведений.ЛогОбменовССайтом.СоздатьМенеджерЗаписи();
	Запись.ПланОбмена = Перечисления.ПланыОбменаССайтом.ПодтвержениеПолученияЗаявок;
	...
	Если СтруктураДокумента.ОшибкаЗаполненияСроковЗаказа Тогда
		ТекстСообщения = "От сайта пришла подтвержденная заявка с незаполненными сроками заказа (Заявка: <" + СтруктураДокумента.СсылкаНаДокумент + ">, (" + СтруктураДокумента.ТекстОшибкиЗаполненияСроковЗаказа + ")";
	...
	Если СтруктураДокумента.ОшибкаСозданияСамовывоза Тогда
		ТекстСообщения = "Ошибка создания реализации(самовывоз) по заявке: <" + СтруктураДокумента.СсылкаНаДокумент + ">, (" + СтруктураДокумента.ТекстОшибкиСозданияСамовывоза + ")";
...
Функция ДокументРеализацииДляСамовывоза(ДокументЗаявки)
	...
	Документ.КонтрагентВзаиморасчетов = Документ.Контрагент;
	Документ.ДоговорКонтрагентаВзаиморасчетов = Документ.ДоговорКонтрагента;
...
КонецПроцедуры 
Процедура ЗаписатьЛогОтменыЗаявки(СтруктураДокумента, НомерСообщения)
	
	Запись = РегистрыСведений.ЛогОбменовССайтом.СоздатьМенеджерЗаписи();
	Запись.ПланОбмена = Перечисления.ПланыОбменаССайтом.ПодтверждениеОтменыЗаявок;
...
Процедура УстановитьРеквизитыКонтрагента(СтруктураОбъекта, UUIDДоговора)
	...
	ДоговорыИз1С77 = РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Сайт", "Договоры 1С77", Истина);
	Если ДоговорыИз1С77 Тогда
		ДанныеДоговора = Договор1С7(UUIDДоговора, СтруктураОбъекта.Контрагент, СтруктураОбъекта.Дата);
	Иначе
		ДанныеДоговора = Договор1С8(UUIDДоговора, СтруктураОбъекта.Дата);
	КонецЕсли;
	...
	Иначе
		СтруктураОбъекта.Отказ = Истина;
		ТекстОшибки = "Не определен договор по UUID: <" + UUIDДоговора + "> - " + ?(ДоговорыИз1С77, "1С77", "1С8");
		ДополнитьОшибку(СтруктураОбъекта.ТекстОшибки, ТекстОшибки);
	КонецЕсли;
	
КонецПроцедуры
Функция Договор1С7(UUIDДоговора, Контрагент, Дата)
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ВЫРАЗИТЬ(_ДляПереносаДанных.Объект КАК Справочник.ДоговорыКонтрагентов) КАК Договор
	                      |ПОМЕСТИТЬ Договоры
	                      |ИЗ
	                      |	РегистрСведений._ДляПереносаДанных КАК _ДляПереносаДанных
	                      |ГДЕ
	                      |	_ДляПереносаДанных.Строка77 = &UUIDДоговора
	                      |;
	                      |
	                      |////////////////////////////////////////////////////////////////////////////////
	                      |ВЫБРАТЬ
	                      |	Договоры.Договор КАК ДоговорКонтрагента,
	                      |	Договоры.Договор.ГлубинаКредита КАК ГлубинаКредита,
	                      |	Договоры.Договор.ВидРасчетаДней КАК ВидРасчетаДней,
	                      |	Договоры.Договор.Организация КАК Организация,
	                      |	Договоры.Договор.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	                      |	Договоры.Договор.ТипЦен КАК ТипЦен,
	                      |	ВЫБОР
	                      |		КОГДА Договоры.Договор.Организация.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ФизЛицо)
	                      |			ТОГДА ЛОЖЬ
	                      |		ИНАЧЕ ЕСТЬNULL(Договоры.Договор.Организация.УчитыватьНДС, ЛОЖЬ)
	                      |	КОНЕЦ КАК УчитыватьНДС,
	                      |	ВЫБОР
	                      |		КОГДА Договоры.Договор.Организация.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо)
	                      |				И ЕСТЬNULL(Договоры.Договор.Организация.УчитыватьНДС, ИСТИНА)
	                      |			ТОГДА ИСТИНА
	                      |		ИНАЧЕ ЛОЖЬ
	                      |	КОНЕЦ КАК СуммаВключаетНДС,
	                      |	ВЫБОР
	                      |		КОГДА Договоры.Договор = Договоры.Договор.Владелец.ОсновнойДоговорКонтрагента
	                      |				И Договоры.Договор.ДоговорНаОферту
	                      |				И Договоры.Договор.ДатаДоговораОферты <> ДАТАВРЕМЯ(1, 1, 1)
	                      |			ТОГДА 0
	                      |		ИНАЧЕ 1
	                      |	КОНЕЦ КАК Приоритет,
	                      |	ВЫБОР
	                      |		КОГДА Договоры.Договор.Владелец.НашРасчетныйСчет <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка)
	                      |				И Договоры.Договор.Владелец.НашРасчетныйСчет.Владелец = Договоры.Договор.Организация
	                      |			ТОГДА ЕСТЬNULL(Договоры.Договор.Владелец.НашРасчетныйСчет, ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка))
	                      |		ИНАЧЕ ЕСТЬNULL(Договоры.Договор.Организация.ОсновнойБанковскийСчет, ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка))
	                      |	КОНЕЦ КАК БанковскийСчет
	                      |ПОМЕСТИТЬ ОсновныеРеквизиты
	                      |ИЗ
	                      |	Договоры КАК Договоры
	                      |ГДЕ
	                      |	Договоры.Договор ССЫЛКА Справочник.ДоговорыКонтрагентов
	                      |	И Договоры.Договор.Владелец = &Контрагент
	                      |	И Договоры.Договор.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПокупателем)
	                      |	И НЕ Договоры.Договор.ПометкаУдаления
	                      |	И НЕ Договоры.Договор.СлужебныйДоговор
	                      |	И НЕ Договоры.Договор.ДоговорПриостановлен
	                      |	И Договоры.Договор.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	                      |;
	                      |
	                      |////////////////////////////////////////////////////////////////////////////////
	                      |ВЫБРАТЬ
	                      |	ОсновныеРеквизиты.ДоговорКонтрагента,
	                      |	ОсновныеРеквизиты.ГлубинаКредита,
	                      |	ОсновныеРеквизиты.ВидРасчетаДней,
	                      |	ОсновныеРеквизиты.Организация,
	                      |	ОсновныеРеквизиты.ВалютаВзаиморасчетов,
	                      |	ОсновныеРеквизиты.ТипЦен,
	                      |	ОсновныеРеквизиты.УчитыватьНДС,
	                      |	ОсновныеРеквизиты.СуммаВключаетНДС,
	                      |	ОсновныеРеквизиты.Приоритет,
	                      |	ОсновныеРеквизиты.БанковскийСчет,
	                      ...
	                      |		ПО ОсновныеРеквизиты.ВалютаВзаиморасчетов = КурсыВалютСрезПоследних.Валюта");
	Запрос.УстановитьПараметр("UUIDДоговора", UUIDДоговора);
...
Функция Договор1С8(UUID, Дата)
	...
	Запрос.УстановитьПараметр("Договор", Справочники.ДоговорыКонтрагентов.ПолучитьСсылку(Новый УникальныйИдентификатор(UUID)));
	Запрос.УстановитьПараметр("Период", Дата);
...
КонецПроцедуры
Процедура УстановитьРеквизитыНоменклатуры(СтруктураОбъекта, Строка, UUID)
	
	Строка.Номенклатура = НоменклатураПоUUID(Строка(UUID), СтруктураОбъекта);
...
Функция ДанныеНоменклатурыПоUUID(UUID, Отказ)
	...
	HTTPЗапрос.УстановитьТелоИзСтроки("uuid=" + UUID, КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
...
КонецФункции
Функция НоменклатураПоUUID(UUID, СтруктураОбъекта)
