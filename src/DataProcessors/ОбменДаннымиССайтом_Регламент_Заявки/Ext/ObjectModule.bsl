Процедура ВыполнитьРегламентноеЗадание(АвтоТест = Ложь, ТестовоеСообщение = "") Экспорт
	...
	Попытка
		//)Семенов И.П.
		Если АвтоТест И ЗначениеЗаполнено(ТестовоеСообщение) Тогда
			// 17.05.19 Строганов Роман > 
	...
	// Переключатель на новую схему загрузки заявок в несколько потоков.
	// Для отключения нужно присвоить значение «Истина» настройке «Загружена тестовая порция заявок покупателей» в регистре сведений «Настройка подсистем». 
	ИспользуетсяСтараяСхемаЗагрузкиЗаявок = РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Сайт", "Загружена тестовая порция заявок покупателей");
	...
	Если ОбъектыXDTO.Количество() > 0 Тогда
		...
		// 07.03.19 Строганов Роман > XX-2047 Ускорить загрузку Заявки Покупателя
		Если ИспользуетсяСтараяСхемаЗагрузкиЗаявок Тогда
		...
		Иначе
			...
			//Семенов И.П. 31.01.2019 XX-1768(
			ОбменДаннымиКлиентСервер.ЗафиксироватьСообщениеВИсторииОбмена(НомерСообщения,Отправитель,РаспакованноеСообщение,,,,ДатаСообщения, ОбъектыXDTO.Количество());
			//)Семенов И.П.
			
			ОбменССайтомСервер.РазобратьПолученныеОбъекты(ОбъектыXDTO, НомерСообщения, "ОбменПартКом83_Сайт_состояние_заявок", ЧислоПотоков, ДатаСообщения);
