...
Функция НайтиСоздатьДокументПоступления(вхДатаДокумента, вхКонтрагент, вхДоговорКонтрагента, вхПериод, вхСклад, СоздаватьНовый = Ложь) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Дата", НачалоДня(вхДатаДокумента));
	...
	Для Каждого Прайс Из ПрайсыПоставщиковРасписание Цикл
		...
		Если ТабТоваров.Количество() > 0 Тогда
			...
			Для А = 0 По МассивДатПрихода.Количество()-1 Цикл
				...
				тДокумент = НайтиСоздатьДокументПоступления(НачалоДня(тДатаПрихода), 
				...
				Для Каждого Товар Из ТабТоваров Цикл
					...
					Если Прайс.Разделять Тогда
						Попытка
							Если Товар.ДатаПрихода <= КонецДня(тДатаПрихода)
								И Товар.ДатаПродажи >= тДатаПрихода И Товар.ДатаПродажи <= Прайс.ДатаКон
						...
						КонецПопытки;
					Иначе
						Если Товар.ДатаПродажи >= тДатаПрихода И Товар.ПопалВОтчет = 0 Тогда
							ДобавитьСтроку = Истина;
						Иначе
							Товар.ПопалВОтчет = 1;
							Продолжить;
...
Процедура ЗаполнитьСписокПрайсов(вхПоставщик, вхДата) Экспорт
	...
	Иначе
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Результат.Выгрузить(), ПрайсыПоставщиковРасписание);
		Если ДоДатыКон Тогда
			Для Каждого Стр Из ПрайсыПоставщиковРасписание Цикл
				Стр.ДатаНач = НачалоДня(ДатаДокумента);
				Стр.ДатаКон = КонецДня(ДатаКон);
				Стр.ДатаДокумента = ДатаДокумента;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтменитьПроведениеДокументов()
	Для Каждого Прайс Из ПрайсыПоставщиковРасписание Цикл
		ДокументОбъект = НайтиСоздатьДокументПоступления(НачалоДня(Прайс.ДатаДокумента), 
