...
Процедура СформироватьСписокПремийНаСервере()
	...
	Запрос.УстановитьПараметр("ФилиалыСПБ",				ПолучитьСписокОрганизацийСПб()); // Филиалы СПБ
...
&НаСервере
Функция ПолучитьСписокОрганизацийСПб()
	
	Запрос = Новый Запрос("Выбрать Ссылка Из Справочник.Организации Где Филиал = &СПб");
	Запрос.УстановитьПараметр("СПб", Справочники.Филиалы.СанктПетербург);
