...
Процедура СохранитьНастройку(Команда)
	...
	СохраненнаяНастройка.Вставить("СписокПоставщиков", Поставщики);
	СохраненнаяНастройка.Вставить("СписокИсключений", ПоставщикиИсключения);
...
Процедура ВосстановитьНастройку(Команда)
	...
	Если Результат <> Неопределено Тогда
		...
		СписокПоставщиков = Неопределено; 
		Если Результат.СохраненнаяНастройка.Свойство("СписокПоставщиков", СписокПоставщиков) Тогда
			Если Не СписокПоставщиков = Неопределено Тогда
				Поставщики = СписокПоставщиков;
			КонецЕсли;
		КонецЕсли;
		
		СписокИсключений = Неопределено;
		Если Результат.СохраненнаяНастройка.Свойство("СписокИсключений", СписокПоставщиков) Тогда
			Если Не СписокПоставщиков = Неопределено Тогда
				ПоставщикиИсключения = СписокИсключений;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокОрганизацийПриИзменении(Элемент)
	...
	Если СписокОрганизацийИсключений.НайтиПоЗначению(СписокОрганизаций[СписокОрганизаций.Количество()-1].Значение) <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Поставщик уже указан качестве исключения и не может быть использован для отбора.");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СписокОрганизацийИсключенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	//Если СписокОрганизаций.НайтиПоЗначению(СписокОрганизацийИсключений[СписокОрганизацийИсключений.Количество()-1].Значение) <> Неопределено Тогда
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Поставщик уже указан для отбора и не может быть использован в качестве исключения.");
...
&НаКлиенте
Процедура СписокОрганизацийИсключенийОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	а = 0;
КонецПроцедуры
