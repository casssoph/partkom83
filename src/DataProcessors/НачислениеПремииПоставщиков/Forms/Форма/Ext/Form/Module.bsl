...
Процедура СформироватьСписокПремийНаСервере()
	...
	|		ВТПоступления.СтрокаПрихода КАК СтрокаПрихода
	|	ИЗ
	|		ВТПоступления КАК ВТПоступления
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл			
		...
		КонецЕсли;
		МассивДокументов.Добавить(Новый Структура("Документ, СуммаСНДС, СуммаБезНДС, СуммаПремии, СтрокаПрихода", Выборка.ДокументСсылка, Выборка.СуммаСНДС, Выборка.СуммаБезНДС, Выборка.СуммаПремии));
	...
	ВыборкаСПб = РезультатЗапроса[3].Выбрать();
	...
	|	ВЗ.СуммаОтгрузки КАК СуммаОтгрузки
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТПоступления.Организация КАК Организация,
...
Процедура ОтгрузкиСПБ(Команда)
	...
	ОткрытьФорму("Обработка.НачислениеПремииПоставщиков.Форма.ФормаРасшифровки", Новый Структура("Адрес", АдресТаблицыДанных));
