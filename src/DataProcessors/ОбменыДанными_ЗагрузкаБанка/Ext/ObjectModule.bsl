...
Перем СписокДляОтправкиСообщений;
Перем ТекстОшибокОферты;
...
Функция СоздатьСтруктуруПараметровДляЗагрузки()
	...
	Если ОбщегоНазначения.ЭтоРабочаяИнформационнаяБаза() Тогда
		Если Косячные тогда
			КаталогОбмена = "\\1csql01-g9\1c_exchange\bank\_Copy\косяк";
		Иначе
			КаталогОбмена = "\\1csql01-g9\1c_exchange\bank\";
		КонецЕсли;
		
	Иначе
		КаталогОбмена = "\\1csql01-g9\1c_exchange\bank\Тест";
	...
	Если Косячные Тогда
		Параметры.Вставить("МаскаИмениФайлаЗагрузки", "116ошибка_банк_*.txt");
	Иначе
	...
	АдресаВФайлеЗагрузки.Вставить("Строка13","ИННорганизации");
	АдресаВФайлеЗагрузки.Вставить("Строка14","БИКбанка");
	АдресаВФайлеЗагрузки.Вставить("Строка15","ВалютаСчета");
	...
	Иначе
		Параметры.Вставить("ТекАдресКому_обОферте", "Marushchenko-MA@part-kom.ru"); //;Pushkin-DS@Part-kom.ru;Chikisheva-TY@part-kom.ru
	КонецЕсли;
...
Процедура ОпросКаталогаВыгрузкиБанковскихВыписок() экспорт
	...
	Если НЕ ПустаяСтрока(ТекстИтоговогоСообщения) Тогда
		ЗаголовокСообщения = "всего файлов на входе : " + СокрЛП(ываы);
		ЗаголовокСообщения = ЗаголовокСообщения + Символы.ПС + "обработано файлов : " + СокрЛП(cxad);
		ЗаголовокСообщения = ЗаголовокСообщения + Символы.ПС + "успешно : " + СокрЛП(cxadап);
		ЗаголовокСообщения = ЗаголовокСообщения + Символы.ПС + "с ошибками : " + СокрЛП(cxadно);
		ТекстИтоговогоСообщения = ЗаголовокСообщения + Символы.ПС + ТекстИтоговогоСообщения;
	КонецЕсли;	
	...
	//////#КонецЕсли
	Если НЕ ПустаяСтрока(ТекстИтоговогоСообщения) Тогда
		РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.Банк, 
		ТекстИтоговогоСообщения, "загрузка банковской выписки " + Формат(ТекущаяДата(), "ДЛФ=DDT"));
	КонецЕсли;
...
Функция	ОбработатьВходящийФайл(ИмяФайлаДляЗагрузки)
	...
	Если Текст_Количество <> КолСтрокВФайлеЗагрузки Тогда
		Если Текст_Количество <> КолСтрокВФайлеЗагрузки - 1 Тогда
			Если Текст_Количество <> КолСтрокВФайлеЗагрузки - 3 Тогда
				Вернуться = Истина;
	...
	Иначе
		//создать р/с организации
		РезультатЗапроса = ЗапросПоИННОрганизации(СтрТЗ.ИННорганизации);
		НеСозданНовыйСчет = Истина;
		Если РезультатЗапроса.Количество() > 0 Тогда
			СтрТЗ.Организация = РезультатЗапроса[0].Ссылка;
			РезультатЗапроса = ЗапросПоБИКБанка(СтрТЗ.БИКбанка);
			Если РезультатЗапроса.Количество() > 0 Тогда
				БанкОрганизации = РезультатЗапроса[0].Ссылка;
				НовыйСчет = Справочники.БанковскиеСчета.СоздатьЭлемент();
				НовыйСчет.Банк = БанкОрганизации;
				НовыйСчет.ВидСчета = "Расчетный";
				ВалютаСчета = Неопределено;
				Если СтрДлина(СтрТЗ.ВалютаСчета) = 3 Тогда
					ВалютаСчета = Справочники.Валюты.НайтиПоКоду(СтрТЗ.ВалютаСчета);
				КонецЕсли;
				Если ВалютаСчета = Неопределено Тогда
					НовыйСчет.ВалютаДенежныхСредств = Константы.ВалютаРубль.Получить();
				КонецЕсли;
				НовыйСчет.Владелец = СтрТЗ.Организация;
				НовыйСчет.Наименование = БанкОрганизации.Наименование + " " + СтрТЗ.Организация.Наименование;
				НовыйСчет.НомерСчета = СтрТЗ.БанковскийСчет_Номер;
				Попытка
					НовыйСчет.Записать();
				    стрТЗ.СчетОрганизации = НовыйСчет.Ссылка;
					СтрТЗ.ВалютаДокумента = НовыйСчет.ВалютаДенежныхСредств;
					НеСозданНовыйСчет = Ложь;
				Исключение
					НеСозданНовыйСчет = Истина;
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
		Если НеСозданНовыйСчет Тогда
			Рез.Вставить("КодОшибки",4);
			стрИнф = ""+ПравилаИгры.ТекущаяДата+" Не загружена банковская выписка - из-за отсутствия расчетного счета организации с номером: "+стрТЗ.БанковскийСчет_Номер + Символы.ПС ;
			стрИнф = "*** содержимое файла выгрузки" + MSG;
			глЗаписьИнформации_ДляОтправкиПоЭлПочте(стрИнф);
			стрИнф = "*** содержимое файла выгрузки" + Символы.ПС + MSG + Символы.ПС + "*** конец файла.";
			ТекстОшибки = стрИнф + Символы.ПС + "! поиск осуществлялся по номеру счета - " + стрТЗ.БанковскийСчет_Номер + Символы.ПС;
	...
	Если Найти(стрТЗ.Прочее, "Ложь") > 0 тогда
		...
		//	Если стрТЗ.Контрагент <> РезультатЗапроса[0].ГоловнойКонтрагент Тогда
		//		стрТЗ.Контрагент = РезультатЗапроса[0].ГоловнойКонтрагент;
		//	КонецЕсли;
		текКА = ЗапросПоКодуКонтрагента(стрТЗ);
		Если ЗначениеЗаполнено(текКА) тогда
	 		стрТЗ.Контрагент = текКА;
			
		Иначе
			Рез.Вставить("КодОшибки",4);
			стрИнф = ""+ПравилаИгры.ТекущаяДата+" Не загружена банковская выписка - из-за отсутствия Контрагента [" + СокрЛП(стрТЗ.Контрагент_Наименование) + "] с кодом : " + стрТЗ.Контрагент_Код + Символы.ПС;
			стрИнф = "*** содержимое файла выгрузки" + Символы.ПС + MSG + Символы.ПС + "*** конец файла.";
			глЗаписьИнформации_ДляОтправкиПоЭлПочте(стрИнф);
			ТекстОшибки = СтрИнф + Символы.ПС + "! поиск осуществлялся по коду - " + стрТЗ.Контрагент_Код + Символы.ПС;
			Рез.Вставить("ТекстОшибки",ТекстОшибки);
	...
	Если Найти(стрТЗ.Прочее, "Ложь") > 0 тогда
		Если ЗначениеЗаполнено(стрТЗ.Контрагент) тогда
			...
			Иначе
				...
				стрИнф = ""+ПравилаИгры.ТекущаяДата+" Не загружена банковская выписка - из-за отсутствия Договора с [" + СокрЛП(стрТЗ.Организация) + "] у контрагента [" + СокрЛП(стрТЗ.Контрагент_Наименование) + "] с кодом : "+стрТЗ.Контрагент_Код + Символы.ПС;
				стрИнф = "*** содержимое файла выгрузки" + Символы.ПС + MSG + Символы.ПС + "*** конец файла.";
				глЗаписьИнформации_ДляОтправкиПоЭлПочте(стрИнф);
				ТекстОшибки = СтрИнф + Символы.ПС + "! Нет договора. ";
				Рез.Вставить("ТекстОшибки",ТекстОшибки);
...
Функция ЗапросПоИННОрганизации(ИНН)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Организации.Ссылка
	|ИЗ
...
Функция ЗапросПоКодуКонтрагента(СтрокаТЗ)
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА К.Ссылка = К.ГоловнойКонтрагент
	|			ТОГДА К.Ссылка
	|		ИНАЧЕ К.ГоловнойКонтрагент
	|	КОНЕЦ КАК Контрагент,
	|	ВЫБОР
	|		КОГДА К.Ссылка = К.ГоловнойКонтрагент
	|			ТОГДА К.Наименование
	|		ИНАЧЕ К.ГоловнойКонтрагент.Наименование
	...
	Запрос.УстановитьПараметр("Код", СтрокаТЗ.Контрагент_Код);
	Результат = Запрос.Выполнить();
	
	текКА = Справочники.Контрагенты.ПустаяСсылка();
	Если НЕ Результат.Пустой() Тогда
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		текКА = Выборка.Контрагент;
		//Если СокрЛП(Выборка.Наименование) = СокрЛП(СтрокаТЗ.Контрагент_Наименование) Тогда
		//	Возврат текКА;
		//КонецЕсли;
	...
	//|	Справочник.Контрагенты КАК К
	//|ГДЕ
	//|	К.Наименование ПОДОБНО &Наименование
	//|	И НЕ К.ПометкаУдаления";
	//	
	//Запрос.УстановитьПараметр("Наименование", СокрЛП(СтрокаТЗ.Контрагент_Наименование));
	//Результат = Запрос.Выполнить();
	//	
	...
	//	
	//Выборка = Результат.Выбрать();
	//текКА_Наим = Справочники.Контрагенты.ПустаяСсылка();
	//Пока Выборка.Следующий() Цикл
	//	Если СокрЛП(Выборка.Наименование) = СокрЛП(СтрокаТЗ.Контрагент_Наименование) Тогда
	//		текКА_Наим = Выборка.Контрагент;
	//		Прервать;
	//	КонецЕсли;
	//		
	//КонецЦикла;
	//
	//Если ЗначениеЗаполнено(текКА) Тогда
	//	Если ЗначениеЗаполнено(текКА_Наим) Тогда
	//		Возврат текКА_Наим;
	//	Иначе
	//		Возврат текКА;
	//	КонецЕсли;
	...
	//	|	ТорговыеТочки.Владелец.ГоловнойКонтрагент КАК ГоловнойКонтрагент
	//	|ИЗ
...
Функция АнализДоговоров(стрТЗ)
	...
	//дата акцепта договора оферты = дата платежного документа
	//ТекДатаДок = ПравилаИгры.ТекущаяДата; 
	т_Год = Число(Прав(СтрТЗ.ДатаДок,4));
	т_Месяц = Число(Сред(СтрТЗ.ДатаДок,4,2));
	т_День = Число(Лев(СтрТЗ.ДатаДок,2));
	ТекДатаДок = Дата(т_Год, т_Месяц, т_День);
		...
		//==============================================================================
			...
			Если ЗначениеЗаполнено(ТекДоговор) тогда
				Если  ТекДоговор.ДоговорНаОферту И                                           
					  (НЕ ТекКА_флОферта_ПроверенСтарыйДоговор) И (НЕ СозданНовыйДоговор) тогда
					...
					Иначе
						...
						Иначе
							...
							СтарыйДоговор 	= тзДолги.Получить(0).ДоговорКонтрагента;
							СтарыйДолг		= тзДолги.Получить(0).СуммаУпрОстаток;
							тмп_НадоЗаменитьДоговор = Истина;
			...
			Если СтрокиСДоговорами.Количество() > 0 Тогда
				...
				Для каждого Договора из СтрокиСДоговорами цикл 
					...
					ИначеЕсли спрДог.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком Тогда
						//Если Договора.ДоговорНаСток ИЛИ Договора.ДоговорНаКросс ИЛИ Договора.ДоговорНаПополнение Тогда
...
Функция ЗапросПоДоговорам(стрТЗ)
	...
	|	ДоговорыКонтрагентов.ДоговорНаПополнение,
	...
	Если НЕ ЗначениеЗаполнено(стрТЗ.Контрагент.Организация) тогда
		Запрос.Текст = Запрос.Текст + " И ДоговорыКонтрагентов.СлужебныйДоговор = Ложь ";
...
Функция СформироватьПлатежныеПорученияНаОснованииСтрокВыписки(СтрокаВыписки, пар_MSG = "")
	...
	Иначе
		//30.06.16 если документ был разделен на несколько, то его не нужно перезаписывать
		...
		Если ВидДокумента = "СтрокаВыпискиРасход" тогда
			ЗапросОснования.УстановитьПараметр("ДокументОснование", ДокументОбъект.Ссылка);
			Если ЗапросОснования.Выполнить().Выгрузить().Количество() > 0 Тогда
				//документ был разделен
	...
	ДатаВыписки = Дата(т_Год, т_Месяц, т_День, т_Час, т_Минута, т_Секунда);
	Если ДатаВыписки < Константы.ДатаНачалаРаботыВзаиморасчеты.Получить() Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ДокументОбъект.Дата = ДатаВыписки;
	Если НачалоДня(ДокументОбъект.Дата) < НачалоДня(ПравилаИгры.ТекущаяДата) тогда
	...
	ВалютаВзаиморасчетов = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ДокументОбъект.ДоговорКонтрагента, "ВалютаВзаиморасчетов");
	...
	Если ДокументОбъект.Дата < '20190101000000' Тогда
		СтрокаПлатеж.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;
	Иначе
		СтрокаПлатеж.СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
	КонецЕсли;
	СтруктураКурсаВзаиморасчетов = МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаВзаиморасчетов, ПравилаИгры.ТекущаяДата);
	...
	Иначе
		СтруктураКурсаВалютыДокумента = МодульВалютногоУчета.ПолучитьКурсВалюты(ДокументОбъект.ВалютаДокумента, ПравилаИгры.ТекущаяДата);
		СтрокаПлатеж.СуммаВзаиморасчетов = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаПлатеж.СуммаПлатежа, 
			ДокументОбъект.ВалютаДокумента, ВалютаВзаиморасчетов,
			СтруктураКурсаВалютыДокумента.Курс,  СтрокаПлатеж.КурсВзаиморасчетов,
			СтруктураКурсаВалютыДокумента.Кратность, СтрокаПлатеж.КратностьВзаиморасчетов);
	...
	Иначе
		...
		Если Проводить ИЛИ Записывать тогда	
			...
			Исключение
				//все равно еще раз попробуем записать документ
				Попытка
					ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
				Исключение
					ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС + "вторая попытка записи документа неудачна";
					ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС + "Не удалось " + ?(Проводить, "провести", "записать") + " документ " + СокрЛП(ДокументОбъект.Ссылка)+".";
					ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС +"Необходимо попытаться провести его самостоятельно, чтобы увидеть причину,";
					ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС +"т.к. он не проведен не из-за блокировки базы.";
				КонецПопытки;
		...
		Иначе
			Попытка
				...
				КонецЕсли;
			Исключение
				ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС + "Не удалось распровести документ " + СокрЛП(ДокументОбъект.Ссылка)+".";
				ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС + "Необходимо попытаться распровести его самостоятельно, чтобы увидеть причину,";
				ТекстИтоговогоСообщения = ТекстИтоговогоСообщения + Символы.ПС + "т.к. он не проведен не из-за блокировки базы.";
			КонецПопытки;	
		КонецЕсли;
	КонецЕсли;
...
Процедура ДописатьТекстОшибокОферты(парам_СтрокаУведомления)	
	Если ПустаяСтрока(ТекстОшибокОферты) Тогда
		ТекстОшибокОферты = парам_СтрокаУведомления;
		
	Иначе
		ТекстОшибокОферты = ТекстОшибокОферты + Символы.ПС + Символы.ПС + "________________________________________________________________________________" + Символы.ПС;
		ТекстОшибокОферты = ТекстОшибокОферты + Символы.ПС + парам_СтрокаУведомления;
...
Процедура глЗаписьИнформации_ДляОтправкиПоЭлПочте(парам_СтрокаУведомления, ОтправитьЗаинтересованномуЛицу = Ложь, ОтправитьЗаинтересованномуЛицуДляОтладки = Ложь)
	...
	//	СписокАдресов.Добавить("Golubev-SV@Part-Kom.ru");
	//	СписокАдресов.Добавить("Kuznetsova-AI@part-kom.ru");
	//	СписокАдресов.Добавить("Norenkova-NM@part-kom.ru");
	//	СписокАдресов.Добавить("Tuktamysheva-RA@part-kom.ru");
	...
	//Если НЕ ОтправитьЗаинтересованномуЛицу Тогда
		//РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(ТекстПисьма, ТемаПисьма, СписокАдресов);
	//КонецЕсли;
	
КонецПроцедуры

Процедура ОтправитьСообщенияОбОшибках()
	Если НЕ ПустаяСтрока(ТекстОшибокОферты) Тогда
		
		РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаАкцептаОферты,ТекстОшибокОферты, "ошибки при загрузке акцепта оферты");
...
Запрос = Новый Запрос(
"ВЫБРАТЬ
|	НастройкиОтправкиСообщенийОбОшибках.АдресЭлПочты КАК Значение,
|	НастройкиОтправкиСообщенийОбОшибках.Пользователь.Наименование КАК Представление
|ИЗ
...
Запрос.УстановитьПараметр("Событие", Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаАкцептаОферты);
СписокДляОтправкиСообщений = Новый СписокЗначений;
Для Каждого Стр Из Запрос.Выполнить().Выгрузить() Цикл
	СписокДляОтправкиСообщений.Добавить(Стр.Значение, Стр.Представление);
