...
Запрос.Текст = 	
"ВЫБРАТЬ
|	&ПараметрДвижения КАК ВидВзаиморасчетов,
|	РасчетыСКонтрагентамиОстатки.ДоговорКонтрагента КАК ДоговорКонтрагента,
|	ПоследнееДвижение.Период КАК ДатаВозникновения,
|	РасчетыСКонтрагентамиОстатки.СуммаУпрОстаток"+?(ТипЗадолженности="<0","*(-1)","")+" КАК СуммаВзаиморасчетов,
...
|				ТОГДА РасчетыСКонтрагентамиОстатки.СуммаУпрОстаток"+?(ТипЗадолженности="<0","*(-1)","")+"
...
|				ТОГДА РасчетыСКонтрагентамиОстатки.СуммаУпрОстаток"+?(ТипЗадолженности="<0","*(-1)","")+" * КурсыДоговоры.Курс * &КратностьДокумента / (&КурсДокумента * КурсыДоговоры.Кратность)
|			ИНАЧЕ 0
|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаПлатежа,
|	ЛОЖЬ КАК ДвиженияДокумента,
|	ПоследнееДвижение.ВидДвижения,
|	РасчетыСКонтрагентамиОстатки.ДокументРасчетов КАК ДокументРасчетовСКонтрагентом,
|	РасчетыСКонтрагентамиОстатки.ДокументРасчетов.ДатаОплаты КАК ДокументРасчетовДатаОплаты
|ИЗ
|	РегистрНакопления.Взаиморасчеты.Остатки(&ДатаПлатежа, ДоговорКонтрагента.Владелец = &Контрагент) КАК РасчетыСКонтрагентамиОстатки
...
|		ИЗ
|			РегистрНакопления.Взаиморасчеты КАК РасчетыСКонтрагентами
|		ГДЕ
...
|	ДокументРасчетовДатаОплаты";
	...
	Запрос.УстановитьПараметр("ПараметрДвижения",?(ТипЗадолженности="<0","Дебиторская", "Кредиторская"));
...
Процедура ЗаполнитьРасшифровкуПоДолгам(ПодборПоСуммеПлатежа=Истина,КурсПоДоговору=Истина) Экспорт
	...
	//Если РеглУчет Тогда
		...
		Для Каждого СтрокаПлатеж Из РасшифровкаПлатежаДок Цикл
			...
			УправлениеДенежнымиСредствами.ЗаполнитьРеквизитыРеглУчета(ДокументСсылка,СтрокаПлатеж, СтавкаНДС,Неопределено);
...
|	Взаиморасчеты.СуммаУпрОстаток,
|	Взаиморасчеты.ДокументРасчетов
|ИЗ
|	РегистрНакопления.Взаиморасчеты.Остатки(, ДоговорКонтрагента = &ДоговорКонтрагента {(ДоговорКонтрагента).*}) КАК Взаиморасчеты";
