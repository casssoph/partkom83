...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветФонаУправляющегоПоля = ЦветаСтиля.ЦветФонаПоля;
	ЦветФонаФормы            = ЦветаСтиля.ЦветФонаФормы;
	АвтоЦвет                 = Новый Цвет;
	СтранаРоссия = Справочники.СтраныМира.Россия;
	//СтранаРоссия = УправлениеКонтактнойИнформациейСлужебный.СтранаРоссия();
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Параметры.ЗначенияПолей) 
		...
		Если РезультатыЧтения.Свойство("ТекстОшибки") Тогда
			...
			//XDTOКонтактная.Состав.Страна = "Россия";
		КонецЕсли;
	Иначе
				...
				//Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							...
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ, НСтр("ru = 'Все действия'"));
...
Процедура КомандаОК(Команда)
	...
	лКлючАлгоритма = "Обработка_ВводКонтактнойИнформации_Форма_ВводАдреса";
	...
	// + Пушкин 20190718 XX-2834
	
	//ХудинВВ 2507019 ХХ-25072019 ((
	СтруктураВозврата = ОбщегоНазначенияКлиентСервер.ПолучитьОтчетОбработку("ПолучитьГеоКоординаты", Ложь, Ложь);
	Обр = СтруктураВозврата.Объект;
	//Обр = Обработки.ПолучитьГеоКоординаты.Создать();
	//))
	
	Обр.Контрагент = Параметры.ОбъектКонтрагент;
	Обр.СтрокаАдреса = ПредставлениеАдреса;
	Обр.ВидКИ = ВидКонтактнойИнформации.Ссылка;
	
	рез = Обр.ПолучитьФорму().ОткрытьМодально();
	
	Если рез = Неопределено Тогда
	Иначе
		
		Если Рез.Проверено = Истина Тогда
			
			ПредставлениеАдреса = Рез.СтрокаАдреса;
			ПредставлениеАдресаИзменено = Истина;
			РазрешитьВводАдресаВСвободнойФорме = Истина;
			
			стр_Комментарий = "";
			Для каждого лстр из Рез.ТЗ цикл
				стр_Комментарий = стр_Комментарий + Символы.ПС + лстр.Описание + " = " + лстр.Значение;
			КонецЦикла;
			
			Комментарий = стр_Комментарий;
			Модифицированность = Истина;
			//Дом
			//Индекс
			//НаселенныйПункт
		...
		Иначе
			...
			Если ВидКонтактнойИнформации.Ссылка = Справочники.ВидыКонтактнойИнформации.АдресДоставкиКонтрагента Тогда
				Предупреждение("Запись данных не возможна. Необходимо продолжить редактирование.", 5, "Адрес не прошел валидацию");
				Возврат;
			КонецЕсли;
...
Процедура ИсторияИзменений(Команда)
	...
	ПараметрыФормы = Новый Структура("СписокКонтактнойИнформации, ВидКонтактнойИнформации", СписокКонтактнойИнформации, ВидКонтактнойИнформации.Ссылка);
...
Функция ЗаполнитьСписокКонтактнойИнформации(ВидКонтактнойИнформации, КонтактнаяИнформацияОписаниеДополнительныхРеквизитов, Контекст)
	...
	Для каждого СтрокаКонтактнойИнформации Из НайденныеСтроки Цикл
		...
		Иначе
			...
			КонтактнаяИнформация.ЗначенияПолей = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(XDTOИнформация);
			Если Контекст.Модифицированность Тогда
				КонтактнаяИнформация.ДействуетС = ТекущаяДатаСеанса();
			Иначе
				КонтактнаяИнформация.ДействуетС = СтрокаКонтактнойИнформации.ДействуетС;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	Если ЗначениеЗаполнено(Параметры.ОбъектКонтрагент) Тогда
		Если Модифицированность Тогда
			Контекст = КонтекстФормыКлиент();
			Результат = РезультатаВыбораПоОбновлениюФлагов(Контекст, ВозвращатьСписокЗначений);
			УправлениеКонтактнойИнформацией.СохранитьКонтактнуюИнформациюВРегистр(Параметры.ОбъектКонтрагент,Результат);
			//ЭтаФорма.ВладелецФормы.ЭлементыФормы.КИОбщая.ТекущиеДанные.Представление = Результат.ДанныеВыбора.Представление;
			//ОповеститьОВыборе(Результат);
			Оповестить("ЗаписанаКИ", Результат);
		Иначе
	...
	Иначе
		Если Модифицированность Тогда // При немодифицированности работает как "отмена".
			...
			#Если ТолстыйКлиентОбычноеПриложение Тогда
				Оповестить("ВведенаКИ",Результат);
			#Иначе
	...
	Если (МодальныйРежим Или ЗакрыватьПриВыборе) И Открыта() Тогда
		...
		СохранитьСостояниеФормы();
		Попытка
			Если НЕ Результат.ДанныеВыбора.Представление="" Тогда 
				Если Результат.ДанныеВыбора.Вид=Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента Тогда 
					НаборЗаписей = РегистрыСведений.КонтактнаяИнформация.СоздатьНаборЗаписей();	
					НаборЗаписей.Отбор.Объект.Значение = Параметры.ОбъектКонтрагент;
					НаборЗаписей.Отбор.Объект.Использование = Истина;
					НаборЗаписей.Отбор.Вид.Значение = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента;
					...
					Если НаборЗаписей.Количество()=0 Тогда 
						...
						СтрокаНабора.Представление=Результат.ДанныеВыбора.Представление;
						...
						СтрокаНабора.Объект=Параметры.ОбъектКонтрагент;
						Наборзаписей.Записать(Истина);
					ИначеЕсли НаборЗаписей.Количество()>0 Тогда 
						Если НаборЗаписей[0].Представление="" Тогда 
							НаборЗаписей.Очистить();
							СтрокаНабора=НаборЗаписей.Добавить();
							СтрокаНабора.Представление=Результат.ДанныеВыбора.Представление;
							СтрокаНабора.Вид=Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента;
						...
						КонецЕсли;	
					КонецЕсли;	
				ИначеЕсли Результат.ДанныеВыбора.Вид=Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента Тогда 	
					НаборЗаписей = РегистрыСведений.КонтактнаяИнформация.СоздатьНаборЗаписей();	
					НаборЗаписей.Отбор.Объект.Значение = Параметры.ОбъектКонтрагент;
					НаборЗаписей.Отбор.Объект.Использование = Истина;
					...
					Если НаборЗаписей.Количество()=0 Тогда 
						...
						СтрокаНабора=НаборЗаписей.Добавить();
						СтрокаНабора.Представление=Результат.ДанныеВыбора.Представление;
						СтрокаНабора.Вид=Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента;
						СтрокаНабора.Тип=Перечисления.ТипыКонтактнойИнформации.Адрес;
						СтрокаНабора.Объект=Параметры.ОбъектКонтрагент;
						Наборзаписей.Записать(Истина);
					ИначеЕсли НаборЗаписей.Количество()>0 Тогда 
