...
Функция ЗагрузитьПеремещение(НовоеИмя, Файл, ТекстОшибки) Экспорт 
	...
	Попытка
		...
		ТекстДок.ДобавитьСтроку("Номер перемещения в 1с8;Код склада отправителя;Код склада получателя;Количество строк;Итого количество");
		ТекстДок.ДобавитьСтроку(Док.Номер+";"+Число(Док.СкладОтправитель.Код)+";"+Число(Док.СкладПолучатель.Код)+";"+Док.Товары.Количество()+";"+Док.товары.Итог("Количество")+";");
		ТекстДок.Записать(ПутьДок);
	Исключение
		//ОтменитьТранзакцию();
		Возврат 2;
	КонецПопытки;
