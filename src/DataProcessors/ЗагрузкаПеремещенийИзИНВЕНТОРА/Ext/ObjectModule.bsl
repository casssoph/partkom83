...
Процедура ВыполнитьРегламентноеЗадание() Экспорт
	...
	Путь="\\srv1c-nn\DataBases_1C\Work_P1C\_CatalogИнвентор_Перемещения.1сk";
	ТекстДок.Прочитать(Путь, "cp866");//,"cp866"); //КодировкаТекста.ANSI
	СтрокаТФ=ТекстДок.ПолучитьСтроку(1);
	КаталогПоиска=СтрокаТФ;
...
Функция ЗагрузитьПеремещение(НовоеИмя, Файл, ТекстОшибки) Экспорт 
	...
	Для счТ = 2 По КолСтрок Цикл
		...
		Если ЗначениеЗаполнено(Ошибки_вСтроке) Тогда
			...
			|Ошибки в строке №" + счТ + ":" + Ошибки_вСтроке;
