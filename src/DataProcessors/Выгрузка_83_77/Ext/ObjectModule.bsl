...
Функция СформироватьТекстЗапросаПоЗарегистированнымОбъектам(пар_ВидОбъекта)
		...
		Если СтрНайти(пар_ВидОбъекта,"Документ")  > 0 тогда
			...
			|ГДЕ
			|	Т.Узел = &Узел И Т.Ссылка.Дата >= &КрайняяДатаВыгрузкиДокументовИз1с8в1с7 " + ?(ЗначениеЗаполнено(ПроблемныйДокДляТочечнойВыгрузки)," И Т.Ссылка = &ПроблемныйДок","  И (НЕ Т.Ссылка В (ВЫБРАТЬ сЦылка ИЗ ОШИБКИ)) ") + "
			
			// И (РазностьДат(&ТекДт,ЕСТЬNULL(ИСТОРИЯ.ДтПоследнейВыгрузки, &ТекДт),минута) = 0 или РазностьДат(&ТекДт,ЕСТЬNULL(ИСТОРИЯ.ДтПоследнейВыгрузки, &ТекДт),минута) > 60)
			
			|УПОРЯДОЧИТЬ ПО
			|	естьNULL(ИСТОРИЯ.Сч,0),
			//|	ЕСТЬNULL(ИСТОРИЯ.ДтПоследнейВыгрузки, &ТекДт) УБЫВ,
			|	Т.Ссылка.Дата,
			|	Т.Ссылка.Номер
			|";
...
Функция Запрос_ПолучитьЗначенияРеквизитовДокумента(пар_Ссылка, пар_имя, пар_что = "*")
	...
	Если СтрНайти(Запрос_Текст,доп_реквизит) > 0 тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,доп_реквизит, "естьNULL(Номенклатура.Изготовитель.Наименование,"""") КАК " + доп_реквизит);
	КонецЕсли;
