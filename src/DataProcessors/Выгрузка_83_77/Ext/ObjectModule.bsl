...
Функция ВыгрузитьДанныеПоЖеланию(пар_МассивСсылокРазличныхВидов, пар_ОпроситьСсылкиИзТЧ = Истина, пар_ОпроситьСсылкиИзРегПартий = Истина, пар_ОпроситьСсылкиИзРегПартийРазмещение = Истина) экспорт
	...
	Возврат Результат;
КонецФункции
Функция ВыгрузитьДанныеПоВодителям(пар_МассивСсылокРазличныхВидов) экспорт
	Результат = Новый Структура("Отказ, МассивДок",Истина,Новый ТаблицаЗначений);
	...
	Попытка 	
		...
		СформироватьФайлыВыгрузки(пар_МассивСсылокРазличныхВидов);
...
Процедура ОбойтиДокументыРекурсией(пар_Массив)
	...
	Для каждого лДок из пар_Массив цикл 
		...
		ИначеЕсли лстр.типвид = "Документ.ВозвратТоваровПоставщику" тогда
			мОснования = ПолучитьПартииДок(лстр.сцылка);
...
Процедура НайтиИуничтожитьПродублированныеФайлыВыгрузки(пар_ИмяФайла,пар_ВерсияДанных = "") экспорт
	...
	возможные_места_дислокации.Добавить("\\nng9-v-1c-07\ДляЗагрузкиОстатков\1c8_1c7\Test\");
...
Процедура СформироватьФайлыВыгрузки(пар_Массив = "")
	...
	Для Каждого ЭлементМассива из ВыборкаИзменений Цикл
		...
		лИгнорировать = л_adoURBD_ПроверитьВерсиюОбъекта(Данные_Ссылка_УникальныйИдентификатор, л_шапочка_данные.ВерсияДанных);
		Если лИгнорировать тогда
			Попытка
				ПланыОбмена.УдалитьРегистрациюИзменений(узел_получатель, Данные.Ссылка);
				ЛогироватьВыгрузку(Данные_Ссылка_УникальныйИдентификатор, л_шапочка_данные.ВерсияДанных, 3, СокрЛП(Данные.Ссылка));
			Исключение
			КОнецПопытки;
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли СтрНайти(НайденнаяСтрока.ОбъектМетаданных_ПолноеИмя,"Справочник") > 0 тогда
			УложитьДанныеВТекстовыйФайлик(содержимое_файла, РазвернутьДатуВстроку(ТекущаяДата(),,Ложь),ТекущийОбъектМетаданных.Имя,?(Число(л_шапочка_данные.Код)=0,Формат(сч, "ЧЦ=15; ЧДЦ=0; ЧРД=,; ЧГ=0"),л_шапочка_данные.Код));
		КонецЕсли;
...
Функция Запрос_ПолучитьЗначенияРеквизитовДокумента(пар_Ссылка, пар_имя, пар_что = "*")
	...
	доп_реквизит = "Водители_ЭтоГруппа";
	Если СтрНайти(Запрос_Текст,доп_реквизит) > 0 тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,доп_реквизит, "ЭтоГруппа КАК " + доп_реквизит);
	КонецЕсли;
	
	доп_реквизит = "Водители_Родитель";
	Если СтрНайти(Запрос_Текст,доп_реквизит) > 0 тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,доп_реквизит, "естьNULL(Родитель,Значение(Справочник.Водители.ПустаяСсылка)) КАК " + доп_реквизит);
	КонецЕсли;
	
	доп_реквизит = "Водители_ИмяРодителЯ";
	Если СтрНайти(Запрос_Текст,доп_реквизит) > 0 тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,доп_реквизит, "естьNULL(Родитель.Наименование,"""") КАК " + доп_реквизит);
	КонецЕсли;
	
	доп_реквизит = "Водители_ТТ";
	Если СтрНайти(Запрос_Текст,доп_реквизит) > 0 тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,доп_реквизит, "естьNULL(ТранспортнаяКомпания.ОсновнаяТорговаяТочка,Значение(Справочник.Контрагенты.ПустаяСсылка)) КАК " + доп_реквизит);
	КонецЕсли;
...
КонецПроцедуры
Функция л_adoURBD_ПроверитьВерсиюОбъекта(пар_УИД, пар_Версия)
	_Игнорировать = Ложь;
	...
	CommandText = "
					|select
					|	e1._id
					|   ,max(e1.current_vers) as current_vers
					|   ,max(e1.latest_step_current_vers) as latest_step_current_vers
					|   ,max(e1.latest_version) as latest_version
					|   ,max(e1.latest_version_dt) as latest_version_dt
					|   ,max(e1.fl) as fl
					|from
					|	(
					|	select 
					...
					|				   _id
					|				  ,_vers
					|				  ,max(_step) as latest_step
					|			FROM Guids_1C8_1C7_monitor whith (nolock)
					|			where
					|					_id = '" + СокрЛП(пар_УИД) + "'
					|					and _vers = '" + пар_Версия + "'
					|			group by
					|				   _id
					|				  ,_vers
					|		) as f1
					...
					//|			  ,''
					//|			  ,0
					//|			  ,t2._vers as  latest_version
					//|			  ,t2._dt as  latest_version_dt
					//|			  ,t2.fl
					//|		from
					//|			(
					//|				select top 1
					...
					//|				  ,t1._dt
					//|				  ,case 
					//|					  when t1._vers = '" + пар_Версия + "' then 1
					//|					  else 0
					//|				   end as fl 
					//|				from
					//|						(
					...
					//|								  _id
					//|								 ,_vers
					//|						) as t1 -- все выгруженные версии объекта
					//|			   order by 
					//|				  t1._dt desc	
					//|			) as t2 -- последняя выгруженная версия с признаком, является ли она текущей
					|	) as e1
					|group by
					|	e1._id
	|";
			...
			//    Подключение_к_SQL_серверу_2();
				...
				Если тчРез.Колонки.Количество() = 0 тогда
					Для ii = 0 ПО ВыборкаSQL_2.Fields.Count-1 Цикл
						...
						имя_К = инд_К;
						//Если ii > 5 тогда
						//	тзДанные77.Колонки.Добавить(инд_К,Новый ОписаниеТипов("Число"));
						//ИначеЕсли ii = 1 тогда
						//	тзДанные77.Колонки.Добавить(инд_К,Новый ОписаниеТипов("Дата"));
						//Иначе
							тчРез.Колонки.Добавить(инд_К,Новый ОписаниеТипов("Строка"	, Новый КвалификаторыСтроки(200, ДопустимаяДлина.Переменная)));
						//КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				...
				Если тчРез.Количество() > 0 тогда
					current_vers = тчРез[0].p1;	
					latest_step_current_vers = Число(тчРез[0].p2);	
					//latest_version = тчРез[0][3];	
					//latest_version_dt = тчРез[0][4];	
					//fl = Число(тчРез[0][5]);
					
					Если latest_step_current_vers > 6 тогда // объект был загружен в 7ку или была снята регистрация по квитанции (на периоде до логирования загрузки)
						Если Найти(Врег(КаталогОбмена),Врег("Hand_made")) = 0 тогда
							_Игнорировать = 1;	
						КонецЕсли;
					//Иначе
					//	Если fl = 0 тогда
...
Функция ЗакрытьСеансПодключенияКSQL_2() экспорт
	...
	Возврат Успех;
КонецФункции
Функция ПоказатьПроблемныеДоки() экспорт
	тчРез =  Новый ТаблицаЗначений;
	...
	|	*
	|	, case
	|		when _step = 3 then 'объект при выгруке из 1С8 проигнориварон'
	|		when _step = 4 then 'выгружен из 1С8'
	|		when _step = 5 then 'файл принят в работу в 1с7'
	|		when _step = 6 then 'файл при загрузке в 1с7 проигнорирован'
	|		when _step = 7 then 'объект записан в 1с7'
	|		when _step = 8 then 'принята квитанция из 1с7'
	|		else ''
	|	end as step_comm
	|	,DATEDIFF(hour,_dt,GETDATE()) as ПрошлоЧасов
	|from
	|	Guids_1C8_1C7_monitor (nolock)
	|where _id in
	|			(
	|			select
	|				t1._id
	...
	|							 , min(_dt) as _dt
	|							 , max(_step) as _step
	|						  FROM Guids_1C8_1C7_monitor (nolock)
	|						  group by
	|							_id				
	|						) as t1
	|			where 
	|				DATEDIFF(hour,t1._dt,GETDATE()) > 24  -- когда с момента выгрузки в каталог обмена прошло более суток
	|				and _step < 7 -- когда конечная стадия, снятия с регистрации в 1с8 не достигнута или загрузка в 7ку не выполнена
	|			)				
	|order by _id, _dt	
	|";
...
Процедура АнализФайловСОшибками()
	...
	нс = ТЗм.Добавить(); нс.пп = 11; нс.меню = "Установить реквШапки ТТ";
	нс = ТЗм.Добавить(); нс.пп = 12; нс.меню = "Перезаписать ССЫЛКИ на № док-та";
	нс = ТЗм.Добавить(); нс.пп = 13; нс.меню = "Выгрузить рекурсивно";
	...
	Иначе
		...
		ИначеЕсли Адрес2.пп = 3 тогда
			...
			Для каждого стр из Строки цикл
				...
				Если ФайлСуществует(стр_ИмяФайла) тогда
					...
					ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(узел_получатель, НайденнаяСсылкаНаДокумент);
					//)Семенов И.П.
		...
		ИначеЕсли Адрес2.пп = 8 ИЛИ Адрес2.пп = 11 ИЛИ Адрес2.пп = 12 ИЛИ Адрес2.пп = 13 тогда
			
			Если Адрес2.пп = 12 тогда
				Подключение_к_SQL_серверу_3();	
			КонецЕсли;
			...
			Для каждого стр из Строки цикл
				...
				Если ФайлСуществует(стр_ИмяФайла) тогда
					...
					Если Адрес2.пп = 12 тогда
						...
						Сообщить( "inf: " +СокрЛП(стрстр) + ". "+ СокрлП(НайденнаяСсылкаНаДокумент) + ". Отказ = " + СокрлП(ПерезаписатьСсылкуВТаблице(л_УИД,ПараметрыДокумента.Номер,л_ТипЗнчДокумента)));
						
					ИначеЕсли Адрес2.пп = 13 тогда
						
						пар_Мас = новый Массив;
						пар_Мас.Добавить(НайденнаяСсылкаНаДокумент);
						Сообщить( "inf: " +СокрЛП(стрстр) + ". "+ СокрлП(НайденнаяСсылкаНаДокумент) + ". Отказ = " + СокрлП(ВыгрузитьДанныеПоЖеланию(пар_Мас).Отказ));
						
						
					Иначе
						
						лД  = НайденнаяСсылкаНаДокумент.ПолучитьОбъект();
						Если Адрес2.пп = 8 тогда
							лД.СозданВ77 = 0;
							Сообщить("inf: " +СокрЛП(стрстр) + ". "+ СокрлП(НайденнаяСсылкаНаДокумент) + " СозданВ77 => 0");
						ИначеЕсли Адрес2.пп = 11 тогда
							лД.ТорговаяТочка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(лД.Контрагент,"ОсновнаяТорговаяТочка");
							Сообщить("inf: " +СокрЛП(стрстр) + ". "+ СокрлП(НайденнаяСсылкаНаДокумент) + "  => ТТ");
						КонецЕсли;
	...
	Для каждого лЭл из 	СоставПланаОбмена цикл
		...
		Если лРез_Количество > 0 тогда
			Для каждого лРезстр из лРез цикл
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(узел_получатель, лРезстр.сЦылка);
				//)Семенов И.П.
			КонецЦикла;
		КонецЕсли;
...
КонецПроцедуры
#КонецЕсли	
Функция Подключение_к_SQL_серверу_3() экспорт
	
	Успех = Истина;	
	
	Попытка
        СоединениеSQL_3  = Новый COMОбъект("ADODB.Connection");
        КомандаSQL_3     = Новый COMОбъект("ADODB.Command");
        ВыборкаSQL_3     = Новый COMОбъект("ADODB.RecordSet");
        ...
        СоединениеSQL_3.ConnectionString = СоединениеSQL_ConnectionString;
		СоединениеSQL_3.ConnectionTimeout = 30;
        СоединениеSQL_3.CommandTimeout = 6000;
		
		СоединениеSQL_3.Open();
		КомандаSQL_3.ActiveConnection   = СоединениеSQL_3;
	...
	Возврат Успех;
КонецФункции	
Функция ЗакрытьСеансПодключенияКSQL_3() экспорт
	Успех = Истина;	
	Попытка 
		СоединениеSQL_3.Close();
	Исключение
		СоединениеSQL_3 = Неопределено;
	КонецПопытки;
	Возврат Успех;
КонецФункции
Функция ПерезаписатьСсылкуВТаблице(пар_УИД,пар_Номер,пар_ВидСтр) Экспорт
	...
	пар_Вид = "0";
	Если Найти(Врег(пар_ВидСтр),Врег("ЗаявкаПокупателя")) тогда
		пар_Вид = "2457";
	ИначеЕсли Найти(Врег(пар_ВидСтр),Врег("ЗаказПоставщику")) тогда
		пар_Вид = "2742";
	ИначеЕсли Найти(Врег(пар_ВидСтр),Врег("ПеремещениеТоваров")) тогда
		пар_Вид = "1628";
	ИначеЕсли Найти(Врег(пар_ВидСтр),Врег("ПоступлениеТоваровУслуг")) тогда
		пар_Вид = "1582";
	ИначеЕсли Найти(Врег(пар_ВидСтр),Врег("ОприходованиеТоваров")) тогда
		пар_Вид = "2106";
	Иначе
		Возврат " не верный вид документа " + пар_Вид;
	КонецЕсли;
	
		КомандаSQL_3_CommandText =
		"UPDATE dbo.adoURBD_Guids
		|   SET 
		|	_MetaType = 12
		|    ,_Id = '" + СокрЛП(пар_УИД) + "'
		| 
		| WHERE 
		|	  _MetaId = " + пар_Вид + " -- Заявка 2457 --_MetaId = 2742 -- Заказ   -- перемещение 1628
		|	  and _Object IN 
		|					 (
		|						SELECT top 1
		|							   IDDOC
		...
		|							  ,CLOSED
		|							  ,ISMARK
		|							  , convert(datetime,left(DATE_TIME_IDDOC,8)) as dt1
		|							  , convert(datetime,'20180602') as dt2
		|							  */
		|						  FROM [dbo].[_1SJOURN] (nolock)
		|						  where IDDOCDEF = " + пар_Вид + " 
		|						        and convert(datetime,left(DATE_TIME_IDDOC,8)) >= convert(datetime,'20180602') and DOCNO = '" + Лев(пар_Номер,3) + Прав(пар_Номер,7) + "' 
		|						  order by convert(datetime,left(DATE_TIME_IDDOC,8)) desc
		|					)";
		//Сообщить(КомандаSQL_3_CommandText);
	
	Попытка
		КомандаSQL_3.CommandText = КомандаSQL_3_CommandText;   
		КомандаSQL_3.Execute();	
	Исключение
		Отказ = ОписаниеОшибки();
	КонецПопытки;
...
Функция ПолучитьСписокРеквизитовШапки(пар_Вид, пар_Тип) экспорт
	...
	Если СтрНайти(пар_Вид,"Водители") > 0 тогда
		
		имя_дополнительного_расчетного_реквизита = "Водители_ЭтоГруппа";
		...
		имя_дополнительного_расчетного_реквизита = "Водители_Родитель";
		Шапка.Вставить(имя_дополнительного_расчетного_реквизита, "Справочник.Водители");
		список_расчетных_реквизитов_шапки = список_расчетных_реквизитов_шапки + имя_дополнительного_расчетного_реквизита + ";";
		
		имя_дополнительного_расчетного_реквизита = "Водители_ТТ";
		Шапка.Вставить(имя_дополнительного_расчетного_реквизита, "Справочник.ТорговыеТочки");
		список_расчетных_реквизитов_шапки = список_расчетных_реквизитов_шапки + имя_дополнительного_расчетного_реквизита + ";";
		
		имя_дополнительного_расчетного_реквизита = "Водители_ИмяРодителЯ";
		Шапка.Вставить(имя_дополнительного_расчетного_реквизита, "Строка");
		список_расчетных_реквизитов_шапки = список_расчетных_реквизитов_шапки + имя_дополнительного_расчетного_реквизита + ";";
		
	ИначеЕсли СтрНайти(пар_Вид,"Контрагенты") > 0 тогда
...
Функция ПолучитьПереченьОбъектовОбмена(пар_ВзятьВсеОбъектыБезУчетаПотоков = Ложь) Экспорт
	...
	лОбъектМетаданных = Метаданные.Справочники.Водители;
        Стр = СоставПланаОбмена.Добавить();
		Стр.ОбъектМетаданных_Имя = лОбъектМетаданных.Имя;
		Стр.ОбъектМетаданных_ПолноеИмя = лОбъектМетаданных.ПолноеИмя();
        Стр.Регистрация = Истина;
		Стр.Реквизиты_Шапка = ПолучитьСписокРеквизитовШапки(Стр.ОбъектМетаданных_Имя,Стр.ОбъектМетаданных_ПолноеИмя);
        Стр.Реквизиты_ТЧ = ПолучитьСписокРеквизитовТЧ(Стр.ОбъектМетаданных_Имя,Стр.ОбъектМетаданных_ПолноеИмя);
			...
			Если Не Посмотреть тогда
				...
				Попытка
						...
						Если НЕ НоваяСтр.ОснованиеРегистрация тогда
							...
							ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(узел_получатель, СсылкаНаДокумент);
							//)Семенов И.П.
						КонецЕсли;
			...
			Если Не Посмотреть тогда
				...
				Попытка
						...
						Если НЕ НоваяСтр.ОснованиеРегистрация тогда
							...
							ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(узел_получатель, СсылкаНаДокументОснование);
							//)Семенов И.П.
						КонецЕсли;
