...
Функция ВыгрузитьДанныеПоЖеланию(пар_МассивСсылокРазличныхВидов, пар_ОпроситьСсылкиИзТЧ = Истина, пар_ОпроситьСсылкиИзРегПартий = Истина, пар_ОпроситьСсылкиИзРегПартийРазмещение = Истина) экспорт
	...
	// для каждой ссылки найдем сопутствующие док-ты
	Для каждого лДок из пар_МассивСсылокРазличныхВидов цикл 
		
		Если лДок = Неопределено тогда
			Продолжить;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(лДок) тогда
			Продолжить;
		КонецЕсли;
		...
		РекурсияДляЗаполненияСпискаОснований(лДок);
	...
	//пар_МассивСсылокРазличныхВидов_Дубль.ВыбратьСтроку();
	
	пар_МассивСсылокРазличныхВидов_ДубльДубль = пар_МассивСсылокРазличныхВидов_Дубль.Скопировать();
	Если пар_ОпроситьСсылкиИзТЧ тогда
		Для каждого лДок из пар_МассивСсылокРазличныхВидов_ДубльДубль цикл	
			
			лМ = Новый Массив;
			
			//СОобщить("док = " + СокрлП(лДок.сцылка));
			
			Если 	  лДок.типвид = "Документ.КорректировкаЗаявкиПокупателя" тогда
				лМ = ПолучитьТчДок(лДок.сцылка,"Товары","СтрокаЗаявки.Заявка","КорректировкаЗаявкиПокупателя");
			ИначеЕсли лДок.типвид = "Документ.ЗаказПоставщику" тогда
				лМ = ПолучитьТчДок(лДок.сцылка,"Товары","СтрокаЗаявки.Заявка","ЗаказПоставщику");
			ИначеЕсли лДок.типвид = "Документ.КорректировкаЗаказаПоставщику" тогда
				лМ = ПолучитьТчДок(лДок.сцылка,"Товары","СтрокаЗаявки.Заявка","КорректировкаЗаказаПоставщику");
			ИначеЕсли лДок.типвид = "Документ.ПоступлениеТоваровУслуг" тогда
				лМ = ПолучитьТчДок(лДок.сцылка,"РазмещениеСтрокПрихода","СтрокаЗаявки.Заявка","ПоступлениеТоваровУслуг");        
				лМ2 = ПолучитьТчДок(лДок.сцылка,"РазмещениеСтрокПрихода","СтрокаЗаявки.Заказ","ПоступлениеТоваровУслуг");        
				Для каждого эл2 из лМ2 цикл
					лМ.Добавить(эл2);	
				КонецЦикла;
			ИначеЕсли лДок.типвид = "Документ.ПеремещениеТоваров" тогда	
				лМ = ПолучитьТчДок(лДок.сцылка,"Товары","СтрокаЗаявки.Заявка","ПеремещениеТоваров");
			ИначеЕсли лДок.типвид = "Документ.РеализацияТоваровУслуг" тогда
				лМ = ПолучитьТчДок(лДок.сцылка,"Товары","СтрокаЗаявки.Заявка","РеализацияТоваровУслуг");
				
				Если пар_ОпроситьСсылкиИзРегПартий тогда
					
					лМ2 = ПолучитьПартииДок(лДок.сцылка);
					//СОобщить("партии");
					Для каждого эл2 из лМ2 цикл
						лМ.Добавить(эл2);	
						//сообщить(эл2);
						
						Если пар_ОпроситьСсылкиИзРегПартийРазмещение тогда
							
							лМ3 = Новый Массив;
							лМ4 = Новый Массив;
							л_ТипЗнчДокумента = ВходитВСоставПланаОбмена(эл2.Метаданные().Имя);
							Если л_ТипЗнчДокумента = "Документ.ПоступлениеТоваровУслуг" тогда
								лМ3= ПолучитьТчДок(эл2,"РазмещениеСтрокПрихода","СтрокаЗаявки.Заявка","ПоступлениеТоваровУслуг");        
								лМ4 = ПолучитьТчДок(эл2,"РазмещениеСтрокПрихода","СтрокаЗаявки.Заказ","ПоступлениеТоваровУслуг");        
							КонецЕсли;
							//СОобщить("размещение заявки");
							Для каждого эл3 из лМ3 цикл
								лМ.Добавить(эл3);
								//сообщить(эл3);
							КонецЦикла;
							//СОобщить("размещение заказы");
							Для каждого эл4 из лМ4 цикл
								лМ.Добавить(эл4);	
								//сообщить(эл4);
							КонецЦикла;
						КонецЕсли;
			...
			Если лМ.Количество() > 0 тогда
				Для каждого эМ из лМ цикл
					
					л_ТипЗнчДокумента = ВходитВСоставПланаОбмена(эМ.Метаданные().Имя);
					Если НЕ ЗначениеЗаполнено(л_ТипЗнчДокумента) тогда
						Продолжить;
					КонецЕсли;
					
					Если пар_МассивСсылокРазличныхВидов_Дубль.Найти(эМ,"сцылка") = Неопределено тогда
						лстр = пар_МассивСсылокРазличныхВидов_Дубль.Добавить();
						лстр.сцылка = эМ;
						лстр.типвид = л_ТипЗнчДокумента;
					КонецЕсли;
	...
	пар_МассивСсылокРазличныхВидов_Дубль = Запрос.Выполнить().Выгрузить();
	...
	Возврат Результат;
КонецФункции
Процедура РекурсияДляЗаполненияСпискаОснований(пар_ссылка)
	
	Попытка
		
		лОбъект = пар_ссылка.ПолучитьОбъект();
		МетаданныеОбъекта = лОбъект.Метаданные();
		Если НЕ МетаданныеОбъекта.Реквизиты.Найти("ДокументОснование") = Неопределено Тогда
			лДокО = ОбщегоНазначения.ПолучитьЗначениеРеквизита(лОбъект.Ссылка,"ДокументОснование");
			Если НЕ лДокО = Неопределено и ЗначениеЗаполнено(лДокО) тогда
				мОснования.Добавить(лДокО);
				
				РекурсияДляЗаполненияСпискаОснований(лДокО)
			КонецЕсли;
		КонецЕсли; 		
...
Функция ПолучитьТчДок(пар_ссылка, пар_тч, пар_что, пар_вид)
	...
	| И НЕ ЗаказПоставщикуТовары." + пар_что + " = ЗНАЧЕНИЕ(Документ."+?(пар_что="СтрокаЗаявки.Заявка","ЗаявкаПокупателя","ЗаказПоставщику")+".ПустаяСсылка)";
	//-
