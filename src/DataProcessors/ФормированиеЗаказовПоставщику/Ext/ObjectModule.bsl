				...
				Если АктуальныеЗаданияБезПотребностиДляРассылки.Количество() > 0 тогда
					...
					Попытка
						...
						Для каждого СтрРассылки из АктуальныеЗаданияБезПотребностиДляРассылки цикл
							...
							текТемаПисьмаЗаказ = СтрЗаменить(ТекстПисьмаПрайсЗагружен, "*ДАТАВРЕМЯЗАПУСКА*", СокрЛП( ПравилаИгры.РасчетнаяДата ));
							
							СтрРассылки_Задание_ТекстПисьма = текТемаПисьмаЗаказ;
