				...
				Если АктуальныеЗаданияБезПотребностиДляРассылки.Количество() > 0 тогда
					...
					Попытка
						...
						Для каждого СтрРассылки из АктуальныеЗаданияБезПотребностиДляРассылки цикл
							...
							|	ШаблоныТекстовПисем.Наименование
							|ИЗ
							...
							Запрос.УстановитьПараметр("Событие", Справочники.СобытияДляОтправкиЭлектронныхПисем.ОтправкаЗаказаПоставщикуНетПотребности);
							Р = Запрос.Выполнить().Выбрать();
							Пока Р.Следующий() Цикл
								Если СтрРассылки.Задание_ПополнениеСкладаОтдельно тогда 
									Если Р.Наименование="Заказ поставщику (нет потребности склад)" Тогда 
										ТемаПисьмаПрайсЗагружен = Р.ТемаПисьма;
										ТекстПисьмаПрайсЗагружен = Р.ТекстПисьма;
									Иначе
										ТекстПисьмаПрайсЗагружен = "Уважаемые Коллеги!" + Символы.ПС + Символы.ПС + " потребность в заказах ( для пополнения склада ) отсутствует"+ Символы.ПС + Символы.ПС + "С уважением,"+ Символы.ПС + "компания ""ПартКом"". Телефон: +7 (831) 233-22-07";
										ТемаПисьмаПрайсЗагружен = "Заказ_от_ПартКом_Потребности_Нет";
									КонецЕсли;
								Иначе 	
									Если Р.Наименование="Заказ поставщику (нет потребности)" Тогда 
										ТемаПисьмаПрайсЗагружен = Р.ТемаПисьма;
										ТекстПисьмаПрайсЗагружен = Р.ТекстПисьма;
									Иначе
										ТекстПисьмаПрайсЗагружен = "Уважаемые Коллеги!" + Символы.ПС + Символы.ПС + " потребность в заказах ( для клиентов ) отсутствует"+ Символы.ПС +  Символы.ПС + "С уважением,"+ Символы.ПС +  "компания ""ПартКом"". Телефон: +7 (831) 233-22-07";;
										ТемаПисьмаПрайсЗагружен = "Заказ_от_ПартКом_Потребности_Нет";
									КонецЕсли;
								КонецЕсли;	
							КонецЦикла;
