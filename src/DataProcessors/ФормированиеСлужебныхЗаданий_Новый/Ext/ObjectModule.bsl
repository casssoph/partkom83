...
Процедура ЗаполнитьРеквизитыЭД(Знач ДокументРеализация, Знач Параметры)
	...
	ДокументРеализация.ВремяДоставкиПо = Параметры.ДатаДоставкиДоКлиента - Отклонение *3600;
...
Функция ЗаполнитьПараметрыОбработкиСогласноОснованию(ПараметрФормированияСЗ, СтрокаП) экспорт
	...
	ПараметрыОснования = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ПараметрФормированияСЗ,"Код,Склад,МаршрутДоставки,Водитель,ТипДоставки,флДополнительнаяДоставка,Выписывать_только_сток_и_vmi,ДниНедели,ДниНедели" + ТекДеньНедели + ",флНеУдалятьМинСум" + ТекДеньНедели+ ",ВремОтгрузки" + ТекДеньНедели + ",ВремДоставки_Региональное" + ТекДеньНедели);
	...
	Параметры.Вставить("ДатаДоставки_Региональное", НачалоДня(СтрокаП.ДатаОтгрузки) + 3600 * Час(ВремДоставки_Региональное) + 60 * Минута(ВремДоставки_Региональное) + Секунда(ВремДоставки_Региональное));
	//- Валиахметов http://jira.part-kom.ru/browse/XX-2238 17.04.2019
