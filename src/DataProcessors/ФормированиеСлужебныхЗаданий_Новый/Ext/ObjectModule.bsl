	...
	Иначе
		...
		Для Каждого СтрокаП Из ПараметрыФормированияСЗ Цикл 
			...
			Если Инд >= МаксКоличествоВПотоке Тогда  
				...
				Инд = 0;
			КонецЕсли;;
			ДобавитьВОчередьФормированияСлужебныхЗаданий(СтрокаП.ПараметрФормированияСЗ, СтрокаП.Параметры, НомерПотока);
			Инд = Инд + 1;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыФормированияСЗ() 
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |				И (&ДтТек МЕЖДУ ДОБАВИТЬКДАТЕ(ДатыФормированияСлужебныхЗаданий.ДатаОкончания, МИНУТА, -6) И ДОБАВИТЬКДАТЕ(ДатыФормированияСлужебныхЗаданий.ДатаОкончания, МИНУТА, 1))
			    	//если время запуска = время окончания, чтобы запустилось хотя бы раз
					|			ИЛИ ДатыФормированияСлужебныхЗаданий.ДатаЗапуска = ДатыФормированияСлужебныхЗаданий.ДатаОкончания
	                |				И ДатыФормированияСлужебныхЗаданий.ДатаФормирования < ДатыФормированияСлужебныхЗаданий.ДатаОкончания
	                |				И (&ДтТек МЕЖДУ ДОБАВИТЬКДАТЕ(ДатыФормированияСлужебныхЗаданий.ДатаОкончания, МИНУТА, -5) И ДОБАВИТЬКДАТЕ(ДатыФормированияСлужебныхЗаданий.ДатаОкончания, МИНУТА, 1)))
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.МаршрутДоставки,
	                |	ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.Склад";
...
Процедура ДобавитьВОчередьФормированияСлужебныхЗаданий(ПараметрФормированияСЗ, Параметры, НомерПотока)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	И ОчередьФормированияСлужебныхЗаданий.ДатаОкончания = &ДатаОкончания
	               |	И ОчередьФормированияСлужебныхЗаданий.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)";
	Запрос.УстановитьПараметр("ПараметрФормированияСЗ", ПараметрФормированияСЗ);
	...
	Если Запрос.Выполнить().Пустой() Тогда 
		...
		Менеджер.ПараметрФормированияСЗ = ПараметрФормированияСЗ;
		Менеджер.НомерПотока = НомерПотока;
		Менеджер.ДатаОкончания = Параметры.ДатаОкончания;
