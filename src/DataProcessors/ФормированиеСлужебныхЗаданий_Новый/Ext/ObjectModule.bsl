...
Процедура СоздатьДокументыСЗ(Параметры) Экспорт 
	...
	лКлючАлгоритма = "Обработка_ФормированиеСлужебныхЗаданий_Новый_МодульОбъекта_СоздатьДокументыСЗ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ДокументМаршрутноеЗадание(Реализация,ДокСЗ)
	...
	//|	И МегаЛогист_МаршрутноеЗадание.Контрагент = &РТУ_Контрагент
	...
	//|	И РАЗНОСТЬДАТ(&СЗ_ДатаДоставки_Региональное, ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(МегаЛогист_МаршрутноеЗадание.ДатаДоставки, ДЕНЬ), МИНУТА, РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), МегаЛогист_МаршрутноеЗадание.ВремяДоставкиПо, МИНУТА)), МИНУТА) МЕЖДУ -15 И 15
	//|;
	//|
	//|////////////////////////////////////////////////////////////////////////////////
	//|ВЫБРАТЬ ПЕРВЫЕ 1
