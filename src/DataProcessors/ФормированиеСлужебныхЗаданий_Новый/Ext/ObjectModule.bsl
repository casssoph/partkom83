...
Функция ПараметрыФормированияСЗ() 
	...
	///ХудинВВ 26062018 По просьбе ДурандинаДА
	//  чтобы запуск состоялся, если задание еще не формировалось, а время отгрузки еще не вышло
	|		ИЛИ   (ДатыФормированияСлужебныхЗаданий.ДатаФормирования = ДатаВремя(1,1,1) И &ДтТек <= ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, ДЕНЬ), МИНУТА, РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОтгрузки" + СокрЛП(ДеньНедели(Дата)) + ", МИНУТА))) )
