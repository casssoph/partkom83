...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	ВремТаблица.Свернуть("Контрагент,ГоловнойКонтрагент,ДоговорГоловногоКонтрагента,МаршрутДоставки,БлокировкаКонтрагента,БлокировкаДоговораКонтрагента,БлокировкаГолКонтрагента,БлокировкаДоговораГолКонтрагента,ПросроченнаяЗадолженность,МинимальнаяСуммаОтгрузки");	
	...
	Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
		...
		//19.06.2018 
		ЭтоVIP = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТЧ.Контрагент, "ОсновнаяТорговаяТочка.VIP") = Истина;
		...
		Если Не ЭтоVIP И Не Параметры.ТипДоставки = Справочники.ТипыДоставки.Самовывоз И Не НеПроверятьМинОтгрузку И Не Параметры.НеУдалятьМинСум И СуммаОтгрузки < СтрокаТЧ.МинимальнаяСуммаОтгрузки И СтрокиРеализаций.Количество() = 0 Тогда  
			Исключен = Истина; 
