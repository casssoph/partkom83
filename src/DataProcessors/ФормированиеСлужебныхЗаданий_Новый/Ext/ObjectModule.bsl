...
Функция ПараметрыФормированияСЗ() 
	...
	|	И &ДтТек <= ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, ДЕНЬ), МИНУТА, РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОтгрузки" + СокрЛП(ДеньНедели(Дата)) + ", МИНУТА))
	//ХудинВВ 25062018 По просьбе ДурандинаДА верхняя граница формирования теперь не время окончания довыписки, а время отгрузки
...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ВЫБРАТЬ
	               |	втДанныеПоГолКАГ.ДоговорКонтрагента,
	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДоговорыКонтрагентов.Ссылка) КАК ДоговорГоловногоКонтрагента
	               |ПОМЕСТИТЬ втДанные2
	               |ИЗ
	               |	втДанныеПоГолКАГ КАК втДанныеПоГолКАГ
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ЕСТЬNULL(ВзаиморасчетыОстатки.ДоговорКонтрагента, ДепозитыКонтрагентовОстатки.ДоговорКонтрагента) КАК ДоговорГоловногоКонтрагента,
	               |	ЕСТЬNULL(ВзаиморасчетыОстатки.СуммаУпрОстаток, 0) + ЕСТЬNULL(ДепозитыКонтрагентовОстатки.СуммаУпрОстаток, 0) КАК СуммаДолга
	               |ПОМЕСТИТЬ втДанные3
	               |ИЗ
	               |	РегистрНакопления.Взаиморасчеты.Остатки(
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	вДоговораГолКАГ.ДоговорКонтрагента,
	               |	ЕСТЬNULL(втДанные3.СуммаДолга, 0) КАК СуммаДолга
	               |ПОМЕСТИТЬ втДолгиГолКонтрагента
	               |ИЗ
	               |	втДоговораГолКАГ КАК вДоговораГолКАГ
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втДанные3 КАК втДанные3
	               |		ПО вДоговораГолКАГ.ДоговорГоловногоКонтрагента = втДанные3.ДоговорГоловногоКонтрагента
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ втДанные3
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(вДоговораГолКАГ.ДоговорГоловногоКонтрагента.ДопустимаяСуммаЗадолженности, втРезервы.ДоговорКонтрагента.ДопустимаяСуммаЗадолженности) КАК ДопустимаяСуммаЗадолженности,
	               ...
	               |	ЕСТЬNULL(вДоговораГолКАГ.ДоговорГоловногоКонтрагента, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)) КАК ДоговорГоловногоКонтрагента
	               |ИЗ
	               |	втРезервы КАК втРезервы
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втДоговораГолКАГ КАК вДоговораГолКАГ
	               |		ПО втРезервы.ДоговорКонтрагента = вДоговораГолКАГ.ДоговорКонтрагента
	               ...
	               |		ПО втРезервы.ДоговорКонтрагента = втДолгиГолКонтрагента.ДоговорКонтрагента
	               |
	               |УПОРЯДОЧИТЬ ПО
    ...
    ВремТаблица = Товары.Скопировать(, "ГоловнойКонтрагент"); //,ДоговорГоловногоКонтрагента,ДопустимаяСуммаЗадолженности,СуммаДолга");
	ВремТаблица.Свернуть("ГоловнойКонтрагент"); //,ДоговорГоловногоКонтрагента,ДопустимаяСуммаЗадолженности,СуммаДолга");
	
	ДолгиГолКонтрагента = Новый ТаблицаЗначений;
	
	ДолгиГолКонтрагента.Колонки.Добавить("ГоловнойКонтрагент");
	//ДолгиГолКонтрагента.Колонки.Добавить("ДоговорГоловногоКонтрагента");
	...
	Для Каждого СтрокаТовары Из ВремТаблица Цикл 
		...
		ВремТаблица2 = Товары.Скопировать(Строки, "ДоговорГоловногоКонтрагента,СуммаДолга,ДопустимаяСуммаЗадолженности,ВидОплаты");
		ВремТаблица2.Свернуть("ДоговорГоловногоКонтрагента,СуммаДолга,ДопустимаяСуммаЗадолженности,ВидОплаты");
		
		СуммаИтоговогоДолга = 0;
		СуммаЛимита = 0;
		
		Для Каждого СтрокаДоговора Из ВремТаблица2 Цикл 
			
			Если Не ЗначениеЗаполнено(СтрокаДоговора.ДоговорГоловногоКонтрагента) Тогда 
				Продолжить;
			КонецЕсли;
						
			СуммаДолга = 0;
			Строки = Товары.НайтиСтроки(Новый Структура("ГоловнойКонтрагент, ДоговорГоловногоКонтрагента", СтрокаТовары.ГоловнойКонтрагент, СтрокаДоговора.ДоговорГоловногоКонтрагента));
			Для Каждого Строка Из Строки Цикл 
				Если СтрокаДоговора.ВидОплаты = Перечисления.ВидыДенежныхСредств.Безналичные Тогда 
			...
			СуммаЛимита =  Макс(0, СуммаЛимита + СтрокаДоговора.ДопустимаяСуммаЗадолженности - СтрокаДоговора.СуммаДолга);
		КонецЦикла;
		
		//Добавляем самого головного КАГ
	...
	ВремТаблица.Свернуть("Контрагент,VIP,ГоловнойКонтрагент,ДоговорГоловногоКонтрагента,МаршрутДоставки,БлокировкаКонтрагента,БлокировкаДоговораКонтрагента,БлокировкаГолКонтрагента,БлокировкаДоговораГолКонтрагента,ПросроченнаяЗадолженность,МинимальнаяСуммаОтгрузки");	
	...
	Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
		...
		ВремТаблица2 = Товары.Скопировать(Строки, "ДоговорКонтрагента,СуммаДолга,ДопустимаяСуммаЗадолженности,ВидОплаты,НеКонтролироватьКредит");
		ВремТаблица2.Свернуть("ДоговорКонтрагента,СуммаДолга,ДопустимаяСуммаЗадолженности,ВидОплаты,НеКонтролироватьКредит");
