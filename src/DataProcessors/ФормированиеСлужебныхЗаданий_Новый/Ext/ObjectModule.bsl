...
Функция ПараметрыФормированияСЗ() 
	...
	Запрос.Текст = "ВЫБРАТЬ                                                                  
	               |	ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ,
	               |	ДатыФормированияСлужебныхЗаданий.ДатаФормирования,
				   | 	ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(ДнПред) + " КАК ДниНедели" + Строка(ДнПред) + ",
				   | 	ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(ДН) + " КАК ДниНедели" + Строка(ДН) + ",
	               |	ВЫБОР
	               |		КОГДА ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(ДнПред) +  " 
	               |			ТОГДА ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн - 2) + " * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска" + Строка(ДнПред) + ", МИНУТА))
	               |		ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн - 1) + "  * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска" + Строка(Дн) + ", МИНУТА))
	               |	КОНЕЦ КАК ДатаЗапуска,
	               |	ВЫБОР
	               |		КОГДА ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(ДнПред) + "
	               |			ТОГДА ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн - 1) + " * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(ДнПред) + ", МИНУТА))
	               |		ИНАЧЕ 
				   |	   ВЫБОР
				   |	КОГДА ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(Дн) + " < ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска" + Строка(Дн) + "
				   |		ТОГДА ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн) + " * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(Дн) + ", МИНУТА))
				   |	ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн - 1) + " * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(Дн) + ", МИНУТА))
				   |КОНЕЦ КОНЕЦ КАК ДатаОкончания 
	               |ПОМЕСТИТЬ ДатыФормированияСлужебныхЗаданий
	               |ИЗ
	               |	РегистрСведений.ДатыФормированияСлужебныхЗаданий КАК ДатыФормированияСлужебныхЗаданий
	               |ГДЕ
	               |	(ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(Дн) + "
	               |				И &ДтТек >= ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, ДЕНЬ), МИНУТА, РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска" + Строка(Дн) + ", МИНУТА))
	               |			ИЛИ ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ДниНедели" + Строка(ДнПред) + "
	               |				И ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(ДнПред) + " < ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремяЗапуска" + Строка(ДнПред) + "
	               |				И &ДтТек < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&ДтТек, НЕДЕЛЯ), МИНУТА, " + Строка(Дн - 1) + " * 24 * 60 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ДатыФормированияСлужебныхЗаданий.ПараметрФормированияСЗ.ВремОкончанияДовыписки" + Строка(ДнПред) + ", МИНУТА)))
	               ...
	               |	Выбор когда ДатыФормированияСлужебныхЗаданий.ДниНедели" + Строка(ДнПред) + " Тогда " + Строка(ДнПред) + " Иначе " + Строка(Дн) + " Конец КАК День
	               |ИЗ
...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	ВремТаблица.Свернуть("Контрагент,СуммаДолга,ДопустимаяСуммаЗадолженности,VIP,ГоловнойКонтрагент,ДоговорГоловногоКонтрагента,МаршрутДоставки,БлокировкаКонтрагента,БлокировкаДоговораКонтрагента,БлокировкаГолКонтрагента,БлокировкаДоговораГолКонтрагента,ПросроченнаяЗадолженность,МинимальнаяСуммаОтгрузки");	
