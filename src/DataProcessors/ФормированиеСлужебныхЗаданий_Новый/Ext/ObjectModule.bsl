...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
		...
		Если Не Параметры.ТипДоставки = Справочники.ТипыДоставки.Самовывоз И Не НеПроверятьМинОтгрузку И Не Параметры.НеУдалятьМинСум И СуммаОтгрузки < СтрокаТЧ.МинимальнаяСуммаОтгрузки И СтрокиРеализаций.Количество() = 0 Тогда  
			Исключен = Истина; 
