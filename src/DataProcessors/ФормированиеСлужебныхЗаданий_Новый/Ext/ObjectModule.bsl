	...
	Иначе
		...
		Для Каждого СтрокаП Из ПараметрыФормированияСЗ Цикл 
			...
			Если Инд >= МаксКоличествоВПотоке Тогда  
				...
				Инд = 0;
			КонецЕсли;;
			ДобавитьВОчередьФормированияСлужебныхЗаданий(СтрокаП, НомерПотока);
			Инд = Инд + 1;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьВОчередьФормированияСлужебныхЗаданий(СтрокаП, НомерПотока)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	ОчередьФормированияСлужебныхЗаданий.ПараметрФормированияСЗ = &ПараметрФормированияСЗ
	               |	И ОчередьФормированияСлужебныхЗаданий.Параметры_ДатаОкончания = &ДатаОкончания
	               |	И ОчередьФормированияСлужебныхЗаданий.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)";
	Запрос.УстановитьПараметр("ПараметрФормированияСЗ", СтрокаП.ПараметрФормированияСЗ);
	...
	Если Запрос.Выполнить().Пустой() Тогда 
		...
		Менеджер.ПараметрФормированияСЗ = СтрокаП.ПараметрФормированияСЗ;
		Менеджер.НомерПотока = НомерПотока;
		
		Менеджер.Параметры_ДатаЗапуска = СтрокаП.ДатаЗапуска;
		Менеджер.Параметры_ДатаНачалаУпаковки = СтрокаП.ДатаНачалаУпаковки;
		Менеджер.Параметры_ДатаОкончания = СтрокаП.ДатаОкончания;
		Менеджер.Параметры_ДатаОтгрузки = СтрокаП.ДатаОтгрузки;
		Менеджер.Параметры_ДеньНедели = СтрокаП.ДеньНедели;
