...
Процедура СоздатьДокументыМЗ(ДокСЗ)
	...
	Для Каждого СтрокаРеализации из ДокСЗ.Покупатели цикл
		...
		Если Общегоназначения.ПолучитьЗначениеРеквизита(МЗ.Ссылка,"ЭкспрессДоставка") = Истина Тогда
			
			Если НЕ ЗначениеЗаполнено(ОбщегоНазначения.ПолучитьЗначениеРеквизита(МЗ.Ссылка,"express_delivery_request_id")) Тогда
				МЗ.express_delivery_request_id = МЗ.Номер;
				МЗ.Записать(РежимЗаписиДокумента.Запись);
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ОбщегоНазначения.ПолучитьЗначениеРеквизита(Реализация,"express_delivery_request_id")) Тогда
