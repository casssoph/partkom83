	...
	Для Каждого СтрокаП Из Покупатели Цикл 
		...
		Для Каждого СтрокаТЧ Из ВремТаблица Цикл 
			...
			Если Параметры.ТипДоставки = Справочники.ТипыДоставки.ЭкспрессДоставка Тогда 
				...
				Если ДокументНаУслугу.ДоговорКонтрагента.ВидОплаты = Перечисления.ВидыДенежныхСредств.Безналичные Тогда 
					УслугаДоставки = у_м.Получить(0);
				Иначе
					УслугаДоставки = у_м.Получить(1);
				КонецЕсли;
				СтрокаСтоимостиДоставки = ДокументНаУслугу.Услуги.Добавить();
				...
				СтрокаСтоимостиДоставки.СтавкаНДС = ?(ДокументНаУслугу.ДоговорКонтрагента.ВидОплаты = Перечисления.ВидыДенежныхСредств.Безналичные, ОбщегоНазначения.ПолучитьЗначениеРеквизита(УслугаДоставки,"СтавкаНДС"), Перечисления.СтавкиНДС.БезНДС);
...
Процедура ЗаполнитьТаблицы(Параметры) Экспорт 
	...
	Если Не Параметры.НеИскать Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |ГДЕ
		               |	НЕ РеализацияТоваровУслуг.ПометкаУдаления
		               |	И (&ТипДоставки = ЗНАЧЕНИЕ(Справочник.ТипыДоставки.ЭкспрессДоставка) Или &ДобавлятьРТУГотовКВыдачеВДоставкуВодителем И &ТипДоставки = ЗНАЧЕНИЕ(Справочник.ТипыДоставки.ДоставкаВодителем))
		               ...
		               |	И РеализацияТоваровУслуг.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.РеализацияТоваровУслугГотовКВыдаче)
		               |	И СлужебноеЗаданиеПокупатели.Ссылка ЕСТЬ NULL";
					   	//Конец Валиахметов http://jira.part-kom.ru/browse/XX-2200  16.04.2019
