...
Функция ЗаполнитьПараметрыОбработкиСогласноОснованию(ПараметрФормированияСЗ, СтрокаП) экспорт
	...
	ПараметрыОснования = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ПараметрФормированияСЗ,"Код,Склад,МаршрутДоставки,Водитель,ТипДоставки,флДополнительнаяДоставка,Выписывать_только_сток_и_vmi,ДниНедели,ДниНедели" + ТекДеньНедели + ",флНеУдалятьМинСум" + ТекДеньНедели+ ",ВремОтгрузки" + ТекДеньНедели);
