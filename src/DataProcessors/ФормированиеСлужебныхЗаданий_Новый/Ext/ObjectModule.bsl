...
Процедура СоздатьДокументыМЗ(ДокСЗ)
	...
	Для Каждого СтрокаРеализации из ДокСЗ.Покупатели цикл
		...
		МЗ = ДокументМаршрутноеЗадание(Реализация,ДокСЗ);
		...
		// + Пушкин 20190507 XX-2448
		МЗ.ДокументОснование = ДокСЗ.Ссылка;
		// - Пушкин 20190507 XX-2448
		...
		Если  Общегоназначения.ПолучитьЗначениеРеквизита(МЗ.Ссылка,"ЭкспрессДоставка") = Истина 
			...
			И НЕ ЗначениеЗаполнено(ОбщегоНазначения.ПолучитьЗначениеРеквизита(МЗ.Ссылка,"express_delivery_request_id")) Тогда
				...
				о_РТУ.Телефон = SMSоповещение.ПроверитьНомерТелефона();
				
				Если о_РТУ.Услуги.Количество() > 0 Тогда
					о_РТУ.ЭД_init = Истина;
				Иначе
					о_РТУ.ЭД_init = Ложь;
				КонецЕсли;
		...
		// + Пушкин 20190507 XX-2448
		СтрокаРеализации.МаршрутноеЗадание = МЗ.Ссылка;
		// - Пушкин 20190507 XX-2448
