...
Процедура ОбновлениеИнформационнойБазы_1_0_4_37() Экспорт
	...
	//ХудинВВ XX-2855 15072019 ((
	ДнейОжиданияДляЗакрытияНеподтвержденныхРознЗаявок = Константы.ДнейОжиданияДляЗакрытияНеподтвержденныхРознЗаявок.Получить();
	Если ДнейОжиданияДляЗакрытияНеподтвержденныхРознЗаявок = 0 Тогда 
		Константы.ДнейОжиданияДляЗакрытияНеподтвержденныхРознЗаявок.Установить(3);
		Сообщить("Установлена константа ""ДнейОжиданияДляЗакрытияНеподтвержденныхРознЗаявок""");
	КонецЕсли;
	//))
	
	//Валиахметов http://jira.part-kom.ru/browse/XX-2791 15.07.2019
	РЗ = Справочники.РегламентныеЗадания.НайтиПоНаименованию("Допроведение резервов");
	Если ЗначениеЗаполнено(РЗ) Тогда 
		РЗОбъект = РЗ.ПолучитьОбъект();
		СтрокаРеквизита = РЗОбъект.Шапка.Найти("РегламентноеЗадание", "ИмяРеквизита");
		Если СтрокаРеквизита = Неопределено Тогда 
			СтрокаРеквизита = РЗОбъект.Шапка.Добавить();
		КонецЕсли;
		СтрокаРеквизита.ИмяРеквизита = "РегламентноеЗадание";
		СтрокаРеквизита.ЗначениеРеквизита = РЗ;
		РЗОбъект.Записать();
	КонецЕсли;
	...
	Попытка
		Менеджер = РегистрыСведений.НастройкиОтправкиСообщенийОбОшибках.СоздатьМенеджерЗаписи();
		Менеджер.Пользователь = Справочники.Пользователи.НайтиПоКоду("Сергеев С.В.");
		Менеджер.Событие = Справочники.СобытияДляОтправкиЭлектронныхПисем.ДопроведениеРезервов;
		Менеджер.АдресЭлПочты = "Sergeev-SV@Part-kom.ru";
		Менеджер.Настройка = Истина;
	...
	//Конец Валиахметов http://jira.part-kom.ru/browse/XX-2791 15.07.2019
	
КонецПроцедуры

Процедура ОбновлениеИнформационнойБазы_1_0_4_40() Экспорт
	
	//ХудинВВ XX-2879 05082019 (( 
	СкладБонусовВиртуальный = Константы.СкладБонусовВиртуальный.Получить();
	Если Не ЗначениеЗаполнено(СкладБонусовВиртуальный) Тогда 
		Константы.СкладБонусовВиртуальный.Установить(Справочники.Склады.НайтиПоКоду("000000031"));
		Сообщить("Установлена константа ""СкладБонусовВиртуальный""");
	КонецЕсли;
	//))
