	...
	Пока лВыборка.Следующий() цикл
		...
		Иначе
			...
			Пока лВыборкаДокументы.Следующий() цикл
				...
				лПараметры.Вставить("ДанныеОбъекта", Новый Структура);
...
Функция ВыполнитьОбработкуФрагмент()
	...
	|	Т1.Регистратор.МоментВремени КАК РегистраторМоментВремени
	|ПОМЕСТИТЬ МаксДокументы_МоментВремени
	|ИЗ
	|	МаксДокументы КАК Т1;
	//	
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ " + ?(Порция > 0, " ПЕРВЫЕ " + Формат(Порция, "ЧГ="), "") + "
	...
	//|		ЛЕВОЕ СОЕДИНЕНИЕ МаксДокументы КАК МаксДокументы // ЛНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ МаксДокументы_МоментВремени КАК МаксДокументы // ЛНА
	...
	|	И МаксДокументы.РегистраторМоментВремени > ПоРасчетамСКонтрагентами.МоментВремени
	|УПОРЯДОЧИТЬ ПО
...
Функция Алгоритм()
	...
	|			лПараметры.Вставить(""РегистрыДвижений"", ""Взаиморасчеты"");
	|			лПараметры.Вставить(""ДобавитьВОчередьОтправкиДокументовОтгрузки"", Новый структура);
	|			лМетаданныеДокумента = лВыборкаДокументы.Регистратор.Метаданные();
	|			
	|			Попытка
	|				РежимУправленияБД = ?(Строка(лМетаданныеДокумента.РежимУправленияБлокировкойДанных) = ""Управляемый"", РежимУправленияБлокировкойДанных.Управляемый, РежимУправленияБлокировкойДанных.Автоматический);
