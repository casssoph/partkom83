...
Функция ЗапроситьБД() Экспорт
	...
	|	
	|	//ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)) КАК z_МЗадание_ВремяДоставкиФакт,
	|	//РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА) КАК z_МЗадание_ВремяДоставкиФакт_Минуты,
	|	
	|	ВЫБОР
	|		КОГДА ЭД_web_Сводка.ed_1cConfirm > ДАТАВРЕМЯ(1, 1, 1)  И РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА) > 0
	|			ТОГДА 
	...
	//|		КОГДА ЭД_web_Сводка.ed_1cConfirm > ДАТАВРЕМЯ(1, 1, 1)  И РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА) > 0
	//|			ТОГДА 
	//|				РАЗНОСТЬДАТ(ЭД_web_Сводка.ed_1cConfirm,ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ЭД_web_Сводка.ed_1cConfirm, ДЕНЬ),МИНУТА,РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА)), МИНУТА)
	//|		ИНАЧЕ
	//|			NULL
	//|	КОНЕЦ КАК ВсегоМинутОтАкцептаДоДоставки_1,
	|	ВЫБОР
	|		КОГДА ЭД_web_Сводка.ed_1cConfirm > ДАТАВРЕМЯ(1, 1, 1)  И РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА) > 0
	|			ТОГДА 
	|				РАЗНОСТЬДАТ(ЭД_web_Сводка.ed_1cConfirm,ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ЭД_web_Сводка.ed_1cConfirm, ДЕНЬ),МИНУТА,РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1), ЕСТЬNULL(МаршрутныеЗадания.Ссылка.ВремяДоставкиФакт, ДАТАВРЕМЯ(1, 1, 1)), МИНУТА)), МИНУТА)
	...
	//|		ИНАЧЕ
	//|			NULL
	//|	КОНЕЦ КАК ВсегоМинутОтАкцептаДоДоставки_3,
	|
	|	
	...
	|	
	|	МИНИМУМ(z_МЗадание_ВремяДоставкиФакт_Дата) КАК z_МЗадание_ВремяДоставкиФакт_Дата,
	|	
	//|	МИНИМУМ(ВсегоМинутОтАкцептаДоДоставки_1) КАК ВсегоМинутОтАкцептаДоДоставки_МИНИМУМ,
	|	СРЕДНЕЕ(ВсегоМинутОтАкцептаДоДоставки_2) КАК ВсегоМинутОтАкцептаДоДоставки_СРЕДНЕЕ,
	//|	МАКСИМУМ(ВсегоМинутОтАкцептаДоДоставки_3) КАК ВсегоМинутОтАкцептаДоДоставки_МАКСИМУМ,
	...
	|	СРЕДНЕЕ(ВсегоМинутОтАкцептаДоДоставки_2 - ПолныйРасклад.d_МинутыОбработкаВТопЛог) КАК ВсегоМинутДоставка_СРЕДНЕЕ,	
	...
	|	
	//|	МИНИМУМ(ВсегоМинутОтАкцептаДоДоставки_МИНИМУМ),
	|	СРЕДНЕЕ(ВсегоМинутОтАкцептаДоДоставки_СРЕДНЕЕ),
	//|	МАКСИМУМ(ВсегоМинутОтАкцептаДоДоставки_МАКСИМУМ),
