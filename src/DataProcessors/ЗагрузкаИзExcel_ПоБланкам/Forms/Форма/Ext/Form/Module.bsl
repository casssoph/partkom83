...
Процедура ОбработатьДанныеЛиста(ДанныеЛиста, ОписаниеТаблицы)		
	...
	ДанныеТаблицы = НайтиИВыбратьСтроки(ДанныеЛиста, ОписаниеТаблицы, 0, 100000);
...
Функция СформироватьОписаниеКолонок()
	...
	ДобавитьОписаниеКолонки(ОписаниеТаблицы, "БезоговорочныйВозврат"	, 	ПреобразоватьВЗаголовокКолонки("Безоговорочный возврат."), Истина);
	ДобавитьОписаниеКолонки(ОписаниеТаблицы, "ВидУценки"				,	ПреобразоватьВЗаголовокКолонки("Вид уценки"				), Истина);
	ДобавитьОписаниеКолонки(ОписаниеТаблицы, "ОтсутствуютСледыУстановки", 	ПреобразоватьВЗаголовокКолонки("Отсутствие следов установки"), Истина);
	ДобавитьОписаниеКолонки(ОписаниеТаблицы, "ЦенаПослеУценки"			, 	ПреобразоватьВЗаголовокКолонки("Цена детали после уценки"), Истина); 
	ДобавитьОписаниеКолонки(ОписаниеТаблицы, "VIN"			, 	ПреобразоватьВЗаголовокКолонки("VIN код"), Истина); 
...
Функция НайтиИЗаполнитьСоответствия(АдресДанных, ЦенаВключаетНДС)
	...
	СоответствиеПричинВозврата.Вставить("Ошибка заказа (возврат товара с уценкой)", Справочники.ПричиныВозврата.ОтказКлиента);
	...
	Ключи = "Штрихкод,ПричинаВозврата,БезоговорочныйВозврат,ВидУценки,ОтсутствуютСледыУстановки,ЦенаПослеУценки, Документ, ДокументНайден, VIN";
	...
	Для Каждого СтрокаДанных Из ДанныеТаблицы Цикл
		...
		Если Ссылка <> Неопределено Тогда
			...
			Строка.БезоговорочныйВозврат = ?(Строка.БезоговорочныйВозврат = "Безоговорочный", Истина, Ложь);
			
			Строка.ВидУценки = ?(СокрЛП(Строка.ВидУценки) = "Обработка одной детали", Перечисления.ВидыУценки.ОбработкаОднойДетали, Перечисления.ВидыУценки.УценкаЗаТовар);
			
			Строка.ОтсутствуютСледыУстановки = ?(Строка.ОтсутствуютСледыУстановки = "TRUE", Истина, Ложь);
			Строка.ЦенаПослеУценки = Число(СтрокаДанных.ЦенаПослеУценки);
...
Процедура ЗаписатьСвойстваСервер()
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если Строка.ДокументНайден Тогда
			...
			пОбъект = Строка.Документ.ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(пОбъект, Строка, "БезоговорочныйВозврат,ПричинаВозврата,VIN");
			
			Если пОбъект.Товары.Количество() = 1 Тогда
				СтрокаТовары = пОбъект.Товары[0];
				ЗаполнитьЗначенияСвойств(СтрокаТовары, Строка, "ОтсутствуютСледыУстановки,ЦенаПослеУценки");
			Иначе
				Продолжить;
			КонецЕсли;
