...
&НаСервере
Функция ВзаимосвязиЭтапа(Этап = Неопределено)
	...
	|ГДЕ
	|	(НЕ ВзаимосвязиСтатусовПроцессаВозвратаОтПокупателя.ПометкаУдаления
	|				И (ВзаимосвязиСтатусовПроцессаВозвратаОтПокупателя.ТекущийСтатус = &Этап
	|					ИЛИ ВзаимосвязиСтатусовПроцессаВозвратаОтПокупателя.СледующийСтатус = &Этап)
	|			ИЛИ &ВсеЭтапы)";
...
Процедура ГрафСхемаПриАктивизацииНаСервере(ИмяЭлемента)
	...
	Иначе
		...
		Если ТипЗнч(Этап) = Тип("СправочникСсылка.СтатусыДокументов") Тогда
			...
			ОбъектСхемы.backColor  = Новый Цвет(64, 94, 172);
			
			Ребра = ВзаимосвязиЭтапа(Этап);	
			...
			Для каждого Ребро Из Ребра Цикл
				Для каждого КлючЗачение Из  ИдентификаторыЭтапов Цикл
					Если ТипЗнч(КлючЗачение.Значение) = Тип("СправочникСсылка.ВзаимосвязиСтатусовПроцессаВозвратаОтПокупателя")
						...
						ОбъектСхемы.lineColor = Новый Цвет(64, 94, 172);
	...
	//  ВыделитьСвязиЭлемента(ИмяЭлемента);
КонецПроцедуры
