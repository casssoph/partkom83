	 ...
	 ОбластьЗагрузки  = ТабличныйДокумент.ПолучитьОбласть(ТабличныйДокумент.Области[0].Имя);
	  ...
	  Для ИндексТекСтроки = НомерПервойСтроки по КоличествоСтрок цикл 
		 ...
		 НоваяЗаписьРезультат.НомерСтроки = ИндексТекСтроки+ НомерПервойСтроки;
		  ...
		  ОбластьСтроки = ОбластьЗагрузки.ПолучитьОбласть("R"+ИндексТекСтроки+"C"+Колонка.Значение); 
		  НоваяЗаписьРезультат[колонка.Ключ] = ПолучитьЗначениеЯчейки(ОбластьСтроки);
		  
		  КонецЦикла;;
		  
	КонецЦикла;	  
	  
     СоздатьДокументыНаСервере(ПолучитьОбработануюТаблицу(ТаблицаРезультата));
