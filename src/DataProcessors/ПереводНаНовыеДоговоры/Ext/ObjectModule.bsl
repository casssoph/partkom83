...
Процедура Проверки(Отказ = Неопределено, Заполнение = Истина, ПоГоловным = Истина, ПоОрганизациям = Истина, УжеПереведенные = Истина) Экспорт 
	Если Заполнение Тогда 
		Отказ = Не ПроверитьЗаполнение();
	КонецЕсли;
	Если ПоГоловным Тогда 
		ПроверитьКонтрагентовПоГоловным();
	КонецЕсли;
	Если ПоОрганизациям Тогда 
		ПроверитьОрганизации(Отказ);
	КонецЕсли;
	Если УжеПереведенные Тогда 
		УстановитьПризнакУжеПереведенных();
	КонецЕсли;
КонецПроцедуры

Процедура ПроверитьКонтрагентовПоГоловным(Отказ = Неопределено) Экспорт 
	...
	тКонтрагенты = Контрагенты.Выгрузить();
	тКонтрагенты.ЗаполнитьЗначения(Ложь, "ЕстьПроблемаСПодчГолов");
	Контрагенты.Загрузить(тКонтрагенты);
	...
	Пока Выборка.Следующий() Цикл 
		...
		//Если УдалитьСтроки Тогда 
			...
			Для Каждого Стр Из Строки Цикл 
				Стр.ЕстьПроблемаСПодчГолов = Истина; 
...
Процедура ПроверитьОрганизации(Отказ  = Неопределено) Экспорт 
	тКонтрагенты = Контрагенты.Выгрузить(, "Организация");
	тКонтрагенты.Свернуть("Организация");
	
	Для Каждого СтрокаТЧ Из тКонтрагенты Цикл 
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Организация.Договор_ОфертаПокупателя_Номер) Тогда 
			Отказ = Истина;
			Сообщить("В организации " + СтрокаТЧ.Организация + " не заполнен номер оферты");
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Организация.Договор_ОфертаПокупателя_Дата) Тогда 
			Отказ = Истина;
			Сообщить("В организации " + СтрокаТЧ.Организация + " не заполнена дата оферты");
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Организация.ТипОплаты) Тогда 
			Отказ = Истина;
			Сообщить("В организации " + СтрокаТЧ.Организация + " не заполнен тип оплаты");
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура УстановитьПризнакУжеПереведенных() Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВЫРАЗИТЬ(Т.Контрагент КАК Справочник.Контрагенты) КАК Контрагент,
	               |	ВЫРАЗИТЬ(Т.Организация КАК Справочник.Организации) КАК Организация
	               |ПОМЕСТИТЬ Т
	               |ИЗ
	               |	&Т КАК Т
	               |;
	               ...
	               |	Т.Контрагент,
	               |	Т.Организация
	               |ПОМЕСТИТЬ ПереведенныеДоговоры
	               |ИЗ
	               |	Т КАК Т
	               |ГДЕ
	               |	Т.Контрагент.ОсновнойДоговорКонтрагента.Организация = Т.Организация
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ЗаявкиПокупателейОстатки.ДоговорКонтрагента.Владелец КАК Контрагент
	               |ПОМЕСТИТЬ втЗаявкиОставшиеся
	               |ИЗ
	               |	РегистрНакопления.ЗаявкиПокупателей.Остатки(
	               |			,
	               |			ДоговорКонтрагента.Владелец В
	               |					(ВЫБРАТЬ
	               |						ПереведенныеДоговоры.Контрагент
	               |					ИЗ
	               |						ПереведенныеДоговоры)
	               |				И ЕСТЬNULL(СтрокаЗаявки.ПоследняяКорректировка.ДоговорКонтрагента, СтрокаЗаявки.Заявка.ДоговорКонтрагента) <> ДоговорКонтрагента.Владелец.ОсновнойДоговорКонтрагента) КАК ЗаявкиПокупателейОстатки
	               |ГДЕ
	               ...
	               |				И ДоговорКонтрагента.ВидОплаты = &ВидОплаты) КАК ВзаиморасчетыОстатки
	               |ГДЕ
	               ...
	               |	ПереведенныеДоговоры.Контрагент
	               |ИЗ
	               |	ПереведенныеДоговоры КАК ПереведенныеДоговоры
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втЗаявкиОставшиеся КАК втЗаявкиОставшиеся
	               |		ПО ПереведенныеДоговоры.Контрагент = втЗаявкиОставшиеся.Контрагент
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втДолгиОставшиеся КАК втДолгиОставшиеся
	               |		ПО ПереведенныеДоговоры.Контрагент = втДолгиОставшиеся.Контрагент
	               |ГДЕ
	               |	(ПереведенныеДоговоры.Контрагент.ОсновнойДоговорКонтрагента.ВидОплаты = &ВидОплаты
	               |			ИЛИ втДолгиОставшиеся.Контрагент ЕСТЬ NULL)
	               |	И втЗаявкиОставшиеся.Контрагент ЕСТЬ NULL";
	Запрос.УстановитьПараметр("ВидОплаты", Перечисления.ВидыДенежныхСредств.Наличные);
	Запрос.УстановитьПараметр("Т", Контрагенты.Выгрузить(, "Контрагент, Организация"));
	Запрос.УстановитьПараметр("Закрыт", Справочники.СтатусыДокументов.ЗаявкаПокупателяЗакрыт);
	
	Выборка = Запрос.Выполнить().Выбрать();
	тКонтрагенты = Контрагенты.Выгрузить();
	тКонтрагенты.ЗаполнитьЗначения(Ложь, "УжеПереведен");
	Контрагенты.Загрузить(тКонтрагенты);
	
	Пока Выборка.Следующий() Цикл 
		Строки = Контрагенты.НайтиСтроки(Новый Структура("Контрагент", Выборка.Контрагент));
		Для Каждого СтрокаТЧ Из Строки Цикл 
			СтрокаТЧ.УжеПереведен = Истина;
		КонецЦикла;;
	КонецЦикла;
КонецПроцедуры
...
Функция ОбработатьСтроки(СоздаватьДоговоры = Ложь, ИзменятьЗаявки = Ложь, СоздаватьКорректировки = Ложь) Экспорт
	...
	_Табл1 = Контрагенты.ВыгрузитьКолонки();
	_Табл2 = Контрагенты.ВыгрузитьКолонки();
	
	Шаг = 1;
	ДатаНачала = ТекущаяДата();
	
	Для каждого СтрокаТЧ Из Контрагенты Цикл
		Прогресс = Окр(100*Шаг/Контрагенты.Количество());
		
		ПрошлоСекунд = ТекущаяДата() - ДатаНачала;
		мОставшеесяВремя = ПрошлоСекунд *(Контрагенты.Количество() - Шаг )/Шаг;
		
		ОставшеесяВремя = Дата(1,1,1) + мОставшеесяВремя;
		
		Шаг = Шаг + 1;
		
		Если Не СтрокаТЧ.Выбрать Или СтрокаТЧ.УжеПереведен Или СтрокаТЧ.ЕстьПроблемаСПодчГолов Тогда
			ЗаполнитьЗначенияСвойств(_Табл1.Добавить(), СтрокаТЧ);
			Продолжить;
		КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(_Табл2.Добавить(), СтрокаТЧ);
		#Если Клиент Тогда 
	...
	Проверки(,Ложь,Ложь,Ложь,Истина);
	
	#Если Клиент Тогда 
		ТабДокумент = Новый ТабличныйДокумент;
		Построитель = Новый ПостроительОтчета;
		Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(_Табл2);       
		Построитель.Вывести(ТабДокумент);
		ТабДокумент.Показать("Обработанные строки"); 
		
		ТабДокумент = Новый ТабличныйДокумент;
		Построитель = Новый ПостроительОтчета;
		Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(_Табл1);       
		Построитель.Вывести(ТабДокумент);
		ТабДокумент.Показать("Проигнорированные строки"); 
	#КонецЕсли
	//УстановитьПризнакУжеПереведенных();
