...
Процедура ДополнитьДаннымиПоПечати(ОбъектXDTO, вхСсылкаНаДокумент, СЗ = Неопределено) Экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ЕСТЬNULL(РеализацияТоваровУслуг.ДоговорКонтрагентаВзаиморасчетов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДоговорКонтрагентаДата,
	                ...
	                |	ВЫБОР
	                |		КОГДА РеализацияТоваровУслуг.Контрагент.ГоловнойКонтрагент = РеализацияТоваровУслуг.Контрагент
	...
	ИначеЕсли ЗначениеЗаполнено(Выборка.ГоловнойКонтрагент) Тогда 
	...
	Иначе 
		ФактическийАдресГрузополучателя  = УправлениеКонтактнойИнформацией.ПолучитьАдресИзКонтактнойИнформации(Выборка.Грузополучатель, "Фактический");
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			ИЛИ РеализацияТоваровУслугУслуги.Номенклатура = &ЭкспрессДоставка)
