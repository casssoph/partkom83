...
Процедура ДополнитьДаннымиПоПечати(ОбъектXDTO, вхСсылкаНаДокумент, СЗ = Неопределено) Экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ВЫБОР
	                |		КОГДА ЕСТЬNULL(РеализацияТоваровУслуг.Контрагент.ГоловнойКонтрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	                |			ТОГДА РеализацияТоваровУслуг.Контрагент
