...
Функция ПолучитьСтруктуруСтрокиИсторииОбъекта()
	...
	Возврат Новый Структура("Период,Объект,Разделитель,ТелоСообщения,Ошибка,ТекстОшибки,ИмяМетаданных");
...
Процедура ДобавитьСтрокуИсторииПоОбъекту(Объект, ТелоСообщения, ИмяМетаданных="", Ошибка=Ложь, ТекстОшибки="") Экспорт 
	...
	Если ПараметрыСеанса.ВключенаЗаписьИсторииОбменаПоОбъектам = Ложь Тогда 
		Возврат;
	КонецЕсли;
...
Процедура ЗафиксироватьСообщениеВИсторииОбмена(НомерСообщения, Узел, ТелоСообщения, Исходящее=Ложь, Ошибка=Ложь, ТекстОшибки="") Экспорт
...
Процедура СоздатьЗаписьИсторииРегистрации(Узел, СсылкаНаОбъект, КонтекстВызова="", ВсеДанныеОбъекта = Ложь)
