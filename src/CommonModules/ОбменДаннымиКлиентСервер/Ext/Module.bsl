...
Процедура ДополнитьДаннымиПоПечати(ОбъектXDTO, вхСсылкаНаДокумент, СЗ = Неопределено) Экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ВЫБОР
	                |		КОГДА РеализацияТоваровУслуг.ДоговорКонтрагентаВзаиморасчетов.Организация.ТипОплаты = ЗНАЧЕНИЕ(Справочник.ВидыОплатЧекаККМ.Любой)
	                ...
	                |		ИНАЧЕ ЕСТЬNULL(РеализацияТоваровУслуг.ДоговорКонтрагентаВзаиморасчетов.Организация.УчитыватьНДС, ЛОЖЬ)
