...
Процедура СдвинутьГраницуПриСменеДатыОплаты(вхОбъект) Экспорт 
	...
	Если Не вхОбъект.ЭтоНовый() Тогда
		...
		Если вхОбъект.ДатаОплаты <> Реквизиты.ДатаОплаты Тогда 
			...
			Результат = Запрос.Выполнить();
			Если Не Результат.Пустой() Тогда 
				Дата = Мин(вхОбъект.Дата, вхОбъект.ДатаОплаты, Реквизиты.Дата, Реквизиты.ДатаОплаты);
				МоментВремени = Новый МоментВремени(Дата, вхОбъект.Ссылка);
				Последовательности.ПоРасчетамСКонтрагентами.УстановитьГраницу(МоментВремени, Новый Структура("ДоговорКонтрагента", вхОбъект.ДоговорКонтрагентаВзаиморасчетов));	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
