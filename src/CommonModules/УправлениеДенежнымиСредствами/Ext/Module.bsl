...
Функция ПолучитьТаблицуПлатежейУпр(ДатаДокумента,ВалютаДокумента,Документ, ВидПлатежногоДокумента) Экспорт
	...
	Если КонтролироватьЗадолженности Тогда
		...
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ НЕ РасшифровкаПлатежаДок.ДоговорКонтрагента.НеКонтролироватьЛимит
...
Процедура ЗаполнитьПриходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		Товары.ЗаявкаПокупателя КАК Сделка,
		|		ВЫБОР
		|			КОГДА Товары.Ссылка.СуммаВключаетНДС
		|				ТОГДА Товары.Сумма
		|			ИНАЧЕ Товары.Сумма + Товары.СуммаНДС
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.СчетНаОплатуПокупателю", "Документ.ЗаявкаПокупателя");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Товары.ЗаявкаПокупателя", "Товары.Ссылка");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Услуги.ЗаявкаПокупателя", "Услуги.Ссылка");
		КонецЕсли;
		Запрос.Текст = ТекстЗапроса;
		
		Запрос.УстановитьПараметр("Основание", Основание);
		ТаблицаОснования = Новый ТаблицаЗначений;
		ТаблицаОснования.Колонки.Добавить("Сделка",    Новый ОписаниеТипов("ДокументСсылка.ЗаявкаПокупателя"));//ТаблицаОснования.Колонки.Добавить("Сделка",    Новый ОписаниеТипов("ДокументСсылка.СчетНаОплатуПокупателю, ДокументСсылка.ЗаявкаПокупателя"));
		ТаблицаОснования.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СтавкиНДС"));
		ТаблицаОснования.Колонки.Добавить("Сумма",     Новый ОписаниеТипов("Число"));
		...
		//	Иначе
		//		
		//		Если ЗначениеЗаполнено(Основание.ЗаявкаПокупателя) Тогда
		//			Сделка = Основание.ЗаявкаПокупателя;
		//		Иначе
		//			Сделка = Неопределено;
		//		КонецЕсли;
		//	
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаПокупателя") И Основание.Проведен Тогда
	...
	Для Каждого Платеж Из РасшифровкаСделка Цикл
		...
		Если Платеж.ВестиПоДокументамРасчетовСКонтрагентом Тогда
			...
			Если Платеж.КонтролироватьЧислоДнейЗадолженности И (НЕ (РасчетыВозврат=Перечисления.РасчетыВозврат.Возврат ИЛИ ВходящийПлатеж)) Тогда
				...
				|		И УпрУчет
				|		И НЕ ДоговорКонтрагента.НеКонтролироватьЛимит
...
Функция ЕстьРасчетыСКонтрагентами(ВидОперации=Неопределено) Экспорт
	...
	Иначе
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийРКО.СнятиеСДепозита
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийПКО.ВнесениеНаДепозит
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВнесениеНаДепозит
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийППИсходящее.СнятиеСДепозита
...
Процедура УстановитьСтатьюДДСПоУмолчанию(СтрокаПлатеж,ВидОперацииДокумент) Экспорт;
	...
	ИначеЕсли ВидОперацииДокумент=Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВнесениеНаДепозит ИЛИ
		ВидОперацииДокумент=Перечисления.ВидыОперацийППИсходящее.СнятиеСДепозита ИЛИ
		ВидОперацииДокумент=Перечисления.ВидыОперацийПКО.ВнесениеНаДепозит ИЛИ
		ВидОперацииДокумент=Перечисления.ВидыОперацийРКО.СнятиеСДепозита Тогда
		
		СтрокаПлатеж.СтатьяДвиженияДенежныхСредств=Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Депозит");
...
Функция ОпределитьПараметрыВыбораСделки(ВидОперации=Неопределено) Экспорт
	...
	ЗаявкаПокупателяПриход =  Новый Структура("ВидДвиженияРасчеты,ТипЗаказа","Расход","ЗаявкаПокупателя"); // Расход по регистру взаиморасчетов
    ЗаявкаПокупателяРасход =  Новый Структура("ВидДвиженияРасчеты,ТипЗаказа","Приход","ЗаявкаПокупателя"); // Приход по регистру взаиморасчетов
	...
	Если ЗначениеЗаполнено(ВидОперации) тогда
		...
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийРКО.ВозвратДенежныхСредствПокупателю,ЗаявкаПокупателяРасход);
		
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийПКО.ОплатаПокупателя,ЗаявкаПокупателяПриход);
		...
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПокупателю,ЗаявкаПокупателяРасход);
		
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ОплатаПокупателя,ЗаявкаПокупателяПриход);
		...
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийППИсходящее.ВозвратДенежныхСредствПокупателю,ЗаявкаПокупателяРасход);
		...
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийЗаявкиНаРасходование.ВозвратДенежныхСредствПокупателю,ЗаявкаПокупателяРасход);
				
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийПланируемоеПоступлениеДС.ОплатаПокупателя,ЗаявкаПокупателяПриход);
		...
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийОплатаОтПокупателяПлатежнойКартой.ОплатаПокупателя,ЗаявкаПокупателяПриход);
		ПараметрыСделкиПоВидуОпераций.Вставить(Перечисления.ВидыОперацийОплатаОтПокупателяПлатежнойКартой.ВозвратДенежныхСредствПокупателю,ЗаявкаПокупателяРасход);
	...
	Иначе
		...
		Возврат ЗаявкаПокупателяПриход;
...
Процедура УстановитьНадписьСделкиПлатеж(СтрокаПлатеж, ФормаДокумента, ВидОперации) Экспорт
	...
	//		НадписьСделка = "Заказ "+?(ТипЗаказа="ЗаявкаПокупателя","покупателя:", "поставщику:");

	//	ИначеЕсли ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам тогда

	//		НадписьСделка = "Счет "+?(ТипЗаказа="ЗаявкаПокупателя","покупателю:", "поставщика:");
...
Процедура ЗаполнитьРасшифровкуПлатежей(СтруктураПараметров) Экспорт
	...
	Если СтруктураПараметров.Интерактивно Тогда
		...
		Иначе	
			ФормаНастройки=ОбработкаОбъект.ПолучитьФорму("Форма");
		КонецЕсли;
