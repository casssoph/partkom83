...
Процедура ПолучитьПараметрыСчетаФактуры(СчетФактура, мВалютаРегламентированногоУчета, Результат) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ПолучитьПараметрыСчетаФактуры";
	...
	Для каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") Тогда
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") Тогда
			
			Запрос.УстановитьПараметр("ДокументОснование_КорректировкаПоступленияТоваровУслуг", ДокументыОснования);
			...
			ТекстЗапроса = 
			"ВЫБРАТЬ
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка КАК Документ,
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка.Организация КАК Организация,
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка.Контрагент КАК Контрагент,
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка.ДоговорКонтрагента КАК Договор,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА 0
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугТовары.Ссылка.СуммаВключаетНДС
			|					ТОГДА ВЫБОР
			|							КОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма - КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения > 0
			|								ТОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма - КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения
			|							ИНАЧЕ 0
			|						КОНЕЦ
			|				ИНАЧЕ ВЫБОР
			|						КОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - (КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения) > 0
			|							ТОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - (КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения)
			|						ИНАЧЕ 0
			|					КОНЕЦ
			|			КОНЕЦ
			...
			|					ТОГДА ВЫБОР
			|							КОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма - КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения < 0
			|								ТОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения - КорректировкаПоступленияТоваровУслугТовары.Сумма
			|							ИНАЧЕ 0
			|						КОНЕЦ
			|				ИНАЧЕ ВЫБОР
			|						КОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - (КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения) < 0
			|							ТОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения - (КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС)
			|						ИНАЧЕ 0
			|					КОНЕЦ
			|			КОНЕЦ
			|	КОНЕЦ КАК СуммаУменьшение,
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА 0
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения > 0
			|					ТОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения
			|				ИНАЧЕ 0
			|			КОНЕЦ
			...
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения < 0
			|					ТОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения - КорректировкаПоступленияТоваровУслугТовары.СуммаНДС
			|				ИНАЧЕ 0
			|			КОНЕЦ
			...
			|			ТОГДА ВЫБОР
			|					КОГДА КорректировкаПоступленияТоваровУслугТовары.Ссылка.СуммаВключаетНДС
			|						ТОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма
			|					ИНАЧЕ КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС
			|				КОНЕЦ
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугТовары.Ссылка.СуммаВключаетНДС
			|					ТОГДА КорректировкаПоступленияТоваровУслугТовары.Сумма - КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения
			|				ИНАЧЕ КорректировкаПоступленияТоваровУслугТовары.Сумма + КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - (КорректировкаПоступленияТоваровУслугТовары.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения)
			|			КОНЕЦ
			|	КОНЕЦ КАК СуммаДокумента,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА КорректировкаПоступленияТоваровУслугТовары.СуммаНДС
			|		ИНАЧЕ КорректировкаПоступленияТоваровУслугТовары.СуммаНДС - КорректировкаПоступленияТоваровУслугТовары.СуммаНДСДоИзменения
			|	КОНЕЦ КАК СуммаНДСДокумента,
			|	ВЫБОР
			|		КОГДА КорректировкаПоступленияТоваровУслугТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
			|			ТОГДА 0
			|		ИНАЧЕ 1
			|	КОНЕЦ КАК ЕстьНДС,
			|	ЛОЖЬ КАК БланкСтрогойОтчетности
			|ПОМЕСТИТЬ КорректировкаПоступленияТоваровУслуг
			|ИЗ
			|	Документ.КорректировкаПоступленияТоваровУслуг.Товары КАК КорректировкаПоступленияТоваровУслугТовары
			|ГДЕ
			|	КорректировкаПоступленияТоваровУслугТовары.Ссылка В(&ДокументОснование_КорректировкаПоступленияТоваровУслуг)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка КАК Документ,
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка.Организация,
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка.Контрагент,
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка.ДоговорКонтрагента,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА 0
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугУслуги.Ссылка.СуммаВключаетНДС
			|					ТОГДА ВЫБОР
			|							КОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма - КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения > 0
			|								ТОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма - КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения
			|							ИНАЧЕ 0
			|						КОНЕЦ
			|				ИНАЧЕ ВЫБОР
			|						КОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - (КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения) > 0
			|							ТОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - (КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения)
			|						ИНАЧЕ 0
			|					КОНЕЦ
			|			КОНЕЦ
			|	КОНЕЦ,
			...
			|					ТОГДА ВЫБОР
			|							КОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма - КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения < 0
			|								ТОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения - КорректировкаПоступленияТоваровУслугУслуги.Сумма
			|							ИНАЧЕ 0
			|						КОНЕЦ
			|				ИНАЧЕ ВЫБОР
			|						КОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - (КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения) < 0
			|							ТОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения - (КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС)
			|						ИНАЧЕ 0
			|					КОНЕЦ
			|			КОНЕЦ
			|	КОНЕЦ,
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка.ВалютаДокумента,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА 0
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения > 0
			|					ТОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения
			|				ИНАЧЕ 0
			|			КОНЕЦ
			|	КОНЕЦ,
			|	ВЫБОР
			...
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения < 0
			|					ТОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения - КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС
			|				ИНАЧЕ 0
			|			КОНЕЦ
			|	КОНЕЦ,
			|	ВЫБОР
			...
			|			ТОГДА ВЫБОР
			|					КОГДА КорректировкаПоступленияТоваровУслугУслуги.Ссылка.СуммаВключаетНДС
			|						ТОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма
			|					ИНАЧЕ КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС
			|				КОНЕЦ
			|		ИНАЧЕ ВЫБОР
			|				КОГДА КорректировкаПоступленияТоваровУслугУслуги.Ссылка.СуммаВключаетНДС
			|					ТОГДА КорректировкаПоступленияТоваровУслугУслуги.Сумма - КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения
			|				ИНАЧЕ КорректировкаПоступленияТоваровУслугУслуги.Сумма + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - (КорректировкаПоступленияТоваровУслугУслуги.СуммаДоИзменения + КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения)
			|			КОНЕЦ
			|	КОНЕЦ,
			|	ВЫБОР
			|		КОГДА НЕ &КорректировочныйСчетФактура
			|			ТОГДА КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС
			|		ИНАЧЕ КорректировкаПоступленияТоваровУслугУслуги.СуммаНДС - КорректировкаПоступленияТоваровУслугУслуги.СуммаНДСДоИзменения
			|	КОНЕЦ,
			|	ВЫБОР
			|		КОГДА КорректировкаПоступленияТоваровУслугУслуги.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
			|			ТОГДА 0
			|		ИНАЧЕ 1
			|	КОНЕЦ,
			|	ЛОЖЬ
			|ИЗ
			|	Документ.КорректировкаПоступленияТоваровУслуг.Услуги КАК КорректировкаПоступленияТоваровУслугУслуги
			|ГДЕ
			|	КорректировкаПоступленияТоваровУслугУслуги.Ссылка В(&ДокументОснование_КорректировкаПоступленияТоваровУслуг)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	КорректировкаПоступленияТоваровУслуг.Документ,
			|	КорректировкаПоступленияТоваровУслуг.Организация,
			|	КорректировкаПоступленияТоваровУслуг.Контрагент,
			|	КорректировкаПоступленияТоваровУслуг.Договор,
			|	КорректировкаПоступленияТоваровУслуг.ВалютаДокумента,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаУвеличение) КАК СуммаУвеличение,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаУменьшение) КАК СуммаУменьшение,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаНДСУвеличение) КАК СуммаНДСУвеличение,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаНДСУменьшение) КАК СуммаНДСУменьшение,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаДокумента) КАК СуммаДокумента,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.СуммаНДСДокумента) КАК СуммаНДСДокумента,
			|	СУММА(КорректировкаПоступленияТоваровУслуг.ЕстьНДС) КАК ЕстьНДС
			|ИЗ
			|	КорректировкаПоступленияТоваровУслуг КАК КорректировкаПоступленияТоваровУслуг
			|
			|СГРУППИРОВАТЬ ПО
			|	КорректировкаПоступленияТоваровУслуг.Организация,
			|	КорректировкаПоступленияТоваровУслуг.Контрагент,
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			...
			Для каждого ДокументОснование Из ДокументыОснования Цикл
				...
				РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "УчитыватьНДС, Организация, Контрагент, ДоговорКонтрагента, ВалютаДокумента"); 
				
				ДанныеДляПечати = Документы.ВозвратТоваровОтПокупателя.ПодготовитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента(ДокументОснование);
				Если ДанныеДляПечати <> Неопределено Тогда
					
					СуммаНДСДоИзменения 			= ДанныеДляПечати.ТабличнаяЧасть.Итог("СуммаНДСДоИзменения");
					СуммаНДСПослеИзменения 			= ДанныеДляПечати.ТабличнаяЧасть.Итог("СуммаНДСПослеИзменения");
					СтоимостьСНДСДоИзменения 		= ДанныеДляПечати.ТабличнаяЧасть.Итог("СтоимостьСНДСДоИзменения");
					СтоимостьСНДСПослеИзменения 	= ДанныеДляПечати.ТабличнаяЧасть.Итог("СтоимостьСНДСПослеИзменения");
					
					СуммаНДСУвеличение = 0;
					Если СуммаНДСПослеИзменения - СуммаНДСДоИзменения > 0 Тогда
					...
					СуммаУменьшение = 0;
					Если СтоимостьСНДСПослеИзменения - СтоимостьСНДСДоИзменения < 0 Тогда
						СуммаУменьшение = СтоимостьСНДСДоИзменения - СтоимостьСНДСПослеИзменения;
					КонецЕсли;
					
					СуммаУвеличение = 0;
					Если СтоимостьСНДСПослеИзменения - СтоимостьСНДСДоИзменения > 0 Тогда
						СуммаУвеличение = СтоимостьСНДСПослеИзменения - СтоимостьСНДСДоИзменения;
					КонецЕсли;
					
					ЗаполнитьЗначенияСвойств(Результат, РеквизитыОснования);
					Результат.Договор 			=  РеквизитыОснования.ДоговорКонтрагента;
					Результат.СчетФактураБезНДС =  НЕ РеквизитыОснования.УчитыватьНДС;

					Результат.СуммаУвеличение    = Результат.СуммаУвеличение + СуммаУвеличение;
					Результат.СуммаНДСУвеличение = Результат.СуммаНДСУвеличение + СуммаНДСУвеличение;
					Результат.СуммаУменьшение    = Результат.СуммаУменьшение + СуммаУменьшение;
					Результат.СуммаНДСУменьшение = Результат.СуммаНДСУменьшение + СуммаНДСУменьшение;
					Результат.СуммаДокумента 	 = Результат.СуммаДокумента + (СтоимостьСНДСПослеИзменения - СтоимостьСНДСДоИзменения);
					Результат.СуммаНДСДокумента  = Результат.СуммаНДСДокумента + (СуммаНДСДоИзменения - СуммаНДСПослеИзменения);
					
					СтрокаСРеквизитами = РеквизитыОснований.Добавить();
					СтрокаСРеквизитами.ДокументОснование 	= ДокументОснование;
					СтрокаСРеквизитами.СуммаУвеличение 		= СуммаУвеличение;
					СтрокаСРеквизитами.СуммаУменьшение 		= СуммаУменьшение;
					СтрокаСРеквизитами.СуммаНДСУвеличение	= СуммаНДСУвеличение;
					СтрокаСРеквизитами.СуммаНДСУменьшение 	= СуммаНДСУменьшение;
					...
					Если ЭтоКорректировочный Тогда
						...
						Если ЭтоПолученныйСФ Тогда
							...
							Если СчетФактура.Исправление Тогда 
								...
								Если ТипЗнч(ИсходныйДокументПоступления) = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") Тогда
...
Функция ПолучитьТекстЗапросаОпределенияПараметровСФ(ДокументыОснования, ЭтоПолученныйСФ, ПараметрыЗапроса, ИмяВременнойТаблицы = "") Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ПолучитьТекстЗапросаОпределенияПараметровСФ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
...
Функция ОрганизацияУчитываетНДС(Организация, Знач Дата) Экспорт //~
	...
	Возврат Организация.УчитыватьНДС;
...
Функция ОпределитьСчетФактураТребуется(ДокументОбъект) Экспорт 
	...
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ОпределитьСчетФактураТребуется";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	ВарианУчетаНДС = УчетНДСПовтИсп.ВариантУчетаНДСОрганизации(ДокументОбъект.Организация);
	ВидОплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.ДоговорКонтрагента,"ВидОплаты");
	УчитыватьНДС = УчетНДСПовтИсп.УчитыватьНДСПоВариантуУчета(ВарианУчетаНДС, ВидОплаты);
			
	
	ПрименяетсяОсвобождениеОтУплатыНДС = ПрименяетсяОсвобождениеОтУплатыНДС(ДокументОбъект.Организация, ДокументОбъект.Дата);
	
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОбъект.ДоговорКонтрагента, "ВидДоговора");
	РеквизитыДоговора.Вставить("УчетАгентскогоНДС", Ложь);
...
// ОпределитьСчетФактураТребуется()
...
Функция РассчитатьСуммуНДС(Сумма, УчитыватьНДС, СуммаВключаетНДС, СтавкаНДС) Экспорт
	Если ТипЗнч(СтавкаНДС) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
		СтНДС = ПолучитьСтавкуНДС(СтавкаНДС);
	Иначе
		СтНДС = СтавкаНДС;
	КонецЕсли;
	
	Если (УчитыватьНДС) И (СуммаВключаетНДС) Тогда
		СуммаБезНДС = Окр(100 * Сумма / (100 + СтНДС), 2);
	...
	Если УчитыватьНДС Тогда 
		Если НЕ СуммаВключаетНДС Тогда
			СуммаНДС = Окр(СуммаБезНДС * СтНДС / 100, 2);
		КонецЕсли;
	Иначе
		СуммаНДС = 0;
	КонецЕсли;
	...
	Для Каждого Строка из ТаблицаДокументов Цикл
		...
		Если ТаблицаСписания.Количество()>0 Тогда
			...
			Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете Тогда
				Если ТипЗнч(СтруктураПараметров.Регистратор) = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг")
...
Процедура СформироватьДвиженияПоНДС(СтруктураПараметров, ТаблицаСписания, МассивНоменклатуры, Организация = Неопределено)
	...
	ИначеЕсли Организация <> Неопределено тогда
		...
		Для Каждого ТекСтрокаПартии Из ТаблицаПартий Цикл
			Если (ЗначениеЗаполнено(ТекСтрокаПартии.ЗаказПартии)) И (ТипЗНЧ(ТекСтрокаПартии.ЗаказПартии) = Тип("ДокументСсылка.ЗаявкаПокупателя")) Тогда 
...
Процедура ВыполнитьДвиженияПоступленияПоРегиструНДСПартииТоваров(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаНДСПартииСписания, Отказ)
	...
	Если ВыполнятьПоступление тогда
		...
		ДанныеДляОбработки.Колонки.ЗаказСписания.Имя="ЗаявкаПокупателя";
		...
		Если СтруктураПараметров.Свойство("СоответствиеКодовОперацийСтрокамНДСПартий") 
			...
			Для Каждого СтрокаТаблицы ИЗ ДанныеДляОбработки Цикл
				Если СтрокаТаблицы.КодОперацииПартииТоваров = Перечисления.КодыОперацийПартииТоваров.РезервированиеПодЗаказ Тогда
					...
					ОбособленныйУчетТоваровПоЗаказамПокупателей = СоответствиеЗаказов[СтрокаТаблицы.ЗаявкаПокупателя];
				
					Если ОбособленныйУчетТоваровПоЗаказамПокупателей = Неопределено Тогда
							
						СтруктураЗаказа = Новый Структура("ДоговорКонтрагента");
						УправлениеЗапасамиПартионныйУчет.ПолучитьРеквизитыОбъекта(СтрокаТаблицы.ЗаявкаПокупателя, СтруктураЗаказа);
		...
		Если ТаблицаДвиженийПартии.Колонки.Найти("ЗаявкаПокупателя")= Неопределено Тогда
			ТаблицаДвиженийПартии.Колонки.Добавить("ЗаявкаПокупателя");
			ТаблицаДвиженийПартии.ЗаполнитьЗначения(Неопределено,"ЗаявкаПокупателя");
		КонецЕсли;
...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(ДанныеДокумента) Экспорт
	...
	ДанныеДляПечати.Вставить("ШК", 	 					ДанныеДокумента.ШК);
	ДанныеДляПечати.Вставить("Комментарий", 	 		ДанныеДокумента.ДокументОснование.Комментарий);
	ДанныеДляПечати.Вставить("НомДок", 	 				ДанныеДокумента.НомДок);
	ДанныеДляПечати.Вставить("КППКонтрагента", 	 		"");
	// +++ Карпычев (14.02.18)
	...
	Попытка
		Если ТипЗнч(ДанныеДокумента.Ссылка)=ТипЗнч(Документы.ВозвратТоваровПоставщику.ПустаяСсылка()) Тогда 
			Для Каждого Стр Из ДанныеДляПечати.ТабличнаяЧасть Цикл
				Если ТипЗнч(ДанныеДокумента.ДокументОснование)=ТипЗнч(Документы.ПоступлениеТоваровУслуг.ПустаяСсылка()) Тогда 
					СТРТТ=ДанныеДокумента.ДокументОснование.Товары.НайтиСтроки(Новый Структура("Номенклатура",Стр.Товар));
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Если ЭтоПечатьУПД Тогда
		...
		Попытка
			...
			ОбластьМакета.Области["Barcode"].Объект.Сообщение  = ДанныеДляПечати.ШК;
			ОбластьМакета.Области["Barcode"].Объект.ТекстКода  = ДанныеДляПечати.ШК;
		Исключение	
			Попытка
				ШКК=ОбщегоНазначения.СформироватьШК_дляДок(ДанныеДляПечати.ДокументОснование);
				ОбластьМакета.Области["Barcode"].Объект.СодержитКС = Ложь;
				ОбластьМакета.Области["Barcode"].Объект.Сообщение  = ШКК;
				ОбластьМакета.Области["Barcode"].Объект.ТекстКода  = ШКК;
			Исключение	
			КонецПопытки;
		КонецПопытки;
		// --- Карпычев (07.06.18)
	...
	Иначе
		...
		// ЛНА XX-1363
		Если ДанныеДляПечати.Свойство("ПредставлениеПоставщика") Тогда
			
			ОбластьМакета.Параметры.ПредставлениеПоставщика = СокрЛП(ДанныеДляПечати.ПредставлениеПоставщика);			
		Иначе
		...
		// ЛНА XX-1363
		Если ДанныеДляПечати.Свойство("АдресПоставщика") Тогда
			
			ОбластьМакета.Параметры.АдресПоставщика = СокрЛП(ДанныеДляПечати.АдресПоставщика);
		...
		// ЛНА XX-1363
		Если ДанныеДляПечати.Свойство("ИННПоставщика") Тогда
			
			ОбластьМакета.Параметры.ИННпоставщика = СокрЛП(ДанныеДляПечати.ИННПоставщика);
	...
	Иначе
		...
		Иначе
			СведенияОГрузоотправителе = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(
				...
				?(ДанныеДляПечати.Грузоотправитель = "он же", ДанныеДляПечати.Поставщик, ДанныеДляПечати.Грузоотправитель)), Дата);
			...
			Если ДанныеДляПечати.Грузоотправитель="он же" И ЭтоПечатьУПД Тогда 
				ДанныеДляПечати.Грузоотправитель= ДанныеДляПечати.Организация.НаименованиеПолное+", "+СведенияОГрузоотправителе.ЮридическийАдрес;
			КонецЕсли;	
			ОбластьМакета.Параметры.ПредставлениеГрузоотправителя = ?(ЭтоПечатьУПД, "", "Грузоотправитель и его адрес: ")
			...
			Если ДанныеДляПечати.Свойство("АдресДоставки") 
				...
				ПредставлениеГрузополучателя = ?(ЭтоПечатьУПД, "", "Грузополучатель и его адрес: ") 
					...
					Иначе
				ФактическийАдресГрузополучателя  = УправлениеКонтактнойИнформацией.ПолучитьАдресИзКонтактнойИнформации(ДанныеДляПечати.Грузополучатель, "Фактический");
				//Если ЗначениеЗаполнено(ФактическийАдресГрузополучателя) Тогда 
				//	ПредставлениеГрузополучателя = ?(ЭтоПечатьУПД, "", "Грузополучатель и его адрес: ") 
				//	+ ?(НЕ ЗначениеЗаполнено(ДанныеДляПечати.Грузополучатель), 
				//	"--",
				//	ФормированиеПечатныхФорм.ОписаниеОрганизации(
				//	СведенияОГрузополучателе, "ПолноеНаименование, ЮридическийАдрес,"));
				//Иначе
					ПредставлениеГрузополучателя = ?(ЭтоПечатьУПД, "", "Грузополучатель и его адрес: ") 
					+ ?(НЕ ЗначениеЗаполнено(ДанныеДляПечати.Грузополучатель), 
					"--",
			...
			// ЛНА
			Если ДанныеДляПечати.Свойство("ПредставлениеГрузополучателя") Тогда
				
				ОбластьМакета.Параметры.ПредставлениеГрузополучателя = СокрЛП(ДанныеДляПечати.ПредставлениеГрузополучателя);
				
			Иначе				
				Если ДанныеДляПечати.Покупатель.ГрузополучательТОРГ12="" Тогда 
					Если ДанныеДляПечати.Покупатель.ГрузополучательСФ="" Тогда 
						ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ПредставлениеГрузополучателя;
					Иначе 
						ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ДанныеДляПечати.Покупатель.ГрузополучательСФ;
					КонецЕсли;
				Иначе 
					ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ДанныеДляПечати.Покупатель.ГрузополучательТОРГ12;
				КонецЕсли;				
			КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(ДанныеДляПечати.Покупатель.НаименованиеПлательщикаСФВУПД) Тогда 
			ОбластьМакета.Параметры.ПредставлениеПокупателя = ДанныеДляПечати.Покупатель.НаименованиеПлательщикаСФВУПД;	
		Иначе
		...
		КонецЕсли;
		Если ДанныеДляПечати.Покупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда 
		...
		Иначе
			ОбластьМакета.Параметры.АдресПокупателя = ?(ЭтоПечатьУПД, "", "Адрес: ")
			+ ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ФактическийАдрес,");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеДляПечати.Покупатель.ИНН_СФ) Тогда 
			//#XX-747 Kalinin V.A. ( 2018-12-19 )
			Если ДанныеДляПечати.Свойство("ДокументОснование") и  ТипЗнч(ДанныеДляПечати.ДокументОснование)=Тип("ДокументСсылка.ВозвратТоваровПоставщику") тогда 
				ОбластьМакета.Параметры.ИННПокупателя  =  ДанныеДляПечати.Покупатель.ИНН_СФ;	
			иначе 			
				ОбластьМакета.Параметры.ИННПокупателя = ?(ЭтоПечатьУПД, "", ДанныеДляПечати.Покупатель.ИНН_СФ);
			Конецесли
		Иначе
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		КонецЕсли;
		Если Строчка.Товар.Услуга Тогда 
			Строчка.ТоварНаименование = Строчка.Товар.Наименование;
	...
	ВыборкаСтрокТовары.Свернуть("Товар, ТоварНаименование, ТоварАртикул, ТоварКодТНВЭД, ЕдиницаИзмерения, ЕдиницаИзмеренияКод, Цена, СтавкаНДС,
		|НомерГТД, ПредставлениеГТД, РегистрационныйНомерТД, СтранаПроисхождения, СтранаПроисхожденияКод, ПредставлениеСтраны, ПроизводительНаименование", 
		"Количество, Сумма, СуммаНДС, СуммаСНДС, СуммаБезНДС, НомерСтроки");
	// --- Карпычев (29.01.18)
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		//	КонецЕсли;
		//КонецЕсли;
		Если ЗначениеЗаполнено(Строчка.НомерГТД) Тогда
			Строчка.ПредставлениеГТД = Строчка.НомерГТД;
		Иначе
			Если ЗначениеЗаполнено(Строчка.ПредставлениеГТД) Тогда 
				Строчка.НомерГТД = Строчка.ПредставлениеГТД;
			Иначе 	
				Строчка.ПредставлениеГТД = "";
			КонецЕсли;	
		КонецЕсли;
		...
		Иначе
			Если НЕ Строчка.Товар.Услуга Тогда 
				ОбластьМакета.Параметры.ТоварНаименование = ?(ПустаяСтрока(Строчка.ТоварАртикул), "", Строчка.ТоварАртикул + " || ") + Строчка.ТоварНаименование + ?(ПустаяСтрока(Строчка.ПроизводительНаименование), "", " || " + Строчка.ПроизводительНаименование);
			КонецЕсли;	
		КонецЕсли;
		...
		Иначе
			...
			Иначе
				Если ЗначениеЗаполнено(Строчка.ПредставлениеСтраны) Тогда				
					ОбластьМакета.Параметры.СтранаПроисхождения = Строчка.ПредставлениеСтраны;
					Попытка
						СТРМ=Справочники.СтраныМира.НайтиПоРеквизиту("НаименованиеПолное",Строчка.ПредставлениеСтраны);
						ОбластьМакета.Параметры.СтранаПроисхожденияКод = СТРМ.Код;
					Исключение
					КонецПопытки;	
					//ОбластьМакета.Параметры.СтранаПроисхожденияКод = "--";
				Иначе	
	...
	Если Свидетельство = "свидетельство  от" Тогда
		Свидетельство = "";
	КонецЕсли;
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	ПараметрыШтрихкода.Вставить("Штрихкод",        ОбщегоНазначения.СформироватьШК_дляДок(ДанныеДляПечати.Ссылка));
	...
	Если ЗначениеЗаполнено(ДанныеДляПечати.НомерСчетФактуры) Тогда
		ОбластьМакета.Параметры.Номер = ДанныеДляПечати.НомерСчетФактуры + ?(ИндексПодразделения <> 0, "/" + ИндексПодразделения, "");
		ОбластьМакета.Параметры.Дата  = Формат(ДанныеДляПечати.ДатаСчетФактуры, "ДЛФ=ДД; ДП=--");
	Иначе
	...
	Если ЭтоПечатьУКД Тогда
		Если ТипЗнч(ДанныеДляПечати.Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			ОбластьМакета.Параметры.НомерОснования 				= ?(ЗначениеЗаполнено(ДанныеДляПечати.НомерОснования),
			ДанныеДляПечати.НомерОснования + ?(ИндексПодразделения <> 0, "/" + ИндексПодразделения, ""),
			"--");
			ОбластьМакета.Параметры.ДатаОсновнания  			= ?(ЗначениеЗаполнено(ДанныеДляПечати.ДатаОснования),
			Формат(ДанныеДляПечати.ДатаОснования, "ДЛФ=ДД; ДП=--"),
			"--");
		...
		ИначеЕсли ДанныеДляПечати.КорректируемыеСчетаФактуры.Количество()>0 Тогда
...
Процедура ВывестиПодвалУниверсальногоПередаточногоДокументаВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Руководители = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизации(ДанныеДляПечати.Организация, ДанныеДляПечати.ДатаСчетФактуры,);
	ОбластьМакета.Параметры.ФИОКладовщика 		     = Руководители.Руководитель;
	ОбластьМакета.Параметры.ДолжностьКладовщика      = Руководители.РуководительДолжность;
    // --- Карпычев (30.01.18)
	Попытка
		ОбластьМакета.Параметры.СопроводительныеДокументы = ДанныеДляПечати.Комментарий;
	Исключение
	КонецПопытки;	
...
Функция ПечатьУниверсальногоПередаточногоДокумента(ДанныеДокумента, СчетФактураТребуется, СтруктураОтбораСчетаФактуры = Неопределено, ТабДокумент = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СчетФактура) ИЛИ СчетФактураТребуется Тогда
		...
		ДанныеДокумента.Вставить("ШК",ОбщегоНазначения.СформироватьШК_дляДок(СчетФактура.Документоснование.Ссылка));
		ДанныеДокумента.Вставить("НомДок",СчетФактура.ДокументОснование.Номер);
	...
	Иначе 	
		ДанныеДокумента.Вставить("Ссылка", 					ДанныеДокумента.ДокументСсылка); 
		ДанныеДокумента.Вставить("Дата", 					ДанныеДокумента.Дата);
		ДанныеДокумента.Вставить("КППКонтрагента", 	 		ДанныеДокумента.Контрагент.КПП);
		ДанныеДокумента.Вставить("ШК",ОбщегоНазначения.СформироватьШК_дляДок(ДанныеДокумента.ДокументСсылка));
		ДанныеДокумента.Вставить("НомДок",ДанныеДокумента.ДокументСсылка.Номер);
	КонецЕсли;
...
Функция ПечатьУниверсальногоКорректировочногоДокумента(ДанныеДокумента, СчетФактураТребуется, СтруктураОтбораСчетаФактуры = Неопределено, ТабДокумент = Неопределено) Экспорт
	...
	Если ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
		 ДанныеДляПечати = Документы.ВозвратТоваровОтПокупателя.ПодготовитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента(ДанныеДокумента);
...
Процедура ЗаполнитьОснование(ДанныеДокумента, ДанныеДляПечати) Экспорт
	...
	ИначеЕсли ДанныеДокумента.Свойство("ДокументОснование") И ЗначениеЗаполнено(ДанныеДокумента.ДокументОснование) 
					И ТипЗнч(ДанныеДокумента.ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")  Тогда
					
		  ПТУ = ДанныеДокумента.ДокументОснование;
	...
	Иначе
		...
		//Если ТипЗнч(ДанныеДокумента.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
		//	ДоговорКонтрагента = ДанныеДокумента.ДокументОснование.ДоговорКонтрагентаВзаиморасчетов;
		//Иначе
		//	ДоговорКонтрагента = ДанныеДокумента.ДоговорКонтрагента;
		//КонецЕсли;
		ДоговорКонтрагента = ДанныеДокумента.ДоговорКонтрагента;
		
		// ЛНА, ошибка если нет реквизита
		ДоговорКонтрагентаВзаиморасчетов = Неопределено;
		Попытка
			ДоговорКонтрагентаВзаиморасчетов = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ДанныеДокумента.ДокументОснование,"ДоговорКонтрагентаВзаиморасчетов");
		Исключение
		КонецПопытки;
		
		Если ТипЗнч(ДанныеДокумента.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") И ЗначениеЗаполнено(ДоговорКонтрагентаВзаиморасчетов) Тогда 
			ДоговорКонтрагента = ДоговорКонтрагентаВзаиморасчетов;
		КонецЕсли;
		
		//ПрефиксОснования = ?(ДоговорКонтрагента.ДоговорНаОферту, "оферта", "договор");
		лПараметрыДоговора = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДоговорКонтрагента,"ДоговорНаОферту,ДатаДоговораОферты,Дата,Номер");
		ПрефиксОснования = ?(лПараметрыДоговора.ДоговорНаОферту, "оферта", "договор");
		ДоговорКонтрагента_Дата = ?(лПараметрыДоговора.ДоговорНаОферту, лПараметрыДоговора.ДатаДоговораОферты, лПараметрыДоговора.Дата);
		//Основание = ПрефиксОснования + " № " + СокрЛп(ДоговорКонтрагента.Номер) + " от " + Формат(ДоговорКонтрагента.Дата,"ДФ=dd.MM.yyyy; ДЛФ=D"); 
		Основание = ПрефиксОснования + " № " + СокрЛп(лПараметрыДоговора.Номер) + " от " + Формат(ДоговорКонтрагента_Дата,"ДФ=dd.MM.yyyy; ДЛФ=D"); 
		// - 20180829 Пушкин XX-622
		
		//Основание = Основание + ?(ПустаяСтрока(ДанныеДокумента.ДоговорКонтрагента.Номер), "", " " + ДанныеДокумента.ДоговорКонтрагента.Номер + " от " + Формат(ДанныеДокумента.ДоговорКонтрагента.Дата, "ДФ='дд.ММ.гггг'") + " г.");
		// --- Карпычев (29.01.18)
	КонецЕсли;
...
Функция ПолучитьСтруктуруШапкиДокументаПоСсылке(Знач Ссылка)
	...
	ИначеЕсли МетаданныеДокумента.Имя = "КорректировкаЗаказаПокупателя" тогда
		...
		СтруктураКорректировкиЗаказа = Новый Структура("ЗаявкаПокупателя, Ссылка, Дата, Номер");
		УправлениеЗапасамиПартионныйУчет.ПолучитьРеквизитыОбъекта(Ссылка,СтруктураКорректировкиЗаказа);
		МетаданныеДокумента = СтруктураКорректировкиЗаказа.ЗаявкаПокупателя.Метаданные();
		Ссылка = СтруктураКорректировкиЗаказа.ЗаявкаПокупателя;
...
Функция ПолучитьИсправляемыйДокументПоступления(ДокПоступления, Исходный = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ДокПоступления) 
		И ТипЗнч(ДокПоступления) = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг")
...
КонецФункции  // --- Карпычев (26.01.18)
