...
Процедура ПолучитьПараметрыСчетаФактуры(СчетФактура, мВалютаРегламентированногоУчета, Результат) Экспорт
	...
	Для каждого ТипОснования Из ТипыОснований Цикл
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			...
			Для каждого ДокументОснование Из ДокументыОснования Цикл //Основание всегда одно
				
				ЗапросДокументОснование= Новый Запрос;
				ЗапросДокументОснование.Текст = 
				"ВЫБРАТЬ
				|	ВозвратТоваровОтПокупателяТовары.Ссылка,
				|	ВозвратТоваровОтПокупателяТовары.Ссылка.УчитыватьНДС КАК УчитыватьНДС,
				|	НЕ ВозвратТоваровОтПокупателяТовары.Ссылка.УчитыватьНДС КАК СчетФактураБезНДС,
				...
				|	ВозвратТоваровОтПокупателяТовары.Ссылка.ДоговорКонтрагента КАК Договор,
				|	ВозвратТоваровОтПокупателяТовары.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
				|	СУММА(ВозвратТоваровОтПокупателяТовары.СуммаНДС) КАК СуммаНДСДокумента,
				|	СУММА(ВозвратТоваровОтПокупателяТовары.Сумма) КАК СуммаДокумента
				|ИЗ
				...
				|	ВозвратТоваровОтПокупателяТовары.Ссылка.ВалютаДокумента";
				
				ЗапросДокументОснование.УстановитьПараметр("Ссылка", ДокументОснование);
				
				РезультатЗапроса = ЗапросДокументОснование.Выполнить();
				
				Выборка = РезультатЗапроса.Выбрать();
				
				Если Выборка.Следующий() Тогда
					
					ЗаполнитьЗначенияСвойств(Результат, Выборка, ,"СуммаНДСДокумента, СуммаДокумента");
					Результат.СуммаУменьшение    = Результат.СуммаУменьшение + Выборка.СуммаДокумента;
					Результат.СуммаНДСУменьшение = Результат.СуммаНДСУменьшение + Выборка.СуммаНДСДокумента;
					...
					СтрокаСРеквизитами.СуммаУменьшение 		= Выборка.СуммаДокумента;
					СтрокаСРеквизитами.СуммаНДСУменьшение 	= Выборка.СуммаНДСДокумента;
...
Функция РассчитатьСуммуНДС(Сумма, УчитыватьНДС, СуммаВключаетНДС, СтавкаНДС) Экспорт
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_РассчитатьСуммуНДС";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если (УчитыватьНДС) И (СуммаВключаетНДС) Тогда
		...
		//#XX-2187 Kalinin V.A. ( 2019-03-26 
		СуммаНДС = Окр(Сумма * СтНДС / (100 + СтНДС), 2);
...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(ДанныеДокумента) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ВывестиСчетФактуруВТабличныйДокумент";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ДанныеДляПечати.Покупатель.ИНН_СФ) Тогда 
			...
			Если ДанныеДляПечати.Свойство("ДокументОснование") 
				И (ТипЗнч(ДанныеДляПечати.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
				ИЛИ ТипЗнч(ДанныеДляПечати.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")) Тогда 
	...
	Если Свидетельство = "свидетельство  от" Или Свидетельство="" Тогда
		...
		//Свидетельство = "";
		Если ДанныеДляПечати.Организация.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ФизЛицо Тогда 
...
Процедура ВывестиПодвалУниверсальногоПередаточногоДокументаВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Отбор.СтруктурнаяЕдиница = ДанныеДляПечати.Организация.Ссылка;
	Отбор.ОтветственноеЛицо   = Перечисления.ОтветственныеЛицаОрганизации.Руководитель;
	СрезПоследних   = РегистрыСведений.ОтветственныеЛицаОрганизации.СрезПоследних(ДанныеДляПечати.Дата, Отбор);
	...
	Если СрезПоследних.Количество() > 0 Тогда
		Рук = СрезПоследних[0].ФизическоеЛицо;
	КонецЕсли;
	ОбластьМакета.Параметры.ФИОКладовщика=Рук;
	// +++ Карпычев (30.01.18)
	Если ДанныеДляПечати.Организация.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ФизЛицо Тогда 
	...
	КонецЕсли;	
   // --- Карпычев (30.01.18)
	//Рудаков задача XX-2092
	Если Не ДанныеДляПечати.Покупатель.ОснованиеДляПечати="" Тогда 
		ОбластьМакета.Параметры.Основание=ДанныеДляПечати.Покупатель.ОснованиеДляПечати;											
	КонецЕсли;	
	Попытка
...
Функция ПечатьУниверсальногоПередаточногоДокумента(ДанныеДокумента, СчетФактураТребуется, СтруктураОтбораСчетаФактуры = Неопределено, ТабДокумент = Неопределено) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_УчетНДС_ПечатьУниверсальногоПередаточногоДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если ТабДокумент = Неопределено Тогда
		...
		// Устанавливаем параметры печати и колонтитулы
		если типзнч(ДанныеДокумента.ДокументСсылка)=типзнч(Документы.ВозвратТоваровПоставщику.ПустаяСсылка()) Тогда 
