...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Иначе
		...
		Если ДанныеДляПечати.Покупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда 
			...
			+ ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ФактическийАдрес,");//"ЮридическийАдрес,");
		Иначе
			ОбластьМакета.Параметры.АдресПокупателя = ?(ЭтоПечатьУПД, "", "Адрес: ")
		...
		Если ЗначениеЗаполнено(КПП) И ДанныеДляПечати.Покупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
			КПП = "/" + КПП;
		КонецЕсли;
