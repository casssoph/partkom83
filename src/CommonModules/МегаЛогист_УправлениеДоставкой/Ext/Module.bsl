...
Процедура ПолучитьТоварыДляМаршрутногоЗадания(Объект,Товары) Экспорт
	...
	Если ТипЗнч(Объект.ЗаказПокупателя) = Тип("ДокументСсылка.МегаЛогист_МаршрутноеЗадание") тогда
		Док=Объект.ЗаказПокупателя.ЗаказПокупателя;
	иначе
		Док=Объект.ЗаказПокупателя;	
	КонецЕсли;
