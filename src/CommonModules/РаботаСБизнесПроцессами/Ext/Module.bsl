...
Процедура ПроверитьЗаполнениеРеквизитов(Задача, Отказ) Экспорт
	...
	Если ТипБизнесПроцесса = Тип("БизнесПроцессСсылка.ВводНовогоПользователя1С") Тогда

		ТочкиМаршрута = БизнесПроцессы.ВводНовогоПользователя1С.ТочкиМаршрута;

		// В данных точках необходимо проверить наличие рецензии в регистре "Рецензии"
		//Если ТочкаМаршрута = ТочкиМаршрута.ВводФИО Тогда
		//	
		//	Если Не ЗначениеЗаполнено(Задача.Фамилия) или Не ЗначениеЗаполнено(Задача.Имя) или Не ЗначениеЗаполнено(Задача.Должность) Тогда
		//		
		//		ОбщегоНазначения.СообщитьОбОшибке(
		//		"Заполните все реквизиты", Отказ, Заголовок);
		//	КонецЕсли;
		//ИначеЕсли  ТочкаМаршрута = ТочкиМаршрута.ВводЛогинаПароля Тогда
		//	Если Не ЗначениеЗаполнено(Задача.Логин) или Не ЗначениеЗаполнено(Задача.АдресЭлектроннойПочты) или Не ЗначениеЗаполнено(Задача.НомерКомнаты) или не ЗначениеЗаполнено(Задача.НомерТелефона) Тогда
		//		ОбщегоНазначения.СообщитьОбОшибке(
		//		"Заполните все реквизиты", Отказ, Заголовок);
		//	КонецЕсли;	
		//	
		//КонецЕсли;
    КонецЕсли;
	Если ТипБизнесПроцесса = Тип("БизнесПроцессСсылка.УстановкаКППКонтрагентов") Тогда

		ТочкиМаршрута = БизнесПроцессы.УстановкаКППКонтрагентов.ТочкиМаршрута;

		Если ТочкаМаршрута = ТочкиМаршрута.Действие1 Тогда
		...
		ИначеЕсли ТочкаМаршрута = ТочкиМаршрута.Действие2 Тогда
...
//  Отказ - булево, признак отказа от открытия формы.
//
Процедура ОткрытьФормуВыполненияЗадачиФормаРаботыСКонтрагентом(Задача,
																	СтандартнаяОбработка = Истина,
																	Отказ = Ложь ) Экспорт
	Отказ                = Истина;
	СтандартнаяОбработка = Ложь;
	Задача.ПолучитьФорму("ФормаРаботыСКонтрагентом").Открыть();
...
//  Отказ - булево, признак отказа от открытия формы.
//
Процедура ОткрытьФормуВыполненияЗадачиФормаЗаявкиНаДобавлениеПользователя(Задача,
																	СтандартнаяОбработка = Истина,
																	Отказ = Ложь ) Экспорт
	Отказ                = Истина;
	СтандартнаяОбработка = Ложь;
	Задача.ПолучитьФорму("ФормаЗаявкиНаДобавлениеПользователя").Открыть();
...
Процедура ОткрытьФормуВыполненияЗадачи(Задача, Отказ) Экспорт
	...
	Если ТипБизнесПроцесса = Тип("БизнесПроцессСсылка.УстановкаКППКонтрагентов") Тогда
		...
		ОткрытьФормуВыполненияЗадачиФормаРаботыСКонтрагентом(Задача,, Отказ);

	ИначеЕсли ТипБизнесПроцесса = Тип("БизнесПроцессСсылка.ВводНовогоПользователя1С") Тогда

		ОткрытьФормуВыполненияЗадачиФормаЗаявкиНаДобавлениеПользователя(Задача,, Отказ);
...
//
// Параметры:
//  ДокументЗаявкаПокупателя - ДокументСсылка.ЗаявкаПокупателя.
//
Процедура ОткрытьСписокСогласованиеЗаказаПокупателяПоДокументу(ДокументЗаявкаПокупателя) Экспорт

	Если ТипЗнч(ДокументЗаявкаПокупателя) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда

		ФормаСписка = БизнесПроцессы.СогласованиеЗаказаПокупателя.ПолучитьФормуСписка();
		ФормаСписка.БизнесПроцессСписок.Отбор.ОбъектСогласования.Установить(ДокументЗаявкаПокупателя);
		ФормаСписка.Открыть();
...
Функция СформироватьПараметрыШапкиЗадач(ВыборкаПараметры) Экспорт
	...
	ПараметрыЗадач.Вставить("Должность");
...
Функция УстановитьПараметрыЗадачи(Параметры, ЗадачаПользователя = Неопределено) Экспорт
	...
	ЗадачаПользователя.Должность   = Параметры.Должность;
