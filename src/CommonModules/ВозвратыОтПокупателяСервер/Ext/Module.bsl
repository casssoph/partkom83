...
Функция ВыполнитьКомандуДляАРВ( пКоманда, пФорма = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	//Установим адресацию
	Если ЭтоКомандаИзмененияСтатуса Тогда
		НовыйОтвественный = пКоманда.СледующийСтатус.Ответственный;
		Если ЗначениеЗаполнено(НовыйОтвественный)
			И (Не ЗначениеЗаполнено(ДокументОбъект.Ответственный) 
					ИЛИ ДокументОбъект.Ответственный.ГруппаДоступаКСтатусамПроцессаВозвратаОтПокупателя <> НовыйОтвественный) Тогда
			
			ДокументОбъект.Ответственный = НовыйОтвественный;
