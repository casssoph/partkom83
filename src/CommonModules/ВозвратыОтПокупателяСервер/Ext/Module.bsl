...
Функция ВыполнитьКомандуДляАРВ( пКоманда, пФорма = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	Если ЭтоКомандаИзмененияСтатуса Тогда
		Для каждого СтрокаУсловия Из пКоманда.ТаблицаУсловий Цикл
			...
			Выполнить(СтрокаУсловия.Условие.Условие);
			...
			Если Результат <>  СтрокаУсловия.Результат Тогда
				ТекстОшибки = "Команда "+пКоманда.Наименование+". Не выполнено условие """+СтрокаУсловия.Условие.Наименование+""" ("+СтрокаУсловия.Условие.Код+")!";
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,,,СтатусСообщения.Внимание);
				Если ЗначениеЗаполнено(СтрокаУсловия.Условие.СообщениеПриНевыполнении) Тогда
					Сообщить(СтрокаУсловия.Условие.СообщениеПриНевыполнении);
				КонецЕсли;	
				Успешно =  Ложь;
			КонецЕсли;
	...
	//Обработчики Команды
	Для каждого СтрокаОбработчика Из пКоманда.Обработчики Цикл
		
		Попытка
			Выполнить(СтрокаОбработчика.Обработчик.Алгоритм);
		Исключение
			ТекстОшибки = "Команда "+пКоманда.Наименование+". Ошибка выполнения алгоритма  """+СтрокаОбработчика.Обработчик+""" ("+СтрокаОбработчика.Обработчик.Код+")! Описание ошибки: "+ОписаниеОшибки();
			ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,,,СтатусСообщения.Внимание);
			Успешно =  Ложь;
		КонецПопытки;
