...
Функция ПолучитьЦенуНоменклатуры(Номенклатура, ХарактеристикаНоменклатуры = Неопределено, Знач ТипЦен, ДатаПараметр = Неопределено, ЕдиницаИзмерения = Неопределено,
	...
	Если ПолученнаяЦена = 0 И ЗначениеЗаполнено(ТипЦен) Тогда // особых условий нет
		...
		Если ТипЦен.Рассчитывается Тогда
			...
			|	ЦеныНоменклатурыСрезПоследних.Номенклатура = ВложенныйЗапрос.Номенклатура
			|";
		КонецЕсли;
		Запрос.Текст = Запрос.Текст + "
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
//
Функция ПолучитьПроцентСкидкиНаценкиЦеныНоменклатуры(Номенклатура,  ТипЦен, Дата = Неопределено, ТолькоИзРегистра = Ложь, НетДанных = Истина, ВернутьВсегда = Ложь) Экспорт
...
Процедура ОбработатьИзмененияПоКнопкеЦеныВалюта(ДокументОбъект, СпособЗаполненияЦен,
	...
	ИспользоватьКолонкуСкладПриПродаже = Ложь;
	...
	НеРассчитыватьПоРозничнымСкладам = Истина;
	...
	Если СтруктураЗначений.ПерезаполнитьЦеныПоТипу и ЕстьЦена Тогда
		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			//   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Склад) Тогда
			//Если НеРассчитыватьПоРозничнымСкладам Тогда
...
Функция ОпределитьНеобходимостьПерерасчетаЦены(СтруктураЗначений) Экспорт
		...
		Возврат (СтруктураЗначений.НовыйУчитыватьНДС <> СтруктураЗначений.ТекущийУчитыватьНДС) ИЛИ
				...
				Или ?(СтруктураЗначений.Свойство("НовыйНДСВключенВСтоимость"),
				СтруктураЗначений.НовыйНДСВключенВСтоимость<>СтруктураЗначений.ТекущийНДСВключенВСтоимость,Ложь);
...
Функция СформироватьЗапросПоЦенам(СтруктураЗначений, СпособЗаполненияЦен, МассивНоменклатуры,
	...
	Если СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры Тогда
		...
		|	0 КАК ПроцентНаценкиПоУсловиямПродаж,
	...
	ИначеЕсли СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатурыКонтрагентов Тогда
		...
		|			ЦеныКонтрагента.Валюта КАК Валюта,
		|			ЦеныКонтрагента.Цена КАК Цена,
		|			ЦеныКонтрагента.ЕдиницаИзмерения КАК ЕдиницаИзмерения
		|	ИЗ (	
		|		ВЫБРАТЬ
...
Функция ПолучитьРежимОкругления(Контрагент) Экспорт
	
	ОкруглениеДо10Рублей = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ОкруглениеДо10Рублей");
	РежимОкругленияПользователя = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ОкруглятьЦеныВЗаявкахИРеализациях"); 
	Если Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		текРежимОкругления = 2;
	ИначеЕсли ОкруглениеДо10Рублей Тогда 
		текРежимОкругления = -1;
	Иначе
		текРежимОкругления = РежимОкругленияПользователя;
	КонецЕсли;
	
	Возврат текРежимОкругления;
	
КонецФункции

Функция ИспользоватьЦенуСоСкидкойВДокументах() Экспорт
	
	 Возврат Константы.ИспользоватьЦенуСоСкидкойВДокументах.Получить();
	
 КонецФункции
 
 Функция МинимальнаяНаценка() Экспорт
	 
	 МинимальнаяНаценка = УправлениеПользователями.ПолучитьЗначениеПраваДляТекущегоПользователя(ПланыВидовХарактеристик.ПраваПользователей.РазрешитьНестандартнуюСкидку, Ложь).Получить(0).Значение;
	 Если МинимальнаяНаценка = Неопределено Или МинимальнаяНаценка = 0 Тогда
		 МинимальнаяНаценка = УправлениеПользователями.ПолучитьМинимальнуюНаценку(ПолныеПрава.ТекущийПользователь());
		 МинимальнаяНаценка = ?(МинимальнаяНаценка = Неопределено Или МинимальнаяНаценка = 0, 10, МинимальнаяНаценка);
	 КонецЕсли;
	 
	 Возврат МинимальнаяНаценка;
