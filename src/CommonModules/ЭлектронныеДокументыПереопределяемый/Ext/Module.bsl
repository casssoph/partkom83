...
Процедура ЗаполнитьПараметрыЭДПоИсточнику(Источник, ПараметрыЭД, ФорматCML = Ложь) Экспорт
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.ЗаявкаПокупателя")
		 ИЛИ ТипИсточника = Тип("ДокументОбъект.ЗаявкаПокупателя") Тогда
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") 
		 ИЛИ ТипИсточника = Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг") Тогда
...
Процедура ПолучитьСтруктуруКлючевыхРеквизитовОбъекта(ИмяОбъекта, СтруктураКлючевыхРеквизитов) Экспорт
	...
	ИначеЕсли ИмяОбъекта = "Документ.ЗаказПоставщику" Тогда
		...
		СтрокаРеквизитовОбъекта = ("Номенклатура, Содержание, Количество, Цена, Сумма, СуммаНДС, СтавкаНДС");
		СтруктураКлючевыхРеквизитов.Вставить("Услуги", СтрокаРеквизитовОбъекта);
	ИначеЕсли ИмяОбъекта = "Документ.ЗаявкаПокупателя" Тогда
		// шапка
		СтрокаРеквизитовОбъекта = ("Дата, Номер, Организация, Контрагент, ВалютаДокумента, СуммаДокумента, УчитыватьНДС");
		СтруктураКлючевыхРеквизитов.Вставить("Шапка", СтрокаРеквизитовОбъекта);
		// ТЧ
...
Процедура ЗаполнитьДанныеПоОтветуНаЗаказ(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента) Экспорт
	...
	|	ДокЗаказ.Сумма КАК Сумма
	|ИЗ
	|	Документ.ЗаявкаПокупателя.Товары КАК ДокЗаказ
	|ГДЕ
	|	ДокЗаказ.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДокЗаказ.Сумма
	|ИЗ
	|	Документ.ЗаявкаПокупателя.Услуги КАК ДокЗаказ
	|ГДЕ
	|	ДокЗаказ.Ссылка = &Ссылка";
...
Процедура ПодготовитьДанныеПоНакладной(СсылкаНаОбъект, СтруктураЭД, СтруктураПараметров) Экспорт
	...
	Если ТипЗнч(РеквизитыШапки.Сделка) = Тип("ДокументСсылка.ЗаявкаПокупателя")
		И ЗначениеЗаполнено(РеквизитыШапки.Сделка) Тогда
		
		ЗаявкаПокупателя = РеквизитыШапки.Сделка;
		
		СтруктураПараметров.Вставить("НомерПоДаннымПоставщика",	ЗаявкаПокупателя.Номер);
		СтруктураПараметров.Вставить("ДатаПоДаннымПоставщика", 	ЗаявкаПокупателя.Дата);
...
Функция СохранитьДанныеОбъектаВБД(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено, Записывать = Истина) Экспорт
	...
	ИначеЕсли СтрокаДляЗагрузки.ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
		НайденныйОбъект = НайтиСоздатьЗаявкаПокупателя(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца, Записывать);
...
Функция ПодготовитьСтруктуруДляЗаказаПокупателя(СтрокаДляЗагрузки, ДеревоРазбора)
	...
	ДанныеДляОбъекта = Новый Структура;
	ДанныеЗаполненияШапки = Новый Структура;
	Товары = Документы.ЗаявкаПокупателя.ПустаяСсылка().Товары.ВыгрузитьКолонки();
	Услуги = Документы.ЗаявкаПокупателя.ПустаяСсылка().Услуги.ВыгрузитьКолонки();
...
Функция ПодготовитьСтруктуруДляКорректировкиПоступления(СтрокаДляЗагрузки, ДеревоРазбора)
	...
	Товары 		   = Документы.КорректировкаПоступленияТоваровУслуг.ПустаяСсылка().Товары.ВыгрузитьКолонки();
	Услуги 		   = Документы.КорректировкаПоступленияТоваровУслуг.ПустаяСсылка().Услуги.ВыгрузитьКолонки();
	СерийныеНомера = Документы.КорректировкаПоступленияТоваровУслуг.ПустаяСсылка().СерийныеНомера.ВыгрузитьКолонки();
	КлючСвязи = 1;
...
Функция НайтиДокумент(ВидЭД, Контрагент, РеквизитыИБ = Неопределено, РеквизитыИБКонтрагента = Неопределено)
	...
	Если ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
		...
		|ИЗ
		|	Документ.ЗаявкаПокупателя КАК ДокументПоиска
		|ГДЕ
...
Функция НайтиСоздатьЗаявкаПокупателя(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено, Записывать = Истина)
	...
	Попытка
		...
		Если ДокументОбъект.ЭтоНовый() И ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
			ДокументОбъект.УчитыватьНДС = УчетНДСПовтИсп.УчитыватьНДСПоВариантуУчета(ДокументОбъект.Организация.ВариантУчетаНДС, ДокументОбъект.ДоговорКонтрагента.ВидОплаты);
			//УчетНДС.ОрганизацияУчитываетНДС(ДокументОбъект.Организация, ДокументОбъект.Дата);
		Иначе
			ДокументОбъект.УчитыватьНДС = Истина;
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение на основании ЭД'"), 
			УровеньЖурналаРегистрации.Ошибка, 
			Метаданные.Документы.ЗаявкаПокупателя, 
			СсылкаНаВладельца, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Функция НайтиСоздатьКорректировкуПоступления(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено, Записывать = Истина)
	...
	Попытка
		...
		Иначе  // создаем новый
			...
			УстановитьПривилегированныйРежим(Истина);
			ДокументОбъект = Документы.КорректировкаПоступленияТоваровУслуг.СоздатьДокумент();
			ДокументОбъект.Дата = ТекущаяДатаСеанса();
...
Функция ПолучитьДанныеРеализации(СсылкаНаОбъект)
	...
	|   РеализацияТоваровУслуг.ЗаявкаПокупателя                                         КАК ДокументОснование,
	...
	|	РеализацияТоваровУслуг.Количество,
	|	0,
	|	РеализацияТоваровУслуг.ЗаявкаПокупателя,
	|	-1
	|ИЗ
...
Функция ПодготовитьДанныеАктаОбОказанииУслуг(СсылкаНаОбъект)
	...
	|   Акт.ЗаявкаПокупателя                          КАК ДокументОснование
	|ИЗ
	|	
	...
	|
	|	//РеализацияТоваровУслуг Акт.ЗаявкаПокупателя,
	|	//КорректировкаРеализации Акт.ЗаявкаПокупателя,
	|
	|	Акт.СтавкаНДС,
	|	Акт.Цена
	|	
...
Функция ПолучитьДанныеСогласованногоИзменения(СсылкаНаОбъект)
	...
	|	КОНЕЦ                                                                           КАК МассаБрутто,
	|   КорректировкаРеализации.ЗаявкаПокупателя                                         КАК ДокументОснование
	|ИЗ
	...
	|	0,
	|   КорректировкаРеализации.ЗаявкаПокупателя
	|	
	|ИЗ
