...
Функция ПолучитьДанныеВзаиморасчетовПоМассивуЛогинов(МассивКонтрагентов,ДатаНачала,ДатаКонца)
	Отчет  =  Отчеты.ВедомостьПоВзаиморасчетамУПР.Создать();
	КомпоновщикНастроек = новый КомпоновщикНастроекКомпоновкиДанных;
	СхемаВыгрузки = Отчет.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
  	 ВариантОтчета =СхемаВыгрузки.ВариантыНастроек.ОборотыПоЛогинам ;
...
функция ПреобразоватьДеревоЗначенийОборотовВСтруктуру(ДеревоЗначений)
	...
	Для каждого СтрокаЛогина из  ДеревоЗначений.Строки цикл 
		...
		Для Каждого СтрокаПериод из СтрокаЛогина.строки  Цикл 
			СтруктураПериода.Вставить(формат(СтрокаПериод.ПериодМесяц,"ДФ=dd.MM.yyyy"),СтрокаПериод.СуммаРублиОборот); 		 
		КонецЦикла;
