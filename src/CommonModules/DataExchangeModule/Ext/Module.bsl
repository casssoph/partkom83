...
Функция AddRegistry(ExchangePlan, ExchangeID, Data) Экспорт  //Зарегистрировать изменения на узле
	...
	ДопустимыеТипыОбъектов.Добавить("Справочники.Контрагенты");
...
Функция  ПолучитьДокументыДляОбменаТоплог(ДатаНачала, ДатаОкончания) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСтроки =  "" + Выборка.ТипДокумента + "," +  Документ.УникальныйИдентификатор() + "," + Документ.Номер + "," + Выборка.СуммаДокумента + Символы.ПС;
