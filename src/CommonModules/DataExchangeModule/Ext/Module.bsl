...
Функция ПроверитьДокументыДляОбменаТоплог(ТекстЗапроса) Экспорт
	...
	СтатусыПеремещение.Вставить("Поступил", Справочники.СтатусыДокументов.ПеремещениеТоваровПоступил);
	...
	Для каждого Строка из Строки  Цикл
		...
		Статус = Реквизиты[2];
		ДатаПервойПечати = Реквизиты[3];
		СуммаПриПервойПечати = Реквизиты[4];
		...
		Иначе	
			...
			//Проверка даты первой печати и суммы документа
			ЗапросСчетФактура = Новый Запрос;
			
			ЗапросСчетФактура.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
			...
			ЗапросСчетФактура.УстановитьПараметр("ДокументОснование", СсылкаНаДокумент);
			
			Результат = ЗапросСчетФактура.Выполнить();
			
			Если Результат.Пустой() Тогда
				
				//Нет счет-фактуры
				СообщениеДиагностики = СообщениеДиагностики + "Нет счет-фактуры.";
				Ошибка = "Да";

			Иначе
				
				//Счет-фактура найдена
				ВыборкаСчетФактура = Результат.Выбрать();
				ВыборкаСчетФактура.Следующий();
				
				
				Если НачалоДня(Дата(ДатаПервойПечати)) <>  НачалоДня(ВыборкаСчетФактура.ДатаПервойПечати) Тогда
					
					СообщениеДиагностики = СообщениеДиагностики + "Не совпадает дата первой печати." + " В ВМС: " + НачалоДня(Дата(ДатаПервойПечати)) + " , в 1с: " +  НачалоДня(ВыборкаСчетФактура.ДатаПервойПечати) + Символы.ПС ;
					Ошибка = "Да";
					
				КонецЕсли;
				
				Если Число(СуммаПриПервойПечати) <>  ВыборкаСчетФактура.Сумма Тогда
					
					СообщениеДиагностики = СообщениеДиагностики + "Не совпадает сумма документа." + " В ВМС: " + Число(СуммаПриПервойПечати) + " , в 1с: " +  ВыборкаСчетФактура.Сумма + Символы.ПС ;
					Ошибка = "Да";
		...
		Строка = "" + ID + "," + Ошибка + "," + СообщениеДиагностики + "," + ТипДокумента + "," + СсылкаНаДокумент.Номер + Символы.ПС;
