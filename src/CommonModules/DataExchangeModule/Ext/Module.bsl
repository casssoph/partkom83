...
Функция GetDocForBit(Data,Number,Answer) Экспорт
	...
	|	РеализацияТоваровУслугТовары.СтавкаНДС КАК nds_use,
...
Функция ПроверитьДокументыДляОбменаТоплог(ТекстЗапроса) Экспорт
	...
	//Статусы
	СтатусыРеализация = Новый Массив;
	СтатусыРеализация.Добавить(Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен);
	
	СтатусыПеремещение = Новый Массив;
	СтатусыПеремещение.Добавить(Справочники.СтатусыДокументов.ПеремещениеТоваровОтгружен);
	
	СтатусыПоступление = Новый Массив;
	СтатусыПоступление.Добавить(Справочники.СтатусыДокументов.ПоступлениеТоваровОтгружен);
	СтатусыПоступление.Добавить(Справочники.СтатусыДокументов.ПоступлениеТоваровПринят);
	...
	Для каждого Строка из Строки  Цикл
		...
		Иначе	
			...
			Поиск = Статусы.Свойство(ТипДокумента, МассивСтатусов);
			Если  МассивСтатусов.Найти(СсылкаНаДокумент.СтатусДокумента) = Неопределено Тогда
				//Статус не совпадает
				СообщениеДиагностики = "Документ проведен, но имеет статус " + СсылкаНаДокумент.СтатусДокумента;
				Найдено = "Не найдено";  
			Иначе
				//Статус совпадает
