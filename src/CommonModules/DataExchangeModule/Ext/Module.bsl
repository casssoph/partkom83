...
Функция  ВыгрузитьОборотыПоКлиентам(UserLoginArray) Экспорт 
МассивКонтрагентов =ПолучитьМассивЛогиновПоПараметру(UserLoginArray);	
ДанныеОборотов = ПолучитьДанныеВзаиморасчетовПоМассивуЛогинов(МассивКонтрагентов);
ЗаписьJSON =  новый ЗаписьJSON;
...
КонецФункции	
Функция ПолучитьДанныеВзаиморасчетовПоМассивуЛогинов(МассивКонтрагентов)
	...
	СхемаВыгрузки = Отчет.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
	 ...
	 ПериодОтчета.ДатаНачала = НачалоМесяца(ДобавитьМесяц(ТекущаяДата(),-2));
