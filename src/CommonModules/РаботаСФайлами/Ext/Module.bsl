...
//   Булево – Истина, если каталог выбран, Ложь, если нет.
//
Функция СохранитьФайлНаДиске(Хранилище, ИмяФайла, ТолькоЧтение, СпособПерезаписи, ВопросОПерезаписи = Истина, ИмяСправочника = "ХранилищеДополнительнойИнформации", ИдентификаторВоВнешнемХранилище = Неопределено) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_СохранитьФайлНаДиске";
	...
	Попытка
		...
		Если ТипЗнч(ИдентификаторВоВнешнемХранилище) = Тип("Число") И ИдентификаторВоВнешнемХранилище <> 0 Тогда
			...
			ИдентификаторВоВнешнемХранилище,
			ТекстОшибки,
			Истина,
			Ложь);
			
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ОбщегоНазначения.СообщитьОбОшибке("Ошибка получения файла из внешнего хранилища: "+ТекстОшибки);
				Возврат Ложь;
			КонецЕсли;
		Иначе
			//Типовой код
...
Процедура ДобавитьФайлы(ОбъектФайла, ВидДанных = Неопределено, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_ДобавитьФайлы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
// Сохранить файл
//
Процедура СохранитьФайл(СтруктураПараметров, Знач ИмяФайла, Хранилище, СпособПерезаписи = "", ИдентификаторВоВнешнемХранилище = Неопределено)
       
	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_СохранитьФайл";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	СохранитьФайлНаДиске(Хранилище, ИмяФайла, СтруктураПараметров.ТолькоЧтение, СпособПерезаписи,,, ИдентификаторВоВнешнемХранилище);

КонецПроцедуры

// Сохранить в файл из хранилища
//
Процедура СохранитьФайлИзХранилища(ИмяФайла, Хранилище, ИмяКаталога = Неопределено, ИмяСправочника = "ХранилищеДополнительнойИнформации", ИдентификаторВоВнешнемХранилище = Неопределено) Экспорт
       
	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_СохранитьФайлИзХранилища";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	СохранитьФайл(СтруктураПараметров, ИмяФайла, Хранилище, СпособПерезаписи, ИдентификаторВоВнешнемХранилище);
...
//                 информации.
//
Процедура СохранитьФайлы(ОбъектФайла, ВыделенныеСтроки = Неопределено, ИмяКаталога = Неопределено, ИмяСправочника = "ХранилищеДополнительнойИнформации", ИдентификаторВоВнешнемХранилище = Неопределено) Экспорт

	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_СохранитьФайлы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если ВыделенныеСтроки = Неопределено Тогда
		...
		СохранитьФайл(СтруктураПараметров, ОбъектФайла.ИмяФайла, ОбъектФайла.Хранилище, СпособПерезаписи, ИдентификаторВоВнешнемХранилище);
		
	Иначе
		
		Для каждого СсылкаФайл из ВыделенныеСтроки Цикл
			
			СохранитьФайл(СтруктураПараметров, СсылкаФайл.ИмяФайла, СсылкаФайл.Хранилище, СпособПерезаписи, СсылкаФайл.ИдентификаторВоВнешнемХранилище);
...
//
Процедура ОткрытьФайлИзХранилища(ИмяФайла, Хранилище, СпособПерезаписи = "", ВопросОПерезаписи = Истина, ИдентификаторВоВнешнемХранилище = Неопределено) Экспорт

	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_ОткрытьФайлИзХранилища";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СохранитьФайлНаДиске(Хранилище, ИмяВременногоФайла, Ложь, СпособПерезаписи, ВопросОПерезаписи,, ИдентификаторВоВнешнемХранилище) Тогда
...
Процедура ОткрытьФайлы(ОбъектФайла, ВыделенныеСтроки = Неопределено, ВопросОПерезаписи = Истина) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_РаботаСФайлами_ОткрытьФайлы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если ВыделенныеСтроки = Неопределено Тогда
		...
		СпособПерезаписи = "";
		ОткрытьФайлИзХранилища(ОбъектФайла.ИмяФайла, ОбъектФайла.Хранилище, СпособПерезаписи, ВопросОПерезаписи, ОбъектФайла.ИдентификаторВоВнешнемХранилище);
	...
	Иначе
		...
		Для каждого СсылкаФайл из ВыделенныеСтроки Цикл
			...
			ОткрытьФайлИзХранилища(СсылкаФайл.ИмяФайла, СсылкаФайл.Хранилище, СпособПерезаписи, ВопросОПерезаписи, СсылкаФайл.ИдентификаторВоВнешнемХранилище);
