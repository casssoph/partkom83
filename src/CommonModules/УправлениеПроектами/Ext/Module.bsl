...
Функция ПолучитьТаблицуРаспределенияЗаявкаПокупателя(Ссылка,Дата)
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	ИЗ
	             |		РегистрНакопления.ЗаказыПокупателей.Остатки(&Дата, ЗаявкаПокупателя = &Ссылка) КАК ЗаказыПокупателейОстатки
	             ...
	             |			    ИЗ
	             |			        РегистрНакопления.ЗаказыПокупателей.Остатки(&Дата, ЗаявкаПокупателя = &Ссылка) КАК ЗаказыПокупателейОстатки)) КАК УстановкаПроектовДляНоменклатурыСрезПоследних
	             ...
	             |	ИЗ
	             |		РегистрНакопления.Продажи.Обороты(, , , ЗаявкаПокупателя = &Ссылка) КАК ПродажиОбороты) КАК ДвиженияПоЗаказу
	             |
	             |СГРУППИРОВАТЬ ПО
...
КонецФункции // ПолучитьТаблицуРаспределенияЗаявкаПокупателя()
...
Процедура РаспределитьПоступлениеДСПоПроектам(ТабДвижений, Строка, ДокументРасчетов, Дата, ВалютаУпрУчета)
	...
	Если ТипДокумента=Тип("ДокументСсылка.ЗаявкаПокупателя")Тогда
		
		ТабРаспределения=ПолучитьТаблицуРаспределенияЗаявкаПокупателя(ДокументРасчетов,Дата);
