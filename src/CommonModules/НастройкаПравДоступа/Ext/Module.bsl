...
Процедура ПередЗаписьюДокументаПроверкаДоступностиПериода(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("СнятьОграничениеПоДатеЗапрета")
		И Не Источник.ДополнительныеСвойства.Свойство("РежимБога") Тогда 
...
Процедура ПередЗаписьюРегистраДатаЗапретаРедактированияПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	//Возврат; //Отключена. Избыточная проверка. Нужна только, если период в регистре не совпадает с датой документа. Валиахметов А.А. 17.04.2018	
	//Включена, проверка нужна, тк движения могут меняться записью набора записей, без записи самого документа. ХудинВВ 09.11.2018
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПередЗаписьюРегистраДатаЗапретаРедактированияПередЗаписью";
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("СнятьОграничениеПоДатеЗапрета")
		И Не Источник.ДополнительныеСвойства.Свойство("РежимБога") Тогда 
		//Семенов И.П. 08.07.2019 XX-2844(
		Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2844) Тогда 
			ПроверкаПериодаЗаписейРегистровСУчетомВидаДокумента(Источник, Отказ);
		Иначе 
		//)Семенов И.П.
...
Процедура ПроверкаПериодаДокумента(ДокументОбъект, Отказ, РежимЗаписи = Неопределено)
	...
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПроверкаПериодаДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	//Семенов И.П. 05.07.2019 XX-2844(
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2844) Тогда 
		ПроверкаПериодаДокументаСУчетомВидаДокумента(ДокументОбъект, Отказ, РежимЗаписи);
	Иначе
	//)Семенов И.П.
...
//Семенов И.П. 05.07.2019 XX-2844(
Процедура ПроверкаПериодаДокументаСУчетомВидаДокумента(ДокументОбъект, Отказ, РежимЗаписи)
	
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПроверкаПериодаДокументаСУчетомВидаДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ТаблицаГраниц = ПараметрыСеанса.ГраницыЗапретаИзмененияДанных.Получить();
	
	Если ТаблицаГраниц <> Неопределено Тогда
	
		ПараметрыПроверкиДокумента = ПолучитьПараметрыПроверкиДокумента(ДокументОбъект);
		
		Если Не ДокументОбъект.ЭтоНовый() Тогда
			СтараяВерсияДокумента = ПолучитьВерсиюДокументаПередИзменением(ДокументОбъект, ПараметрыПроверкиДокумента);
	    	ПроверитьВерсиюДокументаСУчетомВидаДокумента(СтараяВерсияДокумента, ПараметрыПроверкиДокумента, ТаблицаГраниц, Отказ);
		КонецЕсли;
				
		Если Не Отказ Тогда
			ПроверитьВерсиюДокументаСУчетомВидаДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, ТаблицаГраниц, Отказ, РежимЗаписи);
		КонецЕсли;
...
Процедура ПроверитьВерсиюДокументаСУчетомВидаДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, ТаблицаГраниц, Отказ, РежимЗаписи = Неопределено)
	
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПроверитьВерсиюДокументаСУчетомВидаДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если НЕ ПараметрыПроверкиДокумента.ПроверятьПроведениеДокумента 
		ИЛИ ДокументОбъект.Проведен 
		ИЛИ РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда			
			
		Организация = Неопределено;
		Если ПараметрыПроверкиДокумента.ЕстьОрганизация Тогда 
			Организация = ДокументОбъект.Организация;
		КонецЕсли;
		ГраницаЗапрета = ПолучитьАктуальнуюГраницуЗапрета(ТаблицаГраниц, Организация, ПараметрыПроверкиДокумента.ДокументПустаяСсылка);
...
Функция ПолучитьАктуальнуюГраницуЗапрета(ТаблицаГраниц, Организация = Неопределено, ВидДокумента = Неопределено)
	
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПолучитьАктуальнуюГраницуЗапрета";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	ТаблицаГраницРезультат = ТаблицаГраниц.СкопироватьКолонки("ГраницаЗапретаИзменений,Рейтинг");
	
	МассивПараметровПоиска = Новый Массив;
	МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Неопределено, Неопределено));
	
	Если Организация <> Неопределено И ВидДокумента <> Неопределено Тогда 		
		МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Организация, ВидДокумента));
		МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Организация, Неопределено));
		МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Неопределено, ВидДокумента));		
	ИначеЕсли Организация <> Неопределено И ВидДокумента = Неопределено Тогда	
		МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Организация, Неопределено));
	ИначеЕсли Организация = Неопределено И ВидДокумента <> Неопределено Тогда
		МассивПараметровПоиска.Добавить(Новый Структура("Организация, ВидДокумента", Неопределено, ВидДокумента));
	КонецЕсли;

	Для Каждого СтруктураПоиска Из МассивПараметровПоиска Цикл 
		
		СтрокиГраниц = ТаблицаГраниц.НайтиСтроки(СтруктураПоиска);
		Если СтрокиГраниц.Количество()>0 Тогда 
			
			ЗаполнитьЗначенияСвойств(ТаблицаГраницРезультат.Добавить(),СтрокиГраниц[0]);
	...
	Если ТаблицаГраницРезультат.Количество() > 0 Тогда 
		
		ТаблицаГраницРезультат.Сортировать("Рейтинг УБЫВ");	
		ГраницаЗапрета = ТаблицаГраницРезультат[0].ГраницаЗапретаИзменений;
		
	КонецЕсли;
		
	Возврат ГраницаЗапрета;
	
КонецФункции

Процедура ПроверкаПериодаЗаписейРегистровСУчетомВидаДокумента(НаборЗаписей, Отказ)
	
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПроверкаПериодаЗаписейРегистровСУчетомВидаДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ЭтоРегистрСведений = Метаданные.РегистрыСведений.Содержит(МетаданныеНабора);
	Если ЭтоРегистрСведений 
		И МетаданныеНабора.ПериодичностьРегистраСведений = Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда 
		Возврат;
	КонецЕсли;
	...
	Если ЭтоРегистрСведений И НЕ МетаданныеНабора.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда 
		ПодчиненРегистратору = Ложь;
	КонецЕсли;
	РегистраторПустаяСсылка = Неопределено;
	Если ПодчиненРегистратору Тогда 
		ИмяДокументаРегистратора = НаборЗаписей.Отбор.Регистратор.Значение.Метаданные().Имя; 
		РегистраторПустаяСсылка = Документы[ИмяДокументаРегистратора].ПустаяСсылка();
	КонецЕсли;
	
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("ТаблицаГраниц"           , ТаблицаГраниц);
	...
	СтруктураПараметров.Вставить("ПодчиненРегистратору"    , ПодчиненРегистратору);
	СтруктураПараметров.Вставить("ЭтоРегистрСведений"      , ЭтоРегистрСведений);
	СтруктураПараметров.Вставить("РегистраторПустаяСсылка" , РегистраторПустаяСсылка);	
	
	//Отдельная проверка с учетом константы "ГраницаЗапретаИзмененияПартий".
	//Контролируем изменение набора записей регистра партий для всех документов, кроме МФП
	ПроверкаСуществующихЗаписейРегистраПартий(НаборЗаписей, СтруктураПараметров, Отказ);
	ПроверкаНовыхЗаписейРегистраПартий(НаборЗаписей, СтруктураПараметров, Отказ);
	
	
	//Обычная проверка по границам запрета
	Если ТаблицаГраниц = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ПроверкаСуществующихЗаписейРегистраСУчетомВидаДокумента(НаборЗаписей, СтруктураПараметров, Отказ);
		
	Если НаборЗаписей.Количество() > 0 И НЕ Отказ Тогда
		
		ПоляВыгрузки = "Период" + ?(ЕстьОрганизация,",Организация","");
		ТаблицаРегистра = НаборЗаписей.Выгрузить(,ПоляВыгрузки);
		ТаблицаРегистра.Свернуть(ПоляВыгрузки);

		ВидДокумента = Неопределено;
		Если ПодчиненРегистратору Тогда 
			ВидДокумента = РегистраторПустаяСсылка;
		КонецЕсли;
		Для Каждого СтрокаТаблицыРегистра Из ТаблицаРегистра Цикл 
			Организация = Неопределено;
			Если ЕстьОрганизация Тогда 
				Организация = СтрокаТаблицыРегистра.Организация;
			КонецЕсли;
			ГраницаЗапрета = ПолучитьАктуальнуюГраницуЗапрета(ТаблицаГраниц, Организация, ВидДокумента);
			Если СтрокаТаблицыРегистра.Период <= ГраницаЗапрета Тогда 
				Отказ = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПроверкаСуществующихЗаписейРегистраСУчетомВидаДокумента(НаборЗаписей, СтруктураПараметров, Отказ)
	
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПроверкаСуществующихЗаписейРегистраСУчетомВидаДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	МетаданныеНабора = СтруктураПараметров.МетаданныеНабора;
	
    ИмяРегистра = МетаданныеНабора.ПолноеИмя();
	
    Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТаблицаГраниц.Организация КАК Организация,
	               |	ТаблицаГраниц.ВидДокумента КАК ВидДокумента,
	               |	ТаблицаГраниц.ГраницаЗапретаИзменений КАК ГраницаЗапретаИзменений,
	               |	ТаблицаГраниц.Рейтинг КАК Рейтинг
	               |ПОМЕСТИТЬ ТаблицаГраниц
	               |ИЗ
	               |	&ТаблицаГраниц КАК ТаблицаГраниц
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Организация,
	               |	ВидДокумента
	               ...
	               |	ТаблицаГраниц.ГраницаЗапретаИзменений,
	               |	ТаблицаГраниц.Рейтинг КАК Рейтинг
	               |ПОМЕСТИТЬ ТаблицаДат
	               |ИЗ
	               |	ЗаписиРегистра КАК ЗаписиРегистра
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаГраниц КАК ТаблицаГраниц
	               |		ПО (ЗаписиРегистра.ВидДокумента = ТаблицаГраниц.ВидДокумента
	               |				ИЛИ ТаблицаГраниц.ВидДокумента = НЕОПРЕДЕЛЕНО)
	               |			И (ЗаписиРегистра.Организация = ТаблицаГраниц.Организация
	               |				ИЛИ ТаблицаГраниц.Организация = НЕОПРЕДЕЛЕНО)
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Период
	               |;
	               ...
	               |	МАКСИМУМ(ТаблицаДат.Рейтинг) КАК Рейтинг
	               |ПОМЕСТИТЬ СамыйВысокийРейтинг
	               |ИЗ
	               ...
	               |	1 КАК Запрет
	               |ИЗ
	               |	СамыйВысокийРейтинг КАК СамыйВысокийРейтинг
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДат КАК ТаблицаДат
	               |		ПО СамыйВысокийРейтинг.Период = ТаблицаДат.Период
	               |			И СамыйВысокийРейтинг.Рейтинг = ТаблицаДат.Рейтинг
	               |ГДЕ
	               |	ТаблицаДат.Период <= ТаблицаДат.ГраницаЗапретаИзменений";
	                                         
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрНакопления.ПартииТоваров", ИмяРегистра);
	
	Если СтруктураПараметров.ЭтоРегистрСведений 
		И НЕ СтруктураПараметров.ПодчиненРегистратору Тогда
		
	    СписокПолейУсловияОтбораТекст = "";
	    Для Каждого ЭлементОтбора Из НаборЗаписей.Отбор Цикл                
	        Если ЭлементОтбора.Использование Тогда
	                          
		        Если СписокПолейУсловияОтбораТекст <> ""  Тогда            
		            СписокПолейУсловияОтбораТекст = СписокПолейУсловияОтбораТекст  + "
																					|И ";
		        КонецЕсли;        
		        
		        СписокПолейУсловияОтбораТекст = СписокПолейУсловияОтбораТекст +" ТаблицаРегистра." + ЭлементОтбора.Имя + " = &" + ЭлементОтбора.Имя;                       
		        Запрос.УстановитьПараметр(ЭлементОтбора.Имя, ЭлементОтбора.Значение);        
			КонецЕсли; 
	    КонецЦикла; 
		
		Если СписокПолейУсловияОтбораТекст = "" Тогда 
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ТаблицаРегистра.Регистратор = &Регистратор", "ЛОЖЬ");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ТаблицаРегистра.Регистратор = &Регистратор", СписокПолейУсловияОтбораТекст);
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ СтруктураПараметров.ЕстьОрганизация Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТаблицаРегистра.Организация", "НЕОПРЕДЕЛЕНО");
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ТаблицаГраниц", СтруктураПараметров.ТаблицаГраниц);
	Запрос.УстановитьПараметр("Регистратор", СтруктураПараметров.РегистраторПустаяСсылка);
...
Функция ПолучитьПараметрыПроверкиДокумента(ДокументОбъект)
	...
	лКлючАлгоритма = "ОбщийМодуль_НастройкаПравДоступа_ПолучитьПараметрыПроверкиДокумента";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	//Семенов И.П. 05.07.2019 XX-2844(
	ПараметрыПроверкиДокумента.Вставить("ДокументПустаяСсылка", Документы[МетаданныеДокумента.Имя].ПустаяСсылка());
	//)Семенов И.П.
...
Функция ЗаполнитьПараметрыПроверкиПоВидуДокумента(ДокументОбъект, ПараметрыПроверкиДокумента)
	...
	//Убрано Валиахметов А.А. 19.04.2018. Оставляем только управлен. граница запрета
	
	//Если ПараметрыПроверкиДокумента.МетаданныеДокумента.Имя = "ЗаявкаПокупателя" 
	//	Или ПараметрыПроверкиДокумента.МетаданныеДокумента.Имя = "КорректировкаЗаявкиПокупателя" Тогда		
		ЗаполнитьПараметрыПроверкиДокументаЗаявкаПокупателя(ДокументОбъект, ПараметрыПроверкиДокумента);
	//Иначе
	//	Возврат Ложь;
	//КонецЕсли;	
...
// Процедура заполнения структуры параметров проверки для документа ЗаявкаПокупателя
//
Процедура ЗаполнитьПараметрыПроверкиДокументаЗаявкаПокупателя(ДокументОбъект, ПараметрыПроверкиДокумента)
...
КонецПроцедуры // ЗаполнитьПараметрыПроверкиДокументаЗаявкаПокупателя()
...
//Отдельная проверка с учетом константы "ГраницаЗапретаИзмененияПартий".
//Контролируем изменение набора записей регистра партий для всех документов, кроме МФП
Процедура ПроверкаНовыхЗаписейРегистраПартий(НаборЗаписей, СтруктураПараметров, Отказ);

	Если Не НужнаПроверкаГраницыЗапретаРегистраПартий(НаборЗаписей, СтруктураПараметров) Тогда
		Возврат;
	КонецЕсли;
	
	ГраницаЗапретаИзмененияПартий = ПроведениеДокументовКлиентСервер.ГраницаЗапретаИзмененияПартий();
	
	Для Каждого Запись ИЗ НаборЗаписей Цикл
		ЕСли Запись.Период <= ГраницаЗапретаИзмененияПартий Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
//Отдельная проверка с учетом константы "ГраницаЗапретаИзмененияПартий".
//Контролируем изменение существующих записей регистра партий для всех документов, кроме МФП
Процедура ПроверкаСуществующихЗаписейРегистраПартий(НаборЗаписей, СтруктураПараметров, Отказ);
	...
	|ГДЕ Набор.Регистратор = &Регистратор
	|И Набор.Период <= &ДатаЗапрета";
	
	Запрос.УстановитьПараметр("Регистратор", НаборЗаписей.Отбор.Регистратор.Значение);				
	Запрос.УстановитьПараметр("ДатаЗапрета", ГраницаЗапретаИзмененияПартий);				
...
//Определяет, нужна ли проверка границы с учетом константы "ГраницаЗапретаИзмененияПартий".
Функция НужнаПроверкаГраницыЗапретаРегистраПартий(НаборЗаписей, СтруктураПараметров) Экспорт
	
	НужнаПроверка = Истина;
	
	ГраницаЗапретаИзмененияПартий = ПроведениеДокументовКлиентСервер.ГраницаЗапретаИзмененияПартий();
	Если Не ЗначениеЗаполнено(ГраницаЗапретаИзмененияПартий) Тогда
		НужнаПроверка = Ложь;
	КонецЕсли;
	
	ИмяРегистра = СтруктураПараметров.МетаданныеНабора.ПолноеИмя();
	Если НЕ ИмяРегистра = "РегистрНакопления.ПартииТоваров" Тогда
		НужнаПроверка = Ложь;
	КонецЕсли;
	...
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		ВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ВидОперации");
	...
	ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ВидОперации") = Перечисления.ВидыОперацийВозвратаОтПокупателя.МФП Тогда
		НужнаПроверка = Ложь;
	КонецЕсли;
...
Процедура  ОпределитьДоступностьВозможностьИзмененияДокументаПоДатеЗапрета(ДокументОбъект, ФормаДокумента) Экспорт
	...
	//Семенов И.П. 05.07.2019 XX-2844(
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2844) Тогда 
		ПроверитьВерсиюДокументаСУчетомВидаДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, СоответствиеГраницЗапрета, ФормаДокумента.ТолькоПросмотр);
	Иначе	
	//)Семенов И.П.
...
//--------------------------------------------------------------------------------------------
//Функция НайтиЭлементВСпискеОграничений(Фрм, ЭлементФормы)
//	Точка = СтрНайти(ЭлементФормы, ".");
//	Если Точка > 0 Тогда
//		Элемент = Фрм[Лев(ЭлементФормы,Точка - 1)];
//		Возврат НайтиЭлементВСпискеОграничений(Элемент, Прав(ЭлементФормы, СтрДлина(ЭлементФормы) - Точка) );
//	Иначе
//		Возврат Фрм[ЭлементФормы];
//	КонецЕсли;
//	
//КонецФункции

Функция УдалитьКнопки(КоманднаяПанель)
	
	Кнопки = КоманднаяПанель.Кнопки;
	
	Если Кнопки = Неопределено Тогда Возврат Истина; КонецЕсли;
	
	Для Каждого Кнопка Из Кнопки Цикл
		Если Строка(Кнопка.ТипКнопки) = "Подменю" Тогда
			Подменю = Кнопка;
			УдалитьКнопки(Подменю);
		КонецЕсли;
		Кнопки.Удалить(Кнопка);
		УдалитьКнопки(Команднаяпанель);		
	КонецЦикла;
...
Функция НайтиЭлементВСпискеОграничений(Фрм, ЭлементФормы)
	Точка = СтрНайти(ЭлементФормы, ".");
	Если Точка > 0 Тогда
		ПервоеПодменю = СтрНайти(ЭлементФормы,"КнопкиПодменю");
		Если ПервоеПодменю > 0 Тогда
			ТоЧтоДоПервогоПодменю = Лев(ЭлементФормы, ПервоеПодменю-2);
			ЭлементПодменю = НайтиЭлементВСпискеОграничений(Фрм, ТоЧтоДоПервогоПодменю);
			//УдалитьКнопки(ЭлементПодменю);
			//Для Каждого Кнопка Из ЭлементПодменю.Кнопки Цикл
			//	ЭлементПодменю.Кнопки.Удалить(Кнопка);
			//КонецЦикла;
...
Функция ПолучитьСписокФорм(Фрм)
	СписокФорм = Новый СписокЗначений;
	Попытка
		СписокФорм.Добавить(Фрм.ИмяФормы);
	Исключение
		Попытка
			Для Каждого ФормаМетаданных Из Фрм.Метаданные().Формы Цикл
				СписокФорм.Добавить(ФормаМетаданных.ПолноеИмя());
			КонецЦикла;
		Исключение
			Попытка   
				ИмяМетаданных = Строка(Фрм.СправочникСписок);
				ИмяМетаданных = СтрЗаменить(ИмяМетаданных,"Список","");
				ОбМетаданных = Метаданные.НайтиПоПолномуИмени(ИмяМетаданных);
				Для Каждого ФормаМетаданных Из ОбМетаданных.Формы Цикл
					СписокФорм.Добавить(ФормаМетаданных.ПолноеИмя());
				КонецЦикла;
			Исключение
				Попытка
					ИмяМетаданных = Строка(Фрм.ДокументСписок);
					ИмяМетаданных = СтрЗаменить(ИмяМетаданных,"Список","");
					ОбМетаданных = Метаданные.НайтиПоПолномуИмени(ИмяМетаданных);
					Для Каждого ФормаМетаданных Из ОбМетаданных.Формы Цикл
						СписокФорм.Добавить(ФормаМетаданных.ПолноеИмя());
				...
				Исключение
					Попытка
						ИмяМетаданных = Строка(Фрм.РегистрСведенийСписок);
						ИмяМетаданных = СтрЗаменить(ИмяМетаданных,"Список","");
						ОбМетаданных = Метаданные.НайтиПоПолномуИмени(ИмяМетаданных);
						Для Каждого ФормаМетаданных Из ОбМетаданных.Формы Цикл
							СписокФорм.Добавить(ФормаМетаданных.ПолноеИмя());
...
Процедура УстановитьДоступностьВидимостьЭлементовФормы(Фрм) Экспорт
	#Если Клиент Тогда 
		Попытка 
			пар = Новый Структура("ОбъектФорма", Фрм.ИмяФормы);
		Исключение
			Возврат;
		КонецПопытки;
		ТабЗ = мОграничениеДоступа.НайтиСтроки(пар);
		Для Каждого Стр Из ТабЗ Цикл
			ЭлементФормы = НайтиЭлементВСпискеОграничений(Фрм.ЭлементыФормы, Стр.ЭлементФормы);
			Если ЭлементФормы <> Неопределено Тогда
				Если ТипЗнч(ЭлементФормы) = Тип("КнопкаКоманднойПанели") Тогда			
					Если Строка(ЭлементФормы.ТипКнопки) = "Подменю" Тогда
						УдалитьКнопки(ЭлементФормы);
					КонецЕсли;
					ЭлементФормы.Доступность = НЕ Стр.Доступность;
					
				Иначе				
					ЭлементФормы.Доступность = НЕ Стр.Доступность;
					ЭлементФормы.Видимость = НЕ Стр.Видимость;    				
...
Процедура УстановитьДоступностьВидимостьЭлемента(Форма, ЭлементФормы, Доступность, Видимость) Экспорт
	#Если Клиент Тогда

		Попытка
			пар = Новый Структура("ОбъектФорма, ЭлементФормы", Форма.ИмяФормы, ЭлементФормы.Имя);
		Исключение
			Возврат;
		КонецПопытки;
		СтрокиПрав = мОграничениеДоступа.НайтиСтроки(пар);
		Если СтрокиПрав.Количество()>0 Тогда
			ЭлементФормы.Доступность = НЕ СтрокиПрав[0].Доступность И Доступность;
			ЭлементФормы.Видимость = НЕ СтрокиПрав[0].Видимость И Видимость;
		Иначе
...
Функция ПолучитьОграничениеДоступаКЭлементамФорм() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПраваПользователей.ОбъектФорма,
	|	ПраваПользователей.ЭлементФормы,
	|	ПраваПользователей.Доступность,
	|	ПраваПользователей.Видимость
	|ИЗ
	|	РегистрСведений.ПраваПользователей КАК ПраваПользователей
	|ГДЕ
	|	ПраваПользователей.Пользователь В(&Пользователь)";
	
	Пользователь = Новый СписокЗначений;
	Пользователь.Добавить(ПараметрыСеанса.ТекущийПользователь);
	...
	ДеревоГрупп = ЗапросГруппы.Выполнить().Выгрузить();
	Для Каждого Гр Из ДеревоГрупп Цикл
		Пользователь.Добавить(Гр.Группа);
	КонецЦикла;
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	Ограничение = Запрос.Выполнить().Выгрузить();
	Возврат Запрос.Выполнить().Выгрузить();
КонецФункции

Функция ПолучитьЗначениеПраваПользователя(Пользователь, Право) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	СписокПрав = Запрос.Выполнить().Выгрузить();
	
	Если СписокПрав.Количество() = 0 Тогда
		Возврат Ложь;
		
	Иначе
		Возврат СписокПрав[0].Значение;
...
// ЛНА, Определение ROOT прав
Функция ЭтоПрограммист(вхПользователь) Экспорт
	
	Возврат РольДоступна("ПолныеПрава") 
	ИЛИ (НастройкаПравДоступа.ПолучитьЗначениеПраваПользователя(вхПользователь, ПланыВидовХарактеристик.ПраваПользователей.Программист) = Истина);
