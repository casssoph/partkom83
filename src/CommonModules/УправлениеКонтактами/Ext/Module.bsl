...
Функция ПолучитьСтруктуруРаспределенныхТаблиц(ИсходнаяТаблицаПотребности, ИсходнаяТаблицаНоменклатурыЗаказов, МаксимальноеЗначениеПрогрессора = 0, ТекущееЗначениеПрогрессора = 0) Экспорт
	...
	ТаблицаПотребности.Индексы.Добавить("Номенклатура, ХарактеристикаНоменклатуры, ЗаявкаПокупателя, Тара");
	...
	Для каждого СтрокаТаблицыНоменклатурыЗаказов Из ТаблицаНоменклатурыЗаказов Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицыНоменклатурыЗаказов.ЗаявкаПокупателя) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокиТаблицыПотребности = ТаблицаПотребности.НайтиСтроки(Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, ЗаявкаПокупателя, Тара", СтрокаТаблицыНоменклатурыЗаказов.Номенклатура, СтрокаТаблицыНоменклатурыЗаказов.ХарактеристикаНоменклатуры, СтрокаТаблицыНоменклатурыЗаказов.ЗаявкаПокупателя, СтрокаТаблицыНоменклатурыЗаказов.Тара));
	...
	Для каждого СтрокаПотребности Из ТаблицаПотребности Цикл
		...
		Для каждого СтрокаТаблицаНоменклатурыЗаказов Из СтрокиТаблицаНоменклатурыЗаказов Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицаНоменклатурыЗаказов.ЗаявкаПокупателя) Тогда
				Продолжить;
			КонецЕсли; 
	...
	Для каждого СтрокаПотребности Из ТаблицаПотребности Цикл
		...
		Для каждого СтрокаТаблицаНоменклатурыЗаказов Из СтрокиТаблицаНоменклатурыЗаказов Цикл
			...
			Если СтрокаТаблицаНоменклатурыЗаказов.ЗаявкаПокупателя <> Неопределено Тогда
				Если НЕ СтрокаТаблицаНоменклатурыЗаказов.ЗаявкаПокупателя.Пустая() Тогда
					Продолжить;
				КонецЕсли; 
			КонецЕсли; 
	...
	Для каждого СтрокаТаблицыНоменклатурыЗаказов Из ТаблицаНоменклатурыЗаказов Цикл
		...
		Иначе
			...
			СтрокаТаблицыПотребности.ЗаявкаПокупателя = СтрокаТаблицыНоменклатурыЗаказов.ЗаявкаПокупателя;
