...
Процедура СоздатьКорректировкуЗаказаЗаявки(знач Документ,СтруктураЗаполнения,ПровестиИзменения = Истина) Экспорт
	...
	Пока ВыборкаРегистраторовКЗакрытию.Следующий() цикл 
		...
		Попытка
			...
			Пока ВыборкаСтрокЗаявокКЗакрытию.Следующий() цикл 
				...
				СтрокаНабора.ДатаОбработки = ДатаДокумента;
...
Процедура ДобавитьДокументВОчередьЗакрытияСтрокЗаявок(ДокументСсылка) экспорт 
	...
	Если НеобходимоПроверитьЗакрытиеСтрокЗаявок(ДокументСсылка,ИмяТЧ) тогда 
		ВыборкаСтрокЗаявок = ОбщегоНазначения.СформироватьЗапросПоТабличнойЧасти(ДокументСсылка, ИмяТЧ, Новый Структура("СтрокаЗаявки","СтрокаЗаявки")).Выбрать();
		...
		Пока ВыборкаСтрокЗаявок.Следующий() цикл 
			...
			СтрокаНабора.СтрокаЗаявки  = ВыборкаСтрокЗаявок.СтрокаЗаявки;		
		КонецЦикла;
		НаборЗаписей.Записать();
	Конецесли	
...
Функция НеобходимоПроверитьЗакрытиеСтрокЗаявок(ДокументСсылка,ИмяТЧ="Товары")
	...
	Если  ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПоставщику") или 
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаЗаказаПоставщику") тогда 
		Возврат истина;	 		 
	КонецЕсли;	 
