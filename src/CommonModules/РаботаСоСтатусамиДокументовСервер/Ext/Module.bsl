...
Функция ЗапросСостояние_АктРассмотренияВозврата()
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Порядок = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтказано)
	|			ТОГДА 11
	|		КОГДА ДанныеДокумента.Порядок = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратНаРассмотрении)
	|			ТОГДА 9
	|		КОГДА ДанныеДокумента.Порядок = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтменен)
	|			ТОГДА 12
	|	КОНЕЦ КАК Порядок,
