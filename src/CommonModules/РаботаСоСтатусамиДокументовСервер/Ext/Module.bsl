...
Функция ЗапросСостояние_ЗаявкаПокупателя()
	...
	Возврат "ВЫБРАТЬ РАЗЛИЧНЫЕ
	        ...
	        |		КОГДА ЗаявкаПокупателяТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаявкаПокупателя.БонусыБезСборки)
	        |			ТОГДА ВЫБОР
	        |					КОГДА ЗаявкаПокупателяТовары.Ссылка.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ЗаявкаПокупателяНеПодтвержден)
	        |						ТОГДА ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.Новый)
	        ...
	        |		КОГДА КорректировкаЗаявкиПокупателяТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаявкаПокупателя.БонусыБезСборки)
	        |			ТОГДА ВЫБОР
	        |					КОГДА КорректировкаЗаявкиПокупателяТовары.Ссылка.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ЗаявкаПокупателяНеПодтвержден)
	        |						ТОГДА ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.Новый)
	        |					ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.Выдано)
	        |				КОНЕЦ
	        |		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.НовыйНеподтвержден)
	...
	//Если ВидОперации = БонусыБезСборки, то для неподтвержденной заявки ставим статус "Новый" 
...
Функция ЗапросСостояние_ВозвратТоваровОтПокупателя()
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2570) Тогда
		ТекстЗапроса = 	
		"ВЫБРАТЬ
		|	ВозвратТовары.СтрокаЗаявки,
		|	ЗНАЧЕНИЕ(Справочник.ИдентификаторыСтрокПриходов.Пустаяссылка) КАК СтрокаПрихода,
		|	ЛОЖЬ КАК Отказ,
		|	9 КАК Порядок,
		|	ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(ВозвратТовары.Ссылка КАК Документ.ВозвратТоваровОтПокупателя).СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ВозвратТоваровОтПокупателяНовый)
		|				И НЕ ВЫРАЗИТЬ(ВозвратТовары.Ссылка КАК Документ.ВозвратТоваровОтПокупателя).АктРассмотренияВозврата.Ссылка ЕСТЬ NULL
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратЖдемТовар)
		|		КОГДА ВЫРАЗИТЬ(ВозвратТовары.Ссылка КАК Документ.ВозвратТоваровОтПокупателя).СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ВозвратТоваровОтПокупателяПринят)
		|				ИЛИ ВЫРАЗИТЬ(ВозвратТовары.Ссылка КАК Документ.ВозвратТоваровОтПокупателя).СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ВозвратТоваровОтПокупателяРазмещен)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтПокупателя)
		|	КОНЕЦ КАК Состояние,
		|	СУММА(ВозвратТовары.Количество) КАК Количество,
		|	ИСТИНА КАК Изменена,
		|	МАКСИМУМ(ВозвратТовары.Ссылка.Склад.ФизическийСклад) КАК Склад,
		|	МАКСИМУМ(ВозвратТовары.Номенклатура) КАК Номенклатура,
		|	ВозвратТовары.СтрокаЗаявки.IDSite КАК IDSite,
		|	&Период,
		|	ВозвратТовары.Ссылка,
		|	СУММА(ВозвратТовары.КоличествоПлан) КАК КоличествоПлан
		|ПОМЕСТИТЬ Данные
		|ИЗ
		|	Документ.ВозвратТоваровОтПокупателя.Товары КАК ВозвратТовары
		|ГДЕ
		|	ВозвратТовары.Ссылка = &ДокументСсылка
		|	И ВозвратТовары.Ссылка.Проведен
		|	И НЕ ВозвратТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВозвратаОтПокупателя.МФП)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВозвратТовары.СтрокаЗаявки,
		|	ВозвратТовары.СтрокаЗаявки.IDSite,
		|	ВозвратТовары.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	Данные.Отказ,
		|	ВЫБОР
		|		КОГДА Данные.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратЖдемТовар)
		|			ТОГДА 10
		|		КОГДА Данные.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтПокупателя)
		|			ТОГДА 13
		|	КОНЕЦ КАК Порядок,
		|	Данные.Состояние,
		|	ВЫБОР
		|		КОГДА Данные.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратЖдемТовар)
		|			ТОГДА Данные.КоличествоПлан
		|		КОГДА Данные.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтПокупателя)
		...
		|	Данные.IDSite,
		|	Данные.Период,
		|	Данные.Ссылка КАК Регистратор,
		|	Данные.КоличествоПлан
		|ПОМЕСТИТЬ ВтДвижения
		|ИЗ
		|	Данные КАК Данные
...
Функция ЗапросСостояние_АктРассмотренияВозврата()
	...
	|	ВозвратТоваровОтПокупателя.Склад.ФизическийСклад КАК Склад,
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтказано)
	|			ТОГДА 11
	|		КОГДА ДанныеДокумента.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратНаРассмотрении)
	|			ТОГДА 9
	|		КОГДА ДанныеДокумента.Состояние = ЗНАЧЕНИЕ(Справочник.СостоянияСтрокДокументов.ВозвратОтменен)
	|			ТОГДА 12
	|	КОНЕЦ КАК Порядок,
...
Процедура ОткатитьСостоянияСтрокЗаявокПриРаспроведенииДокумента(ДокументСсылка,МассивСтрокЗаявок)
	...
	ВыборкаСтрок = ВыбратьСстрокиЗаявокКОтменеЗакрытия(МассивСтрокЗаявок,ДокументСсылка);
	Пока ВыборкаСтрок.Следующий() цикл 
		...
		Если  ВыборкаСтрок.ОткатитьЗаявку  тогда 
			СтрокаЗаявкиОбъект.СостояниеЗаявки = ПредопределенноеЗначение("Справочник.СтатусыДокументов.ЗаявкаПокупателяПодтвержден");
...
Функция ВыбратьСстрокиЗаявокКОтменеЗакрытия(МассивСтрокЗаявок,ДокументСсылка)
