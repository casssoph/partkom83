...
Функция ПолучитьМаршрутДоставки(ДокументРеализации)
	...
	Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1
	             ...
	             |ГДЕ
	             |	МаршрутыДоставки.Родитель = &Родитель
...
Функция ПолучитьМЗ(Склад,ВремяВыписки)
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |ГДЕ
	             |	МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка.ЭкспрессДоставка
	             |	И МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка.ДатаДоставки = &ДатаДоставки
	             |	И МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка.Статус = &КРаспределению
	             |	И МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.ДокументСсылка.Склад = &Склад
	             |	И МегаЛогист_МаршрутноеЗаданиеДокументыРеализации.Ссылка.ВремяДоставкиС < &ВремяВыписки";
	...
	ЗАпрос.УстановитьПараметр("ВремяВыписки",ВремяВыписки);
...
Процедура КонтрольВыписки() Экспорт
	...
	Расписание=ПолучитьРасписаниеЭД(ТекДата);
	Для Каждого СтрокаРасписания Из Расписание Цикл
		ВремяДедлайна=СтрокаРасписания.ВремяВыписки+20*60;
		Если ТекВремя>ВремяДедлайна тогда			
			ТаблицаЗаданий=ПолучитьМЗ(СтрокаРасписания.Склад,ВремяДедлайна);
