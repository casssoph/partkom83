...
//
// Возвращаемое значение:
//   Объект Описание типов с типом ЗаявкаПокупателя
//
Функция ПолучитьОписаниеТиповЗаказ()

	МассивТипов = Новый Массив;
	МассивТипов.Добавить(Тип("ДокументСсылка.ЗаявкаПокупателя"));
...
Процедура СформироватьДвиженияСписанияНаСебестоимостьПродажУпр(СтрокаДокумента, СтруктураПараметров, Строка, ТипЗаписи = "Прямая")
	...
	Если  ЗначениеЗаполнено(СтрокаДокумента.ЗаказСписания) Тогда
		...
		Движение.ЗаявкаПокупателя = СтрокаДокумента.ЗаказСписания;
		
	ИначеЕсли  ЗначениеЗаполнено(Строка.Заказ) Тогда
		
		Движение.ЗаявкаПокупателя = Строка.Заказ;
		
	Иначе
		
		Движение.ЗаявкаПокупателя = Неопределено;
...
Процедура ЗаполнитьЗаказВДвижении(Учет, Движение, СтрокаДокумента, Организация = Неопределено, СтруктураПараметров)
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.ЗаказСписания) Тогда
		Если ТипЗнч(СтрокаДокумента.ЗаказСписания) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
...
Процедура ПоступлениеТоваровПоОрдеруУпр(СтрокаДокумента, СтруктураПараметров)
	...
	|			ТОГДА Затраты.Заказ
	|		КОГДА (НЕ Себестоимость.ЗаявкаПокупателя ЕСТЬ NULL )
	|			ТОГДА Себестоимость.ЗаявкаПокупателя
...
Функция ПолучитьДеревоПартийНаСкладахУпр (МоментКон, СтруктураПараметров)
	...
	Запрос.УстановитьПараметр("ПустойЗаказ", Документы.ЗаявкаПокупателя.ПустаяСсылка());
...
//{{ХудинВВ 20180407
Процедура ПерезаполнитьТЧПеремещенийТоваровПоПартиям(знач ДокументПеремещение, Записывать = Истина, Опереативно= Истина, ИзменятьКоличество = Истина) Экспорт 
		
	Если ТипЗнч(ДокументПеремещение) = Тип("ДокументСсылка.ПеремещениеТоваров")  тогда 
		ДокументПеремещение = ДокументПеремещение.ПолучитьОбъект();
	КонецЕсли;	
	
	Если Опереативно тогда 
		ДокументПеремещение.дата = текущаяДата();
	КонецЕсли;	
	
	Запрос = Новый Запрос;
	Запрос.Текст =  "ВЫБРАТЬ
	                |	ТчДокументаБезГруппировки.Номенклатура,
	                |	СУММА(ТчДокументаБезГруппировки.Количество) КАК Количество
	                |ПОМЕСТИТЬ ТчДокумента
	                |ИЗ
	                |	Документ.ПеремещениеТоваров.Товары КАК ТчДокументаБезГруппировки
	                |ГДЕ
	                |	ТчДокументаБезГруппировки.Ссылка = &ДокументПеремещение
	                |
	                |СГРУППИРОВАТЬ ПО
	                |	ТчДокументаБезГруппировки.Номенклатура
	                |;
	                ...
	                |	МИНИМУМ(ПартииТоваровОстатки.СтрокаПрихода.Дата) КАК СтрокаПриходаДата
	                |ПОМЕСТИТЬ ТчПартий
	                |ИЗ
	                ...
	                |					ИЗ
	                |						ТчДокумента)
	                |				И Склад = &СкладОтправитель) КАК ПартииТоваровОстатки
	                |
	                |СГРУППИРОВАТЬ ПО
	                |	ПартииТоваровОстатки.Номенклатура,
	                |	ПартииТоваровОстатки.Организация
	                ...
	                |					ИЗ
	                |						ТчДокумента)
	                |				И Склад = &СкладОтправитель) КАК РезервыТоваровОстатки
	                |
	                |СГРУППИРОВАТЬ ПО
	                |	РезервыТоваровОстатки.Номенклатура,
	                ...
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	ТчПартий.Номенклатура,
	                |	ТчПартий.Организация,
	                |	ВЫБОР
	                |		КОГДА ТчПартий.Организация = &Организация
	                |			ТОГДА 1
	                |		ИНАЧЕ 0
	                |	КОНЕЦ КАК ЭтоОрганизацияДокумента,
	                |	ВЫБОР
	                |		КОГДА ТчПартий.КоличествоОстаток <= ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0)
	                |			ТОГДА 0
	                |		ИНАЧЕ ТчПартий.КоличествоОстаток - ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0)
	                |	КОНЕЦ КАК КоличествоОстаток,
	                |	ТчПартий.СтрокаПриходаДата КАК СтрокаПриходаДата
	                |ПОМЕСТИТЬ ОстаткиПартий
	                |ИЗ
	                |	ТчПартий КАК ТчПартий
	                |		ЛЕВОЕ СОЕДИНЕНИЕ ТчРезервов КАК ТчРезервов
	                |		ПО ТчПартий.Номенклатура = ТчРезервов.Номенклатура
	                |			И ТчПартий.Организация = ТчРезервов.Организация
	                |ГДЕ
	                |	ТчПартий.КоличествоОстаток - ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0) > 0
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	ОстаткиПартий.Номенклатура,
	                |	ОстаткиПартий.ЭтоОрганизацияДокумента,
	                |	ОстаткиПартий.Организация,
	                |	ОстаткиПартий.КоличествоОстаток,
	                |	ОстаткиПартий.СтрокаПриходаДата
	                |ИЗ
	                |	ОстаткиПартий КАК ОстаткиПартий
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	ОстаткиПартий.Номенклатура,
	                |	ОстаткиПартий.ЭтоОрганизацияДокумента УБЫВ,
	                |	ОстаткиПартий.СтрокаПриходаДата";
	
	Запрос.УстановитьПараметр("ДатаДокумента", 			Новый Граница(ДокументПеремещение.МоментВремени(), ВидГраницы.Исключая));
	Запрос.УстановитьПараметр("Организация", 			ДокументПеремещение.Организация);
	Запрос.УстановитьПараметр("СкладОтправитель", 		ДокументПеремещение.СкладОтправитель);
	Запрос.УстановитьПараметр("ДокументПеремещение",	ДокументПеремещение.Ссылка);

	Остатки = Запрос.Выполнить().Выгрузить();
	
	ТЧ 		=  ДокументПеремещение.Товары.Выгрузить();
	ТЧНовая =  ДокументПеремещение.Товары.ВыгрузитьКолонки();
	
	Для каждого СтрокаТЧ Из ТЧ Цикл
		
		//ВсегоРаспределить =  СтрокаТЧ.Количество;
		
		СтрокиОстатков = Остатки.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТЧ.Номенклатура));
		
		НоваяСтрока = Неопределено;
		Для каждого СтрокаОстатков ИЗ СтрокиОстатков Цикл
			
			Распределено = Мин(СтрокаОстатков.КоличествоОстаток, СтрокаТЧ.Количество);
			
			Если Распределено = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = ТЧНовая.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ,,"Количество, КоличествоПлан, Организация");
			НоваяСтрока.Количество = Распределено;
			НоваяСтрока.КоличествоПлан = Распределено;
			НоваяСтрока.Организация = ?(СтрокаОстатков.ЭтоОрганизацияДокумента, Справочники.Организации.ПустаяСсылка(), СтрокаОстатков.Организация);
			
			СтрокаТЧ.Количество = СтрокаТЧ.Количество - Распределено;
			СтрокаОстатков.КоличествоОстаток =  СтрокаОстатков.КоличествоОстаток - Распределено;
		...
		Если СтрокаТЧ.Количество > 0 Тогда
			Если НЕ НоваяСтрока = Неопределено Тогда
				//Кидаем нераспределенное количество в последнюю строку
				НоваяСтрока.КоличествоПлан = НоваяСтрока.КоличествоПлан + СтрокаТЧ.Количество;
				
				Если НЕ ИзменятьКоличество Тогда //Оставим общее количество неизменным
					НоваяСтрока.Количество = НоваяСтрока.Количество + СтрокаТЧ.Количество;
				КонецЕсли;
			Иначе
				//В остатках по партиям такой номенклатуры нет, очистим организацию, если была
				НоваяСтрока = ТЧНовая.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ,,"Количество, КоличествоПлан, Организация");
				НоваяСтрока.КоличествоПлан 	 = СтрокаТЧ.Количество;
				Если НЕ ИзменятьКоличество Тогда //Оставим общее количество неизменным
	...
	ДокументПеремещение.Товары.Загрузить(ТЧНовая);
...
Процедура ПерезаполнитьТЧПеремещенийТоваровПоПартиям_Старая(знач ДокументПеремещение,Записывать = Истина,Опереативно= Истина,ИзменятьКоличество = Истина) Экспорт 
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ВЫБРАТЬ
	                |	ТчПартий.Номенклатура,
	                |	СУММА(ТчПартий.КоличествоОстаток) КАК КоличествоОстаток
	                |ПОМЕСТИТЬ ОбПарт
	                |ИЗ
	                |	ТчПартий КАК ТчПартий
	                |
	                ...
	                |	ТчПартий.Организация,
	                |	ВЫБОР
	                |		КОГДА ОбПарт.КоличествоОстаток <= ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0)
	                |			ТОГДА 0
	                |		ИНАЧЕ ТчПартий.КоличествоОстаток - ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0)
	                |	КОНЕЦ КАК КоличествоОстаток,
	                |	ТчПартий.СтрокаПриходаДата КАК СтрокаПриходаДата
	                ...
	                |ИЗ
	                |	ОбПарт КАК ОбПарт
	                |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТчПартий КАК ТчПартий
	                |			ЛЕВОЕ СОЕДИНЕНИЕ ТчРезервов КАК ТчРезервов
	                |			ПО ТчПартий.Номенклатура = ТчРезервов.Номенклатура
	                |		ПО ОбПарт.Номенклатура = ТчПартий.Номенклатура
	                |ГДЕ
	                |	ТчПартий.КоличествоОстаток - ЕСТЬNULL(ТчРезервов.КоличествоОстаток, 0) > 0
	                |;
	                |
	                ...
	                |	ВЫБОР
	                |		КОГДА ОстаткиПартий.Организация = (ВЫРАЗИТЬ(&Организация КАК Справочник.Организации))
	                |			ТОГДА ИСТИНА
	                |		ИНАЧЕ ЛОЖЬ
	                |	КОНЕЦ КАК ЭтоОрганизацияДокумента,
	                |	ЕСТЬNULL(ОстаткиПартий.Организация, ВЫРАЗИТЬ(&Организация КАК Справочник.Организации)) КАК Организация,
	                ...
	                |		ПО ТчДокумента.Номенклатура = ОстаткиПартий.Номенклатура
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	Номенклатура,
	...
	Запрос.УстановитьПараметр("ДатаДокумента", ДокументПеремещение.Дата);
	Запрос.УстановитьПараметр("Организация", ДокументПеремещение.Организация);
	Запрос.УстановитьПараметр("СкладОтправитель", ДокументПеремещение.СкладОтправитель);
	Запрос.УстановитьПараметр("ДокументПеремещение",ДокументПеремещение.Ссылка);
	...
	ТабНоменклатурыСОстатком = новый ТаблицаЗначений;
	ТабНоменклатурыСОстатком.Колонки.Добавить("Номенклатура",новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТабНоменклатурыСОстатком.Колонки.Добавить("Количество",новый ОписаниеТипов("Число"));
	ТабНоменклатурыСОстатком.Колонки.Добавить("Организация",новый ОписаниеТипов("СправочникСсылка.Организации"));
	
	//Считаем нарастающий итог  по ФИФО	общий по номенклатуре 
	Пока ВыборкаНоменклатура.Следующий() Цикл
		ВыборкаДетальныеЗаписи = ВыборкаНоменклатура.Выбрать();
		КоличествоПоНоменклатуре = Неопределено;
		
		Пока ВыборкаДетальныеЗаписи.Следующий()  цикл 
			
			Если КоличествоПоНоменклатуре = 0 тогда 
				прервать 
			иначеесли 	КоличествоПоНоменклатуре= Неопределено тогда 	
				КоличествоПоНоменклатуре = ВыборкаДетальныеЗаписи.Количество;
			конецЕсли;	
			
			НовСтрока =  ТабНоменклатурыСОстатком.Добавить();
			НовСтрока.Номенклатура =  ВыборкаНоменклатура.Номенклатура;
			
			Если не  ВыборкаДетальныеЗаписи.ЭтоОрганизацияДокумента тогда 
				НовСтрока.Организация = ВыборкаДетальныеЗаписи.Организация
			КонецЕсли;
			
			ОстатокПоОрганизации =   ВыборкаДетальныеЗаписи.КоличествоОстаток;
			Если КоличествоПоНоменклатуре > ОстатокПоОрганизации тогда 
				
				НовСтрока.КОличество = ОстатокПоОрганизации;
				
				КоличествоПоНоменклатуре  =   КоличествоПоНоменклатуре- ОстатокПоОрганизации  ;
			иначе 	
				
				НовСтрока.КОличество = КоличествоПоНоменклатуре;
				
				КоличествоПоНоменклатуре  =  0
	...
	Для каждого Строка из  ТабНоменклатурыСОстатком цикл 
		НайдСтроки = Товары.НайтиСтроки(Новый Структура("Номенклатура", Строка.Номенклатура)); 
		Остаток =  Строка.Количество;
		Для каждого НайдСтрока из НайдСтроки цикл 
			Если НайдСтрока.Количество >= Остаток тогда 
				найдстрока.Организация = Строка.Организация;
				если ИзменятьКоличество тогда 
				НайдСтрока.количество = Остаток;
				НайдСтрока.КоличествоПлан =  НайдСтрока.количество ;
				КонецЕсли;
				Остаток=0;
			иначе 
				найдстрока.Организация = Строка.Организация;
