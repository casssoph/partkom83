...
Функция ТаблицыИдентичны(вхИсходная, вхТребуемая, выхДобавить, выхУдалить) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_РаботаСПоследовательностямиКлиентСервер_ТаблицыИдентичны";
...
Процедура ЗакончитьКорректировкуГраницПоследовательности(вхСсылкаНаДокумент, вхПоследовательность, вхПараметры) Экспорт
	...
	Для Каждого лСтрокаГраницы Из лГраницы цикл
		...
		Если НЕ лИзменен тогда
			...
			УстановитьПериодВМоментеВремениИзПоследовательности(лГраница, лМетаданныеПоследовательности.Имя);
			//Дату для момента времени тоже получим из последовательности
			УстановитьПериодВМоментеВремениИзПоследовательности(лМоментВремени, лМетаданныеПоследовательности.Имя);
		...
		//ИначеЕсли лГраница.Дата = Дата(1,1,1) Тогда
		//	лМенеджерПоследовательности.УстановитьГраницу(Новый МоментВремени(Дата(2010,1,1)), лОтбор);
		КонецЕсли;
	КонецЦикла;
	
	//Сдвигаем границу если запись по договору есть в последовательности, но движений по нему не было и нет
	Если лМетаданныеПоследовательности = Метаданные.Последовательности.ПоРасчетамСКонтрагентами И ТипЗнч(лФильтр) = Тип("Структура") и лФильтр.Свойство("ДоговорКонтрагента") Тогда 
...
Процедура УстановитьПериодВМоментеВремениИзПоследовательности(вхМоментВремени, ИмяПоследовательности)
	
	ПериодИзПоследовательности = ПериодИзПоследовательности(вхМоментВремени.Ссылка, ИмяПоследовательности);
	Если ЗначениеЗаполнено(ПериодИзПоследовательности) И вхМоментВремени.Дата <> ПериодИзПоследовательности Тогда
		вхМоментВремени = Новый МоментВремени(ПериодИзПоследовательности, вхМоментВремени.Ссылка); 
	КонецЕсли;
...
Функция ОпределитьНеобходимостьСдвигаГраницы(ДокументМоментВремени, ИмяПоследовательности, Отбор = Неопределено) Экспорт
	...
	//Дату для границы надо получить из данных последовательности (в границе она равна дате документа)
	УстановитьПериодВМоментеВремениИзПоследовательности(ГраницаМоментВремени, ИмяПоследовательности);
