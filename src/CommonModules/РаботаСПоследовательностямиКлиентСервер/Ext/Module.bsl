...
Процедура ЗакончитьКорректировкуГраницПоследовательности(вхСсылкаНаДокумент, вхПоследовательность, вхПараметры) Экспорт
	...
	Для Каждого лСтрокаГраницы Из лГраницы цикл
		...
		Если НЕ лИзменен тогда
			...
			ПериодИзПоследовательности = ПериодИзПоследовательности(лГраница.Ссылка, лМетаданныеПоследовательности.Имя);
			Если ЗначениеЗаполнено(ПериодИзПоследовательности) И лГраница.Дата <> ПериодИзПоследовательности Тогда
				Параметр_лГраница = Новый МоментВремени(ПериодИзПоследовательности, лГраница.Ссылка); 
			Иначе
				Параметр_лГраница = лГраница; 
			КонецЕсли;
			
			лРезультатСравнения = Параметр_лГраница.Сравнить(лМоментВремени);
