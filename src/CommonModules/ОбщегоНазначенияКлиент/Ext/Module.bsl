...
// Выводит в окно служебных сообщий GUID переданной ссылки
//
// Параметры:
//  ЛюбаяСсылка  - <ЛюбаяСсылка> - Любая ссылка, кроме пустой
//                 Ссылка на объект, GUID которого нужно отобразить
//
Процедура ПоказатьGUID(ЛюбаяСсылка) Экспорт

	_ТипЗнч = ТипЗнч(ЛюбаяСсылка);
	
	Если ОбщегоНазначения.ЭтоСсылка(_ТипЗнч) Тогда
		
		ГУИД = ЛюбаяСсылка.УникальныйИдентификатор();
		ТекстСообщения = "";
		ТекстСообщения = ТекстСообщения + _ТипЗнч + ": " + ЛюбаяСсылка + Символы.ПС;
		ТекстСообщения = ТекстСообщения + "GUID: " + ГУИД + Символы.ПС;
		ТекстСообщения = ТекстСообщения + "GUIDHEX: " + ОбщегоНазначения.ПреобразоватьGUIDВФорматHEX(Строка(ГУИД)) + Символы.ПС;
		
		Сообщить(ТекстСообщения, СтатусСообщения.Информация);
		
	КонецЕсли;

КонецПроцедуры // ПоказатьGUID()

Процедура УстановитьДоступностьЭлементовФормы(ЭлементыФормы, Доступность=Истина) Экспорт
	Для Каждого ТекЭлемент Из ЭлементыФормы Цикл
		...
		Если ТипЗнч(ТекЭлемент) = Тип("КоманднаяПанель") Тогда
			Для Каждого ТекКнопка Из ТекЭлемент.Кнопки Цикл
				ТекКнопка.Доступность = Доступность;
			КонецЦикла;
		ИначеЕсли ТипЗнч(ТекЭлемент) = Тип("ТабличноеПоле") ИЛИ ТипЗнч(ТекЭлемент) = Тип("ПолеВвода") Тогда
			ТекЭлемент.ТолькоПросмотр = НЕ Доступность;
		ИначеЕсли ТипЗнч(ТекЭлемент) = Тип("Панель") Тогда
			Продолжить;
		Иначе
			Попытка
...
Процедура ПоказатьИсториюИзмененияОбъекта(вхСсылка) Экспорт
	Отчет = Отчеты.внИсторияИзмененийОбъектов.Создать();
	Отчет.ВыбранныйОбъект = вхСсылка;
	Форма = Отчет.ПолучитьФорму("ФормаОтчетаОбычная");
	Форма.Открыть();
...
Процедура ПоказатьИсториюМенеджеровТорговойТочки(вхВладелецФормы, вхТорговаяТочка, вхВидМенеджера) Экспорт
	ФормаСписка = РегистрыСведений.МенеджерыТорговыхТочек.ПолучитьФормуСписка("ФормаИстории", вхВладелецФормы);
	ФормаСписка.ТорговаяТочка = вхТорговаяТочка;
	ФормаСписка.ВидМенеджера = вхВидМенеджера;
	ФормаСписка.Открыть();
КонецПроцедуры
