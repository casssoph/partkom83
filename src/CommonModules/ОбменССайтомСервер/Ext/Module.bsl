...
Процедура ОбработатьОбъектыОбмена(УзелОбмена, НомерПотока = Неопределено) Экспорт
	...
	Пока ВыборкаПоНомеруСообщения.Следующий() цикл 	
		...
		Если ВыборкаПоНомеруСообщения.ВидОбмена = Перечисления.ВидыОбменов.ОбменПартКом83_TopLog Тогда
			СтруктураЗаписи  = ПолучитьСтруктуЗаписиСообщения(ВыборкаПоНомеруСообщения, УзелПланаОбмена("ОбменПартКом83_TopLog",Истина,УзелОбмена),ВыборкаСтрок.Данные);
		ИначеЕсли ВыборкаПоНомеруСообщения.ВидОбмена = Перечисления.ВидыОбменов.ОбменПартКом83_TopLog_РТУ Тогда
			СтруктураЗаписи  = ПолучитьСтруктуЗаписиСообщения(ВыборкаПоНомеруСообщения, УзелПланаОбмена("ОбменПартКом83_TopLog_РТУ",Истина,УзелОбмена),ВыборкаСтрок.Данные);
		Иначе
...
Процедура ПроверитьКорректностьЗаказа(ДокументЗаказ,GUIDЗаказа,СтруктураОтчета)
	...
	лКлючАлгоритма = "ОбщийМодуль_ОбменССайтомСервер_ЗагрузкаОбъекта_ПроверитьКорректностьЗаказа";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументЗаказ,"СтатусДокумента") = Справочники.СтатусыДокументов.ЗаказПоставщикуЗакрыт Тогда
		...
		//# Kalinin V.A. ( 2019-06-11 )  /*
		//Исправление ошибки оживания допереходных заказов
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументЗаказ,"Дата")<  глЗначениеПеременной("ДатаЗапускаПервогоЭтапа") тогда 
		ДобавитьОшибкуВСтруктуруОтчета(СтруктураОтчета, "ЗаказПоставщикуМодульМенеджера.ЗагрузитьЭлемент(): Не возможно загрузить изменения допереходного заказа <" + ДокументЗаказ + ">");
	КонецЕсли;
...
Процедура ЗаписатьДокументЗаявки(СтруктураЗаявки, СтруктураОтчета, ДобавитьУслугуДоставки, СтоимостьДоставки = Неопределено, АдресДоставки = Неопределено)
	...
	Если ДобавитьУслугуДоставки Тогда
		ОписаниеОшибки = "";
		Если НЕ ЗначениеЗаполнено(СтоимостьДоставки) Тогда
		...
		НоваяСтрока.Номенклатура = Константы.УслугаДоставкаРозница.Получить();
...
Процедура УстановитьПризнакОбработаноДляЗаписи(ДанныеСтроки,СтруктураОтчета) 
	...
	ЗаполнитьЗначенияСвойств(Запись, СтруктураОтчета, "Ошибка,Обработано");
	
	Если ТипЗнч(СтруктураОтчета.ТекстОшибки)  = Тип("Массив")  тогда 
		Запись.ТекстОшибки = СтруктураОтчета.ТекстОшибки[0].ТекстДляОднойОшибки ;
	Иначе 
		 Запись.ТекстОшибки =  СтруктураОтчета.ТекстОшибки;
	КонецЕсли;	 
				...
				Если СписокПараллельныхОбъектов[ВидОбъекта] = Истина Тогда
					...
					// 27.03.19 Строганов Роман > Аукционы
						...
						Если Не ЗначениеЗаполнено(Запись.НомерПотока) Тогда
							...
							// 06.06.19 Строганов Роман > 
							Если (ВидОбъекта = Перечисления.ВидыОбъектовОбмена.РезультатСборки 
								Или ВидОбъекта = Перечисления.ВидыОбъектовОбмена.ЗаказНаОтгрузкуСтатус)
								И НомерПотока = 1 Тогда
								НомерПотока = 2;
							КонецЕсли;
							// 06.06.19 Строганов Роман <
...
Функция ОбъектыКОбработке(ВидыОбмена,КолВоОбъектов = 1000, НомерПотока = Неопределено)
	...
	|ИТОГИ
	|	МИНИМУМ(Период), МАКСИМУМ(ВидОбмена) 
	|ПО
	|	НомерСообщения";
