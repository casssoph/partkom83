	...
	Пока ВыборкаПоНомеруСообщения.Следующий() цикл 	
		...
		Пока ВыборкаСтрок.Следующий() Цикл
			...
			Если ВидОбмена <> Перечисления.ВидыОбменов.ОбменПартКом83_TopLog Тогда // 05.04.19 Строганов Роман > По ТопЛогу история по объектам фиксируется на уровне модуля менеджера.	
				...
				//	СписокОшибок  = ПолучитьСписокОшибокПоСТрокамДокумента(СтруктураОтчета.ТекстОшибки);
				//	Для каждого СтрокаОшибки из СписокОшибок цикл 
				//		ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(СтруктураОтчета.СсылкаНаОбъект, ТелоСообщения,,СтруктураОтчета.Ошибка,СтрокаОшибки.Значение,СтрокаОшибки.Ключ);	
				//	КонецЦикла;						
				//иначе 	
...
РеквизитыФилала  = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Филиал,"Склад,ОсновнойРегионФилиала,ОсновнаяОрганизация");
...
Если ЗначениеЗаполнено(СтруктураДанныхСайта.filial_uuid) тогда 
Менеджер = Справочники.Менеджеры.ПолучитьСсылку(СтруктураДанныхСайта.filial_uuid);
