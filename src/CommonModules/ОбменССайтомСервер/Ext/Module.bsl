...
Процедура ОбработатьОбъектыОбмена(УзелОбмена, НомерПотока = Неопределено) Экспорт
	...
	Если ИмяПланаОбмена = "ОбменПартКом83_TopLog" Тогда
		ВидОбмена = Новый Массив;
		ВидОбмена.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОбменов.ОбменПартКом83_TopLog"));
		ВидОбмена.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОбменов.ОбменПартКом83_TopLog_РТУ"));
	Иначе
