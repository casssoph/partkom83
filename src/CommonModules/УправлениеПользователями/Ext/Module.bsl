...
Функция ОпределитьТекущегоПользователя() экспорт
	...
	Если ЭтоАктивнаяЗадачаJirа(ПредопределенноеЗначение("Справочник.ЗадачиJira.XX2405")) Тогда
...
Функция ПрочитатьЗначениеПраваДляТекущегоПользователя(Право, ЗначениеПоУмолчанию)
	...
	|ГДЕ
	|	ЗначенияДополнительныхПравПользователя.Право = &ПравоПользователя
...
Функция ПолучитьМинимальнуюНаценку(Пользователь) Экспорт 
	
	Если Пользователь = глЗначениеПеременной("глТекущийПользователь") Тогда		
		КэшНастроекПользователей = глЗначениеПеременной("МинимальныеНаценки");
		ЗначениеНастройки = КэшНастроекПользователей[Пользователь];
		Если ЗначениеНастройки = Неопределено Тогда
			ЗначениеНастройки = ПрочитатьМинимальнуюНаценку(Пользователь);
			КэшНастроекПользователей[Пользователь] = ЗначениеНастройки;
			#Если Сервер Тогда
				глЗначениеПеременнойУстановить("МинимальныеНаценки", КэшНастроекПользователей, Истина);
			#КонецЕсли
		КонецЕсли;	
		Возврат ЗначениеНастройки;		
	КонецЕсли;
...
Функция  ПрочитатьМинимальнуюНаценку(Пользователь)
	
	Запрос = Новый Запрос;
	Запрос.Текст =  "ВЫБРАТЬ
	                |	МинимальныеНаценкиПоГруппамПользователям.Наценка
	                |ИЗ
	                |	РегистрСведений.МинимальныеНаценкиПоГруппамПользователям КАК МинимальныеНаценкиПоГруппамПользователям
	                |ГДЕ
	                |	МинимальныеНаценкиПоГруппамПользователям.ГруппаПользователей.ПользователиГруппы.Пользователь = &Пользователь
	                |
	                |ОБЪЕДИНИТЬ ВСЕ
	                |
	                |ВЫБРАТЬ
	                ...
	                |	РегистрСведений.МинимальныеНаценкиПоГруппамПользователям КАК МинимальныеНаценкиПоГруппамПользователям
	                |ГДЕ
	                |	МинимальныеНаценкиПоГруппамПользователям.ГруппаПользователей = ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ВсеПользователи)";
	Запрос.УстановитьПараметр("Пользователь", Пользователь);				
	Выборка = Запрос.Выполнить().Выбрать();
