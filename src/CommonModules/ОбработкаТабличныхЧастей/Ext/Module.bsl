...
Процедура ПриИзмененииСуммыТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ТекПользователь, РасчетАвтоматическихСкидок = Ложь, 
                                    ...
                                    ИмяТабЧастиДокумента = Неопределено,
									Отказ = 0) Экспорт
	...
	Если (СтрокаТабличнойЧасти.Количество - Отказ) = 0 Тогда
		...
		Если ЕстьРеквизитПроцентСкидкиНаценки Тогда
			...
			Если ПересчитыватьСкидку Тогда
				...
				Иначе
					...
					СуммаБезСкидок  = СтрокаТабличнойЧасти.Цена * (СтрокаТабличнойЧасти.Количество -  Отказ);
			...
			Иначе
				...
				Иначе
					...
					СтрокаТабличнойЧасти.Цена = СуммаБезСкидок / (СтрокаТабличнойЧасти.Количество - Отказ);
					
				КонецЕсли;
			КонецЕсли;
		Иначе
			СтрокаТабличнойЧасти.Цена = СтрокаТабличнойЧасти.Сумма / (СтрокаТабличнойЧасти.Количество - Отказ);
		КонецЕсли;
	КонецЕсли;
...
Функция СтруктураДляРасчетаРезерваПриПодбореНоменклатуры(ДокументФорма, ЕстьЗаказВТабличнойЧасти = Ложь, ИмяРеквизитаЗаказаВШапке = "", ВидРегистраПлюсСвободныйОстаток = "ТоварыВРезервеНаСкладах", НазваниеПоляРезерваВРегистре = "ДокументРезерва", ИмяКолонкиРезерваВТабличнойЧасти = "ЗаявкаПокупателя", ИспользоватьЗначениеКонстанты = Истина) Экспорт
	...
	СтруктураВременнойТаблицы.Вставить("ДокументРезерва","ДокументСсылка.ЗаявкаПокупателя");
	...
	Если ЕстьЗаказВТабличнойЧасти Тогда
		...
		КонецЦИкла;
	КонецЕсли;
	тчТовары.Свернуть("Номенклатура,  ДокументРезерва","");
...
//  ТаблицаЗначений.
//
Функция ЗапросПоСкидкам(ДатаСкидок, МассивНоменклатуры,  МассивКачество,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		СпрНоменклатура.Ссылка КАК Номенклатура КАК ХарактеристикаНоменклатуры
	|	ИЗ
...
Процедура РассчитатьСкидкиПриПродаже(ДокументОбъект, ТабличнаяЧастьНоменклатуры,
	...
	ТаблицаСкидок = ЗапросПоСкидкам(ДатаСкидок, МассивНоменклатуры,  МассивКачество,
	...
	Если ТаблицаСкидок.Найти(УсловиеКолво, "НУсловие, ЦУсловие") <> Неопределено Тогда
		ТаблицаКоличестваТоваров = ТаблицаТоваров.Скопировать(,
		   "Номенклатура,  Количество, Коэффициент");

		Для Каждого СтрокаТЧ Из ТаблицаКоличестваТоваров Цикл
			СтрокаТЧ.Количество = СтрокаТЧ.Количество * СтрокаТЧ.Коэффициент;
		КонецЦикла;

		ТаблицаКоличестваТоваров.Свернуть("Номенклатура", "Количество");

		ТаблицаКоличестваТоваров.Индексы.Добавить("Номенклатура");

		СтруктураПоискаКоличества = Новый Структура("Номенклатура");
	КонецЕсли;
...
//  ДокументОбъект           - объект редактируемого документа.
//
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "", вхДата = Неопределено, вхОрганизация = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
		...
		ИначеЕсли ВидДокумента = "Реализация" Тогда
			...
			Иначе
				Отказ = Ложь;
				Если вхДата = Неопределено И вхОрганизация = Неопределено Тогда
				...
				Иначе
					ПараметрыУП = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиНалоговыйУчет(вхДата, Отказ, вхОрганизация);
				КонецЕсли;
...
Процедура ЗаполнитьЕдиницуЦенуПродажиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета,
	...
	//	КонецЕсли;
	//Иначе
		Цена = Ценообразование.ПолучитьЦенуНоменклатуры(СтрокаТабличнойЧасти.Номенклатура, , ТипЦен);
...
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, СпособРасчета = Неопределено) Экспорт
	...
	//Добавлено Валиахметов А.А. 15.03.2018
	Если (МетаданныеДокумента = Метаданные.Документы.ЗаявкаПокупателя Или МетаданныеДокумента = Метаданные.Документы.КорректировкаЗаявкиПокупателя) И ИмяТабличнойЧасти = "Товары" Тогда 
		Строки = ДокументОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи));
		Для Каждого СтрокаТЧ Из Строки Цикл 
			КоличествоОтказано = КоличествоОтказано + СтрокаТЧ.Количество;
		КонецЦикла;
 	КонецЕсли;
	
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ПроцентСкидкиНаценки", МетаданныеДокумента,
		  ИмяТабличнойЧасти) Тогда
		ПроцентСкидкиНаценки = СтрокаТабличнойЧасти.ПроцентСкидкиНаценки;	  
	КонецЕсли;
	//Конец Добавлено Валиахметов А.А. 15.03.2018	
	...
	СтрокаТабличнойЧасти.Сумма = (СтрокаТабличнойЧасти.Количество - КоличествоОтказано)* (СтрокаТабличнойЧасти.Цена - СтрокаТабличнойЧасти.Цена * ПроцентСкидкиНаценки/100);
...
Процедура РассчитатьПлановуюСебестоимостьВСтрокеТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	ПлановаяСебестоимость = Ценообразование.ПолучитьЦенуНоменклатуры(СтрокаТабличнойЧасти.Номенклатура, 
...
Процедура ЗаполнитьСуммуИОтклонениеОтПлановойСебестоимостиТабЧасти(ОформлениеСтроки, СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ЗаполнитьСуммуИОтклонениеОтПлановойСебестоимостиТабЧасти";
...
//Пересчет специфических полей
Процедура ПересчитатьСтрокуТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыУчетаНДС;
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьСтрокуТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ПересчитатьЦенуСоСкидкойСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьЦенуПослеУценкиСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьСуммуСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьСуммуАРВСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьПрибыльСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	УстановитьЦенуЗакупкиСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьСебестоимостьСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ПересчитатьЦенуПоставщикуСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
	Если СтруктураДействий.Свойство("РассчитатьСуммуНДС", ПараметрыУчетаНДС) Тогда
		
		ТекущаяСтрока.СуммаНДС = УчетНДС.РассчитатьСуммуНДС(ТекущаяСтрока.Сумма,
		ПараметрыУчетаНДС.УчитыватьНДС, ПараметрыУчетаНДС.СуммаВключаетНДС,
		УчетНДС.ПолучитьСтавкуНДС(ТекущаяСтрока.СтавкаНДС));
		
	КонецЕсли;
	Если СтруктураДействий.Свойство("РассчитатьСуммуНДСПлан", ПараметрыУчетаНДС) Тогда
		
		ТекущаяСтрока.СуммаНДСПлан = УчетНДС.РассчитатьСуммуНДС(ТекущаяСтрока.СуммаПлан,
		ПараметрыУчетаНДС.УчитыватьНДС, ПараметрыУчетаНДС.СуммаВключаетНДС,
		УчетНДС.ПолучитьСтавкуНДС(ТекущаяСтрока.СтавкаНДС));
...
Процедура ПересчитатьЦенуСоСкидкойСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	Перем мРежимОкругления;
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьЦенуСоСкидкойСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуСоСкидкой", мРежимОкругления) Тогда
		ТекущаяСтрока.ЦенаСоСкидкой = ЦенаСоСкидкойСтрокиТабличнойЧасти(ТекущаяСтрока, мРежимОкругления);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьСуммуСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	Перем ОтказыПоТекущемуДокументу;
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьСуммуСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("ПересчитатьСумму", ОтказыПоТекущемуДокументу) Тогда
		
		ТекущаяСтрока.Сумма = Окр((ТекущаяСтрока.Количество - ОтказыПоТекущемуДокументу) * ТекущаяСтрока.ЦенаСоСкидкой, 2);
		
	КонецЕсли;

КонецПроцедуры

Процедура ПересчитатьПрибыльСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьПрибыльСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("ПересчитатьПрибыль") Тогда
		ТекущаяСтрока.Прибыль = ТекущаяСтрока.Количество * (ТекущаяСтрока.ЦенаСоСкидкой - ТекущаяСтрока.ЦенаЗакупки);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьЦенуЗакупкиСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_УстановитьЦенуЗакупкиСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("УстановитьЦенуЗакупки") Тогда
		
		ТекущаяСтрока.ЦенаЗакупки = 0;
		
		Попытка
			ТекущаяСтрока.ЦенаЗакупки = ТекущаяСтрока.СтрокаЗаявки.ЦенаЗакупки;
		Исключение
		КонецПопытки;
		
		Если ТекущаяСтрока.ЦенаЗакупки = 0 Тогда
			...
			|ГДЕ
			|	ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.Номенклатура = &Номенклатура";
...
Функция ЦенаСоСкидкойСтрокиТабличнойЧасти(ТекущаяСтрока, мРежимОкругления) Экспорт
	
	Возврат Окр(ТекущаяСтрока.Цена - ТекущаяСтрока.Цена * ТекущаяСтрока.ПроцентСкидкиНаценки / 100, мРежимОкругления);
	
КонецФункции

Функция МинимальнаяЦенаСтрокиТабличнойЧасти(ТекущаяСтрока, мРежимОкругления, МинимальнаяНаценка) Экспорт
	
	Возврат Окр(ТекущаяСтрока.ЦенаЗакупки + ТекущаяСтрока.ЦенаЗакупки * МинимальнаяНаценка / 100, мРежимОкругления);
	
КонецФункции

Функция СуммаБезСкидкиСтрокиТабличнойЧасти(ТекущаяСтрока, ОтказыПоТекущемуДокументу = 0) Экспорт
	
	СуммаБезСкидки  =  Окр((ТекущаяСтрока.Количество - ОтказыПоТекущемуДокументу) * ТекущаяСтрока.Цена, 2);
	
	Возврат СуммаБезСкидки;
	
КонецФункции

Функция СуммаСкидкиСтрокиТабличнойЧасти(ТекущаяСтрока, ОтказыПоТекущемуДокументу = 0) Экспорт
	
	СуммаБезСкидки = СуммаБезСкидкиСтрокиТабличнойЧасти(ТекущаяСтрока, ОтказыПоТекущемуДокументу);
	
	СуммаСкидки = ?(СуммаБезСкидки = 0, 0, СуммаБезСкидки - ТекущаяСтрока.Сумма); 
	
	Возврат СуммаСкидки;
	
КонецФункции

Процедура ПересчитатьСуммуАРВСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьСуммуАРВСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуАРВ") Тогда
		ТекущаяСтрока.Сумма 	= ТекущаяСтрока.Количество * ТекущаяСтрока.ЦенаПослеУценки;
		ТекущаяСтрока.СуммаПлан = ТекущаяСтрока.КоличествоПлан * ТекущаяСтрока.ЦенаПослеУценки;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьЦенуПослеУценкиСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьЦенуПослеУценкиСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуПослеУценки") Тогда
		ТекущаяСтрока.ЦенаПослеУценки = ТекущаяСтрока.Цена - ТекущаяСтрока.СуммаУценки;
		ТекущаяСтрока.ЦенаПослеУценки = ТекущаяСтрока.ЦенаПослеУценки - Окр(ТекущаяСтрока.ЦенаПослеУценки*ТекущаяСтрока.ПроцентУценки/100, 2);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьСебестоимостьСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьСебестоимостьСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	СтрокаИменКоличества = Неопределено;
	Если СтруктураДействий.Свойство("ПересчитатьСебестоимость", СтрокаИменКоличества) Тогда
		
		Если СтрокаИменКоличества = Неопределено Тогда
			СтрокаИменКоличества = "Количество";
		КонецЕсли;
		
		МассивИменКоличества = СтрРазделить(СтрокаИменКоличества, ",");
		
		Для Каждого ИмяПоля Из МассивИменКоличества Цикл
			ТекущаяСтрока.Себестоимость = ТекущаяСтрока.СебестоимостьЦена * ТекущаяСтрока[ИмяПоля];
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьЦенуПоставщикуСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	лКлючАлгоритма = "ОбщийМодуль_ОбработкаТабличныхЧастей_ПересчитатьЦенуПоставщикуСтрокиТабличнойЧасти";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	СебестоимостьЦена = 0;	
	Если СтруктураДействий.Свойство("ПересчитатьЦенуПоставщикуПослеУценки", СебестоимостьЦена) Тогда
		
		Если СебестоимостьЦена = Неопределено Тогда
			СебестоимостьЦена = 0;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СебестоимостьЦена) Тогда
			//ХудинВВ XX-2851 01082019
			ОбщегоНазначения.СообщитьОбОшибке("Не удалось определить цену возврата поставщику",,,СтатусСообщения.Важное);
		КонецЕсли;
		
		ТекущаяСтрока.ЦенаПоставщикуПослеУценки = СебестоимостьЦена - ТекущаяСтрока.СуммаУценкиПоставщику;
		ТекущаяСтрока.ЦенаПоставщикуПослеУценки = ТекущаяСтрока.ЦенаПоставщикуПослеУценки - Окр(ТекущаяСтрока.ЦенаПоставщикуПослеУценки*ТекущаяСтрока.ПроцентУценкиПоставщику/100, 2);
	КонецЕсли;
