...
Функция РассчитатьУслугуЭД(пар_express_delivery_request_id, ИспользоватьФильтр = Истина) экспорт
	...
	|	И Реализация.Контрагент.Регион = &Регион
	...
	Если РегионОтбора = Справочники.Регионы.ПустаяСсылка() ИЛИ НЕ ИспользоватьФильтр тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"И Реализация.Контрагент.Регион = &Регион","");
	КонецЕсли;
		...
		Для каждого рту из  Выборка цикл
			...
			Иначе
				Если рту.Услуги_Сч > 0 тогда
					Ответ = УдалитьУслугу(рту.РТУ,у_м);	
					Если Не Ответ тогда
						Прервать;
					КонецЕсли;
				КонецЕсли;
			...
			Если рту.Товары_СуммаЭд > 0 тогда
				Ответ = УбратьРазмазывание(рту.РТУ);	
				Если Не Ответ тогда
					Прервать;
				КонецЕсли;
			КонецЕсли;
