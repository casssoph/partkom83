...
Процедура СформироватьSMS_ОтказГрупногабарит(пар_Отказы) экспорт
	...
	id_ЭД = ОбщегоНазначения.ПолучитьЗначениеРеквизита(пар_Отказы[0].Регистратор,"express_delivery_request_id");
	Если ЗначениеЗаполнено(id_ЭД) тогда
		РассчитатьУслугуЭД(id_ЭД);
	КонецЕсли;
...
// пересчет услуги ЭД
Процедура РассчитатьУслугуЭД(пар_express_delivery_request_id) экспорт
	...
	|	И Реализация.Дата МЕЖДУ &Дата1 И &Дата2
	...
	|	СУММА(РеализацияТоваровУслугТовары.ЭД_Цена_Номенклатура * РеализацияТоваровУслугТовары.Количество) КАК СуммаДо,
	...
	Запрос.УстановитьПараметр("Дата1", ДобавитьМесяц(ТекущаяДата(),-1));
	...
	ВыборкаИтоги.Свернуть("ЭД_СуммаБесплатнойЭД,ЭД_СтоимостьУслугиЭДрубли,ЭД_СтоимостьУслугиЭДбонусы,ЭД_ВидОплаты","Сч,СуммаДокумента,Товары_Сч,Товары_Сумма,Товары_СуммаНДС,Товары_СуммаДо,Товары_СуммаЭд,Услуги_Сч,Услуги_Сумма,Услуги_СуммаНДС");
	
	ЭД_СуммаЗаказа = ВыборкаИтоги[0].Товары_Сумма; 
	ЭД_Бесплатно = Истина;
	...
	Если ЭД_Бесплатно = Ложь И ВыборкаИтоги[0].ЭД_ВидОплаты = Перечисления.ВидыОплатыЭД.Рубли тогда	
		...
		Для каждого рту из  Выборка цикл
			Если рту.ЭтоКорень тогда
				ДобавитьУслугу(АдресКорня.РТУ,ВыборкаИтоги[0].ЭД_СтоимостьУслугиЭДрубли,у_м);
			Иначе
				Если рту.Услуги_Сч > 0 тогда
					УдалитьУслугу(рту.РТУ,у_м);	
				КонецЕсли;
			КонецЕсли;
			Если рту.Товары_СуммаЭд > 0 тогда
				УбратьРазмазывание(рту.РТУ);	
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ЭД_Бесплатно = Ложь И ВыборкаИтоги[0].ЭД_ВидОплаты = Перечисления.ВидыОплатыЭД.ВЦенеТовара тогда
		...
		Для каждого рту из  Выборка цикл
			Если рту.ЭтоКорень тогда
				Размазать(АдресКорня.РТУ,ВыборкаИтоги[0].ЭД_СтоимостьУслугиЭДрубли);
			Иначе
				Если рту.Товары_СуммаЭд > 0 тогда
					УбратьРазмазывание(рту.РТУ);	
				КонецЕсли;
...
КонецФункции
Процедура УдалитьУслугу(пар_Ссылка,пар_услуги)
...
КонецПроцедуры
Процедура УбратьРазмазывание(пар_Ссылка)
	Размазать(пар_Ссылка,0);	
КонецПроцедуры
Процедура Размазать(пар_Ссылка,пар_сумма)
...
КонецПроцедуры
Процедура ДобавитьУслугу(пар_ссылка,пар_сумма,пар_услуги)
...
КонецПроцедуры
Процедура ПересчитатьСтрокуРТУ(пар_строка)
