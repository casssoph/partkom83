...
Процедура СформироватьSMS_ОтказГрупногабарит(пар_Отказы) экспорт
	...
	//вызывается из модуля менеджера РТУ при проведении
	//пар_Отказы = ТаблицаЗначений, где
	//Регистратор = РТУссылка
	//Период = РТУдата
	//СтрокаЗаявки
	//Заявка
	//ПричинаОтказа
	...
	Если  пар_Отказы.Количество() = 0 тогда
		Возврат;
	КонецЕсли;
	
	пар_Триггер = Справочники.НастройкаSMSоповещения.Триггер_04;
	пар_Источник = Новый Массив;
