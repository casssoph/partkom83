...
Процедура ОткрытьФормуВыбораПечатныхФормОбъекта(Объект, ФормаВладелец) Экспорт
	...
	Для каждого СтрокаКнопки Из Форма.ЭлементыФормы.ДеревоМакетовПечати.ВыделенныеСтроки Цикл
		...
		Расшифровка = СтрокаКнопки.Расшифровка;
		ИмяПечатнойФормы = СтрокаКнопки.Текст;
		...
		Если ТипЗнч(Расшифровка) = Тип("Структура") и Расшифровка.ВидПечатнойФормы = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма Тогда
			...
			//Добавлено Валиахметов А.А. 02.03.2018
			СменитьСтатусАктаПриемкиДляПТУ(Объект, Расшифровка);
			//Конец Добавлено Валиахметов А.А. 02.03.2018
			
			ТабДокумент = НапечататьВнешнююФорму(Объект.Ссылка, Расшифровка);
			
			ЭтоДокумент = Метаданные.Документы.Содержит(Объект.Метаданные());
			НапечататьДокумент(ТабДокумент, 0, Форма.НаПринтер, ?(ЭтоДокумент, ОбщегоНазначения.СформироватьЗаголовокДокумента(Объект), Строка(Объект)), Объект.Ссылка, ИмяПечатнойФормы);
		Иначе
	...
	Пока ВыборкаЗаменяемыхФорм.Следующий() Цикл
		...
		Пока ВыборкаСсылок.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				// +++ Карпычев (15.02.18)
				ДополнительныеПараметры = СтруктураРасшифровки.СсылкаНаВнешнююОбработку.Принадлежность[СтруктураРасшифровки.НомерСтроки - 1].ДополнительныеПараметрыОбработки.Получить();
				Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
					Если ДополнительныеПараметры.Свойство("ПредставлениеМакета") Тогда
						НаименованиеПечатнойФормы = ДополнительныеПараметры.ПредставлениеМакета;
					КонецЕсли;
				КонецЕсли;
				// --- Карпычев (15.02.18)
...
Функция НапечататьДокументПоУмолчанию(Объект, КоличествоЭкземпляров = 0, НаПринтер = Ложь) Экспорт
	...
	Если ЭтоДокумент И Объект.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить и Не УправлениеДопПравамиПользователей.РазрешитьПечатьНепроведенныхДокументов(Объект.Проведен) Тогда
		Если Не Строка(Объект.Метаданные())="Возврат товаров поставщику" Тогда 
			Сообщить("Недостаточно полномочий для печати непроведенного документа!"); 
			Возврат Ложь;
		ИначеЕсли Не НастройкаПравДоступа.ПолучитьЗначениеПраваПользователя(УправлениеПользователями.ОпределитьТекущегоПользователя(), ПланыВидовХарактеристик.ПраваПользователей.ПечатьНеПроведенногоВозвратаПоставщику) Тогда 
			Сообщить("Недостаточно полномочий для печати непроведенного документа!"); 
			Возврат Ложь;
		КонецЕсли;	
	КонецЕсли;
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") и Расшифровка.ВидПечатнойФормы = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма Тогда
		...
		НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, 
		 ?(ЭтоДокумент, ОбщегоНазначения.СформироватьЗаголовокДокумента(Объект), Строка(Объект)), Объект.Ссылка,СтрокаПоУмолчанию.Текст); 
	Иначе
...
//Добавлено Валиахметов А.А. 02.03.2018
Процедура СменитьСтатусАктаПриемкиДляПТУ(Объект, Расшифровка)
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") 
		И ТипЗнч(Расшифровка.СсылкаНаВнешнююОбработку) = Тип("СправочникСсылка.ВнешниеОбработки") 
		И Расшифровка.СсылкаНаВнешнююОбработку.ИмяОбъектаМетаданных =  "АктПриемки" Тогда 
		Если Не (Объект.СтатусАктаПриемки = Справочники.СтатусыДокументов.АктПриемкиПолученОтвет Или 
			Объект.СтатусАктаПриемки = Справочники.СтатусыДокументов.АктПриемкиРаботаЗавершена) Тогда  
			Объект.СтатусАктаПриемки = Справочники.СтатусыДокументов.АктПриемкиРаспечатанДляПоставщика;
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
//Конец Добавлено Валиахметов А.А. 02.03.2018
...
//
Процедура НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, ИмяПечатнойФормы = Неопределено) Экспорт
	...
	Если ТипЗнч(НаПринтер) = Тип("Булево") Тогда
		...
		Иначе
			...
			ФормаПечати.ПечатныйДокумент.Защита = УправлениеДопПравамиПользователей.ЗащитаТаблиц();
			ФормаПечати.ИмяПечатнойФормы   = ИмяПечатнойФормы;
			ФормаПечати.Открыть();
		КонецЕсли;
