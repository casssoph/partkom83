...
Функция НадоПриниматьПисьмо(УчетнаяЗапись, ИнтернетПисьмо, МассивПисемДляУдаления)
	...
	лКлючАлгоритма = "ОбщийМодуль_УправлениеЭлектроннойПочтой_НадоПриниматьПисьмо";
...
Процедура ПолучениеОтправкаПисем(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, МассивУчетныхЗаписей = Неопределено, 
	...
	лКлючАлгоритма = "ОбщийМодуль_УправлениеЭлектроннойПочтой_ПолучениеОтправкаПисем";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если Отправить Тогда
		...
		ИначеЕсли МассивПисем = Неопределено И ТипЗнч(МассивУчетныхЗаписей) = Тип("Массив") Тогда
			...
			|	И ЭлектронноеПисьмо.УчетнаяЗапись В(&СписокВладельцев)
			|	И ЭлектронноеПисьмо.СтатусПисьма = &Исходящее
			|	И Не ЭлектронноеПисьмо.ПометкаУдаления
			|ИТОГИ ПО
			|	УчетнаяЗапись";
		...
		ИначеЕсли ТипЗнч(МассивПисем) = Тип("Массив")  Тогда
	...
	Если СоответствиеУчетныхЗаписейВсего.Количество() > 0 Тогда
				...
				Если ТипЗнч(ДеревоУчетныхЗаписейИПисем) = Тип("ДеревоЗначений") Тогда
					...
					Если СтрокаДереваУчетнойЗаписи <> Неопределено Тогда
						...
						Для каждого СтрокаПисьма Из СтрокаДереваУчетнойЗаписи.Строки Цикл
							...
							|	ВложенияЭлектронныхПисем.ИдентификаторВоВнешнемХранилище
							|ИЗ
							...
							Пока Выборка.Следующий() Цикл
								...
								//ХудинВВ XX-2297 14052019
								Если Выборка.ИдентификаторВоВнешнемХранилище <> 0 Тогда
									
									ТекстОшибкиПолученияФайлаИзВХ = "";
									
									ДвоичныеДанные = РаботаСВнешнимФайловымХранилищем.ПолучитьФайлПоИдентификатору(
									Выборка.ИдентификаторВоВнешнемХранилище,
									ТекстОшибкиПолученияФайлаИзВХ,
									Ложь,
									Ложь);
									
									//Игнорируем ошибку получения
				...
				Если МассивЗаголовковПисем.Количество() > 0 Тогда
					...
					Для каждого ИнтернетПисьмо Из МассивПисем Цикл
						...
						Если НЕ НадоПриниматьПисьмо(УчетнаяЗапись, ИнтернетПисьмо, МассивПисемДляУдаления) Тогда
							...
							Если УчетнаяЗапись.ФиксироватьОшибкиПолученияОтправки Тогда
								КритическиеСобытияСервер.ЗарегистрироватьКритическоеСобытие(
								, 
								Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаПолученияПочты,
								"Отмена загрузки письма учетной записи (НЕ НадоПриниматьПисьмо()): " + УчетнаяЗапись,
								,
								Истина,
								ОписаниеДопОшибкиДляИнтернетПисьма(ИнтернетПисьмо),
						...
						Если ТипЗнч(Письмо) <> Тип("ДокументОбъект.ЭлектронноеПисьмо") Тогда
							...
							Если УчетнаяЗапись.ФиксироватьОшибкиПолученияОтправки Тогда
								...
								, 
								Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаПолученияПочты,
								"Отмена загрузки письма учетной записи (не удалось получить объект): " + УчетнаяЗапись,
								,
								Истина,
								ОписаниеДопОшибкиДляИнтернетПисьма(ИнтернетПисьмо),
								"УправлениеЭлектроннойПочтой.ПолучениеОтправкаПисем");
						...
						Если НЕ ПустаяСтрока(Письмо.Тема) Тогда
							ЗаполнитьКонтрагентаИАктВозвратаПисьма(Письмо);
						КонецЕсли;
						...
						Попытка
							...
							//Если это письмо по процессу возвратов, то его надо будет обработать
							Если ЗначениеЗаполнено(Письмо.АктРассмотренияВозврата) Тогда
								РегистрыСведений.СобытияКОбработкеАктовРассмотренияВозврата.Добавить(Письмо.АктРассмотренияВозврата, 
								Перечисления.ВидыСобытийКОбработкеПроцессаВозвратов.ВходящееЭлектронноеПисьмо,
								Письмо.Ссылка, Ложь);								
							КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибок) И УчетнаяЗапись.ФиксироватьОшибкиПолученияОтправки  Тогда
			
			КритическиеСобытияСервер.ЗарегистрироватьКритическоеСобытие(
			, 
			Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаПолученияПочты,
			"Ошибка получения/отправки почты для учетной записи: " + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + ">",
			,
			Истина,
			"Ошибка: "+ТекстОшибок,
			"УправлениеЭлектроннойПочтой.ПолучениеОтправкаПисем");
...
Функция ОписаниеДопОшибкиДляИнтернетПисьма(ИнтернетПисьмо)
	
	лКлючАлгоритма = "ОбщийМодуль_УправлениеЭлектроннойПочтой_ОписаниеДопОшибкиДляИнтернетПисьма";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если ТипЗнч(ИнтернетПисьмо.Отправитель) = Тип("Строка") Тогда
		ОтправительАдресЭлектроннойПочты = ИнтернетПисьмо.Отправитель;
	Иначе
		ОтправительАдресЭлектроннойПочты = ИнтернетПисьмо.Отправитель.Адрес;
	КонецЕсли;
	
	ДопОписаниеОшибки = "ДатаОтправления: "+ИнтернетПисьмо.ДатаОтправления+", Отправитель: "+ОтправительАдресЭлектроннойПочты+", Идентификатор: "+ИнтернетПисьмо.ИдентификаторСообщения;
	ДопОписаниеОшибки = ДопОписаниеОшибки+Символы.ПС+"Тема: "+ИнтернетПисьмо.Тема;
	Для каждого ТекстПочтовогоСообщения Из ИнтернетПисьмо.Тексты Цикл
		ДопОписаниеОшибки = ДопОписаниеОшибки+Символы.ПС+"Текст: "+ТекстПочтовогоСообщения.Текст
	КонецЦикла;
	
	Возврат ДопОписаниеОшибки;
...
Функция НаписатьПисьмо(ТекущийПользователь, СтруктураНовогоПисьма = Неопределено, ПеренестиВложенияИзОснования = Ложь, Копирование = Ложь,
	...
	СтруктураНовогоПисьма.Свойство("АктРассмотренияВозврата", АктРассмотренияВозврата);
	Если ЗначениеЗаполнено(АктРассмотренияВозврата) Тогда
		Письмо.АктРассмотренияВозврата = АктРассмотренияВозврата;
	КонецЕсли;
	
	Контрагент = Неопределено;
	СтруктураНовогоПисьма.Свойство("Контрагент", Контрагент);
	Если ЗначениеЗаполнено(Контрагент) Тогда
		Письмо.Контрагент = Контрагент;
	КонецЕсли;
	...
	Если НЕ ОткрыватьПисьмо Тогда
		...
		Письмо.ДатаТранспорта  = ТекущаяДата();
		Если ПустаяСтрока(Письмо.КодировкаПисьма) Тогда
			Письмо.КодировкаПисьма = "UTF8";
		КонецЕсли;
...
Процедура ОтветитьНаПисьмо(ТекущийПользователь, Письмо, ФормаВладелец = Неопределено) Экспорт
	...
	СтруктураПисьма.Вставить("АктРассмотренияВозврата"      , Письмо.АктРассмотренияВозврата);
	СтруктураПисьма.Вставить("Контрагент",		Письмо.Контрагент);
...
Процедура ЗаполнитьКонтрагентаИАктВозвратаПисьма(Письмо) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_УправлениеЭлектроннойПочтой_ЗаполнитьКонтрагентаИАктВозвратаПисьма";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	лУчетнаяЗапись = ВозвратыОтПокупателяСервер.УчетнаяЗаписьЭлектроннойПочтыВозвраты();
	
	Если Письмо.УчетнаяЗапись = лУчетнаяЗапись Тогда
		
		//Определим акт возврата по штрихкоду из темы
		Штрихкод = "";
		НачСимвол = СтрНайти(ПреобразованнаяТема, "#");
		Если НачСимвол > 0 Тогда
			КонСимвол  = СтрНайти(ПреобразованнаяТема, "#",,НачСимвол+1);
			Если КонСимвол > 0 Тогда
				Штрихкод = Сред(ПреобразованнаяТема, НачСимвол + 1, КонСимвол - НачСимвол - 1);
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(Штрихкод) Тогда
			Письмо.АктРассмотренияВозврата = Документы.АктРассмотренияВозврата.НайтиПоРеквизиту("Штрихкод", Штрихкод);
		КонецЕсли;
		
		//Определим контрагента
		//Тк по одному адресу элпочты может найтись несколько контрагентов, то
		//в первую очередь подставляем контрагента из акта
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	АктРассмотренияВозврата.Поставщик КАК Контрагент
		|ПОМЕСТИТЬ ПриоритетныеКонтрагенты
		|ИЗ
		|	Документ.АктРассмотренияВозврата КАК АктРассмотренияВозврата
		|ГДЕ
		|	АктРассмотренияВозврата.Ссылка = &АктРассмотренияВозврата
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ЭлПочтаКонтрагентов.Объект
		|ИЗ
		|	ЭлПочтаКонтрагентов КАК ЭлПочтаКонтрагентов
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПриоритетныеКонтрагенты КАК ПриоритетныеКонтрагенты
		|		ПО ЭлПочтаКонтрагентов.Объект = ПриоритетныеКонтрагенты.Контрагент
		|ГДЕ
		|	ЭлПочтаКонтрагентов.Представление ПОДОБНО &Представление
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВЫБОР
		|		КОГДА ПриоритетныеКонтрагенты.Контрагент ЕСТЬ NULL
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ";
		
		Запрос.УстановитьПараметр("АктРассмотренияВозврата", Письмо.АктРассмотренияВозврата);
		Запрос.УстановитьПараметр("Тип", Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		Запрос.УстановитьПараметр("Представление", "%"+Письмо.ОтправительАдресЭлектроннойПочты+"%");
		...
		Если Выборка.Следующий() Тогда
			Письмо.Контрагент = Выборка.Объект;
		КонецЕсли;;
...
Функция ПолучитьДоступныеУчетныеЗаписи(Пользователь) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты) Тогда
		...
		Если ЗначениеЗаполнено(Пользователь) Тогда
			...
			|		(УчетныеЗаписиЭлектроннойПочтыДоступКУчетнойЗаписи.Пользователь = &ТекПользователь ИЛИ УчетныеЗаписиЭлектроннойПочтыДоступКУчетнойЗаписи.Пользователь = &ПустойПользователь)
			|ГДЕ
...
Процедура ОтправитьОтчет(Параметры) Экспорт 
	
	//Отчет = Параметры.Отчет;
	//ШаблонПисьма = Параметры.ШаблонПисьма;
	//ТемаСообщения = Параметры.ТемаСообщения;
	//ВидТекстаПисьма = Параметры.ВидТекстаПисьма;
	//АдресЭлектроннойПочтыКонтрагента = Параметры.АдресЭлектроннойПочтыКонтрагента;
	//ОбъектПечати = Параметры.ОбъектПечати;
	//ВложенияHTML = Параметры.ВложенияHTML;
	//ВложенияTXT = Параметры.ВложенияTXT;
	//ВложенияXLS = Параметры.ВложенияXLS;
	//ВложенияMXL = Параметры.ВложенияMXL;
	//ИмяФайлаВложения = Параметры.ИмяФайлаВложения;
	//ЗаполнятьТекстПисьма = Параметры.ЗаполнятьТекстПисьма;
	
	СтруктураНовогоПисьма = Новый Структура;
	
	//#PK83-573 Kalinin V.A. ( 2018-05-25 )
	//Подружил даннуюОбработку с шаблонами
	Если ЗначениеЗаполнено(Параметры.ШаблонПисьма) тогда 
		ТемаПисьма = Параметры.ШаблонПисьма.ТемаПисьма;
		ТекстПисьма = Параметры.ШаблонПисьма.ТекстПисьма;
		
		СоответсвиеПолей = новый Соответствие;
		
		СоответсвиеПолей.Вставить("Дата",формат(Параметры.ОбъектПечати.Дата,"ДФ=dd.MM.yyyy"));
		СоответсвиеПолей.Вставить("Номер",Параметры.ОбъектПечати.Номер);
		Если ТипЗнч(Параметры.ОбъектПечати) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") тогда 
			СоответсвиеПолей.Вставить("ВхНомер",Параметры.ОбъектПечати.НомерВходящегоДокумента);
			СоответсвиеПолей.Вставить("ВхДата", формат(Параметры.ОбъектПечати.ДатаВходящегоДокумента,"ДФ=dd.MM.yyyy"));
		КонецЕсли;
		
		Для каждого ЭлСоответствия из СоответсвиеПолей цикл
			ТемаПисьма = СтрЗаменить(ТемаПисьма,"*"+ЭлСоответствия.Ключ+"*",ЭлСоответствия.Значение);
			ТекстПисьма = СтрЗаменить(ТекстПисьма,"*"+ЭлСоответствия.Ключ+"*",ЭлСоответствия.Значение);
		КонецЦикла;	
	...
	иначе 
		...
		Если Параметры.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.Текст Тогда
			
			СтруктураНовогоПисьма.Вставить("ВидТекста", Перечисления.ВидыТекстовЭлектронныхПисем.Текст);
			ФайлВФорматеТекст = ПолучитьИмяВременногоФайла();
			Параметры.Отчет.Записать(ФайлВФорматеТекст, ТипФайлаТабличногоДокумента.TXT);
		...
		ИначеЕсли Параметры.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.HTML Тогда
			...
			Параметры.Отчет.Записать(ФайлВФорматеHTML, ТипФайлаТабличногоДокумента.HTML);
			ТекстПисьма = Новый ТекстовыйДокумент;
			ТекстПисьма.Прочитать(ФайлВФорматеHTML);
			
			ХТМЛТекст = ТекстПисьма.ПолучитьТекст();
			
			//Кто будет это когда-нить дописывать. у 1с есть нативный механизм ДокументHTML, с ним гораздо проще работать
			НовыйHTMLДокумент = Новый COMОбъект("HtmlFile"); 
			НовыйHTMLДокумент.open("text/html");
		...
		СтруктураНовогоПисьма.Вставить("Тема", Параметры.ТемаСообщения);
		
		
	КонецЕсли;
	
	
	СтруктураНовогоПисьма.Вставить("Кому", Параметры.АдресЭлектроннойПочтыКонтрагента);
	
	СписокФайловВложений = Новый СписокЗначений;
	
	Если Не ЗначениеЗаполнено(Параметры.ИмяФайлаВложения) Тогда
		Параметры.ИмяФайлаВложения = Параметры.ТемаСообщения;
	КонецЕсли; 
	
	Если Параметры.ВложенияHTML Тогда
		Если ФайлВФорматеHTML = Неопределено Тогда
			ФайлВФорматеHTML = ПолучитьИмяВременногоФайла();
			Параметры.Отчет.Записать(ФайлВФорматеHTML, ТипФайлаТабличногоДокумента.HTML);
		КонецЕсли;
		СписокФайловВложений.Добавить(Новый Структура("Хранилище, ИмяФайла, Наименование", ПолучитьДвоичныеДанные(ФайлВФорматеHTML), (Параметры.ИмяФайлаВложения + ".HTM"), Параметры.ИмяФайлаВложения));
	КонецЕсли; 
	
	Если Параметры.ВложенияTXT Тогда
		Если ФайлВФорматеТекст = Неопределено Тогда
			ФайлВФорматеТекст = ПолучитьИмяВременногоФайла();
			Параметры.Отчет.Записать(ФайлВФорматеТекст, ТипФайлаТабличногоДокумента.TXT);
			
		КонецЕсли; 
		СписокФайловВложений.Добавить(Новый Структура("Хранилище, ИмяФайла, Наименование", ПолучитьДвоичныеДанные(ФайлВФорматеТекст), (Параметры.ИмяФайлаВложения + ".TXT"), Параметры.ИмяФайлаВложения));		
	КонецЕсли; 
	
	Если Параметры.ВложенияMXL Тогда
		ФайлВФорматеMXL = ПолучитьИмяВременногоФайла();
		Параметры.Отчет.Записать(ФайлВФорматеMXL, ТипФайлаТабличногоДокумента.MXL);
		СписокФайловВложений.Добавить(Новый Структура("Хранилище, ИмяФайла, Наименование", ПолучитьДвоичныеДанные(ФайлВФорматеMXL), (Параметры.ИмяФайлаВложения + ".MXL"), Параметры.ИмяФайлаВложения));
	КонецЕсли; 
	
	Если Параметры.ВложенияXLS Тогда
		ФайлВФорматеXLS = ПолучитьИмяВременногоФайла();
		Параметры.Отчет.Записать(ФайлВФорматеXLS, ТипФайлаТабличногоДокумента.XLS);
		СписокФайловВложений.Добавить(Новый Структура("Хранилище, ИмяФайла, Наименование", ПолучитьДвоичныеДанные(ФайлВФорматеXLS), (Параметры.ИмяФайлаВложения + ".XLS"), Параметры.ИмяФайлаВложения));
	КонецЕсли;
	...
	Если Параметры.ЗаполнятьТекстПисьма Тогда
		...
		СтруктураНовогоПисьма.Тело, СтруктураНовогоПисьма.Тема, СтруктураНовогоПисьма.Кому,,,СтруктураНовогоПисьма.СписокФайловВложений);
	Иначе
		РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.ОтправкаФайлаОтчета,
		, СтруктураНовогоПисьма.Тема, СтруктураНовогоПисьма.Кому,,,СтруктураНовогоПисьма.СписокФайловВложений);
	КонецЕсли;
