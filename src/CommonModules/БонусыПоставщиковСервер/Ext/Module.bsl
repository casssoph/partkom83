...
Функция ПолучитьТекстПисьмаПоУмолчанию(ДокументСсылка)
	...
	|Данная премия начисляется 1 раз в квартал и составляет  " + ДокументСсылка.ПроцентБонуса + "% ("+ ЧислоПрописью(ДокументСсылка.ПроцентБонуса,"НП=Ложь",",,,,,,,,0") + " %) от общей стоимости Товаров, отгруженных Покупателю Поставщиком за указанный период.
	|
	|Необходимо: 
