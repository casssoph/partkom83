		...
		Пока Номер >= 0  Цикл
			...
			Если СтрокаТаблицы = Неопределено
				...
				Если СтрокаТаблицы <> Неопределено И Элемент.Дата < СтрокаТаблицы.ДатаСвертки Тогда
					...
					ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзловДляПередачи[Номер], НаборЗаписей);
					//)Семенов И.П.
				КонецЕсли;
...
Процедура ДополнитьМассивУзловДляВыгрузкиСправочника(Элемент, МассивУзловДляПередачи, 
	...
	Если Элемент.Предопределенный 
	   ...
	   И НЕ (ТипЭлемента = Тип("СправочникОбъект.ВидыКонтактнойИнформацииФИАС") Или ТипЭлемента = Тип("СправочникСсылка.ВидыКонтактнойИнформацииФИАС"))Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникОбъект.ВидыКонтактнойИнформацииФИАС")
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.ВидыКонтактнойИнформацииФИАС") Тогда
		
		МенеджерСправочника = Справочники.ВидыКонтактнойИнформацииФИАС;
				...
				Если ТипЭлемента = Тип("РегистрСведенийНаборЗаписей.ЗначенияСвойствОбъектов") Тогда 
					...
					Для Каждого Запись Из Элемент Цикл
						...
						Если ТипЗнч(ЗаписьЗначение) <> Тип("Булево") 
							...
							И ТипЗнч(ЗаписьЗначение) <> Тип("СправочникСсылка.СтраныМира")
...
Процедура ЗарегистрироватьИзмененияПериодаВыгрузкиСебестоимости(ТаблицаПериодов, Узел)
	...
	Для Каждого СтрокаПериод Из ТаблицаПериодов Цикл
		...
		ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Узел, НаборЗаписей);
		//)Семенов И.П.
	КонецЦикла;
