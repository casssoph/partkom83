...
//
Процедура ПоказатьРезультатВыполнения(ЭтаФорма, Результат, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Сценарий.Вставить("Форма", ЭтаФорма);
...
Процедура РазвернутьУзлыДерева(Форма, РазворачиваемыйУзел) Экспорт
	ТаблицаЭлемент = Форма.Элементы[СокрЛП(РазворачиваемыйУзел.ИмяТаблицы)];
	Если РазворачиваемыйУзел.Идентификатор = "*" Тогда
		Узлы = Форма[СокрЛП(РазворачиваемыйУзел.ИмяТаблицы)].ПолучитьЭлементы();
