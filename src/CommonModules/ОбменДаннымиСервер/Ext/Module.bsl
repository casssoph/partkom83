...
Процедура ВыполнитьСтандартнуюВыгрузкуИзмененийДляУзла(Отказ,
	...
	Если ИспользоватьТранзакции Тогда
		НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый); // ЛНА Борьба с блокировками
	КонецЕсли;
...
Процедура ЗарегистрироватьДанныеПоДатеНачалаВыгрузкиИОрганизациям(Знач Получатель, ДатаНачалаВыгрузки,
	...
	Если Не ОтборПоОрганизациям И Не ОтборПоДатеНачалаВыгрузки Тогда
		...
		Если ТипЗнч(Данные) = Тип("Массив") Тогда
			...
			Для Каждого ОбъектМетаданных Из Данные Цикл
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Получатель, ОбъектМетаданных);
				//)Семенов И.П.
			КонецЦикла;
		...
		Иначе
			...
			ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Получатель, Данные);
			//)Семенов И.П.
		КонецЕсли;
	...
	Для Каждого ЭлементСоставаПланаОбмена Из СоставПланаОбмена Цикл
			...
			Если ОбщегоНазначения.ЭтоДокумент(ЭлементСоставаПланаОбмена.Метаданные) Тогда // Документы
				...
				Если ОтборПоДатеНачалаВыгрузкиИОрганизациям
					...
					Пока Выборка.Следующий() Цикл
						...
						ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Получатель, Выборка.Ссылка);
						//)Семенов И.П.
					КонецЦикла;
				...
				Если ОбщегоНазначения.ЭтоРегистрСведений(ЭлементСоставаПланаОбмена.Метаданные)
					...
					Если Выборка <> Неопределено Тогда
						...
						Пока Выборка.Следующий() Цикл
							...
							ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Получатель, НаборЗаписей);
							//)Семенов И.П.
						КонецЦикла;
		...
		ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Получатель, ЭлементСоставаПланаОбмена.Метаданные);
		//)Семенов И.П.
	КонецЦикла;
	...
	Если СтандартнаяОбработка Тогда
		...
		Если ТипЗнч(Данные) = Тип("Массив") Тогда
			...
			Для Каждого ОбъектМетаданных Из Данные Цикл
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(УзелИнформационнойБазы, ОбъектМетаданных);
				//)Семенов И.П.
			КонецЦикла;
		...
		Иначе
			...
			ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(УзелИнформационнойБазы, Данные);
			//)Семенов И.П.
		КонецЕсли;
...
//PaSe
Функция РаспаковатьСообщениеОбмена(СообщениеОбмена) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбменДаннымиСервер_РаспаковатьСообщениеОбмена";
	...
	Сообщение = ОбменДаннымиВызовСервера.РаспаковатьСообщениеОбменаССайтом(СообщениеОбмена);
	
	
	Если Прав(Сообщение, 1) = Символ(0) Тогда
		Сообщение = Лев(Сообщение, СтрДлина(Сообщение)-1);
	КонецЕсли;
	
	если ЭтоАктивнаяЗадачаJirа(ПредопределенноеЗначение("Справочник.ЗадачиJira.XX2733")) тогда 
		//#XX-2733 Kalinin V.A. ( 2019-07-08 )  /*
		Возврат ПолучитьОтформатированноеСообщениеXML(Сообщение)
	иначе 	
		Возврат Сообщение;
	КонецЕсли;
...
Функция ПолучитьОтформатированноеСообщениеXML(ИсходноеСообщение) Экспорт
	лКлючАлгоритма = "ОбщийМодуль_ОбменДаннымиСервер_ПолучитьОтформатированноеСообщениеXML";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	ТекстПреобразованияXSL = ПолучитьОбщийМакет("ФорматированиеXML").ПолучитьТекст();
	Преобразование.ЗагрузитьИзСтроки(ТекстПреобразованияXSL);
	Возврат Преобразование.ПреобразоватьИзСтроки(ИсходноеСообщение);	
...
Функция РазобратьСтруктуруJSON(ЧтениеJSON) Экспорт
	...
	Пока ЧтениеJSON.ТипТекущегоЗначения <> ТипЗначенияJSON.КонецОбъекта Цикл
		...
		ИначеЕсли ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.Число Тогда
			Значение = Число(ЧтениеJSON.ТекущееЗначение);
		ИначеЕсли ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.НачалоМассива Тогда
			...
			Пока ЧтениеJSON.ТипТекущегоЗначения <> ТипЗначенияJSON.КонецМассива Цикл
				Значение.Добавить(РазобратьСтруктуруJSON(ЧтениеJSON));
				ЧтениеJSON.Прочитать();
			КонецЦикла;
		ИначеЕсли ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.НачалоОбъекта Тогда
			Значение = РазобратьСтруктуруJSON(ЧтениеJSON);
		ИначеЕсли ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.Булево Тогда
			Значение = ЧтениеJSON.ТекущееЗначение;
		ИначеЕсли ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.Null Тогда
			Значение = NULL;
		Иначе
			Значение = "Кривое свойство";
		КонецЕсли;
