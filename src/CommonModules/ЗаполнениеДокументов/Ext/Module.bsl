...
Процедура ЗаполнитьШапкуДокументаПоОснованию(ДокументОбъект, ДокументОснование) Экспорт
	...
	// Филиал.
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("Филиал", МетаданныеДокумента)
	   И ОбщегоНазначения.ЕстьРеквизитДокумента("Филиал", МетаданныеДокументаОснования) Тогда
		ДокументОбъект.Филиал = ДокументОснование.Филиал;
	КонецЕсли;
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, ТипОперации = "", ПараметрОбъектКопирования = Неопределено, ПараметрОснование = неопределено, СтруктураПараметровДляПолученияДоговора = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("Организация", МетаданныеДокумента)
	   ...
	   //{{ХудинВВ 20180620 ХХ-185
	   СчетОрганизации = Неопределено;
	   Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДоговорКонтрагента", МетаданныеДокумента)
		   И ЗначениеЗаполнено(ДокументОбъект.ДоговорКонтрагента) Тогда
		   СчетОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.ДоговорКонтрагента, "БанковскийСчет");
	   КонецЕсли;
	   Если Не ЗначениеЗаполнено(СчетОрганизации) Тогда
		   СчетОрганизации = ДокументОбъект.Организация.ОсновнойБанковскийСчет;
	   КонецЕсли;
	   //}}
		...
		Если ОбщегоНазначения.ЕстьРеквизитДокумента("БанковскийСчетОрганизации", МетаданныеДокумента) Тогда
			Если НЕ ЗначениеЗаполнено(ДокументОбъект.БанковскийСчетОрганизации) Тогда
				ДокументОбъект.БанковскийСчетОрганизации = СчетОрганизации;
		...
		Если ОбщегоНазначения.ЕстьРеквизитДокумента("СтруктурнаяЕдиница", МетаданныеДокумента) Тогда
			Если НЕ ЗначениеЗаполнено(ДокументОбъект.СтруктурнаяЕдиница) Тогда
				ДокументОбъект.СтруктурнаяЕдиница = СчетОрганизации;
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("Контрагент", МетаданныеДокумента) Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДоговорКонтрагента", МетаданныеДокумента) Тогда
			Если НЕ ЗначениеЗаполнено(ДокументОбъект.ДоговорКонтрагента) 
			   ...
			   //ХудинВВ 25062018 По-умолчанию берем договор из регистра
			   Если Справочники.ДоговорыКонтрагентов.ИспользоватьРегистрОсновныеДоговорыКонтрагентов() Тогда
				   ...
				   ИначеЕсли Нрег(ТипОперации) = "ответхранение" Тогда
				   ...
				   ДокументОбъект.ДоговорКонтрагента = РегистрыСведений.ОсновныеДоговорыКонтрагентов.ПолучитьДоговор(Организация, ДокументОбъект.Контрагент, ВидДоговора);
			...
			Если ЗначениеЗаполнено(ДокументОбъект.ДоговорКонтрагента) Тогда
				...
				Если ОбщегоНазначения.ЕстьРеквизитДокумента("Сделка", МетаданныеДокумента) Тогда
					...
					Если ДокументОбъект.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком ИЛИ
						...
						ЗаявкаПокупателя = Ложь;
					КонецЕсли;
					ЗаполнениеДокументов.УстановитьТипСделки(ДокументОбъект, ЗаявкаПокупателя);
				КонецЕсли;
...
//
Процедура УстановитьТипСделки(ДокументОбъект, ЗаявкаПокупателя = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Если ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом
		 Или ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам Тогда
			...
			Сделка = Документы["Заказ"+?(ЗаявкаПокупателя, "Покупателя", "Поставщику")].ПустаяСсылка()
		...
		ИначеЕсли ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам тогда
			...
			Сделка = Документы["СчетНаОплату"+?(ЗаявкаПокупателя, "Покупателю", "Поставщика")].ПустаяСсылка()
...
Процедура РедактированиеАдресаДоставки(АдресДоставкиКонтрагента, ПолеВводаАдреса, ДокументОбъект) Экспорт
	...
	УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПрочитать(СтруктураАдреса, Получатель);
	...
	//Запрос.УстановитьПараметр("АдресДоставки", СокрЛП(СтруктураАдреса.Представление));
	//Вид = Неопределено;
	//Выборка = Запрос.Выполнить().Выбрать();
	//Если Выборка.Следующий() Тогда
	//	Вид = Выборка.Вид;
	...
	ПараметрыОткрытия.Вставить("ОбъектКонтрагент", Получатель);
	ПараметрыОткрытия.Вставить("ВидКонтактнойИнформации", СтруктураАдреса.Вид);
	ПараметрыОткрытия.Вставить("ЗначенияПолей", СтруктураАдреса.ЗначенияПолей);
	ПараметрыОткрытия.Вставить("Представление", СтруктураАдреса.Представление);
	ПараметрыОткрытия.Вставить("КонтактнаяИнформацияОписаниеДополнительныхРеквизитов", Неопределено);
		
	УправлениеКонтактнойИнформациейКлиент.ОткрытьФормуКонтактнойИнформации(ПараметрыОткрытия, ДокументОбъект);
...
Процедура ПриИзмененииЗначенияДоговора(ДокументОбъект, СтараяВалютаВзаиморасчетов, ВалютаРегламентированногоУчета,
	...
	Если МетаданныеДокумента.Реквизиты.Найти("Сделка") <> Неопределено Тогда
		Если ЗначениеЗаполнено(ДокументОбъект.Сделка) И 
			(ТипЗнч(ДокументОбъект.Сделка)=Тип("ДокументСсылка.ЗаявкаПокупателя") ИЛИ
			ТипЗнч(ДокументОбъект.Сделка)=Тип("ДокументСсылка.ЗаказПоставщику")) Тогда
...
//Добавлено Валиахметов А.А. 28.02.2018
Процедура ЗаполнитьКлючиСвязи(ДокументОбъект) Экспорт  
	
	Если ДокументОбъект.Товары.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	ВремТаблица = ДокументОбъект.Товары.Выгрузить(, "КлючСвязи");
	ВремТаблица.Колонки.Добавить("Счетчик", ОбщегоНазначения.ОписаниеТипаЧисло(10,0));
	ВремТаблица.ЗаполнитьЗначения(1, "Счетчик");
	ВремТаблица.Свернуть("КлючСвязи", "Счетчик");
	ВремТаблица.Сортировать("Счетчик Убыв");
	
	Если ВремТаблица.Получить(0).Счетчик = 1 Тогда 
		Возврат; //Все ключи связи уникальны, не требуется заполнение
	КонецЕсли;
	
	Если ДокументОбъект.ПричиныОтказов.Количество() > 0 Тогда 
		//ВызватьИсключение "В табличной части ""Причины отказов"" есть строки, но в табличной части ""Товары"" есть неуникальные ключи"; 
		Возврат;
	КонецЕсли;
	
	ВремТаблица.Сортировать("КлючСвязи Убыв");

	мКлючСвязи = ВремТаблица.Получить(0).КлючСвязи;
	Если мКлючСвязи = 0 Тогда 
		мКлючСвязи = 1;
	КонецЕсли;
	
	Для Каждого СтрокаТЧ Из ДокументОбъект.Товары Цикл 
		Если СтрокаТЧ.КлючСвязи = 0 Тогда 
			СтрокаТЧ.КлючСвязи = мКлючСвязи;
...
//Конец Добавлено Валиахметов А.А. 28.02.2018
