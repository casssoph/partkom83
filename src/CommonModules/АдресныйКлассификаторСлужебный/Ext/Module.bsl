...
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Идентификатор) Экспорт
	...
	Исключение
		Адрес = ФабрикаXDTO.Создать( ФабрикаXDTO.Тип("http://www.v8.1c.ru/ssl/contactinfo", "АдресРФ") );
	КонецПопытки;
...
Процедура УстановитьИдентификаторыНаселенногоПункта(ЧастиАдреса, ИдентификаторНаселенногоПункта) Экспорт
	...
	ПоляАдреса.Вставить("Регион", ЧастиАдреса.Регион);
	ПоляАдреса.Вставить("Округ", ЧастиАдреса.Округ);
	ПоляАдреса.Вставить("Район", ЧастиАдреса.Район);
	ПоляАдреса.Вставить("Город", ЧастиАдреса.Город);
	ПоляАдреса.Вставить("ВнутригородскойРайон", ЧастиАдреса.ВнутригРайон);
	ПоляАдреса.Вставить("НаселенныйПункт", ЧастиАдреса.НаселенныйПункт);
	ПоляАдреса.Вставить("Улица", ЧастиАдреса.Улица);
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		ЗаписьЖурналаРегистрации("Фоновое задание.Сообщение пользователю",УровеньЖурналаРегистрации.Информация, , ,"начало загрузки АК по региону " + Строка(СубъектРФ));
		#КонецЕсли
		...
		Попытка
			...
			ИнфоОВерсии = ИнформацияОЗагрузкеВерсии(СубъектРФ);
			ВерсияСубъектаРФ = ИнфоОВерсии.Версия;
			Если ВерсияСубъектаРФ <> СлужебныеАдресныеСведения.Версия Тогда
				...
				#Если НЕ Клиент Тогда
				ЗаписьЖурналаРегистрации("Фоновое задание.Сообщение пользователю",УровеньЖурналаРегистрации.Информация, , ,"данные по региону " + Строка(СубъектРФ) + " были обновлены" + Символ(13) + "текущая версия ФИАС - " + Строка(ВерсияСубъектаРФ) + " от " + Строка(ИнфоОВерсии.ДатаВерсии));
				#КонецЕсли
			Иначе
				#Если НЕ Клиент Тогда
				ЗаписьЖурналаРегистрации("Фоновое задание.Сообщение пользователю",УровеньЖурналаРегистрации.Информация, , ,"данные по региону " + Строка(СубъектРФ) + " не требуют обновления" + Символ(13) + "текущая версия ФИАС - " + Строка(ВерсияСубъектаРФ) + " от " + Строка(ИнфоОВерсии.ДатаВерсии));
				#КонецЕсли
			КонецЕсли;
		...
		#Если НЕ Клиент Тогда
		ЗаписьЖурналаРегистрации("Фоновое задание.Сообщение пользователю", УровеньЖурналаРегистрации.Информация, , , "окончание загрузки АК по региону " + Строка(СубъектРФ) + Символ(13) + "текущая версия ФИАС - " + Строка(ВерсияСубъектаРФ) + " от " + Строка(ИнфоОВерсии.ДатаВерсии));
		#КонецЕсли
	КонецЦикла;
	...
	//#Если НЕ Клиент Тогда
	//ЗаписьЖурналаРегистрации("окончание загрузки АК", УровеньЖурналаРегистрации.Информация);
	//#КонецЕсли
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Новый ФорматированнаяСтрока(СтрокаПоиска, Новый Шрифт(,, Истина) , ЦветаСтиля.ТекстИнформационнойНадписи),
...
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено)
	...
	Авторизация = Новый Структура("Логин,Пароль","10089542","rfc123$qwe");
	...
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		//ЗаписьЖурналаРегистрации("начало загрузки АК по региону " + КодСубъектаРФ.Наименование,УровеньЖурналаРегистрации.Информация);
