...
Процедура ПрочитатьСтатусы() Экспорт
	...
	Пока ВыборкаProviderID.Следующий() Цикл
		...
		ПоследнийTimestamp = ПолучитьTimeStamp(ВыборкаProviderID.Код);
		...
		ЗафиксироватьTimeStamp(НовыйTimestamp, ВыборкаProviderID.Код);
...
Функция ПолучитьКонтрагентаEMEX() Экспорт
