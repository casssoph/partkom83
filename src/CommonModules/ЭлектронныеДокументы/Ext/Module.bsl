...
Процедура ПриИзмененииРеализацииВДокументе(Документ) Экспорт
	...
	Если Не ЗначениеЗаполнено(Реализация)
		...
		ИЛИ ТипЗнч(Реализация.Сделка) <> Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
...
//**********************************************************************************
//
Функция ЗагрузитьПрайсПоставщикаИзОП(вхПоставщик, ДатаЗагрузкиПрайсаПоставщиком = Неопределено, вхТабТоваров) Экспорт
	лКлючАлгоритма = "ОбщийМодуль_ЭлектронныеДокументы_ЗагрузитьПрайсПоставщикаИзОП";
	...
	ДокументСоздан = Ложь;
	ТекДокумент = Документы.ПереоценкаОстатковПоставщика.ПустаяСсылка();
	ДатаДок = ?(ЗначениеЗаполнено(ДатаЗагрузкиПрайсаПоставщиком), КонецДня(ДатаЗагрузкиПрайсаПоставщиком), КонецДня(ТекущаяДата()));
	
	Если НЕ ЗначениеЗаполнено(вхПоставщик) ИЛИ ТипЗнч(вхПоставщик) <> Тип("СправочникСсылка.Контрагенты") Тогда
		ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС) + "Неверно передан контрагент";
		Возврат Новый Структура("Результат,ОписаниеОшибкиСистема,ОписаниеОшибкиДляКлиента,СсылкаНаДокумент", ДокументСоздан, ОписаниеОшибкиСистема, ОписаниеОшибкиДляКлиента, ТекДокумент.Ссылка);
	КонецЕсли;
	
	Если вхТабТоваров.Количество() = 0 Тогда
		ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС) + "Передана пустая таблица товаров";
		Возврат Новый Структура("Результат,ОписаниеОшибкиСистема,ОписаниеОшибкиДляКлиента,СсылкаНаДокумент", ДокументСоздан, ОписаниеОшибкиСистема, ОписаниеОшибкиДляКлиента, ТекДокумент.Ссылка);
	КонецЕсли;
	
	ОшибочныеАртикулы = "";
	Инд = 1;
	ЗагружатьПоКодамПоставщика = Ложь;
	КоличествоЗаполненныхКодовПоставщика = 0;
	
	Для Каждого Товар Из вхТабТоваров Цикл
		Если ПустаяСтрока(Товар.ИзготовительПоставщика) Тогда
			ОшибочныеАртикулы = ?(ПустаяСтрока(ОшибочныеАртикулы), ОшибочныеАртикулы, ОшибочныеАртикулы + Символы.ПС + Символы.Таб);
			ОшибочныеАртикулы = ОшибочныеАртикулы + "в строке " + Строка(Инд) + Товар.ИзготовительПоставщика;
			ОшибочныеАртикулы = ОшибочныеАртикулы + " цена (руб.): " + Строка(Товар.Цена);
			ОшибочныеАртикулы = ОшибочныеАртикулы + "в строке " + Строка(Инд) + " не заполнен изготовитель";
		КонецЕсли;
		Если ПустаяСтрока(Товар.АртикулПоставщика) И ПустаяСтрока(Товар.КодПоставщика) Тогда
			ОшибочныеАртикулы = ?(ПустаяСтрока(ОшибочныеАртикулы), ОшибочныеАртикулы, ОшибочныеАртикулы + Символы.ПС  + Символы.Таб);
			ОшибочныеАртикулы = ОшибочныеАртикулы + "в строке " + Строка(Инд) +": " + Товар.ИзготовительПоставщика;
			ОшибочныеАртикулы = ОшибочныеАртикулы + " цена (руб.): " + Строка(Товар.Цена);
			ОшибочныеАртикулы = ОшибочныеАртикулы +  " не заполнен артикул или код поставщика";
		КонецЕсли;
		Если НЕ ПустаяСтрока(Товар.КодПоставщика) Тогда
			КоличествоЗаполненныхКодовПоставщика = КоличествоЗаполненныхКодовПоставщика + 1;
		КонецЕсли;
	...
	Если КоличествоЗаполненныхКодовПоставщика >= Окр(вхТабТоваров.Количество() / 2, 0) Тогда
		ЗагружатьПоКодамПоставщика = Истина;
	КонецЕсли;
	
	ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС) + ОшибочныеАртикулы;
	ОписаниеОшибкиДляКлиента = ?(ПустаяСтрока(ОписаниеОшибкиДляКлиента), ОписаниеОшибкиДляКлиента, ОписаниеОшибкиДляКлиента + Символы.ПС) + ОшибочныеАртикулы;
	...
	|	И НЕ Д.ДоговорПриостановлен
	|	И НЕ Д.СлужебныйДоговор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПрайсыПоставщиковVMI.Ссылка КАК Прайс,
	|	ЕСТЬNULL(ПереоценкаОстатковПоставщика.Ссылка, ЗНАЧЕНИЕ(Документ.ПереоценкаОстатковПоставщика.ПустаяСсылка)) КАК Документ
	|ИЗ
	|	Справочник.ПрайсыПоставщиковVMI КАК ПрайсыПоставщиковVMI
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПереоценкаОстатковПоставщика КАК ПереоценкаОстатковПоставщика
	|		ПО ПрайсыПоставщиковVMI.Ссылка = ПереоценкаОстатковПоставщика.ПрайсПоставщикаVMI
	|			И (ПереоценкаОстатковПоставщика.Дата = &Дата)
	|ГДЕ
	|	ПрайсыПоставщиковVMI.Владелец = &Контрагент
	|	И НЕ ПрайсыПоставщиковVMI.ПометкаУдаления"
	);
	Запрос.УстановитьПараметр("Дата", ДатаДок);
	Запрос.УстановитьПараметр("Контрагент", вхПоставщик);
	Результат = Запрос.ВыполнитьПакет();
	
	ВыборкаПоДоговорам = Результат[0].Выбрать();
	Если ВыборкаПоДоговорам.Следующий() Тогда
		тДоговор = ВыборкаПоДоговорам.Договор;
		тОрганизация = ВыборкаПоДоговорам.Организация;
	Иначе
		ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС);
		ОписаниеОшибкиСистема = "Не найден договор VMI;" + Символы.ПС + ОписаниеОшибкиСистема;
		тхтОш = "не найден действующий договор vmi";
		ОписаниеОшибкиДляКлиента = ?(ПустаяСтрока(ОписаниеОшибкиДляКлиента), тхтОш, тхтОш + Символы.ПС + ОписаниеОшибкиДляКлиента);
		Возврат Новый Структура("Результат,ОписаниеОшибкиСистема,ОписаниеОшибкиДляКлиента,СсылкаНаДокумент", ДокументСоздан, ОписаниеОшибкиСистема, ОписаниеОшибкиДляКлиента, ТекДокумент.Ссылка);
	КонецЕсли;
	
	ВыборкаПоПрайсам = Результат[1].Выбрать();
	Если ВыборкаПоПрайсам.Следующий() Тогда
		Прайс = ВыборкаПоПрайсам.Прайс;
	Иначе
		НовПрайс = Справочники.ПрайсыПоставщиковVMI.СоздатьЭлемент();
		НовПрайс.Владелец = вхПоставщик;
		НовПрайс.Наименование = СокрЛП(вхПоставщик.Код) + "_" + СокрЛП(вхПоставщик.Наименование);
		НовПрайс.Записать();
		Прайс = НовПрайс.Ссылка;
	КонецЕсли;
	
	ПроцентОтклоненияПоПрайсу = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Прайс, "ПроцентОтклонения");
	
	ВыборкаПоДокументам = Результат[1].Выбрать();
	Если ВыборкаПоДокументам.Следующий() И НЕ ВыборкаПоДокументам.Документ.Пустая() 
		и ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаПоДокументам.Документ,"ПометкаУдаления") = ложь Тогда //#XX-2288 Kalinin V.A. ( 2019-07-09 )  /*
		ТекДокумент = ВыборкаПоДокументам.Документ.ПолучитьОбъект();
	Иначе
		ТекДокумент = Документы.ПереоценкаОстатковПоставщика.СоздатьДокумент();
		ТекДокумент.Дата = ДатаДок;
		ТекДокумент.ВалютаДокумента = Константы.ВалютаРубль.Получить();
		ТекДокумент.КурсДокумента = 1;
		ТекДокумент.КратностьДокумента = 1;
		ТекДокумент.Контрагент = вхПоставщик;
		ТекДокумент.ПрайсПоставщикаVMI = Прайс;
		ТекДокумент.Организация = тОрганизация;
		ТекДокумент.ДоговорКонтрагента = тДоговор;
		ТекДокумент.Ответственный = ПараметрыСеанса.ТекущийПользователь;
		
	КонецЕсли;
	
	ТекДокумент.ОшибкиПриЗагрузке = ОшибочныеАртикулы;
	
	ТекДокумент.ТоварыПоставщика.Очистить();
	//20181001 при загрузке из ОП с сайта приходит дифф
	//дописываем новые строки в документ на случай повторной загрузки поставщиком того же прайса
	//20190304 загрузка дифф отключена по задаче XX-1995
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТабТоваров.АртикулПоставщика,
	|	ТабТоваров.ИзготовительПоставщика,
	|	ТабТоваров.Артикул,
	|	ТабТоваров.Изготовитель КАК ИзготовительСтрока,
	|	ТабТоваров.МинимальныйОстаток,
	|	ТабТоваров.КодПоставщика,
	|	ТабТоваров.Цена
	|ПОМЕСТИТЬ ТабТоваров0
	|ИЗ
	...
	|	ТабТоваров.ИзготовительПоставщика,
	|	ТабТоваров.Артикул,
	|	ЕСТЬNULL(Спр.Ссылка, ЗНАЧЕНИЕ(Справочник.Изготовители.ПустаяСсылка)) КАК Изготовитель,
	|	ТабТоваров.МинимальныйОстаток,
	|	ТабТоваров.КодПоставщика,
	|	ТабТоваров.Цена
	|ПОМЕСТИТЬ ТабТоваров
	|ИЗ
	|	ТабТоваров0 КАК ТабТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Изготовители КАК Спр
	|		ПО ТабТоваров.ИзготовительСтрока = Спр.Наименование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	С.Артикул,
	|	С.Изготовитель,
	|	С.Ссылка КАК НоменклатураПоставщика
	|ПОМЕСТИТЬ С
	|ИЗ
	|	Справочник.НоменклатураКонтрагентов КАК С
	|ГДЕ
	|	С.Владелец = &Контрагент
	|	И НЕ С.ПометкаУдаления" + ?(ЗагружатьПоКодамПоставщика, "
	|	И (С.КодПоставщика, С.Изготовитель) В
	|			(ВЫБРАТЬ
	|				ТабТоваров.КодПоставщика,
	|				ТабТоваров.Изготовитель
	|			ИЗ
	|				ТабТоваров КАК ТабТоваров)", "
	|	И (С.Артикул, С.Изготовитель) В
	|			(ВЫБРАТЬ
	|				ТабТоваров.Артикул,
	|				ТабТоваров.Изготовитель
	|			ИЗ
	...
	|	Т.Артикул,
	|	Т.Изготовитель,
	|	Т.МинимальныйОстаток,
	|	Т.КодПоставщика,
	|	Т.Цена,
	|	ЕСТЬNULL(С.НоменклатураПоставщика, ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка)) КАК НоменклатураПоставщика
	|ПОМЕСТИТЬ Т0
	|ИЗ
	|	ТабТоваров КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ С КАК С
	|		ПО Т.Артикул = С.Артикул
	|			И Т.Изготовитель = С.Изготовитель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НП.Номенклатура КАК НоменклатураПоставщика,
	|	СРЕДНЕЕ(НП.Цена) КАК ЦенаСтарая
	|ПОМЕСТИТЬ Ц
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(
	|			&ДатаДокумента,
	|			ПрайсПоставщика = &ПрайсПоставщика
	|				И Номенклатура В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						Т0.НоменклатураПоставщика
	|					ИЗ
	|						Т0 КАК Т0
	|					ГДЕ
	|						Т0.НоменклатураПоставщика <> ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка))) КАК НП
	|
	|СГРУППИРОВАТЬ ПО
	|	НП.Номенклатура
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Т0.АртикулПоставщика,
	|	Т0.ИзготовительПоставщика,
	|	Т0.Артикул,
	|	Т0.НоменклатураПоставщика КАК НоменклатураПоставщика,
	|	Т0.НоменклатураПоставщика.Номенклатура КАК НПНоменклатура,
	|	Т0.НоменклатураПоставщика.Изготовитель КАК НПИзготовитель,
	|	Т0.Изготовитель,
	|	Т0.МинимальныйОстаток,
	|	Т0.КодПоставщика,
	|	Т0.НоменклатураПоставщика.МинКолПартии КАК НПМинимальныйОстаток,
	|	Т0.Цена,
	|	ЕСТЬNULL(Ц.ЦенаСтарая, 0) КАК ЦенаСтарая,
	|	ЕСТЬNULL(Номенклатура.Ссылка, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Ц.ЦенаСтарая, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ (Т0.Цена - ЕСТЬNULL(Ц.ЦенаСтарая, 0)) / ЕСТЬNULL(Ц.ЦенаСтарая, 0) * 100
	|	КОНЕЦ КАК ПроцентОтклонения
	|ИЗ
	|	Т0 КАК Т0
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатура
	|		ПО Т0.Артикул = Номенклатура.Артикул
	|			И Т0.Изготовитель = Номенклатура.Изготовитель
	|		ЛЕВОЕ СОЕДИНЕНИЕ Ц КАК Ц
	|		ПО Т0.НоменклатураПоставщика = Ц.НоменклатураПоставщика
	|ГДЕ
	|	Т0.Цена <> ЕСТЬNULL(Ц.ЦенаСтарая, 0)";
	Запрос.УстановитьПараметр("Контрагент", вхПоставщик);
	Запрос.УстановитьПараметр("ДатаДокумента", ДатаДок);
	Запрос.УстановитьПараметр("ПрайсПоставщика", Прайс);
	
	КоличествоСтрокПрайса = вхТабТоваров.Количество();
	ОграниченнаяТаблицаТоваров = вхТабТоваров.СкопироватьКолонки();
	
	ИндексСтроки = 0;
	ОграничениеВыборки = 1000;
	Пока ИндексСтроки < КоличествоСтрокПрайса Цикл
		
		//Обработка табличной части частями//
		СчетчикВыборки = ОграничениеВыборки;
		ОграниченнаяТаблицаТоваров.Очистить();
		Пока СчетчикВыборки > 0 И ИндексСтроки < КоличествоСтрокПрайса Цикл
			ЗаполнитьЗначенияСвойств(ОграниченнаяТаблицаТоваров.Добавить(), вхТабТоваров[ИндексСтроки]);
			СчетчикВыборки = СчетчикВыборки - 1;
			ИндексСтроки = ИндексСтроки + 1;
		КонецЦикла;
		
		Запрос.УстановитьПараметр("ТабТоваров", ОграниченнаяТаблицаТоваров);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ПараметрОтбора = Новый Структура("НоменклатураПоставщика", Выборка.НоменклатураПоставщика);
			НайденныеСтроки = ТекДокумент.ТоварыПоставщика.НайтиСтроки(ПараметрОтбора);
			Если НайденныеСтроки.Количество() = 0 Тогда
				НоваяСтрока = ТекДокумент.ТоварыПоставщика.Добавить();
			Иначе
				НоваяСтрока = НайденныеСтроки[0];
				
			КонецЕсли;
					
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка,,"НоменклатураПоставщика");
			СтруктураНП = НоменклатураПоставщика(Выборка, вхПоставщик);
			НоваяСтрока.НоменклатураПоставщика = СтруктураНП.НоменклатураПоставщика;
			Если СтруктураНП.НеНайденИзготовитель
				ИЛИ ПустаяСтрока(НоваяСтрока.АртикулПоставщика)
				ИЛИ ПустаяСтрока(НоваяСтрока.ИзготовительПоставщика)
				ИЛИ НоваяСтрока.Цена = 0 Тогда
				НоваяСтрока.НеПереоценивать = Истина;
			КонецЕсли;
	...
	Если ПроцентОтклоненияПоПрайсу > 0 Тогда
		ПроцентОтклоненияПлюс = ПроцентОтклоненияПоПрайсу;
		Если ПроцентОтклоненияПлюс > 100 Тогда
			ПроцентОтклоненияМинус = -(ПроцентОтклоненияПлюс - 100);
		Иначе
			ПроцентОтклоненияМинус = -ПроцентОтклоненияПлюс;
		КонецЕсли;
		
		Для Каждого Товар Из ТекДокумент.ТоварыПоставщика Цикл
			Если Товар.ПроцентОтклонения > ПроцентОтклоненияПлюс ИЛИ Товар.ПроцентОтклонения < ПроцентОтклоненияМинус Тогда
				Товар.НеПереоценивать = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
	ИначеЕсли ПроцентОтклоненияПоПрайсу = 0 Тогда
		ПроцентОтклоненияПлюс = 150;
		ПроцентОтклоненияПлюс = -50;
	...
	Попытка
		...
		ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС) + "Документ записан";
	Исключение
		ОписаниеОшибкиСистема = ?(ПустаяСтрока(ОписаниеОшибкиСистема), ОписаниеОшибкиСистема, ОписаниеОшибкиСистема + Символы.ПС) + "Документ создан, но не сохранен" + Символы.ПС + ОписаниеОшибки();
	КонецПопытки;
...
Функция НоменклатураПоставщика(ДанныеНоменклатуры, Поставщик)
	
	НоменклатураПоставщика = ДанныеНоменклатуры.НоменклатураПоставщика;
	НеНайденИзготовитель = Ложь;

	Если НЕ ЗначениеЗаполнено(НоменклатураПоставщика) Тогда
		НоменклатураОбновлена = Ложь;
		Если НЕ ПустаяСтрока(ДанныеНоменклатуры.КодПоставщика) Тогда
			//пробуем найти по артикул постащика = код поставщика
			Запрос = Новый Запрос(
			"ВЫБРАТЬ
			...
			|ГДЕ
			|	НоменклатураКонтрагентов.АртикулПоставщика = &АртикулПоставщика
			...
			Запрос.УстановитьПараметр("АртикулПоставщика", ДанныеНоменклатуры.АртикулПоставщика);
			Запрос.УстановитьПараметр("ИзготовительПоставщика", ДанныеНоменклатуры.ИзготовительПоставщика);
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				НоменклатураПоставщика = Выборка.Ссылка;
				Об = Выборка.Ссылка.ПолучитьОбъект();
				Об.КодПоставщика = ДанныеНоменклатуры.КодПоставщика;
				Об.Артикул = ДанныеНоменклатуры.Артикул;
				Об.Изготовитель = ДанныеНоменклатуры.Изготовитель;
				Об.Номенклатура = ДанныеНоменклатуры.Номенклатура;
				Об.МинКолПартии = ДанныеНоменклатуры.МинимальныйОстаток;
				Об.ОбменДанными.Загрузка = Истина;
				Об.Записать();
				НоменклатураОбновлена = Истина;
				
				//Если (ДанныеНоменклатуры.НПНоменклатура <> ДанныеНоменклатуры.Номенклатура И ЗначениеЗаполнено(ДанныеНоменклатуры.Номенклатура))
				//	ИЛИ (ДанныеНоменклатуры.МинимальныйОстаток <> ДанныеНоменклатуры.НПМинимальныйОстаток) Тогда
				//	Об = ДанныеНоменклатуры.НоменклатураПоставщика.ПолучитьОбъект();
				//	Об.Артикул = ДанныеНоменклатуры.Артикул;
				//	Об.Изготовитель = ДанныеНоменклатуры.Изготовитель;
				//	Об.Номенклатура = ДанныеНоменклатуры.Номенклатура;
				//	Об.МинКолПартии = ДанныеНоменклатуры.МинимальныйОстаток;
			...
			КонецЕсли;
		КонецЕсли;
		Если НЕ НоменклатураОбновлена Тогда
			НоваяНоменклатура = Справочники.НоменклатураКонтрагентов.СоздатьЭлемент();
			НоваяНоменклатура.Владелец = Поставщик;
			НоваяНоменклатура.АртикулПоставщика = ДанныеНоменклатуры.АртикулПоставщика;
			НоваяНоменклатура.ИзготовительПоставщика = ДанныеНоменклатуры.ИзготовительПоставщика;
			НоваяНоменклатура.Артикул = ДанныеНоменклатуры.Артикул;
			НоваяНоменклатура.Изготовитель = ДанныеНоменклатуры.Изготовитель;
			НоваяНоменклатура.Номенклатура = ДанныеНоменклатуры.Номенклатура;
			НоваяНоменклатура.Наименование = ДанныеНоменклатуры.Артикул + "_" + ДанныеНоменклатуры.Изготовитель;
			НоваяНоменклатура.МинКолПартии = ДанныеНоменклатуры.МинимальныйОстаток;
			НоваяНоменклатура.КодПоставщика = ДанныеНоменклатуры.КодПоставщика;
			НоваяНоменклатура.Записать();
		
			НоменклатураПоставщика = НоваяНоменклатура.Ссылка;
	...
	Если НЕ ЗначениеЗаполнено(НоменклатураПоставщика.Изготовитель) 
		И НЕ ПустаяСтрока(НоменклатураПоставщика.ИзготовительПоставщика) Тогда
		НеНайденИзготовитель = Истина;
			
	КонецЕсли;

	Возврат Новый Структура("НоменклатураПоставщика,НеНайденИзготовитель", НоменклатураПоставщика, НеНайденИзготовитель);

КонецФункции

Функция ЗагрузитьПрайсПоставщикаИзОП_1(вхПоставщик, вхДатаЗагрузкиПрайсаПоставщиком = Неопределено, вхТабТоваров) Экспорт
	ОписаниеОшибки = "";
	ДокументСоздан = Ложь;
	ТекДокумент = Документы.ПереоценкаОстатковПоставщика.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(вхДатаЗагрузкиПрайсаПоставщиком) Тогда
		ДатаДок = КонецДня(вхДатаЗагрузкиПрайсаПоставщиком);
	Иначе
		ДатаДок = КонецДня(ТекущаяДата());
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(вхПоставщик)
		ИЛИ ТипЗнч(вхПоставщик) <> Тип("СправочникСсылка.Контрагенты") тогда
		ОписаниеОшибки = ОписаниеОшибки + "неверно передан контрагент;";
		Возврат Новый Структура("Результат,ОписаниеОшибки,СсылкаНаДокумент", ДокументСоздан, ОписаниеОшибки, ТекДокумент.Ссылка);
	КонецЕсли;
	...
	|ВЫБРАТЬ
	|	&Контрагент,
	|	ЕСТЬNULL(Д.Ссылка, ЗНАЧЕНИЕ(Документ.ПереоценкаОстатковПоставщика.ПустаяСсылка)) КАК ДокументПереоценки
	|ПОМЕСТИТЬ Д
	|ИЗ
	|	Документ.ПереоценкаОстатковПоставщика КАК Д
	|ГДЕ
	...
	|ВЫБРАТЬ
	|	П.Ссылка КАК Прайс,
	|	Д.ДокументПереоценки
	|ИЗ
	|	Справочник.ПрайсыПоставщиковVMI КАК П
	|		ЛЕВОЕ СОЕДИНЕНИЕ Д КАК Д
	|		ПО П.Владелец = Д.Контрагент
	|ГДЕ
	|	П.Владелец = &Контрагент"
	);
	...
	Если Р[0].Пустой() Тогда
		//нет действующего договора VMI
		ОписаниеОшибки = ОписаниеОшибки + "не найден договор;";
		Возврат Новый Структура("Результат,ОписаниеОшибки,СсылкаНаДокумент", ДокументСоздан, ОписаниеОшибки, ТекДокумент.Ссылка);
	Иначе
		Рез = Р[0].Выбрать();
		Пока Рез.Следующий() Цикл
			тДоговор = Рез.Договор;
			тОрганизация = Рез.Организация;
			
		КонецЦикла;
		
	КонецЕсли;
		
	Если Р[2].Пустой() Тогда
		НовПрайс = Справочники.ПрайсыПоставщиковVMI.СоздатьЭлемент();
		НовПрайс.Владелец = вхПоставщик;
		СтруктураПараметров = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(вхПоставщик, "Код,Наименование");
		НовПрайс.Наименование = СокрЛП(СтруктураПараметров.Код) + "_" + СокрЛП(СтруктураПараметров.Наименование);
		НовПрайс.Записать();
		...
		ТекДокумент.КратностьДокумента = 1;
		ТекДокумент.Контрагент = вхПоставщик;
		ТекДокумент.ПрайсПоставщикаVMI = НовПрайс.Ссылка;
		ТекДокумент.Организация = тОрганизация;
		ТекДокумент.ДоговорКонтрагента = тДоговор;
	...
	Иначе
		Выборка = Р[2].Выбрать();
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ДокументПереоценки) Тогда
				ТекДокумент = Выборка.ДокументПереоценки.ПолучитьОбъект();
				ТекДокумент.ТоварыПоставщика.Очистить();
			...
			Иначе
				...
				ТекДокумент.КратностьДокумента = 1;
				ТекДокумент.Контрагент = вхПоставщик;
				ТекДокумент.ПрайсПоставщикаVMI = Выборка.Прайс;
				ТекДокумент.Организация = тОрганизация;
				ТекДокумент.ДоговорКонтрагента = тДоговор;
	...
	вхТабТоваров.Колонки.Добавить("Обработано", Новый ОписаниеТипов("Булево"));
	...
	|ВЫБРАТЬ
	|	С.Артикул,
	|	С.Изготовитель.Наименование КАК Изготовитель,
	|	МАКСИМУМ(С.Ссылка) КАК НоменклатураПоставщика
	|ПОМЕСТИТЬ С
	|ИЗ
	|	Справочник.НоменклатураКонтрагентов КАК С
	|ГДЕ
	...
	|	Т0.НоменклатураПоставщика КАК НоменклатураПоставщика,
	|	Т0.НоменклатураПоставщика.Номенклатура КАК НПНоменклатура,
	|	Т0.НоменклатураПоставщика.Изготовитель КАК Изготовитель,
	|	Т0.МинимальныйОстаток,
	|	Т0.НоменклатураПоставщика.МинКолПартии КАК НПМинимальныйОстаток,
	|	Т0.Цена,
	|	ЕСТЬNULL(Ц.ЦенаСтарая, 0) КАК ЦенаСтарая,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатура
	|		ПО Т0.Артикул = Номенклатура.Артикул
	|			И Т0.НоменклатураПоставщика.Изготовитель = Номенклатура.Изготовитель
	|		ЛЕВОЕ СОЕДИНЕНИЕ Ц КАК Ц
	|		ПО Т0.НоменклатураПоставщика = Ц.НоменклатураПоставщика";
			
	Запрос.УстановитьПараметр("Контрагент", вхПоставщик);
	Запрос.УстановитьПараметр("ТабТоваров", вхТабТоваров);
	Запрос.УстановитьПараметр("ДатаДокумента", ДатаДок);
	
	Р = Запрос.Выполнить().Выгрузить();
	
	//ПроцентОтклоненияПлюс = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТекДокумент.ПрайсПоставщикаVMI, "ПроцентОтклонения");
	//Если ПроцентОтклоненияПлюс > 100 Тогда
	//	ПроцентОтклоненияМинус = ПроцентОтклоненияПлюс - 100;
	//	
	//Иначе
	...
	Для Каждого Товар Из Р Цикл
		Если НЕ ЗначениеЗаполнено(Товар.НоменклатураПоставщика) Тогда
			НовНП = Справочники.НоменклатураКонтрагентов.СоздатьЭлемент();
			НовНП.Владелец = вхПоставщик;
			НовНП.АртикулПоставщика = Товар.АртикулПоставщика;
			НовНП.ИзготовительПоставщика = Товар.ИзготовительПоставщика;
			НовНП.Артикул = Товар.Артикул;
			НовНП.Изготовитель = Товар.Изготовитель;
			НовНП.Номенклатура = Товар.Номенклатура;
			НовНП.Наименование = Товар.АртикулПоставщика + "_" + Товар.ИзготовительПоставщика;
			НовНП.МинКолПартии = Товар.МинимальныйОстаток;
			НовНП.Записать();
			Товар.НоменклатураПоставщика = НовНП.Ссылка;
			
		Иначе
			Если (Товар.НПНоменклатура <> Товар.Номенклатура И ЗначениеЗаполнено(Товар.Номенклатура))
				ИЛИ (Товар.МинимальныйОстаток <> Товар.НПМинимальныйОстаток) Тогда
				
				НПИзменена = Ложь;
				Об = Товар.НоменклатураПоставщика.ПолучитьОбъект();
				Если НЕ ПустаяСтрока(Товар.Артикул) Тогда
					Об.Артикул = Товар.Артикул;
					Об.Изготовитель = Товар.Изготовитель;
					Об.Номенклатура = Товар.Номенклатура;
					НПИзменена = Истина;
				КонецЕсли;
				
				Если Товар.МинимальныйОстаток <> Товар.НПМинимальныйОстаток Тогда
					Об.МинКолПартии = Товар.МинимальныйОстаток;
				КонецЕсли;
				
				Если НПИзменена Тогда
					Об.Записать();
				КонецЕсли;
		...
		//		Разница = Товар.Цена - Товар.ЦенаСтарая;
		//		Разница = ?(Разница < 0, -Разница, Разница);
		//		Товар.ПроцентОтклонения = Разница/Товар.Цена * 100;
		//		
		//	Иначе
		//		Товар.ПроцентОтклонения = 100;
		//		
		//	КонецЕсли;
		//	
		//ИначеЕсли Товар.ЦенаСтарая = 0 Тогда
		//	Товар.ПроцентОтклонения = 100;
		//	
		//КонецЕсли;
	...
	ТекДокумент.ТоварыПоставщика.Очистить();
	ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Р, ТекДокумент.ТоварыПоставщика);
		
	Попытка
		ТекДокумент.Записать(РежимЗаписиДокумента.Запись);
		ДокументСоздан = Истина;
		ОписаниеОшибки = ОписаниеОшибки + "документ записан";
	Исключение
		ОписаниеОшибки = ОписаниеОшибки + "документ создан, но не сохранен" + Символы.ПС + ОписаниеОшибки();
	КонецПопытки;
...
Функция ЗагрузитьПрайсПоставщикаИзОП_косая(вхПоставщик, вхДатаЗагрузкиПрайсаПоставщиком = Неопределено, вхТабТоваров) Экспорт
	ОписаниеОшибки = "";
	ДокументСоздан = Ложь;
	ТекДокумент = Документы.ПереоценкаОстатковПоставщика.ПустаяСсылка();
	...
	|	Т.МинимальныйОстаток,
	|	Т.Цена,
	|	МАКСИМУМ(ЕСТЬNULL(НП.Ссылка, ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка))) КАК НоменклатураПоставщика
	|ПОМЕСТИТЬ Т0
	|ИЗ
	|	ТабТоваров КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НП
	|		ПО Т.Артикул = НП.Артикул
	|			И Т.Изготовитель = НП.Изготовитель.Наименование
	|			И (НП.Владелец = &Контрагент)
	|			И (НЕ НП.ПометкаУдаления)
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.АртикулПоставщика,
	|	Т.ИзготовительПоставщика,
	...
	Пока Р.Количество() > 0 Цикл
		...
		Для Каждого Товар Из Р Цикл
			...
			пар = Новый Структура("АртикулПоставщика,ИзготовительПоставщика", Товар.АртикулПоставщика, Товар.ИзготовительПоставщика);
			Строки = вхТабТоваров.НайтиСтроки(пар);
			Для Каждого вхСтрока Из Строки Цикл
				вхСтрока.Обработано = Истина;
			КонецЦикла;
...
Функция ПолучитьНастройкуFTP(вхКонтрагент, вхОбъектМетаданных, вхТЧОбъектаМетаданных, вхВыгрузка) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	НастройкиFTP.Ссылка,
	|	НастройкиFTP.ИмяФайлаОтвета
	|ИЗ
	|	Справочник.НастройкиFTP.Контрагенты КАК НастройкиFTP
	|ГДЕ
	|	НастройкиFTP.Контрагент = &Контрагент
	...
	Если вхВыгрузка Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И НастройкиFTP.Ссылка.Выгрузка";
	Иначе
		Запрос.Текст = Запрос.Текст + "
		|	И НастройкиFTP.Ссылка.Загрузка";
	КонецЕсли;
	Результат = Запрос.Выполнить().Выгрузить();
	
	Если Результат.Количество() = 0 Тогда
		Возврат Новый Структура("Настройка,ИмяФайлаОтвета", Неопределено, "");
		
	Иначе
		Возврат Новый Структура("Настройка,ИмяФайлаОтвета", Результат[0].Ссылка, Результат[0].ИмяФайлаОтвета);
		
	КонецЕсли;
	
КонецФункции
	
Функция ВыгрузитьФайлFTP(вхСсылка, вхКонтрагент, вхОбъектМетаданных, вхТЧОбъектаМетаданных) Экспорт
	
	СтруктураНастройки = ПолучитьНастройкуFTP(вхКонтрагент, вхОбъектМетаданных, вхТЧОбъектаМетаданных, Истина);
	
	Если СтруктураНастройки.Настройка = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Обработка = Обработки.УниверсальнаяОбработкаЗагрузкиВыгрузкиИзВФайл.Создать();	
	Обработка.Настройка = СтруктураНастройки.Настройка;
	//Обработка.ИмяФайлаОтвета = СтруктураНастройки.ИмяФайлаОтвета;
	Обработка.ТекущийОбъект = вхСсылка;
...
Функция ПрочитатьОтветыFTP(вхСсылка, вхКонтрагент, вхОбъектМетаданных, вхТЧОбъектаМетаданных, вхВыгрузка) Экспорт
	
	СтруктураНастройки = ПолучитьНастройкуFTP(вхКонтрагент, вхОбъектМетаданных, вхТЧОбъектаМетаданных, вхВыгрузка);
...
Процедура ЗарегистрироватьВОбменеFTP(вхСтруктураДокумента) Экспорт
	Если ТипЗнч(вхСтруктураДокумента) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	Если вхСтруктураДокумента.Свойство("Ссылка") = Неопределено 
		ИЛИ вхСтруктураДокумента.Свойство("Контрагент") = Неопределено
		ИЛИ вхСтруктураДокумента.Свойство("СтатусДокумента") = Неопределено 
		ИЛИ вхСтруктураДокумента.Свойство("ВидОперации") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСтруктураДокумента.Контрагент, "РаботаетЧерезFTP") Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(вхСтруктураДокумента.Ссылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		Если вхСтруктураДокумента.ВидОперации = Перечисления.ВидыПоступленияТоваров.ОтветХранение Тогда
			НеРегистрировать = Истина;
			Если вхСтруктураДокумента.СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят
				ИЛИ вхСтруктураДокумента.СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровРазмещен Тогда
				НеРегистрировать = Ложь;
			КонецЕсли;
			...
			|	ИсторияОбмена1с8FTP.ФормаОбъекта,
			|	ИсторияОбмена1с8FTP.ДатаОбмена,
			|	ИсторияОбмена1с8FTP.Выгружен,
			|	ИсторияОбмена1с8FTP.Получен,
			...
			Запрос.УстановитьПараметр("Объект", вхСтруктураДокумента.Ссылка);
			Выборка = Запрос.Выполнить().Выбрать();
			
			Если Выборка.Количество() = 0 Тогда
				//непонятно как загружен МХ-1 и непонятно, нужно ли выгружать результаты приемки
				
			Иначе
				Выборка.Следующий();
				ЗаписьИстории = РегистрыСведений.ИсторияОбмена1с8FTP.СоздатьНаборЗаписей();
				ЗаписьИстории.Отбор.Объект.Установить(Выборка.Объект);
				ЗаписьИстории.Отбор.ФормаОбъекта.Установить(Выборка.ФормаОбъекта);
				ЗаписьИстории.Прочитать();
				Для Каждого СтрокаИстории Из ЗаписьИстории Цикл
					СтрокаИстории.Выгружен = Ложь;
				КонецЦикла;
				
				ЗаписьИстории.Записать(Истина);
				
			КонецЕсли;
			
		ИначеЕсли вхСтруктураДокумента.ВидОперации = Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Тогда
...
Процедура ИзменитьРегистрациюДокументаВОбменеFTP(вхСсылкаНаДокумент, вхФормаОбъекта = Неопределено, вхДатаОбмена, вхВыгружен = Ложь, вхПолучен = Ложь, вхНомер = Неопределено, вхДата = Неопределено) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ИсторияОбмена1с8FTP.Объект,
	|	ИсторияОбмена1с8FTP.ДатаОбмена
	...
	Если ЗначениеЗаполнено(вхФормаОбъекта) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И ИсторияОбмена1с8FTP.ФормаОбъекта = &Форма";
		Запрос.УстановитьПараметр("Форма", вхФормаОбъекта);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Объект", вхссылкаНаДокумент);
	Результат = Запрос.Выполнить().Выбрать();
	Пока Результат.Следующий() Цикл
		Если вхДатаОбмена <> Результат.ДатаОбмена Тогда
			вхДатаОбмена = Результат.ДатаОбмена;
		КонецЕсли;
	КонецЦикла;
	
	Регистрировать = Истина;
	Если НЕ ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСсылкаНаДокумент, "Контрагент"), "РаботаетЧерезFTP") Тогда
		Регистрировать = Ложь;
	КонецЕсли;
	//Если ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	//	Склад = ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСсылкаНаДокумент, "Склад");
	//	Если НЕ ОбщегоНазначения.ПолучитьЗначениеРеквизита(Склад, "СкладVMI") Тогда
	//		Регистрировать = Ложь;
	//	КонецЕсли;
	//	
	//КонецЕсли;
	...
	Если Регистрировать Тогда
		аЗапись = РегистрыСведений.ИсторияОбмена1с8FTP.СоздатьМенеджерЗаписи();
		аЗапись.Объект = вхСсылкаНаДокумент;
		аЗапись.ФормаОбъекта = вхФормаОбъекта;
		аЗапись.ДатаОбмена = вхДатаОбмена;
		аЗапись.Выгружен = вхВыгружен;
		аЗапись.Получен = вхПолучен;
		аЗапись.КодФайла = вхНомер;
		аЗапись.ДатаФайла = вхДата;
...
Процедура ЗарегистрироватьДляОтправкиПочты(ОбъектРегистрации, РучнаяРегистрация = Ложь, ОтправитьЗагрузку = Неопределено) Экспорт 
	лКлючАлгоритма = "ОбщийМодуль_ЭлектронныеДокументы_ЗарегистрироватьДляОтправкиПочты";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);		
	КонецЕсли;
	...
	|	И ИсторияОтправкиПисемПоставщикам.НомерРегистрации = &НомерРегистрации"
	);
	
	НомерРегистрации = 0;
	Если ТипЗнч(ОбъектРегистрации) = Тип("ДокументСсылка.ПереоценкаОстатковПоставщика") Тогда
		Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОбъектРегистрации, "Проведен") Тогда
			Если ОтправитьЗагрузку = Неопределено Тогда
				НомерРегистрации = 1;
				
			ИначеЕсли ОтправитьЗагрузку Тогда
				НомерРегистрации = 0;
	...
	ИначеЕсли ТипЗнч(ОбъектРегистрации) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда 
		//#XX-254 Kalinin V.A. ( 2018-06-29 )
		ВидОперации =   ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОбъектРегистрации, "ВидОперацииПоступления");
		если ВидОперации =  Перечисления.ВидыПоступленияТоваров.ОтветХранение
			или ВидОперации= Перечисления.ВидыПоступленияТоваров.НаСклад Тогда
			Возврат;
		КонецЕсли;
		
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Объект", ОбъектРегистрации);
	Запрос.УстановитьПараметр("НомерРегистрации", НомерРегистрации);
	Р = Запрос.Выполнить();
	Если НЕ Р.Пустой() И НЕ РучнаяРегистрация Тогда
		Возврат;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	П.АдресЭлПочтыТорговойТочки КАК АдресКА,
	|	П.АдресЭлПочтыОтправителя КАК АдресМенеджера
	|ИЗ
	|	Справочник.ПараметрыФормированияЗаказовПоставщикам КАК П
	|ГДЕ
	|	П.Владелец = &ОснТТ
	|	И НЕ П.ПометкаУдаления";
	пар = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ОбъектРегистрации, "Контрагент,Ответственный");
	Запрос.УстановитьПараметр("ОснТТ", ОбщегоНазначения.ПолучитьЗначениеРеквизита(пар.Контрагент, "ОсновнаяТорговаяТочка"));
	...
	Пока Р.Следующий() Цикл
		АдресКА = ?(ПустаяСтрока(АдресКА), АдресКА, АдресКА + ";") + СокрЛП(Р.АдресКА);
		АдресМенеджера = ?(ПустаяСтрока(АдресМенеджера), АдресМенеджера, АдресМенеджера + ";") + СокрЛП(Р.АдресМенеджера);
	КонецЦикла;
	
	АдресКА = СокрЛП(АдресКА);
	Если ПустаяСтрока(АдресКА) Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	КонтактнаяИнформация.Представление КАК АдресКА
		|ИЗ
		|	РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
		|ГДЕ
		|	КонтактнаяИнформация.Объект = &Контрагент
		|	И КонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)
		|	И КонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailДополнительный)";//Голубев 20190717
		//|	И КонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailДляОбменаДокументами)";
		Запрос.УстановитьПараметр("Контрагент", пар.Контрагент);
		Р = Запрос.Выполнить().Выбрать();
		Пока Р.Следующий() Цикл
			АдресКА = ?(ПустаяСтрока(АдресКА), АдресКА, АдресКА + ";") + СокрЛП(Р.АдресКА);
		КонецЦикла;
	...
	АдресКА = СтрЗаменить(АдресКА," ","");
	АдресКА = СтрЗаменить(АдресКА,Символы.ПС,"");
	АдресКА = СтрЗаменить(АдресКА,Символы.ВК,"");
	АдресКА = СтрЗаменить(АдресКА,Символы.НПП,"");
	АдресКА = СтрЗаменить(АдресКА,Символы.ПФ,"");
	АдресКА = СтрЗаменить(АдресКА,Символы.Таб,"");
	АдресКА = СтрЗаменить(АдресКА,Символы.ВТаб,"");
	АдресКА = СтрЗаменить(АдресКА,",",";");
	СписокАдресов = Новый СписокЗначений;
	МассивАдресов = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(АдресКА, ";");
	Для А = 0 По МассивАдресов.Количество()-1 Цикл
		текАдрес = МассивАдресов[А];
		Если СтрНайти(текАдрес,"@") = 0 ИЛИ СтрНайти(текАдрес,".") = 0 Тогда
			Продолжить;
		КонецЕсли;
		Если НЕ ПустаяСтрока(текАдрес) Тогда
			Если СписокАдресов.НайтиПоЗначению(текАдрес) = Неопределено Тогда
				СписокАдресов.Добавить(текАдрес);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	АдресКА = "";
	Для Каждого КлючИЗначение Из СписокАдресов Цикл
		АдресКА = ?(ПустаяСтрока(АдресКА), АдресКА, АдресКА + ";");
		АдресКА = АдресКА + КлючИЗначение.Значение;
		
	КонецЦикла;
			
	АдресМенеджера = СокрЛП(АдресМенеджера);
	АдресМенеджера = СтрЗаменить(АдресМенеджера," ","");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.ПС,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.ВК,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.НПП,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.ПФ,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.Таб,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,Символы.ВТаб,"");
	АдресМенеджера = СтрЗаменить(АдресМенеджера,",",";");
	
	СписокАдресов = Новый СписокЗначений;
	МассивАдресов = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(АдресМенеджера, ";");
	Для А = 0 По МассивАдресов.Количество()-1 Цикл
		текАдрес = МассивАдресов[А];
		Если СтрНайти(текАдрес,"@") = 0 ИЛИ СтрНайти(текАдрес,".") = 0 Тогда
			Продолжить;
	...
	АдресМенеджера = "";
	Для Каждого КлючИЗначение Из СписокАдресов Цикл
		АдресМенеджера = ?(ПустаяСтрока(АдресМенеджера), АдресМенеджера, АдресМенеджера + ";");
		АдресМенеджера = АдресМенеджера + КлючИЗначение.Значение;
		
	КонецЦикла;
	
	ПараметрыАдресов.Вставить("АдресКА", АдресКА);
	ПараметрыАдресов.Вставить("АдресМенеджера", АдресМенеджера);
	
	аЗапись = РегистрыСведений.ИсторияОтправкиПисемПоставщикам.СоздатьМенеджерЗаписи();
	//возможен косяк, если у последней корректировки и у начального заказа разные контрагенты
	аЗапись.Объект = ОбъектРегистрации.Ссылка;
	аЗапись.ДатаРегистрации = ТекущаяДата();
	аЗапись.Ответственный = ПараметрыСеанса.ТекущийПользователь;
	аЗапись.Отправлен = Ложь;
	аЗапись.Ошибка = Ложь;
	Если ПараметрыАдресов.Свойство("АдресКА") Тогда
		аЗапись.АдресПоставщика = ПараметрыАдресов.АдресКА;
	КонецЕсли;
	Если ПараметрыАдресов.Свойство("АдресМенеджера") Тогда
		аЗапись.КопияАдресМенеджера = ПараметрыАдресов.АдресМенеджера;
	КонецЕсли;
	аЗапись.НомерРегистрации = НомерРегистрации;
	аЗапись.Записать(Истина);
	
КонецПроцедуры
	
Процедура РегистрацияОтправкиНаПочтуПриЗаписи(Источник, Отказ) Экспорт
	//Если ТипЗнч(Источник) = Тип("ДокументОбъект.КорректировкаЗаказаПоставщику") Тогда
	//	Если НЕ Источник.ДокументОснование.ДополнительныеСвойства.Свойство("ЗарегистрироватьДляОтправкиНаПочту") Тогда
	//		Возврат;
	//	КонецЕсли;
	//	
	//Иначе
	//	Если НЕ Источник.ДополнительныеСвойства.Свойство("ЗарегистрироватьДляОтправкиНаПочту") Тогда
	//		Возврат;
	//	КонецЕсли;
	//	
	//КонецЕсли;
	
	//Добавлено Валиахметов А.А. 07.03.2017 PK83-283 
	Перем ЭтоНовый, РежимЗаписи, Регистрировать;
	...
	Если Не (ЭтоНовый = Истина // ЭтоНовый может быть Неопределено, поэтому явное указание
		Или РежимЗаписи = РежимЗаписиДокумента.Проведение) Тогда  
		Возврат;
	КонецЕсли;
	//Конец Добавлено Валиахметов А.А. 07.03.2017 PK83-283 
	
	Источник.ДополнительныеСвойства.Свойство("ЗарегистрироватьДляОтправкиНаПочту", Регистрировать);
	Если НЕ Регистрировать = Истина Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.КорректировкаЗаказаПоставщику") Тогда
		ОбРегистрации = Источник.ДокументОснование;
		Если ОбменДаннымиКлиентСервер.ЭтоБитаяСсылка(ОбРегистрации) 
			ИЛИ ОбРегистрации = Документы.ЗаказПоставщику.ПустаяСсылка() Тогда
			Возврат;
		КонецЕсли;
		
		ЗарегистрироватьДляОтправкиПочты(ОбРегистрации);
		
	Иначе
		ЗарегистрироватьДляОтправкиПочты(Источник.Ссылка);
	КонецЕсли;
	
КонецПроцедуры

Функция НормализоватьСтрокуАртикула(лСтрока) Экспорт
	тСтрока = лСтрока;
	
	тСтрока = СокрЛП(тСтрока);
	тСтрока = Врег(тСтрока);
	тСтрока = СтрЗаменить(тСтрока,"А","A");
	тСтрока = СтрЗаменить(тСтрока,"В","B");
	тСтрока = СтрЗаменить(тСтрока,"Е","E");
	тСтрока = СтрЗаменить(тСтрока,"К","K");
	тСтрока = СтрЗаменить(тСтрока,"М","M");
	тСтрока = СтрЗаменить(тСтрока,"Н","H");
	тСтрока = СтрЗаменить(тСтрока,"О","O");
	тСтрока = СтрЗаменить(тСтрока,"Р","P");
	тСтрока = СтрЗаменить(тСтрока,"С","C");
	тСтрока = СтрЗаменить(тСтрока,"Т","T");
	тСтрока = СтрЗаменить(тСтрока,"Х","X");
	аСтрока = "";
	Для А = 1 По СтрДлина(тСтрока) Цикл
		Символ = Сред(тСтрока,А,1);
		
		Если Символ = "("
			ИЛИ Символ = ")" Тогда
			//можно скобки
			аСтрока = аСтрока + Символ;
			Продолжить;
		КонецЕсли;
		
		Если КодСимвола(Символ) >= КодСимвола("0")
			И КодСимвола(Символ) <= КодСимвола("9") Тогда
			//можно цифры
			аСтрока = аСтрока + Символ;
			Продолжить;
		КонецЕсли;
		
		Если КодСимвола(Символ) < КодСимвола("A")
			ИЛИ КодСимвола(Символ) > КодСимвола("Z") Тогда
			//нельзя ничего кроме латинских заглавных
			Символ = "";
...
Функция НормализоватьСтрокуИзготовителя(лСтрока) Экспорт
	тСтрока = лСтрока;
	
	тСтрока = СокрЛП(тСтрока);
	
	//удаляем кавычки
	Символ = Лев(тСтрока,1);		
	Если КодСимвола(Символ) = 34
		ИЛИ КодСимвола(Символ) = 39 Тогда		
		тСтрока = Прав(тСтрока, СтрДлина(тСтрока) - 1);
	КонецЕсли;	
	Символ = Прав(тСтрока,1);		
	Если КодСимвола(Символ) = 34
		ИЛИ КодСимвола(Символ) = 39 Тогда
		тСтрока = Лев(тСтрока, СтрДлина(тСтрока) - 1);
	КонецЕсли;
			
	Возврат тСтрока;
	
КонецФункции
#Область ЗагрузкаТабЧастиИзФайла
Функция ПроверитьНастройку(СтруктураФайла)
	ОписаниеОшибки = "";
	Загружать = Истина;
	ФорматФайла = "";
	
	Если СтруктураФайла.Свойство("ИмяФайла") = Неопределено Тогда
		ОписаниеОшибки = ОписаниеОшибки + "не задано имя файла;";
		Загружать = Ложь;
	КонецЕсли;
	...
	Если СтруктураФайла.Свойство("СтруктураТабЧасти") = Неопределено Тогда
		...
		КонецЕсли;
		ОписаниеОшибки = ОписаниеОшибки + "не задана (или задана неверно) структура считываемой табличной части;";
		Загружать = Ложь;
	КонецЕсли;
	
	Расширение = Прав(СокрЛП(СтруктураФайла.ИмяФайла),4);
	ОшибкаФорматаФайла = Ложь;
	Если Лев(Расширение,1) <> "." Тогда
		Расширение = Прав(СокрЛП(СтруктураФайла.ИмяФайла),5);
		Если Лев(Расширение,1) <> "." Тогда
			ОшибкаФорматаФайла = Истина;
		Иначе
			Если Прав(Расширение, 4) = "xlsx" Тогда
				ФорматФайла = "xls";
			Иначе
				ОшибкаФорматаФайла = Истина;
			КонецЕсли;
		КонецЕсли;
	Иначе
		ФорматФайла = Прав(Расширение,3);
		Если ФорматФайла = "txt"
			ИЛИ ФорматФайла = "exh"
			ИЛИ ФорматФайла = "xls" Тогда
			
		Иначе
			ОшибкаФорматаФайла = Истина;
		КонецЕсли;
	КонецЕсли;
	Если ОшибкаФорматаФайла Тогда
		Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
		КонецЕсли;
		ОписаниеОшибки = ОписаниеОшибки + "не определен формат файла;";
		Загружать = Ложь;
	КонецЕсли;
	
	Возврат Новый Структура("Загружать,ФорматФайла,ОписаниеОшибки", Загружать, ФорматФайла, ОписаниеОшибки);
	
КонецФункции

Функция СформироватьТаблицуРезультата(СтруктураФайла)
	...
	Для Каждого КлючИЗначение Из СтруктураФайла.СтруктураТабЧасти Цикл
		...
		ТабРезультат.Колонки.Добавить(ИмяКолонки, ТипДанных);
...
Функция ПрочитатьФайлXLS(СтруктураФайла)
	ОписаниеОшибки = "";
	ТабРезультат = СформироватьтаблицуРезультата(СтруктураФайла);
	...
	Исключение
		ОписаниеОшибки = "Ошибка при открытии файла с помощью Excel! Загрузка не будет произведена!";
		ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + ОписаниеОшибки();
		Возврат Новый Структура("ТабРезультат,ОписаниеОшибки", ТабРезультат, ОписаниеОшибки);
	...
	Исключение
		...
		Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
		КонецЕсли;
		ОписаниеОшибки = ОписаниеОшибки + "Файл "+Строка(СтруктураФайла.ИмяФайла)+" не соответствует необходимому формату! Первый лист не найден!";
		Возврат Новый Структура("ТабРезультат,ОписаниеОшибки", ТабРезультат, ОписаниеОшибки);
	...
	Для НС = СтруктураФайла.НомерПервойСтроки по СтруктураФайла.НомерПоследнейСтроки Цикл // НС указываем с какой строки начинать обработку
		#Если Клиент Тогда
			Состояние("Файл "+Строка(СтруктураФайла.ИмяФайла)+": Обрабатывается первый лист "+Строка(Формат(?(ФайлСтрок=0,0,((100*НС)/ФайлСтрок)),"ЧЦ=3; ЧДЦ=0"))+" %");
		...
		Для НомерКолонки = 1 по ТабРезультат.Колонки.Количество() Цикл
			...
			Если ИмяКолонки = "ИзготовительСтрока" Тогда
				ИмяКолонки = "Изготовитель";
			КонецЕсли;
			
			ТекНомерКолонки = СтруктураФайла.СтруктураТабЧасти[ИмяКолонки];
			ТекущееЗначение = Excel.Cells(НС, ТекНомерКолонки).Text;
			
			Если ИмяКолонки = "Изготовитель" Тогда
				ИмяКолонки = "ИзготовительСтрока";
			КонецЕсли;
			...
			Исключение
				Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
					ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
				КонецЕсли;
				ОписаниеОшибки = ОписаниеОшибки + "не было считано значение в строке " + Строка(НС) + " , в колонке " + Строка(ТекНомерКолонки);
				
			КонецПопытки;
			
		КонецЦикла;
		
		Если ЗначениеЗаполнено(СтруктураФайла.Контрагент) Тогда
			НоваяСтрока.Контрагент = СтруктураФайла.Контрагент;
		КонецЕсли;
	...
	Исключение
		Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
		КонецЕсли;
		ОписаниеОшибки = ОписаниеОшибки + "не удалось закрыть файл";
...
Функция ПрочитатьТекстФайл(СтруктураФайла)
	ОписаниеОшибки = "";
	ТабРезультат = СформироватьтаблицуРезультата(СтруктураФайла);
	...
	Попытка
		ЗагружаемыйФайл.Прочитать(СтруктураФайла.ИмяФайла, СтруктураФайла.КодировкаФайла);
	Исключение
		ОписаниеОшибки = "ошибка при чтении текстового файла";
		ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + ОписаниеОшибки();
		Возврат Новый Структура("ТабРезультат,ОписаниеОшибки", ТабРезультат, ОписаниеОшибки);
	КонецПопытки;
	...
	Если МассивКол.Количество() <> ТабРезультат.Колонки.Количество() Тогда
		ОписаниеОшибки = "структура файла не соответствует количеству колонок в итоговой таблице";
		Возврат Новый Структура("ТабРезультат,ОписаниеОшибки", ТабРезультат, ОписаниеОшибки);
		
	КонецЕсли;
	
	Для НомерСтроки = СтруктураФайла.НомерПервойСтроки по ЗагружаемыйФайл.КоличествоСтрок() Цикл
		...
		Если ЗначениеЗаполнено(СтруктураФайла.Контрагент) Тогда
			нс.Контрагент = СтруктураФайла.Контрагент;
		КонецЕсли;
...
Функция СформироватьКонечнуюТаблицуТоваров()
	ТабТоваров = Новый ТаблицаЗначений;
	
	ТабТоваров.Колонки.Добавить("АртикулПоставщика", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(30)));
	ТабТоваров.Колонки.Добавить("ИзготовительПоставщика", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(100)));
	ТабТоваров.Колонки.Добавить("НоменклатураПоставщика", Новый ОписаниеТипов("СправочникСсылка.НоменклатураКонтрагентов"));
	ТабТоваров.Колонки.Добавить("Артикул", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(25)));
	ТабТоваров.Колонки.Добавить("ИзготовительСтрока", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(100)));
	ТабТоваров.Колонки.Добавить("Изготовитель", Новый ОписаниеТипов("СправочникСсылка.Изготовители"));
	ТабТоваров.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТабТоваров.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
	ТабТоваров.Колонки.Добавить("Цена", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,4)));
	ТабТоваров.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТабТоваров.Колонки.Добавить("СуммаНДС", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТабТоваров.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СтавкиНДС"));
...
Процедура ОбработатьЗагруженныеДанные(ТабТоваров)
	Для Каждого Товар Из ТабТоваров Цикл
		Если НЕ ПустаяСтрока(Товар.АртикулПоставщика) И ПустаяСтрока(Товар.Артикул) Тогда
			Товар.Артикул = ЭлектронныеДокументы.НормализоватьСтрокуАртикула(Товар.АртикулПоставщика);
		КонецЕсли;
		Если НЕ ПустаяСтрока(Товар.ИзготовительПоставщика) И ПустаяСтрока(Товар.ИзготовительСтрока) Тогда
			Товар.ИзготовительСтрока = ЭлектронныеДокументы.НормализоватьСтрокуИзготовителя(Товар.ИзготовительПоставщика);
		КонецЕсли;
	...
	|	Т.АртикулПоставщика,
	|	Т.ИзготовительПоставщика,
	|	ЕСТЬNULL(НП.Ссылка, ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка)) КАК НоменклатураПоставщика,
	|	Т.Артикул,
	|	Т.ИзготовительСтрока,
	|	ЕСТЬNULL(Изг.Ссылка, ЗНАЧЕНИЕ(Справочник.Изготовители.ПустаяСсылка)) КАК Изготовитель,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НП
	|		ПО Т.Артикул = НП.Артикул
	|			И Т.ИзготовительСтрока = НП.Изготовитель.Наименование
	|			И (НП.Владелец = Т.Контрагент)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Изготовители КАК Изг
	|		ПО Т.ИзготовительСтрока = Изг.Наименование
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Н
	|		ПО Т.Артикул = Н.Артикул
	|			И Т.ИзготовительСтрока = Н.Изготовитель.Наименование"
	);
	...
	ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Результат, ТабТоваров);
...
//структура файла:
//					"ИмяФайла", Строка;
//					"КодировкаФайла", Строка;
//					"НомерПервойСтроки", Число;
//					"НомерПоследнейСтроки", Число;
//					"Контрагент", Справочник;
//					"СтруктураТабЧасти", Структура(НаименованиеКолонки, НомерКолонки);
//
//возвращает структуру (ТабТоваров, ОписаниеОшибки)
//
Функция ЗагрузитьТаблицуТоваровИзФайла(вхСтруктураФайла) Экспорт
	ТабТоваров = СформироватьКонечнуюТаблицуТоваров();
	
	ОписаниеОшибки = "";
	РезультатПроверки = ПроверитьНастройку(вхСтруктураФайла);
	Если РезультатПроверки.Загружать Тогда
		Если РезультатПроверки.ФорматФайла = "xls" Тогда
			РезультатЗагрузки = ПрочитатьФайлXLS(вхСтруктураФайла);
		Иначе
			РезультатЗагрузки = ПрочитатьТекстФайл(вхСтруктураФайла);
		КонецЕсли;
		
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(РезультатЗагрузки.ТабРезультат, ТабТоваров);
		
		ОбработатьЗагруженныеДанные(ТабТоваров);
		
	Иначе
		ОписаниеОшибки = "таблица товаров не сформирована" + Символы.ПС + РезультатПроверки.ОписаниеОшибки;
		
	КонецЕсли;
	
	Возврат Новый Структура("ТабТоваров,ОписаниеОшибки", ТабТоваров, ОписаниеОшибки);
