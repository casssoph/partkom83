...
Функция ПолучитьСтрокуСМетодамиОбъекта(ИмяОбъекта) Экспорт
	
	Возврат DataExchangeПовтИсп.ПолучитьСтрокуСМетодамиОбъекта(ИмяОбъекта);
	
КонецФункции

Функция ПолучитьРаспакованноеСообщение(лОтветСервера) Экспорт
...
КонецФункции
Функция ЗапаковатьСообщение(Сообщение) Экспорт
	
	Упаковщик = DataExchangeПовтИсп.ПолучитьУпаковщик();
	
	Возврат Упаковщик.PackWideString(Сообщение);
	
КонецФункции


Функция ВыполнитьКомандуНаСервере(Строка, Параметры = Неопределено) Экспорт //Семенов И.П. 06.08.2019 XX-2990 добавил параметры
	Выполнить(Строка);
КонецФункции

Функция ПолучитьТаблицуФорм() Экспорт
	
	ТабФорм = Новый ТаблицаЗначений;
	ТабФорм.Колонки.Добавить("ВидОбъекта", 	Новый ОписаниеТипов("Строка"));
	ТабФорм.Колонки.Добавить("ИмяОбъекта", 	Новый ОписаниеТипов("Строка"));
	ТабФорм.Колонки.Добавить("ИмяФормы", 	Новый ОписаниеТипов("Строка"));
	...
	Для Каждого ВидОбъекта Из ВидыОбъектов Цикл
		...
		Для Каждого ОбъектМетаданных Из Метаданные[ВидОбъекта] Цикл
			Если ОбъектМетаданных.ОсновнаяФормаОбъекта <> Неопределено Тогда
				НоваяСтрока = ТабФорм.Добавить();
				НоваяСтрока.ВидОбъекта = ВидОбъекта;
				НоваяСтрока.ИмяОбъекта = ОбъектМетаданных.Имя;
				НоваяСтрока.ИмяФормы = ОбъектМетаданных.ОсновнаяФормаОбъекта.Имя; 
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
