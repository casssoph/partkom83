...
Функция ОтложенноеСнятиеБлокировкиКонтрагентов(ДокументОплаты) Экспорт
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ Контрагент.Пустая() И КонтрагентЗаблокирован(Контрагент) Тогда
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОплаты, "Дата, СуммаДокумента");
		
		Если НетПросроченныхЗадолженностейПоРеализациям(Контрагент, Реквизиты.СуммаДокумента, ДокументОплаты.МоментВремени()) Тогда
