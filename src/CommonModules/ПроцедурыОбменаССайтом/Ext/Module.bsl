...
Процедура ЗарегистрироватьИзменения(Объект)
	...
	Если ТипОбъекта = Тип("РегистрСведенийНаборЗаписей.ЗначенияСвойствОбъектов") Тогда
		...
		Для каждого Запись Из Объект Цикл
			...
			Если ТипДанных = Тип("СправочникСсылка.Номенклатура") Тогда
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.Объект);
				//)Семенов И.П.
			ИначеЕсли ТипДанных = Тип("СправочникСсылка.ХарактеристикиНоменклатуры") Тогда
				Если Запись.Объект.ПолучитьОбъект() <> Неопределено Тогда
					...
					ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.Объект.Владелец);
					//)Семенов И.П.
				КонецЕсли;
			КонецЕсли;	
	...
	ИначеЕсли ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.ТоварыНаСкладах")
		...
		Для каждого Запись Из Объект Цикл
			...
			ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.Номенклатура);
			//)Семенов И.П.
		КонецЦикла;
	...
	ИначеЕсли ТипОбъекта = Тип("РегистрСведенийНаборЗаписей.ШтрихКоды") Тогда
		...
		Для каждого Запись Из Объект Цикл
			...
			Если ТипЗнч(Запись.Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.Владелец);
				//)Семенов И.П.
			КонецЕсли;
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникОбъект.ХранилищеДополнительнойИнформации") Тогда
		...
		Если ТипЗнч(Объект.Объект) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Объект.Объект);
			//)Семенов И.П.
			...
			Пока Выборка.Следующий() Цикл
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Выборка.Ссылка);
				//)Семенов И.П.
			КонецЦикла; 
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникОбъект.ХарактеристикиНоменклатуры")
		...
		Если ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда 
			...
			ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Объект.Владелец);
			//)Семенов И.П.
		КонецЕсли;
	...
	ИначеЕсли ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.ЗаказыПокупателей") Тогда
		...
		Для каждого Запись Из Объект Цикл
			...
			Если ЗначениеЗаполнено(Запись.ЗаявкаПокупателя) Тогда
				//Семенов И.П. 31.01.2019 XX-1768(
				//ПланыОбмена.ЗарегистрироватьИзменения(МассивУзлов, Запись.ЗаявкаПокупателя);
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.ЗаявкаПокупателя);
				//)Семенов И.П.
			КонецЕсли;
	...
	ИначеЕсли ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.ВзаиморасчетыСКонтрагентами") Тогда
		...
		Для каждого Запись Из Объект Цикл
			...
			Если ТипЗнч(Запись.Сделка) = Тип("ДокументСсылка.ЗаявкаПокупателя")
				И ЗначениеЗаполнено(Запись.Сделка) Тогда
				//Семенов И.П. 31.01.2019 XX-1768(
				//ПланыОбмена.ЗарегистрироватьИзменения(МассивУзлов, Запись.Сделка);
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Запись.Сделка);
				//)Семенов И.П.
			КонецЕсли;
	...
	ИначеЕсли ТипОбъектаДокумент Тогда
		...
		Если ОбщегоНазначения.ЕстьТабЧастьДокумента("Товары", МетаданныеДокумента) Тогда
			Для каждого СтрокаТЧ Из Объект.Товары Цикл
				...
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, СтрокаТЧ.Номенклатура);
				//)Семенов И.П.
			КонецЦикла;
		ИначеЕсли ОбщегоНазначения.ЕстьТабЧастьДокумента("Заказы", МетаданныеДокумента) Тогда
			Для каждого Заказ Из Объект.Заказы Цикл
				...
				Иначе
					ИмяРеквизитаТЧ = "ЗаявкаПокупателя";
				КонецЕсли;
...
Функция ПолучитьЗаказыСОплатойИОтгрузкойПоКатегориям(МассивИзменений, МассивЗагруженныхДокументов) Экспорт
	...
	|		ИЗ
	|			Документ.ЗаявкаПокупателя КАК Заказ
	...
	|				ПО Заказ.Ссылка = ЗаказыПокупателейОстатки.ЗаявкаПокупателя
	...
	|				ПО Заказ.Ссылка = ЗаказыПокупателей.ЗаявкаПокупателя
	...
	Если КоличествоЗаказовОграничения > 0 Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОграниченияПоЗаказамОстатки", " ЗаявкаПокупателя В (&МассивДокументовСсылок) ");	
...
//Снятие блокировок
Процедура СнятиеБлокировкиКонтрагентовОбработкаПроведения(ДокументОплаты, Отказ, РежимПроведения) Экспорт
	
	Если НЕ РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Блокировки контрагентов", "Автоснятие блокировки", Ложь) Тогда
		Возврат;
	КонецЕсли;
	
	Контрагент = ДокументОплаты.Контрагент;
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ Контрагент.Пустая() И КонтрагентЗаблокирован(Контрагент) Тогда
		
		Если НетЗадолженностейПоРеализациям(Контрагент) Тогда
			
			КонтрагентыДляРазблокировки = КонтрагентыДляРазблокировки(Контрагент);
			
			Если РазрешеноАвтоснятиеБлокировки(Контрагент) Тогда
				ТекстПисьма = "";
				СнятьБлокировкуКонтрагентаВ1С(КонтрагентыДляРазблокировки, ДокументОплаты, ТекстПисьма);
				РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.СнятиеБлокировкиКонтрагентов, ТекстПисьма, "Снятие блокировки с контрагента");
			КонецЕсли;
			
			Если РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Elma", "Снимать блокировки при поступлении оплаты", Ложь) Тогда
				ОтправитьРазблокировкуВЭЛМА(КонтрагентыДляРазблокировки, ДокументОплаты.Движения.Взаиморасчеты);
			КонецЕсли;
		...
		МЗ.Документ = ДокументОплаты.Ссылка;
		МЗ.Записать();
...
Функция ОтложенноеСнятиеБлокировкиКонтрагентов(ДокументОплаты) Экспорт
	...
	Если НЕ РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Блокировки контрагентов", "Автоснятие блокировки", Ложь) Тогда
		Возврат Успешно;
	КонецЕсли;
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОплаты, "Дата, СуммаДокумента,Контрагент");
	Контрагент = Реквизиты.Контрагент;
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ Контрагент.Пустая() И КонтрагентЗаблокирован(Контрагент) Тогда
		
		//Если НетПросроченныхЗадолженностейПоРеализациям(Контрагент, Реквизиты.СуммаДокумента, ТекущаяДата()) Тогда
		Если Справочники.Контрагенты.ПросроченнаяЗадолженностьКонтрагента(Контрагент, ТекущаяДата()) <= 0 Тогда  	
			КонтрагентыДляРазблокировки = КонтрагентыДляРазблокировки(Контрагент);
			
			Если РазрешеноАвтоснятиеБлокировки(Контрагент) Тогда
				ТекстПисьма = "";
				Успешно = СнятьБлокировкуКонтрагентаВ1С(КонтрагентыДляРазблокировки, ДокументОплаты, ТекстПисьма);
				РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.СнятиеБлокировкиКонтрагентов, ТекстПисьма, "Снятие блокировки с контрагента");
			КонецЕсли;
			...
			Если РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Elma", "Снимать блокировки при поступлении оплаты", Ложь) Тогда
				...
				НЗ.Отбор.Регистратор.Установить(ДокументОплаты);
				
				ОтправитьРазблокировкуВЭЛМА(КонтрагентыДляРазблокировки, НЗ);
			КонецЕсли;
...
Функция СнятьБлокировкуКонтрагентаВ1С(КонтрагентыДляРазблокировки, ДокументОплаты, ТекстПисьма)
	
	Успешно = Истина;

	Для Каждого Контрагент Из КонтрагентыДляРазблокировки Цикл
		
		КонтрагентОбъект = Контрагент.ПолучитьОбъект();
		КонтрагентОбъект.Блокировка = Ложь;
		КонтрагентОбъект.Блокировка_Отгрузок_Дата = '00010101';
		КонтрагентОбъект.Блокировка_Заказов_Дата = '00010101';
		КонтрагентОбъект.Блокировка_Отгрузок_Коммент = "";
		КонтрагентОбъект.Блокировка_Заказов_Коммент = "";
		Попытка
			Текст =		Символы.ПС + "*СНЯТИЕ БЛОКИРОВКИ КОНТРАГЕНТА*" + Символы.ПС + Символы.ПС + "Была снята блокировка с контрагента - " + Контрагент.Наименование + "<" + Контрагент.Код + ">" + Символы.ПС +
						"Причина: " + ДокументОплаты + Символы.ПС + Символы.ПС +
						"Старые значения блокировок: " + Символы.ПС +
						?(ЗначениеЗаполнено(Контрагент.Блокировка_Отгрузок_Дата), ">Блокировка отгрузок: " + Формат(Контрагент.Блокировка_Отгрузок_Дата, "ДФ=dd.MM.yyyy") + "(" + СокрЛП(Контрагент.Блокировка_Отгрузок_Коммент) + ")", "")+ Символы.ПС  + 
						?(ЗначениеЗаполнено(Контрагент.Блокировка_Заказов_Дата), ">Блокировка заказов: " + Формат(Контрагент.Блокировка_Заказов_Дата, "ДФ=dd.MM.yyyy") + "(" + СокрЛП(Контрагент.Блокировка_Заказов_Коммент) + ")", "")+ Символы.ПС ;
			КонтрагентОбъект.Записать();
			
			//ЗаписьЖурналаРегистрации("[СнятьБлокировкуКонтрагентаВ1С]",
			//УровеньЖурналаРегистрации.Ошибка,, КонтрагентОбъект.Ссылка,"Блокировка снята, документ: "+ДокументОплаты);				

		Исключение
			Текст = "Не удалось снять блокировку с контрагента " + Контрагент.Наименование + "<" + Контрагент.Код + ">" + Символы.ПС + ОписаниеОшибки();
			Успешно = Ложь;	
		КонецПопытки;
		...
		,
		Справочники.СобытияДляОтправкиЭлектронныхПисем.СнятиеБлокировкиКонтрагентов, 
		Текст);
		
		ТекстПисьма = ТекстПисьма + Текст + Символы.ПС;
...
КонецФункции
Процедура ОтправитьРазблокировкуВЭЛМА(КонтрагентыДляРазблокировки, ДвиженияВзаиморасчеты)
	
	//Рудаков При ошибке разблокировки вываливается с ошибкой т.к. не определен "ТекстПисьма"
	ТекстПисьма="";
	//Рудаков Конец
	Если БылаОплатаПросроченнойРеализации(ДвиженияВзаиморасчеты) Тогда
		
		Настройка = Справочники.НастройкиРеквизитовДляОбменов.ПапкаОбменаСнятиеБлокировокЭЛМА;
		Папка = ?(ОбщегоНазначения.ЭтоРабочаяИнформационнаяБаза(), Настройка.СтрокаДляРабочейБазы, Настройка.СтрокаДляТестовойБазы);
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Для Каждого Контрагент Из КонтрагентыДляРазблокировки Цикл
			ИмяФайла = "unlock_client_" + СокрЛП(Контрагент.Код) + ".xml";
			ТекстСообщения =	"<?xml version=""1.0"" encoding=""windows-1251"" ?>
								|<workflow-run xmlns=""workflow"">
								|<login Required=""true"">admin</login>
								|<password Required=""true"">bpmn17</password>
								|<token Required=""true"">complete_block_client</token>
								|<workflow-name Required=""true"">Убить процесс Блокировка клиента</workflow-name>
								|<variable-list>
								|<string-variable name=""oid"">" + СокрЛП(Контрагент.Код) + "</string-variable>
								|</variable-list>
								|</workflow-run>";
			
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ТекстСообщения);
			ПолноеИмяФайла = Папка + "\" + ИмяФайла;
			Попытка
				ТекстовыйДокумент.Записать(ПолноеИмяФайла, КодировкаТекста.ANSI);
			Исключение
				ТекстПисьма = ТекстПисьма + "Не удалось записать файл разблокировки контрагентов в Элма: " + ПолноеИмяФайла + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Ошибка = Истина;
			КонецПопытки;
		КонецЦикла;
		
		Если Ошибка Тогда
			РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.СнятиеБлокировкиКонтрагентов, ТекстПисьма, "Снятие блокировки с контрагента в Элма");
		КонецЕсли;;
		
	КонецЕсли;
	
КонецПроцедуры
Функция БылаОплатаПросроченнойРеализации(Движения)
	
	БылаОплатаПросроченнойРеализации = Ложь;
	ТекущаяДата = ТекущаяДата();
	Движения.Прочитать();
	
	Для Каждого Движение Из Движения Цикл
		Документ = Движение.ДокументРасчетов;
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") И Документ.ДатаОплаты < ТекущаяДата Тогда
			БылаОплатаПросроченнойРеализации = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат БылаОплатаПросроченнойРеализации;
	
КонецФункции

Функция КонтрагентыДляРазблокировки(Контрагент)
	
	Массив = Новый Массив;
	Если ЗначениеЗаполнено(Контрагент.ГоловнойКонтрагент)
		И Контрагент <> Контрагент.ГоловнойКонтрагент Тогда
		Массив.Добавить(Контрагент.ГоловнойКонтрагент);
...
КонецФункции
Функция ПоГоловномуКонтрагенту(Контрагент)
	
	Возврат ЗначениеЗаполнено(Контрагент.ГоловнойКонтрагент) И Контрагент <> Контрагент.ГоловнойКонтрагент;
	
КонецФункции
Функция КонтрагентЗаблокирован(Контрагент)
	
	Возврат Справочники.Контрагенты.ДанныеБлокировкиКонтрагента(Контрагент).Заблокирован;
	
КонецФункции
Функция НетЗадолженностейПоРеализациям(Контрагент)
	
	Возврат Справочники.Контрагенты.ЗадолженностьКонтрагента(Контрагент) <= 0;
	
КонецФункции
Функция НетПросроченныхЗадолженностейПоРеализациям(Контрагент, СуммаДокумента, Дата)
	
	ПросроченнаяЗадолженность = Справочники.Контрагенты.ПросроченнаяЗадолженностьКонтрагента(Контрагент, Дата);
	
	Возврат СуммаДокумента >= ПросроченнаяЗадолженность;
	
КонецФункции
Функция РазрешеноАвтоснятиеБлокировки(Контрагент)
	
	Возврат НЕ ?(ПоГоловномуКонтрагенту(Контрагент), Контрагент.ГоловнойКонтрагент.Блокировка_Автом_НеОтключать, Контрагент.Блокировка_Автом_НеОтключать);
