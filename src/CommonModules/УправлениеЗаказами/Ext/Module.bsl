...
Функция ПроверитьЦеныНаУслуги(парам_ТЧ) Экспорт
	...
	|	И НЕ ФрмУслуги.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)";
	Запрос.УстановитьПараметр("ФрмУслуги", парам_ТЧ.Скопировать(,"НомерСтроки,Номенклатура,Цена"));
	...
	Попытка
		...
		Если Результат.Количество() > 0 тогда
			...
			Сообщить("Данные об Услугах указаны с ошибкой: либо услуга не указана либо услуга с ценой менее " + СокрЛП(лМинЦена) + "р. Документ не может быть записан.");
			
			Для каждого лстр из Результат цикл
				Сообщить("Номер Строки: " + СокрЛП(лстр.НомерСтроки) + ", Услуга: " + СокрЛП(лстр.Номенклатура) + ", Цена: " + СокрЛП(лстр.Цена));
			КонецЦикла;
