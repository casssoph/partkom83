...
Процедура ПроверитьРеквизитыЗаказов(ДокументОбъект, ИмяТабличнойЧасти, ИмяРевизитаЗаказ, 
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ]) И 
			(ТипЗнч(СтрокаТаблицы[ИмяРевизитаЗаказ]) = Тип("ДокументСсылка.ЗаявкаПокупателя") 
...
Процедура ПроверитьРезервированиеТарыПодЗаказСОбособленнымУчетом(ДокументОбъект,ИмяРевизитаЗаказ="", ЗаказВШапке=ложь, ИмяРеквизитаСклад="", Отказ, Заголовок) Экспорт
	...
	Если ЗаказВШапке Тогда
		...
		Если не ЗначениеЗаполнено(Заказ) Тогда
			Возврат;
		КонецЕсли;
		Если ТипЗнч(Заказ)=Тип("ДокументСсылка.ЗаявкаПокупателя") И Заказ.ДоговорКонтрагента.ОбособленныйУчетТоваровПоЗаказамПокупателей Тогда
...
// Параметры:
//  ДокументОбъект       - объект редактируемого документа;
Процедура ЗаполнитьЗаявкаПокупателяВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	ИмяРеквЗаказ = "ЗаявкаПокупателя";
	Если ВидОперации = "Реализация" ИЛИ ВидОперации = "ВозвратОтПокупателя" Тогда
		ИмяРеквЗаказ = "Сделка";
	КонецЕсли;
		
	БратьЗаказИзШапки = ТипЗнч(ДокументОбъект[ИмяРеквЗаказ]) = Тип("ДокументСсылка.ЗаявкаПокупателя");
	ЗаказИзШапки     = ?(БратьЗаказИзШапки, ДокументОбъект[ИмяРеквЗаказ], Документы.ЗаявкаПокупателя.ПустаяСсылка());
	
	Для каждого СтрокаТЧ Из ДокументОбъект.Товары Цикл
		СтрокаТЧ.ЗаявкаПокупателя = ЗаказИзШапки;
	КонецЦикла;
	Если НЕ (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.КорректировкаРеализации")
		ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг"))
	Тогда
...
КонецПроцедуры //ЗаполнитьЗаявкаПокупателяВТЧ()
...
Процедура ЗаполнитьЗаказПоставщикуВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	КонецЦикла;
	Если ТипЗнч(ДокументОбъект) <> Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг") Тогда
		Для каждого СтрокаТЧ Из ДокументОбъект.ВозвратнаяТара Цикл
			СтрокаТЧ.ЗаказПоставщику = ЗаказИзШапки;
		КонецЦикла;
	КонецЕсли;
...
Функция ОстаткиТоваровПоЗаказуПокупателя( Заказ, Договор, КонДата, Знач СтатусПартии = Неопределено) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыПокупателей.Остатки(&КонДата, ЗаявкаПокупателя = &Заказ 
...
Функция ОстаткиУслугПоЗаказуПокупателя( Заказ, Договор, Знач КонДата, Знач СтатусПартии = Неопределено) Экспорт
	...
	|	ИЗ
	|		Документ.ЗаявкаПокупателя.Услуги
	|			ГДЕ Ссылка = &Заказ
	|		СГРУППИРОВАТЬ ПО
	|			Номенклатура
	|	) КАК Док
...
Функция ОстаткиТоваровПоЗаказуПоставщику( Заказ, Договор, КонДата, ДопПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса =
		...
		|	НЕОПРЕДЕЛЕНО КАК ЗаявкаПокупателя,
		|	0 КАК КолРазмещено
		|ИЗ
		...
		|	0,
		|	РегРазмещениеЗаказов.ЗаявкаПокупателя,
...
//
Функция ПолучитьОстатокПоВнутреннемуЗаказу( Заказ, Количество, Товар, ЕдиницаИзмерения,
	...
	|		И Номенклатура = &Товар ) КАК ВнутренниеЗаказыОстатки
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьТабЧастьТоварыПоЗаказу(ДокСсылка, ТабЧасть, Заказ, ТабОстатки, ДопПараметры = Неопределено, ОчищатьСтроки = Ложь) Экспорт
	...
	ИмяРеквизитаЗаказ = "ЗаявкаПокупателя";
			...
			Пока Обход.Следующий() Цикл
				...
				Иначе
					...
					Пока НадоРазместить > 0 И МассивОстатков.Количество() > 0 Цикл
						...
						НоваяСтрока[ИмяРеквизитаЗаказ] = Обход.ЗаявкаПокупателя;
...
Процедура ЗаполнитьТабЧастьУслугиПоЗаказу(ДокОбъект, ТабЧасть, Заказ, ТабОстатки, ДопПараметры = Неопределено, ОчищатьСтроки = Ложь) Экспорт
	...
	Для Каждого СтрокаОстатков Из ТабОстатки Цикл
		...
		НоваяСтрока.СтавкаНДС       = СтрокаОстатков.СтавкаНДС;
		//НоваяСтрока.ЗаявкаПокупателя = Заказ;
...
Процедура ЗаполнитьТабЧастьТараПоЗаказуПокупателя(ДокОбъект, ТабЧасть, Заказ, Договор, КонДата, ОчищатьСтроки = Ложь) Экспорт
	...
	|	РегЗаказы.ЕдиницаИзмерения.Коэффициент КАК Коэффициент
	|ИЗ
	|	РегистрНакопления.ЗаказыПокупателей.Остатки(&КонДата, ДоговорКонтрагента = &Договор И СтатусПартии = &Статус И ЗаявкаПокупателя = &Заказ) КАК РегЗаказы
	|
	|ГДЕ
...
//
Процедура ЗаполнитьПоСериям(ЗаявкаПокупателя, Товары, ИмяРеквизитаРазмещение="Размещение", РезервированиеТоваров=ложь) 
	...
	ТаблицаСерий         = УправлениеЗапасами.ПолучитьТаблицуСвободныхОстатковПоСериям(МассивСклады, ЗаявкаПокупателя.Организация, МассивНоменклатуры);
	...
	Для Каждого СтрокаВременнаяТаблица Из ВременнаяТаблица Цикл
		...
		Для Каждого СтрокаОстаток Из НайденныеСтроки Цикл
			...
			ОбработкаТабличныхЧастей.РассчитатьКоличествоМестТабЧасти(НоваяСтрока, ЗаявкаПокупателя);
			Если не РезервированиеТоваров Тогда
				ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(НоваяСтрока, ЗаявкаПокупателя);
				ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(НоваяСтрока, ЗаявкаПокупателя);
			КонецЕсли;
...
//Выполняет авторезервирование и авторазмещение в документах ЗаявкаПокупателя, ВнутреннийЗаказ, 
...
Процедура Заказ_ЗаполнитьТабличныеЧастиВозможнымРазмещением(Параметры, Товары, ВозвратнаяТара) Экспорт
	...
	Если ТипЗнч(ЗаказСсылка)=Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
	...
	Если Параметры.Свойство("РезервированиеТоваров") Тогда
        ...
        ИначеЕсли ТипЗнч(ЗаказСсылка)=Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
			ТекстЗапросаПоНоменклатуре = "
			|	ВЫБРАТЬ
			|		РегистрЗаказыПокупателейОстатки.Номенклатура
			|	ИЗ 
			|		РегистрНакопления.ЗаказыПокупателей.Остатки(, ЗаявкаПокупателя = &СсылкаЗаказ
...
Функция ПодготовитьТаблицуДляЗакрытияРазмещения(ДокСсылка,МоментВремени,СписокЗаказов,РазмещенияВЗаказе,РазмещенияЗаказа) Экспорт
	...
	Если РазмещенияЗаказа Тогда
		ТекстУсловие = ТекстУсловие + ?(ТекстУсловие="",""," ИЛИ ")+" ЗаявкаПокупателя В (&СписокЗаказов) ";
	КонецЕсли;
...
Процедура СформироватьОтчетАнализЗаказа(ЗаказСсылка,флПроверятьЗаписьЗаказа=ложь, флСообщатьОбОшибках) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ЗаказСсылка) 
		ИЛИ (ТипЗнч(ЗаказСсылка) <> Тип("ДокументСсылка.ЗаявкаПокупателя") И
...
// + 20181012 Пушкин XX-1069
Функция ПроверитьЦеныНаУслуги(парам_ТЧ) Экспорт
	лОтказ = Ложь;
	
	лМинЦена = Константы.МинимальнаяЦенаУслуги.Получить();
	Если лМинЦена = 0 тогда лМинЦена = 10 КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ФрмУслуги.НомерСтроки КАК ЧИСЛО(15, 0)) КАК НомерСтроки,
	|	ВЫРАЗИТЬ(ФрмУслуги.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ВЫРАЗИТЬ(ФрмУслуги.Цена КАК ЧИСЛО(15, 2)) КАК Цена,
	|	ВЫРАЗИТЬ(ФрмУслуги.Количество КАК ЧИСЛО(15, 2)) КАК Количество
	|ПОМЕСТИТЬ ФрмУслуги
	|ИЗ
	|	&ФрмУслуги КАК ФрмУслуги
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФрмУслуги.НомерСтроки,
	|	ФрмУслуги.Номенклатура,
	|	ФрмУслуги.Цена,
	|	ФрмУслуги.Количество
	|ИЗ
	|	ФрмУслуги КАК ФрмУслуги
	|ГДЕ
	|	(ФрмУслуги.Цена < &МинЦена
	|			ИЛИ ФрмУслуги.Количество < 1
	|			ИЛИ ФрмУслуги.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))";
	
	
	Если ТипЗнч(парам_ТЧ) = Тип("ТаблицаЗначений") Тогда
		Запрос.УстановитьПараметр("ФрмУслуги", парам_ТЧ.Скопировать(,"НомерСтроки,Номенклатура,Цена,Количество"));
	Иначе
		Запрос.УстановитьПараметр("ФрмУслуги", парам_ТЧ.Выгрузить().Скопировать(,"НомерСтроки,Номенклатура,Цена,Количество"));
	КонецЕсли;
	
	Запрос.УстановитьПараметр("МинЦена", лМинЦена);
	...
	Попытка
		...
		Если Результат.Количество() > 0 тогда
			...
			Сообщить("Данные об Услугах указаны с ошибкой: либо услуга не указана либо количество либо услуга с ценой менее " + СокрЛП(лМинЦена) + "р. Документ не может быть записан.");
			
			Для каждого лстр из Результат цикл
				Сообщить("Номер Строки: " + СокрЛП(лстр.НомерСтроки) + ", Услуга: " + СокрЛП(лстр.Номенклатура) + ", Цена: " + СокрЛП(лстр.Цена) + ", Количество: " + СокрЛП(лстр.Количество));
			КонецЦикла;
...
Функция РедактироватьПоследнююКорректировкуЗаказа() Экспорт
	
	Возврат Константы.РедактироватьПоследнююКорректировкуЗаказа.Получить();
	
КонецФункции

Функция ИспользоватьНоменклатуруЗаказаВПТУ() Экспорт
	
 	Возврат Константы.ИспользоватьНоменклатуруЗаказаВПТУ.Получить();
