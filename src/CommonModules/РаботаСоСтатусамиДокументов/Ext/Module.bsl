...
Процедура ИзменитьПризнакДокументПроверен(вхСсылкаНаДокумент, ПризнакПроверен, вхОтветственный, ПричинаОтмены = "") Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_РаботаСоСтатусамиДокументов_ИзменитьПризнакДокументПроверен";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Иначе
		...
		ПризнакПроверенСтарый = Выборка.ПризнакПроверен;
	...
	//ХудинВВ XX-2668 15072019
	Если ПризнакПроверенСтарый <> ПризнакПроверен Тогда
		 УстановитьСтатусПроверкиДокументовПоставщикавАРВ(ПризнакПроверен, вхСсылкаНаДокумент);
	КонецЕсли;
...
//ХудинВВ XX-2668 15072019
Процедура  УстановитьСтатусПроверкиДокументовПоставщикавАРВ(ДокументыПроверены, вхСсылкаНаДокумент) 
	...
	Если ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Номер, АктРассмотренияВозврата, АктРассмотренияВозврата.СтатусПроверкиДокументовПоставщика");
		Если ЗначениеЗаполнено(Реквизиты.АктРассмотренияВозврата) Тогда
			
			НовыйСтатусПроверки = ?(ДокументыПроверены, Перечисления.АРВ_СтатусыПроверкиДокументовПоставщика.Проверены, Перечисления.АРВ_СтатусыПроверкиДокументовПоставщика.НеПроверены);
			
			Если  ЗначениеЗаполнено(Реквизиты.АктРассмотренияВозврата)
				И НовыйСтатусПроверки <> Реквизиты.АктРассмотренияВозвратаСтатусПроверкиДокументовПоставщика Тогда
				
				АктОбъект =  Реквизиты.АктРассмотренияВозврата.ПолучитьОбъект();
				АктОбъект.СтатусПроверкиДокументовПоставщика = НовыйСтатусПроверки;
				АктОбъект.ДополнительныеСвойства.Вставить("СохранитьВИсториюИзменения", "Статус проверки документов поставщика: "+НовыйСтатусПроверки+" (Возврат поставщику №"+Реквизиты.Номер+")");
				АктОбъект.Записать();
				
				РегистрыСведений.СобытияКОбработкеАктовРассмотренияВозврата.Добавить(Реквизиты.АктРассмотренияВозврата, 
				Перечисления.ВидыСобытийКОбработкеПроцессаВозвратов.ОбработатьЭтапПроцесса,
				АктОбъект.СтатусДокумента);
