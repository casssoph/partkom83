...
Процедура ГраницыЗапретаИзмененияДанныхСВидамиДокументов() 
	
	лКлючАлгоритма = "ОбщийМодуль_ПолныеПрава_ГраницыЗапретаИзмененияДанныхСВидамиДокументов";
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	               |			ТОГДА НЕОПРЕДЕЛЕНО
	               |		ИНАЧЕ ГраницыЗапретаИзмененияДанных.Организация
	               |	КОНЕЦ КАК Организация,
	               |	ГраницыЗапретаИзмененияДанных.ВидДокумента,
	               |	МАКСИМУМ(ВЫБОР
	               |			КОГДА ГраницыЗапретаИзмененияДанных.ГраницаЗапретаИзменений = ДАТАВРЕМЯ(1, 1, 1)
	               |				ТОГДА ГраницыЗапретаИзмененияДанных.ГраницаЗапретаИзменений
	               |			ИНАЧЕ КОНЕЦПЕРИОДА(ГраницыЗапретаИзмененияДанных.ГраницаЗапретаИзменений, ДЕНЬ)
	               |		КОНЕЦ) КАК ГраницаЗапретаИзменений,
	               |	ВЫБОР
	               |		КОГДА ГраницыЗапретаИзмененияДанных.ВидДокумента <> НЕОПРЕДЕЛЕНО
	               |			ТОГДА 1
	               |		ИНАЧЕ 0
	               |	КОНЕЦ + ВЫБОР
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	               ...
	               |		ИНАЧЕ 0
	               |	КОНЕЦ + ВЫБОР
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Пользователь ССЫЛКА Справочник.ГруппыПользователей
	               |				И ГраницыЗапретаИзмененияДанных.Пользователь <> ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ПустаяСсылка)
	               |			ТОГДА 4
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Пользователь ССЫЛКА Справочник.Пользователи
	               |				И ГраницыЗапретаИзмененияДанных.Пользователь <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	               |			ТОГДА 8
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК Рейтинг
	               |ИЗ
	               ...
	               |			ИЛИ ГраницыЗапретаИзмененияДанных.Пользователь В
	               |				(ВЫБРАТЬ
	               |					ГруппыПользователя.Ссылка
	               |				ИЗ
	               |					ГруппыПользователя КАК ГруппыПользователя)
	               |			ИЛИ ГраницыЗапретаИзмененияДанных.Пользователь В (ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка), НЕОПРЕДЕЛЕНО))
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ГраницыЗапретаИзмененияДанных.ВидДокумента,
	               |	ВЫБОР
	...
	ТаблицаГраниц = Запрос.Выполнить().Выгрузить();	
	ТаблицаГраниц.Индексы.Добавить("Организация,ВидДокумента");
	
	ПараметрыСеанса.ГраницыЗапретаИзмененияДанных = Новый ХранилищеЗначения(ТаблицаГраниц, Новый СжатиеДанных(0));
...
Процедура УстановитьПараметрГраницыЗапретаИзмененияДанных(ИмяПараметра = Неопределено, УстановленныеПараметры = Неопределено) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_ПолныеПрава_УстановитьПараметрГраницыЗапретаИзмененияДанных";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	//Голубев. исправил немного
	////Сергеев. Так как регистра ни хрена нет, поэтому всегда возврат
	//Возврат;
	////

	
	//Если РольДоступна("ПолныеПрава") И НЕ Константы.ПрименятьДатуЗапретаДляПолныхПрав.Получить()
	//	ИЛИ РольДоступна("ГлавныйБухгалтер") ИЛИ РольДоступна("ГлавныйБухгалтерПросмотр") Тогда
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2844) Тогда 
		ГраницыЗапретаИзмененияДанныхСВидамиДокументов();
	Иначе
	//)Семенов И.П.
...
Процедура ОпределитьПараметрыСеансаДляОбменаДанными() Экспорт
	...
	ПараметрыСеанса.РасширенныйЛогОбменаПартком77_83 = Константы.РасширенныйЛогОбменаПартком77_83.Получить() 
	И ОбщегоНазначения.ЭтоРабочаяИнформационнаяБаза();	
...
Процедура УстановитьПараметрыСеансаДатыПерехода() Экспорт
	
	ДатаНачалаРаботыВзаиморасчеты = Константы.ДатаНачалаРаботыВзаиморасчеты.Получить();
	ДатаНачалаРаботыТовары = Константы.ДатаНачалаРаботыТовары.Получить();
	
	Если ЗначениеЗаполнено(ДатаНачалаРаботыВзаиморасчеты) Тогда
		ПараметрыСеанса.ДатаНачалаРаботыВзаиморасчеты = ДатаНачалаРаботыВзаиморасчеты;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДатаНачалаРаботыТовары) Тогда
		Если ПараметрыСеанса.ТекущийПользователь = Справочники.Пользователи.ПолучитьСсылку(Новый УникальныйИдентификатор("c6b6c245-fb2c-11e5-80e1-005056817b9c")) Тогда
			ПараметрыСеанса.ДатаНачалаРаботыТовары = '20170701000000';
		Иначе
			ПараметрыСеанса.ДатаНачалаРаботыТовары = ДатаНачалаРаботыТовары;
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыСеанса.ДатаНачалаПроведенияПоРегиструТоварыНаСкладах = 
		Константы.ДатаНачалаПроведенияПоРегиструТоварыНаСкладах.Получить();
	
КонецПроцедуры

Процедура УстановитьОсновныеВалютыУпрУчета() Экспорт
	ПараметрыСеанса.ВалютаРубль 	= Константы.ВалютаРубль.Получить();
	ПараметрыСеанса.ВалютаДоллар 	= Константы.ВалютаДоллар.Получить();
	ПараметрыСеанса.ВалютаЕвро 		= Константы.ВалютаЕвро.Получить();
	
КонецПроцедуры

Процедура УстановитьДатыНачалаРаботы(вхИмя, вхЗначение) Экспорт
	
	Если вхИмя = "ДатаНачалаРаботыВзаиморасчеты" И ЗначениеЗаполнено(вхЗначение) И ТипЗнч(вхЗначение) = Тип("Дата") Тогда
		Константы.ДатаНачалаРаботыВзаиморасчеты.Установить(вхЗначение);
	КонецЕсли;
	
	Если вхИмя = "ДатаНачалаРаботыТовары" И ЗначениеЗаполнено(вхЗначение) И ТипЗнч(вхЗначение) = Тип("Дата") Тогда
		Константы.ДатаНачалаРаботыТовары.Установить(вхЗначение);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьПараметрСеансаИзКонстанты(вхИмяКонстанты) Экспорт
	
	Константа = Метаданные.Константы.Найти(вхИмяКонстанты);
	ПараметрСеанса = Метаданные.ПараметрыСеанса.Найти(вхИмяКонстанты);
	Если Константа <> Неопределено И ПараметрСеанса <> Неопределено Тогда
		ПараметрыСеанса[вхИмяКонстанты] = Константы[вхИмяКонстанты].Получить();
	КонецЕсли;
...
Функция ПроверитьНаличиеСсылокНаДоговорКонтрагента(ДоговорКонтрагента) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрНакопления.Взаиморасчеты.ДоговорКонтрагента
	|ГДЕ
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрНакопления.ЗаявкиПокупателей.ДоговорКонтрагента
	|ГДЕ
	|	ДоговорКонтрагента = &ДоговорКонтрагента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрНакопления.Закупки.ДоговорКонтрагента
	|ГДЕ
	|	ДоговорКонтрагента = &ДоговорКонтрагента
	|";
...
Процедура УстановитьПараметрВариантПроведенияДокументов(ИмяПараметра = Неопределено, УстановленныеПараметры = Неопределено) Экспорт
	ПараметрыСеанса.ВариантПроведенияДокументов = Константы.ВариантПроведенияДокументов.Получить();
КонецПроцедуры

//[Тарасов А.В.]
Процедура ЗаполнитьСписокРегистровЗапрещенныхКПроведению() Экспорт
	
	МассивРегистров = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫРАЗИТЬ(РегистрЗначениеПрав.Значение КАК Справочник.РегистрыЗапрещенныеКПроведению) КАК Значение
	|ИЗ
	...
	|			ВЫБРАТЬ
	|				&Пользователь)
	|	И (ВЫРАЗИТЬ(РегистрЗначениеПрав.Значение КАК Справочник.РегистрыЗапрещенныеКПроведению)) <> ЗНАЧЕНИЕ(Справочник.РегистрыЗапрещенныеКПроведению.ПустаяСсылка)";
	Запрос.УстановитьПараметр("ПравоПользователя", ПланыВидовХарактеристик.ПраваПользователей.СписокРегистровЗапрещенныхКПроведению);
	Запрос.УстановитьПараметр("Пользователь",      ПараметрыСеанса.ТекущийПользователь);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		мСписокРегистров = Выборка.Значение.СписокРегистров;
		Для Каждого мСтрока ИЗ мСписокРегистров Цикл
			Если МассивРегистров.Найти(мСтрока.ИмяРегистра) = Неопределено Тогда
				МассивРегистров.Добавить(мСтрока.ИмяРегистра);
			КонецЕсли;		
		КонецЦикла;
	КонецЦикла;
	
	ПараметрыСеанса.СписокРегистровЗапрещенныхКПроведению = Новый ФиксированныйМассив(МассивРегистров);
...
Функция ОбщаяДатаЗапретаРедактирования() Экспорт 
	Дата = Дата(3999, 12, 31);
	ТаблицаГраниц = ПараметрыСеанса.ГраницыЗапретаИзмененияДанных.Получить();
	Если ТипЗнч(ТаблицаГраниц) = Тип("ТаблицаЗначений") Тогда 
		Строка = ТаблицаГраниц.Найти(0, "Рейтинг");
		Если Строка <> Неопределено Тогда 
			Дата = Строка.ГраницаЗапретаИзменений;
		КонецЕсли;
	КонецЕсли;
	Возврат Дата;
КонецФункции
