...
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено) Экспорт 
	...
	КонецЕсли;
	Если Товары.Колонки.Найти("СписыватьВПределахОстатка") = Неопределено Тогда //Если истина, то списывается не больше чем остаток, затем списывается с физического склада
		Товары.Колонки.Добавить("СписыватьВПределахОстатка", Новый ОписаниеТипов("Булево"));
		Товары.ЗаполнитьЗначения(Ложь, "СписыватьВПределахОстатка");
	КонецЕсли;
	Если Товары.Колонки.Найти("ПоЦенеЗакупки") = Неопределено Тогда //Если истина, то со склада vmi списывается по цене поставщика
		Товары.Колонки.Добавить("ПоЦенеЗакупки", Новый ОписаниеТипов("Булево"));
		Товары.ЗаполнитьЗначения(Ложь, "ПоЦенеЗакупки");
	КонецЕсли;
	Если Товары.Колонки.Найти("ЦенаЗакупки") = Неопределено Тогда //Цена закупки
		КвалификаторыЧисла = Новый КвалификаторыЧисла(10, 2, ДопустимыйЗнак.Любой);
		Товары.Колонки.Добавить("ЦенаЗакупки", Новый ОписаниеТипов("Число",,,КвалификаторыЧисла));
		Товары.ЗаполнитьЗначения(0, "ЦенаЗакупки");
	КонецЕсли;
