...
Процедура ЗарегистрироватьОбъектВОбменПартКом83_TopLog(вхИсточник, вхОтказ)
	...
	Если МетаданныеИсточника = Метаданные.Документы.РеализацияТоваровУслуг И Константы.ОбменСТопЛог_ИспользоватьОтдельныйПотокДляРТУ.Получить() Тогда 
...
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено, ПараметрыСписания = Неопределено) Экспорт 
	...
	СписыватьТолькоПоСвоейОрганизации = Неопределено;
	ПараметрыСписания.Свойство("СписыватьТолькоПоСвоейОрганизации", СписыватьТолькоПоСвоейОрганизации);
	...
	Если ТаблицаПоFIFO.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента), СписыватьТолькоПоСвоейОрганизации);
		СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ПараметрыСписания);
	КонецЕсли;
...
Процедура СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ПараметрыСписания = Неопределено)
...
Функция ПеремещатьНаФизическийСкладВсеОтказыVMI(ДатаДокумента) Экспорт
	...
	Если ЗначениеЗаполнено(ПеремещатьНаФизическийСкладВсеОтказыVMI)
			И ДатаДокумента >= ПеремещатьНаФизическийСкладВсеОтказыVMI Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;	
