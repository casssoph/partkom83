...
Функция ОтменитьРегистрациюОбъектаПоУсловиям(МетаданныеИсточника, вхИсточник)
	...
	ИначеЕсли ВРЕГ(МетаданныеИсточника.Имя) = ВРЕГ("ПоступлениеТоваровУслуг") Тогда
		...
		вхИсточник.ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.Прочее ИЛИ вхИсточник.СтатусДокумента <> Справочники.СтатусыДокументов.ПоступлениеТоваровОтгружен ИЛИ 
...
Процедура ОчиститьДвиженияПриСдвигеДаты(ДокументОбъект, РежимЗаписи, ОчищаемыеРегистры) Экспорт
	...
	Если Не ДокументОбъект.ЭтоНовый() И (РежимЗаписи = РежимЗаписиДокумента.Проведение ИЛИ РежимЗаписи = РежимЗаписиДокумента.Запись и ДокументОбъект.Проведен) Тогда 
		...
		Если ДатаДо < ДокументОбъект.Дата И ДокументОбъект.Дата >= глЗначениеПеременной("ДатаЗапускаПервогоЭтапа") Тогда  
			ПроведениеДокументовКлиентСервер.ОчиститьДвиженияДокумента(ДокументОбъект.Ссылка, ОчищаемыеРегистры);
		КонецЕсли;
	КонецЕсли;
