...
Функция УсловиеПроведенияРазмещенияПТУНаОснованииВозвратаМХ3(вхСсылкаНаДокумент, СозданНаОснованииМХ3, вхСсылкаНаДокументОснование = Неопределено) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ПроведениеДокументовКлиентСервера_УсловиеПроведенияРазмещенияПТУНаОснованииВозвратаМХ3";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если вхСсылкаНаДокументОснование = Неопределено Тогда
		вхСсылкаНаДокументОснование = вхСсылкаНаДокумент.ДокументОснование;
	КонецЕсли;
	
	СозданНаОснованииМХ3 = ТипЗнч(вхСсылкаНаДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") И ЗначениеЗаполнено(вхСсылкаНаДокументОснование);
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "ДатаВходящегоДокумента, ДатаВходящегоДокументаСФ, НомерВходящегоДокумента, НомерВходящегоДокументаСФ");
	ЗаполненыВхНомераДокументов = ЗначениеЗаполнено(Реквизиты.ДатаВходящегоДокумента) И ЗначениеЗаполнено(Реквизиты.ДатаВходящегоДокументаСФ) И ЗначениеЗаполнено(Реквизиты.НомерВходящегоДокумента) И ЗначениеЗаполнено(Реквизиты.НомерВходящегоДокументаСФ);
	
	Возврат НЕ СозданНаОснованииМХ3 Или (СозданНаОснованииМХ3 И ЗаполненыВхНомераДокументов);
