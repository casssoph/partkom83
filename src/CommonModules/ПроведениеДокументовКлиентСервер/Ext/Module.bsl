...
Процедура ЗаписатьДвижения(вхСсылкаНаДокумент, вхОбъектМетаданных, вхДанныеДвижений, вхБазовыеДвижения = Неопределено) Экспорт
	...
	// запись движений
	ОбщегоНазначения.ЗаписатьДвиженияДокументаБезОбработки(вхСсылкаНаДокумент, лМенеджерОбъекта, лТабДвиж, вхДанныеДвижений.Замещать);
...
Функция ОтменитьРегистрациюОбъектаПоУсловиям(МетаданныеИсточника, вхИсточник) Экспорт 
	...
	Если ВРЕГ(МетаданныеИсточника.Имя) = ВРЕГ("Номенклатура") Тогда
		Если вхИсточник.ЭтоГруппа ИЛИ вхИсточник.Услуга Тогда
	...
	ИначеЕсли ВРЕГ(МетаданныеИсточника.Имя) = ВРЕГ("ПоступлениеТоваровУслуг") Тогда
		...
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхИсточник.Склад, "ОбменСTopLog") <> Истина;
	...
	ИначеЕсли ВРЕГ(МетаданныеИсточника.Имя) = ВРЕГ("ПеремещениеТоваров") Тогда
		...
		//СкладПолучательОбменСTopLog = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхИсточник.СкладОтправитель, "ОбменСTopLog");
		//
		//Если СкладОтправительОбменСTopLog И СкладПолучательОбменСTopLog Тогда 
		//	ОтказОтРегистрации = Истина;
		//ИначеЕсли СкладОтправительОбменСTopLog И вхИсточник.СтатусДокумента = Справочники.СтатусыДокументов.ПеремещениеТоваровПоступил Тогда
		//	ОтказОтРегистрации = Ложь;
		//ИначеЕсли СкладПолучательОбменСTopLog И вхИсточник.СтатусДокумента = Справочники.СтатусыДокументов.ПеремещениеТоваровОтгружен Тогда 
		//	ОтказОтРегистрации = Ложь;
		//Иначе
		//	ОтказОтРегистрации = Истина;
		//КонецЕсли;
...
// Партии товаров
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено) Экспорт 
	...
	Если ТаблицаПоСтрокамПриходаПоПоcтавщику.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = Истина;
		СписатьПоСтрокамПриходаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаПоПоcтавщику, ТаблицаПоFIFOПоПоставщику, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	
	Если ТаблицаПоСтрокамПрихода.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = Истина;
		СписатьПоСтрокамПрихода(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	
	Если ТаблицаПоFIFOПоПоставщику.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента);
		СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOПоПоставщику, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	
	Если ТаблицаПоFIFO.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента);
		СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	
	Если ТаблицаПоСтрокамПриходаБезУчетаСтатусаПоПоставщику.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = Истина;
		СписатьПоСтрокамПриходаБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатусаПоПоставщику, ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	КонецЕсли;

	Если ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = Истина;
		СписатьПоСтрокамПриходаБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	КонецЕсли;
	
	Если ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента);
		СписатьПоFIFOБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)	
	КонецЕсли;
	
	Если ТаблицаПоFIFOБезУчетаСтатуса.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента);
		СписатьПоFIFOБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)	
	КонецЕсли;
	...
	Если ТаблицаДвижений.Колонки.Найти("ОприходоватьПоVMI") <> Неопределено Тогда 
		...
		Если СтрокиVMI.Количество() > 0 Тогда 
			...
			Для Каждого СтрокаДвижений Из ТаблицаДвиженийVMI Цикл
				...
				Цена = ПолучитьЦенуНоменклатурыКонтрагента(СтрокаДвижений.ДоговорКонтрагента, СтрокаДвижений.Номенклатура, СтрокаДвижений.Период);
				Если Цена = 0 И СтрокаДвижений.Количество <> 0 Тогда
					Цена = Окр(СтрокаДвижений.СуммаРубли/СтрокаДвижений.Количество, 4);
				КонецЕсли;
				СтрокаДвижений.Цена = Цена; 
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьЦенуНоменклатурыКонтрагента(ДоговорПоставщика, Номенклатура, Дата)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	|			&Период,
	|			ПрайсПоставщика.Владелец = &Владелец
	|				И Номенклатура.Артикул = &Артикул
	|				И Номенклатура.Изготовитель = &Изготовитель) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних"
	);
	
	Поставщик = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ДоговорПоставщика, "Владелец");
	ПараметрыНоменклатуры = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Номенклатура, "Артикул,Изготовитель");
	
	Запрос.УстановитьПараметр("Период", Дата);
	Запрос.УстановитьПараметр("Владелец", Поставщик);
	Запрос.УстановитьПараметр("Артикул", ПараметрыНоменклатуры.Артикул);
	Запрос.УстановитьПараметр("Изготовитель", ПараметрыНоменклатуры.Изготовитель);
...
Функция СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента)
...
Процедура ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, 
												...
												ТаблицаДвижений,
												вхОтказ,	
												ТаблицаПоFIFO = Неопределено, ОтсортированаПоОрганизации = Неопределено) 
	...
	Для Каждого СтрокаТовары Из ТаблицаДляРаспределения Цикл 
		...
		Если ОтсортированаПоОрганизации <> Неопределено Тогда 
			Если ОтсортированаПоОрганизации <> СтрокаТовары.Организация Тогда 
				...
				Если ЭтоСписаниеПоFIFO Тогда 
					ОстаткиПоПартиям.Сортировать("ЭтаОрганизация Убыв, ОрганизацияНаименование, СтрокаПриходаДата")
				Иначе
		...
		Если КоличествоРаспределить > 0 Тогда
			Если Не ЭтоСписаниеПоFIFO Тогда 
...
Процедура СписатьПоСтрокамПриходаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	
	Перем ОтсортированаПоОрганизации;
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, СтатусПартии, Организация, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |				(ВЫБРАТЬ
 ...
 Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, СтатусПартии, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |					(ВЫБРАТЬ
		               |						ТаблицаПоСтрокамПрихода.Номенклатура,
		               |						ТаблицаПоСтрокамПрихода.Склад,
		               |						ТаблицаПоСтрокамПрихода.Качество,
	...
	ПередаваемыеПараметры.Вставить("СписыватьТолькоПоСвоейОрганизации", СписыватьТолькоПоСвоейОрганизации);
	
	ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, ТаблицаПоСтрокамПрихода, ПередаваемыеПараметры, ОстаткиПоПартиям, ТаблицаДвижений, вхОтказ, ТаблицаПоFIFO, ОтсортированаПоОрганизации);
	
КонецПроцедуры

Процедура СписатьПоСтрокамПрихода(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтатусПартии, Организация, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |				(ВЫБРАТЬ
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтатусПартии, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |					(ВЫБРАТЬ
		               |						ТаблицаПоFIFO.Номенклатура,
		               |						ТаблицаПоFIFO.Склад,
		               |						ТаблицаПоFIFO.Качество,
	...
	ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, ТаблицаПоFIFO, ПередаваемыеПараметры, ОстаткиПоПартиям, ТаблицаДвижений, вхОтказ,,ОтсортированаПоОрганизации);
	
КонецПроцедуры
	
Процедура СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоСтрокамПриходаБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, Организация, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |				(ВЫБРАТЬ
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |					(ВЫБРАТЬ
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Номенклатура,
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Склад,
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Качество,
	...
	ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ПередаваемыеПараметры, ОстаткиПоПартиям, ТаблицаДвижений, вхОтказ, ТаблицаПоFIFOБезУчетаСтатуса,ОтсортированаПоОрганизации);

КонецПроцедуры

Процедура СписатьПоСтрокамПриходаБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	ПартииТоваровОстатки.Организация,
		               |	ПартииТоваровОстатки.СтрокаПрихода,
		               |	ПартииТоваровОстатки.СтрокаПрихода.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
		               |	ПартииТоваровОстатки.СтатусПартии,
		               |	СУММА(ПартииТоваровОстатки.КоличествоОстаток) КАК Количество,
		               |	СУММА(ПартииТоваровОстатки.СуммаРублиОстаток) КАК СуммаРубли,
		               |	СУММА(ПартииТоваровОстатки.СуммаДолларыОстаток) КАК СуммаДоллары,
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, СтрокаПрихода.ДоговорКонтрагента.ВидДоговора, Организация) В
		               |				(ВЫБРАТЬ
		               |					ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Номенклатура,
		               |					ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Склад,
		               |					ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Качество,
		               ...
		               |	ПартииТоваровОстатки.Организация,
		               |	ПартииТоваровОстатки.СтрокаПрихода,
		               |	ПартииТоваровОстатки.СтрокаПрихода.ДоговорКонтрагента.ВидДоговора,
		               |	ПартииТоваровОстатки.СтатусПартии,
		               |	ПартииТоваровОстатки.Организация.Наименование
		               |
		               |ДЛЯ ИЗМЕНЕНИЯ
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода, СтрокаПрихода.ДоговорКонтрагента.ВидДоговора) В
		               |					(ВЫБРАТЬ
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Номенклатура,
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Склад,
		               |						ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Качество,
...
Процедура СписатьПоFIFOБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, Организация, СтрокаПрихода.ТорговаяТочка.Владелец) В
		               |				(ВЫБРАТЬ
	...
	ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, ТаблицаПоFIFOБезУчетаСтатуса, ПередаваемыеПараметры, ОстаткиПоПартиям, ТаблицаДвижений, вхОтказ,,ОтсортированаПоОрганизации);
КонецПроцедуры

Процедура СписатьПоFIFOБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	...
	Если СписыватьТолькоПоСвоейОрганизации Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода.ДоговорКонтрагента.ВидДоговора, Организация) В
		               |				(ВЫБРАТЬ
		               |					ТаблицаПоFIFOБезУчетаСтатуса.Номенклатура,
		               |					ТаблицаПоFIFOБезУчетаСтатуса.Склад,
		               |					ТаблицаПоFIFOБезУчетаСтатуса.Качество,
	...
	Иначе
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	РегистрНакопления.ПартииТоваров.Остатки(
		               |			&КонПериода,
		               |			(Номенклатура, Склад, Качество, СтрокаПрихода.ДоговорКонтрагента.ВидДоговора) В
		               |					(ВЫБРАТЬ
		               |						ТаблицаПоFIFOБезУчетаСтатуса.Номенклатура,
		               |						ТаблицаПоFIFOБезУчетаСтатуса.Склад,
		               |						ТаблицаПоFIFOБезУчетаСтатуса.Качество,
...
Функция ЗапрещенныеОрганизацииПродавцы(ОрганизацияПокупатель, ДатаПроведения)
...
Процедура КонтрольОстатков(вхСсылкаНаДокумент, вхОтказ, СУчетомРезерва = Истина, вхПараметры = Неопределено) Экспорт 
	...
	Запрос.Текст + "ВЫБРАТЬ
	               ...
	               |		ПО втТовары.Склад = втРезервы.Склад
	               |			И втТовары.Номенклатура = втРезервы.Номенклатура
...
//Резервы товаров
Процедура СписатьРезервы(Товары, Остатки, ТаблицаДвижений, вхСсылкаНаДокумент) Экспорт
