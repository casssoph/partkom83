...
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено, ПараметрыСписания = Неопределено) Экспорт 
	...
	Если ТаблицаПоСтрокамПриходаПоПоcтавщику.Количество() > 0 Тогда 
		...
		СписатьПоСтрокамПриходаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаПоПоcтавщику, ТаблицаПоFIFOПоПоставщику, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	
	Если ТаблицаПоСтрокамПрихода.Количество() > 0 Тогда 
		//СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, Истина, СписыватьТолькоПоСвоейОрганизации);
		СписатьПоСтрокамПрихода(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	...
	Если ТаблицаПоFIFOПоПоставщику.Количество() > 0 Тогда 
		...
		СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOПоПоставщику, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ);
	КонецЕсли;
	...
	Если ТаблицаПоСтрокамПриходаБезУчетаСтатусаПоПоставщику.Количество() > 0 Тогда 
		//СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, Истина, СписыватьТолькоПоСвоейОрганизации);
		СписатьПоСтрокамПриходаБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатусаПоПоставщику, ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	КонецЕсли;

	Если ТаблицаПоСтрокамПриходаБезУчетаСтатуса.Количество() > 0 Тогда 
		//СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, Истина, СписыватьТолькоПоСвоейОрганизации);
		СписатьПоСтрокамПриходаБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
	КонецЕсли;
	
	Если ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику.Количество() > 0 Тогда 
		//СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента), СписыватьТолькоПоСвоейОрганизации);
		СписатьПоFIFOБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатусаПоПоставщику, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)	
	КонецЕсли;
	
	Если ТаблицаПоFIFOБезУчетаСтатуса.Количество() > 0 Тогда 
		//СписыватьТолькоПоСвоейОрганизации = ?(СписыватьТолькоПоСвоейОрганизации = Неопределено, СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента), СписыватьТолькоПоСвоейОрганизации);
		СписатьПоFIFOБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)	
	КонецЕсли;
...
Процедура СписатьПоСтрокамПриходаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоСтрокамПрихода(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПрихода, ТаблицаПоFIFO, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоСтрокамПриходаБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоСтрокамПриходаБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоСтрокамПриходаБезУчетаСтатуса, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоFIFOБезУчетаСтатусаПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
...
Процедура СписатьПоFIFOБезУчетаСтатуса(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOБезУчетаСтатуса, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ)
