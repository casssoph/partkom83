...
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено, ПараметрыСписания = Неопределено) Экспорт 
	...
	Если ТаблицаПоFIFOПоПоставщику.Количество() > 0 Тогда 
		...
		СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFOПоПоставщику, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ПараметрыСписания);
	КонецЕсли;
	...
	Если СтратегияПогашенияПоОрганизации.ИспользуютсяМФП Тогда
		...
		Если НЕ СтратегияПогашенияПоОрганизации.ПараметрыПолитикиМФП = Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
				...
				СтратегияПогашенияПоОрганизации.ПараметрыПолитикиМФП.УчитыватьОрганизациюТаблицыСписания = Истина;
...
Процедура СписатьПоFIFOПоПоставщику(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СтратегияПогашенияПоОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ПараметрыСписания = Неопределено)
