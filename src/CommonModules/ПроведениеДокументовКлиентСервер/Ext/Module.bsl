...
// Партии товаров
Функция ПогашениеПартийТоваровНовое(вхСсылкаНаДокумент, вхОтказ, КонтрольНаличияПартий = Истина, вхФильтр = Неопределено, Товары = Неопределено, ВызыватьИсключение = Истина) Экспорт 
	...
	Товары = ?(Товары = Неопределено, МенеджерОбъекта.ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр), Товары);
	...
	Если ТаблицаПоFIFO.Количество() > 0 Тогда 
		СписыватьТолькоПоСвоейОрганизации = СтратегияПогашенияПоОрганизации(вхСсылкаНаДокумент, ДатаДокумента);
		СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ВызыватьИсключение);
	КонецЕсли;
...
Процедура ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, 
												...
												ТаблицаДвижений,
												вхОтказ,	
												ТаблицаПоFIFO = Неопределено, ОтсортированаПоОрганизации = Неопределено, ВызыватьИсключение = Истина) 
...
Процедура СписатьПоFIFO(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаПоFIFO, СписыватьТолькоПоСвоейОрганизации, ЗаполняемыеРеквизиты, КонтрольНаличияПартий, вхОтказ, ВызыватьИсключение = Истина)
	...
	ПогашениеПартийТоваровНовоеФрагмент(вхСсылкаНаДокумент, ТаблицаПоFIFO, ПередаваемыеПараметры, ОстаткиПоПартиям, ТаблицаДвижений, вхОтказ,,ОтсортированаПоОрганизации, ВызыватьИсключение);
...
Функция НовоеСписаниеПоПартиямИТоварамДляРазмещений(вхСсылкаНаДокумент) Экспорт  
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ИСТИНА КАК ВнутренееПеремещение,
	               ...
	               |	втТоварыСвернутая.ВнутренееПеремещение,
