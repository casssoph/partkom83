...
Функция ОтменитьРегистрациюОбъектаПоУсловиям(МетаданныеИсточника, вхИсточник) Экспорт 
	...
	ИначеЕсли ВРЕГ(МетаданныеИсточника.Имя) = ВРЕГ("ПеремещениеТоваров") Тогда
		...
		Если вхИсточник.флНеВыгружатьВТопЛог Тогда 
			ОтказОтРегистрации = Истина;
...
Процедура ПередЗаписьюПартииТоваровСохранитьДатуДляМФППередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если НужноСохранитьДатуДляФормированияМФП(Источник, СтруктураПараметров) Тогда
		...
		Для Каждого ДатаКонтроля ИЗ МассивДат Цикл
			РегистрыСведений.ДатыДляСозданияМФП.Добавить(Регистратор, ДатаКонтроля);			
		КонецЦикла;
...
Функция НужноСохранитьДатуДляФормированияМФП(НаборЗаписей, СтруктураПараметров) Экспорт
	...
	СохранитьДатуДляМФП = Ложь;

	Регистратор = НаборЗаписей.Отбор.Регистратор.Значение;
	
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ВидОперации") = Перечисления.ВидыОперацийРеализацияТоваров.МФП Тогда
		СохранитьДатуДляМФП = Истина;
