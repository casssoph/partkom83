...
// Функция преобразует переданный GUID в шестнадцатиричный формат
//
//	Параметры: 
//		GUID - 			GUID в формате строки. 
//
//	Возвращаемое значение:
//		Строка - представление GUID в шестнадцатиричном формате
//
Функция ПреобразоватьGUIDВФорматHEX(GUID) Экспорт
	
	Возврат "0x" + ВРЕГ(Сред(GUID, 20, 4) + Сред(GUID, 25, 12) + Сред(GUID, 15, 4) + Сред(GUID, 10, 4) + Сред(GUID, 1, 8));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	|";
	//Закомментировал попытку Валиахметов 
	//Рудаков. Падает в ошибку при установке параметров ЭД в карточке КА под обычными пользователями.
	//Попытка
...
//**************************************************************************************************************
	// Пока не переносим справочник Характеристика Номенклатуры, но нужно проверить механизм категорий\свойств,
	// поэтому в этой функции заменяем в нужных строчках!!!
	
	//ЭтоХарактеристика = (ТипСсылки = Тип("СправочникСсылка.ХарактеристикиНоменклатуры"));
      ЭтоХарактеристика = (ТипСсылки = Тип("СправочникСсылка.Филиалы"));
	...
	//Запрос.УстановитьПараметр("НазначениеХарактеристика", ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Справочник_ХарактеристикиНоменклатуры);
      Запрос.УстановитьПараметр("НазначениеХарактеристика", ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Справочник_Филиалы);
...
// +++ Карпычев (29.01.18)
Функция ПолучитьОбработкуПечати() Экспорт
	...
	Запрос.УстановитьПараметр("Наименование", "Групповая печать первички");
	Рез = Запрос.Выполнить();
	Если Рез.Пустой() Тогда
		#Если Клиент Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не найдена внешняя обработка ""Групповая печать первички""!";
		Сообщение.Сообщить();
		#КонецЕсли
		Возврат Ложь;
	КонецЕсли;
...
КонецФункции  // --- Карпычев (29.01.18)	
...
Функция ПолучитьЗначениеПеременной(ИмяПараметра, Кэш = Неопределено, КэшИзменен = Ложь) Экспорт
	...
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ЗначенияДополнительныхПравПользователя") Тогда
		НайденноеЗначение = Новый Соответствие;
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("МинимальныеНаценки") Тогда
		НайденноеЗначение = Новый Соответствие;
	...
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаЗаявкиСоздаютсяВ83") Тогда
		НайденноеЗначение = Константы.ДатаЗаявкиСоздаютсяВ83.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("РеализацииВыгружаютсяИз77") Тогда
		НайденноеЗначение = Константы.РеализацииВыгружаютсяИз77.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ПоступленияВыгружаютсяИз77") Тогда
		НайденноеЗначение = Константы.ПоступленияВыгружаютсяИз77.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ПеремещенияВыгружаютсяИз77") Тогда
		НайденноеЗначение = Константы.ПеремещенияВыгружаютсяИз77.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаЗапускаПервогоЭтапа") Тогда
		НайденноеЗначение = Константы.ДатаЗапускаПервогоЭтапа.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("НовоеПроведениеПоВзаиморасчетам") Тогда
		НайденноеЗначение = Константы.НовоеПроведениеПоВзаиморасчетам.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаВозвратыПоставщиковСоздаютсяВ83") Тогда
		НайденноеЗначение = Константы.ДатаВозвратыПоставщиковСоздаютсяВ83.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаЗапускаПроведенияПоПартиямРезервам") Тогда
		НайденноеЗначение = Константы.ДатаЗапускаПроведенияПоПартиямРезервам.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаПереоценкиОстатковПоставщикаСоздаютсяВ83") Тогда
		НайденноеЗначение = Константы.ДатаПереоценкиОстатковПоставщикаСоздаютсяВ83.Получить();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаНачалаРегистрацииОбъектовВТопЛог") Тогда
		Если ОбщегоНазначения.ЭтоРабочаяИнформационнаяБаза() Тогда 
			НайденноеЗначение = Справочники.НастройкиРеквизитовДляОбменов.ДатаНачалаРегистрацииОбъектовВТопЛог.ДатаДляРабочейБазы;
		Иначе
			НайденноеЗначение = Справочники.НастройкиРеквизитовДляОбменов.ДатаНачалаРегистрацииОбъектовВТопЛог.ДатаДляТестовойБазы;
		КонецЕсли;
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаНачалаРазделенияРТУ_СПБ") Тогда
		НайденноеЗначение = Константы.ДатаНачалаРазделенияРТУ_СПБ.Получить();
	//Конец Валиахметов http://jira.part-kom.ru/browse/XX-2529 03.06.201	
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ДатаНачалаИспользованияСтатусовВВозвратеПоставщику") Тогда
		НайденноеЗначение = Константы.ДатаНачалаИспользованияСтатусовВВозвратеПоставщику.Получить();
...
Процедура ДобавитьПрефиксОрганизации(ДокументОбъект, Префикс) Экспорт
	...
	//Если ДокументОбъект.ДополнительныеСвойства.Свойство("Организация") Тогда 
	//	Префикс = ДокументОбъект.ДополнительныеСвойства.Организация.Префикс;
	//Иначе
...
Процедура ПриУстановкеНовогоКодаСправочникаПриУстановкеНовогоКода(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	лОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(Источник));
	Если (лОбъектМетаданных = Метаданные.Справочники.Контрагенты)
...
Функция ЭтоРабочаяИнформационнаяБаза() Экспорт
	
	Возврат (ВРег(СокрЛП(Константы.СтрокаСоединенияИнформационнойБазы.Получить()))
	= ВРег(СтрокаСоединенияИнформационнойБазы()));
	
КонецФункции

Функция НайтиСтрокиТаблицы(вхТаблица, вхФильтр, вхУчитыватьРегистр = Истина) Экспорт
	Если вхУчитыватьРегистр тогда
		Возврат вхТаблица.НайтиСтроки(вхФильтр);
	Иначе
		Результат = Новый Массив;
		лТипСтрока = Тип("Строка");
		Для Каждого лСтрокаТаблицы Из вхТаблица цикл
			лНайдено = Истина;
			Для Каждого лЭлементСтруктуры Из вхФильтр цикл
				лЗначениеТаблицы = лСтрокаТаблицы[лЭлементСтруктуры.Ключ];
				лЗначениеФильтра = лЭлементСтруктуры.Значение;
				лТипТаблицы = ТипЗнч(лЗначениеТаблицы);
				лТипФильтра = ТипЗнч(лЗначениеФильтра);
				Если (лТипТаблицы = лТипСтрока) И (лТипФильтра = лТипСтрока) тогда
					Если (СокрЛП(ВРег(лЗначениеТаблицы)) <> СокрЛП(ВРег(лЗначениеФильтра))) тогда
						лНайдено = Ложь;
						Прервать;
					КонецЕсли;
				ИначеЕсли (лЗначениеТаблицы <> лЗначениеФильтра) тогда
					лНайдено = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если лНайдено тогда
				Результат.Добавить(лСтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
		Возврат Результат;
	КонецЕсли;
...
Функция НайтиСтрокуТаблицы(вхТаблица, вхФильтр, вхУчитыватьРегистр = Истина) Экспорт
	
	Результат = Неопределено;
	лСтрокиТаблицы = НайтиСтрокиТаблицы(вхТаблица, вхФильтр, вхУчитыватьРегистр);
	Если (лСтрокиТаблицы.Количество() > 0) тогда
		Результат = лСтрокиТаблицы[0];
	КонецЕсли;
...
Функция ПодсистемаСуществует(ПолноеИмяПодсистемы) Экспорт
	...
	ИменаПодсистем = Новый Соответствие;
  	...
  	ВставитьИменаПодчиненныхПодсистем(ИменаПодсистем, Метаданные);
...
Процедура ПолучитьПараметрыПроведения(вхСсылкаНаДокумент, вхРегистр, выхМетаданныеРегистра, выхМенеджерОбъекта)
	
	лТипДокумента = ТипЗнч(вхСсылкаНаДокумент);
	лМетаданныеДокумента = Метаданные.НайтиПоТипу(лТипДокумента);
	Если лМетаданныеДокумента.Проведение <> Метаданные.СвойстваОбъектов.Проведение.Разрешить тогда
		ВызватьИсключение "[ПолучитьПараметрыПроведения]: проведение документов """ + лМетаданныеДокумента.Имя + """ запрещено.";
	КонецЕсли;
		
	лТипРегистра = ТипЗнч(вхРегистр);
	лМетаданныеРегистра = Неопределено;
	Если (лТипРегистра = Тип("Строка")) тогда
		лМетаданныеРегистра = Метаданные.РегистрыНакопления.Найти(вхРегистр);
		Если (лМетаданныеРегистра <> Неопределено) тогда
			лМенеджерОбъекта = РегистрыНакопления[лМетаданныеРегистра.Имя];
		Иначе
			лМетаданныеРегистра = Метаданные.РегистрыСведений.Найти(вхРегистр);
			Если (лМетаданныеРегистра <> Неопределено) тогда
				лМенеджерОбъекта = РегистрыСведений[лМетаданныеРегистра.Имя];
			Иначе
				лМетаданныеРегистра = Метаданные.РегистрыБухгалтерии.Найти(вхРегистр);
				Если (лМетаданныеРегистра <> Неопределено) тогда
					лМенеджерОбъекта = РегистрыБухгалтерии[лМетаданныеРегистра.Имя];
				Иначе
					лМетаданныеРегистра = Метаданные.РегистрыРасчета.Найти(вхРегистр);
					Если (лМетаданныеРегистра <> Неопределено) тогда
						лМенеджерОбъекта = лМетаданныеРегистра[лМетаданныеРегистра.Имя];
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли (лТипРегистра = Тип("ОбъектМетаданных")) тогда
		Если Метаданные.РегистрыНакопления.Содержит(вхРегистр) тогда
			лМетаданныеРегистра = вхРегистр;
			лМенеджерОбъекта = РегистрыНакопления[лМетаданныеРегистра.Имя];
		ИначеЕсли Метаданные.РегистрыСведений.Содержит(вхРегистр) тогда
			лМетаданныеРегистра = вхРегистр;
			лМенеджерОбъекта = РегистрыСведений[лМетаданныеРегистра.Имя];
		ИначеЕсли Метаданные.РегистрыБухгалтерии.Содержит(вхРегистр) тогда
			лМетаданныеРегистра = вхРегистр;
			лМенеджерОбъекта = РегистрыБухгалтерии[лМетаданныеРегистра.Имя];
		ИначеЕсли Метаданные.РегистрыРасчета.Содержит(вхРегистр) тогда
			лМетаданныеРегистра = вхРегистр;
			лМенеджерОбъекта = РегистрыРасчета[лМетаданныеРегистра.Имя];
		КонецЕсли;
	КонецЕсли;
	
	Если (лМетаданныеРегистра = Неопределено) тогда
		ВызватьИсключение "[ПолучитьПараметрыПроведения]: неправильный параметр номер 2.";
	КонецЕсли;
		
	// проверяем, входит ли регистр в движения документа
	Если НЕ лМетаданныеДокумента.Движения.Содержит(лМетаданныеРегистра) тогда
		ВызватьИсключение "[ПолучитьПараметрыПроведения]: регистр не входит в набор движений документа.";
	КонецЕсли;
	
	выхМетаданныеРегистра = лМетаданныеРегистра;
	выхМенеджерОбъекта = лМенеджерОбъекта;
	
КонецПроцедуры

Процедура СоздатьСтруктуруРегистраНакопления(вхРегистр, выхТабДвиж) Экспорт
	
	лТип = ТипЗнч(вхРегистр);
	лМетаданныеРегистра = Неопределено;
	Если (лТип = Тип("Строка")) тогда
		лМетаданныеРегистра = Метаданные.РегистрыНакопления.Найти(вхРегистр);
	ИначеЕсли (лТип = Тип("ОбъектМетаданных")) И Метаданные.РегистрыНакопления.Содержит(вхРегистр) тогда
		лМетаданныеРегистра = вхРегистр;
	КонецЕсли;
	
	Если (лМетаданныеРегистра = Неопределено) тогда
		ВызватьИсключение "[СоздатьСтруктуруРегистраНакопления]: неправильный параметр номер 1.";
	КонецЕсли;
	...
	Если (лМетаданныеРегистра.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки) тогда
		выхТабДвиж.Колонки.Добавить("ВидДвижения", лМетаданныеРегистра.СтандартныеРеквизиты.ВидДвижения.Тип);
	КонецЕсли;
...
Процедура ОтчетПоОстаткам(Элемент) Экспорт
	...
	//заполняем список отбираемых значений
	СписокОтбираемыхЗначений = Новый СписокЗначений;
	Если ТипЗнч(Элемент) = Тип("СписокЗначений") Тогда
		МетаданныеИмя = Элемент[0].Значение.Метаданные().Имя;
		СписокОтбираемыхЗначений = Элемент;
	ИначеЕсли ТипЗнч(Элемент) = Тип("ВыделенныеСтрокиТабличногоПоля") Тогда
		МетаданныеИмя = Элемент[0].Ссылка.Метаданные().Имя;
		Для каждого ВыделеннаяСтрока Из Элемент Цикл
			СписокОтбираемыхЗначений.Добавить(ВыделеннаяСтрока.Ссылка);
		КонецЦикла;
	Иначе
		МетаданныеИмя = Элемент.Метаданные().Имя;
		СписокОтбираемыхЗначений.Добавить(Элемент);
	КонецЕсли;
	
	//формируем список отбираемых значений с учетом иерархии
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка ИЗ Справочник." + МетаданныеИмя + " ГДЕ Ссылка В ИЕРАРХИИ (&СписокОтбираемыхЗначений)");
	Запрос.УстановитьПараметр("СписокОтбираемыхЗначений", СписокОтбираемыхЗначений);
	СписокОтбираемыхЗначений.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
	//определяем настройки вывода отчета
	ИмяКлючевогоПоля = "Номенклатура";
	ИменаГруппировок = "Номенклатура.Артикул,Номенклатура.Изготовитель,Склад";
		
	//настраиваем универсальный отчет
	УниверсальныйОтчет.ДобавитьОтбор(ИмяКлючевогоПоля, Истина, ВидСравнения.ВСписке, СписокОтбираемыхЗначений);
	УниверсальныйОтчет.ДобавитьИзмерениеСтроки(ИмяКлючевогоПоля);
	
	ИменаГруппировок = СтрЗаменить(ИменаГруппировок, ",", Символы.ПС);
	Для н=1 По СтрЧислоСтрок(ИменаГруппировок) Цикл
		УниверсальныйОтчет.ДобавитьИзмерениеСтроки(СтрПолучитьСтроку(ИменаГруппировок, н));
	КонецЦикла;
	
	ПоказательРодитель = УниверсальныйОтчет.Показатели.Строки.Найти("Количество");
	Для каждого ПоказательЭлемент Из ПоказательРодитель.Строки Цикл
		Если Не ПоказательЭлемент.Имя = "КоличествоКонечныйОстаток" Тогда
			ПоказательЭлемент.Использование = Ложь;
		КонецЕсли; 
	КонецЦикла; 
	
	Форма = УниверсальныйОтчет.ПолучитьФорму("Форма");
	Форма.ОтчетОбъект.ДатаКон=ТекущаяДата();
	Форма.ОтчетОбъект.ДатаНач=ТекущаяДата();
	Форма.ОбновитьОтчет();
	Форма.Открыть();
КонецПроцедуры

Процедура СоздатьСтруктуруРегистраСведений(вхРегистр, выхТабДвиж) Экспорт
	...
	Если (лМетаданныеРегистра = Неопределено) тогда
		ВызватьИсключение "[СоздатьСтруктуруРегистраСведений]: неправильный параметр номер 1.";
	КонецЕсли;
...
Функция СоздатьСтруктуруПоследовательности(вхПоследовательность) Экспорт
	...
	Если (лМетаданныеПоследовательности = Неопределено) тогда
		ВызватьИсключение "[СоздатьСтруктуруПоследовательности]: неправильный параметр номер 1.";
	КонецЕсли;
	
	лДанныеПоследовательности = ПроведениеДокументовКлиентСервер.ДанныеСтруктурыПоследовательности(лМетаданныеПоследовательности);
	Результат = Новый ТаблицаЗначений;
	Для Каждого лЭлементСтруктуры Из лДанныеПоследовательности цикл
		Результат.Колонки.Добавить(лЭлементСтруктуры.Ключ, лЭлементСтруктуры.Значение);
	КонецЦикла;                    	
...
Процедура ЗаписатьДвиженияДокументаБезОбработки(вхСсылкаНаДокумент, вхМенеджерОбъекта, вхТабДвиж, вхЗамещать = Истина, вхПараметры = Неопределено) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначения_ЗаписатьДвиженияДокументаБезОбработки";
	...
	лНаборЗаписей = вхМенеджерОбъекта.СоздатьНаборЗаписей();
	Если вхПараметры <> Неопределено И ТипЗнч(вхПараметры) = Тип("Структура") Тогда
		лПараметры = вхПараметры;
		Если вхПараметры.Свойство("ДанныеОбъекта") Тогда
			лПараметры = вхПараметры.ДанныеОбъекта;
		КонецЕсли;		
		Если лПараметры.Свойство("СнятьОграничениеПоДатеЗапрета") ИЛИ лПараметры.Свойство("РежимБога") Тогда
			лНаборЗаписей.ДополнительныеСвойства.Вставить("РежимБога", Истина);
			лНаборЗаписей.ДополнительныеСвойства.Вставить("СнятьОграничениеПоДатеЗапрета", Истина); //ХудинВВ XX-3070 05082019
		КонецЕсли;
		
		//ХудинВВ 31052019 XX-2357 ((
		СтатусДокументаСтароеЗначение = Неопределено;
		Если лПараметры.Свойство("СтатусДокументаСтароеЗначение", СтатусДокументаСтароеЗначение) Тогда
			лНаборЗаписей.ДополнительныеСвойства.Вставить("СтатусДокументаСтароеЗначение", СтатусДокументаСтароеЗначение);
		КонецЕсли;
		//)
	...
	лНаборЗаписей.Загрузить(вхТабДвиж);
	лНаборЗаписей.Записать(вхЗамещать);
	
КонецПроцедуры
	
Процедура ЗаписатьДвиженияДокумента(вхСсылкаНаДокумент, вхРегистр, вхТабДвиж, вхЗамещать = Истина) Экспорт
	
	Перем лМетаданныеРегистра;
	Перем лМенеджерОбъекта;
	
	ПолучитьПараметрыПроведения(вхСсылкаНаДокумент, вхРегистр, лМетаданныеРегистра, лМенеджерОбъекта);
	...
	Для Каждого лКолонка Из вхТабДвиж.Колонки цикл
		Если (лМетаданныеРегистра.Ресурсы.Найти(лКолонка.Имя) = Неопределено) тогда
	...
	Если НЕ ПустаяСтрока(лГруппировки) тогда
		лГруппировки = Лев(лГруппировки, СтрДлина(лГруппировки) - 1);
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(лРесурсы) тогда
		лРесурсы = Лев(лРесурсы, СтрДлина(лРесурсы) - 1);
	КонецЕсли;
	
	вхТабДвиж.Свернуть(лГруппировки, лРесурсы);
	
	// удаляем незначащие движения
	Если Метаданные.РегистрыНакопления.Содержит(лМетаданныеРегистра) тогда
		лСтрокиУдаления = вхТабДвиж.НайтиСтроки(лФильтр);	
		Для Каждого лСтрокаУдаления Из лСтрокиУдаления цикл
			вхТабДвиж.Удалить(лСтрокаУдаления);
		КонецЦикла;
	КонецЕсли;
	
	//Добавлено Валиахметов А.А. 06.12.2017
	//РегистрыИзмененныхДвижений = ПроведениеДокументовКлиентСервер.РегистрыИзмененныхДвижений();
	//
	//Если РегистрыИзмененныхДвижений.Получить(лМетаданныеРегистра) <> Неопределено Тогда 
		//ТаблицаДвижений = Неопределено;
		//ОбщегоНазначения.СоздатьСтруктуруРегистраНакопления(лМетаданныеРегистра, ТаблицаДвижений);
		//ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(вхТабДвиж, ТаблицаДвижений);
	//	
	//	ПроведениеДокументовКлиентСервер.ЗаписатьИзмененныеДвижения(вхСсылкаНаДокумент, , ТаблицаДвижений, лМетаданныеРегистра); 
	//Иначе
		ЗаписатьДвиженияДокументаБезОбработки(вхСсылкаНаДокумент, лМенеджерОбъекта, вхТабДвиж, вхЗамещать);	
	//КонецЕсли;
	
КонецПроцедуры

Функция МенеджерОбъектаПоМетаданным(вхОбъектМетаданных) Экспорт
	
	Если Метаданные.Справочники.Содержит(вхОбъектМетаданных) тогда
		Возврат Справочники[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.Документы.Содержит(вхОбъектМетаданных) тогда
		Возврат Документы[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.Последовательности.Содержит(вхОбъектМетаданных) тогда
		Возврат Последовательности[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.БизнесПроцессы.Содержит(вхОбъектМетаданных) тогда
		Возврат БизнесПроцессы[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(вхОбъектМетаданных) тогда
		Возврат ПланыВидовХарактеристик[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.ПланыСчетов.Содержит(вхОбъектМетаданных) тогда
		Возврат ПланыСчетов[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(вхОбъектМетаданных) тогда
		Возврат ПланыВидовРасчета[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.Задачи.Содержит(вхОбъектМетаданных) тогда
		Возврат Задачи[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.ПланыОбмена.Содержит(вхОбъектМетаданных) тогда
		Возврат ПланыОбмена[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.Перечисления.Содержит(вхОбъектМетаданных) тогда
		Возврат Перечисления[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.РегистрыСведений.Содержит(вхОбъектМетаданных) тогда
		Возврат РегистрыСведений[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.РегистрыНакопления.Содержит(вхОбъектМетаданных) тогда
		Возврат РегистрыНакопления[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.РегистрыБухгалтерии.Содержит(вхОбъектМетаданных) тогда
		Возврат РегистрыБухгалтерии[вхОбъектМетаданных.Имя];
		
	ИначеЕсли Метаданные.РегистрыРасчета.Содержит(вхОбъектМетаданных) тогда
		Возврат РегистрыРасчета[вхОбъектМетаданных.Имя];
...
Функция ВидОбъектаПоМетаданным(вхОбъектМетаданных) Экспорт
...
Функция ЗначенияРеквизитовТабличнойЧастиОбъекта(Ссылка, ТабличнаяЧасть, Реквизиты) Экспорт
	...
	|" + ТекстПолей + "
	|ИЗ
	|	" + Ссылка.Метаданные().ПолноеИмя() + "." + ТабличнаяЧасть + " КАК ПсевдонимЗаданнойТаблицы
	|ГДЕ
	|	ПсевдонимЗаданнойТаблицы.Ссылка = &Ссылка
	|УПОРЯДОЧИТЬ ПО
	|	ПсевдонимЗаданнойТаблицы.НомерСтроки";
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции
	
Функция НормализоватьАртикул(вхАртикул) Экспорт
	
	лНедопустимыеСимволы = ",.-/\_'*#""";
	лРусскиеСимволы = "АВЕКМНОРСТХУавекмнорстху";
	лЛатинскиеСимволы = "ABEKMHOPCTXYabekmhopctxy";
	
	Результат = "";
	Для к = 1 по СтрДлина(вхАртикул) цикл
		лСимвол = Сред(вхАртикул, к, 1);
		лКодСимвола = КодСимвола(лСимвол);
		Если (лКодСимвола <= 32) ИЛИ (Найти(лНедопустимыеСимволы, лСимвол) > 0) тогда
			Продолжить;
		Иначе
			лИндекс = Найти(лРусскиеСимволы, лСимвол);
			Если (лИндекс > 0) тогда
				Результат = Результат + Сред(лЛатинскиеСимволы, лИндекс, 1);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		Результат = Результат + ВРег(лСимвол);
	КонецЦикла;
...
Функция ЭтоПрограммныйИдентификатор(вхСтрока, вхРазрешитьТочки = Ложь) Экспорт
	
	лПодстроки = РазложитьСтрокуВМассивПодстрок(вхСтрока, ".");
	Если НЕ вхРазрешитьТочки И (лПодстроки.Количество() <> 1) тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого лПодстрока Из лПодстроки цикл
		Попытка
			тмп = Новый Структура(лПодстрока);
			Если (тмп.Количество() <> 1) тогда
				Возврат Ложь;
			КонецЕсли;			
		Исключение
			Возврат Ложь;
...
Функция ОписаниеТиповСсылкаПоОбъектуМетаданных(вхОбъектМетаданных) Экспорт
	
	Результат = Неопределено;
	Если Метаданные.Справочники.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("СправочникСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.Документы.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ДокументСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.ПланыОбмена.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ПланОбменаСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.Перечисления.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ПеречислениеСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.ПланыСчетов.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ПланСчетовСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ПланВидовРасчетаСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.БизнесПроцессы.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("БизнесПроцессСсылка." + вхОбъектМетаданных.Имя);
	ИначеЕсли Метаданные.Задачи.Содержит(вхОбъектМетаданных) тогда
		Результат = Новый ОписаниеТипов("ЗадачаСсылка." + вхОбъектМетаданных.Имя);
	КонецЕсли;
...
Функция ИспользоватьПогашениеПоРасчетнымДокументам(вхДоговорКонтрагента) Экспорт
	
	Если НЕ ЗначениеЗаполнено(вхДоговорКонтрагента)
		ИЛИ (ТипЗнч(вхДоговорКонтрагента) <> Тип("СправочникСсылка.ДоговорыКонтрагентов")) тогда
		ВызватьИсключение "[ИспользоватьПогашениеПоРасчетнымДокументам]: неправильный параметр #1.";
	КонецЕсли;
	
	лКонтрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхДоговорКонтрагента, "Владелец");
	Если НЕ ЗначениеЗаполнено(лКонтрагент) тогда
		ВызватьИсключение "[ИспользоватьПогашениеПоРасчетнымДокументам]: в договоре контрагента не указан владелец.";
	КонецЕсли;
	
	лОрганизация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(лКонтрагент, "Организация");
	Возврат (НЕ ЗначениеЗаполнено(лОрганизация));
	
КонецФункции

Процедура УдалитьДвиженияПриПометкеУдаления(ДокументОбъект, РежимЗаписи) Экспорт 
	
	Если Не ДокументОбъект.ЭтоНовый() И РежимЗаписи = РежимЗаписиДокумента.Запись И ДокументОбъект.ПометкаУдаления Тогда 
...
//Обмен данными//
Функция МетаданныеПланаОбмена(ПланОбмена) Экспорт
	
	Тип = ТипЗнч(ПланОбмена);
	Если Тип = Тип("Строка") тогда
		МетаданныеПланаОбмена = Метаданные.ПланыОбмена.Найти(ПланОбмена);
	ИначеЕсли (Тип = Тип("ОбъектМетаданных")) И Метаданные.ПланыОбмена.Содержит(ПланОбмена) тогда
		МетаданныеПланаОбмена = ПланОбмена;
	Иначе
		МетаданныеПланаОбмена = Неопределено;
	КонецЕсли;
	
	Возврат МетаданныеПланаОбмена;
	
КонецФункции

Процедура УдалитьКолонки(ТаблицаИсточник, ТаблицаПриемник) Экспорт 
	КоличествоКолонок = ТаблицаПриемник.Колонки.Количество();
	Для Шаг = 0 По КоличествоКолонок - 1 Цикл 
		лКолонка = ТаблицаПриемник.Колонки.Получить(КоличествоКолонок - 1 - Шаг);
		Если ТаблицаИсточник.Колонки.Найти(лКолонка.Имя) = Неопределено Тогда 
			ТаблицаПриемник.Колонки.Удалить(лКолонка);	
		КонецЕсли;		
	КонецЦикла;
КонецПроцедуры

Процедура СменаНомераДокументаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт 
	
	Если Источник.ЭтоНовый() Или Источник.ОбменДанными.Загрузка ИЛИ Источник.ДополнительныеСвойства.Свойство("НеМенятьНомер") Тогда //Семенов 14.12.2018
		Возврат;
	КонецЕсли;
	
	Если ЕстьРеквизитДокумента("Организация", Источник.Метаданные()) Тогда 
		Если Источник.Организация <> ЗначениеРеквизитаОбъекта(Источник.Ссылка, "Организация") Тогда 
...
// Добавлено Валиахметов А.А. ноябрь 2017 перенесено из 7.7.
Функция СформироватьШК_дляДок(ТекДок) Экспорт
	
	тмпНомер = СокрЛП(ТекДок.Номер);
	Организация  = ТекДок.Организация;
	тмпПрефиксНомеровДокументов = СокрЛП(Организация.Префикс);
	лок_ПрефиксИБ = ПолныеПрава.ПрефиксУзлаРаспределеннойИнформационнойБазы();
	КодОрганизации = СтроковыеФункцииКлиентСервер.УбратьЛидирующиеНули(Организация.Код);
	
	Если Лев(тмпНомер, СтрДлина(лок_ПрефиксИБ+тмпПрефиксНомеровДокументов)) = лок_ПрефиксИБ + тмпПрефиксНомеровДокументов Тогда
		// новая нумерация - ПрефиксИБ + ПрефиксФирмы
		// с 16.10.2015 префиксИБ не учитываем (из-за того, что он стал русской буквой, а компонента все равно его не учитывает)
		//тмпНомер   = СокрЛП(ЮрЛицо.Код) + Сред(тмпНомер, СтрДлина(лок_ПрефиксИБ + тмпПрефиксНомеровДокументов) + 1);
		// с 11.01.2016 вернули печать префикса, т.к. он стал "англ"
		//24.05.2018 в 8ке префикс - кририллический, так что убираем
		//тмпНомер = лок_ПрефиксИБ + СокрЛП(КодОрганизации) + Сред(тмпНомер, СтрДлина(лок_ПрефиксИБ + тмпПрефиксНомеровДокументов) + 1);
		тмпНомер = СокрЛП(КодОрганизации) + Сред(тмпНомер, СтрДлина(лок_ПрефиксИБ + тмпПрефиксНомеровДокументов) + 1);
	ИначеЕсли Лев(тмпНомер, СтрДлина(тмпПрефиксНомеровДокументов)) = тмпПрефиксНомеровДокументов Тогда
		// старая нумерация - ПрефиксИБ не использовался
		тмпНомер = СокрЛП(КодОрганизации) + Сред(тмпНомер, СтрДлина(тмпПрефиксНомеровДокументов) + 1);
	Иначе	
		тмпНомер=СокрЛП(КодОрганизации)+Сред(тмпНомер,СтрДлина(лок_ПрефиксИБ+тмпПрефиксНомеровДокументов)+1);
	КонецЕсли;
	
	Если ТипЗнч(ТекДок) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		тмпНомер = "pkdrn" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.СчетФактураВыданный")  Тогда
		тмпНомер = "pkdsf" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.СлужебноеЗадание") Тогда
		тмпНомер = "pkdzd" + тмпНомер;
	//ИначеЕсли ТекДок.Вид() = "СлужебноеЗадание_Снабжение" Тогда
	//	тмпНомер = "pkdzs" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")Тогда
		тмпНомер = "pkdvs" + тмпНомер;
	//ИначеЕсли ТекДок.Вид() = "АктВозврата_АТ" Тогда
	//	тмпНомер = "pkdav" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
		тмпНомер = "pkdza" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		тмпНомер = "pkdzp" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		тмпНомер = "pkdpr" + тмпНомер;
	ИначеЕсли ТипЗнч(ТекДок) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
		тмпНомер = "pkdvp" + тмпНомер;
	Иначе
		//Сооб("Не определен вид документа для Шрих-кодирования!", 10);
		Возврат "";
	КонецЕсли;
	
	Возврат тмпНомер;
КонецФункции	// глСформироватьШК_дляДок
// Конец Добавлено Валиахметов А.А.

Функция ПолучитьПоследнююКорректировкуЗаказа(Заказ) Экспорт
	Запрос = Новый Запрос();
	Запрос.Текст = "
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаЗаказаПоставщику.ДокументОснование КАК ДокументОснование,
	|	МАКСИМУМ(КорректировкаЗаказаПоставщику.Ссылка) КАК ПослКорр
	|ПОМЕСТИТЬ втДатыПослКорр
	|ИЗ
	|	Документ.КорректировкаЗаказаПоставщику КАК КорректировкаЗаказаПоставщику
	|ГДЕ
	|	КорректировкаЗаказаПоставщику.ДокументОснование В
	|			(ВЫБРАТЬ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаЗаказаПоставщику.ДокументОснование
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОснование,
	|	ПослКорр
	...
	|	ЗаказыПоставщикам.Ссылка,
	|	ВЫБОР
	|		КОГДА втДатыПослКорр.ПослКорр ЕСТЬ NULL
	|			ТОГДА ЗаказыПоставщикам.Ссылка
	|		ИНАЧЕ втДатыПослКорр.ПослКорр
	|	КОНЕЦ КАК ПоследняяКорректировка
	|ИЗ
	|	ЗаказыПоставщикам КАК ЗаказыПоставщикам
	|		ЛЕВОЕ СОЕДИНЕНИЕ втДатыПослКорр КАК втДатыПослКорр
	|		ПО ЗаказыПоставщикам.Ссылка = втДатыПослКорр.ДокументОснование";
	
	ЗаказыПоставщикам = Новый ТаблицаЗначений;
	ЗаказыПоставщикам.Колонки.Добавить("Ссылка",Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
	Стр = ЗаказыПоставщикам.Добавить();
	Если СОКРЛП(ТипЗНЧ(Заказ)) = "Корректировка заказа поставщику" Тогда
		Стр.Ссылка = Заказ.ДокументОснование.Ссылка; 
	Иначе
		Стр.Ссылка = Заказ.Ссылка; 
	КонецЕсли;
	Запрос.УстановитьПараметр("ЗаказыПоставщикам",ЗаказыПоставщикам);
	
	Выборка = Запрос.Выполнить().Выгрузить();
	Если Выборка.Количество() > 0 Тогда
		Возврат Выборка[0].ПоследняяКорректировка;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
КонецФункции

Функция ПолучитьПоследнююКорректировкуЗаявки(Заявка) Экспорт
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	Заявки.Ссылка КАК Ссылка
	               |ПОМЕСТИТЬ втЗаявки
	               |ИЗ
	               |	&ЗаявкиПокупателя КАК Заявки
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Ссылка
	               |;
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	КорректировкаЗаявкиПокупателя.ДокументОснование КАК ДокументОснование,
	               |	МАКСИМУМ(КорректировкаЗаявкиПокупателя.Ссылка) КАК ПослКорр
	               |ПОМЕСТИТЬ втДатыПослКорр
	               |ИЗ
	               |	Документ.КорректировкаЗаявкиПокупателя КАК КорректировкаЗаявкиПокупателя
	               |ГДЕ
	               |	КорректировкаЗаявкиПокупателя.ДокументОснование В
	               |			(ВЫБРАТЬ
	               |				втЗаявки.Ссылка
	               |			ИЗ
	               |				втЗаявки)
	               ...
	               |	втЗаявки КАК Заявки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втДатыПослКорр КАК втДатыПослКорр
	               |		ПО Заявки.Ссылка = втДатыПослКорр.ДокументОснование";
	
	ЗаявкиПокупателя = Новый ТаблицаЗначений;
	ЗаявкиПокупателя.Колонки.Добавить("Ссылка",Новый ОписаниеТипов("ДокументСсылка.ЗаявкаПокупателя"));
	Стр = ЗаявкиПокупателя.Добавить();
	Если СОКРЛП(ТипЗНЧ(Заявка)) = "Корректировка заявки покупателя" Тогда
		Стр.Ссылка = Заявка.ДокументОснование.Ссылка; 
	Иначе
		Стр.Ссылка = Заявка.Ссылка; 
	КонецЕсли;
	Запрос.УстановитьПараметр("ЗаявкиПокупателя",ЗаявкиПокупателя);
...
Функция СкладНедостач(Склад) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначения_СкладНедостач";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	ОсновнойСклад = Константы.СкладОсновной.Получить();
	
	Если Склад = ОсновнойСклад Тогда 
		Возврат Константы.СкладНедостач.Получить();
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "СкладНедостач");
	
КонецФункции

Функция СкладНедостачVMI(Склад) Экспорт 
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначения_СкладНедостачVMI";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	Склады.СкладДубль = &СкладДубль";
	Запрос.УстановитьПараметр("СкладДубль", Склад);
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() Тогда 
		ВызватьИсключение "Не найден склад со складом-дублем: " + Склад;
	Иначе
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		//Возврат Выборка.Ссылка;
		//Валиахметов 27.03.2019  http://jira.part-kom.ru/browse/XX-2091
		Возврат ОбщегоНазначения.СкладНедостач(Выборка.Ссылка);
		//Конец Валиахметов  27.03.2019 
	КонецЕсли;

КонецФункции

Функция ПолучитьТТПополнениеСклада(вхСсылкаНаДокумент) Экспорт 
	Перем СкладВДокументе;
	
	Если ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда 
		СкладВДокументе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхСсылкаНаДокумент, "СкладПолучатель");
	Иначе
		//Добавлено 25.01.2019 Валиахметов А.А. Упростили условие для ПТУ, XX-1709 Исправить подбор склада (ТТ.пополнения склада) для виртуальных ssid 
		СкладВДокументе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхСсылкаНаДокумент, "Склад");
	КонецЕсли;
	
	Склад = ПолучитьТТПополнениеСкладаФрагмент(вхСсылкаНаДокумент, СкладВДокументе);
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "КонтрагентПополнениеСклада.ОсновнаяТорговаяТочка");
	
КонецФункции

Функция ПолучитьТТПополнениеСкладаФрагмент(Знач вхСсылкаНаДокумент, Знач СкладВДокументе)
	
	НеИмеетСобственногоСтока = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СкладВДокументе, "НеИмеетСобственногоСтока");
	
	Если НеИмеетСобственногоСтока Тогда 
		Склад = Константы.СкладОсновной.Получить(); 	
	Иначе
...
Процедура ЗаполнитьДопСвойстваДокумента(вхОбъект, РежимЗаписи) Экспорт
	вхОбъект.ДополнительныеСвойства.Вставить("ЭтоНовый", вхОбъект.ЭтоНовый());
	вхОбъект.ДополнительныеСвойства.Вставить("РежимЗаписи", РежимЗаписи);
КонецПроцедуры

// Процедура - Установить период динамического списка по умолчанию
// Параметры:
//  ЭлСписок - Элементформы	 
//
Процедура УстановитьПериодДинамическогоСпискаПоУмолчанию(ЭлСписок) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначения_УстановитьПериодДинамическогоСпискаПоУмолчанию";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура РаспределитьТаблицу1ПоТаблице2 (Отбор, Таблица1, Таблица2, ВыходнаяТаблица = Неопределено, ИмяКолонки = "Количество", ПереноситьКолонкиИзТаблицы1 = "") Экспорт 
	Для Каждого СтрокаТовары Из Таблица1 Цикл   
		...
		Пока СтрокаТовары[ИмяКолонки] > 0 И Индекс < Строки.Количество() Цикл  
			Строка = Строки.Получить(Индекс);
			СписываемоеКоличество = Мин(СтрокаТовары[ИмяКолонки], Строка[ИмяКолонки]);
			
			Если СписываемоеКоличество > 0 Тогда 
				Если ТипЗнч(ВыходнаяТаблица) = Тип("ТаблицаЗначений") Тогда 
					НоваяСтрока = ВыходнаяТаблица.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
					Если Не ПустаяСтрока(ПереноситьКолонкиИзТаблицы1) Тогда 
						ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТовары, ПереноситьКолонкиИзТаблицы1);
					КонецЕсли;
					НоваяСтрока[ИмяКолонки]= СписываемоеКоличество;
				КонецЕсли;
				СтрокаТовары[ИмяКолонки] = СтрокаТовары[ИмяКолонки]- СписываемоеКоличество;
				Строка[ИмяКолонки]= Строка[ИмяКолонки] - СписываемоеКоличество;
			КонецЕсли;
...
Процедура ОчиститьТаблицуСНулевымКол(Таблица) Экспорт 
...
Функция ПреобразоватьНомер(НомерДокумента, Длина = 11) Экспорт 
	
	ДлинаНомера = СтрДлина(НомерДокумента);
	
	Если ДлинаНомера = Длина Тогда 
	...
	Если Не ПустаяСтрока(НомерДокумента) Тогда 
		...
		ДобавленныеНули = "";
		КоличествоДобавляемыхНулей = Длина - ДлинаНомера;
		Для Шаг = 1 По КоличествоДобавляемыхНулей Цикл 
			ДобавленныеНули = ДобавленныеНули + "0";
		КонецЦикла;
		НомерДокумента83 = Лев(НомерДокумента, 3) + ДобавленныеНули + Прав(НомерДокумента, Мин(ДлинаНомера - 3, Длина - 3));
		
	КонецЕсли;
	
	Возврат НомерДокумента83;
	
КонецФункции

Функция ОтобратьУпорядочитьТаблицуЗначений(Таблица, ТекстУсловияОтбора = "", ТекстУсловияСортировки = "") Экспорт
	...
	Для каждого КолонкаТаблицы Из Таблица.Колонки Цикл
		ПоляВыбора = ПоляВыбора + КолонкаТаблицы.Имя + ", ";
	КонецЦикла;
	ПоляВыбора = Лев(ПоляВыбора,СтрДлина(ПоляВыбора)-2);
	
	УсловиеОтбора 	= ?(ТекстУсловияОтбора = "", 	 Истина, ТекстУсловияОтбора);
	УсловиеПорядка 	= ?(ТекстУсловияСортировки = "", Истина, ТекстУсловияСортировки);
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПорядка", УсловиеПорядка);
...
Процедура УбратьСимволы(Строка, ЗапрещенныеСимволы) Экспорт
	
	Для Индекс = 1 По СтрДлина(ЗапрещенныеСимволы) Цикл
		Символ = Сред(ЗапрещенныеСимволы, Индекс, 1);
		Строка = СтрЗаменить(Строка, Символ, "");
	КонецЦикла;
	
КонецПроцедуры

Функция НаценкаКонтрагента(Знач Контрагент, Знач Организация) Экспорт 
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	НаценкаСПБ.ПроцентНаценки
	               |ИЗ
	               |	РегистрСведений.НаценкаСПБ КАК НаценкаСПБ
	               |ГДЕ
	               |	НаценкаСПБ.ОрганизацияПродавец = &ОрганизацияПродавец
	               |	И НаценкаСПБ.КонтрагентПокупатель = &КонтрагентПокупатель";
	
	Запрос.УстановитьПараметр("ОрганизацияПродавец", Организация);
	Запрос.УстановитьПараметр("КонтрагентПокупатель", Контрагент);
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		Возврат Результат.Выгрузить().Получить(0).ПроцентНаценки; 
	КонецЕсли;
...
Функция ПолучитьМассивСтатусовДокументаРТУ(Назначение = Неопределено) Экспорт 
	Массив = Новый Массив;
	Если Назначение = Неопределено Тогда
		Массив.Добавить(Справочники.СтатусыДокументов.РеализацияТоваровУслугНовый);
		Массив.Добавить(Справочники.СтатусыДокументов.РеализацияТоваровУслугСборка);
...
//Семенов И.П. 23.07.2019 XX-2911(
Функция ПолучитьЗначениеСвойстваОбъекта(Объект, Свойство, ЗначениеПоУмолчанию = Неопределено) Экспорт
...
Процедура ЗаписатьЗначениеСвойставаОбъекта(Объект, Свойство, Значение) Экспорт
