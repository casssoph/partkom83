...
Процедура ЗаписатьДвиженияДокументаБезОбработки(вхСсылкаНаДокумент, вхМенеджерОбъекта, вхТабДвиж, вхЗамещать = Истина) Экспорт
...
Функция ПолучитьТТПополнениеСклада(вхСсылкаНаДокумент) Экспорт 
	...
	Если ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда 
		...
		//Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "СкладПолучатель, СкладПолучатель.НеИмеетСобственногоСтока");
		//Если Реквизиты.СкладПолучательНеИмеетСобственногоСтока Тогда 
		//	Склад = Константы.СкладОсновной.Получить(); 	
		//Иначе
		//	Склад = Реквизиты.СкладПолучатель;		
		//КонецЕсли;
	//ИначеЕсли ТипЗнч(вхСсылкаНаДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
	//	Запрос = Новый Запрос;
	//	Запрос.Текст = "ВЫБРАТЬ
	//	               |	ЕСТЬNULL(ПоступлениеТоваровУслугРазмещениеСтрокПрихода.СтрокаЗаявки.Заказ.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК Склад,
	//	               |	СУММА(1) КАК Счетчик
	//	               |ИЗ
	...
	//				   // Если больше всего виртуальных строк, то перемещаем на склад в шапке (если есть собств. сток, иначе на ЦС)
	//				   // |	И ЕСТЬNULL(ПоступлениеТоваровУслугРазмещениеСтрокПрихода.СтрокаЗаявки.Заказ.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	//	               |
	//	               |СГРУППИРОВАТЬ ПО
	//	               |	ЕСТЬNULL(ПоступлениеТоваровУслугРазмещениеСтрокПрихода.СтрокаЗаявки.Заказ.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
	//	               |
	//	               |УПОРЯДОЧИТЬ ПО
	//	               |	Счетчик УБЫВ";
	...
	//	Результат = Запрос.Выполнить();
	//	Если Результат.Пустой() Тогда 
	//		Склад = ПолучитьТТПополнениеСкладаФрагмент(вхСсылкаНаДокумент);
	//	Иначе
	//		Выборка = Результат.Выбрать();
	//		Выборка.Следующий();
	//		Склад = Выборка.Склад;
