...
Процедура ОткрытьОтчетОбработку(Ключ, ЭтоОтчет = Истина) Экспорт
	...
	#Если Клиент Тогда
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			Попытка
				Если ЭтоОтчет Тогда
					ОткрытьФорму("Отчет." + Ключ + ".Форма");
				Иначе
					ОткрытьФорму("Обработка." + Ключ + ".Форма");
				КонецЕсли;
			Исключение
		...
		Иначе
			...
			Попытка
				...
				Если Форма = Неопределено Тогда
					...
					Форма = ВнешнийОбъект.ПолучитьФорму(); 
				КонецЕсли;
				
				Постфикс = " [Внешний" + ?(ЗначениеЗаполнено(ОбработкаСсылка.Версия), " " + ОбработкаСсылка.Версия, "") + "]"; 
				Форма.Заголовок = Форма.Заголовок + Постфикс; 
				Форма.Открыть();
				
				УдалитьФайлы(ИмяФайла);
				
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Выбранный файл не является внешней обработкой.
