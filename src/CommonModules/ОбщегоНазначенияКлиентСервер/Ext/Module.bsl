...
Функция ПолучитьВиртуальныйSSID(НомерДокумента)  Экспорт   //Не используется
	
	_номерИДДляСтрокиЗаявки = 0;
	
	Попытка
		ЯС = Новый COMОбъект("MSScriptControl.ScriptControl");
		ЯС.Language = "javascript";
		_номерИДДляСтрокиЗаявки = ЯС.Eval("new Date().getTime()");
		_номерИДДляСтрокиЗаявки = Лев(Формат(_номерИДДляСтрокиЗаявки, "ЧГ=0"), 11);
		_номерИДДляСтрокиЗаявки = Формат(Число(Прав(НомерДокумента, 7)),"ЧГ=0") + _номерИДДляСтрокиЗаявки;
		_номерИДДляСтрокиЗаявки = Число(_номерИДДляСтрокиЗаявки);
	Исключение
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
		                      ...
		                      |ГДЕ
		                      |	ИдентификаторыСтрокЗаявок.IDSite ПОДОБНО ""VL%""
		                      |
		                      |УПОРЯДОЧИТЬ ПО
		                      |	IDSite УБЫВ");
		...
		Иначе
			...
			Выборка.Следующий();
			_номерИДДляСтрокиЗаявки = ?(ЗначениеЗаполнено(Выборка.IDSite), Число(Прав(Выборка.IDSite,СтрДлина(Выборка.IDSite) - 2)), 0);
		КонецЕсли;
	КонецПопытки;

	Возврат _номерИДДляСтрокиЗаявки;
	
КонецФункции

Функция ВиртуальнаяСтрокаЗаявки() Экспорт 
	
	Уид = Новый УникальныйИдентификатор;
	IDSite = "VL" + Уид;
	Об = Справочники.ИдентификаторыСтрокЗаявок.СоздатьЭлемент();
	Об.IDSite = IDSite;
	Об.Виртуальная = Истина;
	Об.Наименование = "Виртуальная " + Об.IDSite;
	Об.Записать();
...
Функция ЗаявкаЗакрыта(Документ) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗакрытиеЗаявокПокупателяЗаявки.Ссылка
	               |ИЗ
	               ...
	               |	И ЗакрытиеЗаявокПокупателяЗаявки.Документ = &Документ";
...
Функция ПолучитьНДС(НДСВХ) Экспорт
	
	Если НДСВХ = Перечисления.СтавкиНДС.НДС0 Тогда
		НДС=0
	ИначеЕсли НДСВХ = Перечисления.СтавкиНДС.НДС10 Тогда
		НДС=0.1
	ИначеЕсли НДСВХ = Перечисления.СтавкиНДС.НДС18 Или НДСВХ = Перечисления.СтавкиНДС.НДС18_118 Тогда
		НДС=0.18
	ИначеЕсли НДСВХ = Перечисления.СтавкиНДС.НДС20 Или НДСВХ = Перечисления.СтавкиНДС.НДС20_120 Тогда
		НДС=0.20
	Иначе 
		НДС=0;	
	КонецЕсли;
...
Процедура ОткрытьОтчетОбработку(Ключ, ЭтоОтчет = Истина) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбменДаннымиКлиентСервер_ОткрытьОтчетОбработку";
	...
	#Если Клиент Тогда
		...
		Если Не ЗначениеЗаполнено(Ключ) Тогда
			ВызватьИсключение "Не удалось открыть отчет/обработку.";
		КонецЕсли;
		
		СтруктураВозврата = ПолучитьОтчетОбработку(Ключ, Ложь, ЭтоОтчет);
		Объект = СтруктураВозврата.Объект;
		ФормаОбъекта = Объект.ПолучитьФорму();
		Если СтруктураВозврата.ВнешнийОбъект Тогда
			Постфикс = " [Внешний" + ?(ЗначениеЗаполнено(СтруктураВозврата.Версия), " " + СтруктураВозврата.Версия, "") + "]";
			ФормаОбъекта.Заголовок = ФормаОбъекта.Заголовок + Постфикс;
		КонецЕсли;
...
Процедура ОткрытьОтчет_СтатусПеремещения() Экспорт
	ОткрытьОтчетОбработку("СтатусПеремещения");
КонецПроцедуры

Процедура ОткрытьОтчет_УниверсальныйОтчетПартком() Экспорт
	ОткрытьОтчетОбработку("УниверсальныйОтчетПартком");
КонецПроцедуры

Процедура ОткрытьОтчет_УниверсальныйОтчет() Экспорт
	ОткрытьОтчетОбработку("УниверсальныйОтчет");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетVMI_ОкноПоставщика() Экспорт
	ОткрытьОтчетОбработку("ОтчетVMI_ОкноПоставщика");
КонецПроцедуры

Процедура ОткрытьОтчет_НеИспользовать_ОтчетМХ3МХ1() Экспорт
	ОткрытьОтчетОбработку("НеИспользовать_ОтчетМХ3МХ1");
КонецПроцедуры

Процедура ОткрытьОтчет_СравнитьЦеныВМИ_НН_СПб() Экспорт
	ОткрытьОтчетОбработку("СравнитьЦеныВМИ_НН_СПб");
КонецПроцедуры

Процедура ОткрытьОтчет_РасхожденияПартийVMI() Экспорт
	ОткрытьОтчетОбработку("РасхожденияПартийVMI");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоСкидкамМХ3() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоСкидкамМХ3");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоРасписаниямВМИ() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоРасписаниямВМИ");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетВМИ() Экспорт
	ОткрытьОтчетОбработку("ОтчетВМИ");
КонецПроцедуры

// + Пушкин 20190709 XX-2508
Процедура ОткрытьОбработку_ЖурналКассовыхДокументов() Экспорт
	ОткрытьОтчетОбработку("ОткрытьЖурналКассовыхДокументов",Ложь);
КонецПроцедуры
// - Пушкин 20190709 XX-2508

Процедура ОткрытьОтчет_ОтчетОстаткиПоставщикам() Экспорт
	ОткрытьОтчетОбработку("ОтчетОстаткиПоставщикам");
КонецПроцедуры

Процедура ОткрытьОтчет_АнализПродаж() Экспорт
	ОткрытьОтчетОбработку("АнализПродаж");
КонецПроцедуры

Процедура ОткрытьОтчет_АнализНезакрытыхЗаявокЗаказов() Экспорт
	ОткрытьОтчетОбработку("АнализНезакрытыхЗаявокЗаказов");
КонецПроцедуры

Процедура ОткрытьОтчет_АнализОтказовЗаявокПокупателей() Экспорт
	ОткрытьОтчетОбработку("АнализОтказовЗаявокПокупателей");
КонецПроцедуры

Процедура ОткрытьОтчет_ВедомостьВзаиморасчетыСКонтрагентами() Экспорт
	ОткрытьОтчетОбработку("ВедомостьВзаиморасчетыСКонтрагентами");
КонецПроцедуры

Процедура ОткрытьОбработку_РасчетОборотовПоВзаиморасчетамПоПоставщику() Экспорт
	ОткрытьОтчетОбработку("РасчетОборотовПоВзаиморасчетамПоПоставщику", Ложь);
КонецПроцедуры

Процедура ОткрытьОтчет_ДвиженияЗаказовПоставщиков() Экспорт
	ОткрытьОтчетОбработку("ДвиженияЗаказовПоставщиков");
КонецПроцедуры

Процедура ОткрытьОтчет_ТоварыКРазмещениюНаСкладах() Экспорт
	ОткрытьОтчетОбработку("ТоварыКРазмещениюНаСкладах");
КонецПроцедуры

Процедура ОткрытьОтчет_ЛогистическиеЦепочки_Мотивация() Экспорт
	ОткрытьОтчетОбработку("ЛогистическиеЦепочки_Мотивация");
КонецПроцедуры

Процедура ОткрытьОтчет_ЛогистическиеЦепочки() Экспорт
	ОткрытьОтчетОбработку("ЛогистическиеЦепочки");
КонецПроцедуры

Процедура ОткрытьОтчет_РезервыТоваров() Экспорт
	ОткрытьОтчетОбработку("РезервыТоваров");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоСписаниям() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоСписаниям");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоОбеспечениюПополненийСклада() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоОбеспечениюПополненийСклада");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоПеремещениям() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоПеремещениям");
КонецПроцедуры

Процедура ОткрытьОтчет_ЗависшиеПТУ() Экспорт
	ОткрытьОтчетОбработку("ЗависшиеПТУ");
КонецПроцедуры

Процедура ОткрытьОтчет_ЗависшиеПеремещенияВПути() Экспорт
	ОткрытьОтчетОбработку("ЗависшиеПеремещенияВПути");
КонецПроцедуры

Процедура ОткрытьОтчет_ПолнотаПоставок() Экспорт
	ОткрытьОтчетОбработку("ПолнотаПоставок");
КонецПроцедуры

Процедура ОткрытьОтчет_ПричиныИзмененияДокументов() Экспорт
	ОткрытьОтчетОбработку("ПричиныИзмененияДокументов");
КонецПроцедуры

Процедура ОткрытьОтчет_КалендарьОплатПоVMIПоставщикам() Экспорт
	ОткрытьОтчетОбработку("КалендарьОплатПоVMIПоставщикам");
КонецПроцедуры

Процедура ОткрытьОтчет_НезакрытыеЗаявкиПоПополнениям() Экспорт
	ОткрытьОтчетОбработку("НезакрытыеЗаявкиПоПополнениям");
КонецПроцедуры

Процедура ОткрытьОтчет_НеотгруженыеТоварыВпути() Экспорт
	ОткрытьОтчетОбработку("НеотгруженыеТоварыВпути");
КонецПроцедуры

Процедура ОткрытьОтчет_ОстаткиИРезервыНоменклатурыПоСкладам() Экспорт
	ОткрытьОтчетОбработку("ОстаткиИРезервыНоменклатурыПоСкладам");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоАктамПриемки() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоАктамПриемки");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоНеверноРазмещеннымПТУ() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоНеверноРазмещеннымПТУ");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоПотокуПриходаТовараНаСклады() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоПотокуПриходаТовараНаСклады");
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПоПричинамОтказаРТУ() Экспорт
	ОткрытьОтчетОбработку("ОтчетПоПричинамОтказаРТУ");
КонецПроцедуры

Процедура ОткрытьОтчет_ПеремещенияПоАвтору() Экспорт
	ОткрытьОтчетОбработку("ПеремещенияПоАвтору");
КонецПроцедуры

Процедура ОткрытьОтчет_ПланированиеВходящегоПотокаНаСклады() Экспорт
	ОткрытьОтчетОбработку("ПланированиеВходящегоПотокаНаСклады");
КонецПроцедуры

Процедура ОткрытьОтчет_ПодробныеДанныеКонтрагентов() Экспорт
	ОткрытьОтчетОбработку("ПодробныеДанныеКонтрагентов");
КонецПроцедуры

Процедура ОткрытьОтчет_ПоступленияПоДатамОплаты() Экспорт
	ОткрытьОтчетОбработку("ПоступленияПоДатамОплаты");
КонецПроцедуры

Процедура ОткрытьОтчет_ПриходПоИзготовителюДляОУТЗТест() Экспорт
	ОткрытьОтчетОбработку("ПриходПоИзготовителюДляОУТЗТест");
КонецПроцедуры

Процедура ОткрытьОтчет_ПриходПоИзготовителю() Экспорт
	ОткрытьОтчетОбработку("ПриходПоИзготовителю");
КонецПроцедуры

Процедура ОткрытьОтчет_РеестрДокументов() Экспорт
	ОткрытьОтчетОбработку("РеестрДокументов");
КонецПроцедуры

Процедура ОткрытьОбработку_ПросмотрИсторииОбъектов() Экспорт
	ОткрытьОтчетОбработку("ПросмотрИсторииОбменов", Ложь);	
КонецПроцедуры

Процедура ОткрытьОтчет_ИсторияИзмененийНастроекПользователя() Экспорт
	ОткрытьОтчетОбработку("ИсторияИзмененийНастроекПользователя");
КонецПроцедуры

// 06.05.19 Строганов Роман <

Процедура ОткрытьОбработку_ПереводНаНовыеДоговоры() Экспорт 
	ОткрытьОтчетОбработку("ПереводНаНовыеДоговоры", Ложь);	
КонецПроцедуры

Процедура ОткрытьОтчет_ОтчетПодчГолКонтрагентыИзРазныхФилиалов() Экспорт 
	ОткрытьОтчетОбработку("ОтчетПодчГолКонтрагентыИзРазныхФилиалов");
КонецПроцедуры

//#XX-2521 Kalinin V.A. ( 2019-06-14 )  /*
Процедура ОткрытьОтчет_РеестрВозвратовТоваровПоставщикам() Экспорт
	ОткрытьОтчетОбработку("РеестрВозвратовТоваровПоставщикам");
КонецПроцедуры

Процедура ОткрытьОбработку_КонсольТехподдержки() Экспорт
	ОткрытьОтчетОбработку("КонсольТехподдержки", Ложь);
КонецПроцедуры

// 14.05.19 ЛНА, открытие отчетов и обработок с заменой, не тестировано
Функция ПолучитьОтчетОбработку(Ключ, ВернутьФорму = Истина, ЭтоОтчет = Истина) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначенияКлиентСервер_ПолучитьОтчетОбработку";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	Если Не ЗначениеЗаполнено(Ключ) Тогда
		ВызватьИсключение "Не удалось открыть отчет/обработку. Передан пустой ключ.";
	КонецЕсли;
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ВнешниеОбработки.Ссылка,
	|	ВнешниеОбработки.Версия,
	|	ВнешниеОбработки.ВидОбработки
	|ИЗ
	|	Справочник.ВнешниеОбработки КАК ВнешниеОбработки
	|ГДЕ
	|	ВнешниеОбработки.Ключ = &Ключ
	|	И ВнешниеОбработки.Автозамена");
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Попытка
			...
			Если ЭтоОтчет Тогда
				Объект = Отчеты[Ключ].Создать();
			Иначе
				Объект = Обработки[Ключ].Создать();
			КонецЕсли;
			
			Если ВернутьФорму = Истина Тогда
				ФормаОбъекта = Объект.ПолучитьФорму();
				Возврат Новый Структура("Объект, ВнешнийОбъект, Версия", ФормаОбъекта, Ложь);
			Иначе
				Возврат Новый Структура("Объект, ВнешнийОбъект, Версия", Объект, Ложь);
			КонецЕсли;
			
		Исключение
			ТекстСообщения = "Не удалось открыть %1. Пожалуйста, проверьте наличие %2 с включенной заменой. Ключ: %3";
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ?(ЭтоОтчет, "отчет", "обработку"), ?(ЭтоОтчет, "внешнего отчета", "внешней обработки"), Ключ);
	...
	Иначе
		...
		Попытка
			...
			Если ВернутьФорму = Истина Тогда
				ФормаОбъекта = ВнешнийОбъект.ПолучитьФорму();
				Возврат Новый Структура("Объект, ВнешнийОбъект, Версия", ФормаОбъекта, Истина, ОбработкаСсылка.Версия);
			Иначе
				Возврат Новый Структура("Объект, ВнешнийОбъект, Версия", ВнешнийОбъект, Истина, ОбработкаСсылка.Версия);
			КонецЕсли;
			
		Исключение
			ТекстСообщения = "Выбранный файл не является внешней обработкой.
			|Либо, данная обработка не предназначена для
			|запуска в этой конфигурации. ("+ОписаниеОшибки()+")";
			ВызватьИсключение ТекстСообщения;
		КонецПопытки;
...
// Функция - Получить значение дополнительных свойства объекта
//
// Параметры:
//  ОбъектСсылка	 - ЛюбаяСсылка	 - Объект значенияСвойства
//  СвойствоОбъекта	 - ПланВидовХаратеристик.СвойствоОбъекта	 -  Характеристика ПВХ
// 
// Возвращаемое значение:
//   - 
//
Функция ПолучитьЗначениеСвойстваОбъекта(ОбъектСсылка,СвойствоОбъекта) Экспорт
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначенияКлиентСервер_ПолучитьЗначениеСвойстваОбъект";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если Не ЗначениеЗаполнено(ОбъектСсылка) или
		не ОбщегоНазначенияПовтИсп.ОписаниеСсылочныхТипов().СодержитТип(Типзнч(ОбъектСсылка)) тогда
		Возврат Неопределено ;
	Конецесли; 	
	
	СтруктураОтбора = новый Структура;
	СтруктураОтбора.Вставить("Объект",ОбъектСсылка);
	СтруктураОтбора.Вставить("Свойство",СвойствоОбъекта);
	СтруктураРегистра = РегистрыСведений.ЗначенияСвойствОбъектов.Получить(СтруктураОтбора);
	Возврат СтруктураРегистра.Значение
