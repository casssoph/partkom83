...
Функция ПолучитьОтчетОбработку(Ключ, ВернутьФорму = Истина, ЭтоОтчет = Истина) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Исключение
			ТекстСообщения = "Не удалось открыть %1. Пожалуйста, проверьте наличие %2 с включенной заменой";
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ?(ЭтоОтчет, "отчет", "обработку"), ?(ЭтоОтчет, "внешнего отчета", "внешней обработки"));
