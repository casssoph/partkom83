...
Функция ЗначениеКонстанты(Наименование, СообщатьОбОшибке = Ложь, ТекстОшибки = "", Отказ = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Значение) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( ТекстСообщения,
										 Метаданные.Константы[Наименование].Синоним);
...
Функция ДатаНачалаПроведенияПоОрганизации(Организация) Экспорт 
	   Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация,"ДатаНачалаПроведенияДокументов")
...
Функция ОписаниеСсылочныхТипов() Экспорт
	//# Kalinin V.A. ( 2019-05-13 )  /* 
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначенияПовтИсп_ОписаниеСсылочныхТипов";
...
// Функция - Получить имена предопределенных по метаданным
//
// Параметры:
//  Метаданные	 - Строка,Метаданные	 - 
// 
// Возвращаемое значение:
//   - Список предопределенных Объектов
//# Kalinin V.A. ( 2019-06-24 )  /*
Функция ПолучитьИменаПредопределенныхПоМетаданным(ИмяМетаданных) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_ОбщегоНазначенияПовтИсп_ПолучитьИменаПредопределенныхПоМетаданным";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если ТипЗнч(ИмяМетаданных) = Тип("Строка") тогда 
		МетаданныеОбъекта =  Метаданные.НайтиПоПолномуИмени(ИмяМетаданных) ;
	иначе 
		МетаданныеОбъекта = ИмяМетаданных;
	КонецЕсли;
	
	Возврат МетаданныеОбъекта.ПолучитьИменаПредопределенных();
