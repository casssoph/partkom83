...
Процедура ДобавитьКнопкуРедактированияОбщедоступныхРеквизитов(Объект,Форма)
	...
	Если  НастройкаПравДоступа.ПолучитьЗначениеПраваПользователя(УправлениеПользователями.ОпределитьТекущегоПользователя(), 
		ПланыВидовХарактеристик.ПраваПользователей.РазрещитьИзменениеРеквизитовВЗакрытомПериоде) тогда 		
		
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ВозвратТоваровПоставщику") Или 
			ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказПоставщику") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаявкаПокупателя") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.ОприходованиеТоваров") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") или 
			ТипЗнч(Объект) = Тип("ДокументОбъект.СписаниеТоваров") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.КорректировкаДолга") или //ХудинВВ 28052019 XX-2572
			ТипЗнч(Объект) = Тип("ДокументОбъект.УстановкаЦенНоменклатурыКонтрагентов") или 
			ТипЗнч(Объект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда 		
			Действие = Новый Действие("ДействиеФормыНажатиеКнопки");	
			...
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.РедактированиеОбщедоступныхРеквизитов.Отображение = ОтображениеКнопкиКоманднойПанели.Картинка;	
