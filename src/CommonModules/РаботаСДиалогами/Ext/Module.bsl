...
Процедура ПриИзмененииОрганизацияЯвляетсяПлательщикомЕНВД(ДокументОбъект,
	...
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитДокумента("ЗаявкаПокупателя", МетаданныеДокумента) Тогда
		НоваяОрганизация = ДокументОбъект.ЗаявкаПокупателя.Организация;
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура НачалоВыбораЗначенияСделки(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
	...
	Если (СтрокаТипаЗаказа = "ЗаказПоставщику") или (СтрокаТипаЗаказа = "ЗаявкаПокупателя") Тогда

		ТипДокументаСделки = СтрокаТипаЗаказа;
		ТипДокументаСчет = ?(СтрокаТипаЗаказа = "ЗаказПоставщику","ЗаказПоставщику","ЗаявкаПокупателя");//ТипДокументаСчет = ?(СтрокаТипаЗаказа = "ЗаказПоставщику","СчетНаОплатуПоставщика","СчетНаОплатуПокупателю");
...
Процедура НачалоВыбораЗначенияДокументаСоставногоТипа(ДокументОбъект, ФормаДокумента, ЭлементФормы, 
	...
	Если ВыбранныйТип = "ДокументРасчетовСКонтрагентом" Тогда
		...
		Возврат;
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг") Тогда
...
//  ДокументОбъект - объект документа, 
//  ФормаДокумента - форма документа.
// ЗаявкаПокупателя - признак заказа покупателю/поставщику
//
Процедура УстановитьНадписьСделки(ДокументОбъект, ФормаДокумента, ЗаявкаПокупателя = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Если ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом
			...
			НадписьСделка = "Заказ "+?(ЗаявкаПокупателя,"покупателя:", "поставщику:");

		ИначеЕсли ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам тогда

			НадписьСделка = "Счет "+?(ЗаявкаПокупателя,"покупателю:", "поставщика:");
...
// Устанавливает видимость колонки "ЗаказПоставщику" или "ЗаявкаПокупателя" в табличной части формы документа
...
//
Процедура УстановитьВидимостьКолонкиЗаказ(ЭлементыФормы, ДокументВидОперации, ДоговорКонтрагента, КолонкиТовары, КолонкиВозвратнаяТара = Неопределено, КолонкиУслуги = Неопределено, КолонкиВзаиморасчеты = Неопределено, ЭУЗаказ, ИмяКолонкиЗаказ = "ЗаявкаПокупателя", ВидОперации = "Реализация", ИмяКолонкиСделка = "Сделка") Экспорт
...
Процедура ВвестиСчетФактуру(ДокументОбъект, ФормаДокумента, ВидСчетаФактуры = "СчетФактураВыданный", РазрешитьВыборСФнаАванс = Истина) Экспорт
	...
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ВвестиСчетФактуру";
	...
	ПоследняяКорректировка = РаботаСоСтатусамиДокументов.ПолучитьПоследнююКорректировку(ДокументОбъект.Ссылка);
	
	Если ПоследняяКорректировка = Неопределено Тогда
	...
	Если (НЕ ЗначениеЗаполнено(СчетФактура))
	   И (Не ДокументОбъект.ПометкаУдаления) Тогда
		...
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВозвратТоваровОтПокупателя") Тогда
			...
			ТекстОшибки = "";
			Если НЕ Документы.ВозвратТоваровОтПокупателя.РазрешенВводСчетФактуры(ДокументОбъект.Ссылка, ТекстОшибки) Тогда
				Предупреждение("Невозможно создать документ: " + ТекстОшибки);
				Возврат;
			КонецЕсли;
		...
		Иначе
			ОбъектСчетаФактуры.Заполнить(ПоследняяКорректировка); 
		КонецЕсли;
...
Процедура ВвестиСчетФактуру_типовая(ДокументОбъект, ФормаДокумента, ВидСчетаФактуры = "СчетФактураВыданный", РазрешитьВыборСФнаАванс = Истина) Экспорт
...
Процедура ИзмененитьРежимИспользованияПлановойСебестоимости(ДокументОбъект, ФормаДокумента, ВалютаРегламентированногоУчета, СтароеЗначениеРежимаИспользовать = Неопределено) Экспорт
	...
	Попытка
		...
		ЕстьЭлементФормыТовары = Истина;
	Исключение		
		Надпись = ФормаДокумента.ЭлементыФормы.ИнфНадписьИтоговоеОтклонениеОтПлановойСебестоимости;
		ЕстьЭлементФормыТовары = Ложь;
	КонецПопытки;
		
	Если ЕстьЭлементФормыТовары Тогда	
...
Процедура ОткрытьЗадачиТекущегоПользователя(ОтборПоОбъекту = Неопределено) Экспорт
	...
	НаборЗаписей = РегистрыСведений.Адресация.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Исполнитель.Установить(ПараметрыСеанса.ТекущийПользователь);
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() > 0 Тогда
		ФормаСпискаЗадач.Отбор.Должность.ВидСравнения     = ВидСравнения.Равно;
		ФормаСпискаЗадач.Отбор.Должность.Использование    = Истина;
		ФормаСпискаЗадач.Отбор.Должность.Значение         = НаборЗаписей.Получить(0).Должность;
	Иначе
...
Процедура ПроверитьЗадачиПоОбъекту(ОбъектЗадачи) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ЗадачиПользователя.Инициатор,
	               |	ЗадачиПользователя.Оповещение,
	               |	ЗадачиПользователя.СрокОповещения,
	               |	ЗадачиПользователя.ПамятнаяДата,
	               |	ЗадачиПользователя.ДатаИсполнения,
	               |	ЗадачиПользователя.Должность,
	               |	Адресация.Исполнитель
	               |ИЗ
	               |	Задача.ЗадачиПользователя КАК ЗадачиПользователя
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Адресация КАК Адресация
	               |		ПО ЗадачиПользователя.Должность = Адресация.Должность
	               |ГДЕ
				   |	ЗадачиПользователя.СрокОповещения < &ДатаСреза
	               |	и ЗадачиПользователя.Оповещение = ИСТИНА
	               |	И ЗадачиПользователя.Выполнена = ЛОЖЬ
	               |	И Адресация.Исполнитель = &ТекПользователь";
...
//
Процедура НачалоВыбораЗначенияДокументаРасчетовСКонтрагентом(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, 
...
Процедура ОбновитьПараметрыИФормуПодбора(ДокументОбъект, ФормаДокумента, ПолноеИмяРеквизита) Экспорт
	...
	Если ФормаПодбора.Открыта() Тогда
		...
		Если ФормаПодбора.ОбработкаОбъект.СтруктураИсходныхПараметров.Свойство(ИмяКлюча) Тогда
			Если СтрНачинаетсяС(ИмяКлюча, "Фрм") Тогда 
				ИмяКлюча = Прав(ИмяКлюча, СтрДлина(ИмяКлюча) - 3);
				ФормаПодбора.ОбработкаОбъект.СтруктураИсходныхПараметров.Вставить(ИмяКлюча, ФормаДокумента[ИмяРеквизита]);
			Иначе
...
////////////////////////////////////////////////////////////////////////////////
//СОХРАНЕНИЕ И ВОССТАНОВЛЕНИЕ ЗНАЧЕНИЙ В ФОРМАХ
//ПРЕДПОЛАГАЕТСЯ ЧТО МОЖНО СОХРАНИТЬ ТОЛЬКО ОДНО ЗНАЧЕНИЕ ДЛЯ КАЖДОГО ВИДА СПРАВОЧНИКА 
//НОМЕНКЛАТУРА, КОНТРАГЕНТЫ, ДОГОВОРЫ КОНТРАГЕНТОВ
Процедура СохранитьТекущееЗначениеДляПоиска(Пользователь, ОбъектМетаданных = Неопределено, СсылкаНаОбъект) Экспорт
	нз = РегистрыСведений.СохраненныеПользователямиОбъектыПоиска.СоздатьМенеджерЗаписи();
	нз.Пользователь = Пользователь;
	нз.ОбъектМетаданных = ОбъектМетаданных;
	нз.СсылкаНаОбъект = СсылкаНаОбъект;
	нз.Записать(Истина);
	
КонецПроцедуры

Функция ПолучитьСохраненноеЗначениеДляПоиска(Пользователь, ОбъектМетаданных) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	СохраненныеПользователямиОбъектыПоиска.СсылкаНаОбъект
	|ИЗ
	|	РегистрСведений.СохраненныеПользователямиОбъектыПоиска КАК СохраненныеПользователямиОбъектыПоиска
	|ГДЕ
	|	СохраненныеПользователямиОбъектыПоиска.Пользователь = &Пользователь
	|	И СохраненныеПользователямиОбъектыПоиска.ОбъектМетаданных = &ОбъектМетаданных"
	);
	...
	Если ТабЗ.Количество() = 0 Тогда
		Возврат Неопределено;
	Иначе
		Возврат ТабЗ[0].СсылкаНаОбъект;
	КонецЕсли;
	
КонецФункции


#Область Партком_СобытияФорм

Процедура ПриОткрытииФормы(Объект,Форма, ИмяКоманднойПанели = "ДействияФормы", ИмяКнопки = "Действия") Экспорт
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ПриОткрытииФормы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	//#XX-91 Kalinin V.A. ( 2018-06-14 )
	//Когда перейдем на упр формы , перенести в ПриЧтенииНаСервере	
	Если не Объект.ЭтоНовый() тогда 
		//УстановкаВидимостиДокумента77(Объект,Форма); // ЛНА, не актуально после отказа от 77		
		УстановкаВидимостиДокументаДоПерехода(Объект,форма);
	КонецЕсли;
	
	//дОБАВЛЕНО В ПОПЫТКУ ,ЧТОБЫ НЕ ВЛИЯЛА В ИСКЛЮЧИТЕЛЬНЫХ СЛУЧАЯХ НА РАБОТУ 	
	Попытка	
	// НП 14.05.2019 Горохов П.Е. http://jira.part-kom.ru/browse/XX-2407
	//ДобавитьКоманднуюПанельОбменаДанными(Объект,Форма);
	ДобавитьПрограммныеКнопки(Объект, Форма, ИмяКоманднойПанели, ИмяКнопки);
	// КП 14.05.2019 Горохов П.Е. http://jira.part-kom.ru/browse/XX-2407
	ДобавитьКнопкуПерепроведения(Объект,Форма);
	ДобавитьКнопкуРедактированияОбщедоступныхРеквизитов(Объект,Форма)
	Исключение
	КонецПопытки;

КонецПроцедуры

Процедура ДобавитьПрограммныеКнопки(Объект, Форма, ИмяКоманднойПанели = "ДействияФормы", ИмяКнопки = "Действия") Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДобавитьПрограммныеКнопки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ДействиеФормы = Новый Действие("ДействиеФормыНажатиеКнопки");
	Если ОбщегоНазначения.ЭтоДокумент(Объект.Метаданные()) Тогда
		КоманднаяПанель = Форма.ЭлементыФормы.Найти(ИмяКоманднойПанели);
		Если КоманднаяПанель <> Неопределено Тогда
			КнопкаКоманднойПанели = КоманднаяПанель.Кнопки.Найти(ИмяКнопки);
			Если КнопкаКоманднойПанели <> Неопределено Тогда
				КнопкаКоманднойПанели.Кнопки.Добавить(, ТипКнопкиКоманднойПанели.Разделитель);
				//#XX-2602 Kalinin V.A. ( 2019-06-18 )  /*
				//Изменил действия чтобы не прописывать их в каждом конкретном документе
				КнопкаКоманднойПанели.Кнопки.Добавить("ДействияФормыПоказатьGUID", ТипКнопкиКоманднойПанели.Действие, "Показать GUID", ДействиеФормы);
				КнопкаКоманднойПанели.Кнопки.Добавить("ДействияФормыИсторияИзменений", ТипКнопкиКоманднойПанели.Действие, "История изменений",ДействиеФормы);
				//<< 03.07.2019 Горохов П.Е. XX-2847
				КнопкаКоманднойПанели.Кнопки.Добавить("ДействияФормыИсторияРегистрацииОбъектов", ТипКнопкиКоманднойПанели.Действие, "История регистрации объектов", ДействиеФормы);
				//>> 03.07.2019 Горохов П.Е. XX-2847
				КнопкаИсторияОбмена = КнопкаКоманднойПанели.Кнопки.Добавить("ИсторияОбменаССайтом",ТипКнопкиКоманднойПанели.Действие,"История обмена",ДействиеФормы );		
				КнопкаИсторияОбмена.Картинка = БиблиотекаКартинок.ОбменДанными32;
			КонецЕсли;
			//Семенов И.П. 25.06.2019 XX-2663(
			ДобавитьПодменюОтчетыВКоманднуюПанель(Объект, Форма, КоманднаяПанель, ИмяКнопки);
			//)Семенов И.П.
		КонецЕсли;
	Иначе
		//<< 03.07.2019 Горохов П.Е. XX-2847
		Попытка
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Подменю.Кнопки.Добавить("ДействияФормыИсторияРегистрацииОбъектов", ТипКнопкиКоманднойПанели.Действие, "История регистрации объектов", ДействиеФормы);
			КнопкаИсторияОбмена = Форма.ЭлементыФормы.ДействияФормы.Кнопки.Подменю.Кнопки.Добавить("ИсторияОбменаССайтом",ТипКнопкиКоманднойПанели.Действие,"История обмена", ДействиеФормы);		
		Исключение
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Действия.Кнопки.Добавить("ДействияФормыИсторияРегистрацииОбъектов", ТипКнопкиКоманднойПанели.Действие, "История регистрации объектов", ДействиеФормы);
			КнопкаИсторияОбмена = Форма.ЭлементыФормы.ДействияФормы.Кнопки.Действия.Кнопки.Добавить("ИсторияОбменаССайтом",ТипКнопкиКоманднойПанели.Действие,"История обмена", ДействиеФормы);		
		КонецПопытки;
		КнопкаИсторияОбмена.Картинка = БиблиотекаКартинок.ОбменДанными32;
		//ДобавитьКоманднуюПанельОбменаДанными(Объект, Форма);
		//>> 03.07.2019 Горохов П.Е. XX-2847
...
Процедура ДобавитьНадписьТолькоПросмотр(Объект, Форма, ИмяКоманднойПанели = "ДействияФормы", ИмяКоманднойПанели2 = "", ПричинаРежимаТолькоПросмотр = "Закрытый период") Экспорт

	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДобавитьНадписьТолькоПросмотр";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	НадписьТолькоПросмотр = Форма.ЭлементыФормы.Добавить(Тип("Надпись"), "НадписьТолькоПросмотр", Истина);
	НадписьТолькоПросмотр.Заголовок = "ТОЛЬКО ПРОСМОТР";
	НадписьТолькоПросмотр.Подсказка = ПричинаРежимаТолькоПросмотр;
	НадписьТолькоПросмотр.ВертикальноеПоложение = ВертикальноеПоложение.Верх;
	НадписьТолькоПросмотр.Высота = 25;
	НадписьТолькоПросмотр.Ширина = 195;
	НадписьТолькоПросмотр.Верх = 0;
	НадписьТолькоПросмотр.Лево = 0;
	НадписьТолькоПросмотр.ЦветФона = ЦветаСтиля.ЦветФонаКнопки;
	НадписьТолькоПросмотр.ЦветТекста = Новый Цвет(255, 0, 0); 
	НадписьТолькоПросмотр.Гиперссылка = Истина;
	НадписьТолькоПросмотр.Шрифт = Новый Шрифт(, 13, Истина);
	НадписьТолькоПросмотр.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.Подчеркивание, 1);
	НадписьТолькоПросмотр.ЦветРамки = ЦветаСтиля.ЦветРамки;
	НадписьТолькоПросмотр.УстановитьДействие("Нажатие", Новый Действие("ГиперссылкаТолькоПросмотрНажатие"));
	КоманднаяПанель.Ширина = КоманднаяПанель.Ширина - НадписьТолькоПросмотр.Ширина;
	Если НЕ ПустаяСтрока(ИмяКоманднойПанели2) Тогда
		КоманднаяПанель2 = Форма.ЭлементыФормы.Найти(ИмяКоманднойПанели2);
		Если КоманднаяПанель2 <> Неопределено Тогда
			КоманднаяПанель2.Лево = КоманднаяПанель.Ширина + НадписьТолькоПросмотр.Ширина;
		КонецЕсли;
	КонецЕсли;
	КоманднаяПанель.Лево = НадписьТолькоПросмотр.Ширина;
	КоманднаяПанель.УстановитьПривязку(ГраницаЭлементаУправления.Лево, НадписьТолькоПросмотр, ГраницаЭлементаУправления.Право);

КонецПроцедуры

Процедура ДобавитьКоманднуюПанельОбменаДанными(Объект,Форма)
	//# Kalinin V.A. ( 2019-04-02 )  /*
	// Заменю на определяемы тип, корень заняли и не отдают
		
	Действие = Новый Действие("ДействиеФормыНажатиеКнопки");	
	Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("ОбменДанными",ТипКнопкиКоманднойПанели.Подменю,"Обмен данными");
	Форма.ЭлементыФормы.ДействияФормы.Кнопки.ОбменДанными.Кнопки.Добавить("ИсторияОбменаССайтом",ТипКнопкиКоманднойПанели.Действие,"История обмена с сайтом",Действие);		
КонецПроцедуры	

Процедура ДобавитьКнопкуРедактированияОбщедоступныхРеквизитов(Объект,Форма)
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДобавитьКнопкуРедактированияОбщедоступныхРеквизитов";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	КонецЕсли;
	//#XX-2481 Kalinin V.A. ( 2019-06-24 )  /*
	
	МассивЭлементов = ОбщегоНазначенияПовтИсп.ПолучитьИменаПредопределенныхПоМетаданным("ПланВидовХарактеристик.ОбщедоступныеРеквизиты");
	ИмяМетаданных = Объект.Метаданные().ПолноеИмя();
	//#XX-2481 Kalinin V.A. ( 2019-06-25 )  /*	
	если ДляПользователяДоступноРедактированиеРеквизитов(ПараметрыСеанса.ТекущийПользователь,ИмяМетаданных) тогда

			Действие = Новый Действие("ДействиеФормыНажатиеКнопки");	
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("РедактированиеОбщедоступныхРеквизитов",ТипКнопкиКоманднойПанели.Действие,"Редактирование реквизитов",Действие);
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.РедактированиеОбщедоступныхРеквизитов.Картинка = БиблиотекаКартинок.ДополнительныеПоляОтчета;
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.РедактированиеОбщедоступныхРеквизитов.Отображение = ОтображениеКнопкиКоманднойПанели.НадписьКартинка;	
			
		КонецЕсли;
	//КонецЕсли;
КонецПроцедуры	

Функция ДляПользователяДоступноРедактированиеРеквизитов(Пользователь,ИмяМетаданных)
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДляПользователяДоступноРедактированиеРеквизитов";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	ПланВидовХарактеристик.ОбщедоступныеРеквизиты КАК ОбщедоступныеРеквизиты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ОбщедоступныеРеквизиты.Пользователи КАК ОбщедоступныеРеквизитыПользователи
		|		ПО (ОбщедоступныеРеквизитыПользователи.Ссылка = ОбщедоступныеРеквизиты.Ссылка)
		|ГДЕ
		|	(ОбщедоступныеРеквизитыПользователи.Пользователь ЕСТЬ NULL
		|			ИЛИ ОбщедоступныеРеквизитыПользователи.Пользователь = &Пользователь)
		|	И ОбщедоступныеРеквизиты.ИмяМетаданных = &ИмяМетаданных
		|	И ОбщедоступныеРеквизиты.ПометкаУдаления = ЛОЖЬ";
...
// Валиахметов 05.04.2019 http://jira.part-kom.ru/browse/XX-1966 

Процедура ДобавитьКнопкуПерепроведения(Объект, Форма) 
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДобавитьКнопкуПерепроведения";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если УправлениеДопПравамиПользователей.ДоступностьКнопкиПерепровести() Тогда 
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Или 
			ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.ВозвратТоваровПоставщику") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.СписаниеТоваров") Или
			ТипЗнч(Объект) = Тип("ДокументОбъект.ПерестикеровкаПереоценка") Тогда 
			Действие = Новый Действие("Перепровести");
			Попытка
				Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("Перепровести", ТипКнопкиКоманднойПанели.Действие," Перепровести", Действие);
			Исключение
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопкиПерепровести(Проведен, Форма) Экспорт 
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_УстановитьДоступностьКнопкиПерепровести";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Попытка
		Форма.ЭлементыФормы.ДействияФормы.Кнопки.Перепровести.Доступность = Проведен; 
	Исключение
	КонецПопытки;	
КонецПроцедуры

Функция НужноПерепровести() Экспорт 
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_НужноПерепровести";
	лЗамена = АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;
		Выполнить(лЗамена);
	...
	Ответ = Вопрос("Перепровести документ с полным пересчетом списанных партий?", РежимДиалогаВопрос.ДаНет);
	Возврат Ответ = КодВозвратаДиалога.Да;
КонецФункции

//Конец Валиахметов 05.04.2019 http://jira.part-kom.ru/browse/XX-1966 

Процедура УстановкаВидимостиДокумента77(Объект,Форма)
	//Если Объект.Дата > '20180602' тогда 	
	//	МетДанные  = Объект.Метаданные();
	//	Если НЕ МетДанные.реквизиты.Найти("СозданВ77") = Неопределено  
	//		и Объект.СозданВ77 тогда 
	//		Если  не РольДоступна("ПолныеПрава") тогда 
	//			Форма.ТолькоПросмотр = Истина;
	//		КонецЕсли;
	//		ПоказатьПредупреждение(,"Документ создан в 1с 7.7 , дальнейшее редактирование запрещено");
	//	КонецЕсли;	 
	//КонецЕсли;	 
КонецПроцедуры // УстановкаВидимостиДокумента77()

Процедура УстановкаВидимостиДокументаДоПерехода(Объект,Форма)
	Если ТипЗнч(Объект) = тип("ДокументОбъект.ЗаявкаПокупателя") 
		или ТипЗнч(Объект) = тип("ДокументОбъект.КорректировкаЗаявкиПокупателя") 
		или ТипЗнч(Объект) = тип("ДокументОбъект.ЗаказПоставщику")                
		или ТипЗнч(Объект) = тип("ДокументОбъект.КорректировкаЗаказаПоставщику")  тогда 
		если Объект.Дата <= глЗначениеПеременной("ДатаЗапускаПервогоЭтапа")  тогда 	
			Форма.ТолькоПросмотр = Истина;
		КонецЕсли;
...
Процедура ОбработкаВыбораТабличнойЧастиЗакрытогоДокумента(Форма,ЭлементФормы,Колонка,СтандартнаяОбработка) Экспорт 
	//#XX-47 Kalinin V.A. ( 2018-06-18 )	
	ТолькоПросмотр = Форма.ТолькоПросмотр;
	
	ИмяТЧ = ЭлементФормы.Имя;
	
	Если  Колонка.ТолькоПросмотр ИЛИ ЭлементФормы.ТолькоПросмотр тогда 
	...
	Попытка
		Значение = ЭлементФормы.ТекущиеДанные[Колонка.Имя];
	Исключение
		Возврат;
	КонецПопытки;
	
	СтандартнаяОбработка = не ТолькоПросмотр;
	
	ИмяТипа = XMLТипЗнч(Значение).ИмяТипа;
	
	Если СтрНайти(ИмяТипа,"CatalogRef") или СтрНайти(ИмяТипа,"DocumentRef") тогда
		
		ОткрытьФорму(Значение.Метаданные().ПолноеИмя() + ".ФормаОбъекта",Новый Структура("Ключ",Значение),Форма);
	КонецЕсли;	
	
КонецПроцедуры	


Процедура ОбновитьДанныеКонтрагентДоговорВзаиморасчетов(Объект,Контрагент)  Экспорт
//* #XX-271 Kalinin V.A. ( 2018-07-02 )	

Покупатель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент,"Покупатель");	
Если не Покупатель = Неопределено и Покупатель
	и УправлениеВзаиморасчетами.КонтрагентВходитВХолдинг(Контрагент) тогда 	
	ЗаполнитьЗначенияСвойств(Объект,УправлениеВзаиморасчетами.ПолучитьСтруктуруХолдинга(Контрагент,Неопределено),
	 "КонтрагентВзаиморасчетов,ДоговорКонтрагентаВзаиморасчетов"); 
 КонецЕсли;	
 
КонецПроцедуры	

// Процедура - Обработать нажатие кнопки формы
//
// Параметры:
//  Кнопка	 - 	 - 
//  Объект	 - 	 - 
//  Форма	 - 	 - 
//
Процедура ОбработатьНажатиеКнопкиФормы(Кнопка,Объект,Форма)  Экспорт
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ОбработатьНажатиеКнопкиФормы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если Кнопка.Имя = "ИсторияОбменаССайтом"  тогда 
		
		Обработки.ПросмотрИсторииОбменов.ОткрытьИсториюОбменаПоОбъекту(Объект.Ссылка);
		
		// ЛНА, не протестировано
		//ОбработкаОбъект = ОбщегоНазначенияКлиентСервер.ПолучитьОтчетОбработку("ПросмотрИсторииОбменов", Ложь);
		//ОбработкаОбъект.ОткрытьИсториюОбменаПоОбъекту(Объект.Ссылка);
		
	ИначеЕсли Кнопка.Имя = "РедактированиеОбщедоступныхРеквизитов" тогда 
		
		Параметры = Новый Структура("ОбъектСсылка",Объект.Ссылка);
		Форма.Закрыть();
		Описание  = новый ОписаниеОповещения("РедактированиеОбщедоступныхРеквизитов_Завершение",РаботаСДиалогами,Объект.Ссылка);
		ОткрытьФорму("ОбщаяФорма.ФормаРедактированияОбщедоступныхРеквизитов",Параметры,,,,,Описание);
		
		//#XX-2602 Kalinin V.A. ( 2019-06-18 )  /*
		//Немного усовершенствовал общую обработку
	ИначеЕсли Кнопка.Имя = "ДействияФормыПоказатьGUID" тогда 	
		ОбщегоНазначенияКлиент.ПоказатьGUID(Объект.Ссылка);
	ИначеЕсли Кнопка.Имя = "ДействияФормыИсторияИзменений" тогда 	
		ОбщегоНазначенияКлиент.ПоказатьИсториюИзмененияОбъекта(Объект.Ссылка);
	//Семенов И.П. 25.06.2019 XX-2663(
	ИначеЕсли Кнопка.Имя = "ОтчетыИсполнениеСтрокЗаявок" тогда
		Отчеты.ИсполнениеСтрокЗаявок.СформироватьОтчет(Объект, Форма);
	//)Семенов И.П.
	//<< 03.07.2019 Горохов П.Е. ХХ-2847
	ИначеЕсли Кнопка.Имя = "ДействияФормыИсторияРегистрацииОбъектов" тогда
		ФормаСписка = РегистрыСведений.ИсторияРегистрацииОбъектов.ПолучитьФормуСписка();
		ФормаСписка.Отбор.Ссылка.Установить(Объект.Ссылка, Истина);
		ФормаСписка.Открыть();
	//>> 03.07.2019 Горохов П.Е. ХХ-2847
	КонецЕсли;	
...
Процедура ОбработатьНажатиеГиперссылкиФормы(Надпись) Экспорт
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ОбработатьНажатиеГиперссылкиФормы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если Надпись.Имя = "НадписьТолькоПросмотр"  тогда 
		
		Сообщить(Надпись.Подсказка);
		
	КонецЕсли;	

КонецПроцедуры

Процедура РедактированиеОбщедоступныхРеквизитов_Завершение(Результат,Ссылка)   Экспорт 
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_РедактированиеОбщедоступныхРеквизитов_Завершение";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	//#Временная реализация, т.к не обновляются реквизиты Kalinin V.A. ( 2019-04-22 )  /*	
...
Процедура ПередЗаписьюФормы(Объект,Форма,Отказ) экспорт
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ПередЗаписьюФормы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ПроверитьКорректностьЗаполненияДат(Объект,Отказ);
КонецПроцедуры	


Процедура ПроверитьКорректностьЗаполненияДат(Объект,Отказ)
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ПроверитьКорректностьЗаполненияДат";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если НЕ (ТипЗнч(Объект) = тип("ДокументОбъект.ПоступлениеТоваровУслуг") 
		или ТипЗнч(Объект) = тип("ДокументОбъект.РеализацияТоваровУслуг"))  тогда 
		Возврат;
	КонецЕсли;
	МетаданныеОбъекта=   Объект.Ссылка.Метаданные() ;
	
	НачалоПериода =  ТекущаяДата() - 60*60*24*365;//-1 год
	КонецПериода =  ТекущаяДата() + 60*60*24*365;//+1 год
	
	
	
	Для каждого Реквизит из МетаданныеОбъекта.Реквизиты цикл 
		Если  Реквизит.Тип = ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя) или 
			Реквизит.Тип = ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата) тогда 
			Если ЗначениеЗаполнено(Объект[Реквизит.Имя])
				и ( Объект[Реквизит.Имя] < НачалоПериода 
				или   Объект[Реквизит.Имя] > КонецПериода )   Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не корректно заполненена дата" +Реквизит.Имя+ ".Значение "
				+Формат(Объект[Реквизит.Имя],"ДФ=dd.MM.yy") +" не соответствует доступному интервалу +- 1 год",,,,Отказ);			
			КонецЕсли;	
		КонецЕсли;
	КонецЦикла;	
...
//Семенов И.П. 25.06.2019 XX-XX-2663(
Процедура ДобавитьПодменюОтчетыВКоманднуюПанель(Объект, Форма, КоманднаяПанель, ИмяКнопкиДействия)
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ДобавитьПодменюОтчетыВКоманднуюПанель";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ПодменюОтчеты = КоманднаяПанель.Кнопки.Найти("Отчеты");
	
	Если ПодменюОтчеты = Неопределено Тогда 
		
		ПодменюДействия = КоманднаяПанель.Кнопки.Найти(ИмяКнопкиДействия);
		Если ПодменюДействия = Неопределено Тогда
			ПодменюОтчеты = КоманднаяПанель.Кнопки.Добавить("Отчеты", ТипКнопкиКоманднойПанели.Подменю);
		Иначе
			ИндексКнопки = КоманднаяПанель.Кнопки.Индекс(ПодменюДействия) + 1;
			ПодменюОтчеты = КоманднаяПанель.Кнопки.Вставить(ИндексКнопки,"Отчеты", ТипКнопкиКоманднойПанели.Подменю);
		КонецЕсли;	
		
	КонецЕсли;
	
	ЗаполнитьПодменюОтчеты(Объект, Форма, ПодменюОтчеты);
	
КонецПроцедуры

Процедура ЗаполнитьПодменюОтчеты(Объект, Форма, ПодменюОтчеты)
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСДиалогами_ЗаполнитьПодменюОтчеты";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ДокументыДляИсполнениеСтрокЗаявок = Новый Массив;
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.ЗаявкаПокупателя"));
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.КорректировкаЗаявкиПокупателя"));
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.ЗаказПоставщику"));
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.КорректировкаЗаказаПоставщику"));
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.ПоступлениеТоваровУслуг"));
	ДокументыДляИсполнениеСтрокЗаявок.Добавить(Тип("ДокументОбъект.РеализацияТоваровУслуг"));
	
	Если ДокументыДляИсполнениеСтрокЗаявок.Найти(ТипЗнч(Объект)) <> Неопределено Тогда 
		
		ИсполнениеСтрокЗаявок = ПодменюОтчеты.Кнопки.Найти("ОтчетыИсполнениеСтрокЗаявок");
		Если ИсполнениеСтрокЗаявок = Неопределено Тогда 
			
			ДействиеФормы = Новый Действие("ДействиеФормыНажатиеКнопки");
			ИсполнениеСтрокЗаявок = ПодменюОтчеты.Кнопки.Добавить("ОтчетыИсполнениеСтрокЗаявок",ТипКнопкиКоманднойПанели.Действие,"Исполнение строк заявок", ДействиеФормы);
