...
Функция ПредставлениеАдреса(Знач АдреснаяСтрока, Способ = 0, ПредставлениеПустого = "<<Адрес не задан>>") Экспорт
	...
	Если АдресСоответствуетТребованиям(СтруктураАдрес) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураАдрес.Страна) Тогда
			СсылкаНаСтрану = Справочники.СтраныМира.НайтиПоКоду(СтруктураАдрес.Страна);
...
Функция ОтветственныеЛицаОрганизации(Организация, ДатаСреза, Исполнитель = Неопределено, ВидДокументаДляДоверености = Неопределено) Экспорт

	лКлючАлгоритма = "ОбщийМодуль_РегламентированнаяОтчетность_ОтветственныеЛицаОрганизации";
	...
	Если Организация <> Неопределено тогда
		...
		ЗаполненыДанныеПоДоверенностям = Ложь;
		Если ВидДокументаДляДоверености <> Неопределено И ЗначениеЗаполнено(ПараметрыСеанса.ТекущийПользователь.ФизЛицо) Тогда
			
			ЗапросПоДоверенностям = Новый Запрос;
			ЗапросПоДоверенностям.УстановитьПараметр("Организация"	, Организация);
			ЗапросПоДоверенностям.УстановитьПараметр("ВидДокумента"	, ВидДокументаДляДоверености);
			ЗапросПоДоверенностям.УстановитьПараметр("ДатаСреза"	, ДатаСреза);
			ЗапросПоДоверенностям.УстановитьПараметр("ФизЛицо"		, ПараметрыСеанса.ТекущийПользователь.ФизЛицо);
			ЗапросПоДоверенностям.Текст = 
			"ВЫБРАТЬ
			|	ДанныеДовереностейСрезПоследних.ОтветственноеЛицо,
			|	ДанныеДовереностейСрезПоследних.ФизическоеЛицо,
			|	ДанныеДовереностейСрезПоследних.НомерДоверенности,
			|	ДанныеДовереностейСрезПоследних.Период
			|ИЗ
			|	РегистрСведений.ДанныеДовереностей.СрезПоследних(
			|			&ДатаСреза,
			|			Организация = &Организация
			|				И ВидДокумента = &ВидДокумента
			|				И ФизическоеЛицо = &ФизЛицо) КАК ДанныеДовереностейСрезПоследних";
			
			Выборка = ЗапросПоДоверенностям.Выполнить().Выбрать();
			
			ШаблонСтроки = "по доверенности №%1 от %2
			|%3";
			
			Пока Выборка.Следующий() Цикл
				Если Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизации.Руководитель Тогда
					Результат.Руководитель = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСтроки, 
					Выборка.НомерДоверенности, 
					Формат(Выборка.Период, "ДФ=dd.MM.yyyy"), 
					ЭлектронныеДокументы.ФамилияИнициалыФизЛица(Выборка.ФизическоеЛицо));
				...
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизации.ГлавныйБухгалтер Тогда
					Результат.ГлавныйБухгалтер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСтроки, 
					Выборка.НомерДоверенности, 
					Формат(Выборка.Период, "ДФ=dd.MM.yyyy"), 
					ЭлектронныеДокументы.ФамилияИнициалыФизЛица(Выборка.ФизическоеЛицо));
				...
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизации.Кассир Тогда
					Результат.Кассир = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСтроки, 
					Выборка.НомерДоверенности, 
					Формат(Выборка.Период, "ДФ=dd.MM.yyyy"), 
					ЭлектронныеДокументы.ФамилияИнициалыФизЛица(Выборка.ФизическоеЛицо));
				...
				КонецЕсли;
				ЗаполненыДанныеПоДоверенностям = Истина;
			КонецЦикла;
		КонецЕсли;

		Если Не ЗаполненыДанныеПоДоверенностям Тогда
		// 28.06.19 Строганов Роман < XX-2720: Добавился параметр ВидДокументаДоверенности. Если он заполнен - подбираются данные о доверенностях ответственных лиц. 
