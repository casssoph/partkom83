...
Процедура ДополнитьМассивУзловДляВыгрузкиДокумента(Элемент, МассивУзловДляПередачи, 
	...
	Если ТипЭлемента = Тип("ДокументСсылка.АвансовыйОтчет")
		...
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг") Тогда
		
		Если ТипЭлемента = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг")
			ИЛИ ТипЭлемента = Тип("ДокументОбъект.КорректировкаПоступленияТоваровУслуг")
		...
		Иначе
			СтруктураПараметров.Вставить("СкладОрдер");
		КонецЕсли;
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.ЗаявкаПокупателя")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.ЗаявкаПокупателя") Тогда
	...
	Если НЕ (
		...
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.ЗаявкаПокупателя")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.ЗаявкаПокупателя")
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.СчетФактураВыданный")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.СчетФактураВыданный")
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.СчетФактураПолученный")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.СчетФактураПолученный")
	...
	КонецЕсли;
	Если ТипЭлемента = Тип("ДокументСсылка.ЗаявкаПокупателя")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.ЗаявкаПокупателя") Тогда
		//ИЛИ ТипЭлемента = Тип("ДокументСсылка.СчетНаОплатуПокупателю")
		...
		Пока Номер <= МассивУзловДляПередачи.Количество() - 1 Цикл
			...
			Если ТипЗнч(УзелДляПередачи) = Тип("ПланОбменаСсылка.ОбменУправлениеТорговлейБухгалтерияКОРП") Тогда
				Если УзелДляПередачи.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю.НеВыгружать Тогда
					МассивУзловДляПередачи.Удалить(Номер);
				ИначеЕсли УзелДляПередачи.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю.ЗаявкаПокупателя Тогда
					//И (ТипЭлемента = Тип("ДокументСсылка.СчетНаОплатуПокупателю") ИЛИ ТипЭлемента = Тип("ДокументОбъект.СчетНаОплатуПокупателю")) Тогда
					МассивУзловДляПередачи.Удалить(Номер);
				ИначеЕсли УзелДляПередачи.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаявкаПокупателяСчетНаОплатуПокупателю.СчетНаОплатуПокупателю
					И (ТипЭлемента = Тип("ДокументСсылка.ЗаявкаПокупателя") ИЛИ ТипЭлемента = Тип("ДокументОбъект.ЗаявкаПокупателя")) Тогда
					МассивУзловДляПередачи.Удалить(Номер);
				Иначе
					Номер = Номер + 1;
				КонецЕсли;
...
Процедура ПриЗаписиРегистрыНакопленияДляОбменаТорговляБухгалтерияКОРППриЗаписи(Источник, Отказ, Замещение) Экспорт
	...
	Если ТипРегистратора = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
		...
		ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(МассивУзлов, Источник.Отбор.Регистратор.Значение);
		//)Семенов И.П.
	КонецЕсли;
