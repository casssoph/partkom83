...
Функция ПолучитьСтруктуруТаблицРазмещения(СтруктураТаблицДокумента,СтруктураПараметров,ОшибкиРазмещения = Неопределено)  Экспорт
	...
	Пока ВыборкаНоменклатуры.Следующий() Цикл
		 //#XX-2423 Kalinin V.A. ( 2019-06-13 )  /* немного поправил поиск по заменам
		...
		Если НоваяСортировка Тогда
			...
			ПараметрыУпорядочивания.Вставить("Номенклатура", 		 	ВыборкаНоменклатуры.НоменклатураПоиска);
		...
		Иначе
			...
			ДанныеЗаказов =  ПолучитьВыборкуЗаказовПоСтроке(МенеджерВремТаблиц,ВыборкаНоменклатуры.НоменклатураПоиска,ВыборкаНоменклатуры.Цена);
		...
		Пока ДанныеЗаказов.Следующий() и КоличествоКРазмещению > 0 цикл
			...
			Если СтрокаЗаказаНеКорректная(ВыборкаНоменклатуры,СтрокаЗаказа,СтруктураПараметров,ОшибкиРазмещения, СообщениеОшибки) тогда 
				...
				Если НЕ ПустаяСтрока(СообщениеОшибки) Тогда
					...
					Если  КоличествоКРазмещению >= СтрокаЗаказа.Количество тогда 
						ДобавитьСтрокуРазмещениявТаблицу(ТаблицаНеРазмещенного,Номенклатура,СтрокаЗаказа.СтрокаЗаявки,ВыборкаНоменклатуры.СтрокаПрихода,СтрокаЗаказа.Количество,ВыборкаНоменклатуры.НомерСтроки, СообщениеОшибки);
						КоличествоКРазмещению = КоличествоКРазмещению - СтрокаЗаказа.Количество;
						СтрокаЗаказа.Количество = 0;
					Иначе 
						ДобавитьСтрокуРазмещениявТаблицу(ТаблицаНеРазмещенного,Номенклатура,СтрокаЗаказа.СтрокаЗаявки,ВыборкаНоменклатуры.СтрокаПрихода,КоличествоКРазмещению,ВыборкаНоменклатуры.НомерСтроки, СообщениеОшибки);
			...
			Если  КоличествоКРазмещению >= СтрокаЗаказа.Количество тогда 
				ДобавитьСтрокуРазмещениявТаблицу(ТаблицаРазмещения,Номенклатура,СтрокаЗаказа.СтрокаЗаявки,ВыборкаНоменклатуры.СтрокаПрихода,СтрокаЗаказа.Количество,ВыборкаНоменклатуры.НомерСтроки);
				КоличествоКРазмещению = КоличествоКРазмещению - СтрокаЗаказа.Количество;
				СтрокаЗаказа.Количество = 0;
			Иначе 
				ДобавитьСтрокуРазмещениявТаблицу(ТаблицаРазмещения,Номенклатура,СтрокаЗаказа.СтрокаЗаявки,ВыборкаНоменклатуры.СтрокаПрихода,КоличествоКРазмещению,ВыборкаНоменклатуры.НомерСтроки);
				СтрокаЗаказа.Количество = СтрокаЗаказа.Количество -КоличествоКРазмещению;
				КоличествоКРазмещению = 0;	 
			КонецЕсли;	 
		КонецЦикла;
		...
		Если КоличествоКРазмещению>0 тогда
			...
			Если НетВЗаказе Тогда
				СообщениеОшибки = "**********"+Символы.ПС+"Строка №"+ВыборкаНоменклатуры.НомерСтроки+", Артикул "+Номенклатура.Артикул+Символы.ПС+
				"Нет в заказе!";
			Иначе
				Разделитель = ?(ЕстьОшибкаВЦене, ""+Символы.ПС, "**********"+Символы.ПС);
				СообщениеОшибки = Разделитель + "Строка №"+ВыборкаНоменклатуры.НомерСтроки+", Артикул "+Номенклатура.Артикул+Символы.ПС+
			...
			КонецЕсли;
			СтрокаЗаявки = "";
			ДобавитьСтрокуРазмещениявТаблицу(ТаблицаНеРазмещенного,Номенклатура,СтрокаЗаявки,ВыборкаНоменклатуры.СтрокаПрихода,КоличествоКРазмещению,ВыборкаНоменклатуры.НомерСтроки, СообщениеОшибки);
			// КП 29.05.2019 Горохов П.Е. http://jira.part-kom.ru/browse/XX-1443
		КонецЕсли;			
	КонецЦикла;
...
Функция УпорядочитьОстаткиЗаказовПоПравиламРазмещения(МенеджерВременныхТаблиц, ПараметрыУпорядочивания) Экспорт
	...
	Если ПараметрыУпорядочивания.Свойство("Контрагент") Тогда
		...
		ПараметрыУпорядочивания.Вставить("ПроцентОтклоненияЦенПрихода", 	?(РеквизитыКонтрагента.ПроцентОтклоненияЦенПрихода = 0, Константы.ПроцентОтклоненияЦенПрихода.Получить(), РеквизитыКонтрагента.ПроцентОтклоненияЦенПрихода));
...
Функция ТексЗапросаРазмещения_ПоступлениеТоваровУслуг()
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2605) Тогда
		...
		ТекстЗапроса =  "ВЫБРАТЬ
		                ...
		                |	ВЫБРАТЬ РАЗЛИЧНЫЕ
		                |		ВЫБОР
		                |			КОГДА ВТТоваров.НоменклатураЗаказа = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		                |				ТОГДА ВТТоваров.Номенклатура
		                |			ИНАЧЕ ВТТоваров.НоменклатураЗаказа
		                |		КОНЕЦ,
		                |		ЛОЖЬ,
		                |		ВЫБОР
		                |			КОГДА ВТТоваров.НоменклатураЗаказа = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		                |				ТОГДА ЗНАЧЕНИЕ(справочник.Номенклатура.ПустаяСсылка)
		                |			ИНАЧЕ ВТТоваров.Номенклатура
		                |		КОНЕЦ
		                |	ИЗ
		                ...
		                |	ВЫБОР
		                |		КОГДА ЕСТЬNULL(втОстаткиПоЗаявкам.Количество, 0) < ВТТаблицаЗаказовБезУчетаЗаявок.Количество
		                |			ТОГДА ЕСТЬNULL(втОстаткиПоЗаявкам.Количество, 0) - ВТТаблицаЗаказовБезУчетаЗаявок.КоличествоРазмещено
