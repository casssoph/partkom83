...
Функция СтрокаЗаказаНеКорректная(ВыборкаНоменклатуры,СтрокаЗаказа,СтруктураПараметров,ОшибкиРазмещения = неопределено)
	...
	Если не СтруктураПараметров.МожноПровести тогда 
		...
		Если ОтклонениеЦены > СтруктураПараметров.ПроцентОтклоненияЦенПрихода Тогда 
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиРазмещения,,СообщениеОшибки);
			Отказ = Истина;
		Конецесли;		
	КонецЕсли;
...
Процедура ДобавитьСтрокуРазмещениявТаблицу(Таблица,Номенклатура,СтрокаЗаявки,СтрокаПрихода,Количество,ПолностьюРазмещен = Ложь)
