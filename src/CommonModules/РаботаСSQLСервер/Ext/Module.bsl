...
Функция ВыполнитьЗапросSQL(ТекстЗапроса) Экспорт
	...
	Попытка
		...
		СоединениеSQL.ConnectionString = РаботаСSQL.СтрокаСоединенияSQL();
...
//ХудинВВ XX-2449 08072019
Функция ОпределитьПараметрыДляSQLПодключения() Экспорт
...
Функция СоздатьВременногоПользователя(ПараметрыВременногоПользователя)
	
	лКлючАлгоритма = "ОбщийМодуль_РаботаСSQLСервер_ПолучитьИмяАдминаНаСервере";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Процедура УдалитьВременногоПользователя(ПараметрыВременногоПользователя)
