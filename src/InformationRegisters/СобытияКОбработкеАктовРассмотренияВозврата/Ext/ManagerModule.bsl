...
Процедура Добавить(АктРассмотренияВозврата = Неопределено, Событие, ПараметрСобытия = Неопределено, НеДобавлятьЕслиУжеЕсть = Истина, КоличествоПопыток = 0, ДополнительныеСведения = Неопределено, Период = Неопределено) Экспорт
	
	лКлючАлгоритма = "РегистрСведений_СобытияКОбработкеАктовРассмотренияВозврата_МодульМенеджера_Добавить";
	...
	Если НужноДобавить Тогда	
		...
		МЗ.ДополнительныеСведения 	= ?(ТипЗнч(ДополнительныеСведения) = Тип("Структура"), ЗначениеВСтрокуВнутр(ДополнительныеСведения), ДополнительныеСведения);
		МЗ.Записать();	
