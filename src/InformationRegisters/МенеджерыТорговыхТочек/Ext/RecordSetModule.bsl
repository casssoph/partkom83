...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Сайт", "Справочник: Контрагенты", Ложь) И НЕ ОбменДанными.Загрузка Тогда
		...
		Для Каждого Запись Из ЭтотОбъект Цикл
			Если ЗначениеЗаполнено(Запись.ТорговаяТочка) И ЗначениеЗаполнено(Запись.ТорговаяТочка.Владелец) И НЕ Запись.ТорговаяТочка.Владелец.ПометкаУдаления Тогда
				//Семенов И.П. 31.01.2019 XX-1768(
				//ПланыОбмена.ЗарегистрироватьИзменения(Узел, Запись.ТорговаяТочка.Владелец);
				ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(Узел, Запись.ТорговаяТочка.Владелец);
				//)Семенов И.П.
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
