...
Процедура ДобавитьДанныеСостояниеСтрокЗаявок(Список, НомерСообщения, СтруктураХранения) Экспорт
	...
	ОтметитьОтправленныеСтрокиВSQL(НомерСообщения, СтрокаОтбораЗаявок, СтруктураХранения);
	
КонецПроцедуры
Процедура ФиксацияПринятогоСообщения(НомерСообщения, СтруктураХранения) Экспорт
	
	ТекстЗапросаФиксацииСообщения = ТекстЗапросаФиксацииСообщения(НомерСообщения, СтруктураХранения);
...
КонецФункции
Процедура ОтметитьОтправленныеСтрокиВSQL(НомерСообщения, СтрокаОтбораSSID, СтруктураХранения)
	
	ТекстЗапросаОтметкиОтправленного = ТекстЗапросаОтметкиОтправленного(НомерСообщения, СтрокаОтбораSSID, СтруктураХранения);
	...
	Попытка
		...
		СоединениеSQL.Open();
		КомандаSQL.ActiveConnection   = СоединениеSQL;
		КомандаSQL.CommandText = ТекстЗапросаОтметкиОтправленного;
		КомандаSQL.Execute();
