...
Процедура ОтметитьОтправленныеСтрокиВSQL(НомерСообщения, СтрокаОтбораSSID)
	...
	Попытка
		...
		КомандаSQL.CommandText = ТекстЗапроса(НомерСообщения, СтрокаОтбораSSID);
		КомандаSQL.Execute();
...
КонецПроцедуры
Функция ТекстЗапроса(НомерСообщения, СтрокаОтбораSSID)
	...
	Если ОписаниеТаблицы = Неопределено Тогда
		Отказ = Истина;
	Иначе
		ИмяТаблицыХранения = ОписаниеТаблицы.ИмяТаблицыХранения;
		ОписаниеПолей = ОписаниеТаблицы.Поля;
		ПолеДатаОтправки = ИмяПоляТаблицы(ОписаниеПолей, "ДатаОтправки", Отказ);
		ПолеНомерСообщенияОтправленного = ИмяПоляТаблицы(ОписаниеПолей, "НомерСообщенияОтправленного", Отказ);
		ПолеIDSite = ИмяПоляТаблицы(ОписаниеПолей, "IDSite", Отказ);
	КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		ТекстЗапроса =	"UPDATE _" + ИмяТаблицыХранения + " set _" + ПолеНомерСообщенияОтправленного + " = " + Формат(НомерСообщения, "ЧГ=") + ", _" + ПолеДатаОтправки + " = '" + Дата + "' " +
						"WHERE _" + ПолеIDSite + " in (" + СтрокаОтбораSSID + ")";
	Иначе
