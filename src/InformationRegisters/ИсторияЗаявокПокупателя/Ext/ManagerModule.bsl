...
Процедура ОтметитьЗаписиКакВыгруженные(Выборка) Экспорт
	...
	ТекстЗапросаSQL = ТекстЗапросаУстановкаРегистрации(ТаблицаОтбора);
	ОписаниеОшибки = DataExchangeModule.ВыполнитьЗапросSQL(ТекстЗапросаSQL)
	
КонецПроцедуры
Функция ТекстЗапросаУстановкаРегистрации(ТаблицаОтбора)
	...
	Если НЕ Отказ Тогда
		ДатаОтправкиПакета = "dateadd(year,2000, '" + Формат(ТекущаяДата(), "ДФ='yyyy-MM-dd HH:mm:ss'") + "')";
		Set = " set " + ПолеНомерСообщенияОтправленного + " = 0, " +
						ПолеДатаОтправки + " = " + ДатаОтправкиПакета + ", " + 
						ПолеПолученоСайтом + " =  1, "+
						ПолеДатаОбработкиСайтом + " = " + ДатаОтправкиПакета + Символы.ПС;
		СтрокаСоединения = " on " + ПолеСтрокаПериод + " = t.p and " +
									ПолеСтрокаРегистратор + " = t.r and " +
									ПолеНомерСтроки + " = t.n and " +
									ПолеСтрокаЗаявки + " = t.sz and " +
