...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Информация.Отчет = "Отчет.ВзаиморасчетыСКонтрагентами";
	...
	//Если ВариантыОтчетовПовтИсп.Параметры().Найти(НастройкиОтчета.ОтчетСсылка) <> Неопределено Тогда
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	ПоказыватьКомандыВариантовОтчетов = Истина ;
...
Процедура ЗаполнитьКомандыВыбораВариантов()
	...
	КлючОтчета = "Отчет" + "." + НастройкиОтчета.ОтчетСсылка.Имя;

	ВсеВариантыОтчета = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьСписок(КлючОтчета);
	Для Каждого ВариантОтчета Из ВсеВариантыОтчета Цикл
		
		ЭлементСправочника = Справочники.ВариантыОтчетов.НайтиПоРеквизиту("КлючВарианта", ВариантОтчета.Значение);
		Найденные = ВариантыФормы.НайтиСтроки(Новый Структура("КлючВарианта, Найден", ВариантОтчета.Значение, Ложь));
		Если Найденные.Количество() = 1 Тогда
			ВариантФормы = Найденные[0];
			ВариантФормы.Найден = Истина;
			Кнопка = Элементы.Найти(ВариантФормы.ИмяКоманды);
			Кнопка.Видимость = Истина;
			Кнопка.Заголовок = ВариантОтчета.Представление;
			Элементы.Переместить(Кнопка, Группа);
		Иначе
			//СтрокаТаблицы.Ссылка.Настройки.Получить()
			ПоследнийИндекс = ПоследнийИндекс + 1;
			ВариантФормы = ВариантыФормы.Добавить();
			ЗаполнитьЗначенияСвойств(ВариантФормы, ВариантОтчета);
			ВариантФормы.Найден = Истина;
		...
		КонецЕсли;
		Кнопка.Пометка = (КлючТекущегоВарианта = ВариантОтчета.Значение);
		...
		//	СтрокаВариантовОтчетов.КлючОбъекта = "Отчет." + НастройкиОтчета.ОтчетСсылка.Имя;
	...
	//	//Если СтрокаТаблицы.КлючВарианта = "РасчетыСКонтрагентами" или СтрокаТаблицы.КлючВарианта = "РасчетыСКлиентами" Тогда 
...
Процедура ОбновитьИнформациюОВариантеОтчета()
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Наименование = "" Тогда
			ЭлСпр = Справочники.ВариантыОтчетов.НайтиПоРеквизиту("КлючВарианта", КлючТекущегоВарианта).ПолучитьОбъект();
			ЭлСпр.Наименование = ОтчетНаименованиеТекущегоВарианта;
			ЭлСпр.Записать();
			КонецЕсли;
		...
		КонецЕсли;
	Иначе
		НовыйОтчет = Справочники.ВариантыОтчетов.СоздатьЭлемент();
		НовыйОтчет.Отчет = НастройкиОтчета.ОтчетССылка;
		НовыйОтчет.КлючВарианта = КлючТекущегоВарианта;
		НовыйОтчет.Наименование = ОтчетНаименованиеТекущегоВарианта;
		НовыйОтчет.Записать();
		НастройкиОтчета.Вставить("ВариантСсылка",          НовыйОтчет.Ссылка);
		//НастройкиОтчета.Вставить("Наименование",          ОтчетНаименованиеТекущегоВарианта);
		
		//НастройкиОтчета.Вставить("КлючЗамеров",            НовыйОтчет.ПредопределенныйВариант.КлючЗамеров);
		//НастройкиОтчета.Вставить("ПредопределенныйСсылка", НовыйОтчет.ПредопределенныйВариант.Ссылка);
		НастройкиОтчета.Вставить("ИмяИсходногоВарианта",   КлючТекущегоВарианта);
		НастройкиОтчета.Вставить("Пользовательский",       НовыйОтчет.Пользовательский);
