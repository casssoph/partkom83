...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Организации.Ссылка) КАК Смм
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если Параметры.ЗначенияДляВыбора.Количество() <> ВыборкаДетальныеЗаписи.смм  и Параметры.Представление = "Организация" Тогда
			Параметры.ЗначенияДляВыбора.Очистить();
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Параметры.ЗначенияДляВыбора.Добавить(ВыборкаДетальныеЗаписи.ссылка);
			КонецЦикла;
