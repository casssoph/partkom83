...
Функция ПроверитьНаименование() Экспорт
	
	лКлючАлгоритма = "Справочник_Номенклатура_МодульОбъекта_ПроверитьНаименование";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	// 15.07.19 Строганов Роман > Вырезаю в рамках XX-2842
	//СписокДляПроверкиНаименования.Добавить(СокрЛП(Врег(Артикул)));
	// 15.07.19 Строганов Роман <
	...
	ЕстьКириллица = СтроковыеФункцииКлиентСервер.ЕстьКириллицаВСтроке(Наименование);
	
	Ошибка = НЕ ЕстьКириллица;
	Если Ошибка Тогда 
		// 15.07.19 Строганов Роман > XX-2842
		Если ЗначениеЗаполнено(Артикул) Тогда
			НовоеНаименование = "Деталь " + Артикул; 
			Наименование = НовоеНаименование;
			НаименованиеПолное = НовоеНаименование;
			Ошибка = Ложь;
		// 15.07.19 Строганов Роман < XX-2842
		Иначе
			ТекстОшибки = "Содержит только латиницу и/или цифры, добавьте слова на русском или заполните артикул"
		КонецЕсли;		 
	КонецЕсли;
	Структура = Новый Структура("Ошибка,ТекстОшибки", Ошибка, ТекстОшибки);
	Возврат Структура;
