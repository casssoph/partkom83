...
Функция КонтрагентыПоМенеджеруФилиалу(СтруктураПараметров) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ИспользоватьОтборФилиал
		|				ТОГДА Контрагенты.Ссылка.Регион.Филиал %ВидСравненияФилиал% (&Филиалы)
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ";
	...
	Запрос.УстановитьПараметр("ИспользоватьОтборФилиал", 	СтруктураПараметров.ОтборФилиал.Филиалы.Количество() > 0);
	Запрос.УстановитьПараметр("Филиалы", 					СтруктураПараметров.ОтборФилиал.Филиалы);
	
	ВнутреннееСоединениеТекст = "ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоМенеджерам КАК ПоМенеджерам ПО Контрагенты.Ссылка = ПоМенеджерам.Контрагент";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ВнутреннееСоединение%", 
		?(ИспользоватьОтборМенеджер, ВнутреннееСоединениеТекст,""));
...
Функция ФилиалКонтрагента(Контрагент) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(Контрагенты.Регион.Филиал, НЕОПРЕДЕЛЕНО) КАК Филиал
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
