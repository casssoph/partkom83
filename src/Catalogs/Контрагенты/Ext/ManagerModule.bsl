...
Функция ПросроченнаяЗадолженностьКонтрагента(Контрагент, ДатаМоментВремени = Неопределено, ПоГоловномуКонтрагенту = Неопределено) Экспорт
	...
	Если ПоГоловномуКонтрагенту Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	ДОБАВИТЬКДАТЕ(РеализацияТоваровУслуг.ДатаОплаты, ДЕНЬ, &ЛимитДнейЗадолженности + ЕСТЬNULL(РеализацияТоваровУслуг.ДопустимоеЧислоДнейЗадолженности, 0)) >= &Период
		               |
		               |СГРУППИРОВАТЬ ПО
	...
	Если ДатаМоментВремени = Неопределено ИЛИ ТипЗнч(ДатаМоментВремени) = Тип("Дата") Тогда
		Запрос.УстановитьПараметр("Период", ?(ЗначениеЗаполнено(ДатаМоментВремени), ДатаМоментВремени, ТекущаяДата()));
	Иначе //МоментВремени
		Запрос.УстановитьПараметр("Период", Новый Граница(ДатаМоментВремени, ВидГраницы.Исключая));
	КонецЕсли;
