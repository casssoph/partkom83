...
Функция КонтрагентыПоМенеджеру(СтруктураПараметров) Экспорт
	...
	Запрос.Текст = 
		...
		|	МенеджерыТорговыхТочекСрезПоследних.ТорговаяТочка.Владелец КАК Контрагент
		|ИЗ
		|	РегистрСведений.МенеджерыТорговыхТочек.СрезПоследних(, ВидМенеджера = &ВидМенеджера) КАК МенеджерыТорговыхТочекСрезПоследних
		|ГДЕ
		|	МенеджерыТорговыхТочекСрезПоследних.Менеджер Не В (&Менеджеры)";
	
	Запрос.УстановитьПараметр("ВидМенеджера", СтруктураПараметров.ВидМенеджера);
	Запрос.УстановитьПараметр("Менеджеры", СтруктураПараметров.Менеджеры);
	
	Если СтруктураПараметров.ВидСравнения = ВидСравнения.ВСписке Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Не В", "В");
	КонецЕсли;
