...
Функция ПроверитьЛогинКонтрагентаВБазеСПб() экспорт
	...
	//предполагается, что перед записью КА обменом с Элма уже записаны логин и основная ТТ
	Если НЕ ЗначениеЗаполнено(ОсновнаяТорговаяТочка) Тогда
		#Если Клиент Тогда
			Сообщить("не указана основная торговая точка, запись элемента невозможна");
		#КонецЕсли
		Возврат Истина;
	...
	Запрос.УстановитьПараметр("Владелец", ОсновнаяТорговаяТочка);
	
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() Тогда
		//нет логина - нечего проверять
		Возврат Ложь;
	КонецЕсли;
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	вхЛогин = СокрЛП(Выборка.Код);
