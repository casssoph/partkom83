...
Процедура ПередЗаписью(Отказ)
	...
	Если ПроверятьПрайсПоставщика И ЗначениеЗаполнено(ПрайсПоставщика) Тогда
		...
		Если Не ОбщегоНазначения.СсылкаСуществует(ПрайсПоставщика) Тогда
			ТекстОшибки = "[КонтрольПрайсаПоставщика] Ошибка записи строки заявки, битая ссылка в прайсе поставщика. Строка заявки: "+ЭтотОбъект+", прайс поставщика: "+ПрайсПоставщика;
		КонецЕсли;
		
		КодПрайса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрайсПоставщика, "Код"); 
		Если КодПрайса = 0 Тогда
			ТекстОшибки = "[КонтрольПрайсаПоставщика] Ошибка записи строки заявки, не заполнен код в прайсе поставщика. Строка заявки: "+ЭтотОбъект;
		КонецЕсли;
