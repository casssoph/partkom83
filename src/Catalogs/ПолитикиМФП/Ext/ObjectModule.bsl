...
Процедура ПроверитьОрганизацииПокупки(Отказ)
	...
	Если Владелец.Франчайзи И РазрешенаЦепочкаЗакупок Тогда
		Сообщить("Для организации-франчайзи запрещено использовать цепочку закупок!");
		Отказ = Истина;
	КонецЕсли;
	
	Для каждого Стр ИЗ СобственныеОрганизацииРазрешенаПокупка Цикл
		Если ЗначениеЗаполнено(Стр.Организация) И Стр.Организация.Франчайзи Тогда
			Сообщить(""+Стр.Организация.Франчайзи+": организация-франчайзи не может выступать в роли организации для закупки!");
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;
