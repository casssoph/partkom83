...
Функция ПолучитьПараметрыПолитикиМФПДляОрганизации(Организация, ДатаДействия, ВызыватьИсключение = Ложь) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПолучитьПараметрыПолитикиМФПДляОрганизации";
	...
	ОрганизацииДляЗакупки.Колонки.Добавить("МаксПриоритет", 		Новый ОписаниеТипов("Число"), Новый КвалификаторыЧисла(15,0));
	...
	МаксПриоритет 	= ?(ПараметрыПолитики.РазрешенаЦепочкаЗакупок, 999, 1);	
	Приоритет 		= 0;
	
	НовСтр 					= ОрганизацииДляЗакупки.Добавить();
	НовСтр.Приоритет 		= Приоритет;
	НовСтр.МаксПриоритет 	= МаксПриоритет;
...
Процедура ДобавитьОрганизацииЗакупкиРекурсивно(ОрганизацииДляЗакупки, Приоритет, ОрганизацииПродажМФП, МаксПриоритет)
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ДобавитьОрганизацииЗакупкиРекурсивно";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Для каждого СтрокаТЗ Из ОрганизациииДляОбхода Цикл
		...
		Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если ОрганизацииДляЗакупки.Найти(НайденнаяСтрока.ОрганизацияПродавец, "Организация") = Неопределено Тогда
				Если Приоритет <= СтрокаТЗ.МаксПриоритет Тогда
					НовСтр 						= ОрганизацииДляЗакупки.Добавить();
					НовСтр.Приоритет 			= Приоритет;
					...
					НовСтр.МаксПриоритет 		= ?(НайденнаяСтрока.ПолитикаРазрешенаЦепочкаЗакупок, 999, НовСтр.Приоритет);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьОрганизацииПродажМФП(ДатаДействия, СпособПередачиТоваров = Неопределено) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПолучитьОрганизацииПродажМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПравилаСозданияМФП(ДатаСоздания, СпособПередачиТоваров = Неопределено) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПравилаСозданияМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Процедура ОбработатьТабРекурсивно(ТаблицаМногошаговыхПродаж, Порядок)
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ОбработатьТабРекурсивно";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ОбработатьСтрокуТаблицы(ПокупательПартии, СтрокаТаблицы, ТаблицаМногошаговыхПродаж,  Порядок)
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ОбработатьСтрокуТаблицы";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция СтруктураСтрокиПравил()
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_СтруктураСтрокиПравил";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПроверитьДобавитьВТаблицу(вхТаблица, ДобавляемыыеДанные, ПоляОтбора = "")
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПроверитьДобавитьВТаблицу";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПравилаСозданияВозвратовМФП(ДатаСоздания) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПравилаСозданияВозвратовМФП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
//Наценка
Процедура ДобавитьПроцентыНаценки(ПравилаСозданияМФП, Сортировка = "УБЫВ", Условие = "ПравилаСозданияМФП.Порядок >= &Порядок") Экспорт
	
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ДобавитьПроцентыНаценки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Для каждого СтрокаЭтапа Из ПравилаСозданияМФП Цикл
		 СтрокаЭтапа.ПроцентыНаценки = ПроцентыНаценкиДляЭтапаЦепочки(ПравилаСозданияМФП, СтрокаЭтапа, Сортировка, Условие);
	КонецЦикла;

КонецПроцедуры

Функция ПроцентыНаценкиДляЭтапаЦепочки(ПравилаСозданияМФП, СтрокаЭтапа, Сортировка = "УБЫВ", Условие = "ПравилаСозданияМФП.Порядок >= &Порядок") Экспорт
	
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПроцентыНаценкиДляЭтапаЦепочки";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "УБЫВ", Сортировка);
...
Функция ПроцентыНаценкиПоСтроке(ПравилаСозданияМФП, ДанныеСтроки) Экспорт
	
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПроцентыНаценкиПоСтроке";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПолитикаУникальна(Ссылка, Владелец, ПериодДействия) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПолитикаУникальна";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПолучитьПолитикуМФПДляОрганизации(Организация, ДатаДействия, ВызыватьИсключение = Ложь) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_ПолучитьПолитикуМФПДляОрганизации";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция РазрешенныеПокупателиДляОрганизацииПродавца(Организация, ДатаДействия) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_РазрешенныеПокупателиДляОрганизацииПродавца";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция СобственыеКонтрагенты(ДатаДействия) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_СобственыеКонтрагенты";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция СобственныеОрганизации(ДатаДействия) Экспорт
	...
	лКлючАлгоритма = "Справочник_ПолитикиМФП_МодульМенеджера_СобственныеОрганизации";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
