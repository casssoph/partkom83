...
Процедура ВыгрузитьДанныеПланаОбменаОбменПартКом83_Сайт(МассивОбъектов, ТаблицаСсылокНаОбъекты)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ТипСклада) Тогда 
...
Функция РабочийСкладСкладаВозвратов(СкладВозвратов) Экспорт
	
	РабочийСклад = Неопределено;
	...
	|	И Склады.СкладВозвратов = &СкладВозвратов
	|	И НЕ ПометкаУдаления";
	
	Запрос.УстановитьПараметр("СкладВозвратов", СкладВозвратов);
	...
	Если Выборка.Следующий() Тогда
		РабочийСклад = Выборка.Ссылка;
	КонецЕсли;
	
	Возврат РабочийСклад;
