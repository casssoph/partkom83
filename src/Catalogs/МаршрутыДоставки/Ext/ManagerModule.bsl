Функция ПолучитьРеквизитыКонтроля(МетаданныеОтбора) Экспорт
	...
	Если МетаданныеОтбора = Метаданные.ПланыОбмена.ОбменПартКом83_Сайт Тогда
		СтруктураПроверяемыхРеквизитов.Вставить("Шапка", "Наименование,Код,Родитель");
	КонецЕсли;
...
Функция ДанныеЗарегистрированныхОбъектов(ТаблицаСсылокНаОбъекты)
	...
	//Семенов И.П. 23.01.2019( в запрос добавил два поля f_sum, f_sum_other
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ЕСТЬNULL(Объекты.Ссылка.МинимальнаяСуммаОтгрузки, 0) КАК f_sum,
	                      |	ЕСТЬNULL(Объекты.Ссылка.МинимальнаяСуммаОтгрузкиФилиал, 0) КАК f_sum_other
	                      |ИЗ
	                      |	Объекты КАК Объекты
	                      |ГДЕ
