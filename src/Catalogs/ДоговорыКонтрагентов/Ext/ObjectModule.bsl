...
Процедура ПередЗаписью(Отказ)
	...
	//09.04.2019 Голубев Сергей исправлено
	//Если ДополнительныеСвойства.Свойство("ОтключитьМеханизмРегистрацииОбъектов") <> Неопределено Тогда
	Если ДополнительныеСвойства.Свойство("ОтключитьМеханизмРегистрацииОбъектов") Тогда
	//09.04.2019 Голубев Сергей
		ЗарегистрироватьВОбменеБИТ = Ложь;
	КонецЕсли;
	// 28.12.18 Строганов Роман <
		...
		Если НЕ РольДоступна("ПолныеПрава") Тогда
			...
			Иначе
				...
				Если  НЕ РольДоступна("ПолныеПрава") И 
					...
					ИЛИ (ВидУсловийДоговора <> Ссылка.ВидУсловийДоговора И Не РольДоступна("ОКРП"))  //Валиахметов А.А. http://jira.part-kom.ru/browse/XX-2099 17.04.2019 
...
Процедура ПриЗаписи(Отказ)
	...
	Если ЗарегистрироватьВОбменеБИТ 
	...
	Если ЗарегистрироватьВОбменеСайт И ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем 
		// 25.12.18 Строганов Роман > 
		И ДополнительныеСвойства.Свойство("ОтключитьМеханизмРегистрацииОбъектов") = Ложь Тогда
		// 25.12.18 Строганов Роман <
		ОбменДаннымиКлиентСервер.ЗарегистрироватьОбъект(ЭтотОбъект, Метаданные.ПланыОбмена.ОбменПартКом83_Сайт, "Справочник.ДоговорыКонтрагентов.МодульОбъекта.ПриЗаписи"); //Семенов И.П. 31.01.2019 XX-1768
	КонецЕсли;
...
Процедура ПередУдалением(Отказ)
	...
	Если РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Сайт", "Справочник: ДоговорыКонтрагентов", Ложь) Тогда
		ОбменДаннымиКлиентСервер.ЗарегистрироватьОбъект(ЭтотОбъект, Метаданные.ПланыОбмена.ОбменПартКом83_Сайт, "Справочник.ДоговорыКонтрагентов.МодульОбъекта.ПередУдалением"); //Семенов И.П. 31.01.2019 XX-1768
	КонецЕсли;
