Функция ПолучитьРеквизитыКонтроля(МетаданныеОтбора) Экспорт
	...
	Если МетаданныеОтбора = Метаданные.ПланыОбмена.ОбменПартКом83_Сайт Тогда
		СтруктураПроверяемыхРеквизитов.Вставить("Шапка", "Наименование,Код,Владелец,Дата,Номер,Организация,СрокДействия,ДопустимаяСуммаЗадолженности,КоэффициентСуммыКредита,ДоговорПриостановлен,ПометкаУдаления");
	КонецЕсли;
...
Функция ДанныеОбъектовПланаОбмена(ТаблицаСсылокНаОбъекты)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	СПодчиненнымиДоговорами.Ссылка.ДоговорПриостановлен
	                      |		ИЛИ СПодчиненнымиДоговорами.Ссылка.ПометкаУдаления
	                      |		ИЛИ СПодчиненнымиДоговорами.Ссылка.СлужебныйДоговор КАК Заблокирован,
