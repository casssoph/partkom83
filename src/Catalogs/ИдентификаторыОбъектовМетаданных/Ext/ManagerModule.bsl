...
Процедура ЗарегистрироватьПолноеИзменениеДляПодчиненныхУзловРИБ() Экспорт
	...
	Если УзлыРИБ.Количество() > 0 Тогда
		...
		ОбменДаннымиКлиентСервер.ЗарегистрироватьИзмененияВПланеОбмена(УзлыРИБ, МетаданныеСправочника);
		//)Семенов И.П.
	КонецЕсли;
...
Функция ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	//Если ДоступныИдентификаторыОбъектовРасширений Тогда
		...
		|ГДЕ
		|	ВерсииИдентификаторов.ПолноеИмяОбъекта = &ПолноеИмя";
...
Процедура ФормаСпискаПриСозданииНаСервере(Форма) Экспорт
	...
	Параметры.Свойство("ВыбиратьГруппыОбъектовМетаданных", Истина);
...
Если НЕ Свойства.ПометкаУдаления
	 ...
	 ИЛИ КлючОбъектаМетаданных <> Неопределено Тогда
		...
		// Установка новых свойств идентификатора объекта метаданных.
		//Свойства.ПометкаУдаления       = Ложь;//Истина;
		//Свойства.Родитель              = ПустаяСсылкаСправочника(ЭтоОбъектРасширений(Свойства.Ссылка));
		//Свойства.Наименование          = ?(Лев(Свойства.Наименование,1) = "?", Сред(Свойства.Наименование,3,СтрДлина(Свойства.Наименование)), Свойства.Наименование);//ВставитьЗнакВопроса(Свойства.Наименование);
		//Свойства.Имя                   = ?(Лев(Свойства.Имя,1) = "?", Сред(Свойства.Имя,3,СтрДлина(Свойства.Имя)),Свойства.Имя);//ВставитьЗнакВопроса(Свойства.Имя);
		//Свойства.Синоним               = ?(Лев(Свойства.Синоним,1) = "?", Сред(Свойства.Синоним,3,СтрДлина(Свойства.Синоним)),Свойства.Синоним);//ВставитьЗнакВопроса(Свойства.Синоним);
		//Свойства.ПолноеИмя             = ?(Лев(УникальноеПолноеИмя(Свойства),1) = "?",Сред(УникальноеПолноеИмя(Свойства),3,СтрДлина(УникальноеПолноеИмя(Свойства))),УникальноеПолноеИмя(Свойства));//УникальноеПолноеИмя(Свойства);
		//Свойства.ПолныйСиноним         = ?(Лев(Свойства.ПолныйСиноним,1) = "?", Сред(Свойства.ПолныйСиноним,3,СтрДлина(Свойства.ПолныйСиноним)),Свойства.ПолныйСиноним);//ВставитьЗнакВопроса(Свойства.ПолныйСиноним);
...
Функция СвойстваОбъектовМетаданных(ОбъектыРасширений, ИменаОбъектовРасширений, СвойстваКоллекций = Неопределено)
	...
	СвойстваОбъектовМетаданных.Колонки.Добавить("Имя",                       Новый ОписаниеТипов("Строка"));
