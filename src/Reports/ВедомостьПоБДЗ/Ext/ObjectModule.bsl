...
Процедура УстановитьТекстЗапроса(ЕстьПолеРегистратор = Истина)
	...
	|}
	|ИЗ
	|	РегистрНакопления.Взаиморасчеты.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор, , {ДоговорКонтрагента.* КАК ДоговорКонтрагента, ДоговорКонтрагента.Владелец.* КАК Контрагент, ДоговорКонтрагента.Организация КАК Организация}) КАК ВзаиморасчетыОстаткиИОбороты
...
Процедура СформироватьОтчет(ТабличныйДокумент) Экспорт
	...
	УниверсальныйОтчет.ДатаНач = ДобавитьМесяц(ДатаКон, -6);
	
    УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("ДатаКонца",УниверсальныйОтчет.ДатаКон);	// Описание исходного текста запроса.
    УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("ДатаНачала",УниверсальныйОтчет.ДатаНач);	// Описание исходного текста запроса.
