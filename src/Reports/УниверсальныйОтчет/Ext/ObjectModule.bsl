...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	лКлючАлгоритма = "Отчет_УниверсальныйОтчет_МодульОбъекта_СформироватьОтчет";
	...
	Если ВыводВозможен() Тогда
		...
		ДоработатьМакетПередВыводом(Отчет, ПостроительОтчета); //ХудинВВ XX-757
...
Процедура ДоработатьМакетПередВыводом(Отчет, ПостроительОтчета); //ХудинВВ XX-757

	Если Отчет = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Отчет.Метаданные().Имя = "ВедомостьДенежныеСредства" Тогда
		Отчет.ДоработатьМакетПередВыводом(ПостроительОтчета);
	КонецЕсли;
