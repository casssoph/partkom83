...
Процедура ОбходПоДокументам(СтруктураОтчета, Выборка, ОстатокПоДоговору)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаДокумент.Параметры, Выборка, "Документ,Дебет,Кредит");
		СтрокаДокумент.Параметры.ДокументПредставление = ПредставлениеДокумента(Выборка.Документ, Выборка.ДокументПредставление);
