...
&НаКлиенте
Процедура ОчиститьДеревоДанных()
	ЭлементыДерева = ДеревоДанных.ПолучитьЭлементы();
	если ЭлементыДерева.Количество() тогда 
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьДеревоДанныхЗавершение", ЭтаФорма,ЭлементыДерева),"Табличная часть будет очищена, продолжить?",РежимДиалогаВопрос.ДаНет);	
	КонецЕсли;	 
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьДеревоДанныхЗавершение(РезультатВопроса, ЭлементыДерева) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да тогда 
		ЭлементыДерева.Очистить(); // ЛНА
	КонецЕсли;
КонецПроцедуры	
...
&НаКлиенте
Процедура ДеревоДанныхДокументПриИзменении(Элемент)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ДеревоДанныхДокументОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	СтандартнаяОбработка = ложь;	
	ЗаполнитьСтрокиЗаявокПриВыборе(ВыбранноеЗначение)//,Элемент.Родитель.ТекущиеДанные);
	
КонецПроцедуры


&НаСервере
Функция ЗаполнитьСтрокиЗаявокПриВыборе(Документ)//,СтрокаДерева)
	
	
	ИмяПоля = ИмяТекущегоПоляДокумента();	
	ДеревоЗаполнения = РеквизитФормыВЗначение("ДеревоДанных");	
	Запрос = Новый Запрос();
	ТекстЗапроса = "ВЫБРАТЬ
	|	Остатки.СтрокаЗаявки,
	|	Остатки.КоличествоОстаток КАК %3
	|ИЗ
	|	РегистрНакопления.%1.Остатки(, СтрокаЗаявки.%2 = &Документ) КАК Остатки";
	запрос.Текст = СтрШаблон(ТекстЗапроса,?(ИмяПоля = "Заказ","ЗаказыПоставщикам","ЗаявкиПокупателей"),
	ИмяПоля,?(ИмяПоля = "Заказ","ОстатокПоЗаказу","ОстатокПоЗаявке"));
	Запрос.УстановитьПараметр("Документ",Документ);
	Выборка = запрос.Выполнить().Выбрать();
	
	СтрокаЗаказа = ДеревоЗаполнения.Строки.Добавить();
	СтрокаЗаказа[ИмяПоля] = Документ;
	Пока Выборка.Следующий() цикл 
		НовСтрока = СтрокаЗаказа.строки.Добавить();	
		НовСтрока[ИмяПоля]  = Документ;
		ЗаполнитьЗначенияСвойств(НовСтрока,Выборка);
	КонецЦикла;	
	ОчиститьПустыеСтроки(ДеревоЗаполнения,ИмяТекущегоПоляДокумента(),Документы.ЗаказПоставщику.ПустаяСсылка());	
	ЗначениеВРеквизитФормы(ДеревоЗаполнения,"ДеревоДанных");
...
Процедура ОчиститьПустыеСтроки(ДеревоЗначений,ИмяДокумента,ПустоеЗначение)
	ПустСтроки =  ДеревоЗначений.строки.НайтиСтроки(Новый  Структура(ИмяДокумента,ПустоеЗначение));
	Для Каждого ПустаяСтрока из ПустСтроки цикл 
		ДеревоЗначений.строки.Удалить(ПустаяСтрока);		
	КонецЦикла;	
	
КонецПроцедуры	

&НаКлиенте
Процедура ДеревоДанныхЗаявкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	СтандартнаяОбработка = ложь;	
	ЗаполнитьСтрокиЗаявокПриВыборе(ВыбранноеЗначение);
