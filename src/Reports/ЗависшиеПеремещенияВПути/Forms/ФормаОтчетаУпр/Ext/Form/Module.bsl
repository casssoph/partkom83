...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ВыпДействие) = Тип("Число") Тогда 
		...
		Форм2 = ПолучитьФорму("Отчет.ЗависшиеПеремещенияВПутиСДокументомДвижения.Форма", , , РасшифрЗначение.Номенклатура); //Форма Расшифровки.
		...
		СтрВозврата = ОбработатьРасшифровкуНаСервере(Форм2.Отчет, "ОтчетОбъект.ЗависшиеПеремещенияВПутиСДокументомДвижения", Форм2.Результат, Форм2.НавигационнаяСсылка, Форм2.УникальныйИдентификатор);
...
Функция ПолучитьРасшифровкуНаСервере(Расшифровка, МассивИменРасшифровки, ЭтоГруппа = Ложь)
	...
	//
	//
	//ПолеНоменкл 	= Поля.Найти("Номенклатура");
	//ПолеДокПрихода  = Поля.Найти("ДокументПрихода");
	//
	//Если ПолеДокПрихода = Неопределено Тогда //нажали на номенклатуру
	//	
	//	
	//	Родители = Данные.Элементы[Расшифровка].ПолучитьРодителей();
	//	Если Родители.Количество() Тогда
	//		Поля = Данные.Элементы.Получить(Родители[0].Идентификатор-1).ПолучитьПоля();
	//		ПолеДокПрихода  = Поля.Найти("ДокументПрихода");
	//	КонецЕсли;
	//	
	//КонецЕсли;
	
	//ВозвращаемоеЗначение = Новый Структура("Номенклатура, ДокументПрихода", 
	//?(ПолеНоменкл = Неопределено, Неопределено, ПолеНоменкл.Значение), 
	//?(ПолеДокПрихода = Неопределено, Неопределено, ПолеДокПрихода.Значение));
	//
	
	//ПолеНоменкл 	= Неопределено;
	//ПолеДокПрихода 	= Неопределено;
	//Для Каждого ИмяРасшифровки Из МассивИменРасшифровки Цикл
	//	Если ПолеНоменкл = Неопределено и ИмяРасшифровки = "Номенклатура" Тогда
	//		ПолеНоменкл = Поля.Найти(ИмяРасшифровки);
	//	КонецЕсли;
	//	Если ПолеДокПрихода = Неопределено и ИмяРасшифровки  = "ДокументПрихода"  Тогда
	//		ПолеДокПрихода = Поля.Найти(ИмяРасшифровки);
	//	КонецЕсли;
	//КонецЦикла;
