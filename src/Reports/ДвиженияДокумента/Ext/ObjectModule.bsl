...
Процедура ВывестиДвиженияДокумента(тДокумент, тРезультат)
	ДвиженияДокумента = тДокумент.Метаданные().Движения;
	...
	ТаблицаДвижений = ПолныеПрава.ОпределитьНаличиеДвиженийПоРегистратору(тДокумент);
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		Если (ВидРегистра = "Сведений") 
			...
			Если ВидРегистра+"."+СвойстваОбъекта.Имя = "Сведений.ДокументыКорректировок"
			...
			ОбработатьВыводДанныхПоМассиву(МассивПолей, СтруктураПредставлениеПолей, Построитель, 
				"Регистр" + ВидРегистра + "." + СвойстваОбъекта.Имя, ВыводитьВВидеТаблицы, тДокумент);
		...
		ИначеЕсли ВидРегистра = "Бухгалтерии" Тогда
			...
			Построитель.Параметры.Вставить("ДокументОтчета", тДокумент);
		...
		Если тДокумент = Документ Тогда
			ОбластьЗаголовок.Параметры.ИмяРегистра = СвойстваОбъекта.Синоним;
		Иначе
			ОбластьЗаголовок.Параметры.ИмяРегистра = СвойстваОбъекта.Синоним + " (" + Строка(тДокумент) + ")";
		КонецЕсли;
		ОбластьЗаголовок.Параметры.ВидРегистра = НРег(ВидРегистра);
		тРезультат.Вывести(ОбластьЗаголовок);
		
		тРезультат.НачатьГруппуСтрок(,Ложь);
		
		Если ВыводитьРасходПриход Тогда
			НомерВыводимойСтроки = тРезультат.ВысотаТаблицы;	
		КонецЕсли;
		
		// вывод данных в табличный документ
		Построитель.Вывести(тРезультат);
		
		// раскрашиваем приход расход в разные цвета
		Если ВыводитьРасходПриход Тогда
			НомерТекущейСтроки = тРезультат.ВысотаТаблицы;
			
			Для Номерстроки = НомерВыводимойСтроки + 2 По НомерТекущейСтроки - 1 Цикл
				
				// вид движения находится во второй колонке всегда
				ОбластьВывода = тРезультат.Область(Номерстроки, 2);
		...
		тРезультат.ЗакончитьГруппуСтрок();
		
	КонецЦикла;
	
КонецПроцедуры

Процедура СформироватьОтчетДляЗаказа(ДокументРезультат)
	ВывестиДвиженияДокумента(Документ, ДокументРезультат);
...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		СформироватьОтчетДляЗаказа(ДокументРезультат);
...
Процедура ОбработатьВыводДанныхПоМассиву(МассивПолей, СтруктураПредставлениеПолей,
			Построитель, Знач ИмяРегистра, Знач ВыводитьВВидеТаблицы = Ложь, тДокумент)
