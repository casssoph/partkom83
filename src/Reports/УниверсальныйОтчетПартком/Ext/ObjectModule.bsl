...
Перем мРедактироватьОбщиеСохраненныеНастройки;
...
Процедура ДобавитьДополнительныеПоля()
	
	//Доп поля Регистратора -ВидОперации
	СтруктураДопПолей = новый Структура;
	СтруктураДопПолей.Вставить("ВидОперации","Регистратор");
	СтруктураДопПолей.Вставить("ВидОперацииПоступления","Регистратор");
	
	Для Каждого ЭлементДопПоля из СтруктураДопПолей цикл
		ОбработатьДобавитьДополнительноеПоле(ЭлементДопПоля.Ключ,ЭлементДопПоля.Значение)	
	КонецЦикла;	
	
КонецПроцедуры	

Процедура ОбработатьДобавитьДополнительноеПоле(ИмяПоля,Владелец)
	
	ДопПоле = ПостроительОтчета.ВыбранныеПоля.Найти(Владелец+ИмяПоля);

	Если ДопПоле = Неопределено и Не ПостроительОтчета.ИзмеренияСтроки.Найти(Владелец) = Неопределено тогда 
		ДобавитьДополнительноеПоле(Владелец+"."+ИмяПоля);
	иначе 
		если Не ДопПоле = Неопределено тогда 
				ПостроительОтчета.ВыбранныеПоля.Удалить(ДопПоле);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ПереустановитьНачальныеНастройки(Объект, Отчет, Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ВариантыОтчетовКлиент.УстановитьЗаголовокОтчета(Отчет, Форма);
...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра) Экспорт 
	...
	Для каждого Ресурс Из МетаданныеРегистра.Ресурсы Цикл
		Если Не Ресурс.Имя="СуммаДоллары" И Не Ресурс.Имя="СуммаЕвро" И Не Ресурс.Имя="СуммаБезНДС" И Не Ресурс.Имя="СуммаУпр" Тогда 
		ОписаниеПолейВыборки = ОписаниеПолейВыборки + ?(ОписаниеПолейВыборки = "", "", ",") + "
	...
	//|   ЕСТЬNULL(ИсточникДанных.Регистратор.ВидОперации, """") КАК ВидОперации,
	//PK83-644 
	...
	КонецЕсли;
		...
		Если Не ПостроительОтчета.ИзмеренияСтроки.Найти("Регистратор") = Неопределено тогда 
		ДобавитьДополнительноеПоле("Регистратор.ВидОперации");
		КонецЕсли;
...
//лСписокПрав = УправлениеПользователями.ПолучитьЗначениеПраваДляТекущегоПользователя(ПланыВидовХарактеристик.ПраваПользователей.РазрешитьРедактированиеМенеджеровСнабжения, Ложь);
//мРедактироватьОбщиеСохраненныеНастройки = (лСписокПрав.НайтиПоЗначению(Истина) <> Неопределено);
