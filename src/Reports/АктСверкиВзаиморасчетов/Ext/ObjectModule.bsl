...
Процедура ПроверкаЗаполненияПараметров(СтруктураОтчета)
	...
	//Либо заполнен договор//
	Если ЗначениеЗаполнено(СтруктураОтчета.Договор) Тогда
...
Функция ДанныеДляОтчета(СтруктураОтчета)
	...
	Если ЗначениеЗаполнено(СтруктураОтчета.Договор) Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |ИЗ
		                      |	РегистрНакопления.ДепозитыКонтрагентов.Остатки(&НачалоПериода, ДоговорКонтрагента = &Договор) КАК ДепозитыКонтрагентовОстатки
		                      |
		                      |ОБЪЕДИНИТЬ ВСЕ
		                      |
		                      |ВЫБРАТЬ
		                      ...
		                      |ИЗ
		                      |	РегистрНакопления.Взаиморасчеты.Остатки(&НачалоПериода, ДоговорКонтрагента = &Договор) КАК ВзаиморасчетыОстатки
		                      |;
		                      |
		                      |////////////////////////////////////////////////////////////////////////////////
		                      |ВЫБРАТЬ
		                      |	ВзаиморасчетыОбороты.Период КАК Дата,
		                      |	ТИПЗНАЧЕНИЯ(ВзаиморасчетыОбороты.Регистратор) КАК ВидДокумента,
		                      ...
		                      |ПОМЕСТИТЬ Обороты
		                      |ИЗ
		                      |	РегистрНакопления.Взаиморасчеты.Обороты(&НачалоПериода, &КонецПериода, Регистратор, ДоговорКонтрагента = &Договор) КАК ВзаиморасчетыОбороты
		                      |
		                      |ОБЪЕДИНИТЬ ВСЕ
		                      |
		                      |ВЫБРАТЬ
		                      ...
		                      |ИЗ
		                      |	РегистрНакопления.ДепозитыКонтрагентов.Обороты(&НачалоПериода, &КонецПериода, Регистратор, ДоговорКонтрагента = &Договор) КАК ДепозитыКонтрагентовОбороты
		                      |;
		                      |
		                      |////////////////////////////////////////////////////////////////////////////////
		                      |ВЫБРАТЬ
...
Процедура УстановитьРеквизитыОтчета(СтруктураОтчета)
	...
	СтруктураОтчета.Вставить("КонтрагентТекст", СтруктураОтчета.Контрагент.НаименованиеПолное);
...
Процедура УстановитьКонтактнуюИнформацию(СтруктураОтчета)
	...
	СтруктураОтчета.Вставить("EMail", ?(ЕстьДанные, Выборка.EMail, "-"));
