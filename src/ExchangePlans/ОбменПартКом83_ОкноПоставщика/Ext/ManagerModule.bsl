	...
	Попытка
		...
		Для Каждого ЭлементСоответствия Из ВыгружаемыеОбъекты цикл
			...
			Если РегистрироватьВыгрузкуОбъектов Тогда
				ОтметитьОтправкуОбъектов(ЭлементСоответствия.Значение, ЗаписьСообщения.НомерСообщения);
			КонецЕсли;
		КонецЦикла;
...
КонецФункции
Процедура ОтметитьОтправкуОбъектов(ТаблицаОбъектов, НомерПакета)
	...
	Если ТаблицаОбъектов.Количество() > 0 И ТаблицаОбъектов.Колонки.Найти("Ссылка") <> Неопределено Тогда
		...
		НаборЗаписей.Отбор.ВидОбмена.Установить(Перечисления.ВидыОбменов.ОбменОП_1С);
		НаборЗаписей.Отбор.НомерПакета.Установить(НомерПакета);
		Для Каждого Строка Из ТаблицаОбъектов Цикл
			Запись = НаборЗаписей.Добавить();
			Запись.Период = ТекущаяДата();
			Запись.ВидОбмена = Перечисления.ВидыОбменов.ОбменОП_1С;
			Запись.НомерПакета = НомерПакета;
			Запись.Ссылка = Строка.Ссылка;
		КонецЦикла;
