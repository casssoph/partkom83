...
Функция СтруктураДокументаЗаявкаПокупателя(ОбъектXDTO, НомерСообщения, СтруктураОтчета = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектXDTO) = Тип("Структура") Тогда
		...
		Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) Тогда
			СтруктураОбъекта.Вставить("ФлагСамовывоз",		?(ОбъектXDTO.Свойство("ФлагСамовывоз")		,ОбъектXDTO.ФлагСамовывоз, Ложь));
			СтруктураОбъекта.Вставить("СамовывозИмя", 		?(ОбъектXDTO.Свойство("СамовывозИмя")		, ОбъектXDTO.СамовывозИмя, ""));
			СтруктураОбъекта.Вставить("СамовывозТелефон",	?(ОбъектXDTO.Свойство("СамовывозТелефон")	, ОбъектXDTO.СамовывозТелефон, ""));
			СтруктураОбъекта.Вставить("СамовывозСклад", 	?(ОбъектXDTO.Свойство("СамовывозСклад")		, Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.СамовывозСклад)), Справочники.Склады.ПустаяСсылка()));
			СтруктураОбъекта.Вставить("ТипДоставки", 		?(СтруктураОбъекта.ФлагСамовывоз, Справочники.ТипыДоставки.СамовывозНовый, Справочники.ТипыДоставки.ПустаяСсылка()));
		КонецЕсли;
		// 25.07.19 Строганов Роман < XX-2633 Загрузка заявки (Самовывоз)
	...
	Иначе
		// 25.07.19 Строганов Роман > XX-2633 Загрузка заявки (Самовывоз) 
		Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) Тогда
			СтруктураОбъекта.Вставить("ФлагСамовывоз",		?(ОбъектXDTO.Свойства().Получить("ФлагСамовывоз") 	<> Неопределено	, ОбъектXDTO.ФлагСамовывоз, Ложь));
			СтруктураОбъекта.Вставить("СамовывозИмя", 		?(ОбъектXDTO.Свойства().Получить("СамовывозИмя") 	<> Неопределено	, ОбъектXDTO.СамовывозИмя, ""));
			СтруктураОбъекта.Вставить("СамовывозТелефон",	?(ОбъектXDTO.Свойства().Получить("СамовывозТелефон")<> Неопределено	, ОбъектXDTO.СамовывозТелефон, ""));
			СтруктураОбъекта.Вставить("СамовывозСклад", 	?(ОбъектXDTO.Свойства().Получить("СамовывозСклад") 	<> Неопределено	, Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.СамовывозСклад)), Справочники.Склады.ПустаяСсылка()));
			СтруктураОбъекта.Вставить("ТипДоставки", 		?(СтруктураОбъекта.ФлагСамовывоз, Справочники.ТипыДоставки.СамовывозНовый, Справочники.ТипыДоставки.ПустаяСсылка()));
		КонецЕсли;
		// 25.07.19 Строганов Роман < XX-2633 Загрузка заявки (Самовывоз)
