...
Функция ОбработатьОбъектЗаявкиССайта(ДанныеОбъекта, НомерСообщения, Экспресс = Ложь, СтруктураОтчета) Экспорт
	...
	СтруктураДокумента = СтруктураДокументаЗаявкаПокупателя(СтруктураДанныхСайта, НомерСообщения, СтруктураОтчета);
	КонтрольЗаполненияДокумента(СтруктураДокумента);
...
//Загрузка заявок
Функция СтруктураДокументаЗаявкаПокупателя(ОбъектXDTO, НомерСообщения, СтруктураОтчета = Неопределено) Экспорт
	...
	УстановитьРеквизитыКонтрагента(СтруктураОбъекта, Строка(ОбъектXDTO.ДоговорКонтрагента), СтруктураОтчета);
	...
	Если ТипЗнч(ОбъектXDTO) = Тип("Структура") Тогда
		...
		Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) Тогда
			СтруктураОбъекта.Вставить("ФлагСамовывоз",		?(ОбъектXDTO.Свойство("ФлагСамовывоз")		И ОбъектXDTO.ФлагСамовывоз 		<> Неопределено	, ОбъектXDTO.ФлагСамовывоз		, Ложь));
			СтруктураОбъекта.Вставить("СамовывозИмя", 		?(ОбъектXDTO.Свойство("СамовывозИмя") 		И ОбъектXDTO.СамовывозИмя 		<> Неопределено	, ОбъектXDTO.СамовывозИмя		, ""));
			СтруктураОбъекта.Вставить("СамовывозТелефон",	?(ОбъектXDTO.Свойство("СамовывозТелефон") 	И ОбъектXDTO.СамовывозТелефон 	<> Неопределено	, ОбъектXDTO.СамовывозТелефон	, ""));
			СтруктураОбъекта.Вставить("СамовывозСклад", 	?(ОбъектXDTO.Свойство("СамовывозСклад")		И ОбъектXDTO.СамовывозСклад 	<> Неопределено	, Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.СамовывозСклад)), Справочники.Склады.ПустаяСсылка()));
			СтруктураОбъекта.Вставить("ТипДоставки", 		?(СтруктураОбъекта.ФлагСамовывоз, Справочники.ТипыДоставки.СамовывозСТочкиВыдачи, Справочники.ТипыДоставки.ПустаяСсылка()));
		КонецЕсли;
		// 25.07.19 Строганов Роман < XX-2633 Загрузка заявки (Самовывоз)
	...
	Иначе
		...
		// 25.07.19 Строганов Роман > XX-2633 Загрузка заявки (Самовывоз) 
		Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) Тогда
			СтруктураОбъекта.Вставить("ФлагСамовывоз",		?(ОбъектXDTO.Свойства().Получить("ФлагСамовывоз") 	<> Неопределено И ОбъектXDTO.ФлагСамовывоз 		<> Неопределено	, ОбъектXDTO.ФлагСамовывоз		, Ложь));
			СтруктураОбъекта.Вставить("СамовывозИмя", 		?(ОбъектXDTO.Свойства().Получить("СамовывозИмя") 	<> Неопределено	И ОбъектXDTO.СамовывозИмя 		<> Неопределено	, ОбъектXDTO.СамовывозИмя		, ""));
			СтруктураОбъекта.Вставить("СамовывозТелефон",	?(ОбъектXDTO.Свойства().Получить("СамовывозТелефон")<> Неопределено	И ОбъектXDTO.СамовывозТелефон	<> Неопределено	, ОбъектXDTO.СамовывозТелефон	, ""));
			СтруктураОбъекта.Вставить("СамовывозСклад", 	?(ОбъектXDTO.Свойства().Получить("СамовывозСклад") 	<> Неопределено И ОбъектXDTO.СамовывозСклад 	<> Неопределено	, Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.СамовывозСклад)), Справочники.Склады.ПустаяСсылка()));
			СтруктураОбъекта.Вставить("ТипДоставки", 		?(СтруктураОбъекта.ФлагСамовывоз, Справочники.ТипыДоставки.СамовывозСТочкиВыдачи, Справочники.ТипыДоставки.ПустаяСсылка()));
		КонецЕсли;
		// 25.07.19 Строганов Роман < XX-2633 Загрузка заявки (Самовывоз)
	...
	Если НЕ СтруктураДокумента.Отказ Тогда
		...
		Иначе
			...
			Попытка
				...
				// 25.07.19 Строганов Роман > XX-2633 Загрузка заявки (Самовывоз) 
				Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) И СтруктураДокумента.ФлагСамовывоз Тогда
...
Процедура УстановитьРеквизитыКонтрагента(СтруктураОбъекта, UUIDДоговора, СтруктураОтчета = Неопределено)
	...
	ДанныеДоговора = ДанныеДоговора(СтруктураОбъекта, UUIDДоговора, СтруктураОтчета);
...
Функция ПолучитьДоговорКонтрагентаЗаявки(СтруктураОбъекта, UUID, СтруктураОтчета = Неопределено)
...
Функция ДанныеДоговора(СтруктураОбъекта, UUID, СтруктураОтчета = Неопределено)
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2851)  Тогда
		//ХудинВВ XX-2851 23072019 
		Договор = ПолучитьДоговорКонтрагентаЗаявки(СтруктураОбъекта, UUID, СтруктураОтчета);
	Иначе
...
Процедура УстановитьВидОперации(СтруктураОбъекта)
	...
	ИначеЕсли СтруктураОбъекта.Склад = Константы.СкладБонусовВиртуальный.Получить() тогда //ХудинВВ XX-2879 05082019
...
Процедура УстановитьСкладДокумента(СтруктураОбъекта, ЦентральныйСклад)
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2633) Тогда
		Если СтруктураОбъекта.ТипДоставки = ПредопределенноеЗначение("Справочник.ТипыДоставки.СамовывозСТочкиВыдачи") И ЗначениеЗаполнено(СтруктураОбъекта.СамовывозСклад) Тогда
