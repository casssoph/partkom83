...
Процедура ОбработатьЗапросОтчетаVMI(ОбрабатываемыйОбъект, УдалятьОбработанныеЗаписи)
	...
	Если НЕ Ошибка Тогда
		Попытка
			//Отчет = Отчеты.ВМИДляПоставщика.Создать(); //Старый//
			//РезультатОтчета = Отчет.ОстаткиДляСайта(Поставщик, Склад, НачалоПериода, КонецДня(КонецПериода));
			Отчет = Отчеты.ОтчетVMI_ОкноПоставщика.Создать();
			РезультатОтчета = Отчет.СформироватьОтчет(Поставщик, Склад, НачалоПериода, КонецДня(КонецПериода));
			Ошибка = РезультатОтчета.Ошибка;
			ТекстОшибки = РезультатОтчета.ОписаниеОшибк;
			ТабличныйДокумент = РезультатОтчета.Результат;
