...
Процедура УстановитьПараметрыОтчетаАктСверки(Отчет, Параметры, СтруктураОтчета)
	...
	Попытка
		...
		Иначе
			//Пока договора кривые, выводим для списка//
			СтруктураОтчета.Договор = ДоговораПокупателя(СтруктураОтчета.Организация, СтруктураОтчета.Контрагент);
		КонецЕсли;
	Исключение
...
Функция ВыгрузитьСообщениеОбмена(ИдентификаторУзлаОбмена, ReceivedNo) Экспорт
	...
	Если ReceivedNo > Исходящий.НомерОтправленного Тогда
		...
		ТекстОшибки = "Расхождение счетчиков, ReceivedNo:" + ReceivedNo + ", НомерОтправленного:" + Исходящий.НомерОтправленного;
		ЗаписьЖурналаРегистрации("Обмен данными.Выгрузка.ОбменПартКом83_Сайт_Файлы", УровеньЖурналаРегистрации.Ошибка,,,ТекстОшибки);
		НомерСообщения = ReceivedNo + 1;
		ReceivedNo = Исходящий.НомерОтправленного;
	Иначе
	...
	Если НЕ ЗначениеЗаполнено(Входящий) Тогда
		ВызватьИсключение "[ВыгрузитьСообщениеОбмена]: неправильный параметр номер 1.";	
	КонецЕсли;
	НомерПринятого = Входящий.НомерПринятого;
...
КонецФункции
Функция ДоговораПокупателя(Организация, Контрагент)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	И НЕ ДоговорыКонтрагентов.ДоговорИнвестКонтракт");
