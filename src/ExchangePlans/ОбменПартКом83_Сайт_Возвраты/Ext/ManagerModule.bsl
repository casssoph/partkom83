...
Функция ТипПоТипуСообщения(ТипСообщения) Экспорт
	...
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_Возвраты_МодульМенеджера_ТипПоТипуСообщения";
...
//Получает сообщение, помещает объекты в регистр
Процедура ЗагрузитьСообщениеОбмена() Экспорт
	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_Возвраты_МодульМенеджера_ВыгрузитьСообщениеОбмена_ЗагрузитьСообщениеОбмена";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);
...
//Обрабатывает объекты из регистра
Процедура ОбработатьОбъекты() Экспорт
	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_Возвраты_МодульМенеджера_ВыгрузитьСообщениеОбмена_ОбработатьОбъекты";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
//Для теста
Процедура ТестоваяЗагрузка(ТекстСообщения) Экспорт
	
	ИмяПланаОбмена = "ОбменПартКом83_Сайт_Возвраты";
	...
	ИмяФ = Каталог+"ТестЗагрузкиАРВ.xml";
	
	Текст = Новый ЗаписьТекста(ИмяФ, КодировкаТекста.UTF8);
	Текст.Записать(ТекстСообщения);
	Текст.Закрыть();
	
	лЧтениеХМЛ = Новый ЧтениеXML;
	лЧтениеХМЛ.ОткрытьФайл(ИмяФ);
	
	лДанныеХДТО = ФабрикаXDTO.ПрочитатьXML(лЧтениеХМЛ, лТипСообщениеОбмена);
	лИмяПланаОбмена = лДанныеХДТО.ПланОбмена;
	
	РаспакованноеСообщение = ТекстСообщения;
	
	НомерСообщения = лДанныеХДТО.НомерСообщения;
	ДатаСообщения = ТекущаяДатаСеанса();
	
	лИсходящий = ПланыОбмена.ОбменПартКом83_Сайт_Возвраты.НайтиПоКоду("3");
	СтруктураУзловОбмена = ОбменССайтомСервер.ПолучитьСтрктуруУзловОбмена(лИсходящий, ИмяПланаОбмена);	
	...
	Если ОбъектыXDTO.Количество() > 0 Тогда
		ОбменДаннымиВызовСервера.ЗарегистрироватьСообщениеВИсторииОбменаССайтом(лИсходящий.ИдентификаторУзла, СтруктураУзловОбмена.КодПолучателя,
		"ответвебсервиса", НомерСообщения);
		ОбменССайтомСервер.РазобратьПолученныеОбъекты(ОбъектыXDTO, НомерСообщения,ИмяПланаОбмена,,ДатаСообщения);
	КонецЕсли;
