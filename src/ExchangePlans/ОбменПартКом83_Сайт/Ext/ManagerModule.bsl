...
Процедура ЗаполнитьРеквизитыДокументаРеализации(Документ, СтруктураДокумента)
	...
	Документ.УчитыватьНДС = Документ.Организация.УчитыватьНДС;
...
Процедура ЗагрузитьМаршрутноеЗадание(Реализация,СтруктураОтчета)
		...
		МЗ.ТипЗадания=Справочники.МегаЛогист_ТипыМаршрутныхЗаданий.ЭкспрессДоставка;
...
Функция ЗагружаемыеОбъектыРеализации(СтруктураОбъекта)
	...
	Для Каждого Группировка Из ВыборкаПоГруппировкам Цикл
		// + Пушкин 20181114 XX-1345 
		НоваяСтруктураОбъекта = Новый Структура("RequestID,Контрагент,Дата,ДатаДоставки,ВремяДоставкиС,ВремяДоставкиПо,Телефон,АдресДоставки,ТорговаяТочка,Менеджер");
