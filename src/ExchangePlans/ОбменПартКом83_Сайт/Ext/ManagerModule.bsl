	...
	Если МассивРТУ_Количество > 0 Тогда
		...
		Иначе
			...
			Для каждого лДок из МассивРТУ цикл
				...
				Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
					Если СтрНайти(Врег(ОписаниеОшибки),Врег(ОписаниеПроблемыБлокировок)) = 0 тогда
						...
						ПересечениеСОсновнойДоставкой = ЗапроситьИсториюЗаявокПокупателя(лДок);
						Если ПересечениеСОсновнойДоставкой.Количество() > 0 Тогда
							...
							ОформитьОтказыПоЗаявкам(лКартинкаПоЗаявкам,ПересечениеСОсновнойДоставкой.ВыгрузитьКолонку("СтрокаЗаявки"));
			...
			Если сч_блокировки > 0 Тогда
				Пауза();	
			Иначе
				Если сч_резервы = 0 ИЛИ сч_хз = 0 Тогда
					СтруктураОтчета.КоличествоПопытокЗалечить = КоличествоПопытокЗалечитьМаксимум;	
				КонецЕсли;
			КонецЕсли;
...
Процедура ОформитьОтказыПоЗаявкам(парам_ТЧ, парам_СтрокиЗаявок = Неопределено)
	...
	Для каждого лСтрТЧ из парам_ТЧ Цикл
		...
		Для каждого лстр ИЗ оРТУ.Товары цикл
			...
			Если НЕ парам_СтрокиЗаявок = Неопределено Тогда
				НайденнаяСтрока = парам_СтрокиЗаявок.Найти(лстр.СтрокаЗаявки);
			...
			лотказ.Количество = лстр.КоличествоПлан;
	...
	Запрос.Текст = 
		...
		|	РеализацияТоваровУслугПричиныОтказов.Ссылка.Контрагент КАК Контрагент,
		|	РеализацияТоваровУслугПричиныОтказов.Ссылка.Телефон КАК Телефон,
		|	РеализацияТоваровУслугПричиныОтказов.Ссылка.Заказчик КАК Заказчик,
		...
		|	РеализацияТоваровУслугПричиныОтказов.Количество КАК Количество
		|ИЗ
		...
		|ГДЕ
		|	РеализацияТоваровУслугПричиныОтказов.Ссылка В(&Ссылки)";
	 Запрос.УстановитьПараметр("Ссылки", парам_ТЧ.ВыгрузитьКолонку("Ссылка"));
