...
КонецПроцедуры
Процедура ОбработкаЗаявокЭД(МассивРТУ,СтруктураОтчета,МестоВозникновения)
	...
	Иначе
		...
		ОписаниеПроблемыБлокировок = "Конфликт блокировок при выполнении транзакции";
		сч_блокировки = 0;
		сч_резервы = 0;
		сч_хз = 0;
			...
			Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
				Если СтрНайти(Врег(ОписаниеОшибки),Врег(ОписаниеПроблемыБлокировок)) = 0 тогда
					...
					Если ПересечениеСОсновнойДоставкой.Количество() > 0 Тогда
						...
						ОформитьОтказыПоЗаявкам(лКартинкаПоЗаявкам.Скопировать(Новый Структура("Проведен",Ложь)),ПересечениеСОсновнойДоставкой.ВыгрузитьКолонку("СтрокаЗаявки"));
...
Функция ЗапроситьИсториюЗаявокПокупателя(лДок)
	...
	|	СУММА(ИсторияЗаявокПокупателяСрезПоследних.Количество) КАК Количество
	|ПОМЕСТИТЬ История
	|ИЗ
	...
	|				И НЕ Регистратор = &Ссылка) КАК ИсторияЗаявокПокупателяСрезПоследних
	|
	|СГРУППИРОВАТЬ ПО
	|	ИсторияЗаявокПокупателяСрезПоследних.СтрокаЗаявки
	|;
	...
	|	РТУ.КоличествоПлан,
	|	РТУ.Количество,
	|	естьNULL(История.Количество,0) КАК КоличествоУЖЕ,
	|	РТУ.Количество - естьNULL(История.Количество,0) КАК Разница
	|ИЗ
	|	РТУ КАК РТУ
	|		ЛЕВОЕ СОЕДИНЕНИЕ История КАК История
	|		ПО РТУ.СтрокаЗаявки = История.СтрокаЗаявки
	|ГДЕ		
	|	естьNULL(История.Количество,0) > 0";
