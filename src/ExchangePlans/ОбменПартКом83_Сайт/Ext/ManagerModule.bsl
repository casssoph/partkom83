...
Функция ВыбратьИзмененияЭкспрессДоставки(НомерПринятогоПакета, пар_1с8_web = Ложь)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
						  ...
						  |	" + ?(пар_1с8_web,"","НЕ") + " ЭкспрессДоставкаОбмен.express_delivery_request_id В (&ВидыКодманд1с)
...
Процедура ExpressDelivery1cCancel(пар_МассивРТУ) Экспорт
	...
	// вызывается из SMSоповещение.СформироватьSMS_ОтказПолный
...
Функция ПолучитьДанныеПоЭД(пар_МассивРТУ)
	...
	|	И РеализацияТоваровУслуг.Контрагент.Регион = &Регион
	...
	Если РегионОтбора = Справочники.Регионы.ПустаяСсылка() тогда
		Запрос_Текст = СтрЗаменить(Запрос_Текст,"И РеализацияТоваровУслуг.Контрагент.Регион = &Регион","");
	КонецЕсли;
	...
	РТУ = Результат_Количество[Результат_Количество - 1].Выгрузить();
	ЭДВ = Результат_Количество[Результат_Количество - 2].Выгрузить();
