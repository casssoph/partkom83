...
Процедура ОбработатьСообщениеССайта() Экспорт
	...
	Исключение
		...
		//Семенов И.П. 07.02.2019 XX-1768(
		ОбменДаннымиКлиентСервер.ЗафиксироватьСообщениеВИсторииОбмена(ДанныеХДТО.НомерСообщения, Исходящий, ТекстСообщения,,Истина,ОписаниеОшибки());
		//)Семенов И.П.
		ВызватьИсключение ;
	КонецПопытки;
	
	//Семенов И.П. 07.02.2019 XX-1768(
	ОбменДаннымиКлиентСервер.ЗафиксироватьСообщениеВИсторииОбмена(ДанныеХДТО.НомерСообщения, Исходящий, ТекстСообщения);
	//)Семенов И.П.
КонецПроцедуры
...
Функция ВыгрузитьСообщениеОбмена(вхИдентификаторУзлаОбмена, ReceivedNo, НеСжиматьСообщение = Ложь) Экспорт
	...
	Исключение
		...
		//Семенов И.П. 07.02.2019 XX-1768(
		ОбменДаннымиКлиентСервер.ЗафиксироватьСообщениеВИсторииОбмена(лЗаписьСообщения.НомерСообщения,лИсходящий,"",Истина,Истина,ОписаниеОшибки());
		//)Семенов И.П.
		ВызватьИсключение ;
	КонецПопытки;
	...
	//Семенов И.П. 07.02.2019 XX-1768(
	ОбменДаннымиКлиентСервер.ЗафиксироватьСообщениеВИсторииОбмена(лЗаписьСообщения.НомерСообщения, лИсходящий, НесжатоеСообщение, Истина);
	//)Семенов И.П.
...
Процедура ОбработкаЗаявокЭД(МассивРТУ,СтруктураОтчета,МестоВозникновения)
	...
	Иначе
		...
		Если СтруктураОтчета.КоличествоПопытокЗалечить >= КоличествоПопытокЗалечитьМаксимум Тогда
