...
Процедура ЗагрузитьРеализацию(СтруктураДокумента, СтруктураОтчета, РеализацияБылаСформирована)
	...
	Если НЕ СтруктураОтчета.Error тогда
		...
		Попытка
			id_ЭД = Документ.express_delivery_request_id;
			Если ЗначениеЗаполнено(id_ЭД) тогда
				SMSоповещение.РассчитатьУслугуЭД(id_ЭД);
			КонецЕсли;
		Исключение
		КонецПопытки;
...
Процедура ExpressDelivery1cConfirm(пар_МассивРТУ) Экспорт
	...
	//	Запрос_Текст = Запрос_Текст + Символы.ПС + "РеализацияТоваровУслуг.Контрагент.Регион = &РегионОтбора";
	//КонецЕсли;
	//
	//Запрос = Новый Запрос;
	...
	//Если НЕ РегионОтбора = Справочники.Регионы.ПустаяСсылка() тогда
	//	Запрос.УстановитьПараметр("РегионОтбора", РегионОтбора);
	//КонецЕсли;
