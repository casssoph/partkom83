...
Процедура ЗагрузитьЭлементЭкспрессДоставки(ОбъектXDTO, НомерСообщенияВходящий) Экспорт
	...
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЗагрузитьЭлементЭкспрессДоставки"; // Вставить название процедуры
...
Процедура ДобавитьСтрокуИсторииПоОбъектуПоЭД(пар_ОбъектXDTO,пар_RequestID,пар_Error,пар_Description)
	...
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ДобавитьСтрокуИсторииПоОбъектуПоЭД"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЭкспрессДоставка_init(СтруктураОбъекта, СтруктураОтчета)
	...
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЭкспрессДоставка_init"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЗагрузитьРеализацию(СтруктураДокумента, СтруктураОтчета, РеализацияБылаСформирована)
	...
	///////////////////////////////////////////////////////////////////////////
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЗагрузитьРеализацию"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЭкспрессДоставка_add(СтруктураОбъекта, СтруктураОтчета)
	...
	//
	/////////////////////////////////////////////////////////////////////////////	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЭкспрессДоставка_add"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция НоменклатураПоАртикулуИКодуИзготовителя(Строка, СтрокаОтчета)
	...
	// + Пушкин 20190722 XX-3000
	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_НоменклатураПоАртикулуИКодуИзготовителя";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
...
Процедура ПолучитьДанныеПоЗаявке(Строка, СтрокаОтчета)
	// + Пушкин 20190726 XX-3000
	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ПолучитьДанныеПоЗаявке";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	|			(ВЫБРАТЬ
	|				СТРОКА_ЗАЯВКИ.ПоследняяКорректировка
	|			ИЗ
	|				СТРОКА_ЗАЯВКИ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	СТРОКА_ЗАЯВКИ.Ссылка,
	|	СТРОКА_ЗАЯВКИ.Заявка,
	|	СТРОКА_ЗАЯВКИ.ПоследняяКорректировка,
	|	СТРОКА_ЗАЯВКИ.ДокументИсточник,
	|	ДокументыИсточники_Товары.Номенклатура
	|ИЗ
	|	СТРОКА_ЗАЯВКИ КАК СТРОКА_ЗАЯВКИ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыИсточники_Товары КАК ДокументыИсточники_Товары
	|		ПО СТРОКА_ЗАЯВКИ.ДокументИсточник = ДокументыИсточники_Товары.Ссылка
	|			И СТРОКА_ЗАЯВКИ.Ссылка = ДокументыИсточники_Товары.СтрокаЗаявки
	|ГДЕ
	|	ДокументыИсточники_Товары.Номенклатура В
	|			(ВЫБРАТЬ
	|				Товары.Ссылка
	|			ИЗ
	|				Товары)";
	...
	Запрос.УстановитьПараметр("Артикул", Строка.number);
	Запрос.УстановитьПараметр("КодИзготовителя", Строка.maker_id);
	Запрос.УстановитьПараметр("IDSite", Строка.motion_id);

	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		
		СтрокаОтчета.ИдентификаторСтрокиЗаявки = Выборка.Ссылка;
		СтрокаОтчета.Номенклатура = Выборка.Номенклатура;
		
	Иначе
		
		СтрокаОтчета.ИдентификаторСтрокиЗаявки = Справочники.ИдентификаторыСтрокЗаявок.ПустаяСсылка();
		СтрокаОтчета.Номенклатура = Справочники.Номенклатура.ПустаяСсылка();
		
		СтрокаОтчета.error = Истина;
		СтрокаОтчета.description = "Не найден IDSite&Номенклатура";
	КонецЕсли;
...
Функция ТаблицаТоваров(items, ЭтоУдаление = Ложь)
	...
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ТаблицаТоваров";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Для Каждого Строка Из items.item Цикл
		...
		// + Пушкин 20190726 XX-3000
		ПолучитьДанныеПоЗаявке(Строка, НоваяСтрока);
...
Процедура ОбработкаЗаявокЭД(МассивРТУ,СтруктураОтчета,МестоВозникновения) экспорт
	...
	//
	/////////////////////////////////////////////////////////////////////////////
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ОбработкаЗаявокЭД"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура тчНеХватаетОстатка_Обработать(оДок,лДок,тчНеХватаетОстатка) 
	...
	//
	/////////////////////////////////////////////////////////////////////////////	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_тчНеХватаетОстатка_Обработать"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ОформитьОтказыПоЗаявкам(парам_ТЧ, парам_СтрокиЗаявок = Неопределено)
	...
	//
	/////////////////////////////////////////////////////////////////////////////	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ОформитьОтказыПоЗаявкам"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЗаписатьОтчетПоЗагрузке(СтруктураОтчета, НомерСообщенияВходящий)
	...
	//
	/////////////////////////////////////////////////////////////////////////////	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЗаписатьОтчетПоЗагрузке"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Процедура ЗарегисрироватьИзмененияДляОбменаПоЭД(пар_МассивРТУ,пар_Команда)
	...
	//
	/////////////////////////////////////////////////////////////////////////////	
	лКлючАлгоритма = "ПланОбмена_ОбменПартКом83_Сайт_МодульМенеджера_ЗарегисрироватьИзмененияДляОбменаПоЭД"; // Вставить название процедуры
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
