	...
	Попытка
		...
		Если ТаблицаОстатков.Количество() > 0 тогда
			...
			Для Каждого лЭлементСоответствия Из ТаблицаОстатков цикл
				...
				лОбъект.СвободныйОстатокКоличество = лЭлементСоответствия.СвободныйОстатокКоличество;
...
Запрос.Текст = 
	...
	|					Прайсы.Склад
	|				ИЗ
	|					Прайсы)) КАК ПартииТоваровОстатки
	|ГДЕ
	|	НЕ ПартииТоваровОстатки.Номенклатура ЕСТЬ NULL
	|	И НЕ ПартииТоваровОстатки.Склад ЕСТЬ NULL
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода ЕСТЬ NULL
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода.ТорговаяТочка ЕСТЬ NULL
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода.ТорговаяТочка.Владелец ЕСТЬ NULL
	|	И НЕ ПартииТоваровОстатки.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И НЕ ПартииТоваровОстатки.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода = ЗНАЧЕНИЕ(Справочник.ИдентификаторыСтрокПриходов.ПустаяСсылка)
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода.ТорговаяТочка = ЗНАЧЕНИЕ(Справочник.ТорговыеТочки.ПустаяСсылка)
	|	И НЕ ПартииТоваровОстатки.СтрокаПрихода.ТорговаяТочка.Владелец = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПартииТоваровОстатки.Номенклатура,
	|	ПартииТоваровОстатки.Склад,
	...
	|					Прайсы.Склад
	|				ИЗ
	|					Прайсы)) КАК РезервыТоваровОстатки
	|ГДЕ
	|	НЕ РезервыТоваровОстатки.Номенклатура ЕСТЬ NULL
	|	И НЕ РезервыТоваровОстатки.Склад ЕСТЬ NULL
	|	И НЕ РезервыТоваровОстатки.СтрокаЗаявки ЕСТЬ NULL
	|	И НЕ РезервыТоваровОстатки.СтрокаЗаявки.Поставщик ЕСТЬ NULL
	|	И НЕ РезервыТоваровОстатки.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И НЕ РезервыТоваровОстатки.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	И НЕ РезервыТоваровОстатки.СтрокаЗаявки = ЗНАЧЕНИЕ(Справочник.ИдентификаторыСтрокЗаявок.ПустаяСсылка)
	|	И НЕ РезервыТоваровОстатки.СтрокаЗаявки.Поставщик = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	РезервыТоваровОстатки.Номенклатура,
	|	РезервыТоваровОстатки.Склад,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварныеОстатки.Номенклатура,
	|	ТоварныеОстатки.Склад,
	|	ТоварныеОстатки.Поставщик,
	|	ТоварныеОстатки.ТТпоставщика,
	|	ТоварныеОстатки.КоличествоОстаток - ЕСТЬNULL(Резервы.КоличествоОстаток, 0) КАК КоличествоОстаток
	|ПОМЕСТИТЬ СвободныйОстаток
	|ИЗ
	|	ТоварныеОстатки КАК ТоварныеОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Резервы КАК Резервы
	|		ПО ТоварныеОстатки.Номенклатура = Резервы.Номенклатура
	|			И ТоварныеОстатки.Склад = Резервы.Склад
	|			И ТоварныеОстатки.Поставщик = Резервы.Поставщик
	|ГДЕ
	|	ТоварныеОстатки.КоличествоОстаток - ЕСТЬNULL(Резервы.КоличествоОстаток, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(СвободныйОстаток.КоличествоОстаток) КАК КоличествоОстаток
	|ПОМЕСТИТЬ СвойТовар
	|ИЗ
	|	СвободныйОстаток КАК СвободныйОстаток
	|ГДЕ
	|	СвободныйОстаток.Склад.СкладVMI = ЛОЖЬ
	|
	|СГРУППИРОВАТЬ ПО
	|	СвободныйОстаток.Номенклатура,
	|	СвободныйОстаток.Склад
	...
	|	СвободныйОстаток КАК СвободныйОстаток
	|ГДЕ
	|	СвободныйОстаток.Склад.СкладVMI = ИСТИНА
	|
	|СГРУППИРОВАТЬ ПО
	|	СвободныйОстаток.Номенклатура,
	|	СвободныйОстаток.Склад,
