...
Функция GetData(date, User) Экспорт 
	...
	Если НачалоДня(date) <>НачалоДня(ТекущаяДата()) тогда
		Возврат СписокЗаданий;	
	КонецЕсли;	
    ...
    Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Телефон) и Лев(Телефон,1)<>"+" тогда
			Телефон="+"+Телефон;
		КонецЕсли;	
		ЗаданиеНаДоставку.Contact = XMLСтрока(" " + Выборка.Contact+" "+Телефон);
