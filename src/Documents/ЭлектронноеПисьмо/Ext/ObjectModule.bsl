...
Процедура ЗаполнитьНаОснованииАктаВозврата(ПараметрыЗаполнения)
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(АктВозврата, "Контрагент, Номер, Штрихкод, Ответственный");
	...
	ИначеЕсли нрег(Кому) = "поставщик" Тогда
		...
		//Поставщик
		Если ТоварыАкта.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(ТоварыАкта[0].Контрагент) Тогда
				КомуКонтрагент = ТоварыАкта[0].Контрагент;
			КонецЕсли;
		КонецЕсли;
	...
	УчетнаяЗапись = Справочники.УчетныеЗаписиЭлектроннойПочты.ВозвратыКлиентов;
