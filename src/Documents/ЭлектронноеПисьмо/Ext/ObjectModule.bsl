...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	лКлючАлгоритма = "Документ_ЭлектронноеПисьмо_МодульОбъекта_ПередЗаписью";
...
Процедура ЗаполнитьНаОснованииАктаВозврата(ПараметрыЗаполнения)
	...
	лКлючАлгоритма = "Документ_ЭлектронноеПисьмо_МодульОбъекта_ЗаполнитьНаОснованииАктаВозврата";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ВидКИ		= Неопределено;
	ПараметрыЗаполнения.Свойство("ВидКИ", ВидКИ);
	
	АктРассмотренияВозврата = АктВозврата;
	
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(АктВозврата, "Контрагент, Номер, Штрихкод, Ответственный, Поставщик, АдресЭлектроннойПочтыПокупателя, ИмяКонтактногоЛицаПокупателя");
	
	ОснованиеПисьма = АктВозврата;
	...
	|	ВЫБОР
	|		КОГДА АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика = ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.Пустаяссылка)
	|			ТОГДА АктРассмотренияВозвратаТовары.СтрокаПрихода.Приход.Контрагент
	|		ИНАЧЕ АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика.Контрагент
	|	КОНЕЦ КАК Контрагент,
	|	ВЫБОР
	|		КОГДА АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика = ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.Пустаяссылка)
	|			ТОГДА АктРассмотренияВозвратаТовары.СтрокаПрихода.Приход.НомерВходящегоДокумента
	|		ИНАЧЕ АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика.НомерВходящегоДокумента
	|	КОНЕЦ КАК НомерВходящегоДокумента,
	|	ВЫБОР
	|		КОГДА АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика = ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.Пустаяссылка)
	|			ТОГДА АктРассмотренияВозвратаТовары.СтрокаПрихода.Приход.ДатаВходящегоДокумента
	|		ИНАЧЕ АктРассмотренияВозвратаТовары.Ссылка.ПоступлениеДругогоПоставщика.ДатаВходящегоДокумента
	...
	Если ТоварыАкта.Количество() > 0 Тогда
		 СтрокаТовараТемы = ""+ ТоварыАкта[0].Изготовитель + " / " 
	...
	Если нрег(Кому) = "покупатель" Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыДокумента.АдресЭлектроннойПочтыПокупателя) Тогда
			 КомуАдрес = РеквизитыДокумента.АдресЭлектроннойПочтыПокупателя;
		КонецЕсли;
	...
	ИначеЕсли нрег(Кому) = "поставщик" Тогда
		...
		Для каждого СтрокаТоваров Из ТоварыАкта Цикл
			СписокТоваровШаблона = СписокТоваровШаблона +"
											...
											|
											| Ссылка на загруженные материалы: "+ВозвратыОтПокупателяСервер.АдресВложенийСайта()+РеквизитыДокумента.Штрихкод+"
											|
											|"; 
		КонецЦикла;
	...
	//Кому
	Если ЗначениеЗаполнено(КомуКонтрагент)
		И НЕ ЗначениеЗаполнено(КомуАдрес) Тогда
		
		КомуАдрес = ВозвратыОтПокупателяСервер.АдресОтправкиПочтыКонтрагенту(КомуКонтрагент, ВидКИ);
		
		//
		//КомуАдрес = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(КомуКонтрагент, Справочники.ВидыКонтактнойИнформации.EmailДляВозвратов);
		//Если Не ЗначениеЗаполнено(КомуАдрес) Тогда
	...
	МассивАдресов = СтрРазделить(КомуАдрес, ",; ");
	Для каждого АдресОтправки Из МассивАдресов Цикл
		Если ЗначениеЗаполнено(СокрЛП(АдресОтправки)) Тогда
			НоваяСтрокаТЧ = КомуТЧ.Добавить();
			НоваяСтрокаТЧ.АдресЭлектроннойПочты = СокрЛП(АдресОтправки);
			НоваяСтрокаТЧ.Представление         = КомуКонтрагент;
		КонецЕсли;
	КонецЦикла;	
