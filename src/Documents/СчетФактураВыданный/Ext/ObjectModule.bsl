...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(ДанныеСчетФактуры = Неопределено) Экспорт
	...
	Попытка
		Если ТипЗнч(ДанныеСчетФактуры.ДокументОснование)=ТипЗнч(Документы.ВозвратТоваровПоставщику.ПустаяСсылка()) Тогда 
			Для Каждого Стр Из ДанныеДляПечати.ТабличнаяЧасть Цикл
				Если ТипЗнч(ДанныеСчетФактуры.ДокументОснование.ДокументОснование)=ТипЗнч(Документы.ПоступлениеТоваровУслуг.ПустаяСсылка()) Тогда 
					СТРТТ=ДанныеСчетФактуры.ДокументОснование.ДокументОснование.Товары.НайтиСтроки(Новый Структура("Номенклатура",Стр.Товар));
...
Функция СобратьДанныеПоРеализацияТоваровУслуг(ДокОснование)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	РеализацияТоваровУслуг.ДоговорКонтрагентаВзаиморасчетов.Организация.УчитыватьНДС КАК УчитыватьНДС,
