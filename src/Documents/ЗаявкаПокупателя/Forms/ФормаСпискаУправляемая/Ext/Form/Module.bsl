...
Процедура ИсторияСтрокЗаявок(Команда)
	...
	Если Элементы.Список.ТекущаяСтрока <> Неопределено Тогда
		...
		СформироватьИсториюСтрокЗаявок(Форма.ПечатныйДокумент, Элементы.Список.ТекущаяСтрока);
		Форма.Открыть();
	КонецЕсли;
	
КонецПроцедуры
&НаСервере
Процедура СформироватьИсториюСтрокЗаявок(ТабличныйДокумент, ЗаявкаСсылка)
	
	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	
	СхемаКомпоновкиДанных = Документы.ЗаявкаПокупателя.ПолучитьМакет("ИсторияСтрокЗаявок");
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных));
 	КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);
 	Настройки = КомпоновщикНастроек.ПолучитьНастройки();
	УстановитьПараметрСКД(Настройки, "ДокументЗаявки", ЗаявкаСсылка);
