...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если не ОбменДанными.Загрузка  Тогда
		Если не ОбщегоНазначения.ПолучитьПоследнююКорректировкуЗаявки(Ссылка) = Ссылка и ПометкаУдаления Тогда
			Сообщить("Удаление заявки невозможно, т.к. существуют не помеченные на удаление корректировки данной заявки. Сначала необходимо их пометить на удаление.", СтатусСообщения.ОченьВажное);	
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2851) Тогда
		//Данный код хоронит идею источника заявки, он глуп и вреден 
		//Партия сказала что так и надо
		Если ЗначениеЗаполнено(ВалютаДокумента) И НЕ ВалютаДокумента = ПараметрыСеанса.ВалютаРубль Тогда
			ИсточникЗаявки = Перечисления.ИсточникиЗаявок.Импорт;
		КонецЕсли;
	КонецЕсли;
	//ХудинВВ XX-2851 10072019 }}
...
Функция ПолучитьПараметрыПечатиСчетаЗаказа() Экспорт
	...
	|	ЕСТЬNULL(КИ.Представление, ""он же"") КАК Грузополучатель,
	...
	|		ПО Док.Контрагент = КИ.Объект
	|			И (ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Адрес) = КИ.Тип)
	|			И (ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.АдресДоставкиКонтрагента) = КИ.Вид)
	|ГДЕ
	...
	ПараметрыПечати.Вставить("ПредставлениеГрузополучателя", ?(Шапка.Грузополучатель = "он же", Шапка.Грузополучатель,
	ОбщегоНазначения.ПолучитьЗначениеРеквизита(Шапка.Получатель, "НаименованиеПолное") + "; " + Шапка.Грузополучатель));
