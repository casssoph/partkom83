...
Процедура ОбновитьПоследнююКорректировку(Знач вхСсылкаНаДокумент) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		лОбъект.УдалитьСостояниеЗаявки = СостояниеЗаявки;
		лОбъект.Записать();
...
Процедура ПоставитьПолныйОтказПоДокументу(ДокументСсылка,Период = Неопределено, ПричинаОтказа = Неопределено) Экспорт
	...
	// РаботаСоСтатусамиДокументовСервер.ИзменитьКорректировкуЗаказаЗаявки(ПоследняяКорректировка,СтруктураЗаполнения,Истина);
	
КонецПроцедуры	

Функция ДанныеКПолномуОтказу(ПоследняяКорректировка, Период = Неопределено)
	...
	ТексЗапроса = "ВЫБРАТЬ
	              ...
	              |		СУММА(ЕСТЬNULL(ЗаявкиПокупателейОстатки.КоличествоОстаток, 0)) КАК КоличествоОстатокЗаказ
	              |	ИЗ
	              ...
	              |
	              |ИМЕЮЩИЕ
	              |	СУММА(ДанныеКЗакрытию.КоличествоОстатокЗаявок - ДанныеКЗакрытию.КоличествоВСборке) > 0";
