...
Процедура ПередЗаписью(Отказ)
	...
	// 23.07.19 Строганов Роман > 
	ДополнитьДвиженияПоРегиструТоварыНаСкладах();
	// 23.07.19 Строганов Роман <
...
Процедура ДополнитьДвиженияПоРегиструТоварыНаСкладах()
	
	лКлючАлгоритма = "Документ_КорректировкаЗаписейРегистров_ФормаДокуента_ДополнитьДвиженияПоРегиструТоварыНаСкладах";
	...
	Если ТаблицаРегистровНакопления.Найти("ПартииТоваров") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТаблицаРегистровНакопления.Найти("ТоварыНаСкладах") = Неопределено Тогда
		НоваяСтрока = ТаблицаРегистровНакопления.Добавить();
		НоваяСтрока.Имя = "ТоварыНаСкладах";
		НоваяСтрока.Представление = "Товары на складах";
	КонецЕсли;
		
	Ключи = "Активность, Период, Номенклатура, Склад, Качество, Количество";
	
	ТаблицаПартииТоваров = Движения.ПартииТоваров.Выгрузить(,Ключи);
	ТаблицаТоварыНаСкладах = Движения.ТоварыНаСкладах.Выгрузить(,Ключи);
	ТаблицаТоварыНаСкладахДляСравнения = Движения.ТоварыНаСкладах.Выгрузить(,Ключи);
	
	ТаблицаДельта = ТаблицаПартииТоваров.СкопироватьКолонки();
	
	РаботаСПоследовательностямиКлиентСервер.ТаблицыИдентичныНовое(ТаблицаТоварыНаСкладахДляСравнения, ТаблицаПартииТоваров, ТаблицаДельта, "Количество");
	
	Если ТаблицаДельта.Количество() Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаДельта Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаТоварыНаСкладах.Добавить(), СтрокаТаблицы);
		КонецЦикла;
		ТаблицаТоварыНаСкладах.Свернуть("Активность, Период, Номенклатура, Склад, Качество","Количество");
		МассивНулевыхСтрок = ТаблицаТоварыНаСкладах.НайтиСтроки(Новый Структура("Количество", 0));
		Если МассивНулевыхСтрок.Количество() Тогда
			КоличествоЭлементов = МассивНулевыхСтрок.Количество();
			Для ОбратныйИндекс = 1 По КоличествоЭлементов Цикл
				ТекЭлемент = МассивНулевыхСтрок[КоличествоЭлементов - ОбратныйИндекс];
				ТаблицаТоварыНаСкладах.Удалить(ТекЭлемент);
			КонецЦикла;
		КонецЕсли;
		Движения.ТоварыНаСкладах.Загрузить(ТаблицаТоварыНаСкладах);
	КонецЕсли;
