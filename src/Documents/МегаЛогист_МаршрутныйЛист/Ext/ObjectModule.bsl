...
Функция ПечатьМаршрутныйЛист(БезТоваров=0)
	...
	|	ВложенныйЗапрос.МаршрутноеЗадание
	|ИЗ
	|	(ВЫБРАТЬ
	...
	Для каждого тСтрока Из ТаблицаЗаданий Цикл 
		...
		Если БезТоваров=0 тогда
			...
			Для Каждого СтрокаРеализации из тСтрока.МаршрутноеЗадание.ДокументыРеализации цикл
				Для Каждого ттСтрока Из СтрокаРеализации.ДокументСсылка.Товары Цикл
				...
				КонецЦикла;
				Если СтрокаРеализации.ДокументСсылка.Товары.Количество() = 0 Тогда
					ОбластьСтрокаТовар	= Макет.ПолучитьОбласть("Строка|Товар");
					ОбластьТовар.Вывести(ОбластьСтрокаТовар);
				КонецЕсли;
			КонецЦикла;	
		...
		//Spoon++
		МетаданныеДокумента = тСтрока.МаршрутноеЗадание.Метаданные();
		...
		ОбластьОкончаниеСтроки.Параметры.Сумма	= "";
		//Spoon--
...
Процедура УстановитьТипДоставки()
	
	Если МаршрутныеЗадания.Количество()>0 тогда
		ЭкспрессДоставка=МаршрутныеЗадания[0].ДокументСсылка.ЭкспрессДоставка;
		ТипДоставки=МаршрутныеЗадания[0].ДокументСсылка.ТипДоставки;
	иначе
		ЭкспрессДоставка=Ложь;
		ТипДоставки=Справочники.ТипыДоставки.ПустаяСсылка();	
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)	
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение и Статус = Перечисления.МегаЛогист_СтатусыМаршрутныхЛистов.Выполняется Тогда
		...
		Если Не ЗначениеЗаполнено(Транспорт) Тогда
			Сообщить("Не заполнен транспорт!", СтатусСообщения.Важное);
			Отказ = Истина;
		КонецЕсли;
...
Функция ВыгрузитьВОбменТоплог() Экспорт
	...
	лКлючАлгоритма = "Документ_МегаЛогист_МаршрутноеЗадание_МодульОбъекта_ВыгрузитьВОбменТоплог";
