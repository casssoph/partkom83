...
Функция РассчитатьСуммуСписания( ТекСтрока, ТабДвижений)
	...
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_РассчитатьСуммуСписания";
...
Функция ПечатьСписаниеТоваров()
	...
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_ПечатьСписаниеТоваров";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	|		Номенклатура.Изготовитель.Наименование КАК Изготовитель,
	|		Коэффициент,
	|		Количество,
	|		Качество,
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		ОбластьДанных.Параметры.Сумма  = ПолучитьСтоимостьТовара(ТаблицаСтоимости, ВыборкаСтрокТовары);
	...
	ОбластьМакета.Параметры.ОтветственныйПредставление = Ответственный.Наименование;
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПолучитьСтоимостьТовара(Таблица, ТекСтрока)
	...
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_ПолучитьСтоимостьТовара";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ПолучитьТаблицуСтоимостиТоваров()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СУММА(ПартииТоваровНаСкладах.СуммаРубли) КАК Стоимость,
	               ...
	               |ИЗ
	               |	РегистрНакопления.ПартииТоваров КАК ПартииТоваровНаСкладах
	               |ГДЕ
	               ...
	               |	ПартииТоваровНаСкладах.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент";
...
Функция ПечатьТОРГ16()
	...
	СтруктраПараметров.Вставить("Филиал",  		   Филиал);
...
//ХудинВВ 21032019 XX-2138
Функция ВыгрузитьВОбменТоплог() Экспорт
	
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_ВыгрузитьВОбменТоплог";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Если Не СтатусДокумента = Справочники.СтатусыДокументов.СписаниеТоваровНовый	Тогда	
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АктРассмотренияВозврата") Тогда
		...
		|	АктРассмотренияВозврата.ДокументПродажи.Склад КАК Склад,
		|	АктРассмотренияВозврата.Организация КАК Организация,
		|	АктРассмотренияВозврата.Контрагент.ОсновнаяТорговаяТочка.Регион.Филиал КАК Филиал
		|ИЗ
		...
		СтатусДокумента = Справочники.СтатусыДокументов.СписаниеТоваровНовый;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_ПередЗаписью";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	// ЛНА http://my.part-kom.ru/Tasks/Task/Execute/10177629
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		И Не ЗначениеЗаполнено(Комментарий) Тогда
		Сообщить("Необходимо заполнить поле ""Комментарий"" (указать обоснование - задача, процесс или решение на акте или бланке)", СтатусСообщения.Важное);
		Отказ = Истина;
	КонецЕсли;
	...
	СтатусДокументаДоЗаписи = ?(Не ЗначениеЗаполнено(Ссылка), Неопределено, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "СтатусДокумента"));
	ЭтотОбъект.ДополнительныеСвойства.Вставить("СтатусДокументаДоЗаписи", СтатусДокументаДоЗаписи);
	...
	Если ЗначениеЗаполнено(АктРассмотренияВозврата)
		...
		И СтатусДокумента = Справочники.СтатусыДокументов.СписаниеТоваровСписан Тогда
	...
	//		Отказ = Истина;
	//		ВызватьИсключение "По документу """+АктРассмотренияВозврата+""" уже создан документ списания для организации "+Организация+"!";
	//	КонецЕсли;
	//КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	лКлючАлгоритма = "Документ_СписаниеТоваров_МодульОбъекта_ПриКопировании";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ВыгруженВТопЛогАвтопроведение = Ложь;
КонецПроцедуры
