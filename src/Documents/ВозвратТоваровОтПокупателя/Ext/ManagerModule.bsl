...
Функция ДобавитьДвиженияПоМФПВТаблицуПередачи(вхСсылкаНаДокумент, ТаблицаКПередаче)
	...
	Для каждого СтрокаТЗ ИЗ  ТЗ Цикл
		...
		//ЦенаРасходСтарая = СтрокаТЗ.ЦенаРасход;
		...
		Для каждого ПроцентНаценки Из ПроцентыНаценки Цикл
			...
			СтрокаТЗ.СуммаДолларыПриход = СтрокаТЗ.СуммаРублиПриход/((100+ПроцентНаценки)/100);
			СтрокаТЗ.СуммаЕвроПриход 	= СтрокаТЗ.СуммаРублиПриход/((100+ПроцентНаценки)/100);
			СтрокаТЗ.СуммаБезНДСПриход 	= СтрокаТЗ.СуммаРублиПриход/((100+ПроцентНаценки)/100);
			
			
			
					//СтрокаТЗ.СуммаРублиПриход 	= СтрокаТЗ.СуммаРублиПриход 	- СтрокаТЗ.СуммаРублиПриход*ПроцентНаценки/100;
					//СтрокаТЗ.СуммаДолларыПриход = СтрокаТЗ.СуммаДолларыПриход 	- СтрокаТЗ.СуммаДолларыПриход*ПроцентНаценки/100;
					//СтрокаТЗ.СуммаЕвроПриход 	= СтрокаТЗ.СуммаЕвроПриход 		- СтрокаТЗ.СуммаЕвроПриход*ПроцентНаценки/100;
					//СтрокаТЗ.СуммаБезНДСПриход 	= СтрокаТЗ.СуммаБезНДСПриход 	- СтрокаТЗ.СуммаБезНДСПриход*ПроцентНаценки/100;
			//КонецЕсли;
			сч =сч+1;
		КонецЦикла;
		...
		Для каждого ПроцентНаценки Из ПроцентыНаценки Цикл
			Если сч < КоличествоПроцентов Тогда
				//СтрокаТЗ.СуммаРубли 	= СтрокаТЗ.СуммаРубли 		- СтрокаТЗ.СуммаРубли*ПроцентНаценки/100;
				//СтрокаТЗ.СуммаДоллары 	= СтрокаТЗ.СуммаДоллары 	- СтрокаТЗ.СуммаДоллары*ПроцентНаценки/100;
				//СтрокаТЗ.СуммаЕвро 		= СтрокаТЗ.СуммаЕвро 		- СтрокаТЗ.СуммаЕвро*ПроцентНаценки/100;
				//СтрокаТЗ.СуммаБезНДС 	= СтрокаТЗ.СуммаБезНДС 		- СтрокаТЗ.СуммаБезНДС*ПроцентНаценки/100;
		...
		//СтрокаТЗ.ЦенаПриход = СтрокаТЗ.ЦенаРасход + СтрокаТЗ.ЦенаРасход*СтрокаТЗ.ПроцентНаценки/100
	...
	Запрос.Текст = "ВЫБРАТЬ
	 ...
	 Для каждого СтрокаВозвратов Из ТаблицаВозвратов Цикл
		 ...
		 Для каждого СтрокаОстатков Из СтрокиОстатков Цикл
			 ...
			 ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаВозвратов);
			 ...
			 Если НоваяСтрока.Количество <> СтрокаОстатков.Количество Тогда 
				 НоваяСтрока.СуммаРубли   	= НоваяСтрока.Количество * НоваяСтрока.СуммаРубли/СтрокаОстатков.Количество;
				 НоваяСтрока.СуммаДоллары 	= НоваяСтрока.Количество * НоваяСтрока.СуммаДоллары/СтрокаОстатков.Количество;
				 НоваяСтрока.СуммаЕвро 		= НоваяСтрока.Количество * НоваяСтрока.СуммаЕвро/СтрокаОстатков.Количество;
				 НоваяСтрока.СуммаБезНДС 	= НоваяСтрока.Количество * НоваяСтрока.СуммаБезНДС/СтрокаОстатков.Количество;
			 КонецЕсли;
