...
Процедура КонтрольОстатковПартий(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	ПодбиратьПартииПоРазмещениям = ВозвратыОтПокупателяСервер.ПодбиратьПартииПоРазмещениям(Реквизиты.Дата);
	
	Запрос.УстановитьПараметр("БезПартий", ПодбиратьПартииПоРазмещениям);
	...
	Для каждого СтрокаОстатков Из ОтрицательныеОстатки Цикл
		ТекстОшибок = ТекстОшибок + Символы.ПС +"Номенклатура: "+СтрокаОстатков.Номенклатура+
						?(ПодбиратьПартииПоРазмещениям, "", ", Строка прихода: "+СтрокаОстатков.СтрокаПрихода)+
...
Функция ПодготовитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента(Ссылка) Экспорт
	...
	"Ссылка,Дата,ВидОперации,СтатусДокумента, Организация,Контрагент,ДоговорКонтрагента,ДокументОснование,ДокументОснование.Номер,ДокументОснование.Дата,ДокументОснование.УчитыватьНДС");
	...
	Для каждого СтрокаВозврата Из ПредыдущиеВозвраты Цикл
		...
		Для каждого СтрокаПродажи Из СтрокиПродажи Цикл
			...
			Если УчитыватьСуммуВозвратов Тогда				
				СтрокаПродажи.СуммаДоИзменения = СтрокаПродажи.СуммаДоИзменения - 
													?(СтрокаВозврата.Количество = Списать, 
	...
	Для каждого СтрокаВозврата Из ТекущиеВозвраты Цикл
		...
		Для каждого СтрокаПродажи Из СтрокиПродажи Цикл
			...
			Если УчитыватьСуммуВозвратов Тогда				
				СтрокаПродажи.СуммаПослеИзменения = СтрокаПродажи.СуммаПослеИзменения - 
													?(СтрокаВозврата.Количество = Списать, 
													 СтрокаВозврата.Сумма, 
													(СтрокаВозврата.Сумма/СтрокаВозврата.Количество)*Списать);
				...
				СтрокаПродажи.СуммаНДСПослеИзменения = УчетНДС.РассчитатьСуммуНДС(СтрокаПродажи.СуммаПослеИзменения, 
																		СтрокаПродажи.УчитыватьНДС, 
																		СтрокаПродажи.СуммаВключаетНДС, 
																		СтрокаПродажи.СтавкаНДС);
													
				СтрокаПродажи.ЦенаПослеИзменения = ?(СтрокаПродажи.КоличествоПослеИзменения = 0,
													 0, 
													?(СтрокаПродажи.СуммаВключаетНДС,
													(СтрокаПродажи.СуммаПослеИзменения-СтрокаПродажи.СуммаНДСПослеИзменения)/СтрокаПродажи.КоличествоПослеИзменения,
													СтрокаПродажи.СуммаПослеИзменения/СтрокаПродажи.КоличествоПослеИзменения));
	...
	Для каждого СтрокаТаблицы Из ТоварыРеализации Цикл
		...
		СуммаДоИзменения 		= СтрокаТаблицы.СуммаДоИзменения; //Окр(СтрокаТаблицы.КоличествоДоИзменения*СтрокаТаблицы.ЦенаСНДСДоИзменения,		2);
		СуммаПослеИзменения 	= СтрокаТаблицы.СуммаПослеИзменения; //Окр(СтрокаТаблицы.КоличествоПослеИзменения*СтрокаТаблицы.ЦенаСНДСПослеИзменения,	2);
