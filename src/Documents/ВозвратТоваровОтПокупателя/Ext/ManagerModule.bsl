...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		|	Склады.СкладВозвратов КАК СкладВозвратов
		|ПОМЕСТИТЬ СкладыВозвратов
		|ИЗ
		|	Справочник.Склады КАК Склады
		|
		|СГРУППИРОВАТЬ ПО
		|	Склады.СкладВозвратов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА НЕ ВозвратТоваровОтПокупателяТовары.Ссылка.Склад.Возвраты
		|			ТОГДА ВозвратТоваровОтПокупателяТовары.Ссылка.Склад.ФизическийСклад
		|		ИНАЧЕ ЕСТЬNULL(СкладыВозвратов.Склад.ФизическийСклад, НЕОПРЕДЕЛЕНО)
		|	КОНЕЦ КАК Склад,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыВозвратов КАК СкладыВозвратов
		|		ПО (СкладыВозвратов.СкладВозвратов = ВозвратТоваровОтПокупателяТовары.Ссылка.Склад)
		|ГДЕ
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Пока лВыборка.Следующий() Цикл
				...
				лОбъект.КодВозврата 		= XMLСтрока(Прав("0"+лВыборка.КодВозврата,2));
