...
Функция ПодготовитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента(Ссылка) Экспорт
	...
	Для каждого СтрокаВозврата Из ТекущиеВозвраты Цикл
		...
		Для каждого СтрокаПродажи Из СтрокиПродажи Цикл
			...
			Списать = Мин(КВозврату, СтрокиПродажи.КоличествоДляРаспределения);
			Если Списать = 0 Тогда
				Продолжить;
			КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из ТекущиеВозвраты Цикл
		...
		СуммаДоИзменения 		= Окр(СтрокаТаблицы.КоличествоДоИзменения*СтрокаТаблицы.ЦенаДоИзменения,2);
		СуммаПослеИзменения 	= Окр(СтрокаТаблицы.КоличествоПослеИзменения*СтрокаТаблицы.ЦенаПослеИзменения,2);
		...
		СтрокаТаблицы.СтоимостьСНДСДоИзменения =  ?(СтрокаТаблицы.СуммаВключаетНДС, СуммаДоИзменения + СтрокаТаблицы.СуммаНДСДоИзменения, СуммаДоИзменения);
		СтрокаТаблицы.СтоимостьСНДСПослеИзменения =  ?(СтрокаТаблицы.СуммаВключаетНДС, СуммаПослеИзменения + СтрокаТаблицы.СуммаНДСПослеИзменения, СуммаПослеИзменения);
