...
Процедура ДополнитьТаблицуДвижениямиМФП(вхСсылкаНаДокумент, вхТаблицаДвижений, ТаблицаДвиженийВозвратыМФП = Неопределено)
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "КурсВзаиморасчетов,КратностьВзаиморасчетов,УчитыватьНДС,СуммаВключаетНДС,Дата,Организация,ВидОперации,Контрагент,АктРассмотренияВозврата,Склад,ВалютаДокумента,ДокументОснование"); 
	...
	Для каждого СтрокаТаблицыКПередаче Из ТаблицаКПередачеСМФП Цикл
		...
		СтрокаРасход 						= вхТаблицаДвижений.Добавить();
		...
		СтрокаПриход 						= вхТаблицаДвижений.Добавить();
	...
	Для каждого СтрокаКПередаче Из ТаблицаКПередаче Цикл
		...
		НоваяСтрока.Цена 						= СтрокаКПередаче.СуммаРубли/СтрокаКПередаче.Количество;
	КонецЦикла;
