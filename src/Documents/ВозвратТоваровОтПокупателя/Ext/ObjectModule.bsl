...
//# Kalinin V.A. ( 2018-06-30 )
	...
	МВТСе6бестоимость = новый МенеджерВременныхТаблиц;
	ЗапросПустых.Текст = "ВЫБРАТЬ
	              |	ТчТоваров.Номенклатура,
	              |	ТчТоваров.Себестоимость,
	              |	ТчТоваров.Количество
	              |ПОМЕСТИТЬ ВтТоваров
	              |ИЗ
	              |	&ТчТоваров КАК ТчТоваров
	              |ГДЕ
	              |	ТчТоваров.Себестоимость = 0
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
	              |	ВтТоваров.Номенклатура,
	              |	ВтТоваров.Себестоимость
	              |ИЗ
	              |	ВтТоваров КАК ВтТоваров";	
	ЗапросПустых.УстановитьПараметр("ТчТоваров",Товары.Выгрузить());
	ЗапросПустых.МенеджерВременныхТаблиц =МВТСе6бестоимость; 
	Результат = ЗапросПустых.Выполнить();
	Если Результат.Пустой() тогда 
		Возврат;
	КонецЕсли;
	 ЗапросСценами  = новый Запрос;
	 
	ЗапросСценами.Текст = "ВЫБРАТЬ
	                      |	ЦеныНоменклатурыСрезПоследних.Номенклатура,
	                      |	МАКСИМУМ(ЦеныНоменклатурыСрезПоследних.Цена) КАК СебестоимостьЦена,
	                      |	ЦеныНоменклатурыСрезПоследних.Цена * ВтТоваров.Количество КАК Себестоимость
	                      |ИЗ
	                      |	ВтТоваров КАК ВтТоваров
	                      ...
	                      |				(ТипЦен, Валюта) В
	                      |						(ВЫБРАТЬ ПЕРВЫЕ 1
	                      |							ЗакупочныйТипЦенРуб.Значение,
	                      |							ВалютаРубль.Значение КАК Валюта
	                      |						ИЗ
	                      |							Константа.ЗакупочныйТипЦенРуб КАК ЗакупочныйТипЦенРуб,
	                      |							Константа.ВалютаРубль КАК ВалютаРубль)
	                      |					И Номенклатура В
	                      |						(ВЫБРАТЬ
	                      |							ВтТоваров.Номенклатура
	                      |						ИЗ
	                      |							ВтТоваров КАК ВтТоваров)) КАК ЦеныНоменклатурыСрезПоследних
	                      |		ПО ВтТоваров.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	ЦеныНоменклатурыСрезПоследних.Номенклатура,
	                      |	ЦеныНоменклатурыСрезПоследних.Цена * ВтТоваров.Количество
	                      |;
	                      |
	                      |////////////////////////////////////////////////////////////////////////////////
	                      |УНИЧТОЖИТЬ ВтТоваров"     ;
	ЗапросСценами.МенеджерВременныхТаблиц = МВТСе6бестоимость;
	ЗапросСценами.УстановитьПараметр("Момент",Ссылка.МоментВремени());
