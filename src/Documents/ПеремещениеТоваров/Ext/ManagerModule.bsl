...
Процедура ЗагрузитьРезультатРазмещения(РазмещениеСсылкаТопЛог, ЗаказНаПриемкуXDTO, ОбъектXDTO, ТаблицаСсылок, Успешно, вхПараметры, СтруктураОтчета = Неопределено)
	...
	ДокОснованиеВозвратПоставщику = Неопределено;
	...
	ИначеЕсли ЗаказНаПриемкуXDTO.ЗаказВидДокумента = "ПеремещениеТоваровVMI" Тогда
		Если ЗначениеЗаполнено(ЗаказНаПриемкуXDTO.ЗаказСсылка) Тогда 
			ДокОснованиеВозвратПоставщику = Документы.ВозвратТоваровПоставщику.ПолучитьСсылку(Новый УникальныйИдентификатор(ЗаказНаПриемкуXDTO.ЗаказСсылка));
		Иначе
			ДокОснованиеВозвратПоставщику = Документы.ВозвратТоваровПоставщику.НайтиПоНомеру(ОбщегоНазначения.ПреобразоватьНомер(ЗаказНаПриемкуXDTO.ЗаказНомер), ТекущаяДата());
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("ДокументОснование", ДокОснованиеВозвратПоставщику);
	...
	Если ЭтоПоступление Тогда 	
		ЗагрузитьРезультатРазмещенияПТУ(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно, вхПараметры, СтруктураОтчета, ДокОснованиеВозвратПоставщику);
	Иначе
...
Процедура ЗагрузитьРезультатРазмещенияПТУ(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно, вхПараметры, СтруктураОтчета = Неопределено, ДокОснованиеВозвратПоставщику = Неопределено)
	...
	Для Каждого КлючЗначение Из СоответствиеСкладов Цикл 
		...
		Попытка
			...
			//<< 25.07.2019 Горохов П.Е. ХХ-2884
			//ДокОснованиеВозвратПоставщику = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ДокументОснование");
			ДопУсловиеПроведения = Истина;
			Если ЗначениеЗаполнено(ДокОснованиеВозвратПоставщику) Тогда
				СозданНаОснованииМХ3 = Ложь;
				ДопУсловиеПроведения = ПроведениеДокументовКлиентСервер.УсловиеПроведенияРазмещенияПТУНаОснованииВозвратаМХ3(ДокументОснование, СозданНаОснованииМХ3, ДокОснованиеВозвратПоставщику);
			КонецЕсли;
			//>> 25.07.2019 Горохов П.Е. ХХ-2884
			
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СтатусДокумента") = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят И ДопУсловиеПроведения Тогда 
				Попытка
		...
		Попытка
			...
			Если Не ПревышаетДокументОснование И (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ОбновленИзТопЛог") = Истина Или 
				...
				Исключение				
					...
					#Если Сервер Тогда 
						ОбменСТопЛогСервер.ОбработатьДействияНадОбъектом(вхПараметры, ДокОбъект.Ссылка, ОписаниеОшибки, "ЗагрузитьРезультатРазмещенияПерем");
					#КонецЕсли
