...
Функция РегистрыНакопления_ТоварыКРазмещениюНаСкладах(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если Не (Реквизиты.СтатусДокумента = Справочники.СтатусыДокументов.ПеремещениеТоваровПоступил И Реквизиты.ВидОперации = Перечисления.ВидыОперацийПеремещенияТоваров.ПриемкаТопЛог) Тогда
		Возврат ТаблицаДвижений;
	КонецЕсли;
...
Процедура ЗагрузитьРезультатРазмещения(РазмещениеСсылкаТопЛог, ЗаказНаПриемкуXDTO, ОбъектXDTO, ТаблицаСсылок, Успешно, вхПараметры)
	...
	//Результат размещения по возврату от покупателя не загружаем, только приемку
