...
Функция РегистрыНакопления_РезервыТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	Если РаботаСоСтатусамиДокументовСервер.НоваяСхемаЗакрытияЗаявок(Реквизиты.Дата) тогда 
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ВЫБОР
		               |		КОГДА ВтЗаказано.Количество - ЕСТЬNULL(ОтказыПоЗаявкамОбороты.КоличествоОборот, 0) >= ВтДанныеДокумента.Количество
		               |			ТОГДА ВтДанныеДокумента.Количество
		               |		ИНАЧЕ ВтЗаказано.Количество - ЕСТЬNULL(ОтказыПоЗаявкамОбороты.КоличествоОборот, 0)
		               |	КОНЕЦ КАК Количество
		               |ИЗ
		               |	ВтДанныеДокумента КАК ВтДанныеДокумента
