...
Процедура ВыполнитьПроведение(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если ПроведениеДокументовКлиентСервер.ПроводитсяПо(вхПараметры, "ПартииТоваров") 
		...
		Если вхПараметры.НеПерепроводитьПартии Тогда 
			...
			ПартииСписаны = СписатьПартииБезПерепроведения(вхСсылкаНаДокумент, вхОтказ, вхПараметры, лФильтр, лБазовая, Структура);  	
		Иначе
			ПартииСписаны = Ложь; 
		КонецЕсли;	
	...
	Если Не ПеремещенияВыгружаютсяИз77 Тогда 
		Если Не (Реквизиты.СтатусДокумента = Справочники.СтатусыДокументов.ПеремещениеТоваровПоступил 
			И (Реквизиты.ВидОперации = Перечисления.ВидыОперацийПеремещенияТоваров.ПриемкаТопЛог
				...
				ИЛИ  Реквизиты.ВидОперации = Перечисления.ВидыОперацийПеремещенияТоваров.ВозвратВПродажу)) Тогда
			Возврат ТаблицаДвижений;
		КонецЕсли;
	Иначе
...
Функция СписатьПартииБезПерепроведения(вхСсылкаНаДокумент, вхОтказ, вхПараметры, вхФильтр, вхБазовая, СтруктураТаблиц)
	...
	ТаблицаДляРасчетаСписанияПоПартиям = Документы.ПеремещениеТоваров.ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр, вхПараметры);
	...
	Для Каждого СтрокаТовары из ТаблицаДляРасчетаСписанияПоПартиям Цикл 
		Отбор = Новый Структура("Номенклатура,Качество,Организация");
...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено, вхПараметры = Неопределено) Экспорт
	...
	Если Не ПеремещенияВыгружаютсяИз77 Тогда 	
		...
		Запрос.Текст =  "ВЫБРАТЬ
						...
						|		ИЛИ ПеремещениеТоваровТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПеремещенияТоваров.ВозвратВПродажу)) %УсловиеПоНоменклатуре%
		                |
		                |ОБЪЕДИНИТЬ ВСЕ
		                |
		                |ВЫБРАТЬ
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		|	ПеремещениеТоваров.ВыгруженВТопЛог,
		|	ЕСТЬNULL(ПеремещениеТоваров.СкладПолучатель.КонтрагентТопЛог.ОсновнаяТорговаяТочка, ЗНАЧЕНИЕ(Справочник.ТорговыеТочки.ПустаяСсылка)),
		|	ЕСТЬNULL(ПеремещениеТоваров.СкладПолучатель.КонтрагентТопЛог.ОсновнаяТорговаяТочка.Город, ЗНАЧЕНИЕ(Справочник.Города.ПустаяСсылка)),
		|	ЕСТЬNULL(ПеремещениеТоваров.СкладПолучатель.КонтрагентТопЛог.ОсновнаяТорговаяТочка.Город.Наименование, """"),
		|	ЕСТЬNULL(ПеремещениеТоваров.СкладПолучатель.КонтрагентТопЛог.ОсновнаяТорговаяТочка.МаршрутДоставки.Код, """"),
		|	ЕСТЬNULL(ПеремещениеТоваров.СкладПолучатель.КонтрагентТопЛог.ОсновнаяТорговаяТочка.МаршрутДоставки.Наименование, """"),
		|	ВЫБОР
		|		КОГДА ПеремещениеТоваров.ДатаОтгрузки = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА КОНЕЦПЕРИОДА(&ТекущаяДата, ГОД)
		|		ИНАЧЕ ПеремещениеТоваров.ДатаОтгрузки
	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog
		...
		Если ОбъектXDTO.Тип().Имя = "РезультатРазмещения" Тогда  
			Попытка
				...
				Документы.РазмещениеТоваров.ЗагрузитьЭлемент(ОбъектXDTO, вхОтправитель, Отказ, вхПараметры);
			Исключение
				СтруктураОшибки = Новый Структура;
				СтруктураОшибки.Вставить("ОбъектXDTO", ОбъектXDTO.Тип().Имя);
				СтруктураОшибки.Вставить("GUID", ОбъектXDTO.РазмещениеСсылка);
				...
				УИДРазмещения = Новый УникальныйИдентификатор(ОбъектXDTO.РазмещениеСсылкаТопЛог);
				...
				ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(Документы.РазмещениеТоваров.ПолучитьСсылку(УИДРазмещения), ОбъектXDTO,,Истина,ОписаниеОшибки());
				//)Семенов И.П.
...
Процедура ЗаполнитьОрганизацииТЧТоварыПоПартиям(ДокОбъект, УдалятьСтроки = Ложь) Экспорт
	...
	//Выполним списание партий, чтобы получить список организаций по которым есть подходящие остатки,
	//эти организации подставим в табчасть, тк они там нужны для 77
