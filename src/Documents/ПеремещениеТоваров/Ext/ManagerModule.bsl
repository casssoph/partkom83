...
Процедура ЗагрузитьРезультатРазмещенияПерем(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно, вхПараметры, СтруктураОтчета = Неопределено)
	...
	Для Каждого КлючЗначение Из СоответствиеСкладов Цикл 
		...
		Попытка
			...
			Если  //http://jira.part-kom.ru/browse/XX-2942 убрал это условие //Не ПревышаетДокументОснование И 
				(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ОбновленИзТопЛог") = Истина Или 
...
Функция ОстаткиСтрокПрихода(ДокументОснование, ДокСсылка)
	...
	Запрос.УстановитьПараметр("СкладОтправитель", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Склад.СкладПриемки"));
...
Функция ОстаткиСтрокПриходаДляПеремещения(ДокументОснование, ДокСсылка) 
	...
	Запрос.УстановитьПараметр("ПустаяЗаявка", Документы.ЗаявкаПокупателя.ПустаяСсылка());	
	//Валиахметов http://jira.part-kom.ru/browse/XX-2859 15.07.2019
	Запрос.УстановитьПараметр("СкладОтправитель", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СкладПолучатель"));
	//Конец Валиахметов http://jira.part-kom.ru/browse/XX-2859 15.07.2019
	Результаты = Запрос.ВыполнитьПакет();
