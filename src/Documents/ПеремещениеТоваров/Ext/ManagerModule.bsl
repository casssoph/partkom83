...
Процедура ЗагрузитьРезультатРазмещенияПерем(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно, вхПараметры)
	...
	РеквизитыОснования 			= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "СкладОтправитель.Возвраты, ВидОперации");
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		РазмещениеВЗонуВозвратов = РеквизитыОснования.СкладОтправительВозвраты 
										И НЕ РеквизитыОснования.ВидОперации = Перечисления.ВидыОперацийПеремещенияТоваров.ВозвратВПродажу;
	КонецЕсли;	
...
Процедура ЗагрузитьРезультатСборки(ОбъектXDTO, вхПараметры)
	...
	Если НужноМенятьШапку Или НужноМенятьТЧ Или НужноМенятьТЧ2 Тогда
		...
		ДокОбъект.СтатусДокумента = Справочники.СтатусыДокументов.ПеремещениеТоваровПоступил;
		ДокОбъект.ДополнительныеСвойства.Вставить("ОтключитьКонтрольОстатков");
		ДокОбъект.ДополнительныеСвойства.Вставить("НеПерепроводитьПартии");
