...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Иначе
		...
		Запрос.Текст =  "ВЫБРАТЬ
		                ...
		                |			ИЛИ ЕСТЬNULL(ПеремещениеТоваровТовары.СтрокаЗаявки.Заявка, &ПустаяЗаявка) <> &ПустаяЗаявка) КАК УчитыватьПоставщика,
...
Процедура КонтрольОстатков(вхСсылкаНаДокумент, вхОтказ, вхПараметры) Экспорт
	...
	втТовары = Запрос.Выполнить().Выгрузить();
		...
		Если ОбъектXDTO.Тип().Имя = "РезультатРазмещения" Тогда  
			Попытка
				...
				Для Каждого ЗаказНаПриемкуXDTO Из Заказы Цикл 
					Попытка
						ЗагрузитьРезультатРазмещения(РазмещениеСсылкаТопЛог, ЗаказНаПриемкуXDTO, ОбъектXDTO, ТаблицаСсылок, Успешно);
...
Процедура ЗагрузитьРезультатРазмещения(РазмещениеСсылкаТопЛог, ЗаказНаПриемкуXDTO, ОбъектXDTO, ТаблицаСсылок, Успешно)
	...
	Если ОбменДаннымиКлиентСервер.ЭтоБитаяСсылка(ДокументОснование) Тогда 
		ВызватьИсключение "Загрузка результата размещения. Не найдено " + ?(ЭтоПоступление,"поступление товаров и услуг", "перемещение товаров") + " с guid = " + ЗаказНаПриемкуXDTO.ЗаказСсылка;
	КонецЕсли;
	...
	Если ЭтоПоступление Тогда 	
		ЗагрузитьРезультатРазмещенияПТУ(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно);
	Иначе
		ЗагрузитьРезультатРазмещенияПерем(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗагрузитьРезультатРазмещенияПТУ(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно)
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "ВидОперацииПоступления,Склад,Дата");
	...
	Для Каждого КлючЗначение Из СоответствиеСкладов Цикл 
		...
		Попытка
			...
			Если Не (Товары.Количество() = 0 И ТоварыСПустымиSSID.Количество() = 0) Тогда 
				ВызватьИсключение "Не удалось распределить весь товар";
			КонецЕсли;
...
Процедура ЗагрузитьРезультатРазмещенияПерем(ДокументОснование, ЗаказНаПриемкуXDTO, ОбъектXDTO, РазмещениеСсылкаТопЛог, ТаблицаСсылок, Успешно)
	...
	Для Каждого КлючЗначение Из СоответствиеСкладов Цикл 
		...
		Попытка
			...
			Отбор = Новый Структура("Номенклатура,СтрокаЗаявки"); 
			ОбщегоНазначения.РаспределитьТаблицу1ПоТаблице2(Отбор, Товары, ОстаткиСтрокПриходаПоСтрокамЗаявок, РазмещенныеТовары, "КоличествоПлан");
			
			Отбор = Новый Структура("Номенклатура"); 
			ОбщегоНазначения.РаспределитьТаблицу1ПоТаблице2(Отбор, ТоварыСПустымиSSID, ОстаткиСтрокПриходаПоСтрокамЗаявок, РазмещенныеТовары, "КоличествоПлан");
			
			Отбор = Новый Структура("Номенклатура"); 
			ОбщегоНазначения.РаспределитьТаблицу1ПоТаблице2(Отбор, Товары, ОстаткиСтрокПрихода, РазмещенныеТовары, "КоличествоПлан");
			
			Отбор = Новый Структура("Номенклатура"); 
			ОбщегоНазначения.РаспределитьТаблицу1ПоТаблице2(Отбор, ТоварыСПустымиSSID, ОстаткиСтрокПрихода, РазмещенныеТовары, "КоличествоПлан");
			
			ОчиститьТаблицуСНулевымКол(Товары, "КоличествоПлан");
			ОчиститьТаблицуСНулевымКол(ТоварыСПустымиSSID, "КоличествоПлан");
			ОчиститьТаблицуСНулевымКол(ОстаткиСтрокПрихода, "КоличествоПлан");
			ОчиститьТаблицуСНулевымКол(ОстаткиСтрокПриходаПоСтрокамЗаявок, "КоличествоПлан");
...
Функция ОстаткиСтрокПрихода(ДокументОснование, ДокСсылка)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	втРазмПоступ.Количество - ЕСТЬNULL(втОтказы.Количество, 0) - ЕСТЬNULL(втРазмещено.КоличествоПлан, 0) КАК КоличествоПлан
		               |ИЗ
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	ПеремещениеТоваровТовары.Ссылка.ДокументОснование = &ДокументОснование
		               |	И НЕ ПеремещениеТоваровТовары.Поступление.ПометкаУдаления
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               ...
		               |	втПлюс.СтрокаЗаявки,
		               |	втПлюс.КоличествоПлан - ЕСТЬNULL(втМинус.КоличествоПлан, 0) КАК КоличествоПлан
		               |ИЗ
		               |	втПлюс КАК втПлюс
		               |		ЛЕВОЕ СОЕДИНЕНИЕ втМинус КАК втМинус
		               |		ПО втПлюс.Номенклатура = втМинус.Номенклатура
		               |			И втПлюс.СтрокаПрихода = втМинус.СтрокаПрихода
		               |			И втПлюс.СтрокаЗаявки = втМинус.СтрокаЗаявки";
		Запрос.УстановитьПараметр("ДокументОснование", ДокументОснование);
	КонецЕсли;
	
	ОстаткиСтрокПрихода = Запрос.Выполнить().Выгрузить();
...
Функция ОстаткиСтрокПриходаДляПеремещения(ДокументОснование, ДокСсылка)
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ПО втПлюс.Номенклатура = втМинус.Номенклатура
		               |			И втПлюс.СтрокаПрихода = втМинус.СтрокаПрихода
		               |			И втПлюс.Организация = втМинус.Организация
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               ...
		               |	втПлюсПоСтрокамЗаявок.КоличествоПлан - ЕСТЬNULL(втМинусПоСтрокамЗаявок.КоличествоПлан, 0) КАК КоличествоПлан
		               |ИЗ
		               |	втПлюсПоСтрокамЗаявок КАК втПлюсПоСтрокамЗаявок
		               |		ЛЕВОЕ СОЕДИНЕНИЕ втМинусПоСтрокамЗаявок КАК втМинусПоСтрокамЗаявок
		               |		ПО втПлюсПоСтрокамЗаявок.Номенклатура = втМинусПоСтрокамЗаявок.Номенклатура
		               |			И втПлюсПоСтрокамЗаявок.СтрокаПрихода = втМинусПоСтрокамЗаявок.СтрокаПрихода
		               |			И втПлюсПоСтрокамЗаявок.СтрокаЗаявки = втМинусПоСтрокамЗаявок.СтрокаЗаявки
		               |			И втПлюсПоСтрокамЗаявок.Организация = втМинусПоСтрокамЗаявок.Организация";
	Запрос.УстановитьПараметр("ДокументОснование", ДокументОснование);
	Запрос.УстановитьПараметр("ПустаяЗаявка", Документы.ЗаявкаПокупателя.ПустаяСсылка());	
	...
	СтруктураТаблиц = Новый Структура;
	СтруктураТаблиц.Вставить("ОстаткиСтрокПрихода", Результаты[Результаты.ВГраница() - 1].Выгрузить());
	СтруктураТаблиц.Вставить("ОстаткиСтрокПриходаПоСтрокамЗаявок", Результаты[Результаты.ВГраница()].Выгрузить());
