...
Функция РегистрыНакопления_ЗаявкиПокупателей(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	ЗапросЗаявок = новый Запрос( "ВЫБРАТЬ
	                             ...
	                             |				ТОГДА 0
	                             |			ИНАЧЕ ЕСТЬNULL(ЗаявкиПокупателей.СуммаРегл, 0) / ЗаявкиПокупателей.Количество * ВТДанныеПеремещения.Количество
	                             |		КОНЕЦ) КАК СуммаРегл,
	                             |	СУММА(ВЫБОР
	                             |			КОГДА ЕСТЬNULL(ЗаявкиПокупателей.Количество, 0) = 0
	                             |				ТОГДА 0
	                             |			ИНАЧЕ ЕСТЬNULL(ЗаявкиПокупателей.СуммаУпр, 0) / ЗаявкиПокупателей.Количество * ВТДанныеПеремещения.Количество
	                             |		КОНЕЦ) КАК СуммаУпр,
	                             |	&Период,
