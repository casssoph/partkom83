...
Функция РегистрыНакопления_ЗаявкиПокупателей(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	ЗапросЗаявок = новый Запрос( "ВЫБРАТЬ
	                             ...
	                             |	СУММА(ВТДанныеПеремещения.Количество) КАК Количество,
	                             |	СУММА(ЗаявкиПокупателей.СуммаРегл / ЗаявкиПокупателей.Количество * ВТДанныеПеремещения.Количество) КАК СуммаРегл,
	                             |	СУММА(ЗаявкиПокупателей.СуммаУпр / ЗаявкиПокупателей.Количество * ВТДанныеПеремещения.Количество) КАК СуммаУпр,
	                             |	&Период,
	                             |	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения
	                             |ИЗ
	                             |	РегистрНакопления.ЗаявкиПокупателей КАК ЗаявкиПокупателей
	                             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеПеремещения КАК ВТДанныеПеремещения
	...
	ЗапросЗаявок.УстановитьПараметр("Период",ОбщегоНазначения.ЗначениеРеквизитаОбъекта(вхСсылкаНаДокумент,"Дата"));
...
Функция РегистрыНакопления_РезервыТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	Запрос = Новый Запрос;
	Если РаботаСоСтатусамиДокументовСервер.НоваяСхемаЗакрытияЗаявок(Реквизиты.Дата) тогда 
		Запрос.Текст = "ВЫБРАТЬ
		...
		|	ПеремещениеТоваровТовары.СтрокаЗаявки,
		|	ПеремещениеТоваровТовары.СтрокаПрихода,
		|	ПеремещениеТоваровТовары.Количество - ЕСТЬNULL(ОтказыПоЗаявкамОбороты.КоличествоОборот, 0) КАК Количество
		|ИЗ
		|	Документ.ПеремещениеТоваров.Товары КАК ПеремещениеТоваровТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОтказыПоЗаявкам.Обороты(, &ДатаДокумента, , ) КАК ОтказыПоЗаявкамОбороты
		|		ПО ПеремещениеТоваровТовары.СтрокаЗаявки = ОтказыПоЗаявкамОбороты.СтрокаЗаявки
		|ГДЕ
		|	ПеремещениеТоваровТовары.Ссылка = &Ссылка
		|	И ПеремещениеТоваровТовары.СтрокаЗаявки <> &ПустаяСтрокаЗаявки
		|	И ПеремещениеТоваровТовары.Количество > 0
...
Функция ОстаткиСтрокПриходаДляПеремещения(ДокументОснование, ДокСсылка)
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	втПлюс.КоличествоПлан КАК КоличествоПлан
		               |ИЗ
		               |	втПлюс КАК втПлюс
		               |;
		               |
		               ...
		               |	втПлюсПоСтрокамЗаявок.КоличествоПлан КАК КоличествоПлан
		               |ИЗ
		               ...
		               |	втМинусПоСтрокамЗаявок.КоличествоПлан
		               |ИЗ
	...
	ВыходнаяТаблица2.Колонки.Добавить("КоличествоПлан", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15,3)));
	
	Отбор = Новый Структура("Номенклатура,Организация,СтрокаЗаявки,СтрокаПрихода");
	ОбщегоНазначения.РаспределитьТаблицу1ПоТаблице2(Отбор,РазмещеноСтрокамЗаявок, ОстаткиСтрокПриходаПоСтрокамЗаявок, ВыходнаяТаблица2, "КоличествоПлан");
