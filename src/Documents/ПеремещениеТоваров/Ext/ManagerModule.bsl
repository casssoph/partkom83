...
Функция РегистрыНакопления_ЗаявкиПокупателей(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	ЗапросЗаявок = новый Запрос( "ВЫБРАТЬ
	                             ...
	                             |	СУММА(ВТДанныеПеремещения.Количество - ЕСТЬNULL(ОтказыПоЗаявкамОбороты.КоличествоОборот, 0)) КАК Количество,
	                             |	СУММА(ВЫБОР
	                             ...
	                             |			И (ЗаявкиПокупателей.Период <= &Период)
	                             |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОтказыПоЗаявкам.Обороты(, &Период, , ) КАК ОтказыПоЗаявкамОбороты
	                             |		ПО (ВТДанныеПеремещения.СтрокаЗаявки = ЗаявкиПокупателей.СтрокаЗаявки)
	                             |
	                             |СГРУППИРОВАТЬ ПО
	                             ...
	                             |
	                             |ИМЕЮЩИЕ
	                             |	СУММА(ВТДанныеПеремещения.Количество - ЕСТЬNULL(ОтказыПоЗаявкамОбороты.КоличествоОборот, 0)) > 0" );
