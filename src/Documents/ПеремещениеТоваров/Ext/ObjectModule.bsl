	...
	#Если Клиент Тогда 
		...
		Если СтрНачинаетсяС(СкладОтправитель.Наименование, "Приемка") Или СтрНачинаетсяС(СкладОтправитель.Наименование, "Товар в пути") Тогда 
			Если Не ЗначениеЗаполнено(ДокументОснование) Тогда 
				...
				Отказ = Истина;
			ИначеЕсли ЭтоНовый() Тогда // Только для новых
				Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
					Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "Склад.ОбменСTopLog,флНеВыгружатьВТопЛог"); 
					Если Реквизиты.СкладОбменСTopLog И Не Реквизиты.флНеВыгружатьВТопЛог И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СкладПолучатель, "Заблокирован") Тогда 
						Сообщить("Документ-основание участвует в обмене с ТопЛог. Запрещено ручное создание перемещений на незаблокированные склады");
						Отказ = Истина;
					КонецЕсли;
				ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПеремещениеТоваров")  
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ВыгруженВТопЛог")
					И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СкладПолучатель, "Заблокирован") Тогда 	
						Сообщить("Документ-основание выгружен в ТопЛог. Запрещено ручное создание перемещений на незаблокированные склады");
						Отказ = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ДополнительныеСвойства.Свойство("ОтключитьКонтрольОстатков") Тогда 
		лПараметры = Новый Структура("ОтключитьКонтрольОстатков");
	КонецЕсли;
