...
Процедура СписаниеПартий(СтрокаТовары, СтруктураПараметров)
	...
	ОтборПоОрганизацииСтратегия = ПроведениеДокументовКлиентСервер.СтратегияПогашенияПоОрганизации(СтруктураПараметров.Регистратор, СтруктураПараметров.Период);
...
Функция  СоздатьДокументыНаОснованииЗаявки(ДанныеЗаполнения, ПараметрыСоздания) Экспорт
	...
	РезультатУслуги = ОстаткиУслугПоЗаявкеКорректировке(ДокументЗаполнения, Склад, ДатаРТУ);
	...
	Для каждого СтрокаТаб Из ОстаткиТоваров Цикл
		...
		Если СтрокаТаб.ДопустимоеЧислоДнейЗадолженностиОУ > 0 И ЗначениеЗаполнено(СтрокаТаб.ВидРасчетаДнейОУ) Тогда
			СтрокаТаб.ДатаОплаты = УправлениеВзаиморасчетами.ПолучитьДатуОплатыПоДатеДокумента(ДатаОплатыОбщая, СтрокаТаб.ДопустимоеЧислоДнейЗадолженностиОУ, СтрокаТаб.ВидРасчетаДнейОУ);
		КонецЕсли;
	КонецЦикла;
...
Функция ОстаткиУслугПоЗаявкеКорректировке(ДокументЗаполнения, Склад, ДатаОстатков)
...
Процедура СоздатьДокументНаОснованииЗаявки_ЗаполнитьТовары(ОбъектРТУ, Товары)
	...
	Для каждого СтрокаТоваров Из Товары Цикл
		...
		Если Не ОбъектРТУ.Организация.УчитыватьНДС Тогда 
		...
		Если ИспользоватьЦенуСоСкидкой И СтрокаТоваров.ЦенаСоСкидкой <> 0 Тогда
			...
			ОбработкаТабличныхЧастей.ПересчитатьСтрокуТабличнойЧасти(СтрокаТоваров, СтруктураДействий, мКэшированныеЗначения); 
...
Процедура СоздатьДокументНаОснованииЗаявки_ЗаполнитьУслуги(ОбъектРТУ, Услуги)
...
Функция ДатаОплатыДляДокумента(ДокументОбъект) Экспорт
	...
	//Добавим отсрочку по особым условиям
	ДопустимоеЧислоДнейЗадолженностиОУ = Справочники.ДоговорыКонтрагентов.ДопустимоеЧислоДнейЗадолженностиОсобыеУсловия(ДокументОбъект.ДоговорКонтрагента, ДокументОбъект.Товары.Выгрузить().Скопировать(,"Номенклатура, СтрокаЗаявки"), ДокументОбъект.Дата);
	Если ДопустимоеЧислоДнейЗадолженностиОУ.ДопустимоеЧислоДнейЗадолженности <> 0 И ЗначениеЗаполнено(ДопустимоеЧислоДнейЗадолженностиОУ.ВидРасчетаДней) Тогда
		ДатаОплаты = УправлениеВзаиморасчетами.ПолучитьДатуОплатыПоДатеДокумента(ДатаОплаты, 
		ДопустимоеЧислоДнейЗадолженностиОУ.ДопустимоеЧислоДнейЗадолженности, 
		ДопустимоеЧислоДнейЗадолженностиОУ.ВидРасчетаДней);
	КонецЕсли;
