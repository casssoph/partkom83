...
Функция РегистрыНакопления_ТоварыНаСкладах(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ИЛИ РеализацияТоваровУслугТовары.Ссылка.СтатусДокумента = &СтатусУпакован
	               |				ИЛИ РеализацияТоваровУслугТовары.Ссылка.СтатусДокумента = &СтатусГотовКВыдаче
	...
	Запрос.УстановитьПараметр("СтатусУпакован", Справочники.СтатусыДокументов.РеализацияТоваровУслугУпакован);
	Запрос.УстановитьПараметр("СтатусГотовКВыдаче", Справочники.СтатусыДокументов.РеализацияТоваровУслугГотовКВыдаче);
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена, ВыгружаемыеОбъекты = Неопределено) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog 
		...
		|	НЕ РеализацияТоваровУслуг.Проведен КАК Невыгружать_Непроведен,
		|	РеализацияТоваровУслуг.ЭтоМФП КАК Невыгружать_ЭтоМФП,
		|	РеализацияТоваровУслуг.флНеВыгружатьВТопЛог КАК Невыгружать_НеВыгружатьВТопЛог,
		|	РеализацияТоваровУслуг.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.РеализацияТоваровУслугНовый)
		|		ИЛИ РеализацияТоваровУслуг.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ПустаяСсылка) КАК Невыгружать_НеверныйСтатус,
		|	НЕ РеализацияТоваровУслуг.Склад.ОбменСTopLog КАК Невыгружать_СкладНеОбмениваетсяСТопЛог
		|ИЗ
		|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
		...
		|	МАКСИМУМ(Невыгружать_НеверныйСтатус),
...
Функция ЗафиксированаОшибка(ВыборкаИзРезультатаЗапроса) 
	...
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_НеверныйСтатус Тогда 
		МассивСообщенийОбОшибке.Добавить("документ можно выгружать в ТопЛог в статусах ""Сборка"", ""Упакован"", ""Готов к выдаче"", ""Отгружен""");	
	КонецЕсли;
	Если МассивСообщенийОбОшибке.Количество() > 0 Тогда 
		...
		КонецЕсли;
		ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(ВыборкаИзРезультатаЗапроса.Ссылка,, УправлениеСвойствамиПовтИсп.ЗаказНаОтгрузку(), Ошибка, СообщениеОбОшибке);
	КонецЕсли;
	Возврат Ошибка;
КонецФункции
...
// - Пушкин 20190730			
	...
	Если НужноМенятьШапку Или НужноМенятьТЧ Тогда 
		...
		КонецЦикла;
			...
			ПоменятьInit = Ложь;
// + Пушкин 20190730					
			Если ДокОбъект.ТипДоставки = Справочники.ТипыДоставки.ЭкспрессДоставка И 
				ДокОбъект.ЭД_ВидОплаты = Перечисления.ВидыОплатыЭД.ВЦенеТовара Тогда
			ИначеЕсли ДокОбъект.ТипДоставки = Справочники.ТипыДоставки.ЭкспрессДоставка 
				И ДокОбъект.ЭД_ВидОплаты = Перечисления.ВидыОплатыЭД.Бонусы Тогда
			Иначе
// - Пушкин 20190730		
