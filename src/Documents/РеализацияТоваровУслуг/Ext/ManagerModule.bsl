...
Функция РегистрыНакопления_ПартииТоваров(вхСсылкаНаДокумент, вхОтказ, вхФильтр, вхПараметры = Неопределено) Экспорт 
	...
	Если РеквизитыСклада.СкладVMI Тогда
		...
		ТаблицаСписания.ЗаполнитьЗначения(РеквизитыСклада.ФизическийСклад, 			"Склад");
		...
		//Теперь спишем непустая строки прихода с подбором партий по FIFO
...
Процедура СписаниеПартий(СтрокаТовары, СтруктураПараметров)
	...
	Для каждого СтрокаПартии Из СтрокиПартий Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаДвижений, СтрокаТовары, "ОприходоватьПоVMI, НомерСтрокиВДокументе, ВнутреннееПеремещение");
...
Процедура КонтрольОстатков(вхСсылкаНаДокумент, вхОтказ, вхПараметры) Экспорт
	...
	Запрос.Текст =  Запрос.Текст + " ;" + "ВЫБРАТЬ
	                                      ...
	                                      |				ИЛИ (Склад, Номенклатура, Качество) В
	                                      |					(ВЫБРАТЬ
	                                      |						&ДопСклад,
	                                      |						втТоварыСвернутая.Номенклатура,
	                                      |						втТоварыСвернутая.Качество
...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |			&Дата,
					   |			(ВЫРАЗИТЬ(ПрайсПоставщика КАК Справочник.ПрайсыПоставщиковVMI).Владелец, Номенклатура.Артикул, Номенклатура.Изготовитель) В
					   |					(ВЫБРАТЬ
					   |						втТовары.Поставщик,
					   |						втТовары.Артикул,
					   ...
					   |	ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.Артикул,
					   |	ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.Изготовитель
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   ...
					   |           ПО втТовары.Артикул = ЦеныКонтрагентов.Артикул
					   |           		И втТовары.Изготовитель = ЦеныКонтрагентов.Изготовитель
					   ...
					   |		ПО (РеализацияТоваровУслугТовары.КлючСвязи = втПеремещение.КлючСвязи)
					   |    		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныКонтрагентов КАК ЦеныКонтрагентов
					   |          		 ПО РеализацияТоваровУслугТовары.Номенклатура.Артикул = ЦеныКонтрагентов.Артикул
					   |             		 И РеализацияТоваровУслугТовары.Номенклатура.Изготовитель = ЦеныКонтрагентов.Изготовитель
