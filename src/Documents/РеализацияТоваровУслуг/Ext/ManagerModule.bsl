...
Процедура ЗагрузитьРезультатСборки(ОбъектXDTO, допПараметры = Неопределено, СтруктураОтчета = Неопределено)
	...
	Если Реквизиты.Проведен И Реквизиты.СтатусДокумента = Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен И Не РазрешенаПовторнаяВыгрузкаРТУ Тогда 
		...
		ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(ДокСсылка, ОбъектXDTO,,,"Реализация товаров и услуг с guid = " + ОбъектXDTO.ЗаказСсылка + " уже проведена и отгружена");
		// 05.04.19 Строганов Роман <

		// 25.03.19 Строганов Роман > 
		ОбменДаннымиКлиентСервер.ЗафиксироватьРезультатВСтруктуреОтчета(СтруктураОтчета, ДокСсылка, Истина, Ложь, "Реализация товаров и услуг с guid = " + ОбъектXDTO.ЗаказСсылка + " уже проведена и отгружена");
		// 25.03.19 Строганов Роман <
	...
	ДельтаУслуги = Неопределено;
	//НужноМенятьТЧУслуги  = ФормированиеСлужебныхЗаданийКлиентСервер.УслугиXDTOиДокументаРазличаются(ОбъектXDTO, ДокСсылка, ДельтаУслуги);
	...
	Если НужноМенятьШапку Или НужноМенятьТЧ Тогда 
		...
		КонецЦикла;
			...
			Попытка
				...
				//+УСЛУГИ
				Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2697) 
					И ДокОбъект.ТипДоставки = Справочники.ТипыДоставки.ЭкспрессДоставка 
					И ЗначениеЗаполнено(ДокОбъект.express_delivery_request_id) 
					И ПоменятьInit Тогда
