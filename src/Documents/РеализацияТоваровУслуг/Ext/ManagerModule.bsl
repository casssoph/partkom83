	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		...
		Для Каждого СтрокаТЧ Из ЭтотОбъект.ПричиныОтказов Цикл 
			Если (СтрокаТЧ.ПричинаОтмены = Справочники.СостоянияСтрокДокументов.НеНайденоПриСборке Или 
				СтрокаТЧ.ПричинаОтмены = Справочники.СостоянияСтрокДокументов.ВыявленБракПриСборкеНаСкладеПартком Или СкладVMI) И Не ЗначениеЗаполнено(СтрокаТЧ.СкладПеремещения) Тогда 
