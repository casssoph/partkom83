...
Процедура ВыполнитьПроведение(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если ПроведениеДокументовКлиентСервер.ПроводитсяПо(вхПараметры, "ПартииТоваров") 
		...
		//Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "СкладVMI") Тогда
		//	Структура = РегистрыНакопления_ПартииТоваров(вхСсылкаНаДокумент, вхОтказ, лФильтр, вхПараметры); //В разработке
		//Иначе
...
Функция РегистрыНакопления_ПартииТоваров(вхСсылкаНаДокумент, вхОтказ, вхФильтр, вхПараметры = Неопределено) Экспорт 
	...
	СтруктураПараметров.Вставить("КонтрольНаличияПартий", Ложь);
	...
	Если Реквизиты.Склад.СкладVMI Тогда
		
		ТаблицаСписания.ЗаполнитьЗначения(Реквизиты.Склад.ФизическийСклад, "Склад");
		ТаблицаСписания.ЗаполнитьЗначения(Ложь, "УчитыватьПоставщика");
		ТаблицаСписания.ЗаполнитьЗначения(Ложь, "ПоЦенеЗакупки");
		...
		СтруктураПараметров.Вставить("КонтрольНаличияПартий", Истина);
		СтруктураПараметров.Вставить("ТаблицаОстатковПартий", ТаблицаОстатковПартий);
	...
	//Делаем перемещение
	Для каждого СтрокаДвижений ИЗ СтруктураПараметров.ТаблицаДвижений Цикл
		Если СтрокаДвижений.ВидСписания = "Перемещение" Тогда 
			СтрокаДвиженийПриход = ТаблицаДвижений.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДвиженийПриход, СтрокаДвижений);
			СтрокаДвиженийПриход.Склад = СтрокаДвижений.СкладПеремещения;
			СтрокаДвиженийПриход.Качество = СтрокаДвижений.КачествоПеремещения;
			СтрокаДвиженийПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
			СтрокаДвиженийПриход.СтатусПартии = СтрокаДвижений.СтатусПартииПеремещения;	
		КонецЕсли;
	КонецЦикла;
	
	//Делаем ОприходоватьПоVMI
...
Функция ТаблицаОстатковПартий(вхСсылкаНаДокумент, ТаблицаДвижений, ТаблицаСписания)
	...
	КолонкиРаспределения = "Номенклатура, Склад, Качество, СтрокаПрихода, СтатусПартии, Поставщик "+?(ОтборПоОрганизации, ",Организация",""); 
	...
	ТаблицаОстатковПартий.Индексы.Добавить(КолонкиРаспределения);
...
Процедура СписаниеПартий(СтрокаТовары, СтруктураПараметров)
	...
	Если ЗначениеЗаполнено(СтрокаТовары.СтрокаПрихода) Тогда
		СтруктураИзмерений.Вставить("СтрокаПрихода");	
	КонецЕсли;
	
	СтруктураИзмерений.Удалить("СтатусПартии");
	Если НЕ СтрокаТовары.Склад.ПриРазмещенииИгнорироватьСтатусПартии Тогда
	...
	Для каждого СтрокаПартии Из СтрокиПартий Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаДвижений, СтрокаТовары, "ОприходоватьПоVMI, НомерСтрокиВДокументе");
	...
	Если СтруктураПараметров.КонтрольНаличияПартий И КоличествоРаспределить > 0 Тогда
		...
		| Списание по всем орг.: " + Формат(Не СтруктураПараметров.ОтборПоОрганизации, "БЛ=Нет; БИ=Да") + "
...
Процедура КонтрольОстатков(вхСсылкаНаДокумент, вхОтказ, вхПараметры) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "СтатусДокумента,ЭтоМФП,Дата");
...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |	И ВЫБОР
					   |			КОГДА РеализацияТоваровУслугТовары.Ссылка.СтатусДокумента = &СтатусДокументаОтгружен
					   |				ТОГДА РеализацияТоваровУслугТовары.Количество
					   |			ИНАЧЕ РеализацияТоваровУслугТовары.КоличествоПлан
					   |		КОНЕЦ > 0
					   ...
					   |			&Дата,
					   |			ВЫРАЗИТЬ(ПрайсПоставщика КАК Справочник.ПрайсыПоставщиковVMI).Владелец  В
					   |					(ВЫБРАТЬ
					   |						втТовары.Поставщик
					   |					ИЗ
					   |						втТовары)
					   ...
					   |    втТовары.Склад.СкладVMI КАК ПоЦенеЗакупки,
					   |    isnull(ЦеныКонтрагентов.Цена, 0) КАК ЦенаЗакупки
		               |ИЗ
		               |	втТовары КАК втТовары
					   ...
					   |    ИСТИНА,
					   |	ИСТИНА,
					   |    РеализацияТоваровУслугТовары.Ссылка.Склад.СкладVMI КАК ПоЦенеЗакупки,
					   |    isnull(ЦеныКонтрагентов.Цена, 0) КАК ЦенаЗакупки
					   |ИЗ
					   |	втПеремещение КАК втПеремещение
					   |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
		...
		Попытка
			...
			Если НеобходимаЗагрузка Тогда 
				...
				Если Не ЗначениеЗаполнено(ДокОбъект.Филиал) Тогда 
					ДокОбъект.Филиал = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.Склад, "Филиал");
				КонецЕсли;
