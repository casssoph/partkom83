	...
	Если СтатусДокумента = Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен Тогда 
		Если РеализацииВыгружаютсяИз77 Тогда 
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |	И (РеализацияТоваровУслугПричиныОтказов.Ссылка.Склад.СкладVMI
...
Процедура ЗаполнитьСкладыДляПеремещения(ЭтотОбъект, РежимЗаписи) Экспорт
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		...
		Если Не СкладVMI Тогда 
			...
			КонецЕсли;
		Иначе
			СкладПеремещения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтотОбъект.Склад, "ФизическийСклад");
		КонецЕсли;
...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	РеализацияТоваровУслугТовары.Ссылка.Склад.СкладVMI КАК ОприходоватьПоVMI,
		               |	РеализацияТоваровУслугТовары.Ссылка.Склад.ПриРазмещенииИгнорироватьСтатусПартии КАК ИгнорироватьСтатусПартии,
		               |	ВЫБОР
		               ...
		               |	РеализацияТоваровУслугТовары.Ссылка.Склад.СкладVMI КАК УчитыватьПоставщика,
		               |	ВЫБОР
		               |		КОГДА РеализацияТоваровУслугТовары.Ссылка.Склад.СкладVMI
		...
		Для Каждого СтрокаДельты Из Дельта Цикл
			...
			Если СтрокаДельты.Недостача > 0 Тогда //добавить недостачу
				...
				Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.Склад, "СкладVMI") Тогда 
					СтрокаОтказа.СкладПеремещения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.Склад, "ФизическийСклад");
				...
				Иначе	
					...
					Если Не ЗначениеЗаполнено(СтрокаОтказа.СкладПеремещения) Тогда 
						ВызватьИсключение "Для склада " + ДокОбъект.Склад + " не указан физический склад"; 
					КонецЕсли;
				КонецЕсли;
		...
		Попытка
			...
			Если НеобходимаЗагрузка Тогда 
				...
				Если Не ЗначениеЗаполнено(ДокОбъект.БанковскийСчетОрганизации) Тогда 
					ДокОбъект.БанковскийСчетОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.Организация, "ОсновнойБанковскийСчет");
				КонецЕсли;	
