...
Процедура ЗагрузитьЗаказНаОтгрузкуСтатус(ОбъектXDTO, допПараметры = Неопределено, СтруктураОтчета = Неопределено)
	...
	Если ДокОбъект.Проведен И (ЗаказСтатус = "Упакован" И мЧисловоеЗначСтатуса >=2 Или ЗаказСтатус = "Готов к выдаче" И мЧисловоеЗначСтатуса >=3) Тогда  
		...
		ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(ДокОбъект.Ссылка, ОбъектXDTO,,,"Повторно не загружаем");		
...
Процедура ЗагрузитьТранзитнуюОтгрузкуВНесколькоРТУ(Строки, ТоварыТреб, ОбъектXDTO, СтрокаТЧ, ДопПараметры)
	...
	Для Каждого Строка Из Строки  Цикл 
		...
		Исключение				
			...
			//}}
			//Семенов И.П. 06.02.2019 XX-1768(
			ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(ДокОбъект.Ссылка, ОбъектXDTO,,Истина,"Не удалось провести, помещен в отложенные: "+ОписаниеОшибки);
			//)Семенов И.П.
			//ХудинВВ 13112018+
			Если СтрНайти(ОписаниеОшибки, "[ПогашениеПартийТоваров]") > 0 
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.Контрагент, "ПокупательИзДрБазы") = Истина Тогда
