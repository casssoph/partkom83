...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |		КОГДА Ссылка.Склад.СкладVMI И &ПеремещатьНаФизическийСкладВсеОтказыVMI
					   |			ТОГДА Истина
					   |		ИНАЧЕ РеализацияТоваровУслугПричиныОтказов.ПричинаОтмены В (&МассивОтказов)
					   |	КОНЕЦ 
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	РеализацияТоваровУслугПричиныОтказов.КлючСвязи,
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog 
		...
		|	ВЫБОР
		|		КОГДА РеализацияТоваровУслуг.ДокументОснование ССЫЛКА Документ.ЗаявкаПокупателя
		|			ТОГДА ЕСТЬNULL(РеализацияТоваровУслуг.ДокументОснование.НомерРозничнойЗаявки, """")
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК НомерРозничнойЗаявки
		|ИЗ
		|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
...
Процедура ДобавитьВОчередьОтправкиДокументовОтгрузки(СсылкаНаДокумент) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент, "СтатусДокумента, Контрагент, Контрагент.ОсновнаяТорговаяТочка.АвтоотправкаОтгрузочныхДокументов");
	
	Если ЗначениеЗаполнено(ТестовыйКонтрагент) Тогда
		//Ветка для тестовой проверки
		Если Реквизиты.Контрагент = ТестовыйКонтрагент
			И Реквизиты.СтатусДокумента = Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен
			И Реквизиты.КонтрагентОсновнаяТорговаяТочкаАвтоотправкаОтгрузочныхДокументов Тогда
			РегистрыСведений.ОчередьОтправкиДокументовОтгрузки.ДобавитьКОтправке(СсылкаНаДокумент);
		КонецЕсли;
	Иначе
