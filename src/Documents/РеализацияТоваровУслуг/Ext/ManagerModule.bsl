...
Процедура ПроверитьВозможностьВыпискиКлиенту(вхСсылкаНаДокумент, вхОтказ, вхПараметры)
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ЕСТЬNULL(втТоварыКОтгрузке.Количество, 0) + втСтрокиЗаявок.Количество - ЕСТЬNULL(втЗаявки.Количество, 0) КАК Количество
	                |ИЗ
	                ...
	                |ГДЕ
	                |	ЕСТЬNULL(втЗаявки.Количество, 0) - ЕСТЬNULL(втТоварыКОтгрузке.Количество, 0) - втСтрокиЗаявок.Количество < 0";
