	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog) Тогда 
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ЗаказНаОтгрузкуСтатус" Тогда	
			...
			Исключение
				...
				ДокСсылка = Документы.РеализацияТоваровУслуг.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.ЗаказСсылка));
				Если НЕ ОбменДаннымиКлиентСервер.ЭтоБитаяСсылка(ДокСсылка) Тогда 
					РегистрыСведений.ИсторияОбменаСТопЛогПоОбъектам.Добавить(ДокСсылка, вхПараметры.НомерСообщения, Истина, "Ошибка загрузки статуса Упакован или Готов к выдаче: "+ОписаниеОшибки(), , Ложь); 
				КонецЕсли;
				//}}
			КонецПопытки;
		Иначе
...
Процедура ЗагрузитьЗаказНаОтгрузкуСтатус(ОбъектXDTO, допПараметры = Неопределено)
	...
	Для Каждого СтрокаТоварыXDTO Из ТоварыXDTO Цикл 
		...
		НоваяСтрока.СтрокаЗаявки = ОбменДаннымиКлиентСервер.НайтиСтрокуЗаявкиВСоответствии(СоотвSSID, СтрокаТоварыXDTO.SSID);
		Если ЗначениеЗаполнено(СтрокаТоварыXDTO.SSID) И Не ЗначениеЗаполнено(НоваяСтрока.СтрокаЗаявки) Тогда 
			ВызватьИсключение "Не найдена строка заявки с IDSite = " + СтрокаТоварыXDTO.SSID;
		КонецЕсли;
	...
	Для Каждого СтрокаТовары Из Товары Цикл 
		...
		Если ВремТаблица.Итог("КоличествоПлан") <> СтрокаТовары.План Тогда 
		...
		Пока КоличествоРаспределить > 0 И Индекс < Строки.Количество() Цикл 
			...
			СписывамоеКоличество = Мин(КоличествоРаспределить, СтрокаТЧ.КоличествоПлан - СтрокаТЧ.Количество);
