...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//PaSe - подстраховка, включить при обновлении//
	//КонтрольБлокировокКонтрагента(РежимЗаписи);
...
Процедура КонтрольБлокировокКонтрагента(РежимЗаписи)
	
	Если	РежимЗаписи = РежимЗаписиДокумента.Проведение И
			РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Блокировки контрагентов", "Контроль лимитов", Ложь) Тогда
			
			ТекстОшибки = "";
			ЕстьБлокировки = Ложь;
			НетПравПревышенияЛимита = НЕ НастройкаПравДоступа.ПолучитьЗначениеПраваПользователя(УправлениеПользователями.ОпределитьТекущегоПользователя(), ПланыВидовХарактеристик.ПраваПользователей.РазрешитьИзменятьПриостановкуОтгрузки);
			...
			Если ЕстьБлокировки Тогда
				...
				#Иначе
					РассылкаСообщенийОбОшибках.ОтправитьЭлектронноеСообщениеБезСохранения(Справочники.СобытияДляОтправкиЭлектронныхПисем.БлокировкаОтгрузки, ТекстОшибки, "Ошибка проведения реализации: " + Номер + " от " + Дата);
				#КонецЕсли
...
Функция ДанныеПревышенияКредита(Договор, СуммаДокумента, Дата)
	...
	Если НЕ Договор.НеКонтролироватьЛимит Тогда
