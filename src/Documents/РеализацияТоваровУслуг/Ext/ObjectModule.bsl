...
Процедура ПроверитьНаличиеИзмененийVMI(Отказ, РежимЗаписи, РежимПроведения)
	...
	Шапка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Склад, Проведен");
	...
	Запрос.Текст = 
		...
		|			ИЛИ ЕСТЬNULL(ТЧ.Количество, 0) <> ЕСТЬNULL(ТЧСсылки.Количество, 0))";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если Не РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = ТекстОшибки+?(ТекстОшибки = "","",Символы.ПС)+"Изменение количества в табличной части проведенного документа запрещено!";
	КонецЕсли;
