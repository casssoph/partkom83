...
Функция ПечатьУниверсальнойФормы(ВыводитьКомментарийССайта = Истина) Экспорт
	...
	|		NULL                                                        КАК Характеристика,
	|		NULL                                                        КАК Серия,
	|		РеализацияТоваровУслуг.СтрокаПрихода.СтранаПроисхождения.Код КАК СтранаПроисхожденияКод,
	|		РеализацияТоваровУслуг.СтрокаПрихода.СтранаПроисхождения.Наименование КАК ПредставлениеСтраны,
	...
	|СГРУППИРОВАТЬ ПО
	|		РеализацияТоваровУслуг.Номенклатура,
	|		РеализацияТоваровУслуг.СтрокаПрихода,
	|       ЕСТЬNULL(ЗаявкаПокупателя.КомментарийИзСайта, """"),
	|		РеализацияТоваровУслуг.Коэффициент,
	|		РеализацияТоваровУслуг.ЕдиницаИзмерения,
	|		РеализацияТоваровУслуг.СтавкаНДС,
	...
	Для Каждого ВыборкаСтрок Из ЗапросТовары Цикл
		...
		Иначе
			...
			Если Ном <> 1 И НЕ ТабДокумент.ПроверитьВывод(МассивВыводимыхОбластей) Тогда
				...
				ТабДокумент.Вывести(ОбластьШапка);
				Если ВыводитьКомментарийССайта Тогда
					ОбластьШапкаКомментарий.Параметры.КомментарийСайта = ВыборкаСтрок.КомментарийСайта;
					ТабДокумент.Присоединить(ОбластьШапкаКомментарий);
				КонецЕсли;
