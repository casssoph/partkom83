...
Функция ПечатьТоварногоЧека(Макет = Неопределено) Экспорт
	...
	ОбластьМакета.Параметры.НомДок= Шапка.Номер;
	ОбластьМакета.Параметры.ШК=ОбщегоНазначения.СформироватьШК_дляДок(ЭтотОбъект.Ссылка);
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПолучитьПараметрыПечатиСчетаЗаказа() Экспорт
	...
	Если ЗначениеЗаполнено(БанковскийСчетОрганизации) Тогда
		Банк       = ?(Не ЗначениеЗаполнено(БанковскийСчетОрганизации.БанкДляРасчетов), БанковскийСчетОрганизации.Банк, БанковскийСчетОрганизации.БанкДляРасчетов);
	...
	ПараметрыПечати.Вставить("НомДок", Шапка.Номер);
	ПараметрыПечати.Вставить("ШК", ОбщегоНазначения.СформироватьШК_дляДок(ЭтотОбъект.Ссылка));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПроведениеДокументовКлиентСервер.ОчиститьДвиженияПриСдвигеДаты(ЭтотОбъект, РежимЗаписи, "ТоварыНаСкладах,РезервыТоваров");
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение //И СтатусДокумента = Справочники.СтатусыДокументов.РеализацияТоваровУслугОтгружен Тогда 
		Тогда 
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента, "ДопустимоеЧислоДнейЗадолженности, ВидРасчетаДней");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаявкаПокупателя") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ЕСТЬNULL(ЗаявкаПокупателя.Контрагент.Блокировка_Отгрузок_Дата < &ТекущаяДата
		               |			И ЗаявкаПокупателя.Контрагент.Блокировка_Отгрузок_Дата <> ДАТАВРЕМЯ(1, 1, 1), ЛОЖЬ) КАК БлокировкаКонтрагента,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.Контрагент.ГоловнойКонтрагент.Блокировка_Отгрузок_Дата, ДАТАВРЕМЯ(3999, 12, 31)) < &ТекущаяДата
		               |		И ЕСТЬNULL(ЗаявкаПокупателя.Контрагент.ГоловнойКонтрагент.Блокировка_Отгрузок_Дата, ДАТАВРЕМЯ(3999, 12, 31)) <> ДАТАВРЕМЯ(1, 1, 1) КАК БлокировкаГолКонтрагента,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.ДоговорКонтрагента.ДоговорПриостановлен, ЛОЖЬ) КАК БлокировкаДоговораКонтрагента,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.Контрагент.ГоловнойКонтрагент.ОсновнойДоговорКонтрагента.ДоговорПриостановлен, ЛОЖЬ) КАК БлокировкаДоговораГолКонтрагента,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.ДоговорКонтрагента.ВидОплаты, ЗНАЧЕНИЕ(Перечисление.ВидыДенежныхСредств.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыДенежныхСредств.Наличные)
		               |		ИЛИ ЕСТЬNULL(ЗаявкаПокупателя.ДоговорКонтрагента.НеКонтролироватьЛимит
		               |				ИЛИ ЗаявкаПокупателя.ДоговорКонтрагента.Организация.НеКонтролироватьКредит, ЛОЖЬ) КАК НеКонтролироватьКредит,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.ТорговаяТочка.VIP, ЗаявкаПокупателя.Контрагент.ОсновнаяТорговаяТочка.VIP) КАК VIP,
		               |	ЕСТЬNULL(ЗаявкаПокупателя.ДоговорКонтрагента.ДопустимаяСуммаЗадолженности, 0) КАК ДопустимаяСуммаЗадолженности
		               |ИЗ
		               |	Документ.ЗаявкаПокупателя КАК ЗаявкаПокупателя
		               |ГДЕ
		               |	ЗаявкаПокупателя.Ссылка = &Ссылка";
		Если ТипЗнч(ДокументЗаполнения) = Тип("ДокументСсылка.КорректировкаЗаявкиПокупателя") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "Документ.ЗаявкаПокупателя", "Документ.КорректировкаЗаявкиПокупателя");
		КонецЕсли;
		
		Запрос.УстановитьПараметр("Ссылка", ДокументЗаполнения);
		Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата());
		
		ВыборкаПоДоговору = Запрос.Выполнить().Выбрать();
		ВыборкаПоДоговору.Следующий();
		Если ВыборкаПоДоговору.БлокировкаКонтрагента Тогда 
			Сообщить("Невозможно выписать реализацию. Заблокирован контрагент");
			Возврат;
		ИначеЕсли ВыборкаПоДоговору.БлокировкаДоговораКонтрагента Тогда 
			Сообщить("Невозможно выписать реализацию. Заблокирован договор контрагента");
			Возврат;
		ИначеЕсли ВыборкаПоДоговору.БлокировкаГолКонтрагента Тогда 
			Сообщить("Невозможно выписать реализацию. Основной договор контрагента приостановлен");
			Возврат;
		ИначеЕсли ВыборкаПоДоговору.БлокировкаДоговораГолКонтрагента Тогда 
			Сообщить("Невозможно выписать реализацию. Заблокирован договор головного контрагента");
			Возврат;
		КонецЕсли;
		...
		Если Результат.Пустой() Тогда 
			СуммаДолга = 0;
		Иначе
			ВыборкаПоДолгам = Результат.Выбрать();
			СуммаДолга = ВыборкаПоДолгам.СуммаДолга;
		КонецЕсли;
		
		
		СтандартнаяОбработка = Ложь;
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДокументЗаполнения, , "Номер, Дата, СтатусДокумента");
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				//НоваяСтрока.Цена = НоваяСтрока.Цена + ЦенаНДС;
		...
		Если Не (ВыборкаПоДоговору.VIP Или ВыборкаПоДоговору.НеКонтролироватьКредит) Тогда 
			Если СуммаОтгрузки + СуммаДолга > ВыборкаПоДоговору.ДопустимаяСуммаЗадолженности Тогда 
				Сообщить("Превышен долг контрагента. Допустимая задолженность по договору: " + ВыборкаПоДоговору.ДопустимаяСуммаЗадолженности + ". Планируемая сумма отгрузки: " + СуммаОтгрузки + ". Имеющийся долг: " + СуммаДолга);
				Товары.Очистить();
				Услуги.Очистить();
			КонецЕсли;
		КонецЕсли;
		
	//ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ДокументОснование") И ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.СлужебноеЗадание") Тогда
	//	
	//	СтандартнаяОбработка = Ложь;
	//	
	...
	//	                      |	РезервыТоваровОстатки.КоличествоОстаток - ЕСТЬNULL(ТоварыКОтгрузкеОбороты.КоличествоОборот, 0) КАК КоличествоПлан,
	//	                      |	РезервыТоваровОстатки.Номенклатура.ЕдиницаХраненияОстатков КАК единицаИзмерения,
	//	                      |	РезервыТоваровОстатки.СтрокаЗаявки КАК СтрокаЗаявки,
	//	                      |	&Контрагент КАК Контрагент
	//	                      |ПОМЕСТИТЬ втСтрокиЗаявки
	//	                      |ИЗ
	//	                      |	РегистрНакопления.РезервыТоваров.Остатки(&ТекущаяДата, СтрокаЗаявки В (&СтрокиЗаявки)) КАК РезервыТоваровОстатки
	//	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Обороты(
	//	                      |				НЕОПРЕДЕЛЕНО,
	//	                      |				&ТекущаяДата,
	//	                      |				,
	//	                      |				СтрокаЗаявки В (&СтрокиЗаявки)
	//	                      |					И Статус = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.РеализацияТоваровУслуг)) КАК ТоварыКОтгрузкеОбороты
	//	                      |		ПО РезервыТоваровОстатки.СтрокаЗаявки = ТоварыКОтгрузкеОбороты.СтрокаЗаявки
	//	                      |ГДЕ
	//	                      |	РезервыТоваровОстатки.КоличествоОстаток > ЕСТЬNULL(ТоварыКОтгрузкеОбороты.КоличествоОборот, 0)
	//	                      |
	//	                      |ИНДЕКСИРОВАТЬ ПО
	//	                      |	СтрокаЗаявки
	//	                      |;
	...
	//	                      |	втСтрокиЗаявки.КоличествоПлан,
	//	                      |	втСтрокиЗаявки.единицаИзмерения,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки,
	//	                      |	втСтрокиЗаявки.Контрагент КАК ТорговаяТочка,
	//	                      |	&ТекущаяДата КАК Дата,
	//	                      |	&Склад,
	//	                      |	&ДокументОснование КАК ДокументОснование,
	//	                      |	ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.РеализацияТоваровУслугНовый) КАК Статус,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.БанковскийСчет КАК БанковскийСчет,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.ДоговорКонтрагента КАК ДоговорКонтрагента,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.Организация КАК Организация,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.Филиал КАК Филиал,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.ТипЦен КАК ТипЦен,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.УчитыватьНДС КАК УчитыватьНДС,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.КурсВзаиморасчетов КАК КурсВзаиморасчетов,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.ВалютаДокумента КАК ВалютаДокумента,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.КратностьВзаиморасчетов КАК КратностьВзаиморасчетов,
	//	                      |	втСтрокиЗаявки.КоличествоПлан КАК Количество,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.IDSite КАК IDSite,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.Контрагент КАК Контрагент,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.Заявка.Менеджер КАК Менеджер,
	//	                      |	втСтрокиЗаявки.СтрокаЗаявки.ЦенаЗакупки КАК ЦенаЗакупки
	//	                      |ИЗ
	//	                      |	втСтрокиЗаявки КАК втСтрокиЗаявки");
	//	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата());
	//	Запрос.УстановитьПараметр("Склад", ДанныеЗаполнения.Склад);
	//	Запрос.УстановитьПараметр("Контрагент", ДанныеЗаполнения.ТорговаяТочка);
	//	Запрос.УстановитьПараметр("ДокументОснование", ДанныеЗаполнения.ДокументОснование);
	//	Запрос.УстановитьПараметр("СтрокиЗаявки", ДанныеЗаполнения.Заявки);
	//	
	//	Результат = Запрос.Выполнить();
	//	
	//	Если Результат.Пустой() Тогда
	...
	//	НоваяСтрока = Товары.Добавить();
	//	ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
	//	ПоследнийДокумент = Документы.ЗаявкаПокупателя.ПолучитьПоследнийДокументКорректировки(Выборка.СтрокаЗаявки.Заявка);
	//	_СтрокаЗаявки = ПоследнийДокумент.Товары.Найти(Выборка.СтрокаЗаявки, "СтрокаЗаявки");
	//	Если _СтрокаЗаявки <> Неопределено Тогда
	//		
	//		ЗаполнитьЗначенияСвойств(НоваяСтрока, _СтрокаЗаявки, "Цена, Сумма, ПроцентСкидкиНаценки, СтавкаНДС, СуммаНДС, КомментарийИзСайта, Коэффициент, ЦенаЗакупки");
	//		НоваяСтрока.Сумма = НоваяСтрока.Сумма * НоваяСтрока.Количество / _СтрокаЗаявки.Количество;
	//		НоваяСтрока.СуммаНДС = НоваяСтрока.СуммаНДС * НоваяСтрока.Количество / _СтрокаЗаявки.Количество;
	//		
	//	КонецЕсли;
	//	
	//КонецЦикла;
	...
	//Если ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.СлужебноеЗадание") Тогда
	//	МаршрутДоставки = ДанныеЗаполнения.МаршрутДоставки
	//КонецЕсли;
	
	//Если ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.КорректировкаЗаявкиПокупателя") Тогда
	//	ДокументОснование = Выборка.ДокументОснование.ДокументОснование
	//КонецЕсли;
...
Процедура УстановитьКонтрагентаВзаиморасчетов() Экспорт
	...
	Если ЗначениеЗаполнено(Контрагент.ГоловнойКонтрагент) Тогда
		КонтрагентВзаиморасчетов = Контрагент.ГоловнойКонтрагент;
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |УПОРЯДОЧИТЬ ПО
		                      |	Приоритет");
		Запрос.УстановитьПараметр("ГоловнойКонтрагент", КонтрагентВзаиморасчетов);
		Запрос.УстановитьПараметр("Организация", Организация);
		Выборка = Запрос.Выполнить().Выбрать();
		ДоговорКонтрагентаВзаиморасчетов = ?(Выборка.Следующий(), Выборка.Договор, ДоговорКонтрагента);
	Иначе
