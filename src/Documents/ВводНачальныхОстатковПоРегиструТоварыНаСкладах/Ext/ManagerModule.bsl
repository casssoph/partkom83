Функция РегистрыНакопления_ТоварыНаСкладах(вхСсылкаНаДокумент, вхОтказ, вхПараметры)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Товары.Ссылка.Дата КАК Период,
	|	Товары.Ссылка КАК Регистратор,
	|	ВЫБОР
	|		КОГДА Товары.ВидДвижения = 0
	...
	|	Товары.Количество
	|ИЗ
	|	Документ.ВводНачальныхОстатковПоРегиструТоварыНаСкладах.Товары КАК Товары
	|ГДЕ
...
Функция РегистрыНакопления_ПартииТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры, вхФильтр = Неопределено)
	...
	|	Док.СуммаЕвро,
	|	Док.СуммаБезНДС,
	|	Док.НомерСтроки КАК НомерСтрокиВДокументе
	|ИЗ
