...
Функция СобратьДанныеПоПоступлениюТоваров(ДокОснование)
	...
	ЗапросПоТоварам.Текст = "ВЫБРАТЬ
	                        ...
	                        |		ТаблицаПоТоварам.СтрокаПрихода.СтранаПроисхождения КАК СтранаПроисхождения,
	                        |		ТаблицаПоТоварам.СтрокаПрихода.НомерГТД КАК НомерГТД,
	                        ...
	                        |		ТаблицаПоТоварам.СтрокаПрихода.СтранаПроисхождения.Код КАК СтранаПроисхожденияКод,
	                        ...
	                        |		ТаблицаПоТоварам.СтрокаПрихода.СтранаПроисхождения,
	                        |		ТаблицаПоТоварам.СтрокаПрихода.НомерГТД,
	                        |		НЕопределено,
	                        |		Неопределено,
	                        |		ТаблицаПоТоварам.Сумма,
	                        |		ТаблицаПоТоварам.СуммаНДС,
	                        |		ТаблицаПоТоварам.СтрокаПрихода.СтранаПроисхождения.Код,
...
Функция ПолучитьТекстЗапросаОпределенияПараметровСФ(вхДокументОснование, ПараметрыЗапроса, ИмяВременнойТаблицы = "")
	...
	ИгнорироватьТЧ.Добавить("ПричиныОтказов");
	//ИгнорироватьТЧ.Добавить("Услуги");
	
	МетаданныеДокумента = вхДокументОснование.Метаданные();
	ТипДокументаОснования = ТипЗнч(вхДокументОснование);
	
	ИмяОбъекта = МетаданныеДокумента.Имя;
	Если СтрНайти(ИмяОбъекта,"ПоступленияТоваровУслуг") > 0 Тогда
		ИмяОбъекта = СтрЗаменить(ИмяОбъекта,"ПоступленияТоваровУслуг","Поступления");
	КонецЕсли;
	
	ПараметрыЗапроса.Вставить("ДокументОснование_" + ИмяОбъекта, вхДокументОснование);
	...
	Если ТипДокументаОснования = Тип("ДокументСсылка.ОтчетКомитентуОПродажах")
		ИЛИ ТипДокументаОснования = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
	...
	Если НЕ ЗначениеЗаполнено(ТекстЗапроса) Тогда // в документе не оказалось табличных частей
		...
		Иначе
			...
			Если ТипДокументаОснования = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
...
Процедура ПолучитьПараметрыСчетаФактуры(вхДокументОснование,  Результат)
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаАванс 
		...
		И ТипЗнч(ДокументыОснования[0].ДокументОснование) = Тип("ДокументСсылка.КорректировкаДолга") Тогда
		// Реквизиты заново не определяются, получаются из счета-фактуры
		Результат.Вставить("Организация"	, Организация);
		Результат.Вставить("Контрагент"		, Контрагент);
		Результат.Вставить("Договор"		, ДоговорКонтрагента);
		Результат.Вставить("ВалютаДокумента", ВалютаДокумента);
		Результат.Вставить("СуммаДокумента"	, СуммаДокумента);
		Возврат;
	КонецЕсли;			
		
	ДокиОснования = ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ДокументыОснования.ВыгрузитьКолонку("ДокументОснование"), Истина);
	
	ТипыОснований = Новый Соответствие();
	Для каждого Основание Из ДокиОснования Цикл
	...
	Для каждого ТипОснования Из ТипыОснований Цикл
		...
		ДокиОснования		= ТипОснования.Значение;
		ТекстЗапроса = "";
		...
		Если ТипДокументаОснования = Тип("ДокументСсылка.ВводНачальныхОстатковНДС") Тогда
			...
			Запрос.УстановитьПараметр("ДокументОснованиеВНО", ДокиОснования);
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
			// Для данного документа применяется специфический запрос для расчета параметров счета-фактуры
			Запрос.УстановитьПараметр("ДокументОснованиеАО", ДокиОснования);
			ТекстЗапроса = 
			"ВЫБРАТЬ
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
			...
			// Для данного документа применяется специфический запрос для расчета параметров счета-фактуры
			Запрос.УстановитьПараметр("ДокументыОснования", ДокиОснования);
			ТекстЗапроса =
			"ВЫБРАТЬ
		...
		Иначе
			...
			ТекстЗапроса = ПолучитьТекстЗапросаОпределенияПараметровСФ(вхДокументОснование, Запрос.Параметры);
				...
				Пока ВыборкаДокументы.Следующий() Цикл
					...
					Если ЭтоКорректировочный Тогда
						...
						Если ЗначениеЗаполнено(ИсходныйДокументПоступления) Тогда
							РеквизитыИсходногоСчетаФактуры = УчетНДС.ПолучитьРеквизитыСчетаФактурыПолученного(ИсходныйДокументПоступления);
						КонецЕсли;
...
//
Процедура ОпределениеПараметровСчетаФактуры(вхДокументОснование, ЗаполнятьРеквизитыСчетаФактуры = Ложь) Экспорт
	
	лКлючАлгоритма = "Документ_СчетФактураПолученный_МодульОбъекта_ОпределениеПараметровСчетаФактуры";
	...
	ПолучитьПараметрыСчетаФактуры(вхДокументОснование, ПараметрыСФ);
	...
	Если Дата >= '20150101'   Тогда
		//И НЕ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") 
...
Функция ПолучитьСписокТиповПоВидуСчетаФактуры(ВидСчетаФактуры, Исправление = Ложь) Экспорт
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда
		СписокТипов.Добавить(Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг"));
	Иначе
...
Процедура ЗаполнитьНаОснованииИсправления(Основание, СтрокаОснования, Отказ)
	...
	ДанныеИсходногоДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ИсходныйДокумент, 
		Новый Структура("ВалютаДокумента", 
			"ВалютаДокумента"));
...
Процедура ОбработкаЗаполнения(Основание)
	...
	лКлючАлгоритма = "Документ_СчетФактураПолученный_МодульОбъекта_ОбработкаЗаполнения";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	ИначеЕсли ЗначениеЗаполнено(Основание) Тогда
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") Тогда
			Дата = Основание.ДокументОснование.Дата;
			СтрДокОснования.ДокументОснование = Основание.ДокументОснование;
		Иначе
			Дата = Основание.Дата;
		...
		ИначеЕсли  ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			...
			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "СуммаДокумента, НомерВходящегоДокументаСФ, ДатаВходящегоДокументаСФ, Контрагент.ОсновнаяТорговаяТочка.КПП");
			УстановитьКодВидаОперации();
			ВидСчетаФактуры 			= Перечисления.ВидСчетаФактурыПолученного.НаПоступление;
			ОпределениеПараметровСчетаФактуры(Основание);			
			НомерВходящегоДокумента 	= Реквизиты.НомерВходящегоДокументаСФ;
			ДатаВходящегоДокумента 		= Реквизиты.ДатаВходящегоДокументаСФ;
			КППКонтрагента 				= Реквизиты.КонтрагентОсновнаяТорговаяТочкаКПП;
			//СуммаДокумента 				= Реквизиты.СуммаДокумента;
			//СуммаНДСДокумента			= Реквизиты.СуммаДокумента;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	вхДокументОснование = РаботаСостатусамиДокументов.ПолучитьПоследнююКорректировку(ДокументОснование);
	...
	Если НЕ СформированПриВводеНачальныхОстатковНДС Тогда
		Если (ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаПоступление
			...
			И НЕ СформированПриВводеНачальныхОстатковНДС Тогда
			Если ТипЗнч(вхДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступленияТоваровУслуг") Тогда
				ОпределениеПараметровСчетаФактуры(вхДокументОснование);
			Иначе
				ОпределениеПараметровСчетаФактуры(ДокументОснование);
			КонецЕсли;
		Иначе
			СводныйКорректировочный = Ложь;
		КонецЕсли;
	...
	Если ДокументыОснования.Количество() > 0 И ТипЗнч(ДокументыОснования[0].ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
