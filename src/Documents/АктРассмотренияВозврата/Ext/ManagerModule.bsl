...
Функция ДокументыЭкспертизыПоАРВ(СсылкаНаДокумент, ТекстУсловия = "", ПараметрыЗапроса = Неопределено) Экспорт
	
	лКлючАлгоритма = "Документ_АктРассмотренияВозврата_МодульМенеджера_ДокументыЭкспертизыПоАРВ";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Запрос.Текст = 
		...
		|	И НЕ Экспертиза.АктРассмотренияВозврата = ЗНАЧЕНИЕ(Документ.АктРассмотренияВозврата.ПустаяСсылка)
		|	И НЕ Экспертиза.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВозвратаОтПокупателя.МФП)
		|	И &ТекстУсловия";
...
Функция ПечатьАктаПриемаПередачиВозврата(вхСсылкаНаДокумент, Тип = 1) Экспорт
	...
	ИначеЕсли Тип = 2 Тогда
		...
		ОбластьШапка3_Розничный.Параметры.Штрихкод 			= Шапка.Штрихкод;
