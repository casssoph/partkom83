...
Функция ТекстПисьмаАктаПриемаПередачи(АктВозврата)
	...
	лКлючАлгоритма = "Документ_АктРассмотренияВозврата_МодульМенеджера_АктВозврата";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
...
Функция ШаблонТекстаПисьмаАктаПриемаПередачи()
	...
	|
	|	2. Доставить товар на склад грузополучателя в срок до [СрокВозвратаКлиента];
	|	
	...
	|		
	|	3. После приемки товара на нашем складе Вам будет:
	|		
...
Процедура ОтправитьЭлмаподобноеСообщениеНаСайт(Акт, Строка2 = "", Строка3, Строка4, Строка5, ТипКаталога = 3, ИмяФайла = "") Экспорт
...
Функция ПечатьАктаПриемаПередачиВозврата(вхСсылкаНаДокумент) Экспорт
	...
	| КодВозврата.Код, СуммаДокумента, ДокументПродажи.Номер, ДокументПродажи.Дата, ДоговорКонтрагента.Наименование, КомментарийГПРК, ПричинаВозврата.Наименование");
	...
	Если ЗначениеЗаполнено(СокрЛП(Шапка.КомментарийГПРК)) Тогда
		ОбластьШапка.Параметры.ДополнительныйКомментарий = "Дополнительный комментарий: " + Шапка.КомментарийГПРК;
	КонецЕсли;
	...
	ОбластьШапка.Параметры.ПричинаВозврата	= Шапка.ПричинаВозвратаНаименование;
	...
	ОбластьШапка.Параметры.ДопТекст1 = "Стороны пришли к соглашению о возврате излишнепоставленного товара и корректировке поставки по документу:";
	ОбластьШапка.Параметры.ДопТекст2 = "Указанный ниже товар не является обратной реализацией (поставкой) и подтверждает возврат товара.";
	ОбластьШапка.Параметры.ДопТекст3 = "Продавец (грузоотправитель) предоставляет покупателю (грузополучателю) корректировочную счет-фактуру.";
