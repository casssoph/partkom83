...
Функция ПечатьАктаПриемаПередачиВозврата(вхСсылкаНаДокумент) Экспорт
	...
	| КодВозврата.Код, СуммаДокумента, ДокументПродажи.Номер, ДокументПродажи.Дата, ДоговорКонтрагента.Наименование, КомментарийГПРК, ПричинаВозврата.Наименование, Контрагент.ОбратнаяРеализацияПриВозвратеОтКлиента");
	...
	//ШАПКА
	ОбластьШапка1.Параметры.ОписаниеТовара = "";
	ОписаниеТоваровАкта = ОписаниеТоваровАкта(вхСсылкаНаДокумент);
	Для каждого СтрокаОписания Из ОписаниеТоваровАкта Цикл
		ОбластьШапка1.Параметры.ОписаниеТовара = ОбластьШапка1.Параметры.ОписаниеТовара +  СтрокаОписания.ИнформацияОбУпаковке + Символы.ПС;
		ОбластьШапка1.Параметры.ОписаниеТовара = ОбластьШапка1.Параметры.ОписаниеТовара +  СтрокаОписания.ИнформацияОСледахУстановки;
		Прервать;
	КонецЦикла;
	Если ЗначениеЗаполнено(СокрЛП(Шапка.КомментарийГПРК)) Тогда
		ОбластьШапка1.Параметры.ДополнительныйКомментарий = "Дополнительный комментарий: " + Шапка.КомментарийГПРК;
	КонецЕсли;
	...
	ОбластьШапка1.Параметры.ПредставлениеГрузополучателя 	= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	ОбластьШапка1.Параметры.ПредставлениеГрузоотправителя 	= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузоотправитель, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	
	ОбластьШапка1.Параметры.КодВозврата 		= "0"+Шапка.КодВозвратаКод;
	ОбластьШапка1.Параметры.СрокДоставки 	= КонецДня(Шапка.СрокВозвратаКлиента);
	ОбластьШапка1.Параметры.ПричинаВозврата	= Шапка.ПричинаВозвратаНаименование;
	ОбластьШапка1.Параметры.Основание		= "Соглашение о возврате";
	...
	ПараметрыШтрихкода = Новый Структура;
	ПараметрыШтрихкода.Вставить("Ширина",          Окр(ОбластьШапка1.Рисунки.КартинкаШтрихкода.Ширина / КоличествоМиллиметровВПикселе));
	ПараметрыШтрихкода.Вставить("Высота",          Окр(ОбластьШапка1.Рисунки.КартинкаШтрихкода.Высота / КоличествоМиллиметровВПикселе));
	ПараметрыШтрихкода.Вставить("Штрихкод",        Шапка.Штрихкод);
	ПараметрыШтрихкода.Вставить("ТипКода",         4); // Code128
	ПараметрыШтрихкода.Вставить("ОтображатьТекст", Истина);
	ПараметрыШтрихкода.Вставить("РазмерШрифта",    12);
	
	ОбластьШапка1.Рисунки.КартинкаШтрихкода.Картинка = МенеджерОборудованияВызовСервера.ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода);
	...
	Если НЕ Шапка.КонтрагентОбратнаяРеализацияПриВозвратеОтКлиента Тогда
		...
		ОбластьШапка2.Параметры.ДопТекст3 = "Продавец (грузоотправитель) предоставляет покупателю (грузополучателю) корректировочную счет-фактуру.";
