...
Процедура ВыгрузитьАктПриемаПередачиНаСайт(АктСсылка) Экспорт
	...
	ТекстСообщенияДляСайта = ""+РеквизитыАРВ.СуммаДокумента
								+Символы.ПС+РеквизитыАРВ.КодВозвратаКод
								+Символы.ПС+КонецДня(РеквизитыАРВ.СрокВозвратаКлиента);
	ИмяФайла = "Readme.txt";
...
Функция ТекстСообщенияПоАктуПриемуПередачи(АктСсылка)
	...
	Возврат ТекстСообщенияДляСайта(АктСсылка);
...
//	ОбластьШапка.Параметры.НомерДокумента 	= Шапка.Номер;
