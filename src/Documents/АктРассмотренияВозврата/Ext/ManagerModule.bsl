...
Функция ТекстСообщенияПоАктуПриемуПередачи(АктСсылка)
	...
	ТекстСообщенияДляСайта = ""+Выборка.СуммаДокумента
	+Символы.ПС+Выборка.КодВозвратаКод
	+Символы.ПС+КонецДня(Выборка.СрокВозвратаКлиента)
...
Процедура ОтправитьАктПриемаПередачиПоПочте(АктВозврата) Экспорт
	...
	ЭП.ТекстПисьма = "Тут будет текст письма";
	...
	ИмяФайла = "akt_"+РеквизитыДокумента.Номер+".pdf";
...
Функция ПечатьАктРассмотренияВозврата(вхСсылкаНаДокумент) Экспорт
	...
	ОбластьШапка.Рисунки.ШК.Объект.ТипКода  = 4;
	ОбластьШапка.Рисунки.ШК.Объект.Сообщение = Шапка.Штрихкод;
