...
Функция СкладПриходаПоДокументам(СсылкаНаДокумент, ДатаПроверки = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	И ВозвратТоваровОтПокупателя.Склад.Возвраты
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И ПеремещениеТоваров.СкладПолучатель.Возвраты
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаДокумента УБЫВ";
...
Функция ПечатьАктаПриемаПередачиВозврата(вхСсылкаНаДокумент) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(АктРассмотренияВозвратаТовары.КоличествоПлан) КАК Количество,
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА СУММА(АктРассмотренияВозвратаТовары.КоличествоПлан) = 0
		|				ТОГДА 0
		|			ИНАЧЕ СУММА(АктРассмотренияВозвратаТовары.СуммаПлан) / СУММА(АктРассмотренияВозвратаТовары.КоличествоПлан)
