...
Процедура ВыгрузитьАктПриемаПередачиНаСайт(АктСсылка) Экспорт
	...
	ТабДок = ПечатьАктаПриемаПередачиВозврата(АктСсылка);
...
Функция ТекстСообщенияПоАктуПриемуПередачи(АктСсылка)
	...
	|	АктРассмотренияВозвратаТовары.Ссылка.СуммаДокумента КАК СуммаДокумента,
	...
	|	АктРассмотренияВозвратаТовары.Ссылка.СуммаДокумента,
	...
	+Символы.ПС+Выборка.ТоварАртикул+"||"+Выборка.ИзготовительНаименование+"||"+Выборка.ТоварНаименование
...
Процедура ОтправитьАктПриемаПередачиПоПочте(АктВозврата) Экспорт
	...
	ТабДок = ПечатьАктаПриемаПередачиВозврата(АктВозврата);
...
Функция ШаблонТекстаПисьмаАктаПриемаПередачи()
	...
	|	
	|	- Распечатать в 2-х экземплярах ""Акт приема-передачи"", который находится во вложении к данному письму;
	|	- Поставить подписи и печать организации грузоотправителя (если она имеется);
	|	- Доставить товар на склад грузополучателя в срок до [СрокВозвратаКлиента];
	...
	|		
	|После приемки товара на нашем складе Вам будет:
	|		
...
Процедура ОтправитьЭлмаподобноеСообщениеНаСайт(Акт, Строка2 = "", Строка3, Строка4, Строка5, ТипКаталога = 3) Экспорт
...
Функция ОтправитьТекстОтказаНаСайт(Акт) Экспорт
	...
	Если ЗначениеЗаполнено(МенеджерОтказаВВозврате) Тогда
	...
	КонецЕсли;
	ОтправитьЭлмаподобноеСообщениеНаСайт(Акт, Строка(МенеджерОтказаВВозврате), "2", "0", ТекстСообщения);
...
Функция СкладВозвратовОстаткаПартииНоменклатуры(Номенклатура, СтрокаПрихода, Дата) Экспорт
...
Функция ОрганизацияОстаткаПартииНоменклатуры(Номенклатура, СтрокаПрихода, Дата) Экспорт
...
Функция ПоставщикПоДаннымПартий(Документ) Экспорт
	...
	ИначеЕсли  ТипЗнч(Документ) = Тип("ДокументОбъект.АктРассмотренияВозврата")
		...
		ПоставщикПоДаннымПартий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ.Товары[0].СтрокаПрихода, "Приход.Контрагент");
...
Функция ПечатьАктаПриемаПередачиВозврата(вхСсылкаНаДокумент) Экспорт
	
	Шапка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, 
	"СрокВозвратаКлиента, Штрихкод, Номер, Дата, Организация, Контрагент, 
	| КодВозврата.Код, СуммаДокумента, ДокументПродажи.Номер, ДокументПродажи.Дата, ДоговорКонтрагента.Наименование");
	...
	Запрос.Текст = 
		...
		|	АктРассмотренияВозвратаТовары.КоличествоПлан КАК Количество,
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА АктРассмотренияВозвратаТовары.КоличествоПлан = 0
		|				ТОГДА 0
		|			ИНАЧЕ АктРассмотренияВозвратаТовары.СуммаПлан / АктРассмотренияВозвратаТовары.КоличествоПлан
	...
	//ШАПКА
	ОбластьШапка.Параметры.ОписаниеТовара = "";
	ОписаниеТоваровАкта = ОписаниеТоваровАкта(вхСсылкаНаДокумент);
	Для каждого СтрокаОписания Из ОписаниеТоваровАкта Цикл
		 ОбластьШапка.Параметры.ОписаниеТовара = ОбластьШапка.Параметры.ОписаниеТовара +  СтрокаОписания.ИнформацияОбУпаковке + Символы.ПС;
		 ОбластьШапка.Параметры.ОписаниеТовара = ОбластьШапка.Параметры.ОписаниеТовара +  СтрокаОписания.ИнформацияОСледахУстановки;
		 Прервать;
	КонецЦикла;
	...
	ОбластьШапка.Параметры.ПредставлениеГрузополучателя 	= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	ОбластьШапка.Параметры.ПредставлениеГрузоотправителя 	= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузоотправитель, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	
	ОбластьШапка.Параметры.КодВозврата 		= "0"+Шапка.КодВозвратаКод;
	ОбластьШапка.Параметры.СрокДоставки 	= КонецДня(Шапка.СрокВозвратаКлиента);
	ОбластьШапка.Параметры.ДатаДокумента 	= Формат(Шапка.Дата, "ДФ=dd.MM.yyyy");
	ОбластьШапка.Параметры.ДанныеДокументаПродажи = " № "+Шапка.ДокументПродажиНомер+" от "+Формат(Шапка.ДокументПродажиДата, "ДФ=dd.MM.yyyy")+"г.";
	ОбластьШапка.Параметры.Основание		= "Соглашение о возврате";
