...
Функция СкладВозвратовОстаткаПартииНоменклатуры(Номенклатура, СтрокаПрихода, Дата) Экспорт
	...
	Запрос.Текст = 
		...
		|				И Склад.Возвраты) КАК ПартииТоваровОстатки";
...
Функция ПродолжительностьНахожденияВСтатусе(СсылкаНаДокумент) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РАЗНОСТЬДАТ(СобытияАктовРассмотренияВозвратаСрезПоследних.Период, &ТекущаяДата, МИНУТА) КАК ПродолжительностьНахожденияВСтатусе
		|ИЗ
