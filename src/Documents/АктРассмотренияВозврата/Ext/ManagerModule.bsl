...
Функция ВыгрузитьПечатныеФормыНаСайт(АктСсылка) Экспорт
	
	КаталогВременныхФайлов = КаталогВременныхФайлов();
	КаталогОбменаССайтом = ВозвратыОтПокупателяСервер.КаталогОбменаССайтом(Ложь, Истина);
	
	РеквизитыАРВ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(АктСсылка, "Штрихкод, Контрагент.Код, ДокументПродажи.ДоговорКонтрагента.ВидОплаты");
	...
	Успешно = ЗапаковатьВZipФайл(ИмяАрхива, МассивИменФайлов);
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МассивИменФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Попытка
		...
		Архиватор.Записать();
	Исключение
		СтрокаСообщения = "Ошибка при запаковке файлов архива: "+ПолноеИмяФайлаАрхива+": "+ОписаниеОшибки();
		Сообщить(СтрокаСообщения);
		ВызватьИсключение СтрокаСообщения;
		Результат = Ложь;
	КонецПопытки;
...
Функция ПечатьСчетаФактуры(вхСсылкаНаДокумент, Макет = Неопределено) Экспорт
	...
	КонецЕсли;
		...
		//Если Шапка.Дата >= Дата(2017, 07, 01, 0, 0, 0) Тогда
		...
		//	Если Шапка.Дата >= Дата(2017, 10, 01, 0, 0, 0) Тогда
	...
	//ВыборкаСтрокТовары = ЗапросТовары;
