...
Процедура ПроверкиПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		 ...
		 Если НЕ (РазрешитьРучноеРедактированиеАРВ ИЛИ РольПП) Тогда
...
Функция ОстаткиПартийПоРТУ()
	...
	|	Максимум(АктРассмотренияВозвратаТовары.СтрокаЗаявки.Поставщик) КАК Поставщик,
	...
	|	Выбор когда втТовары.Поставщик is null тогда 1 Иначе 0 Конец КАК ПорядокСписания,
	...
	|	ОстаткиПартий.Номенклатура,
	|	ОстаткиПартий.СтрокаПрихода,
	|	Выбор когда втТовары.Поставщик is null тогда 1 Иначе 0 Конец
	|   
	|ИМЕЮЩИЕ
