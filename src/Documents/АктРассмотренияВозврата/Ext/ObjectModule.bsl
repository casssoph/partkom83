...
Процедура ЗаполнитьДанныеПрайсаПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если СтатусДокумента = Справочники.СтатусыДокументов.АРВ_Новый
		И Товары.Количество() > 0 Тогда
		  ...
		  РеквизитыСЗ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаЗаявки, "ТипПоставки, ПрайсПоставщика.ДоставкаДоКлиента, ПрайсПоставщика.БезДосмотра");
...
Процедура ПересчитатьСуммуНДС() Экспорт
	...
	Если ЗначениеЗаполнено(ДатаНакладной) Тогда
		МенятьНаНДС18 = ДатаНакладной < Дата(2019,1,1);
	КонецЕсли;
...
Функция ОстаткиПартийПоРазмещениям() Экспорт
	...
	|ГДЕ  СтрокаЗаявки <> Значение(Справочник.ИдентификаторыСтрокЗаявок.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И НЕ ЕСТЬNULL(ВозвратТоваровОтПокупателя.СтатусДокумента, ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ВозвратТоваровОтПокупателяНовый)) = &СтатусВозвратОтказ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("СтатусВозвратОтказ", Справочники.СтатусыДокументов.ВозвратТоваровОтПокупателяОтказ);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если Справочники.УчетныеЗаписиСайта.ДанныеУчетнойЗаписиКонтрагента(Контрагент).УчетнаяЗапись <> Неопределено Тогда
