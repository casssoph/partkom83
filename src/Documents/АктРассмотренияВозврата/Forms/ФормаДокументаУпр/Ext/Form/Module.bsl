...
&НаКлиенте
Процедура ПоступлениеДругогоПоставщикаПриИзменении(Элемент)
	
	лКлючАлгоритма = "Документ_АктРассмотренияВозврата_ФормаДокументаУпр_ПоступлениеДругогоПоставщикаПриИзменении";
	...
	Если ЗначениеЗаполнено(Объект.ПоступлениеДругогоПоставщика) Тогда
		Если ДокументСодержитНоменклатуру(Объект.ПоступлениеДругогоПоставщика, Объект.Товары[0].Номенклатура) Тогда //сообщим об ошибке
			Объект.Поставщик = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПоступлениеДругогоПоставщика, "Контрагент");
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		...
		?(ЗначениеЗаполнено(Объект.ПоступлениеДругогоПоставщика), 
		Документы.ПоступлениеТоваровУслуг.ЦенаПоступления(Объект.ПоступлениеДругогоПоставщика, СтрокаТабличнойЧасти.Номенклатура), 
...
//ХудинВВ XX-2837 09072019
&НаСервереБезКонтекста
Функция ДокументСодержитНоменклатуру(ДокСсылка, Номенклатура, СообщатьОбОшибке = Истина)
	
	лКлючАлгоритма = "Документ_АктРассмотренияВозврата_ФормаДокументаУпр_ДокументСодержитНоменклатуру";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
	...
	ДокументСодержитНоменклатуру =  ДокСсылка.Товары.Найти(Номенклатура, "Номенклатура") <> Неопределено;	
	
	Если НЕ ДокументСодержитНоменклатуру И СообщатьОбОшибке Тогда
		Сообщить("Выбранный документ поступления "+ДокСсылка+" не содержит товара """+Номенклатура+""", проверьте правильность выбора!", СтатусСообщения.Важное);			
	КонецЕсли;
	
	Возврат ДокументСодержитНоменклатуру;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//ХудинВВ XX-2837 09072019
	Если ЗначениеЗаполнено(Объект.ПоступлениеДругогоПоставщика) 
		И НЕ ДокументСодержитНоменклатуру(Объект.ПоступлениеДругогоПоставщика, Объект.Товары[0].Номенклатура) Тогда
		Отказ = Истина;			
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	лКлючАлгоритма = "Документ_АктРассмотренияВозврата_ФормаДокументаУпр_ПриЗаписиНаСервере";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	СтатусПроверкиДокументовПоставщикаСтарый = Неопределено;
	СтатусПроверкиДокументовПокупателяСтарый = Неопределено;
	Если ТекущийОбъект.ДополнительныеСвойства.Свойство("РеквизитыСсылкиПередЗаписью") Тогда
		ТекущийОбъект.ДополнительныеСвойства.РеквизитыСсылкиПередЗаписью.Свойство("СтатусПроверкиДокументовПоставщика", СтатусПроверкиДокументовПоставщикаСтарый);
		ТекущийОбъект.ДополнительныеСвойства.РеквизитыСсылкиПередЗаписью.Свойство("СтатусПроверкиДокументовПокупателя", СтатусПроверкиДокументовПокупателяСтарый);
	КонецЕсли;
	...
	//Если изменили отметку о проверке документов, то добавим запись в регистр
	Если НЕ ТекущийОбъект.СтатусПроверкиДокументовПоставщика = СтатусПроверкиДокументовПоставщикаСтарый	Тогда
		
		НовыйПризнак = ТекущийОбъект.СтатусПроверкиДокументовПоставщика = Перечисления.АРВ_СтатусыПроверкиДокументовПоставщика.Проверены;
		
		ДокументыВозврата = Документы.АктРассмотренияВозврата.ДокументыВозвратаПоставщикуПоАРВ(ТекущийОбъект.Ссылка);
		Для каждого СтрокаДокумента Из ДокументыВозврата Цикл
			РаботаСоСтатусамиДокументов.ИзменитьПризнакДокументПроверен(СтрокаДокумента.Ссылка, НовыйПризнак, ПараметрыСеанса.ТекущийПользователь);
		КонецЦикла;
