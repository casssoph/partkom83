...
Функция РегистрыНакопления_ЗаявкиПокупателей(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Запрос = новый запрос("ВЫБРАТЬ
	                      ...
	                      |	ВТОтказы.Качество,
	                      |	ВТОтказы.СтрокаЗаявки,
	                      |	ЕСТЬNULL(ЗаявкиПокупателейОстатки.ТорговаяТочка, ВТОтказы.ТорговаяТочка) КАК ТорговаяТочка,
	                      |	ЕСТЬNULL(ЗаявкиПокупателейОстатки.ДоговорКонтрагента, ВТОтказы.ДоговорКонтрагента) КАК ДоговорКонтрагента,
	                      |	ВЫБОР
	                      |		КОГДА ВТОтказы.КоличествоДокумент - ВТОтказы.КоличествоРегистр > ЗаявкиПокупателейОстатки.СтрокаЗаявки.Количество
	                      |			ТОГДА ЗаявкиПокупателейОстатки.СтрокаЗаявки.Количество
	                      |		ИНАЧЕ ВТОтказы.КоличествоДокумент - ВТОтказы.КоличествоРегистр
	                      |	КОНЕЦ КАК Количество,
	                      |	ВЫБОР
	                      |		КОГДА ЕСТЬNULL(ЗаявкиПокупателейОстатки.КоличествоОстаток, 0) = 0
	                      |			ТОГДА 0
	                      |		ИНАЧЕ ЗаявкиПокупателейОстатки.СуммаРеглОстаток / ЗаявкиПокупателейОстатки.КоличествоОстаток * (ВТОтказы.КоличествоДокумент - ВТОтказы.КоличествоРегистр)
	                      |	КОНЕЦ КАК СуммаРегл,
	                      |	ВЫБОР
	                      |		КОГДА ЕСТЬNULL(ЗаявкиПокупателейОстатки.КоличествоОстаток, 0) = 0
	                      |			ТОГДА 0
	                      |		ИНАЧЕ ЗаявкиПокупателейОстатки.СуммаУпрОстаток / ЗаявкиПокупателейОстатки.КоличествоОстаток * (ВТОтказы.КоличествоДокумент - ВТОтказы.КоличествоРегистр)
	                      |	КОНЕЦ КАК СуммаУпр,
	                      |	ИСТИНА КАК Отказ,
...
Функция РегистрыНакопления_ЗаказыПоставщикам(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	//Иначе
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	КорректировкаЗаказаПоставщикуПричиныОтказов.КлючСвязи,
		|	СУММА(КорректировкаЗаказаПоставщикуПричиныОтказов.Количество) КАК Количество,
		|	КорректировкаЗаказаПоставщикуПричиныОтказов.СтрокаЗаявки
