...
Функция РегистрыНакопления_ЗаказыПоставщикам(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		|	втИтоговая.КоличествоТовары - втИтоговая.КоличествоРегистр КАК Количество,
		|	втИтоговая.СуммаРеглТовары - втИтоговая.СуммаРеглРегистр КАК СуммаРегл,
		|	втИтоговая.СуммаУпрТовары - втИтоговая.СуммаУпрРегистр КАК СуммаУпр
		|ИЗ
...
Функция РегистрыНакопления_ЗаказыПоставщикам_НеИспользуется(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		|	ЗаказыПоставщикамОбороты.ТорговаяТочка КАК ТорговаяТочка,
		|	ЗаказыПоставщикамОбороты.ДоговорКонтрагента КАК ДоговорКонтрагента,
		|	СУММА(ЗаказыПоставщикамОбороты.Количество) КАК Количество,
		|	СУММА(ЗаказыПоставщикамОбороты.СуммаРегл) КАК СуммаРегл,
		|	СУММА(ЗаказыПоставщикамОбороты.СуммаУпр) КАК СуммаУпр
		|ПОМЕСТИТЬ втЗаказыПоставщикам
		|ИЗ
		|	РегистрНакопления.ЗаказыПоставщикам КАК ЗаказыПоставщикамОбороты
		|ГДЕ
		|	(ЗаказыПоставщикамОбороты.Регистратор ССЫЛКА Документ.ЗаказПоставщику
		|			ИЛИ ЗаказыПоставщикамОбороты.Регистратор ССЫЛКА Документ.КорректировкаЗаказаПоставщику)
		|	И ЗаказыПоставщикамОбороты.СтрокаЗаявки В
		|			(ВЫБРАТЬ
		|				втСтрокиЗаявки.СтрокаЗаявки
		|			ИЗ
		|				втСтрокиЗаявки)
		|	И ЗаказыПоставщикамОбороты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|	И НЕ ЗаказыПоставщикамОбороты.Регистратор = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗаказыПоставщикамОбороты.Склад,
		|	ЗаказыПоставщикамОбороты.Номенклатура,
