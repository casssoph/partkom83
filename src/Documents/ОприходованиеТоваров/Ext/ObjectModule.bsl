...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	//#XX-2321 Kalinin V.A. ( 2019-04-16 )  /*
	ПроверкаЗаполненияСклада(Отказ);
...
Процедура ПроверкаЗаполненияСклада(Отказ)
	
	Если ДополнительныеСвойства.Свойство("РежимБога") тогда 
		Возврат ;
	КонецЕсли;
	
	Если Склад.СкладVMI тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Запрещено оприходование по складам Ответ хранения",,,,Отказ)	
	КонецЕсли;		
