...
Функция РегистрыСведений_ЦеныНоменклатуры(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	ПараметрыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(вхСсылкаНаДокумент, "Дата, флОбновлятьЦены, ТипЦен, ВалютаДокумента, КратностьВзаиморасчетов, КурсВзаиморасчетов");
	...
	//Если цена в справочнике есть, тогда обновляем только при отсутствии остатка на всех незаблокированных складах. 
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	|			&ДатаОстатков,
	|			НЕ Склад.Заблокирован
	|				И Номенклатура В
	|					(ВЫБРАТЬ
	|						втТовары.Номенклатура
	|					ИЗ
	|						втТовары)
	|				И СтатусПартии = &Собственный) КАК ПартииТоваровОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО втТовары.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|ГДЕ
	|	(ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) = 0
	|			ИЛИ ЕСТЬNULL(ОстаткиТоваров.КоличествоОстаток, 0) = 0)");
	
	Запрос.УстановитьПараметр("Собственный", Перечисления.СтатусыПартии.Собственный);
	Запрос.УстановитьПараметр("ТипЦен", ПараметрыДокумента.ТипЦен);
	Запрос.УстановитьПараметр("Ссылка", вхСсылкаНаДокумент);
	Запрос.УстановитьПараметр("ДатаОстатков", Новый Граница(вхСсылкаНаДокумент.МоментВремени(), ВидГраницы.Исключая));
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Цена 		= МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.Цена,
				...
				ПараметрыДокумента.КурсВзаиморасчетов, КурсРуб.Курс,
				ПараметрыДокумента.КратностьВзаиморасчетов, КурсРуб.Кратность);
		...
		Движение.Цена 		= МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.Цена,
				...
				ПараметрыДокумента.КурсВзаиморасчетов, КурсДолл.Курс,
				ПараметрыДокумента.КратностьВзаиморасчетов, КурсДолл.Кратность);
		...
		Движение.Цена 		= МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.Цена,
				...
				ПараметрыДокумента.КурсВзаиморасчетов, КурсЕвро.Курс,
				ПараметрыДокумента.КратностьВзаиморасчетов, КурсЕвро.Кратность);
