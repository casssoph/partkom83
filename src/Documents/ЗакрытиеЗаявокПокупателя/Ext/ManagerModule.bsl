...
Функция ПолучитьТаблицуСтрокЗаявок(вхСсылкаНаДокумент)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РегистрНакопления.ЗаявкиПокупателей.Остатки(
	               |			,
	               |			СтрокаЗаявки.Заявка В
	               |				(ВЫБРАТЬ
	               |					втЗаявки.Документ
	               |				ИЗ
	               |					втЗаявки)) КАК ЗаявкиПокупателейОстатки
	               |ГДЕ
	               |	(ЗаявкиПокупателейОстатки.КоличествоОстаток <> 0
	               |			ИЛИ ЗаявкиПокупателейОстатки.СуммаРеглОстаток <> 0
	               |			ИЛИ ЗаявкиПокупателейОстатки.СуммаУпрОстаток <> 0)";
