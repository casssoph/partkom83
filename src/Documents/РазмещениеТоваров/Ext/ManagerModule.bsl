...
Процедура ЗагрузитьРезультатРазмещения(ОбъектXDTO, ДопПараметры = Неопределено, СтруктураОтчета = Неопределено)
	...
	Для каждого ЗаказНаПриемкуXDTO Из Заказы Цикл
		Если ЗаказНаПриемкуXDTO.ЗаказВидДокумента = "ВозвратТоваровОтПокупателя"
			...
			ИЛИ ЗаказНаПриемкуXDTO.ЗаказВидДокумента = "ОтказПоставщикаВВозврате" Тогда
			Загружать = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ЗаказНаПриемкуXDTO Из Заказы Цикл
		...
		ИначеЕсли ЗаказНаПриемкуXDTO.ЗаказВидДокумента = "ОтказПоставщикаВВозврате" Тогда
			ИмяТаблицыОснования = "ОтказПоставщикаВВозврате";
		Иначе
			Продолжить;
		КонецЕсли;
