...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)	
	...
	Для Каждого Товар Из Товары Цикл
		...
		Если Товар.СтавкаНДС = Перечисления.СтавкиНДС.НДС18 Тогда // ЛНА http://jira.part-kom.ru/browse/XX-1841
...
Функция ВыгрузитьВОбменТоплог() Экспорт
	...
	Если Результат.Пустой() Тогда 
		Ошибка = Истина;
		СообщениеДиагностики = СообщениеДиагностики + "Нет количества в табл. части ""Товары"" для выгрузки. Для документов, созданных из акта, количество выгружаемое в ТопЛог считается как Брак + Излишек - Возвращено" + Символы.ПС;	
	КонецЕсли;
