		...
		Если РаботаСПоследовательностямиКлиентСервер.ТаблицыИдентичны(ТЗТоварыПТУ, ТЗТоварыВозврат, ТЗДобавить, ТЗУдалить) = Истина Тогда
			Для Каждого СтрокаВозврата Из Товары Цикл
				...
				Если СтрокаПТУ.Количество <> СтрокаВозврата.Количество Тогда
					//<< 07.08.2019 Горохов П.Е. ХХ-2884
					Если СтрокаПТУ.Количество < СтрокаВозврата.Количество Тогда
						НадоЗаписатьПТУ = Истина;
						СтрокаПТУ.Количество = СтрокаВозврата.Количество;
					ИначеЕсли СтрокаПТУ.Количество <> СтрокаВозврата.Количество + СтрокаПТУ.КоличествоНеПринято Тогда
						НадоЗаписатьПТУ = Истина;
						СтрокаПТУ.КоличествоНеПринято = СтрокаПТУ.Количество - СтрокаВозврата.Количество;
						КоличествоВОтказ = СтрокаПТУ.КоличествоНеПринято;
						
						ПараметрыОтбора = Новый Структура;
						ПараметрыОтбора.Вставить("Номенклатура", СтрокаПТУ.Номенклатура);
						ПараметрыОтбора.Вставить("СтрокаПрихода", СтрокаПТУ.СтрокаПрихода);
						СтрокиРазмещения = ДокПТУ.РазмещениеСтрокПрихода.НайтиСтроки(ПараметрыОтбора);
						
						Для Каждого СтрокаРазм Из СтрокиРазмещения Цикл
							СтрокаОтказа = ДокПТУ.ПричиныОтказов.Добавить();
							СтрокаОтказа.КлючСвязи = СтрокаПТУ.КлючСвязи;
							СтрокаОтказа.СтрокаПрихода = СтрокаПТУ.СтрокаПрихода;
							...
							Если СтрокаРазм.Количество >= КоличествоВОтказ Тогда
								СтрокаОтказа.Количество = КоличествоВОтказ;
								Прервать;
							Иначе
								СтрокаОтказа.Количество = СтрокаРазм.Количество;
								КоличествоВОтказ = КоличествоВОтказ - СтрокаРазм.Количество;
							КонецЕсли;
						КонецЦикла;
					Иначе
						Продолжить;
