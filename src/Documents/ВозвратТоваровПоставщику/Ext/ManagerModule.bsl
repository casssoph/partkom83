...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена, ВыгружаемыеОбъекты = Неопределено) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда
		...
		лЗапрос.УстановитьПараметр("Свойство", УправлениеСвойствамиПовтИсп.Свойство_ВидПечатнойФормыДляВозвратаПоставщику());
		
		лЗапрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫРАЗИТЬ(Т.Ссылка КАК Документ.ВозвратТоваровПоставщику) КАК Ссылка
		|ПОМЕСТИТЬ Объекты
		|ИЗ
		|	&ТаблицаСсылок КАК Т
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Объекты.Ссылка.Контрагент КАК Контрагент,
		|	ВЫРАЗИТЬ(ЗначенияСвойствОбъектов.Значение КАК Справочник.ЗначенияСвойствОбъектов).Наименование КАК ВидПФ
		|ПОМЕСТИТЬ втСвойстваКонтрагентов_ПФ
		|ИЗ
		|	РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Объекты КАК Объекты
		|		ПО ЗначенияСвойствОбъектов.Объект = Объекты.Ссылка.Контрагент
		|ГДЕ
		...
		|	ЕСТЬNULL(ВозвратТоваровПоставщику.АктРассмотренияВозврата.Штрихкод, """") КАК Штрихкод,
		...
		|	ВЫБОР
		|		КОГДА ВозвратТоваровПоставщику.СкладПолучатель <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|			ТОГДА ЕСТЬNULL(ВозвратТоваровПоставщику.СкладПолучатель.ФизическийСклад.КонтрагентТопЛог.ОсновнаяТорговаяТочка, ЗНАЧЕНИЕ(Справочник.ТорговыеТочки.ПустаяСсылка))
		|		ИНАЧЕ ЕСТЬNULL(ВозвратТоваровПоставщику.Контрагент.ОсновнаяТорговаяТочка, ЗНАЧЕНИЕ(Справочник.ТорговыеТочки.ПустаяСсылка))
		...
		|	ЕСТЬNULL(ВозвратТоваровПоставщику.Контрагент.ОсновнаяТорговаяТочка.Город, ЗНАЧЕНИЕ(Справочник.Города.ПустаяСсылка)) КАК Город,
		|	ЕСТЬNULL(ВозвратТоваровПоставщику.Контрагент.ОсновнаяТорговаяТочка.Город.Наименование, """") КАК ГородНаименование,
		|	ЕСТЬNULL(ВозвратТоваровПоставщику.Контрагент.ОсновнаяТорговаяТочка.МаршрутДоставки.Код, """") КАК МаршрутДоставкиКод,
		|	ЕСТЬNULL(ВозвратТоваровПоставщику.Контрагент.ОсновнаяТорговаяТочка.МаршрутДоставки.Наименование, """") КАК МаршрутДоставкиНаименование,
		...
		|				И ЕСТЬNULL(втСвойстваКонтрагентов_ПФ.ВидПФ, """") = ""ТОРГ-2""
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ПечататьТОРГ2,
		|	ВЫБОР
		|		КОГДА ВозвратТоваровПоставщику.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВозвратПоставщику.ОбратнаяРеализация)
		|			ТОГДА 1
		|		КОГДА ВозвратТоваровПоставщику.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВозвратПоставщику.КорректировочныйСчетФактура)
		|				И ЕСТЬNULL(втСвойстваКонтрагентов_ПФ.ВидПФ, """") = ""ТОРГ-2""
		|			ТОГДА 0
		...
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК ПечататьСФ
		|ИЗ
		|	Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщикуТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику КАК ВозвратТоваровПоставщику
		|			ЛЕВОЕ СОЕДИНЕНИЕ втСвойстваКонтрагентов_ПФ КАК втСвойстваКонтрагентов_ПФ
		|			ПО ВозвратТоваровПоставщику.Контрагент = втСвойстваКонтрагентов_ПФ.Контрагент
		...
		|		КОНЕЦ > 0
		|ИТОГИ
		|	МАКСИМУМ(ПечататьТОРГ2),
		|	МАКСИМУМ(СтатусУПД),
		|	МАКСИМУМ(ПечататьСФ)
		|ПО
		|	Ссылка";
		
		//<< 17.07.2019 Горохов П.Е. ХХ-2884
		//Добавил в запрос новое условие выборки для полей "ВидДокумента" и "Контрагент" для возвратов с заполненным реквизитом "СкладПолучатель"  
		//>> 17.07.2019 Горохов П.Е. ХХ-2884
		...
		Если НЕ лРезультат.Пустой() Тогда
			...
			Пока лВыборка.Следующий() Цикл
				...
				Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2724) Тогда 
					//лОбъект.ВидПечФормы_ВозвратаПоставщику = лВыборка.ВидПечФормы_ВозвратаПоставщику;		
					
					лОбъект.ПечататьТОРГ2 		= лВыборка.ПечататьТОРГ2; 
					лОбъект.ПечататьСФ 			= лВыборка.ПечататьСФ; 
					лОбъект.СтатусУПД 			= лВыборка.СтатусУПД;
					лОбъект.ПечататьТОРГ12 		= лОбъект.ПечататьСФ; 
				КонецЕсли;
...
Процедура ВыгрузитьДокументыДляОкнаПоставщика(вхТаблицаСсылокНаОбъекты, Результат)
	...
	лКлючАлгоритма = "Документ_ВозвратТоваровПоставщику_МодульМенеджера_ВыгрузитьДокументыДляОкнаПоставщика";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
...
Функция ПолучитьЗапросДляВыгрузкиВОкноПоставщика(вхТаблицаСсылокНаОбъекты)
	...
	лКлючАлгоритма = "Документ_ВозвратТоваровПоставщику_МодульМенеджера_ПолучитьЗапросДляВыгрузкиВОкноПоставщика";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |     ВозвратТоваровПоставщикуТовары.Ссылка.ДатаПодтвержденияПоставщика КАК supplier_confirm_date,
	               ...
	               |		ИНАЧЕ ВозвратТоваровПоставщикуТовары.КолНач
	               |	КОНЕЦ КАК amount,
	               ...
	               |	МАКСИМУМ(supplier_confirm_date),
	...
	//ХудинВВ XX-2545 11072019 - Добавил supplier_confirm_date
	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog
		...
		Если ОбъектXDTO.Тип().Имя = "РезультатСборки" Тогда 
			...
			Исключение
				...
				#Если Сервер Тогда 
					ОбменСТопЛогСервер.ОбработатьДействияНадОбъектом(вхПараметры, ДокументСсылка, ОписаниеОшибки(), "ЗагрузитьРезультатСборки");
				#КонецЕсли
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
				...
				Если СтрокаДельты.Факт < 0 Тогда  
					...
					Пока КоличествоРаспределить > 0 И Индекс < Строки.Количество() Цикл 
						...
						Иначе
							...
							СтрокаТЧ.КолНач = СтрокаТЧ.Количество; // 07.08.2019 Горохов П.Е. ХХ-2884
...
Функция ФлагНеВыгружатьВТоплогПоУмолчанию(вхПараметры) Экспорт
	
	лКлючАлгоритма = "Документ_ВозвратТоваровПоставщику_МодульМенеджера_ФлагНеВыгружатьВТоплогПоУмолчанию";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	ФлагНеВыгружатьВТоплог = Ложь;
	
	Если вхПараметры.СозданИзАкта Тогда
		ФлагНеВыгружатьВТоплог = Истина;
	ИначеЕсли вхПараметры.СпособПроведения = Перечисления.СпособыПроведенияДокументов.ТолькоВзаиморасчеты
		ИЛИ вхПараметры.СпособПроведения = Перечисления.СпособыПроведенияДокументов.ТолькоТовары Тогда
		ФлагНеВыгружатьВТоплог = Истина;
	ИначеЕсли вхПараметры.СтатусДокумента <> Справочники.СтатусыДокументов.ВозвратТоваровПоставщикуНовый Тогда
		ФлагНеВыгружатьВТоплог = Истина;
	КонецЕсли;
	
	Возврат ФлагНеВыгружатьВТоплог;
