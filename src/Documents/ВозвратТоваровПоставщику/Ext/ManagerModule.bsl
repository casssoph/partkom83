...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда
		...
		"ВЫБРАТЬ
		|	Т.Ссылка,
		|	ВЫРАЗИТЬ(Т.Ссылка КАК Документ.ВозвратТоваровПоставщику).Контрагент КАК Контрагент
		|ПОМЕСТИТЬ Объекты
		|ИЗ
		|	&ТаблицаСсылок КАК Т
		|;
		...
		|		ПО ЗначенияСвойствОбъектов.Объект = Объекты.Контрагент
		|ГДЕ
		...
		|			ТОГДА ""ТОРГ-2""
		|		ИНАЧЕ ""УПД-2""
		|	КОНЕЦ КАК ВидПечФормы_ВозвратаПоставщику
		|ИЗ
		|	Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщикуТовары
		...
		|		КОНЕЦ > 0
		|ИТОГИ
		|	МАКСИМУМ(ВидПечФормы_ВозвратаПоставщику)
		|ПО
		|	Ссылка";
