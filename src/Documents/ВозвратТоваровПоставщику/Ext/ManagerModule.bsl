...
Функция ТаблицыДляРасчетаСписанияПоПартиям(вхСсылкаНаДокумент, вхФильтр = Неопределено) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Дата, СтатусДокумента, Склад, АктРассмотренияВозврата");
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда
		...
		Если НЕ лРезультат.Пустой() Тогда
			...
			Пока лВыборка.Следующий() Цикл
				...
				лОбъект.ДокументОснованиеСсылка = ?(ЗначениеЗаполнено(лВыборка.ДокументОснование), XMLСтрока(лВыборка.ДокументОснование), "");
