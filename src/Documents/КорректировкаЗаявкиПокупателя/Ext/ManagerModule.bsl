...
Функция РегистрыНакопления_ЗаявкиПокупателей_НоваяСхема(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Дата, СтатусДокумента, Подтверждена,ВидОперации,ВалютаДокумента");
...
Функция РегистрыНакопления_ЗаявкиПокупателей(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	ТаблицаДвижений = Неопределено;
	ОбщегоНазначения.СоздатьСтруктуруРегистраНакопления("ЗаявкиПокупателей", ТаблицаДвижений);
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Дата, СтатусДокумента, Подтверждена, ВалютаДокумента");
...
Функция РегистрыНакопления_РезервыТоваров_Новая(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено, ДобавленныеОтказы = Неопределено) Экспорт
	...
	|ГДЕ
	|	ВТДвижений.СтрокаЗаявки.ТипПоставки <> ЗНАЧЕНИЕ(Перечисление.ТипПоставки.Кросс)";
	
	//ХудинВВ XX-2806 07082019 ((
	//Исключаем кросс из движений по резервам ВТДвижений.СтрокаЗаявки.ТипПоставки <> ЗНАЧЕНИЕ(Перечисление.ТипПоставки.Кросс)
    //))
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена, ВыгружаемыеОбъекты = Неопределено) Экспорт
	...
	Если лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		Если НЕ лРезультатЗапроса.Пустой() Тогда
			...
			//Валиахметов http://jira.part-kom.ru/browse/XX-2608 05.07.2019
			ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(лВыборка.СтрокаЗаявки, лОбъект, "СнятиеРезерва");
			//Конец Валиахметов http://jira.part-kom.ru/browse/XX-2608 05.07.2019
