...
Процедура ВыполнитьПроведение(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если ПроведениеДокументовКлиентСервер.ПроводитсяПо(вхПараметры, "РезервыТоваров") Тогда
		...
		ОбновитьДокумент(вхСсылкаНаДокумент, ДобавленныеОтказы, ОбновитьОтказы);
	КонецЕсли; 
...
Процедура ОбновитьДокумент(вхСсылкаНаДокумент, ДобавленныеОтказы, ОбновитьОтказы) 
...
Функция РегистрыНакопления_ЗаявкиПокупателей_НоваяСхема(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Иначе   
		...
		//Честно не понимаю нахрена на приход ставить блокировку ,  да и хрен с ней оставлю
		...
		|    ВЫБОР
		|        КОГДА втТовары.Количество ЕСТЬ NULL
		|            ТОГДА ЕСТЬNULL(втЗаявкиПокупателей.КоличествоОстаток, 0)
		|    ИНАЧЕ ЕСТЬNULL(втТовары.Количество, 0) - ЕСТЬNULL(втЗаявкиПокупателей.КоличествоОстаток, 0) 
		|    КОНЕЦ КАК Количество,
		|    ВЫБОР
		|        КОГДА втТовары.Количество ЕСТЬ NULL
		|            ТОГДА ЕСТЬNULL(втЗаявкиПокупателей.СуммаУпрОстаток, 0)
		|    ИНАЧЕ ЕСТЬNULL(втТовары.СуммаУпр, 0) - ЕСТЬNULL(втЗаявкиПокупателей.СуммаУпрОстаток, 0) 
		|    КОНЕЦ КАК СуммаУпр,
		|    ВЫБОР
		|        КОГДА втТовары.Количество ЕСТЬ NULL
		|    		ТОГДА ЕСТЬNULL(втЗаявкиПокупателей.СуммаРеглОстаток, 0)
		|    ИНАЧЕ ЕСТЬNULL(втТовары.СуммаРегл, 0) - ЕСТЬNULL(втЗаявкиПокупателей.СуммаРеглОстаток, 0)
		|    КОНЕЦ КАК СуммаРегл,		
		...
		|	втТовары.ТорговаяТочка,
		|	втТовары.ДоговорКонтрагента,
		|	КорректировкаЗаявкиПокупателяПричиныОтказов.Количество,
		|	ВЫБОР
		|		КОГДА втТовары.Количество = 0
		|			ТОГДА 0
		|		ИНАЧЕ втТовары.СуммаУпр / втТовары.Количество * КорректировкаЗаявкиПокупателяПричиныОтказов.Количество
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА втТовары.Количество = 0
		|			ТОГДА 0
		|		ИНАЧЕ втТовары.СуммаРегл / втТовары.Количество * КорректировкаЗаявкиПокупателяПричиныОтказов.Количество
		|	КОНЕЦ,
		|	&Ссылка,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	&Дата,
		...
		|ИЗ
		|	втТовары КАК втТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаЗаявкиПокупателя.ПричиныОтказов КАК КорректировкаЗаявкиПокупателяПричиныОтказов
		|		ПО (КорректировкаЗаявкиПокупателяПричиныОтказов.Ссылка = &Ссылка)
		|			И втТовары.КлючСвязи = КорректировкаЗаявкиПокупателяПричиныОтказов.КлючСвязи";
...
Функция РегистрыНакопления_РезервыТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено, ОбновитьОтказы = Ложь, ДобавленныеОтказы = Неопределено) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Дата,СтатусДокумента,ДокументОснование,Подтверждена");
