...
Функция РегистрыНакопления_РезервыТоваров_Новая(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено, ДобавленныеОтказы = Неопределено) Экспорт
	...
	|ВЫБРАТЬ
	|	ВТТаблицаТоваров.СтрокаЗаявки,
	|	СУММА(ВТТаблицаТоваров.Количество - ЕСТЬNULL(ВТДвижений.Количество, 0) - ЕСТЬNULL(ВТРезервыНоменклатура.КоличествоОстаток, 0)) КАК Количество,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВТТаблицаТоваров.Количество - ЕСТЬNULL(ВТДвижений.Количество, 0) - ЕСТЬNULL(ВТРезервыНоменклатура.КоличествоОстаток, 0)) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
