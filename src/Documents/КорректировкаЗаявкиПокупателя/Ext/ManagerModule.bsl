...
Функция РегистрыНакопления_РезервыТоваров_Новая(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено, ДобавленныеОтказы = Неопределено) Экспорт
	...
	|	ВложенныйЗапрос.Склад КАК Склад1,
	...
	|					ВТТаблицаТоваров.СтрокаЗаявки
	|				ИЗ
	|					ВТТаблицаТоваров КАК ВТТаблицаТоваров)) КАК ЗаявкиПокупателейОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА ВТСвободныеОстатки.КоличествоСвободныйОстаток >= ЕСТЬNULL(ВТОстаткиЗаявок.КоличествоОстаток, 0) - ЕСТЬNULL(ВТТаблицаОтказов.Количество, 0) - ЕСТЬNULL(ВТРезервыНоменклатура.КоличествоОстаток, 0)
	|				ТОГДА ЕСТЬNULL(ВТОстаткиЗаявок.КоличествоОстаток, 0) - ЕСТЬNULL(ВТТаблицаОтказов.Количество, 0) - ЕСТЬNULL(ВТРезервыНоменклатура.КоличествоОстаток, 0)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиЗаявок КАК ВТОстаткиЗаявок
	|			ПО ВТТаблицаТоваров.СтрокаЗаявки = ВТОстаткиЗаявок.СтрокаЗаявки
	|	ГДЕ
	|		ВТТаблицаТоваров.СтрокаЗаявки.ТипПоставки В (ЗНАЧЕНИЕ(Перечисление.ТипПоставки.Сток), ЗНАЧЕНИЕ(Перечисление.ТипПоставки.VMI))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	ДобавленныеОтказы= РезультатЗапроса[8].Выгрузить();
	ТаблицаДвижений  = РезультатЗапроса[9].Выгрузить();
