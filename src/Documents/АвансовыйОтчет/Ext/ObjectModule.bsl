...
Функция ПечатьАвансовогоОтчета()
	...
	|				И Валюта = &Валюта
	|				И ФизЛицо = &ФизЛицо)
	|				КАК ВзаиморасчетыСПодотчетнымиЛицамиОстатки";
...
Процедура ОбработкаЗаполненияУпр(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") 
...
Процедура ПодготовитьТаблицуТоваровУпр(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		СтрокаТаблицы.СуммаУпр = 0; //МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтоимостьСНДС, ВалютаДокумента,
		...
		СтрокаТаблицы.НДСУпр = 0; //МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаТаблицы.НДС, ВалютаДокумента,
										 //СтруктураШапкиДокумента.ВалютаУправленческогоУчета,
										 //СтруктураШапкиДокумента.КурсДокумента, 
										 //СтруктураШапкиДокумента.КурсВалютыУправленческогоУчета,
										 //СтруктураШапкиДокумента.КратностьДокумента, 
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	УправлениеЗапасами.КорректировкаСтруктурыОбязательныхПолей(ЭтотОбъект, "Товары", Неопределено, СтруктураОбязательныхПолей);
...
Процедура ДвиженияПоПодотчетникамУпр(СтруктураШапкиДокумента, ТаблицаПоВыданныеАвансы, Режим, Отказ, Заголовок)
	...
	Если Не Отказ Тогда
	//Движения = РегистрыНакопления.Взаиморасчеты.СоздатьНаборЗаписей();
...
Процедура ОплатаПоставщикамУпр(РежимПроведения, ТаблицаПоОплатамПоставщикам, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	ТаблицаДвижений = Движения.Взаиморасчеты.ВыгрузитьКолонки();
	...
	Если РежимПроведения = РежимПроведенияДокумента.Оперативный Тогда
		Движения.Взаиморасчеты.КонтрольОстатков(ЭтотОбъект, "ОплатаПоставщикам", СтруктураШапкиДокумента, Отказ, Заголовок, "СуммаВзаиморасчетов");
	КонецЕсли;
					
	Если Не Отказ Тогда
		Движения.Взаиморасчеты.ВыполнитьПриход();
	КонецЕсли;
...
Процедура ДвиженияПоРегиструТоварыОрганизаций(РежимПроведения, СтруктураШапкиДокумента, 
	ТаблицаПоТоварам,  ТаблицаПоВыданныеАвансы, ТаблицаПоОплатаПоставщикам, ТаблицаПоПрочее,
	Отказ, Заголовок);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СкладИзШапки    = (ВидПоступления <> Перечисления.ВидыПоступленияТоваров.НаСклад);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПроведениеДокументовКлиентСервер.ОчиститьДвиженияДокумента(Ссылка, "ВзаиморасчетыСПодотчетнымиЛицами");
	...
	//						ТаблицаПоТоварам,  ТаблицаПоАвансам, ТаблицаПоОплате, ТаблицаПоПрочее,
	//						Отказ, Заголовок);
	//КонецЕсли;
	
	лПараметры = Новый Структура;
	лПараметры.Вставить("РежимПроведения", РежимПроведения);
	лПараметры.Вставить("ТаблицаПоТоварам", ТаблицаПоТоварам);
	лПараметры.Вставить("СтруктураШапкиДокумента", СтруктураШапкиДокумента);
	лПараметры.Вставить("ТаблицаПоПрочее", ТаблицаПоПрочее);
	лПараметры.Вставить("ТаблицаПоАвансам", ТаблицаПоАвансам);
	лПараметры.Вставить("ТаблицаПоОплате", ТаблицаПоОплате);

	
	Документы.АвансовыйОтчет.ВыполнитьПроведение(Ссылка, Отказ, лПараметры);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Документы.АвансовыйОтчет.ВыполнитьОтменуПроведения(Ссылка, Отказ);
КонецПроцедуры
