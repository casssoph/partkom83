...
Процедура ПроставитьПолныйОтказНаДокумент(ДокументСсылка,ПричинаОтказа = Неопределено, ДопКомментарий = "") Экспорт //Семенов И.П. 28.12.2018 добавил ДопКомментарий = ""
	...
	ВыборкаСтроккЗакрытию = ВыбратьДанныеКЗакрытию(ПоследняяКорректировка);
...
Функция ВыбратьДанныеКЗакрытию(ДокументСсылка)
ЗапросДанныхкОтмене =  новый Запрос ;
	...
	Возврат ЗапросДанныхкОтмене.Выполнить().Выбрать();
