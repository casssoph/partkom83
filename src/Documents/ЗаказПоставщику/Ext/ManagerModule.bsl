...
Функция ДанныеДляВыгрузкиВОКПоЗаказам(МассивЗаказов)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДатаПриемки КАК ВТ_ДатаПриемки
...
Процедура ПроставитьПолныйОтказНаДокумент(ДокументСсылка,ПричинаОтказа = Неопределено) Экспорт
	...
	Документы.КорректировкаЗаказаПоставщику.ИзменитьКорректировкуЗаказа(ПоследняяКорректировка,СтруктураЗаполнения);
