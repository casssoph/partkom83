...
Функция ПолучитьМакетДляОкнаПоставщика(вхСсылкаНаДокумент) Экспорт
	...
	Если ВыполнитьЗапросПоОбъекту Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(, ПрайсПоставщика.Владелец = &Контрагент) КАК Цены
...
Процедура ЗагрузитьЭлемент(ОбъектXDTO, Отправитель, ОшибкаЗагрузкиПакета = Ложь) Экспорт
