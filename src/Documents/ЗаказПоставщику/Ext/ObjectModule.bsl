...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// ЛНА, Замер  APDEX ++(
	Попытка		
		APDEX_ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("ЗаказПоставщику_Проведение", СокрЛП(Ссылка), , Ссылка);
	Исключение
	КонецПопытки;
	//)--
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если СтатусДокумента = Справочники.СтатусыДокументов.ПолученОтветПоставщика И НЕ ЗначениеЗаполнено(ДатаПодтверждения) тогда
		ДатаПодтверждения = ДатаПодтверждения;
	КонецЕсли;
	// - 20180918 Пушкин XX-628
