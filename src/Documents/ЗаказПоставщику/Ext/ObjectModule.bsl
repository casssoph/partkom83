...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//#XX-2936 Kalinin V.A. ( 2019-08-05 )  /*
	ЗаполнитьТипЗаказа();	
	// */
...
Процедура ЗаполнитьТипЗаказа()
	лКлючАлгоритма = "Документ_ЗаказПоставщику_МодульОбъекта_ЗаполнитьТипЗаказа";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2851) Тогда
		//Бестолковый и вредный реквизит, для определения валютого заказа достаточно валюты документа, но раз надо, значит надо...
		Если   ВалютаДокумента = ПараметрыСеанса.ВалютаРубль тогда 
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента,"ВидДоговора")  = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение") тогда 
				ТипЗаказа = ПредопределенноеЗначение("Перечисление.ТипыЗаказаПоставщику.VMI");
			иначе 
				ТипЗаказа = ПредопределенноеЗначение("Перечисление.ТипыЗаказаПоставщику.Обычный");		
			КонецЕсли;	
		Иначе 
			ТипЗаказа = ПредопределенноеЗначение("Перечисление.ТипыЗаказаПоставщику.Импорт");		
		КонецЕсли;	
		//ТипЗаказа = ?(ВалютаДокумента = ПараметрыСеанса.ВалютаРубль, Перечисления.ТипыЗаказаПоставщику.Обычный, Перечисления.ТипыЗаказаПоставщику.Импорт);		
	Иначе  
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента,"ВидДоговора")  = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.ОтветХранение") тогда 
			ТипЗаказа = ПредопределенноеЗначение("Перечисление.ТипыЗаказаПоставщику.VMI");
		иначе 
