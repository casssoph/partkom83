...
Процедура КонтрольЗаполнениеПриЗаписиДокумента(Отказ, РежимЗаписи,Ошибки = Неопределено)
	...
	Если  РежимЗаписи = РежимЗаписиДокумента.Проведение
		И ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ОтветХранение Тогда
		...
		Запрос.УстановитьПараметр("ОсновнаяТТ", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ОсновнаяТорговаяТочка"));
		Запрос.УстановитьПараметр("Склад", Склад);
		
		Результат = Запрос.Выполнить();
		
		Если Результат.Пустой() Тогда
			ТекстОшибки = "Для основной торговой точки поставщика """+Контрагент+""" по складу """+Склад+""" не найдено прайса поставщика!";
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,,ТекстОшибки);
		КонецЕсли;
