...
Функция СформироватьСтруктуруПараметровАвторазмещения() Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ?(ЗначениеЗаполнено(ПоставщикЗаказовДругой), ДругойПоставщик, Контрагент));
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
...
Процедура СоздатьВиртуальныеЗаявкиПоНеРазмещеннымСтрокам()
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Т.Номенклатура,
	|	Т.СтрокаПрихода,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПроведениеДокументовКлиентСервер.ОчиститьДвиженияПриСдвигеДаты(ЭтотОбъект, РежимЗаписи, "ТоварыНаСкладах,ЦеныНоменклатуры");
	...
	Иначе 	
		...
		СкладVMI = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "СкладVMI");
		 Если Не (((ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Или ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ОтветХранение) И СкладVMI)
			Или (Не (ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Или ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ОтветХранение) И Не СкладVMI)) Тогда 
			Отказ = Истина;
	...
	Если (Не (ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Или ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.Прочее) 
		...
		ИЛИ (РежимЗаписи = РежимЗаписиДокумента.Проведение И СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровОтгружен) Тогда
		
		Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(Контрагент, "ЗаказРавенПриходу") Тогда
		...
		Если ЗначениеЗаполнено(ДокументОснование) И ОбщегоНазначения.ПолучитьЗначениеРеквизита(Контрагент, "ЗаказРавенПриходу") Тогда
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 	
		мДата = ?(ЗначениеЗаполнено(ДатаВходящегоДокумента), ДатаВходящегоДокумента, Дата);
	...
	РаспределитьСкидкиПриОтчетеПоставщику();
	...
	Если СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят Или
		...
		КонецЦикла;
	КонецЕсли;
	Если СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровРазмещен
		И ВидОперацииПоступления <> Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Тогда
		Если Не Товары.Итог("Количество")=Товары.Итог("КоличествоРазмещено") Тогда 
			Отказ=Истина;
			Сообщить("Нельзя записать документ в статусе РАЗМЕЩЕН если не все строки размещены!");
			Возврат
		КонецЕсли;
	КонецЕсли;
	
	Если ЭтоНовый() И Не (ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Или ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.Прочее) Тогда 
...
Процедура ПриЗаписи(Отказ)
	...
	Если НастройкаFTP <> Неопределено Тогда
		Если СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят Тогда
			...
			КонецЕсли;
		ИначеЕсли СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровЗачетVMI Тогда
			Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ИсторияОбмена1с8FTP.Объект,
			|	ИсторияОбмена1с8FTP.ДатаОбмена,
...
Процедура РаспределитьСкидкиПриОтчетеПоставщику()
	
	Если ОбменДанными.Загрузка Или ВидОперацииПоступления <> Перечисления.ВидыПоступленияТоваров.ЗачетТовараVMI Тогда 
		Возврат;
	КонецЕсли;
	...
	Если Выборка.ВидСкидки = Перечисления.ВидыСкидок.БезОграниченияПоОбщейСумме Тогда 
		Для Каждого СтрокаТЧ Из Товары Цикл 
			...
			СтрокаТЧ.Сумма =  СтрокаТЧ.Количество * СтрокаТЧ.Цена - СтрокаТЧ.Количество * СтрокаТЧ.Цена * СтрокаТЧ.ПроцентСкидкиНаценки/100;
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ИЗ
		               |	РегистрНакопления.СкидкиVMI.Остатки(&КонПериода, Контрагент = &Контрагент) КАК СкидкиVMIОстатки
		               |ГДЕ
		...
		Пока Сумма > 0 И Индекс < Товары.Количество() Цикл 
			...
			ВозможнаяСуммаСкидки = СтрокаТЧ.Цена * СтрокаТЧ.Количество * Выборка.ПроцентСкидки/100;
