...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// 19.11.18 Строганов Роман > XX-74 Исправление ситуации, когда итог документа рассчитывался только при записи из формы.
	Если РежимЗаписи <> РежимЗаписиДокумента.ОтменаПроведения И ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.ОтветХранение Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(&Период, ) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних
		|		ПО НоменклатураКонтрагентов.Ссылка = ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура
		|ГДЕ
		|	НоменклатураКонтрагентов.Номенклатура В(&СписокНоменклатуры)"; 
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Для каждого СтрокаТовары Из Товары Цикл
				Если Не ЗначениеЗаполнено(СтрокаТовары.Цена) Или Не ЗначениеЗаполнено(СтрокаТовары.Сумма) Тогда
					ЦенаНоменклатуры = СоответствиеЦен[СтрокаТовары.Номенклатура];
					Если ЦенаНоменклатуры <> Неопределено Тогда
						Если УчитыватьНДС И СуммаВключаетНДС Тогда 
							СуммаНДС = ЦенаНоменклатуры * ОбщегоНазначенияКлиентСервер.ПолучитьНДС(СтрокаТовары.Номенклатура.СтавкаНДС);
							СтрокаТовары.Цена = ЦенаНоменклатуры + СуммаНДС;
						Иначе
							СтрокаТовары.Цена = ЦенаНоменклатуры;
						КонецЕсли;
						СтрокаТовары.Сумма = (СтрокаТовары.Количество - СтрокаТовары.КоличествоНеПринято) * СтрокаТовары.Цена;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла; 
		КонецЕсли;
