...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Иначе 	
		...
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение И Не (Склад.Бонусный Или Склад.Сувенирный) И НЕ КорректировкаПоставщиком Тогда 		 
...
Функция ВыгрузитьВОбменТоплог() Экспорт
	...
	//_Товары = Товары.Выгрузить(, "СтрокаПрихода,Количество");
	//_Товары.Свернуть("СтрокаПрихода", "Количество");
	//
	//_Разм = РазмещениеСтрокПрихода.Выгрузить(, "СтрокаПрихода,Количество");
