...
Процедура ВыполнитьПроведение(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если ПроведениеДокументовКлиентСервер.ПроводитсяПо(вхПараметры, "РазмещенияСтрокЗаказов") тогда                   
		...
		Если ТаблицаДвиженийРазмещения.Количество() тогда 
			РеквизитыДокумента =  ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Контрагент,ДокументОснование");
...
Процедура ДополнитьНоменклатуруПоЗаказу(ТаблицаТоваров,ЗаказПоставщику)
	...
	|			И (ВтТовары.НоменклатураЗамены = ЗНАЧЕНИЕ(Справочник.Номенклатура.Пустаяссылка))
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА НЕ ВтТовары.НоменклатураЗамены = ЗНАЧЕНИЕ(Справочник.Номенклатура.Пустаяссылка)
	|			ТОГДА ВтТовары.НоменклатураЗамены
...
Процедура ДополнитьНоменклатуруПоОстаткамЗаказов(ТаблицаТоваров,ТорговаяТочка,Склад)
	...
	Запрос  = новый запрос("ВЫБРАТЬ
                         ...
                         |							ИЛИ ВтТовары.НоменклатураЗамены = ЗНАЧЕНИЕ(Справочник.Номенклатура.Пустаяссылка)))) КАК ЗаказыПоставщикамОстатки
                         ...
                         |	ВТНайденаяНоменклатура.НоменклатураЗамены
                         |ИЗ
                         |	ВТНайденаяНоменклатура КАК ВТНайденаяНоменклатура
