...
Функция РегистрыСведений_ЦеныНоменклатуры(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСсылкаНаДокумент, "ЭтоМФП")
		ИЛИ НЕ (ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСсылкаНаДокумент, "СтатусДокумента") = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят
		ИЛИ ОбщегоНазначения.ПолучитьЗначениеРеквизита(вхСсылкаНаДокумент, "СтатусДокумента") = Справочники.СтатусыДокументов.ПоступлениеТоваровРазмещен) Тогда
		Возврат ТабЦен;
	КонецЕсли;
...
Функция ПечатьМХ3(текДокумент) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ДШ.Организация КАК Хранитель,
	|	ДШ.Организация.КодПоОКПО КАК ХранительОКПО,
...
Процедура ЗагрузитьЭлемент(ОбъектXDTO, вхОтправитель, Отказ, вхПараметры = Неопределено) Экспорт
	...
	ИначеЕсли (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog) Тогда 
		Попытка
			ЗагрузитьРезультатПриемки2(ОбъектXDTO);
		Исключение
...
//Процедура ЗагрузитьРезультатПриемки(ОбъектXDTO)
//	
...
//			Если Не ЗначениеЗаполнено(НоваяСтрока.СтрокаЗаявки) Тогда 
...
//	КоличествоПереданное = Товары.Итог("Количество");
//	
//	Запрос = Новый Запрос;
//	Запрос.Текст = "ВЫБРАТЬ
...
//	               |		ЛЕВОЕ СОЕДИНЕНИЕ втОтказыПоБраку КАК втОтказыПоБраку
//	               |		ПО втРазм.СтрокаПрихода = втОтказыПоБраку.СтрокаПрихода
//	               |			И втРазм.СтрокаЗаявки = втОтказыПоБраку.СтрокаЗаявки
//	               |ГДЕ
//	               |	ПоступлениеТоваровУслугТовары.Ссылка = &Ссылка
//	               |;
//	               |
...
//	               |ВЫБРАТЬ
//	               |	ПоступлениеТоваровУслугТовары.Номенклатура,
//	               |	СУММА(втОтказыПоБраку.Количество) КАК Количество
//	               |ИЗ
//	               |	втОтказыПоБраку КАК втОтказыПоБраку
//	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
//	               |		ПО втОтказыПоБраку.СтрокаПрихода = ПоступлениеТоваровУслугТовары.СтрокаПрихода
//	               |ГДЕ
//	               |	ПоступлениеТоваровУслугТовары.Ссылка = &Ссылка
//	               |
//	               |СГРУППИРОВАТЬ ПО
...
//		ТоварыДокумента = Результаты[Результаты.ВГраница() - 1].Выгрузить();
...
//		КонецЕсли;
//		
//		ТоварыДокументаБрак = Результаты[Результаты.ВГраница()].Выгрузить();
//		
//		Запрос = Новый Запрос;
//		Запрос.Текст = "ВЫБРАТЬ
...
//		               |	И (ПоступлениеТоваровУслуг.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ПоступлениеТоваровПринят)
//		               |			ИЛИ ПоступлениеТоваровУслуг.СтатусДокумента = ЗНАЧЕНИЕ(Справочник.СтатусыДокументов.ПоступлениеТоваровРазмещен))
...
//		
//		Дельта = Неопределено; 
//		ДельтаБрак = Неопределено;
//		НужноМенятьТЧ1 = Не РаботаСПоследовательностямиКлиентСервер.ТаблицыИдентичныНовое(ТоварыДокумента, Товары, Дельта, "Количество");
//		НужноМенятьТЧ2 = Не РаботаСПоследовательностямиКлиентСервер.ТаблицыИдентичныНовое(ТоварыДокументаБрак, ТоварыБрак, ДельтаБрак, "Количество");
//		
//		Запрос = Новый Запрос;
//		Запрос.Текст =  "ВЫБРАТЬ
...
//		КонецЦикла;
//		
//		ДельтаАктаПриемки = Неопределено;
//		НужноМенятьАктПриемки = Не РаботаСПоследовательностямиКлиентСервер.ТаблицыИдентичныНовое(ДокументАктПриемки, АктПриемки, ДельтаАктаПриемки, "КоличествоПлан,Количество,КоличествоНедостача,КоличествоБрак,КоличествоИзлишек,КоличествоВернули");

//		Если НужноМенятьШапку Или НужноМенятьТЧ1 Или НужноМенятьТЧ2 Или НужноМенятьАктПриемки Тогда 
//			ДокОбъект = ДокСсылка.ПолучитьОбъект();
//			//ДокОбъект.ПричиныОтказов.Очистить();
//			//Для Каждого СтрокаТовары Из Товары Цикл 
//			//	СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаТовары.СтрокаЗаявки));		
//			//	
//			//КонецЦикла;
//			ДокОбъект.ПричиныОтказов.Очистить();
//			//Для Каждого СтрокаТЧ Из ДокОбъект.Товары Цикл 
//			//	СтрокаТЧ.КоличествоНеПринято = СтрокаТЧ.Количество;
//			//КонецЦикла;
//			
...
//				Отбор = Новый Структура("СтрокаЗаявки,Номенклатура", СтрокаТЧ.СтрокаЗаявки, СтрокаТЧ.Номенклатура);
//				СтрокиРазм = РазмещенныеТовары.НайтиСтроки(Отбор);
//				КоличествоРаспределить = СтрокаТЧ.Количество;
//				Индекс = 0;
//				Пока КоличествоРаспределить > 0 И Индекс < СтрокиРазм.Количество() Цикл 
//					Строка = СтрокиРазм.Получить(Индекс);
//					Если Строка.Количество > 0 Тогда 
//						СписываемоеКоличество = Мин(КоличествоРаспределить, Строка.Количество);
//						НоваяСтрока = ПринятыеТовары.Добавить();
...
//					ВызватьИсключение "Некорректные данные из ТопЛог. Либо принята не та номенклатура по SSID, либо принято SSID, по которому нет размещения в документе, либо принято больше, чем требуется. SSID = " + СтрокаТЧ.СтрокаЗаявки.IDSite;
//				КонецЕсли;
//			КонецЦикла;
//			
//			//Для Каждого СтрокаТовары Из Товары Цикл 
...
//			//	Индекс = 0;
//			//	
//			//	Пока КоличествоРаспределить > 0 И Индекс < СтрокиРазмещения.Количество() Цикл 
//			//		СтрокаРазм = СтрокиРазмещения.Получить(Индекс);
//			//		СтрокаТЧ = ДокОбъект.Товары.Найти(СтрокаРазм.СтрокаПрихода, "СтрокаПрихода");
//			//		Если СтрокаТЧ.Номенклатура = СтрокаТовары.Номенклатура Тогда 
//			//			
//			//			КоличествоРаспределитьСтрока = Мин(СтрокаТЧ.КоличествоНеПринято, СтрокаРазм.Количество, КоличествоРаспределить);
//			//			
//			//			СтрокаТЧ.КоличествоНеПринято = СтрокаТЧ.КоличествоНеПринято - КоличествоРаспределитьСтрока;
//			//			КоличествоРаспределить = КоличествоРаспределить - КоличествоРаспределитьСтрока;
//			//			//МассивОбработанныхНомеровСтрок.Добавить(СтрокаТЧ.НомерСтроки);
//			//		КонецЕсли;
//			//		Индекс = Индекс + 1;
//			//	КонецЦикла;
//			//	
//			//	Если КоличествоРаспределить > 0 Тогда 
//			//		ВызватьИсключение "Не удалось распределить номенклатуру " + СтрокаТовары.Номенклатура + " и SSID " + СтрокаТовары.СтрокаЗаявки.IDSite;
//			//	КонецЕсли;
//			//КонецЦикла;
//			
//			ОчиститьТаблицуСНулевымКол(РазмещенныеТовары);

//			
//			//// устанавливаем КоличествоНеПринято для не обработанных строк
//			//Для Каждого СтрокаТЧ Из ДокОбъект.Товары Цикл 
//			//	Если МассивОбработанныхНомеровСтрок.Найти(СтрокаТЧ.НомерСтроки) = Неопределено Тогда 
//			//		СтрокаТЧ.КоличествоНеПринято = СтрокаТЧ.Количество;
//			//	КонецЕсли;
//			//КонецЦикла;
//			
//			// заполняем брак
//			Для Каждого СтрокаТовары Из ТоварыБрак Цикл 
//				Строки = ДокОбъект.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТовары.Номенклатура));
//				
//				КоличествоРаспределить = СтрокаТовары.Количество;
//				Индекс = 0;
//				
...
//					
//					МожноДобавитьОтказов = Мин(КоличествоРаспределить, СтрокаТЧ.Количество - КоличествоПринято);
//					Если МожноДобавитьОтказов > 0 Тогда 
//						СтрокиРазм = РазмещенныеТовары.НайтиСтроки(Новый Структура("СтрокаПрихода", СтрокаТЧ.СтрокаПрихода));
//						Индекс2 = 0;
//						Пока МожноДобавитьОтказов > 0 И Индекс2 < СтрокиРазм.Количество() Цикл 
//							СтрокаРазм = СтрокиРазм.Получить(Индекс2);
//							СписываемоеКоличество = Мин(МожноДобавитьОтказов, СтрокаРазм.Количество);
//							Если СписываемоеКоличество > 0 Тогда 
//								МожноДобавитьОтказов = МожноДобавитьОтказов - СписываемоеКоличество;
//								СтрокаРазм.Количество = СтрокаРазм.Количество - СписываемоеКоличество;
//								КоличествоРаспределить = КоличествоРаспределить - СписываемоеКоличество;
//								
//								НоваяСтрока = ДокОбъект.ПричиныОтказов.Добавить();
//								ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ, "КлючСвязи,СтрокаПрихода");
//								НоваяСтрока.СтрокаЗаявки = СтрокаРазм.СтрокаЗаявки;
//								НоваяСтрока.Количество = СписываемоеКоличество;
//								НоваяСтрока.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке;
//							КонецЕсли;	
//							Индекс2 = Индекс2 + 1;
//						КонецЦикла;
//						Если МожноДобавитьОтказов > 0 Тогда 
//							НоваяСтрока = ДокОбъект.ПричиныОтказов.Добавить();
//							ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ, "КлючСвязи,СтрокаПрихода");
//							НоваяСтрока.Количество = МожноДобавитьОтказов;
//							НоваяСтрока.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке;
//							КоличествоРаспределить = КоличествоРаспределить - МожноДобавитьОтказов;
//							МожноДобавитьОтказов = 0;
//						КонецЕсли;
//					КонецЕсли;
//					
//					//СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаПрихода", СтрокаТЧ.СтрокаПрихода));		
//					//КоличествоРаспределитьСтрока = Мин(СтрокаТЧ.КоличествоНеПринято, КоличествоРаспределить); 
//					//ИндексСтрока = 0;
//					//Пока КоличествоРаспределитьСтрока > 0 И ИндексСтрока < СтрокиРазмещения.Количество() Цикл 
//					//	СтрокаРазм = СтрокиРазмещения.Получить(ИндексСтрока);	
//					//	
//					//	КоличествоДобавитьОтказов = Мин(КоличествоРаспределитьСтрока, СтрокаРазм.Количество);
//					//	
//					//	СтрокаОтказа = ДокОбъект.ПричиныОтказов.Добавить();
//					//	ЗаполнитьЗначенияСвойств(СтрокаОтказа,СтрокаТЧ, "КлючСвязи,СтрокаПрихода");
//					//	СтрокаОтказа.СтрокаЗаявки = СтрокаРазм.СтрокаЗаявки;
//					//	СтрокаОтказа.Количество = КоличествоДобавитьОтказов;
//					//	СтрокаОтказа.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке;
//					//	
//					//	КоличествоРаспределитьСтрока = КоличествоРаспределитьСтрока - КоличествоДобавитьОтказов;
//					//	КоличествоРаспределить = КоличествоРаспределить - КоличествоДобавитьОтказов;
//					//	
...
//				КонецЦикла;
//				Если КоличествоРаспределить > 0 Тогда 
//					ВызватьИсключение "Из ТопЛог переданы некорректные данные. брак + количество факт > количество план";
//				КонецЕсли;
//			КонецЦикла;
//			
//			Для Каждого СтрокаТЧ Из ДокОбъект.Товары Цикл 
...
//				НоваяСтрока.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаНедостача;
//			КонецЦикла;
//			
//			// устанавливаем недостачи на неразм. товар
//			Для Каждого СтрокаТЧ Из ДокОбъект.Товары Цикл 
//				Если СтрокаТЧ.КоличествоНеПринято > 0 Тогда 
//					КоличествоОтказаноПоБраку = 0;
//					Строки = ДокОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("СтрокаПрихода", СтрокаТЧ.СтрокаПрихода));
//					Для Каждого СтрокаОтказа из Строки Цикл 
//						КоличествоОтказаноПоБраку = КоличествоОтказаноПоБраку + СтрокаОтказа.Количество;
//					КонецЦикла;
//					
//					Если СтрокаТЧ.КоличествоНеПринято - КоличествоОтказаноПоБраку > 0 Тогда 
//						СтрокаОтказа = ДокОбъект.ПричиныОтказов.Добавить();
//						СтрокаОтказа.КлючСвязи = СтрокаТЧ.КлючСвязи;
//						СтрокаОтказа.СтрокаПрихода = СтрокаТЧ.СтрокаПрихода;
//						СтрокаОтказа.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаНедостача;
//						СтрокаОтказа.Количество = МожноДобавитьОтказов;
//						КоличествоРаспределить = СтрокаТЧ.КоличествоНеПринято - КоличествоОтказаноПоБраку
//					КонецЕсли;
//				КонецЕсли;
//			КонецЦикла;
//			
//			//Для Каждого СтрокаДельты Из Дельта Цикл 
//			//	Если СтрокаДельты.КоличествоБрак > 0 Тогда  // Добавить отказы по браку
//			//		КоличествоДобавитьОтказов = СтрокаДельты.КоличествоБрак;
//			//		СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаДельты.СтрокаЗаявки));		
//			//		Индекс = 0;
//			//		Пока КоличествоДобавитьОтказов > 0 И Индекс < СтрокиРазмещения.Количество() Цикл  
//			//			СтрокаРазм = СтрокиРазмещения.Получить(Индекс);
//			//			СтрокаТовары = ДокОбъект.Товары.Найти(СтрокаРазм.СтрокаПрихода, "СтрокаПрихода");
//			//			
//			//			Если СтрокаТовары.Номенклатура <> СтрокаДельты.Номенклатура Тогда 
//			//				Индекс = Индекс + 1; //Прислали замену
//			//				Продолжить;
//			//			КонецЕсли;
//			//			
//			//			КоличествоРазмещено = СтрокаРазм.Количество;
//			//			СтрокиОтказов = ДокОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("СтрокаЗаявки, СтрокаПрихода", СтрокаДельты.СтрокаЗаявки, СтрокаРазм.СтрокаПрихода));
//			//			ВремТаблица = ДокОбъект.ПричиныОтказов.Выгрузить(СтрокиОтказов, "Количество");
//			//			ВремТаблица.Свернуть(,"Количество");
//			//			Если ВремТаблица.Количество() = 0 Тогда 
//			//				КоличествоОтказано = 0;
//			//			Иначе
//			//				КоличествоОтказано = ВремТаблица.Получить(0).Количество;
//			//			КонецЕсли;
//			//			
//			//			МожноДобавитьОтказов =  Мин(КоличествоРазмещено - КоличествоОтказано, КоличествоДобавитьОтказов);
//			//			
//			//			НоваяСтрокаОтказов = ДокОбъект.ПричиныОтказов.Добавить();
//			//			НоваяСтрокаОтказов.СтрокаПрихода = СтрокаРазм.СтрокаПрихода;
//			//			НоваяСтрокаОтказов.КлючСвязи = СтрокаТовары.КлючСвязи;
//			//			НоваяСтрокаОтказов.СтрокаЗаявки = СтрокаДельты.СтрокаЗаявки;
//			//			НоваяСтрокаОтказов.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке;
//			//			НоваяСтрокаОтказов.Количество = МожноДобавитьОтказов;
//			//			КоличествоДобавитьОтказов = КоличествоДобавитьОтказов - МожноДобавитьОтказов;
//			//			
//			//			Индекс = Индекс + 1;
//			//		КонецЦикла;
//			//	ИначеЕсли  СтрокаДельты.КоличествоБрак < 0 Тогда //Убираем отказы по браку
//			//		КоличествоУбратьОтказов = - СтрокаДельты.КоличествоБрак;
//			//		СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаДельты.СтрокаЗаявки));
//			//		Индекс = 0;
//			//		Пока КоличествоУбратьОтказов > 0 И Индекс < СтрокиРазмещения.Количество() Цикл
//			//			СтрокаРазм = СтрокиРазмещения.Получить(Индекс);
//			//			СтрокаТовары = ДокОбъект.Товары.Найти(СтрокаРазм.СтрокаПрихода, "СтрокаПрихода");
//			//			
//			//			Если СтрокаТовары.Номенклатура <> СтрокаДельты.Номенклатура Тогда 
//			//				Индекс = Индекс + 1; //Не та строка
//			//				Продолжить;
//			//			КонецЕсли;
//			//			
//			//			СтрокиОтказов = ДокОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("СтрокаЗаявки, СтрокаПрихода, ПричинаОтказа", СтрокаДельты.СтрокаЗаявки, СтрокаРазм.СтрокаПрихода, Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке));
//			//			Для Каждого СтрокаОтк Из СтрокиОтказов Цикл 
//			//				Если КоличествоУбратьОтказов = 0 Тогда 
//			//					Прервать;
//			//				КонецЕсли;
//			//				
//			//				МожноУбратьОтказов = Мин(КоличествоУбратьОтказов, СтрокаОтк.Количество);
//			//				
//			//				СтрокаОтк.Количество = СтрокаОтк.Количество - МожноУбратьОтказов;
//			//				КоличествоУбратьОтказов = КоличествоУбратьОтказов - МожноУбратьОтказов;
//			//				
//			//				НоваяСтрокаОтказов = ДокОбъект.ПричиныОтказов.Добавить();
//			//				НоваяСтрокаОтказов.СтрокаПрихода = СтрокаРазм.СтрокаПрихода;
//			//				НоваяСтрокаОтказов.КлючСвязи = СтрокаТовары.КлючСвязи;
//			//				НоваяСтрокаОтказов.СтрокаЗаявки = СтрокаДельты.СтрокаЗаявки;
//			//				НоваяСтрокаОтказов.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаНедостача;
//			//				НоваяСтрокаОтказов.Количество = МожноУбратьОтказов;
//			//											
//			//			КонецЦикла;
//			//			Индекс = Индекс + 1;
//			//		КонецЦикла;
//			//	КонецЕсли;
//			//	Если СтрокаДельты.КоличествоПринято < 0 Тогда  //Добавить отказы
//			//		КоличествоДобавитьОтказов = - СтрокаДельты.КоличествоПринято;
//			//		СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаДельты.СтрокаЗаявки));		
//			//		Индекс = 0;
//			//		Пока КоличествоДобавитьОтказов > 0 И Индекс < СтрокиРазмещения.Количество() Цикл  
//			//			СтрокаРазм = СтрокиРазмещения.Получить(Индекс);
...
//			//		КонецЦикла;
//			//	Иначе  //Убрать отказы
//			//		КоличествоУбратьОтказов = СтрокаДельты.КоличествоПринято;
//			//		СтрокиРазмещения = ДокОбъект.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаДельты.СтрокаЗаявки));
//			//		Индекс = 0;
//			//		Пока КоличествоУбратьОтказов > 0 И Индекс < СтрокиРазмещения.Количество() Цикл
//			//			СтрокаРазм = СтрокиРазмещения.Получить(Индекс);
...
//			//			КонецЕсли;
//			//			
//			//			СтрокиОтказов = ДокОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("СтрокаЗаявки, СтрокаПрихода, ПричинаОтказа", СтрокаДельты.СтрокаЗаявки, СтрокаРазм.СтрокаПрихода, Справочники.СостоянияСтрокДокументов.ОтказПоставщикаНедостача));
//			//			Для Каждого СтрокаОтк Из СтрокиОтказов Цикл 
//			//				Если КоличествоУбратьОтказов = 0 Тогда 
//			//					Прервать;
//			//				КонецЕсли;
...
//			//		КонецЦикла;
//			//		Если КоличествоУбратьОтказов > 0 Тогда 
//			//			//SSID не найден или номенклатура не та
//			//		КонецЕсли;
//			//	КонецЕсли;
//			//КонецЦикла;
//			
...
//			КонецЕсли;
//			
//			////Оставляем только отказы по браку и недостаче
//			//Для Каждого СтрокаОтк Из ДокОбъект.ПричиныОтказов Цикл 
//			//	Если Не(СтрокаОтк.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаНедостача Или 
//			//		СтрокаОтк.ПричинаОтказа = Справочники.СостоянияСтрокДокументов.ОтказПоставщикаБракПриПриемке) Тогда
//			//		СтрокаОтк.Количество = 0; // Удаление ниже
//			//	КонецЕсли;
//			//КонецЦикла;
//			
...
//			//КонецЦикла;
//			
//			////Установка колонки КоличествоНеПринято
//			//Для Каждого СтрокаТовары Из ДокОбъект.Товары Цикл 
//			//	СтрокиОтказов = ДокОбъект.ПричиныОтказов.НайтиСтроки(Новый Структура("СтрокаПрихода", СтрокаТовары.СтрокаПрихода));
//			//	ВремТаблица = ДокОбъект.ПричиныОтказов.Выгрузить(СтрокиОтказов, "Количество");
//			//	ВремТаблица.Свернуть(, "Количество");
//			//	КоличествоНеПринято = 0;
//			//	Если ВремТаблица.Количество() > 0 Тогда 
//			//		КоличествоНеПринято = ВремТаблица.Получить(0).Количество;
//			//	КонецЕсли;
//			//	СтрокаТовары.КоличествоНеПринято = КоличествоНеПринято;
//			//КонецЦикла; 
//			
...
Процедура ЗагрузитьРезультатПриемки2(ОбъектXDTO)
...
Функция СтруктураШапкиДокумента(ОбъектXDTO, ТаблицаОшибок, Отказ)
	...
	Структура.Вставить("СсылкаНаДокумент", СсылкаНаДокумент(Структура.НомерВходящегоДокумента, Структура.ДатаВходящегоДокумента, Структура.Организация, ТаблицаОшибок, Отказ));
	...
	//Структура.Вставить("СтатусДокумента", Справочники.СтатусыДокументов.ПоступлениеТоваровДоставлен);//
	//PaSe - изменено, что бы распределение по заказам проводилось заново//
	//бред, но так настаивают//
...
КонецФункции
Функция СсылкаНаДокумент(НомерВходящегоДокумента, ДатаВходящегоДокумента, Организация, ТаблицаОшибок, Отказ)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	                      |ГДЕ
	                      |	ПоступлениеТоваровУслуг.НомерВходящегоДокумента = &Номер
	                      |	И ПоступлениеТоваровУслуг.ДатаВходящегоДокумента = &Дата
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Проведен Тогда
			СообщениеОбОшибке = "Выгруженное из окна поставщика поступление уже загружено и проведено в 1С (заказы распределены). (number: " + НомерВходящегоДокумента +
								", document_date: " + ДатаВходящегоДокумента + ")";
			СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке)
		КонецЕсли;
		Если Выборка.ДокументЗагружен Тогда
			Отказ = Истина;
			СообщениеОбОшибке = "Документ (number: " + НомерВходящегоДокумента + ", document_date: " + ДатаВходящегоДокумента + ") уже загружен и не может быть изменен";
			СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке)
		КонецЕсли;
...
Процедура РаспределитьПоступлениеВОдинЗаказ(ШапкаДокумента, ТаблицаПоступления, РазмещениеСтрокПрихода, ТаблицаОшибок, Отказ)
	...
	Если НЕ ПоступлениеРаспределено Тогда
		Отказ = Истина;
		СообщениеОбОшибке = "Нет подходящего заказа для распределения поступления в один заказ. (number:" + ШапкаДокумента.НомерВходящегоДокумента + ")";
	...
	Если НЕ ЗначениеЗаполнено(обДокумент.ДокументОснование) Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = "Ошибка при проведении документа: " + обДокумент + 
								" (document_date:" + ШапкаДокумента.ДатаВходящегоДокумента + ", number:" + ШапкаДокумента.НомерВходящегоДокумента + ") - (" + ОписаниеОшибки + ")";
			СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗаписатьОшибкиЗагрузки(ШапкаДокумента, ТаблицаОшибок)
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		...
		ТаблицаОшибок.ЗаполнитьЗначения(ШапкаДокумента.НомерВходящегоДокумента, "number");
		ТаблицаОшибок.ЗаполнитьЗначения(ШапкаДокумента.ДатаВходящегоДокумента, "document_date");
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_ОкноПоставщика) тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ПоступлениеТоваровУслуг.НомерВходящегоДокумента КАК number,
	...
	ИначеЕсли лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		|	ЕСТЬNULL(ПоступлениеТоваровУслугРазмещениеСтрокПрихода.СтрокаЗаявки.Заявка.ТорговаяТочка, ЗНАЧЕНИЕ(Справочник.ТорговыеТочки.ПустаяСсылка)) КАК Клиент,
		|	ЕСТЬNULL(ПоступлениеТоваровУслугРазмещениеСтрокПрихода.СтрокаЗаявки.Заявка.ТорговаяТочка.Город, ЗНАЧЕНИЕ(Справочник.Города.ПустаяСсылка)) КАК Город,
		|	ПоступлениеТоваровУслугРазмещениеСтрокПрихода.Количество,
