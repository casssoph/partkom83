...
//Функция ПечатьТорг12Поставщика(текДокумент) Экспорт
//	Перем ПодразделениеОтветственныхЛиц;
//	Перем ЕдиницаИзмеренияВеса;
//	
...
//	
//	// разные настройки
//	чВысотаРазделителей = 6;
//	
//	Запрос = Новый Запрос(
//	"ВЫБРАТЬ
//	|	Д.Номер,
//	|	Д.Дата КАК ДатаДокумента,
//	|	Д.Контрагент КАК Организация,
//	|	Д.Контрагент КАК ЮрФизЛицо,
//	|	Д.Контрагент КАК Поставщик,
//	|	Д.Контрагент КАК Контрагент,
//	|	Д.Контрагент КАК Руководители,
//	|	ЕСТЬNULL(КИ.Представление, """") КАК АдресДоставки,
//	|	ВЫБОР
//	|		КОГДА Д.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
//	|			ТОГДА Д.Организация
//	|		ИНАЧЕ Д.Грузополучатель
//	|	КОНЕЦ КАК Грузополучатель,
//	|	ВЫБОР
//	|		КОГДА Д.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
//	|			ТОГДА Д.Контрагент
//	|		ИНАЧЕ Д.Грузоотправитель
//	|	КОНЕЦ КАК Грузоотправитель,
//	|	Д.БанковскийСчетКонтрагента КАК БанковскийСчет,
//	|	Д.Организация КАК Покупатель,
//	|	Д.Организация КАК Плательщик,
//	|	Д.ДокументОснование КАК Сделка,
//	|	Д.ДоговорКонтрагента.Представление КАК Основание,
//	|	"""" КАК ВедениеВзаиморасчетов,
//	|	ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка) КАК ОтветственноеЛицо,
//	|	"""" КАК Подразделение,
//	|	Д.ВалютаДокумента,
//	|	Д.КурсВзаиморасчетов КАК Курс,
//	|	Д.КратностьВзаиморасчетов КАК Кратность,
//	|	Д.УчитыватьНДС,
//	|	Д.СуммаВключаетНДС,
//	|	"""" КАК ОтпускРазрешил,
//	|	"""" КАК ОтпускПроизвел,
...
//	|		ПО Д.Организация = КИ.Объект
//	|ГДЕ
//	|	Д.Ссылка = &ТекущийДокумент
//	|	И КИ.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Адрес)
//	|	И КИ.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ФактАдресОрганизации)"
//	);
//	Запрос.УстановитьПараметр("ТекущийДокумент", текДокумент);
//	
//	Шапка = Запрос.Выполнить().Выбрать();
...
//	|		ПоступлениеТоваровУслуг.СтавкаНДС КАК СтавкаНДС,
//	|		ПоступлениеТоваровУслуг.Цена КАК Цена,
//	|		ПоступлениеТоваровУслуг.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
//	|		СУММА(ПоступлениеТоваровУслуг.Количество) КАК Количество,
...
//	|		ПоступлениеТоваровУслуг.СтавкаНДС,
//	|		ПоступлениеТоваровУслуг.Цена,
//	|		ПоступлениеТоваровУслуг.ПроцентСкидкиНаценки,
//	|		ПоступлениеТоваровУслуг.ЕдиницаИзмерения,
//	|		ПоступлениеТоваровУслуг.ЕдиницаИзмерения.Коэффициент) КАК ВложенныйЗапрос";
//	
//	тзТовары = Запрос.Выполнить().Выгрузить();
//		
//	КоличествоСтрок = тзТовары.Количество();
//	
//	Для каждого стзТовары Из тзТовары Цикл
//		Если стзТовары.Количество = 0 Тогда
//			Если Шапка.СуммаВключаетНДС Тогда
//				чМаксЦена = Макс(чМаксЦена, стзТовары.Сумма - стзТовары.СуммаНДС);
//			Иначе
//				чМаксЦена = Макс(чМаксЦена, стзТовары.Сумма);
//			КонецЕсли;
//		Иначе
//			Если Шапка.СуммаВключаетНДС Тогда
//				чМаксЦена = Макс(чМаксЦена, (стзТовары.Сумма - стзТовары.СуммаНДС) / стзТовары.Количество);
//			Иначе
//				чМаксЦена = Макс(чМаксЦена, стзТовары.Сумма / стзТовары.Количество);
//			КонецЕсли;
//		КонецЕсли;
//	КонецЦикла;
//	
...
//	облВерх = Макет.ПолучитьОбласть("Верх1");
//	облВерх.Параметры.Заполнить(Шапка);
//	облВерх.Параметры.НомерДокумента = ОбщегоНазначения.ПолучитьНомерНаПечать(текДокумент);
//	облВерх.Параметры.ДатаДокумента  = Формат(Шапка.ДатаДокумента, "ДФ=dd.MM.yyyy");

//	Если Шапка.ЮрФизЛицо = Шапка.Грузоотправитель Тогда
//		облВерх.Параметры.Грузоотправитель1 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "ПолноеНаименование,ЮридическийАдрес");
//		облВерх.Параметры.Грузоотправитель2 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "Телефоны,НомерСчета,Банк,БИК,КоррСчет,ИНН");
//		облВерх.Область("Грузоотправитель1").ЦветТекста = WebЦвета.Синий;
//	Иначе
//		облВерх.Параметры.Грузоотправитель1 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузоотправителе, "ПолноеНаименование,ФактическийАдрес");
//		облВерх.Параметры.Грузоотправитель2 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузоотправителе, "Телефоны,НомерСчета,Банк,БИК,КоррСчет,ИНН");
//	КонецЕсли;

//	облВерх.Параметры.Подразделение = Шапка.Подразделение;
//	
//	Если СокрЛП(Шапка.АдресДоставки) <> "" Тогда
//		облВерх.Параметры.Грузополучатель1 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе,"ПолноеНаименование") + ", " + Шапка.АдресДоставки;
//	Иначе
//		облВерх.Параметры.Грузополучатель1 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе,"ПолноеНаименование,ФактическийАдрес");
//	КонецЕсли;
//	облВерх.Параметры.Грузополучатель2 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе, "Телефоны,НомерСчета,Банк,БИК,КоррСчет,ИНН");
//		
//	Если Шапка.Грузополучатель = Шапка.Покупатель И СокрЛП(Шапка.АдресДоставки) = "" Тогда
//		// грузополучатель неактуален
//		облВерх.Область("ГрузополучательНадпись").ЦветТекста = WebЦвета.Синий;
//		облВерх.Область("Грузополучатель1").ЦветТекста = WebЦвета.Синий;
//	КонецЕсли;

//	облВерх.Параметры.Поставщик1  = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "ПолноеНаименование,ЮридическийАдрес");
//	облВерх.Параметры.Поставщик2  = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "Телефоны,НомерСчета,Банк,БИК,КоррСчет,ИНН");
//		
//	облВерх.Параметры.Плательщик1 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,ЮридическийАдрес");
//	облВерх.Параметры.Плательщик2 = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "Телефоны,НомерСчета,Банк,БИК,КоррСчет,ИНН");
...
//	//		облВерх.Параметры.Основание = Шапка.Основание;
//	//Иначе
//	//	Если ЗначениеЗаполнено(Шапка.Сделка) Тогда
//	//		облВерх.Параметры.Основание = фБиблиотека.ОбщегоНазначенияСформироватьЗаголовокДокумента(Шапка.Сделка, Строка(Шапка.Сделка.Метаданные().Синоним));
//	//	КонецЕсли;
//	//КонецЕсли;
//	
//	// Выводим всевозможные коды
//	облВерх.Параметры.ГрузоотправительОКПО = СведенияОГрузоотправителе.КодПоОКПО;
//	облВерх.Параметры.ГрузополучательОКПО = СведенияОГрузополучателе.КодПоОКПО;
//	облВерх.Параметры.ПлательщикОКПО      = СведенияОПокупателе.КодПоОКПО;
//	
//	//облВерх.Параметры.ОКДП = "";
//	облВерх.Параметры.ПоставщикОКПО       = СведенияОПоставщике.КодПоОКПО;
//	облВерх.Параметры.ОснованиеНомер        = "";
//	облВерх.Параметры.ОснованиеДата         = "";
//	облВерх.Параметры.ТТННомер = "";
//	облВерх.Параметры.ТТНДата  = "";
//	облВерх.Область("Верх1Разделитель1").ВысотаСтроки = чВысотаРазделителей;
//    	
//	Т.Вывести(облВерх);
...
//	ВерхТаблицы = Макет.ПолучитьОбласть("ВерхТаблицы1");
//	ОтрегулироватьКолонкиВерх(ВерхТаблицы, ст);
//	Т.Вывести(ВерхТаблицы);

//	// это не вполне правильно из-за пропусков строк с неуказанной номенклатурой
//	чКоличествоСтрок = тзТовары.Количество();
//	Если Шапка.СуммаВключаетНДС Тогда
//		чВсегоСумма = тзТовары.Итог("Сумма");
//	Иначе
//		чВсегоСумма = тзТовары.Итог("Сумма") + тзТовары.Итог("СуммаНДС");
//	КонецЕсли;
//	
//	// подготовимся к выводу нижней части
//	// так как в ней могут быть переносы строк
//	// Выводим подвал документа
//	облНиз = Макет.ПолучитьОбласть("Низ1");
//	облНиз.Параметры.ДатаДокументаПрописью = Формат(Шапка.ДатаДокумента, "ДФ=""дд ММММ гггг """"года""""""");
//	
//	Если сТипКонфиг = "Торговля" Тогда
//		Если сВерсияКонфиг = "10.2" Тогда
//			Руководители = фБиблиотека.РегламентированнаяОтчетностьОтветственныеЛицаОрганизации(Шапка.Руководители, Шапка.ДатаДокумента,);
//			Руководитель = Руководители.Руководитель;
//			Бухгалтер    = Руководители.ГлавныйБухгалтер;
//			облНиз.Параметры.ОтпускРазрешилДолжность = Руководители.РуководительДолжность;
//			облНиз.Параметры.ОтпускРазрешилФИО       = Руководитель;
//			облНиз.Параметры.БухгалтерФИО     = Бухгалтер;
//			ФамилияИмяОтчествоФизЛица                     = фБиблиотека.ФормированиеПечатныхФормФамилияИмяОтчество(Шапка.ОтветственноеЛицо, Шапка.ДатаДокумента);
...
//			облНиз.Параметры.ОтпускПроизвелФИО         = фБиблиотека.ОбщегоНазначенияФамилияИнициалыФизЛица(ФамилияИмяОтчествоКладовщика);
//			
//			ФИООтпускПроизвел = "";
//			
//		ИначеЕсли сВерсияКонфиг = "10.3" Тогда
//			Руководители = фБиблиотека.РегламентированнаяОтчетностьОтветственныеЛицаОрганизации(Шапка.Руководители, Шапка.ДатаДокумента,);
//			Руководитель = Руководители.Руководитель;
//			Бухгалтер    = Руководители.ГлавныйБухгалтер;
//			
...
//			Иначе
//				ФамилияИмяОтчествоФизЛица        = фБиблиотека.ФормированиеПечатныхФормФамилияИмяОтчество(Шапка.ОтпускРазрешил, Шапка.ДатаДокумента);
//				ПодразделениеДолжностьФизЛица    = фБиблиотека.ФормированиеПечатныхФормДолжностьОтветственногоЛицаОрганизации(Шапка.ОтпускРазрешил, Шапка.ДатаДокумента, СсылкаНаОбъект.Организация);
...
//				ФИООтпускРазрешил                = фБиблиотека.ОбщегоНазначенияФамилияИнициалыФизЛица(ФамилияИмяОтчествоОтпускРазрешил);
//				ДолжностьОтпускРазрешил          = ПодразделениеДолжностьФизЛица.Должность;
//			КонецЕсли;
//			
//			облНиз.Параметры.ОтпускРазрешилДолжность = ДолжностьОтпускРазрешил;
//			облНиз.Параметры.ОтпускРазрешилФИО       = ФИООтпускРазрешил;
//			
//			// Отпуск товара произвел
//			Если НЕ ЗначениеЗаполнено(Шапка.ОтпускПроизвел) Тогда
//				ФамилияИмяОтчествоФизЛица     = фБиблиотека.ФормированиеПечатныхФормФамилияИмяОтчество(Шапка.ОтветственноеЛицо, Шапка.ДатаДокумента);
//				ПодразделениеДолжностьФизЛица = фБиблиотека.ФормированиеПечатныхФормДолжностьОтветственногоЛицаОрганизации(Шапка.ОтветственноеЛицо, Шапка.ДатаДокумента, СсылкаНаОбъект.Организация);
//			Иначе
//				ФамилияИмяОтчествоФизЛица     = фБиблиотека.ФормированиеПечатныхФормФамилияИмяОтчество(Шапка.ОтпускПроизвел, Шапка.ДатаДокумента);
//				ПодразделениеДолжностьФизЛица = фБиблиотека.ФормированиеПечатныхФормДолжностьОтветственногоЛицаОрганизации(Шапка.ОтпускПроизвел, Шапка.ДатаДокумента, СсылкаНаОбъект.Организация);
//			КонецЕсли;
//			
...
//			ФИООтпускПроизвел                = фБиблиотека.ОбщегоНазначенияФамилияИнициалыФизЛица(ФамилияИмяОтчествоОтпускПроизвел);
//			ДолжностьОтпускПроизвел          = ПодразделениеДолжностьФизЛица.Должность;
//			
//			облНиз.Параметры.ОтпускПроизвелДолжность = ДолжностьОтпускПроизвел;
//			облНиз.Параметры.ОтпускПроизвелФИО       = ФИООтпускПроизвел;
//			
//			// Доверенность
//			облНиз.Параметры.ДоверенностьНомер     = Шапка.ДоверенностьНомер;
//			облНиз.Параметры.ДоверенностьДата      = Формат(Шапка.ДоверенностьДата, "ДФ='дд ММММ гггг'");
//			облНиз.Параметры.ДоверенностьВыдана    = Шапка.ДоверенностьВыдана;
//			облНиз.Параметры.ДоверенностьЧерезКого = Шапка.ДоверенностьЧерезКого;
//			
//		КонецЕсли;
//	ИначеЕсли сТипКонфиг = "Бухгалтерия" Тогда
//		Если сВерсияКонфиг = "1.6" Тогда
//			Руководители = фБиблиотека.ОбщегоНазначенияОтветственныеЛица(Шапка.Руководители, Шапка.ДатаДокумента);
//		ИначеЕсли сВерсияКонфиг = "2.0" Тогда
//			Руководители = фБиблиотека.ОбщегоНазначенияОтветственныеЛица(Шапка.Руководители, Шапка.ДатаДокумента, ПодразделениеОтветственныхЛиц);
//		КонецЕсли;

//		облНиз.Параметры.ОтпускРазрешилФИО       = Руководители.РуководительПредставление;
//		облНиз.Параметры.ОтпускРазрешилДолжность = Руководители.РуководительДолжность;
//		облНиз.Параметры.БухгалтерФИО		     = Руководители.ГлавныйБухгалтерПредставление;
//		
//		ФИООтпускПроизвел = "";
//	КонецЕсли;

//	// для того, чтобы дальше не думать, что в какой накладной нужно
//	// пока берем итоги из таблицы
//	// потом будем уточнять (во всех типовых очень вольно обращаются с округлениями)
//	стИтогиНиза = Новый Структура;
//	
//	// эти два показателя надо подсчитать
//	чВсегоКоличествоМестПредв = 0;
//	чВсегоМассаБруттоПредв = 0;
//	Для каждого стзТовары Из тзТовары Цикл
//		Если ЗначениеЗаполнено(стзТовары.КоличествоМест) Тогда
//			чВсегоКоличествоМестПредв = чВсегоКоличествоМестПредв + стзТовары.КоличествоМест;
//		КонецЕсли;
//		Если ЗначениеЗаполнено(стзТовары.КоличествоМест) Тогда
//			чВсегоМассаБруттоПредв = чВсегоМассаБруттоПредв + стзТовары.МассаБрутто;
//		КонецЕсли;
//	КонецЦикла;
//	
//	Если сТипКонфиг = "Торговля" Тогда
//		Если ЗначениеЗаполнено(ЕдиницаИзмеренияВеса) Тогда
//			стИтогиНиза.Вставить("ВсегоМассаБрутто", чВсегоМассаБруттоПредв);
//			Если чВсегоМассаБруттоПредв > 0 Тогда
//				стИтогиНиза.Вставить("ВсегоМассаБруттоПрописью",
//					ЧислоПрописью(чВсегоМассаБруттоПредв, ,",,,,,,,,0")+ " " +ЕдиницаИзмеренияВеса.Наименование);
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;

//	стИтогиНиза.Вставить("ВсегоКоличествоМест", чВсегоКоличествоМестПредв);
//	Если тзТовары.Итог("КоличествоМест") > 0 Тогда
//		облНиз.Параметры.ВсегоКоличествоМестПрописью = ЧислоПрописью(чВсегоКоличествоМестПредв, ,",,,,,,,,0");
//	КонецЕсли;

//	// а для остальных сгодятся итоги таблицы
//	стИтогиНиза.Вставить("КоличествоНаименований", чКоличествоСтрок);
//	стИтогиНиза.Вставить("КоличествоНаименованийПрописью", ЧислоПрописью(чКоличествоСтрок, ,",,,,,,,,0"));
//	
//	стИтогиНиза.Вставить("КоличествоСтрок", чКоличествоСтрок);
//	стИтогиНиза.Вставить("КоличествоСтрокПрописью", ЧислоПрописью(чКоличествоСтрок, ,",,,,,,,,0"));
//	
//	облНиз.Параметры.СуммаПрописью = фБиблиотека.ОбщегоНазначенияСформироватьСуммуПрописью(чВсегоСумма, мВалютаРегламентированногоУчета);
//	
//	облНиз.Параметры.Заполнить(стИтогиНиза);
//	
//	
//	// инициализация итогов по странице
//	стВсегоСтр = Новый Структура("МассаБрутто,КоличествоМест,Количество,СуммаБезНДС,НДС,СуммаСНДС,МассаБруттоТонн", 0, 0, 0, 0, 0, 0, 0);

//	// инициализация итогов по документу
//	стВсего = Новый Структура("МассаБрутто,КоличествоМест,Количество,СуммаБезНДС,НДС,СуммаСНДС,МассаБруттоТонн", 0, 0, 0, 0, 0, 0, 0);
//	
//	// для последующей вставки номеров страниц
//	маНомераСтраниц = Новый Массив;
//	
//	// Создаем массив для проверки вывода
//	маОбласти = Новый Массив;
//	
//	// Выводим многострочную часть докмента
//	ОтрегулироватьКолонкиСтрока(облСтрока, ст);
//	облСтрока.Область(сКолМБ).Формат = "ЧДЦ=" + Формат(чТочностьМассаБрутто, "ЧН=");
//	облСтрока.Область("Количество").Формат = "ЧДЦ=" + Формат(чТочностьКоличество, "ЧН=");
//	
//	облВсегоСтр			= Макет.ПолучитьОбласть("Всего1");
//	облВсегоСтр.Параметры.ЗаголовокВсего = "Итого";
//	ОтрегулироватьКолонкиСтрока(облВсегоСтр, ст);
//	облВсегоСтр.Область("Всего" + сКолМБ).Формат = "ЧДЦ=" + Формат(чТочностьМассаБрутто, "ЧН=");
//	облВсегоСтр.Область("ВсегоКоличество").Формат = "ЧДЦ=" + Формат(чТочностьКоличество, "ЧН=");
//	
//	облВсего			= Макет.ПолучитьОбласть("Всего1");
//	облВсего.Параметры.ЗаголовокВсего = "Всего по накладной";
//	ОтрегулироватьКолонкиСтрока(облВсего, ст);
//	облВсего.Область("Всего" + сКолМБ).Формат = "ЧДЦ=" + Формат(чТочностьМассаБрутто, "ЧН=");
//	облВсего.Область("ВсегоКоличество").Формат = "ЧДЦ=" + Формат(чТочностьКоличество, "ЧН=");
//	
//	Если сВидНакладной = "ТТН" Тогда
//		облНизТаблицы = Макет.ПолучитьОбласть("НизТаблицы1");
//	КонецЕсли;
//	
//	// пошел вывод табличной части ------------------------------------------------
//	Для Каждого стзТовары Из тзТовары Цикл
//		
//		Если Не ЗначениеЗаполнено(стзТовары.Номенклатура) Тогда
//			Сообщить("В одной из строк не заполнено значение номенклатуры - строка при печати пропущена.", СтатусСообщения.Важное);
//			Продолжить;
//		КонецЕсли;
//		
//		чНомерСтроки = тзТовары.Индекс(стзТовары);

//		облСтрока.Параметры.ТоварКод = СокрЛП(стзТовары.ТоварКод);
//		облСтрока.Параметры.ЕдИзмНаименование	= стзТовары.БазоваяЕдиницаНаименование;
//		Если сВидНакладной = "ТОРГ12" Тогда
//			облСтрока.Параметры.НомерСтроки			= тзТовары.Индекс(стзТовары) + 1;
//			облСтрока.Параметры.ЕдИзмКод			= стзТовары.БазоваяЕдиницаКодПоОКЕИ;
//			облСтрока.Параметры.СтавкаНДС			= стзТовары.СтавкаНДС;
//		ИначеЕсли сВидНакладной = "ТТН" Тогда
//			облСтрока.Параметры.МассаБруттоТонн		= стзТовары.МассаБруттоТонн;
//		КонецЕсли;
//		
//		Если сТипКонфиг = "Торговля" Тогда
//			облСтрока.Параметры.ТоварНаименование = СокрЛП(стзТовары.ТоварНаименование)
//			                                          + фБиблиотека.ФормированиеПечатныхФормПредставлениеСерий(стзТовары)
//			                                          + ?(стзТовары.Метка = 2, " (возвратная тара)", "");
//		ИначеЕсли сТипКонфиг = "Бухгалтерия" Тогда
//			Если НЕ ЗначениеЗаполнено(стзТовары.КоличествоМест) Тогда
//				облСтрока.Параметры.ВидУпаковки           = "";
//				//Если сВидНакладной = "ТОРГ12" Тогда
//				//	облСтрока.Параметры.КоличествоВОдномМесте = "";
//				//КонецЕсли;
//			КонецЕсли;
//			
//			облСтрока.Параметры.ТоварНаименование = СокрЛП(стзТовары.ТоварНаименование);
//		КонецЕсли;
//		
//		КоличествоМест        = стзТовары.КоличествоМест;
//		КоличествоМест        = ?(КоличествоМест <> Неопределено И КоличествоМест <> NULL, КоличествоМест, 0);

//		Количество  = стзТовары.Количество;
//		Если сТипКонфиг = "Торговля" Тогда
//			СуммаСНДС   = Окр((стзТовары.Сумма + ?(Шапка.СуммаВключаетНДС, 0, стзТовары.СуммаНДС)), 2);
//			НДС			= Окр(стзТовары.СуммаНДС, 2);
//		Иначе
//			СуммаСНДС	= стзТовары.Сумма + ?(Шапка.СуммаВключаетНДС, 0, стзТовары.СуммаНДС);
//			НДС			= стзТовары.СуммаНДС;
//		КонецЕсли;
//		
...
//				Или стзТовары.ЕстьСкидкиПоСтроке Тогда
//				облСтрока.Параметры.ЦенаБезНДС = Окр(?(Количество = 0, 0, СуммаБезНДС / Количество),2,1);
//			Иначе
//				облСтрока.Параметры.ЦенаБезНДС = стзТовары.Цена;
//			КонецЕсли;
//		ИначеЕсли сТипКонфиг = "Бухгалтерия" Тогда
//			Если Шапка.ВалютаДокумента = мВалютаРегламентированногоУчета И (НЕ Шапка.СуммаВключаетНДС ИЛИ НДС = 0) Тогда
//				облСтрока.Параметры.ЦенаБезНДС    = стзТовары.Цена;
//			Иначе
//				облСтрока.Параметры.ЦенаБезНДС    = СуммаБезНДС / ?(Количество = 0, 1, Количество);
//			КонецЕсли;
//		КонецЕсли;
//		
//		стСтрока = Новый Структура("МассаБрутто,КоличествоМест,Количество,КоличествоВОдномМесте,СуммаБезНДС,НДС,СуммаСНДС,МассаБруттоТонн",
//									стзТовары.МассаБруттоЕд,КоличествоМест,Количество,стзТовары.КоличествоВОдномМесте,СуммаБезНДС,НДС,СуммаСНДС,0);
//		
//		облСтрока.Параметры.Заполнить(стСтрока);

//		маОбласти.Очистить();
//		маОбласти.Добавить(облСтрока);
//		маОбласти.Добавить(облВсегоСтр);
//		Если чНомерСтроки = КоличествоСтрок Тогда
//			Если чНомерСтраницы > 1 Тогда
//				маОбласти.Добавить(облВсего);
//			КонецЕсли;
//			Если сВидНакладной = "ТТН" Тогда
//				маОбласти.Добавить(облНизТаблицы);
//			КонецЕсли;	
//			маОбласти.Добавить(облНиз);
//		КонецЕсли;
//		
//		Если НЕ Т.ПроверитьВывод(маОбласти) Тогда

//			// Строка с итогом не вмещается
//			// причем выводится она даже если одна строка не лезет
//			облВсегоСтр.Параметры.Заполнить(стВсегоСтр);
//			стВсегоСтр.МассаБруттоТонн = Окр(стВсегоСтр.МассаБрутто * чКтТонн, чОкрТонн);

//			//ВерхТаблицы.Параметры.НомерСтраницы = "Страница " + НомерСтраницы;
//			Т.Вывести(облВсегоСтр);
//			Если сВидНакладной = "ТТН" Тогда
//				Т.Вывести(облНизТаблицы);
//			КонецЕсли;
//			
//			// очистим итоги по странице
//			стВсегоСтр = Новый Структура("МассаБрутто,КоличествоМест,Количество,СуммаБезНДС,НДС,СуммаСНДС", 0, 0, 0, 0, 0, 0);
//			
//			маНомераСтраниц.Добавить(Т.ВысотаТаблицы);

//			НомерСтраницы = НомерСтраницы + 1;
//			Т.ВывестиГоризонтальныйРазделительСтраниц();
//			Т.Вывести(Макет.ПолучитьОбласть("ВерхСтраницы"));
//			
//			Т.Вывести(ВерхТаблицы);
//		КонецЕсли;		
...
//		ДобавитьКСтруктуре(стВсего, стСтрока);
//		
//		// увеличим итоги по дукументу
//		ДобавитьКСтруктуре(стВсегоСтр, стСтрока);

//	КонецЦикла;

//	Если сВидНакладной = "ТОРГ12" Тогда
//		чВсегоСтраниц = чНомерСтраницы;
//	Иначе
//		чВсегоСтраниц = чНомерСтраницы + 1;
//	КонецЕсли;
//	
//	Если чВсегоСтраниц > 1 Тогда
//		// Выводим итоги по последней странице
//		Если чНомерСтраницы > 1 Тогда
//			стВсегоСтр.МассаБруттоТонн = Окр(стВсегоСтр.МассаБрутто * чКтТонн, чОкрТонн, 1);
//			облВсегоСтр.Параметры.Заполнить(стВсегоСтр);
//			Т.Вывести(облВсегоСтр);
//		КонецЕсли;
//		
...
//		КонецЕсли;
//		
//		Для чИнд = 1 По маНомераСтраниц.Количество() Цикл
//			Т.Область(маНомераСтраниц[чИнд-1],чКолонкаСНомером).Текст = "Страница " + чИнд + " из " + чВсегоСтраниц;
//		КонецЦикла
//	КонецЕсли;
//	
//	// Выводим итоги по документу в целом
//	стВсего.МассаБруттоТонн = Окр(стВсего.МассаБрутто * чКтТонн, чОкрТонн, 1);
//	облВсего.Параметры.Заполнить(стВсего);
//	Т.Вывести(облВсего);
//	Если сВидНакладной = "ТТН" Тогда
//		Т.Вывести(облНизТаблицы);
//	КонецЕсли;
...
//	// вывод низа
//	Если чВсегоСтраниц > 1 Тогда
//		облНиз.Параметры.НомерСтраницы = "Страница " + чНомерСтраницы + " из " + чВсегоСтраниц;
//	КонецЕсли;
//	Т.Вывести(облНиз);

//	Если сВидНакладной = "ТТН" Тогда
//		// выведем вторую страничцу
//		Т.ВывестиГоризонтальныйРазделительСтраниц();
//		облВерх = Макет.ПолучитьОбласть("Верх2");
//		облВерх.Параметры.НомерДокумента = фБиблиотека.ОбщегоНазначенияПолучитьНомерНаПечать(Шапка);
//		
//		облВерх.Параметры.СрокДоставки = Формат(СрокДоставки, сФорматДат);
//		
//		облВерх.Параметры.АвтомобильМарка = МаркаАвтомобиля;
//		облВерх.Параметры.АвтомобильГосНомер = ГосНомерАвтомобиля;
//		
//		облВерх.Параметры.Перевозчик2 = Перевозчик;
//		облВерх.Параметры.Заказчик2 = Заказчик;
//		
//		облВерх.Параметры.ВодительФИО = Водитель;
//		облВерх.Параметры.ВодительУдостоверение = ВодительскоеУдостоверение;
//		                                                                       
//		облВерх.Область("ЛицензионнаяКарточкаСтандартная").Шрифт = Новый Шрифт(,,,,, Не ЛицензионнаяКарточка);
//		облВерх.Область("ЛицензионнаяКарточкаОграниченная").Шрифт = Новый Шрифт(,,,,, ЛицензионнаяКарточка);
//		
//		облВерх.Параметры.ВидПеревозки = ВидПеревозки;
//		
//		облВерх.Параметры.ПунктПогрузки = ПунктПогрузки;
//		облВерх.Параметры.ПунктРазгрузки = ПунктРазгрузки;
//		
//		облВерх.Параметры.ПрицепМарка = МаркаПрицепа;
//		облВерх.Параметры.ПрицепГосНомер = ГосНомерПрицепа;
//		
//		Т.Вывести(облВерх);
//		
//		Т.Вывести(Макет.ПолучитьОбласть("ВерхТаблицы2"));
//		облСтрока = Макет.ПолучитьОбласть("Строка2");
//		Для чИнд = 1 По 3 Цикл
//			облСтрока.Параметры.СведенияНаименование = "" + чИнд + ".";
//			Т.Вывести(облСтрока);
//		КонецЦикла;
//		
//		облНиз = Макет.ПолучитьОбласть("Низ2");
//		облНиз.Параметры.ОтпускПроизвелФИО = ФИООтпускПроизвел;
//		облНиз.Параметры.ГрузПринялФИО = Водитель;
//		
//		облНиз.Параметры.ВсегоКоличествоМест	= стВсего.КоличествоМест;
//		облНиз.Параметры.ВсегоМассаБруттоТонн	= стВсего.МассаБруттоТонн;
//		
//		облНиз.Параметры.НомерСтраницы = "Страница " + чВсегоСтраниц + " из " + чВсегоСтраниц;
//		
//		облНиз.Область("Низ2Разделитель1").ВысотаСтроки = чВысотаРазделителей;
//		облНиз.Область("Низ2Разделитель2").ВысотаСтроки = чВысотаРазделителей;
//		облНиз.Область("Низ2Разделитель3").ВысотаСтроки = чВысотаРазделителей;
//		
//		Т.Вывести(облНиз);
//	КонецЕсли;
//	
...
//	
//	ОтрегулироватьЦвет(Т);

//	#Если Клиент Тогда
//	Если сПрямойВывод = "Да" ИЛИ (сПрямойВывод = "Авто" И сКонфиг = "Бухгалтерия 2.0") Тогда
