...
Процедура ВыполнитьПроведение(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено) Экспорт
	...
	Если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2639) тогда 
		Если ПроведениеДокументовКлиентСервер.ПроводитсяПо(вхПараметры, "ПриемкиТоваров") тогда
			ОбщегоНазначения.ЗаписатьДвиженияДокумента(вхСсылкаНаДокумент, "ПриемкиТоваров",
			РегистрыНакопления_ПриемкиТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры));
		КонецЕсли;
	КонецЕсли;	
...
Процедура ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(СтруктураШапкиДокумента,ТаблицаТоваров,ОшибкиЗагрузки)
	...
	//ХудинВВ XX-3001 22072019 Если были ошибки загрузки и тч заполнена, то не перезаполняем
	//ПерезаполнятьТЧ = НЕ (ОшибкиЗагрузки <> Неопределено И ДокументПоступления.Товары.Количество() > 0);
	
	//ХудинВВ XX-2992 29072019 Если были ошибки загрузки, то не перезаполняем
	ПерезаполнятьТЧ = ОшибкиЗагрузки = Неопределено;
	...
	Если ПерезаполнятьТЧ Тогда
		...
		Для Каждого ТекСтрока Из ТаблицаТоваров Цикл
			...
			//ХудинВВ XX-3001 22072019 не надо создавать, если уже есть ((
			Если Не ЗначениеЗаполнено(НоваяСтрока.СтрокаПрихода) ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(НоваяСтрока.СтрокаПрихода) Тогда
	...
	ДокументПоступления.ДополнительныеСвойства.Вставить("ЗагруженИзОП");
	 ...
	 ПровестиДокументПТУПриЗагрузкеИзОП(ДокументПоступления,ОшибкиЗагрузки,СтруктураШапкиДокумента)

	
КонецПроцедуры	

Процедура ПровестиДокументПТУПриЗагрузкеИзОП(ДокументПоступления,ОшибкиЗагрузки,СтруктураШапкиДокумента)
	лКлючАлгоритма = "Документ_ПоступлениеТоваровУслуг_МодульМенеджера_ПровестиДокументПТУПриЗагрузкеИзОП";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		Выполнить(лЗамена);
		Возврат;
	...
	Если не ОшибкиЗагрузки = Неопределено тогда 
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументПоступления,Истина);
		Возврат 
	КонецЕсли;	
	...
	ИначеЕсли ДокументПоступления.ЭтоНовый() Тогда
		
		//ХудинВВ XX-2933 16072019 Ошибок нет, но документ не записан, запишем его и зафиксируем ошибку 
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиЗагрузки,,"Документ не был записан при загрузке. Записан при фиксации ошибки.");
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументПоступления);
...
Процедура ПроверитьКорректностьТаблицыТоваров(ТаблицаТоваров,ОшибкиРазмещения)
	...
	Для каждого СтрокаНеНайденаяНоменклатура из НеНайденаяНоменклатура цикл
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиРазмещения,,"Не удалось найти номенклатуру по артикулу: "
		+  СтрокаНеНайденаяНоменклатура.Артикул +?(ЗначениеЗаполнено(СтрокаНеНайденаяНоменклатура.Номенклатура)," (наименоваие: "+СтрокаНеНайденаяНоменклатура.Номенклатура+")",""));
	КонецЦикла;
Конецпроцедуры	
...
Процедура  ДополнитьНоменклатуруПоОстаткамЗаказов(ТаблицаТоваров,ТорговаяТочка,Склад,ОшибкиЗагрузки)
	...
	Для каждого СтрокаДокумента из ТаблицаТоваров цикл 
		...
		иначе
			//#XX-2423 Kalinin V.A. ( 2019-07-09 )  /*
			Если не ЗначениеЗаполнено(СтрокаДокумента.НоменклатураЗаказа) и НайденаяНоменклатура[0].НайденаНоменклатура  = Ложь  тогда 
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиЗагрузки,,"Не найдена номенклатура  в заказах по артикулу: " +СтрокаДокумента.Артикул+". Необходимо проверить корректность");
			КонецЕсли;	
...
Процедура ЗагрузитьРезультатПриемки(ОбъектXDTO, ДопПараметры = Неопределено, СтруктураОтчета = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		//#XX-2639 Kalinin V.A. ( 2019-06-28 )  /*
		если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2639) тогда 
		 АктПриемки = ЗаполнитьАктПриемкиПриЗагрузке(ТоварыXDTO,ДокСсылка);	
		Иначе 	
	...
	Если Не (СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят Или 
		...
		КонецЕсли;
			...
			//#XX-2639 Kalinin V.A. ( 2019-06-28 )  /*
			если ЭтоАктивнаяЗадачаJirа(Справочники.ЗадачиJira.XX2639) тогда 
				ДокОбъект.АктПриемки.Загрузить(АктПриемки);				
			иначе 
...
Функция ПолучитьТаблицуАктаПриемкиПоДаннымТоплог(СписокXDTO,ДокументСсылка)
	АктПриемки =   ДокументСсылка.АктПриемки.ВыгрузитьКолонки("СтрокаЗаявки,Номенклатура,ЕдиницаИзмерения,КачествоИзТопЛог,КоличествоПлан,Количество,КоличествоНедостача,КоличествоБрак,КоличествоИзлишек,КоличествоВернули");
	
	//Временно переписал цикл , т.к нет точной уверености ,
	
	Для Каждого ОбъектXDTO из СписокXDTO цикл          
				
		Номенклатура = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.НоменклатураСсылка));
		
		Если ЗначениеЗаполнено(ОбъектXDTO.SSID)  тогда 
			СтрокаЗаявки   = Справочники.ИдентификаторыСтрокЗаявок.НайтиПоРеквизиту("IDSite",ОбъектXDTO.SSID);
			СтрокаТЧ = АктПриемки.Найти(СтрокаЗаявки, "СтрокаЗаявки");	
		иначе
			СтрокаЗаявки = Справочники.ИдентификаторыСтрокЗаявок.ПустаяСсылка();
			СтрокаТЧ = АктПриемки.Найти(Номенклатура, "Номенклатура");
		КонецЕсли;
		...
		Если СтрокаТЧ = Неопределено Тогда 
			СтрокаТЧ = АктПриемки.Добавить();
			СтрокаТЧ.Номенклатура = Номенклатура;
			СтрокаТЧ.СтрокаЗаявки = СтрокаЗаявки;
			СтрокаТЧ.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "ЕдиницаХраненияОстатков");
		КонецЕсли;
		
		СтрокаТЧ.КоличествоВернули = СтрокаТЧ.КоличествоВернули + ОбъектXDTO.КоличествоВозвращено;
		
		Если ОбъектXDTO.ЭтоБрак И ОбъектXDTO.Качество = "Излишек" Тогда 
			СтрокаТЧ.КоличествоИзлишек = СтрокаТЧ.КоличествоИзлишек + ОбъектXDTO.КоличествоПринято;
		ИначеЕсли ОбъектXDTO.ЭтоБрак Тогда 
			СтрокаТЧ.КоличествоБрак = СтрокаТЧ.КоличествоБрак + ОбъектXDTO.КоличествоПринято;
		Иначе	
			СтрокаТЧ.Количество = СтрокаТЧ.Количество + ОбъектXDTO.КоличествоПринято;
		КонецЕсли;
		
		
		Если Не (ЗначениеЗаполнено(СтрокаТЧ.КачествоИзТопЛог) И ОбъектXDTO.Качество = "Излишек") Тогда //Если передано качество Брака, то не затираем
			СтрокаТЧ.КачествоИзТопЛог = ОбъектXDTO.Качество;
		КонецЕсли;		
	КонецЦикла;
...
Функция ЗаполнитьАктПриемкиПриЗагрузке(СписокXDTO,ДокументСсылка)
	
	    АктПриемки =  ПолучитьТаблицуАктаПриемкиПоДаннымТоплог(СписокXDTO,ДокументСсылка);

		
		Для Каждого СтрокаТЧ Из АктПриемки Цикл 
			
			Если ЗначениеЗаполнено(СтрокаТЧ.СтрокаЗаявки) тогда 
				Строки = ДокументСсылка.РазмещениеСтрокПрихода.НайтиСтроки(Новый Структура("СтрокаЗаявки", СтрокаТЧ.СтрокаЗаявки)); 		  
			Иначе 	  
				Строки = ДокументСсылка.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТЧ.Номенклатура));  
			КонецЕсли;	 
		...
		СтрокиКУдалению = АктПриемки.найтистроки(Новый Структура("КоличествоНедостача,КоличествоБрак,КоличествоИзлишек,КоличествоВернули",0,0,0,0)); 
		Для Каждого СтрокаУдаления из СтрокиКУдалению цикл 
			 	АктПриемки.Удалить(СтрокаУдаления);	
		КонецЦикла;	
...
Функция ЗаказПоставщику(order_uuid,Поставщик, ОшибкиЗагрузки,Отказ = ложь)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Поставщик,"ЗаказРавенПриходу") 
		и Не ЗначениеЗаполнено(ЗаказПоставщику) тогда
	  	Отказ = Истина;
		СообщениеОбОшибке = "Не найден заказ поставщику по ссылке: """+order_uuid+""" (Заказ=СФ)";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиЗагрузки,,СообщениеОбОшибке);
...
Функция ТаблицаТоваров(СписокТоварыXDTO, Контрагент, ОшибкиЗагрузки, НеЗагружатьДокумент)
	...
	Для Каждого Строка Из СписокТоварыXDTO Цикл
		...
		СтруктураСтроки.Вставить("Наименование", Строка.name); //ХудинВВ XX-2992 29072019
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена, ВыгружаемыеОбъекты = Неопределено) Экспорт
	...
	ИначеЕсли лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		|	втСтрокиЗаявки.СтрокаПрихода,
		|	втСтрокиЗаявки.СтрокаПрихода ЕСТЬ NULL КАК Невыгружать_ПроблемаСРазмещением,
		|	ПоступлениеТоваровУслуг.ЭтоМФП КАК Невыгружать_ЭтоМФП,
		|	ПоступлениеТоваровУслугТовары.Ссылка ЕСТЬ NULL КАК Невыгружать_НетТоваров,
		|	НЕ ПоступлениеТоваровУслуг.Проведен КАК Невыгружать_Непроведен,
		|	ПоступлениеТоваровУслуг.ВидОперацииПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыПоступленияТоваров.ЗачетТовараVMI)
		|		ИЛИ ПоступлениеТоваровУслуг.ВидОперацииПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыПоступленияТоваров.Прочее) КАК Невыгружать_НеподходящийВидОперации,
		|	НЕ ЕСТЬNULL(ПоступлениеТоваровУслуг.Склад.ОбменСTopLog, ЛОЖЬ) КАК Невыгружать_СкладНеОбмениваетсяСТопЛог,
		|	ПоступлениеТоваровУслуг.флНеВыгружатьВТопЛог КАК Невыгружать_НеВыгружатьВТопЛог
		|ИЗ
		|	Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		...
		|			ПоступлениеТоваровУслугТовары.имп_СуммаРуб КАК имп_СуммаРуб,
		...
		|						Объекты.Ссылка
		|					ИЗ
		|						Объекты)) КАК ПоступлениеТоваровУслугТовары
		...
		|	МАКСИМУМ(Невыгружать_ПроблемаСРазмещением),
		|	МАКСИМУМ(Невыгружать_ЭтоМФП),
		|	МАКСИМУМ(Невыгружать_НетТоваров),
		|	МАКСИМУМ(Невыгружать_Непроведен),
		|	МАКСИМУМ(Невыгружать_НеподходящийВидОперации),
		|	МАКСИМУМ(Невыгружать_СкладНеОбмениваетсяСТопЛог),
		|	МАКСИМУМ(Невыгружать_НеВыгружатьВТопЛог)
		|ПО
		|	Ссылка";
		...
		Если НЕ лРезультатыЗапроса[3].Пустой() Тогда
			...
			Пока лВыборка.Следующий() Цикл
                Если Не ЗафиксированаОшибка(лВыборка) Тогда  
					лОбъект = ФабрикаXDTO.Создать(лТипОбъектаXDTO);
...
Функция ЗафиксированаОшибка(ВыборкаИзРезультатаЗапроса) 
	Ошибка = Ложь;
	МассивСообщенийОбОшибке = Новый Массив;
	
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_ЭтоМФП Тогда 
		МассивСообщенийОбОшибке.Добавить("это документ МФП");;
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_Непроведен Тогда 
		МассивСообщенийОбОшибке.Добавить("документ не проведен");
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_НеподходящийВидОперации Тогда 
		МассивСообщенийОбОшибке.Добавить("в документе неподходяший вид операции");
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_НетТоваров Тогда 
		МассивСообщенийОбОшибке.Добавить("в документе пустая табличная часть ""Товары""");
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_ПроблемаСРазмещением Тогда 
		МассивСообщенийОбОшибке.Добавить("в документе табличная часть ""Размещение"" не соответствует табличной части ""Товары"". Ошибки можно посмотреть по кнопке ""Выгрузить в ТопЛог""");
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_СкладНеОбмениваетсяСТопЛог Тогда 
		МассивСообщенийОбОшибке.Добавить("склад в шапке документа не обменивается с ТопЛог");
	КонецЕсли;
	Если ВыборкаИзРезультатаЗапроса.Невыгружать_НеВыгружатьВТопЛог Тогда 
		МассивСообщенийОбОшибке.Добавить("в документе установлен флаг ""Не выгружать в Топлог""");	
	КонецЕсли;
	
	Если МассивСообщенийОбОшибке.Количество() > 0 Тогда 
		Ошибка = Истина;
		СообщениеОбОшибке = "Документ не выгружен. Причины невыгрузки: ";
		Для Каждого ОписаниеОшибки Из МассивСообщенийОбОшибке Цикл 
			СообщениеОбОшибке = СообщениеОбОшибке + ОписаниеОшибки + ",";
		КонецЦикла;
		
		Если СтрЗаканчиваетсяНа(СообщениеОбОшибке, ",") Тогда 
			СообщениеОбОшибке = Лев(СообщениеОбОшибке, СтрДлина(СообщениеОбОшибке) - 1);
		КонецЕсли;
		ОбменДаннымиКлиентСервер.ДобавитьСтрокуИсторииПоОбъекту(ВыборкаИзРезультатаЗапроса.Ссылка,, УправлениеСвойствамиПовтИсп.ЗаказНаПриемку(), Ошибка, СообщениеОбОшибке);
	КонецЕсли;
	Возврат Ошибка;
КонецФункции
	...
	Если вхПараметры.СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят
		...
		Если Не ПоступленияВыгружаютсяИз77 Тогда 			
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |			ТОГДА ПоступлениеТоваровУслуг.Ссылка.Склад
			               |		КОГДА (ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Ссылка.ДокументОснование КАК Документ.ВозвратТоваровПоставщику)) <> ЗНАЧЕНИЕ(Документ.ВозвратТоваровПоставщику.ПустаяСсылка)
			               |			ТОГДА ПоступлениеТоваровУслуг.Ссылка.Склад.СкладПриемки
...
Функция РегистрыНакопления_ПриемкиТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)  Экспорт
	лКлючАлгоритма = "ОбщийМодуль_ПоступлениеТоваровУслуг_РегистрыНакопления_ЗаказыПоставщикам";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
	...
	ТаблицаДвижений = Неопределено;
	ОбщегоНазначения.СоздатьСтруктуруРегистраНакопления("ПриемкиТоваров", ТаблицаДвижений);
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(вхСсылкаНаДокумент, "Дата,СтатусАктаПриемки");
		
	Если Реквизиты.Дата < ПараметрыСеанса.ДатаНачалаРаботыТовары 
		или не ЗначениеЗаполнено(Реквизиты.СтатусАктаПриемки) Тогда
		Возврат ТаблицаДвижений;
	КонецЕсли;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПоступлениеТоваровУслугАктПриемки.СтрокаЗаявки,
		|	ПоступлениеТоваровУслугАктПриемки.Номенклатура,
		|	ПоступлениеТоваровУслугАктПриемки.Ссылка.ДоговорКонтрагента,
		|	ПоступлениеТоваровУслугАктПриемки.Ссылка.ТорговаяТочка,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоПлан,
		|	ПоступлениеТоваровУслугАктПриемки.Количество,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоНедостача,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоБрак,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоИзлишек,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоВернули,
		|	ПоступлениеТоваровУслугАктПриемки.КоличествоВернулиПозже,
		|	ПоступлениеТоваровУслугАктПриемки.Ссылка.СтатусАктаПриемки,
		|	ПоступлениеТоваровУслугАктПриемки.Ссылка.ДатаЗавершенияРаботПоАктуПриемки,
		|	ПоступлениеТоваровУслугАктПриемки.ОтветПоставщика,
		|	ВЫРАЗИТЬ(ПоступлениеТоваровУслугАктПриемки.Комментарий КАК СТРОКА(1024)) КАК Комментарий,
		|	ПоступлениеТоваровУслугАктПриемки.КачествоИзТопЛог,
		|	&Период
		|ИЗ
		|	Документ.ПоступлениеТоваровУслуг.АктПриемки КАК ПоступлениеТоваровУслугАктПриемки
		|ГДЕ                           
		|	ПоступлениеТоваровУслугАктПриемки.Ссылка = &ДокументСсылка";
...
//ХудинВВ 01082019 XX-2837
Функция ЦенаПоступления(ДокументСсылка, Номенклатура) Экспорт
	
	лКлючАлгоритма = "Документ_ПоступлениеТоваровУслуг_ЦенаПоступления";
	лЗамена =  АлгоритмыПолучитьЗамену(лКлючАлгоритма);
	Если Не лЗамена = Неопределено Тогда
		АлгоритмыЗначениеВозврата = Неопределено;		
		Выполнить(лЗамена);		
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(МАКСИМУМ(ПоступлениеТоваровУслугТовары.Цена), 0) КАК Цена
		|ИЗ
		|	Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
		|ГДЕ
		|	ПоступлениеТоваровУслугТовары.Ссылка = &Ссылка
	...
	Если Выборка.Следующий() Тогда
		...
		ЦенаПоступления = Выборка.Цена;
		
	КонецЕсли;
	
	Возврат ЦенаПоступления;
