...
Функция ТаблицаТоваров(СписокТоварыXDTO, Контрагент, ТаблицаОшибок, НеЗагружатьДокумент)
	...
	Для Каждого Строка Из СписокТоварыXDTO Цикл
		...
		Если ИспользоватьНоменклатуруЗаказаВПТУ Тогда
			СтруктураСтроки.Вставить("НоменклатураПоступления", НоменклатураПоступления(Строка.articul, Строка.src_articul, Строка.src_brand, СтруктураСтроки.КлючСвязи, ТаблицаОшибок, Отказ));
		КонецЕсли;
		Если Не Отказ Тогда
...
Функция НоменклатураПоступления(articul, src_articul, src_brand,  НомерСтроки, ТаблицаОшибок, Отказ)
	...
	Если ЗначениеЗаполнено(articul) И ЗначениеЗаполнено(src_articul) Тогда
		Если ЗначениеЗаполнено(src_brand) Тогда
			...
			Иначе
				Отказ = Истина;
				СообщениеОбОшибке = "Не удалось найти номенклатуру поступления (articul: " + articul + ", src_brand: " + src_brand + ")";
				СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке, СообщениеОбОшибке, НомерСтроки);
			КонецЕсли;
		Иначе
			Отказ = Истина;
			СообщениеОбОшибке = "Указан артикул товара поступления, но не указан бренд товара заказа (src_brand) (articul: " + articul + ")";
			СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке, СообщениеОбОшибке, НомерСтроки);
		КонецЕсли;
	КонецЕсли;
		...
		Если ЦенаВПределахОтклонения(Строка.Цена, СтрокаПоступления.Цена, ДопустимыйПроцентОтклонения) Тогда
			...
			Если КоличествоКРаспределению > 0 Тогда
				...
				ИначеЕсли  ТипРаспределения = "ПоНоменклатуреЗаказа" Тогда
					СтрокаПоступления.Номенклатура = СтрокаПоступления.НоменклатураПоступления;
	...
	Для Каждого СтрокаПоступления Из ТаблицаПоступления Цикл
		...
		Пока ВыборкаОстатковПоНоменклатуре.Следующий() И КоличествоПоступление > 0 Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаПоступления.АртикулЗаказа) Тогда
				НоменклатураДляПоступления = СтрокаПоступления.НоменклатураПоступления;
			Иначе
			...
			Если НЕ ЗначениеЗаполнено(СтрокаПоступления.Номенклатура) ИЛИ (СтрокаПоступления.Номенклатура = НоменклатураДляПоступления) Тогда
				...
				Если Количество > 0 Тогда
					...
					Если ЗначениеЗаполнено(СтрокаПоступления.АртикулЗаказа) И НЕ ЭтоНоменклатураЗамены Тогда
						//Если поставщик передал нам артикул номенклатуры заказа, то в качестве номенклатуры прихода подставим первую найденную по артикулу
...
Функция ТаблицаТовары()
	...
	Таблица.Колонки.Добавить("НоменклатураПоступления", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
