...
КонецФункции
Функция НоменклатураПоАртикулу(Артикул,  НомерСтроки, ТаблицаОшибок, Отказ)
	...
	Если ЗначениеЗаполнено(Артикул) Тогда
		//Если ЗначениеЗаполнено(Бренд) Тогда
			...
			//Запрос.УстановитьПараметр("Наименование", Бренд);
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				Номенклатура = Выборка.Ссылка;
			Иначе
				Отказ = Истина;
				СообщениеОбОшибке = "Не удалось найти номенклатуру (articul: " + Артикул+")";// + ", src_brand: " + Бренд + ")";
				СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке, СообщениеОбОшибке, НомерСтроки);
			КонецЕсли;
		//Иначе
		//	Отказ = Истина;
		//	СообщениеОбОшибке = "Указан артикул замены, но не указан бренд (src_articul: " + Артикул + ")";
		//	СформироватьТекстОшибкиПоступления(ТаблицаОшибок, СообщениеОбОшибке, СообщениеОбОшибке, НомерСтроки);
		//КонецЕсли;
	КонецЕсли;
		...
		Если ЦенаВПределахОтклонения(Строка.Цена, СтрокаПоступления.Цена, ДопустимыйПроцентОтклонения) Тогда
			...
			Если КоличествоКРаспределению > 0 Тогда
				...
				ИначеЕсли  ТипРаспределения = "ПоНоменклатуреЗаказа" Тогда
					СтрокаПоступления.Номенклатура = НоменклатураПоАртикулу(СтрокаПоступления.Артикул, СтрокаПоступления.КлючСвязи, ТаблицаОшибок, Отказ);
	...
	Для Каждого СтрокаПоступления Из ТаблицаПоступления Цикл
		...
		Пока ВыборкаОстатковПоНоменклатуре.Следующий() И КоличествоПоступление > 0 Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаПоступления.АртикулЗаказа) Тогда
				НоменклатураДляПоступления = НоменклатураПоАртикулу(СтрокаПоступления.Артикул, СтрокаПоступления.КлючСвязи, ТаблицаОшибок, Отказ);
			Иначе
