...
Процедура ЗагрузитьЭлемент(ОбъектXDTO, вхОтправитель, Отказ, вхПараметры = Неопределено, НомерСообщения = 0, СтруктураОтчета = Неопределено) Экспорт
	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_ОкноПоставщика) тогда
			...
			ТаблицаТовары = ТаблицаТоваров(ОбъектXDTO.items, ШапкаДокумента.Контрагент, ТаблицаОшибок, Отказ);	
			Иначе 
			...
			Если ЗаказыПоставщикамСервер.ЭтоСкладНовойСхемы(ШапкаДокумента.Склад) тогда 
				...
				Если не  ТаблицаОшибок.Количество() тогда 
					ОшибкиРазмещения  =  Неопределено;		
					
					ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(ШапкаДокумента,ТаблицаТовары,ТаблицаОшибок);			
...
Процедура ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(СтруктураШапкиДокумента,ТаблицаТоваров,ТаблицаОшибок)
	...
	Если не КонтрольЗагрузкиПройден_ПоступлениеТоваровУслуг(СсылкаНаДокумент,ТаблицаОшибок) Тогда
		Возврат
	КонецЕсли;
	...
	ПроверитьКорректностьТаблицыТоваров(ТаблицаТоваров,ОшибкиРазмещения);
	
	Если не ОшибкиРазмещения = Неопределено тогда 
	 ЗарегистрироватьОшибкуЗагрузкиДокумента(СтруктураШапкиДокумента,ОшибкиРазмещения,ТаблицаОшибок);	
	КонецЕсли;	
	...
	Попытка 
	ЗаказыПоставщикамКлиентСервер.РазместитьДокументПоступленияПоЗаказам(ДокументПоступления,СтруктураПараметровРазмещения,ОшибкиРазмещения);
	Исключение 
...
Процедура ЗарегистрироватьОшибкуЗагрузкиДокумента(СтруктураШапкиДокумента,ОшибкиРазмещения,ТаблицаОшибок)
	...
	КонецЕсли;
	///////////////////////////////////////////////////////////////////////////
	СообщениеОбОшибке = СформироватьСообщениеОбОшибке(СтруктураШапкиДокумента,ОшибкиРазмещения);
...
Функция КонтрольЗагрузкиПройден_ПоступлениеТоваровУслуг(ПоступлениеСсылка,ТаблицаОшибок)
...
Функция СтруктураШапкиДокумента(ОбъектXDTO, ТаблицаОшибок, Отказ)
	...
	Структура.Вставить("ЗаказДляОтбора", ЗаказПоставщику(ОбъектXDTO.order_uuid,Структура.Контрагент,ТаблицаОшибок,Отказ));
...
Функция ЗаказПоставщику(order_uuid,Поставщик, ТаблицаОшибок,Отказ = ложь)
