...
Функция ПечатьТорг12Поставщика(СсылкаНаОбъект, ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	ВложенныйЗапрос.ТоварКод КАК ТоварКод,
	...
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|			ТОГДА ВложенныйЗапрос.СтрокаПрихода.НомерГТД.Код
	|		ИНАЧЕ ВложенныйЗапрос.НомерГТД.Код
	|	КОНЕЦ КАК НомерГТД,
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА ВложенныйЗапрос.СтрокаПрихода.СтранаПроисхождения.Код
	|		ИНАЧЕ ВложенныйЗапрос.СтранаПроисхождения.Код
	|	КОНЕЦ КАК СтранаКод,
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА ВложенныйЗапрос.СтрокаПрихода.СтранаПроисхождения.Наименование
	|		ИНАЧЕ ВложенныйЗапрос.СтранаПроисхождения.Наименование
	|	КОНЕЦ КАК СтранаНаименование,
	|	ВложенныйЗапрос.Метка КАК Метка
	|ИЗ
	|	(ВЫБРАТЬ
	|		ПоступлениеТоваровУслуг.Номенклатура КАК Номенклатура,
	|		ЕСТЬNULL(НоменклатураПоставщика.АртикулПоставщика, """") КАК ТоварКод,
	|		ПоступлениеТоваровУслуг.Коэффициент КАК Коэффициент,
	|		ПоступлениеТоваровУслуг.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	...
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслуг
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураПоставщика
	|			ПО ПоступлениеТоваровУслуг.Номенклатура.Артикул = НоменклатураПоставщика.Артикул
	|				И ПоступлениеТоваровУслуг.Номенклатура.Изготовитель = НоменклатураПоставщика.Изготовитель
	|				И ПоступлениеТоваровУслуг.Ссылка.Контрагент = НоменклатураПоставщика.Владелец
	|	ГДЕ
	|		ПоступлениеТоваровУслуг.Ссылка = &ТекущийДокумент
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ПоступлениеТоваровУслуг.Номенклатура,
	|		ЕСТЬNULL(НоменклатураПоставщика.АртикулПоставщика, """"),
	|		ПоступлениеТоваровУслуг.Коэффициент,
	|		ПоступлениеТоваровУслуг.ЕдиницаИзмерения,
	|		ПоступлениеТоваровУслуг.СтавкаНДС,
	|		ПоступлениеТоваровУслуг.Цена,
	|		ПоступлениеТоваровУслуг.ЕдиницаИзмерения.Коэффициент,
	|		ПоступлениеТоваровУслуг.НомерГТД,
	|		ПоступлениеТоваровУслуг.СтранаПроисхождения,
	...
	Макет = ПолучитьМакет("ТОРГ12Поставщика");
...
Процедура ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(СтруктураШапкиДокумента,ТаблицаТоваров,ОшибкиЗагрузки)
	...
	Иначе 	
	ДополнитьНоменклатуруПоОстаткамЗаказов(ТаблицаТоваров,СтруктураШапкиДокумента.ТорговаяТочка,СтруктураШапкиДокумента.Склад);
	КонецЕсли;
...
Процедура ДополнитьНоменклатуруПоОстаткамЗаказов(ТаблицаТоваров,ТорговаяТочка,Склад)
...
Функция ТаблицаТоваров(СписокТоварыXDTO, Контрагент, ОшибкиЗагрузки, НеЗагружатьДокумент)
	...
	Для Каждого Строка Из СписокТоварыXDTO Цикл
		...
		СтруктураСтроки.Вставить("НоменклатураЗамены", НоменклатураЗамены(Строка.src_articul, Строка.src_brand, СтруктураСтроки.КлючСвязи, ОшибкиЗагрузки, Отказ));
...
Функция ВыгрузитьЭлементы(вхТаблицаСсылокНаОбъекты, вхПланОбмена) Экспорт
	...
	ИначеЕсли лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_TopLog Тогда 
		...
		лЗапрос.УстановитьПараметр("КонтрагентEMEX", РегистрыСведений.НастройкиПодсистем.ЗначениеПараметра("Обмен 1С-Сайт", "EMEX: Контрагент"));
		// 30.05.19 Строганов Роман <
		...
		|	МАКСИМУМ(ВЗ.ШтрихкодУпаковки) КАК ШтрихкодУпаковки
		...
		|	ГДЕ
		|		втСтрокиЗаявки.Клиент В(&КонтрагентEMEX)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
		|			ПО втСтрокиЗаявки.СтрокаПрихода = ЗначенияСвойствОбъектов.Объект
		|				И (ЗначенияСвойствОбъектов.Свойство = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СвойстваОбъектов.EMEX_Boxbarcode))
		|	ГДЕ
		|		втСтрокиЗаявки.Клиент В(&КонтрагентEMEX)) КАК ВЗ
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЗ.СтрокаПрихода
		|;
		...
		Если НЕ лРезультатыЗапроса[3].Пустой() Тогда
			...
			// 30.05.19 Строганов Роман > 
			Если Не лРезультатыЗапроса[3].Пустой() Тогда
				ВыборкаШтрихкоды = лРезультатыЗапроса[3].Выбрать();
				СоответствиеШтрихкодов = Новый Соответствие;
				Пока ВыборкаШтрихкоды.Следующий() Цикл
					СтруктураШтрихкодов = Новый Структура("Штрихкод,ШтрихкодУпаковки");
			...
			Пока лВыборка.Следующий() Цикл
				...
				Пока ВыборкаПоТоварам.Следующий() Цикл
					...
					Если Не Штрихкоды = Неопределено Тогда
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Штрихкоды, "Штрихкод,ШтрихкодУпаковки");
					КонецЕсли;
					// 30.05.19 Строганов Роман <
