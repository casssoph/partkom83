...
Функция РегистрыСведений_ЦеныНоменклатуры_Новое(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	Если НЕ Реквизиты.флОбновлятьЦены Тогда
		...
		КонецЕсли;
		Если НЕ ОбщегоНазначения.ПолучитьЗначениеРеквизита(Реквизиты.ТорговаяТочка, "ВсегдаОбновлятьЦены") Тогда
			ВсегдаОбновлятьЦены = Ложь;
		КонецЕсли;
	КонецЕсли;
