...
Процедура ВзаиморасчетыИмпорт(Знач вхСсылкаНаДокумент, Знач СтруктураРеквизитов, ТабВзаиморасчеты)
	...
	Если СтруктураРеквизитов.Дата > Дата(2018,2,1) Тогда  //Актуальный вариант 
	СтруктураИмпорта = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(вхСсылкаНаДокумент, 
...
Процедура ЗагрузитьЭлемент(ОбъектXDTO, вхОтправитель, Отказ, вхПараметры = Неопределено, НомерСообщения = 0) Экспорт
	...
	Если (лМетаданныеПланаОбмена = Метаданные.ПланыОбмена.ОбменПартКом83_ОкноПоставщика) тогда
			...
			Если ЗаказыПоставщикамСервер.ЭтоСкладНовойСхемы(ШапкаДокумента.Склад) тогда 
				...
				ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(ШапкаДокумента,ТаблицаТовары);			
...
Процедура ЗагрузитьПоступлениеТоваровИзОкнаПоставщика(СтруктураШапкиДокумента,ТаблицаТоваров)
	...
	ДокументПоступления	  = ДокументПоступления(СсылкаНаДокумент);
	...
	Попытка 
	ЗаказыПоставщикамКлиентСервер.РазместитьДокументПоступленияПоЗаказам(ДокументПоступления,СтруктураПараметровРазмещения);
	Исключение 
	ДокументПоступления.записать();
	КритическиеСобытияСервер.ЗарегистрироватьКритическоеСобытие(ДокументПоступления.Ссылка,Справочники.СобытияДляОтправкиЭлектронныхПисем.ОшибкаЗагрузкиИзОП,ОписаниеОшибки());	
	КонецПопытки;
...
Процедура ДополнитьНоменклатуруПоЗаказу(ТаблицаТорваров,ЗаказПоставщику)
	...
	ЗапросПоЗаказу.УстановитьПараметр("Товары",ТаблицаТорваров);
...
Функция РаспределитьПоступлениеПоЗаказам(ШапкаДокумента, ТаблицаПоступления, ТаблицаОшибок, Отказ)  Экспорт
	...
	//Если ЗаказыПоставщикамСервер.ЭтоСкладНовойСхемы(ШапкаДокумента.Склад) тогда 
	//	Документ = Документы.ПоступлениеТоваровУслуг.СоздатьДокумент();//ДокументПоступления(ШапкаДокумента.СсылкаНаДокумент);
	//	СтруктураПараметров = ПолучитьСтруктуруПараметровРазмещения(ШапкаДокумента);	
	//	ЗаполнитьЗначенияСвойств(Документ,ШапкаДокумента);//,"Дата,Комментарий,ТорговаяТочка,"
	//	Документ.Товары.Загрузить(ТаблицаПоступления);	
	//	ЗаказыПоставщикамКлиентСервер.РазместитьДокументПоступленияПоЗаказам(Документ,СтруктураПараметров);
	//	Отказ = Истина;//Второй раз не перезаписываем	
	//Иначе  
...
Функция РегистрыНакопления_ПартииТоваров(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено, вхФильтр = Неопределено)
	...
	КонецЕсли;
		...
		Иначе
				...
				|	ПоступлениеТоваровУслуг.НомерСтроки КАК НомерСтрокиВДокументе,
				|	ПоступлениеТоваровУслуг.Количество - ПоступлениеТоваровУслуг.КоличествоНеПринято КАК Количество,
				|	(ПоступлениеТоваровУслуг.Количество - ПоступлениеТоваровУслуг.КоличествоНеПринято) * ПоступлениеТоваровУслуг.имп_СуммаРуб / ПоступлениеТоваровУслуг.Количество КАК имп_СуммаРубли,
				|	(ПоступлениеТоваровУслуг.Количество - ПоступлениеТоваровУслуг.КоличествоНеПринято) * ПоступлениеТоваровУслуг.Сумма / ПоступлениеТоваровУслуг.Количество КАК СуммаРубли,
				|	(ПоступлениеТоваровУслуг.Количество - ПоступлениеТоваровУслуг.КоличествоНеПринято) * ПоступлениеТоваровУслуг.Сумма / ПоступлениеТоваровУслуг.Количество КАК СуммаДоллары,
				|	(ПоступлениеТоваровУслуг.Количество - ПоступлениеТоваровУслуг.КоличествоНеПринято) * ПоступлениеТоваровУслуг.Сумма / ПоступлениеТоваровУслуг.Количество КАК СуммаЕвро,
				|	ВЫБОР
...
Процедура СоздатьОбновитьСтрокиПрихода(ЭтотОбъект) Экспорт
	...
	Если ЭтотОбъект.ВидОперацииПоступления = Перечисления.ВидыПоступленияТоваров.МФП Тогда
		Возврат;
	КонецЕсли;
