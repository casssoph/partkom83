...
Функция РегистрыСведений_ЦеныНоменклатуры(вхСсылкаНаДокумент, вхОтказ, вхПараметры = Неопределено)
	...
	Если НЕ Реквизиты.СтатусДокумента = Справочники.СтатусыДокументов.ПоступлениеТоваровПринят Тогда
		Возврат ТабЦен;
	КонецЕсли; 
	...
	ВсегдаОбновлятьЦены = Ложь;
	Если НЕ ЗначениеЗаполнено(Реквизиты.ТорговаяТочка) Тогда
		ТорговаяТочка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Реквизиты.Контрагент, "ОсновнаяТорговаяТочка");
	КонецЕсли;
	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(Реквизиты.ТорговаяТочка, "ВсегдаОбновлятьЦены") Тогда
		ВсегдаОбновлятьЦены = Истина;
	КонецЕсли;
